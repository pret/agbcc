...
struct local_symbol *local_symbol_make(const char *name, segT section, valueT val, fragS *frag)
{
...
static symbolS *local_symbol_convert(struct local_symbol *locsym)
{
	symbolS *ret;
...
symbolS *colon( /* Just seen "x:" - rattle symbols & frags.  */
...
static int use_complex_relocs_for(symbolS * symp)
{
...
static void report_op_error(symbolS *symp, symbolS *left, operatorT op, symbolS *right)
{
	const char *file;
	unsigned int line;
...
static void resolve_local_symbol(const char *key ATTRIBUTE_UNUSED, void *value)
{
	if (value != NULL) {
...
static long dollar_label_instance(long label)
{
	long *i;
...
static long fb_label_instance(long label)
{
	long *i;
...
int S_CAN_BE_REDEFINED(const symbolS *s)
{
	if (LOCAL_SYMBOL_CHECK(s)) {
...
expressionS *symbol_get_value_expression(symbolS *s)
{
	if (LOCAL_SYMBOL_CHECK(s)) {
		s = local_symbol_convert((struct local_symbol *)s);
	}
...
OBJ_SYMFIELD_TYPE *symbol_get_obj(symbolS *s)
{
	if (LOCAL_SYMBOL_CHECK(s)) {
		s = local_symbol_convert((struct local_symbol *)s);
	}
...
TC_SYMFIELD_TYPE *symbol_get_tc(symbolS *s)
{
	if (LOCAL_SYMBOL_CHECK(s)) {
		s = local_symbol_convert((struct local_symbol *)s);
	}
...
char *symbol_relc_make_sym(symbolS * sym)
{
	char * terminal = NULL;
	const char * sname;
	char typetag;
...
char *symbol_relc_make_value(offsetT val)
{
...
char *symbol_relc_make_expr(expressionS * exp)
{
