...
void macro_init(int alternate, int mri, int strip_at,
...
void macro_set_alternate(int alternate)
{
    macro_alternate = alternate;
}
...
static size_t get_token(size_t idx, sb *in, sb *name)
{
    if (idx < in->len
...
static size_t getstring(size_t idx, sb *in, sb *acc)
{
    while (idx < in->len
           && (in->ptr[idx] == '"'
...
static size_t get_any_string(size_t idx, sb *in, sb *out)
{
    sb_reset(out);
...
static void del_formal(formal_entry *formal)
{
...
static size_t do_formals(macro_entry *macro, size_t idx, sb *in)
{
...
static void free_macro(macro_entry *macro)
{
    formal_entry *formal;
...
const char *define_macro(size_t idx, sb *in, sb *label,
                         size_t (*get_line)(sb *),
...
static size_t get_apost_token(size_t idx, sb *in, sb *name, int kind)
{
    idx = get_token(idx, in, name);
    if (idx < in->len
        && in->ptr[idx] == kind
...
static size_t sub_actual(size_t start, sb *in, sb *t, struct hash_control *formal_hash,
...
static const char *macro_expand_body(sb *in, sb *out, formal_entry *formals,
...
int check_macro(const char *line, sb *expand,
...
const char *expand_irp(int irpc, size_t idx, sb *in, sb *out, size_t (*get_line)(sb *))
{
    sb sub;
    formal_entry f;
