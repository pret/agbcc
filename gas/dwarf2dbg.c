...
static struct line_subseg *get_line_subseg(segT seg, subsegT subseg, bfd_boolean create_p)
{
...
static unsigned int get_filenum(const char *filename, unsigned int num)
{
...
void dwarf2_directive_file(int dummy ATTRIBUTE_UNUSED)
{
...
static void scale_addr_delta(addressT *addr_delta)
{
...
static void emit_inc_line_addr(int line_delta, addressT addr_delta, char *p, int len)
{
    unsigned int tmp, opcode;
    int need_copy = 0;
    char *end = p + len;
...
static void emit_fixed_inc_line_addr(int line_delta, addressT addr_delta, fragS *frag,
                                     char *p, int len)
{
    expressionS *pexp;
    char *end = p + len;
