...
static unsigned int encoding_size(unsigned char encoding)
{
...
static void emit_expr_encoded(expressionS *exp, int encoding, bfd_boolean emit_encoding)
{
...
static void dwcfi_hash_insert(const char *name, struct dwcfi_seg_list *item)
{
...
void cfi_add_label(const char *name)
{
...
void cfi_add_CFA_val_offset(unsigned regno, offsetT offset)
{
    unsigned int abs_data_align;
...
static segT get_cfi_seg(segT cseg, const char *base, flagword flags, int align)
{
...
static void output_cie(struct cie_entry *cie, bfd_boolean eh_frame, int align)
{
...
static struct cie_entry *select_cie_for_fde(struct fde_entry *fde, bfd_boolean eh_frame,
...
static void cfi_change_reg_numbers(struct cfi_insn_data *insn, segT ccseg)
{
...
#if SUPPORT_COMPACT_EH
static void cfi_emit_eh_header(symbolS *sym, bfd_vma addend)
{
    expressionS exp;
...
static void output_eh_header(struct fde_entry *fde)
{
    char *p;
    bfd_vma addend;
...
static void dot_cfi_dummy(int ignored ATTRIBUTE_UNUSED)
{
