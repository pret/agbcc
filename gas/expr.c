...
int expr_symbol_where(symbolS *sym, const char **pfile, unsigned int *pline)
{
...
static void mri_char_constant(expressionS *expressionP)
{
	int i;
...
void current_location(expressionS *expressionp)
{
...
void add_to_result(expressionS *resultP, offsetT amount, int rhs_highbit)
{
...
void subtract_from_result(expressionS *resultP, offsetT amount, int rhs_highbit)
{
	valueT ures = resultP->X_add_number;
	valueT uamount = amount;
...
segT expr(int rankarg,               /* Larger # is higher rank.  */
...
char restore_line_pointer(char c)
{
	*input_line_pointer = c;
	if (c == '"') {
		c = *++input_line_pointer;
