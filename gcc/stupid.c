...
#define REG_WHERE_BORN(N) (reg_where_born_exact[(N)] - reg_where_born_clobber[(N)])
...
static int stupid_find_reg(int, enum reg_class, enum machine_mode, int, int, int);
...
static void find_clobbered_regs(rtx reg, rtx setter)
{
    int regno, nregs;
...
void stupid_life_analysis(rtx f, int nregs, FILE *file)
{
    ...
    after_insn_hard_regs = (HARD_REG_SET *)xmalloc(max_suid * sizeof(HARD_REG_SET));
    ...
    for (insn = last; insn; insn = PREV_INSN(insn))
    {
       ...
       based on the pattern of this insn.  */
        ...
        if (GET_CODE(insn) == NOTE && NOTE_LINE_NUMBER(insn) == NOTE_INSN_SETJMP)
       ...
       of the insns before the call.  */
        ...
        if (GET_CODE(insn) == CALL_INSN)
        {
            ...
            {
                IOR_COMPL_HARD_REG_SET(after_insn_hard_regs[last_call_suid], fixed_reg_set);
                for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
                    if (!fixed_regs[i])
                        regs_live[i] = 0;
            }
            ...
            {
                IOR_HARD_REG_SET(after_insn_hard_regs[last_call_suid], call_used_reg_set);
                for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
                    if (call_used_regs[i])
                        regs_live[i] = 0;
            }
    ...
    qsort(&reg_order[LAST_VIRTUAL_REGISTER + 1], max_regno - LAST_VIRTUAL_REGISTER - 1, sizeof(int),
        stupid_reg_compare);
    ...
    for (i = LAST_VIRTUAL_REGISTER + 1; i < max_regno; i++)
    {
       ...
       Also ignore registers we didn't see during the scan.  */
        if (regno_reg_rtx[r] == 0 || regs_crosses_setjmp[r]
            ...
            || (REG_N_CALLS_CROSSED(r) > 0 && current_function_has_nonlocal_label))
            continue;
    ...
    for (i = LAST_VIRTUAL_REGISTER + 1; i < max_regno; i++)
    {
        ...
        if (INSN_SUID(chain->insn) == reg_where_born_exact[i] && reg_where_born_clobber[i])
            SET_REGNO_REG_SET(chain->live_before, i);
    }
    ...
    int born_insn, int dead_insn, int changes_size ATTRIBUTE_UNUSED)
{
    ...
    COPY_HARD_REG_SET(used, call_preserved ? call_used_reg_set : fixed_reg_set);
...
static void stupid_mark_refs(rtx x, struct insn_chain *chain)
{
                          ...
                          && (REGNO(SUBREG_REG(SET_DEST(x))) >= FIRST_PSEUDO_REGISTER))))
        {
            ...
            if (regno < FIRST_PSEUDO_REGISTER)
            {
                ...
                while (--j >= 0)
                {
                    ...
                    if (code == CLOBBER && INSN_SUID(insn) > 0)
                        SET_HARD_REG_BIT(after_insn_hard_regs[INSN_SUID(insn) - 1], regno + j);
                }
            }
            ...
            else
            {
               ...
               an insn setting CC) will operate correctly.  */
                if (GET_CODE(SET_DEST(x)) == REG && REGNO_FIRST_UID(regno) == INSN_UID(insn)
                    && REGNO_LAST_UID(regno) == INSN_UID(insn)
                    && (code == CLOBBER || !reg_mentioned_p(SET_DEST(x), SET_SRC(x))))
                    REG_NOTES(insn) = gen_rtx_EXPR_LIST(REG_UNUSED, SET_DEST(x), REG_NOTES(insn));
            }
        }
    ...
    else if (code == SUBREG && GET_CODE(SUBREG_REG(x)) == REG
        && REGNO(SUBREG_REG(x)) >= FIRST_PSEUDO_REGISTER
        && (GET_MODE_SIZE(GET_MODE(x)) != GET_MODE_SIZE(GET_MODE(SUBREG_REG(x))))
        && (INTEGRAL_MODE_P(GET_MODE(x)) || INTEGRAL_MODE_P(GET_MODE(SUBREG_REG(x)))))
