...
restart:
    ...
    case CONST_DOUBLE:
    {
        int32_t l1 = CONST_DOUBLE_LOW(x);
        int32_t h1 = CONST_DOUBLE_HIGH(x);
        int32_t l2 = c;
        int32_t h2 = c < 0 ? ~0 : 0;
        int32_t lv, hv;
    ...
    case MEM:
       ...
       valid, don't return it because we have no way to validize it.  */
        if (GET_CODE(XEXP(x, 0)) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P(XEXP(x, 0)))
        {
            ...
            tem = force_const_mem(GET_MODE(x), plus_constant(get_pool_constant(XEXP(x, 0)), c));
            pop_obstacks();
    ...
    case PLUS:
       ...
       all_constant gets lost.  */
        ...
        {
            x = gen_rtx_PLUS(mode, plus_constant(XEXP(x, 0), c), XEXP(x, 1));
            c = 0;
        }
...
rtx eliminate_constant_term(rtx x, rtx *constptr)
{
    ...
    if (GET_CODE(XEXP(x, 1)) == CONST_INT
        && 0 != (tem = simplify_binary_operation(PLUS, GET_MODE(x), *constptr, XEXP(x, 1)))
    ...
    if ((x1 != XEXP(x, 1) || x0 != XEXP(x, 0))
        && 0 != (tem = simplify_binary_operation(PLUS, GET_MODE(x), *constptr, tem))
        && GET_CODE(tem) == CONST_INT)
    {
        *constptr = tem;
...
rtx expr_size(tree exp)
{
    ...
    return expand_expr(size, NULL_RTX, TYPE_MODE(sizetype), EXPAND_MEMORY_USE_BAD);
}
...
rtx memory_address(enum machine_mode mode, rtx x)
{
    ...
    else if (GET_CODE(x) == QUEUED && GET_CODE(QUEUED_VAR(x)) == REG)
        ;
    ...
    else
    {
       ...
       in registers often become common subexpressions.  */
        if (GET_CODE(x) == PLUS)
        {
            ...
            if (constant_term == const0_rtx || !memory_address_p(mode, y))
    ...
    win2:
        x = oldx;
    win:
        if (flag_force_addr && !cse_not_expected
            && GET_CODE(x) != REG
...
rtx force_reg(enum machine_mode mode, rtx x)
{
    ...
    if (CONSTANT_P(x) && (set = single_set(insn)) != 0 && SET_DEST(set) == temp)
    {
...
rtx round_push(rtx size)
{
    ...
    else
    {
       ...
       TRUNC_DIV_EXPR.  */
        size = expand_binop(
            Pmode, add_optab, size, GEN_INT(align - 1), NULL_RTX, 1, OPTAB_LIB_WIDEN);
        size = expand_divmod(0, TRUNC_DIV_EXPR, Pmode, size, GEN_INT(align), NULL_RTX, 1);
...
void optimize_save_area_alloca(rtx insns)
{
    rtx insn;
        ...
        for (note = REG_NOTES(insn); note; note = XEXP(note, 1))
        {
            ...
            if (!current_function_calls_setjmp)
            {
               ...
               is the HPPA and it does not define SETJMP_VIA_SAVE_AREA.  */
                if (GET_CODE(pat) != SET || SET_DEST(pat) != stack_pointer_rtx
...
rtx allocate_dynamic_stack_space(rtx size, rtx target, int known_align)
{
    ...
    || !defined(PREFERRED_STACK_BOUNDARY)
#define MUST_ALIGN 1
#else
#define MUST_ALIGN (PREFERRED_STACK_BOUNDARY < BIGGEST_ALIGNMENT)
    ...
    {
        if (GET_CODE(size) == CONST_INT)
            size = GEN_INT(INTVAL(size) + (BIGGEST_ALIGNMENT / BITS_PER_UNIT - 1));
        else
            size = expand_binop(Pmode, add_optab, size,
                GEN_INT(BIGGEST_ALIGNMENT / BITS_PER_UNIT - 1), NULL_RTX, 1, OPTAB_LIB_WIDEN);
    }
    ...
    {
        rtx dynamic_offset = expand_binop(Pmode, sub_optab, virtual_stack_dynamic_rtx,
...
#if !defined(PREFERRED_STACK_BOUNDARY) || !defined(MUST_ALIGN)                                     \
    || (PREFERRED_STACK_BOUNDARY != BIGGEST_ALIGNMENT)
            ...
            else
            {
                ...
                setjmpless_size
                    = expand_divmod(0, TRUNC_DIV_EXPR, Pmode, size, GEN_INT(align), NULL_RTX, 1);
                setjmpless_size = expand_mult(Pmode, setjmpless_size, GEN_INT(align), NULL_RTX, 1);
            }
        ...
        size = expand_binop(Pmode, add_optab, size, dynamic_offset, NULL_RTX, 1, OPTAB_LIB_WIDEN);
    }
#endif /* SETJMP_VIA_SAVE_AREA */
    ...
    if (HAVE_allocate_stack)
    {
        ...
        if (insn_operand_predicate[(int)CODE_FOR_allocate_stack][0]
            && !((*insn_operand_predicate[(int)CODE_FOR_allocate_stack][0])(target, Pmode)))
            target = copy_to_mode_reg(Pmode, target);
        size = convert_modes(mode, ptr_mode, size, 1);
        if (insn_operand_predicate[(int)CODE_FOR_allocate_stack][1]
            && !((*insn_operand_predicate[(int)CODE_FOR_allocate_stack][1])(size, mode)))
    ...
    if (MUST_ALIGN)
    {
       ...
       TRUNC_DIV_EXPR.  */
        ...
        target = expand_divmod(0, TRUNC_DIV_EXPR, Pmode, target,
            GEN_INT(BIGGEST_ALIGNMENT / BITS_PER_UNIT), NULL_RTX, 1);
        target
            = expand_mult(Pmode, target, GEN_INT(BIGGEST_ALIGNMENT / BITS_PER_UNIT), NULL_RTX, 1);
    }
...
void probe_stack_range(int32_t first, rtx size)
{
    /* First see if we have an insn to check the stack.  Use it if so.  */
#ifdef HAVE_check_stack
    if (HAVE_check_stack)
    {
        rtx last_addr = force_operand(
            gen_rtx_STACK_GROW_OP(Pmode, stack_pointer_rtx, plus_constant(size, first)), NULL_RTX);

        if (insn_operand_predicate[(int)CODE_FOR_check_stack][0]
            && !((*insn_operand_predicate[(int)CODE_FOR_check_stack][0])(last_address, Pmode)))
    ...
    if (GET_CODE(size) == CONST_INT && INTVAL(size) < 10 * STACK_CHECK_PROBE_INTERVAL)
    {
        int32_t offset;
       ...
       at LAST.  */
        for (offset = first + STACK_CHECK_PROBE_INTERVAL; offset < INTVAL(size);
             offset = offset + STACK_CHECK_PROBE_INTERVAL)
            emit_stack_probe(
                gen_rtx_fmt_ee(STACK_GROW_OP, Pmode, stack_pointer_rtx, GEN_INT(offset)));

        emit_stack_probe(
            gen_rtx_fmt_ee(STACK_GROW_OP, Pmode, stack_pointer_rtx, plus_constant(size, first)));
    }
    ...
    {
        rtx test_addr = force_operand(gen_rtx_fmt_ee(STACK_GROW_OP, Pmode, stack_pointer_rtx,
        ...
        if (GET_CODE(test_addr) != REG || REGNO(test_addr) < FIRST_PSEUDO_REGISTER)
...
#define CMP_OPCODE GTU
        temp = expand_binop(Pmode, sub_optab, test_addr, incr, test_addr, 1, OPTAB_WIDEN);
#else
#define CMP_OPCODE LTU
        temp = expand_binop(Pmode, add_optab, test_addr, incr, test_addr, 1, OPTAB_WIDEN);
#endif
...
rtx hard_function_value(tree valtype, tree func)
{
    ...
    if (GET_CODE(val) == REG && GET_MODE(val) == BLKmode)
    {
