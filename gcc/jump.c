    ...
    if (!reload_completed && after_regscan)
        for (insn = f; insn; insn = next)
        {
            ...
            if (set && GET_CODE(SET_DEST(set)) == REG
                ...
                && !side_effects_p(SET_SRC(set)) && !find_reg_note(insn, REG_RETVAL, 0))
                delete_insn(insn);
        }
	    ...
	    if (GET_CODE (insn) == INSN || GET_CODE (insn) == JUMP_INSN)
	      peephole (insn);
            ...
            if (after_regscan && GET_CODE(insn) == NOTE
                ...
                && (temp1 = next_nonnote_insn(insn)) != 0 && simplejump_p(temp1))
            {
                temp = PREV_INSN(insn);
            ...
            if (GET_CODE(PATTERN(insn)) == ADDR_VEC || GET_CODE(PATTERN(insn)) == ADDR_DIFF_VEC)
            {
                int i;
                ...
                for (i = 0; i < len; i++)
                    if (XEXP(XVECEXP(pat, diff_vec_p, i), 0)
                        ...
                        break;
                if (i == len && dispatch != 0 && GET_CODE(dispatch) == JUMP_INSN
                    && JUMP_LABEL(dispatch) != 0
                    /* Don't mess with a casesi insn.  */
                    && !(GET_CODE(PATTERN(dispatch)) == SET
                           && (GET_CODE(SET_SRC(PATTERN(dispatch))) == IF_THEN_ELSE))
                    && next_real_insn(JUMP_LABEL(dispatch)) == insn)
                {
                    redirect_tablejump(dispatch, XEXP(XVECEXP(pat, diff_vec_p, 0), 0));
                    changed = 1;
                }
            }
            ...
            if (JUMP_LABEL(insn)
                && (next_active_insn(JUMP_LABEL(insn)) == 0
                       || GET_CODE(PATTERN(next_active_insn(JUMP_LABEL(insn)))) == RETURN))
            ...
            if (this_is_simplejump && (temp = prev_nonnote_insn(insn)) != 0
                && GET_CODE(temp) == INSN && GET_CODE(PATTERN(temp)) == USE
                && (temp1 = prev_nonnote_insn(JUMP_LABEL(insn))) != 0
                && (GET_CODE(temp1) == BARRIER
                       || (GET_CODE(temp1) == INSN && rtx_equal_p(PATTERN(temp), PATTERN(temp1))))
                /* Don't do this optimization if we have a loop containing only
                   ...
                   exit.  */
                && !((temp2 = prev_nonnote_insn(temp)) != 0 && temp2 == JUMP_LABEL(insn)
                       && LABEL_NUSES(temp2) == 1))
            {
            ...
            if (reload_completed && optimize && this_is_condjump && !this_is_simplejump
                && !this_is_condjump_in_parallel && MAX_CONDITIONAL_EXECUTE > 0
            ...
            if (this_is_simplejump && (temp3 = prev_active_insn(insn)) != 0
                && GET_CODE(temp3) == INSN && (temp4 = single_set(temp3)) != 0
                && GET_CODE(temp1 = SET_DEST(temp4)) == REG
                && (!SMALL_REGISTER_CLASSES || REGNO(temp1) >= FIRST_PSEUDO_REGISTER)
                && (temp2 = next_active_insn(insn)) != 0 && GET_CODE(temp2) == INSN
                && (temp4 = single_set(temp2)) != 0 && rtx_equal_p(SET_DEST(temp4), temp1)
                && !side_effects_p(SET_SRC(temp4)) && !may_trap_p(SET_SRC(temp4))
                && (REG_NOTES(temp2) == 0
                       || ((REG_NOTE_KIND(REG_NOTES(temp2)) == REG_EQUAL
                               || REG_NOTE_KIND(REG_NOTES(temp2)) == REG_EQUIV)
                              && XEXP(REG_NOTES(temp2), 1) == 0
                              && rtx_equal_p(XEXP(REG_NOTES(temp2), 0), SET_SRC(temp4))))
                && (temp = prev_active_insn(temp3)) != 0 && condjump_p(temp)
                && !simplejump_p(temp)
                ...
                && (reallabelprev == temp2
                       || ((temp5 = next_active_insn(temp2)) != 0 && simplejump_p(temp5)
                              && JUMP_LABEL(temp5) == JUMP_LABEL(insn))))
            {
                ...
                for (p = temp; nuses && p; p = prev_nonnote_insn(p))
                {
                    ...
                    {
                        if (condjump_p(p) && !simplejump_p(p) && JUMP_LABEL(p) == target)
                        {
                            nuses--;
                            if (nuses == 0)
                                break;
                ...
                if (q && GET_RTX_CLASS(GET_CODE(q)) == 'i' && sets_cc0_p(PATTERN(q)))
                    p = q;
#endif
                ...
                /* If we found all the uses and there was no data conflict, we
                   ...
                   from somewhere.  */
                if (nuses == 0 && p && no_labels_between_p(p, insn)
                    ...
                    && !regs_set_between_p(PATTERN(temp), PREV_INSN(temp2), NEXT_INSN(temp2)))
                {
            ...
            if (this_is_simplejump && (temp2 = next_active_insn(insn)) != 0
                && GET_CODE(temp2) == INSN && (temp4 = single_set(temp2)) != 0
                && GET_CODE(temp1 = SET_DEST(temp4)) == REG
                && (!SMALL_REGISTER_CLASSES || REGNO(temp1) >= FIRST_PSEUDO_REGISTER)
                && (temp3 = prev_active_insn(insn)) != 0 && GET_CODE(temp3) == INSN
                && (temp4 = single_set(temp3)) != 0 && rtx_equal_p(SET_DEST(temp4), temp1)
                && !side_effects_p(SET_SRC(temp4)) && !may_trap_p(SET_SRC(temp4))
                && (REG_NOTES(temp3) == 0
                       || ((REG_NOTE_KIND(REG_NOTES(temp3)) == REG_EQUAL
                               || REG_NOTE_KIND(REG_NOTES(temp3)) == REG_EQUIV)
                              && XEXP(REG_NOTES(temp3), 1) == 0
                              && rtx_equal_p(XEXP(REG_NOTES(temp3), 0), SET_SRC(temp4))))
                && (temp = prev_active_insn(temp3)) != 0 && condjump_p(temp)
                && !simplejump_p(temp)
                /* TEMP must skip over the "x = a;" insn */
                && prev_real_insn(JUMP_LABEL(temp)) == insn && no_labels_between_p(temp, insn))
            {
                ...
                if (insert_after && no_labels_between_p(insert_after, temp)
                    && !reg_referenced_between_p(temp1, insert_after, temp3)
                    && !reg_referenced_between_p(temp1, temp3, NEXT_INSN(temp2))
                    && !reg_set_between_p(temp1, insert_after, temp)
                    && !modified_between_p(SET_SRC(temp4), insert_after, temp)
                    /* Verify that registers used by the jump are not clobbered
                       by the instruction being moved.  */
                    && !regs_set_between_p(PATTERN(temp), PREV_INSN(temp3), NEXT_INSN(temp3))
                    && invert_jump(temp, JUMP_LABEL(insn)))
                {
                    emit_insn_after_with_line_notes(PATTERN(temp3), insert_after, temp3);
                    delete_insn(temp3);
                    delete_insn(insn);
                    /* Set NEXT to an insn that we know won't go away.  */
                    next = temp2;
            ...
            if (!reload_completed && this_is_condjump && !this_is_simplejump && BRANCH_COST >= 3
                && (temp = next_nonnote_insn(insn)) != 0 && GET_CODE(temp) == INSN
                && REG_NOTES(temp) == 0
                && (reallabelprev == temp
                       || ((temp2 = next_active_insn(temp)) != 0 && simplejump_p(temp2)
                ...
                && (!SMALL_REGISTER_CLASSES || REGNO(temp2) >= FIRST_PSEUDO_REGISTER)
                && GET_CODE(SET_SRC(temp1)) != REG && GET_CODE(SET_SRC(temp1)) != SUBREG
                && GET_CODE(SET_SRC(temp1)) != CONST_INT && !side_effects_p(SET_SRC(temp1))
                && !may_trap_p(SET_SRC(temp1)) && rtx_cost(SET_SRC(temp1), SET) < 10)
            {
                    ...
                    && validate_change(temp, &SET_DEST(temp1), new, 0))
                {
                    ...
                    emit_insn_after_with_line_notes(PATTERN(temp), PREV_INSN(temp3), temp);
                    delete_insn(temp);
                    reallabelprev = prev_active_insn(JUMP_LABEL(insn));
            ...
            if (!reload_completed && this_is_condjump && !this_is_simplejump && BRANCH_COST >= 4
                && (temp = next_nonnote_insn(insn)) != 0 && GET_CODE(temp) == INSN
                && REG_NOTES(temp) == 0 && (temp3 = next_nonnote_insn(temp)) != 0
                && GET_CODE(temp3) == INSN && REG_NOTES(temp3) == 0
                && (reallabelprev == temp3
                       || ((temp2 = next_active_insn(temp3)) != 0 && simplejump_p(temp2)
                              && JUMP_LABEL(temp2) == JUMP_LABEL(insn)))
                && (temp1 = single_set(temp)) != 0
                && (temp2 = SET_DEST(temp1), GET_CODE(temp2) == REG)
                ...
                /* END CYGNUS LOCAL -- meissner/nortel */
                && (!SMALL_REGISTER_CLASSES || REGNO(temp2) >= FIRST_PSEUDO_REGISTER)
                && !side_effects_p(SET_SRC(temp1)) && !may_trap_p(SET_SRC(temp1))
                && rtx_cost(SET_SRC(temp1), SET) < 10 && (temp4 = single_set(temp3)) != 0
                && rtx_equal_p(SET_DEST(temp4), temp2) && !side_effects_p(SET_SRC(temp4))
                && !may_trap_p(SET_SRC(temp4)) && rtx_cost(SET_SRC(temp4), SET) < 10)
            {
                rtx new = gen_reg_rtx(GET_MODE(temp2));
                    ...
                    && validate_change(temp, &SET_DEST(temp1), new, 0))
                {
                    ...
                    if (temp5 != insn)
                        temp6 = temp5;
                    next = emit_insn_after(gen_move_insn(temp2, new), insn);
                    emit_insn_after_with_line_notes(PATTERN(temp), PREV_INSN(temp6), temp);
                    emit_insn_after_with_line_notes(
                        replace_rtx(PATTERN(temp3), temp2, new), PREV_INSN(temp6), temp3);
                    delete_insn(temp);
                    delete_insn(temp3);
                    reallabelprev = prev_active_insn(JUMP_LABEL(insn));
            ...
            if (!reload_completed && after_regscan && this_is_condjump && !this_is_simplejump
                && BRANCH_COST >= 4 && (temp = next_nonnote_insn(insn)) != 0
                && GET_CODE(temp) == INSN && REG_NOTES(temp) == 0
                && (temp3 = next_nonnote_insn(temp)) != 0 && GET_CODE(temp3) == INSN
                && REG_NOTES(temp3) == 0
                && (reallabelprev == temp3
                       || ((temp2 = next_active_insn(temp3)) != 0 && simplejump_p(temp2)
                              && JUMP_LABEL(temp2) == JUMP_LABEL(insn)))
                ...
                && (temp5 = SET_DEST(temp1),
                       (GET_CODE(temp5) == REG
                           || (GET_CODE(temp5) == SUBREG
                                  && (temp5 = SUBREG_REG(temp5), GET_CODE(temp5) == REG))))
                ...
                && !may_trap_p(SET_SRC(temp4)) && rtx_cost(SET_SRC(temp4), SET) < 10)
            {
                    ...
                    && validate_change(temp3, &SET_DEST(temp4), new, 0))
                {
                    ...
                    next = emit_insn_after(gen_move_insn(temp2, new), insn);
                    emit_insn_after_with_line_notes(PATTERN(temp), PREV_INSN(temp6), temp);
                    emit_insn_after_with_line_notes(PATTERN(temp3), PREV_INSN(temp6), temp3);
                    delete_insn(temp);
                    delete_insn(temp3);
                    reallabelprev = prev_active_insn(JUMP_LABEL(insn));
            ...
            if (/* We can't do this after reload has completed.  */
                !reload_completed && this_is_condjump
                && !this_is_simplejump
                ...
                && (reallabelprev == temp
                       || ((temp4 = next_active_insn(temp)) != 0 && simplejump_p(temp4)
            ...
            /* Even if branches are cheap, the store_flag optimization
               ...
               expressed directly.  */
                ...
                && !((temp6 = prev_nonnote_insn(insn)) != 0 && GET_CODE(temp6) == INSN
            ...
            )
            {
                ...
                /* First try a conditional move.  */
                {
                    ...
                    start_sequence();
                    target = emit_conditional_move(var, code, cond0, cond1, VOIDmode, aval, bval,
                        GET_MODE(var), (code == LTU || code == GEU || code == LEU || code == GTU));
                ...
                if (GET_MODE_CLASS(GET_MODE(temp1)) == MODE_INT
                    && ((GET_CODE(temp3) == CONST_INT)
                           /* Make the latter case look like
                              x = x; if (...) x = 0;  */
                           || (temp3 = temp1,
                                  ((BRANCH_COST >= 2 && temp2 == const0_rtx) || BRANCH_COST >= 3)))
                    ...
                    && (((BRANCH_COST >= 2 || STORE_FLAG_VALUE == -1
                             || (STORE_FLAG_VALUE == 1
                           ...
                           || ((BRANCH_COST >= 2 || STORE_FLAG_VALUE == -1
                                   || (STORE_FLAG_VALUE == 1 && GET_CODE(temp2) == CONST_INT
                                          && exact_log2(INTVAL(temp2)) >= 0))
                                  && temp3 == const0_rtx
                                  && (reversep = can_reverse_comparison_p(temp4, insn)))
                           || (BRANCH_COST >= 2 && GET_CODE(temp2) == CONST_INT
                                  && GET_CODE(temp3) == CONST_INT
                           ...
                           || BRANCH_COST >= 3))
                {
                    ...
                    start_sequence();
                    target = emit_store_flag(gen_reg_rtx(GET_MODE(var)), code, XEXP(temp4, 0),
                        XEXP(temp4, 1), VOIDmode,
                        (code == LTU || code == LEU || code == GEU || code == GTU), normalizep);
                    if (target)
                    {
                        rtx seq;
                        rtx before = insn;
                        ...
                        if (cval != const0_rtx && uval != const0_rtx)
                        {
                            ...
                            if (GET_CODE(cval) == CONST_INT && GET_CODE(uval) == CONST_INT
                                && (INTVAL(cval) & INTVAL(uval)) == INTVAL(cval))
                                tem2 = cval;
                            else
                            {
                                tem2 = expand_unop(
                                    GET_MODE(var), one_cmpl_optab, target, NULL_RTX, 0);
                                tem2 = expand_and(cval, tem2, (GET_CODE(tem2) == REG ? tem2 : 0));
                            }
                            ...
                            target = expand_binop(
                                GET_MODE(var), ior_optab, tem1, tem2, target, 1, OPTAB_WIDEN);
                        }
                        else if (normalizep != 1)
                        {
                            ...
                            target = expand_and(uval, target,
                                (GET_CODE(target) == REG && !preserve_subexpressions_p()
                                        ? target
                                        : NULL_RTX));
                        }
            ...
            if ((BRANCH_COST >= 2
                ...
                && !reload_completed && this_is_condjump && !this_is_simplejump
                && (temp = next_nonnote_insn(insn)) != 0 && (temp1 = single_set(temp)) != 0
                && (temp2 = SET_DEST(temp1), GET_MODE_CLASS(GET_MODE(temp2)) == MODE_INT)
                && GET_CODE(SET_SRC(temp1)) == PLUS
                && (XEXP(SET_SRC(temp1), 1) == const1_rtx || XEXP(SET_SRC(temp1), 1) == constm1_rtx)
                && rtx_equal_p(temp2, XEXP(SET_SRC(temp1), 0)) && !side_effects_p(temp2)
                && !may_trap_p(temp2)
                /* INSN must either branch to the insn after TEMP or the insn
                   after TEMP must branch to the same place as INSN.  */
                && (reallabelprev == temp
                       || ((temp3 = next_active_insn(temp)) != 0 && simplejump_p(temp3)
                              && JUMP_LABEL(temp3) == JUMP_LABEL(insn)))
                && (temp3 = get_condition(insn, &temp4)) != 0
                /* We must be comparing objects whose modes imply the size.
                   We could handle BLKmode if (1) emit_store_flag could
                   and (2) we could find the size reliably.  */
                && GET_MODE(XEXP(temp3, 0)) != BLKmode && can_reverse_comparison_p(temp3, insn))
            {
                ...
                if ((temp5 = prev_active_insn(insn)) != 0 && no_labels_between_p(temp5, insn)
                    && GET_CODE(temp5) == INSN && (temp6 = single_set(temp5)) != 0
                    && rtx_equal_p(temp2, SET_DEST(temp6))
                    && (CONSTANT_P(SET_SRC(temp6)) || GET_CODE(SET_SRC(temp6)) == REG
                ...
                if (CONSTANT_P(init) || !reg_set_between_p(init, PREV_INSN(temp4), insn))
                    target = emit_store_flag(gen_reg_rtx(GET_MODE(temp2)), code, XEXP(temp3, 0),
                        XEXP(temp3, 1), VOIDmode,
                        (code == LTU || code == LEU || code == GTU || code == GEU), 1);
                ...
                if (target)
                    target = expand_binop(GET_MODE(temp2),
                        (XEXP(SET_SRC(temp1), 1) == const1_rtx ? add_optab : sub_optab), temp2,
                        target, temp2, 0, OPTAB_WIDEN);
            ...
            /* ?? This optimization is only safe before cse is run if HAVE_cc0
               ...
               meantime.  */
                ...
                && (temp = prev_active_insn(insn)) && no_labels_between_p(temp, insn)
                && GET_CODE(temp) == INSN && GET_CODE(PATTERN(temp)) == SET
                && GET_CODE(SET_DEST(PATTERN(temp))) == REG && CONSTANT_P(SET_SRC(PATTERN(temp)))
                ...
                && condjump_p(temp1))
            {
                ...
                if (GET_CODE(choice) == IF_THEN_ELSE)
                {
                    ...
                    rtx cond = simplify_relational_operation(
                        code, GET_MODE(SET_DEST(PATTERN(temp))), val, const0_rtx);
                    rtx ultimate;
            ...
            /* Detect a conditional jump going to the same place
               as an immediately following unconditional jump.  */
                ...
                && (next_active_insn(JUMP_LABEL(insn)) == next_active_insn(JUMP_LABEL(temp))))
            {
                rtx tem = temp;
            ...
            else if ((this_is_condjump || this_is_condjump_in_parallel) && !this_is_simplejump
                && reallabelprev != 0 && GET_CODE(reallabelprev) == JUMP_INSN
                && prev_active_insn(reallabelprev) == insn
                && no_labels_between_p(insn, reallabelprev) && simplejump_p(reallabelprev))
            {
            ...
            else
            {
                ...
                if (nlabel != JUMP_LABEL(insn) && redirect_jump(insn, nlabel))
                {
                    changed = 1;
                    next = insn;
                }
                   ...
                   label2:  */
                {
                    ...
                    if (!first
                        ...
                        && condjump_p(insn) && label1 != 0 && JUMP_LABEL(insn) == label1
                        && LABEL_NUSES(label1) == 1 && GET_CODE(range1end) == JUMP_INSN
                        && simplejump_p(range1end))
                    {
                        rtx label2 = next_label(label1);
                        rtx range2end = label2 ? prev_active_insn(label2) : 0;
                        if (range1end != range2end && JUMP_LABEL(range1end) == label2
                            ...
                            && invert_jump(insn, label1))
                        {
                            ...
                            while (PREV_INSN(range1beg) && GET_CODE(PREV_INSN(range1beg)) == NOTE)
                                range1beg = PREV_INSN(range1beg);

                            while (PREV_INSN(range2beg) && GET_CODE(PREV_INSN(range2beg)) == NOTE)
                            ...
                            for (; range2after != label2; range2after = rangenext)
                            {
                                ...
                                {
                                    if (NOTE_LINE_NUMBER(range2after) == NOTE_INSN_LOOP_END)
                                    {
                                        NEXT_INSN(PREV_INSN(range2after)) = rangenext;
                                        PREV_INSN(rangenext) = PREV_INSN(range2after);
                                        PREV_INSN(range2after) = PREV_INSN(range1beg);
                                        NEXT_INSN(range2after) = range1beg;
                                        NEXT_INSN(PREV_INSN(range1beg)) = range2after;
                                        PREV_INSN(range1beg) = range2after;
                                    }
                                    else if (NOTE_LINE_NUMBER(range2after) == NOTE_INSN_LOOP_BEG)
                                        break;
                                }
                            }
                            changed = 1;
                ...
                if (cross_jump && condjump_p(insn))
                {
                    ...
                    if (x != 0)
                        find_cross_jump(insn, x, 2, &newjpos, &newlpos);
                    ...
                    if (newjpos != 0)
                    {
                        ...
                        SET_SRC(PATTERN(insn)) = gen_rtx_LABEL_REF(VOIDmode, JUMP_LABEL(insn));
                        INSN_CODE(insn) = -1;
                        ...
                        {
                            jump_chain[INSN_UID(insn)] = jump_chain[INSN_UID(JUMP_LABEL(insn))];
                ...
                if (cross_jump && simplejump_p(insn))
                {
                    ...
                    /* TARGET is nonzero if it is ok to cross jump
                       to code before TARGET.  If so, see if matches.  */
                    find_cross_jump(insn, JUMP_LABEL(insn), 1, &newjpos, &newlpos);
                ...
                if (cross_jump && GET_CODE(PATTERN(insn)) == RETURN)
                {
                    ...
                    /* If cannot cross jump to code before the label,
                       ...
                       the same label.  */
                    /* Try each other jump to this label.  */
                    for (target = jump_chain[0]; target != 0 && newjpos == 0;
                         target = jump_chain[INSN_UID(target)])
                        if (target != insn && !INSN_DELETED_P(target)
...

    {
        ...
        for (insn = f; insn; insn = NEXT_INSN(insn))
            if (GET_CODE(insn) == NOTE && NOTE_LINE_NUMBER(insn) >= 0)
            {
                ...
                if (last_note && NOTE_SOURCE_FILE(insn) == NOTE_SOURCE_FILE(last_note)
...
static void delete_barrier_successors(rtx f)
{
    rtx insn;
    ...
    for (insn = f; insn;)
    {
        ...
        else if (GET_CODE(insn) == JUMP_INSN && SET_SRC(PATTERN(insn)) == pc_rtx
...
static void mark_all_labels(rtx f, int cross_jump)
{
    rtx insn;
...
static rtx delete_unreferenced_labels(rtx f)
{
    rtx final = NULL_RTX;
    rtx insn;
...
static void delete_noop_moves(rtx f)
{
    rtx insn, next;
        ...
        if (GET_CODE(insn) == INSN)
        {
            ...
            if (GET_CODE(body) == SET
                && (SET_DEST(body) == SET_SRC(body)
                       || (GET_CODE(SET_DEST(body)) == MEM && GET_CODE(SET_SRC(body)) == MEM
                              && rtx_equal_p(SET_SRC(body), SET_DEST(body))))
                && !(GET_CODE(SET_DEST(body)) == MEM && MEM_VOLATILE_P(SET_DEST(body)))
                && !(GET_CODE(SET_SRC(body)) == MEM && MEM_VOLATILE_P(SET_SRC(body))))
            ...
            else if (GET_CODE(body) == SET)
            {
                ...
                else if (sreg >= 0 && dreg >= 0)
                {
                    ...
                    if (tem != 0 && GET_MODE(tem) == GET_MODE(SET_DEST(body)))
                    {
                        ...
                        if ((trial = find_regno_note(insn, REG_DEAD, sreg)))
                        {
                            ...
                            PATTERN(insn) = gen_rtx_USE(VOIDmode, XEXP(trial, 0));
                            INSN_CODE(insn) = -1;
                           ...
                           SET_SRC(body), insn, (enum reg_class)0, dreg, NULL, 0, GET_MODE(SET_DEST(body))))
                {
                    ...
                    {
                        if ((GET_CODE(in_insn) == INSN || GET_CODE(in_insn) == JUMP_INSN)
            ...
            else if (GET_CODE(body) == PARALLEL)
            {
                ...
                for (i = XVECLEN(body, 0) - 1; i >= 0; i--)
                {
                    ...
                    if (GET_CODE(tem) != SET || (sreg = true_regnum(SET_SRC(tem))) < 0
                        || (dreg = true_regnum(SET_DEST(tem))) < 0 || dreg != sreg)
                        break;
                }
            ...
            else if (GET_CODE(body) == SET && GET_CODE(SET_DEST(body)) == ZERO_EXTRACT
                && XEXP(SET_DEST(body), 2) == const0_rtx && XEXP(SET_DEST(body), 0) == SET_SRC(body)
                && !(GET_CODE(SET_SRC(body)) == MEM && MEM_VOLATILE_P(SET_SRC(body))))
                delete_insn(insn);
        }
        insn = next;
...
static int calculate_can_reach_end(rtx last, int check_deleted, int delete_final_note)
{
    rtx insn = last;
    int n_labels = 1;
    ...
    while (insn != NULL_RTX)
    {
        ...
        else if (GET_CODE(insn) == NOTE && NOTE_LINE_NUMBER(insn) != NOTE_INSN_FUNCTION_END)
            ok = 1;
    ...
    if (insn != NULL_RTX && GET_CODE(insn) == NOTE
        ...
        && (check_deleted == 0 || !INSN_DELETED_P(insn)))
    {
...
static int duplicate_loop_exit_test(rtx loop_start)
{
    ...
    for (insn = exitcode;
         insn && !(GET_CODE(insn) == NOTE && NOTE_LINE_NUMBER(insn) == NOTE_INSN_LOOP_END);
         insn = NEXT_INSN(insn))
    {
        ...
        case INSN:
            ...
            while ((p = find_reg_note(insn, REG_WAS_0, NULL_RTX)) != 0)
                remove_note(insn, p);
            if (++num_insns > LOOP_TEST_THRESHOLD || find_reg_note(insn, REG_RETVAL, NULL_RTX)
                || find_reg_note(insn, REG_LIBCALL, NULL_RTX) || asm_noperands(PATTERN(insn)) > 0)
                return 0;
            break;
        default:
            break;
    ...
    for (insn = exitcode; insn != lastexit; insn = NEXT_INSN(insn))
        if (GET_CODE(insn) == INSN && (set = single_set(insn)) != 0
            && ((reg = SET_DEST(set), GET_CODE(reg) == REG)
                   || (GET_CODE(reg) == SUBREG && (reg = SUBREG_REG(reg), GET_CODE(reg) == REG)))
            && REGNO(reg) >= FIRST_PSEUDO_REGISTER && REGNO_FIRST_UID(REGNO(reg)) == INSN_UID(insn))
        {
    ...
    for (insn = exitcode; insn != lastexit; insn = NEXT_INSN(insn))
        ...
        case INSN:
            ...
            for (link = REG_NOTES(insn); link; link = XEXP(link, 1))
                if (REG_NOTE_KIND(link) != REG_LABEL)
                    REG_NOTES(copy) = copy_rtx(
                        gen_rtx_EXPR_LIST(REG_NOTE_KIND(link), XEXP(link, 0), REG_NOTES(copy)));
        ...
        case JUMP_INSN:
            ...
            if (INSN_UID(copy) < max_jump_chain && JUMP_LABEL(copy) && simplejump_p(copy))
            {
                jump_chain[INSN_UID(copy)] = jump_chain[INSN_UID(JUMP_LABEL(copy))];
    ...
    {
        copy = emit_jump_insn_before(gen_jump(get_label_after(insn)), loop_start);
        mark_jump_label(PATTERN(copy), copy, 0);
        if (INSN_UID(copy) < max_jump_chain && INSN_UID(JUMP_LABEL(copy)) < max_jump_chain)
        {
            jump_chain[INSN_UID(copy)] = jump_chain[INSN_UID(JUMP_LABEL(copy))];
            jump_chain[INSN_UID(JUMP_LABEL(copy))] = copy;
        }
...
rtx squeeze_notes(rtx start, rtx end)
{
    rtx insn;
    rtx next;
    ...
    *f2 = 0;
    ...
    while (1)
    {
        ...
        /* Avoid moving insns across EH regions if either of the insns
           can throw.  */
        if (flag_exceptions && (asynchronous_exceptions || GET_CODE(i1) == CALL_INSN)
        ...
        if (GET_CODE(i1) == CALL_INSN
            && !rtx_equal_p(CALL_INSN_FUNCTION_USAGE(i1), CALL_INSN_FUNCTION_USAGE(i2)))
            lose = 1;
        ...
        if (lose || GET_CODE(p1) != GET_CODE(p2) || !rtx_renumbered_equal_p(p1, p2))
        {
            ...
            if (!lose && GET_CODE(p1) == GET_CODE(p2)
                ...
                /* If the equivalences are not to a constant, they may
                   ...
                   use them.  */
                && CONSTANT_P(XEXP(equiv1, 0)) && rtx_equal_p(XEXP(equiv1, 0), XEXP(equiv2, 0)))
            {
                rtx s1 = single_set(i1);
                rtx s2 = single_set(i2);
                if (s1 != 0 && s2 != 0 && rtx_renumbered_equal_p(SET_DEST(s1), SET_DEST(s2)))
                {
            ...
            /* If cross-jumping here will feed a jump-around-jump
               ...
               the minimum.  */
            if (GET_CODE(i1) == JUMP_INSN && JUMP_LABEL(i1) && prev_real_insn(JUMP_LABEL(i1)) == e1)
                --minimum;
            break;
        }
...
static void do_cross_jump(rtx insn, rtx newjpos, rtx newlpos)
{
    ...
    if (GET_CODE(PATTERN(insn)) == RETURN)
    {
        ...
        if (INSN_UID(label) < max_jump_chain && INSN_UID(insn) < max_jump_chain)
        {
    ...
    while (newjpos != insn)
    {
        ...
        for (lnote = REG_NOTES(newlpos); lnote; lnote = XEXP(lnote, 1))
            if ((REG_NOTE_KIND(lnote) == REG_EQUAL || REG_NOTE_KIND(lnote) == REG_EQUIV)
...
static int jump_back_p(rtx insn, rtx target)
{
    rtx cinsn, ctarget;
    enum rtx_code codei, codet;

    if (simplejump_p(insn) || !condjump_p(insn) || simplejump_p(target)
    ...
    return (codei == codet && rtx_renumbered_equal_p(XEXP(cinsn, 0), XEXP(ctarget, 0))
...
int can_reverse_comparison_p(rtx comparison, rtx insn)
{
    rtx arg0;
    ...
    if (TARGET_FLOAT_FORMAT != IEEE_FLOAT_FORMAT
        ...
        || flag_fast_math || GET_CODE(comparison) == NE || GET_CODE(comparison) == EQ)
        return 1;
    ...
    return (GET_CODE(arg0) == CONST_INT
        || (GET_MODE(arg0) != VOIDmode && GET_MODE_CLASS(GET_MODE(arg0)) != MODE_CC
...
{
    return (GET_CODE(insn) == JUMP_INSN && GET_CODE(PATTERN(insn)) == SET
...
int condjump_p(rtx insn)
{
    ...
    if (XEXP(SET_SRC(x), 2) == pc_rtx
        && (GET_CODE(XEXP(SET_SRC(x), 1)) == LABEL_REF || GET_CODE(XEXP(SET_SRC(x), 1)) == RETURN))
        return 1;
    if (XEXP(SET_SRC(x), 1) == pc_rtx
        && (GET_CODE(XEXP(SET_SRC(x), 2)) == LABEL_REF || GET_CODE(XEXP(SET_SRC(x), 2)) == RETURN))
        return 1;
    return 0;
}
...
int sets_cc0_p(rtx x ATTRIBUTE_UNUSED)
{
    ...
    if (GET_CODE(x) == PARALLEL)
    {
        ...
        {
            if (GET_CODE(XVECEXP(x, 0, i)) == SET && SET_DEST(XVECEXP(x, 0, i)) == cc0_rtx)
                sets_cc0 = 1;
...
rtx follow_jumps(rtx label)
{
    ...
    for (depth = 0;
         (depth < 10 && (insn = next_active_insn(value)) != 0 && GET_CODE(insn) == JUMP_INSN
             && ((JUMP_LABEL(insn) != 0 && simplejump_p(insn)) || GET_CODE(PATTERN(insn)) == RETURN)
             && (next = NEXT_INSN(insn)) && GET_CODE(next) == BARRIER);
         depth++)
    {
        ...
        if (!reload_completed)
            for (tem = value; tem != insn; tem = NEXT_INSN(tem))
                if (GET_CODE(tem) == NOTE && (NOTE_LINE_NUMBER(tem) == NOTE_INSN_LOOP_BEG))
                    return value;
        ...
        if (tem && (GET_CODE(PATTERN(tem)) == ADDR_VEC || GET_CODE(PATTERN(tem)) == ADDR_DIFF_VEC))
            break;
    ...
    case LABEL_REF:
    {
        ...
        for (next = NEXT_INSN(label); next; next = NEXT_INSN(next))
        {
            if (GET_CODE(next) == CODE_LABEL)
                ...
                && (GET_CODE(PATTERN(next)) == USE || GET_CODE(PATTERN(next)) == CLOBBER))
                continue;
        ...
        if (insn)
        {
            ...
            else if (label != olabel && (note = find_reg_note(insn, REG_LABEL, olabel)) != 0)
                XEXP(note, 0) = label;
            ...
            else if (!find_reg_note(insn, REG_LABEL, label))
            {
                ...
                REG_NOTES(insn) = gen_rtx_EXPR_LIST(REG_LABEL, label, REG_NOTES(insn));
            }
        }
        return;
    }
...
static void delete_computation(rtx insn)
{
    ...
    if (reg_referenced_p(cc0_rtx, PATTERN(insn)))
    {
        ...
        if (prev && GET_CODE(prev) == INSN && sets_cc0_p(PATTERN(prev)))
        {
            if (sets_cc0_p(PATTERN(prev)) > 0 && !FIND_REG_INC_NOTE(prev, NULL_RTX))
                delete_computation(prev);
            else
                /* Otherwise, show that cc0 won't be used.  */
                REG_NOTES(prev) = gen_rtx_EXPR_LIST(REG_UNUSED, cc0_rtx, REG_NOTES(prev));
        }
    }
#endif
    ...
    if (!reload_completed)
        ...
        for (note = REG_NOTES(insn); note; note = next)
        {
            ...
            for (our_prev = prev_nonnote_insn(insn); our_prev && GET_CODE(our_prev) == INSN;
                 our_prev = prev_nonnote_insn(our_prev))
            {
                ...
                if (reg_set_p(XEXP(note, 0), PATTERN(our_prev)))
                {
                    ...
                    if (GET_CODE(PATTERN(our_prev)) == PARALLEL)
                    {
                        ...
                        for (i = 0; i < XVECLEN(PATTERN(our_prev), 0); i++)
                        {
                            ...
                            if (GET_CODE(part) == SET && SET_DEST(part) != XEXP(note, 0))
                                break;
                        }
                ...
                if (reg_overlap_mentioned_p(XEXP(note, 0), PATTERN(our_prev)))
                {
...
rtx delete_insn(rtx insn)
{
    ...
    /* Don't delete user-declared labels.  Convert them to special NOTEs
       instead.  */
    if (was_code_label && LABEL_NAME(insn) != 0 && optimize && !dont_really_delete)
    {
        PUT_CODE(insn, NOTE);
    ...
    if (optimize && !dont_really_delete)
    {
        if (prev)
        {
            ...
            if (GET_CODE(prev) == INSN && GET_CODE(PATTERN(prev)) == SEQUENCE)
                NEXT_INSN(XVECEXP(PATTERN(prev), 0, XVECLEN(PATTERN(prev), 0) - 1)) = next;
        }
    ...
    if (GET_CODE(insn) == JUMP_INSN
        && (GET_CODE(PATTERN(insn)) == ADDR_VEC || GET_CODE(PATTERN(insn)) == ADDR_DIFF_VEC))
    {
        rtx pat = PATTERN(insn);
    ...
    if (was_code_label && NEXT_INSN(insn) != 0 && GET_CODE(NEXT_INSN(insn)) == JUMP_INSN
    ...
    if (was_code_label && prev && GET_CODE(prev) == BARRIER)
    {
        ...
        {
            if (code == NOTE && NOTE_LINE_NUMBER(next) != NOTE_INSN_FUNCTION_END)
                next = NEXT_INSN(next);
...
int invert_jump(rtx jump, rtx nlabel)
{
    ...
    if (code == IF_THEN_ELSE)
    {
        ...
        if (can_reverse_comparison_p(comp, insn)
            && validate_change(insn, &XEXP(x, 0),
                   gen_rtx_fmt_ee(reverse_condition(GET_CODE(comp)), GET_MODE(comp), XEXP(comp, 0),
                       XEXP(comp, 1)),
                   0))
            return 1;
...
int redirect_jump(rtx jump, rtx nlabel)
{
    ...
    if (jump_chain && (simplejump_p(jump) || GET_CODE(PATTERN(jump)) == RETURN))
    {
        int label_index = nlabel ? INSN_UID(nlabel) : 0;

        delete_from_jump_chain(jump);
        if (label_index < max_jump_chain && INSN_UID(jump) < max_jump_chain)
        {
...
static void delete_from_jump_chain(rtx jump)
{
    int index;
    rtx olabel = JUMP_LABEL(jump);

    /* Handle unconditional jumps.  */
    if (jump_chain && olabel != 0 && INSN_UID(olabel) < max_jump_chain && simplejump_p(jump))
    ...
    else
    {
        ...
        for (insn = jump_chain[index]; insn != 0; insn = jump_chain[INSN_UID(insn)])
...
int redirect_exp(rtx *loc, rtx olabel, rtx nlabel, rtx insn)
{
    ...
    if (code == SET && nlabel == 0 && SET_DEST(x) == pc_rtx && GET_CODE(SET_SRC(x)) == LABEL_REF
...
static int delete_labelref_insn(rtx insn, rtx label, int delete_this)
{
    int deleted = 0;
    rtx link;

    if (GET_CODE(insn) != NOTE && reg_mentioned_p(label, PATTERN(insn)))
    {
        if (delete_this)
        {
            delete_insn(insn);
...
int rtx_renumbered_equal_p(rtx x, rtx y)
{
    ...
    case LABEL_REF:
        ...
        return (next_real_insn(XEXP(x, 0)) == next_real_insn(XEXP(y, 0)));
    ...
    if ((code == EQ || code == NE || GET_RTX_CLASS(code) == 'c') && code != PLUS)
...
static void mark_modified_reg(rtx dest ATTRIBUTE_UNUSED, rtx x)
{
    int regno, i;
...
void thread_jumps(rtx f, int max_reg, int flag_before_loop)
{
    ...
    while (changed)
    {
        ...
        for (b1 = f; b1; b1 = NEXT_INSN(b1))
        {
            ...
            if (GET_CODE(b1) != JUMP_INSN || !condjump_p(b1) || simplejump_p(b1)
                || JUMP_LABEL(b1) == 0)
                continue;
            ...
            copy_memory((char *)all_reset, (char *)same_regs, max_reg * sizeof(int));
            num_same_regs = 0;
            ...
            for (b2 = NEXT_INSN(label); b2; b2 = NEXT_INSN(b2))
            {
                ...
                else if (GET_CODE(b2) == JUMP_INSN)
                {
                    ...
                    if (simplejump_p(b2) && JUMP_LABEL(b2) != 0 && LABEL_NUSES(JUMP_LABEL(b2)) == 1)
                    {
                        b2 = JUMP_LABEL(b2);
                        continue;
                    }
                ...
                if (GET_CODE(b2) == CALL_INSN)
                {
                    ...
                    for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
                        if (call_used_regs[i] && !fixed_regs[i] && i != STACK_POINTER_REGNUM
            ...
            /* Check the next candidate branch insn from the label
               of the first.  */
            if (b2 == 0 || GET_CODE(b2) != JUMP_INSN || b2 == b1 || !condjump_p(b2)
                || simplejump_p(b2))
                continue;
            ...
            if (rtx_equal_for_thread_p(b1op0, b2op0, b2) && rtx_equal_for_thread_p(b1op1, b2op1, b2)
                && (comparison_dominates_p(code1, code2)
                       || (comparison_dominates_p(code1, reverse_condition(code2))
                              && can_reverse_comparison_p(XEXP(SET_SRC(PATTERN(b1)), 0), b1))))
            {
                ...
                while (t1 != 0 && t2 != 0)
                {
                    if (t2 == label)
                    {
                        ...
                        if (JUMP_LABEL(b1) != new_label)
                        {
                            ...
                            if (flag_before_loop && GET_CODE(prev) == NOTE
...
int rtx_equal_for_thread_p(rtx x, rtx y, rtx yinsn)
{
    ...
    /* For floating-point, consider everything unequal.  This is a bit
       ...
       anyway.  */
    if (TARGET_FLOAT_FORMAT == IEEE_FLOAT_FORMAT && FLOAT_MODE_P(GET_MODE(x)) && !flag_fast_math)
        return 0;
    ...
    case REG:
        ...
        /* If neither is user variable or hard register, check for possible
           equivalence.  */
        if (REG_USERVAR_P(x) || REG_USERVAR_P(y) || REGNO(x) < FIRST_PSEUDO_REGISTER
    ...
    case SET:
        /* Cancel a pending `same_regs' if setting equivalenced registers.
           Then process source.  */
        if (GET_CODE(SET_DEST(x)) == REG && GET_CODE(SET_DEST(y)) == REG)
        {
        ...
        else if (rtx_equal_for_thread_p(SET_DEST(x), SET_DEST(y), yinsn) == 0)
            return 0;
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        ...
        case 'E':
            ...
            for (j = 0; j < XVECLEN(x, i); j++)
                if (rtx_equal_for_thread_p(XVECEXP(x, i, j), XVECEXP(y, i, j), yinsn) == 0)
                    return 0;
            break;
...
static rtx find_insert_position(rtx insn, rtx new)
{
    int i;
    rtx prev;
    ...
    for (i = XVECLEN(PATTERN(new), 0) - 1; i >= 0; i--)
        if (GET_CODE(XVECEXP(PATTERN(new), 0, i)) == CLOBBER
            && reg_overlap_mentioned_p(XEXP(XVECEXP(PATTERN(new), 0, i), 0), insn))
            break;
    ...
    for (i = XVECLEN(PATTERN(new), 0) - 1; i >= 0; i--)
        if (GET_CODE(XVECEXP(PATTERN(new), 0, i)) == CLOBBER
            && reg_overlap_mentioned_p(XEXP(XVECEXP(PATTERN(new), 0, i), 0), insn)
            && !modified_in_p(XEXP(XVECEXP(PATTERN(new), 0, i), 0), prev))
            return 0;
...
int condjump_expect_p(rtx insn)
{
    ...
    lab1 = XEXP(src, 1);
    lab2 = XEXP(src, 2);
    if (lab2 == pc_rtx && (GET_CODE(lab1) == LABEL_REF || GET_CODE(lab1) == RETURN))
        return retval;

    if (lab1 == pc_rtx && (GET_CODE(lab2) == LABEL_REF || GET_CODE(lab2) == RETURN))
        return -retval;
...
static rtx conditional_execution(rtx insn)
{
    /* Array of insns to make conditionally execute, along with the new pattern
     * created.  */
    typedef struct
    {
        rtx insn;
    ...
    for (p = next_nonnote_insn(insn);; p = next_nonnote_insn(p))
    {
        ...
        if (GET_CODE(p) == INSN || GET_CODE(p) == CALL_INSN)
        {
            ...
            must_be_last = (GET_CODE(p) == CALL_INSN || modified_in_p(if_test, PATTERN(p)));
    ...
    /* If this is has an if-then-else clause, move the else insns after the
     * if-then insns */
    if (if_end && else_start)
        reorder_insns(else_start, else_end, if_end);

    /* Figure out where the next insn is (label or insn after it if we delete the
     * label) */
...
static rtx branch_predict_move(rtx insn, rtx cur_line, rtx barrier, rtx barrier_end)
{
    ...
    /* Add a line number note, if the first insn in the moved insns
       is not one.  */
    if (cur_line && (GET_CODE(after_insn) != NOTE || NOTE_LINE_NUMBER(after_insn) <= 0))
    {
        barrier_end = emit_line_note_after(
            NOTE_SOURCE_FILE(cur_line), NOTE_LINE_NUMBER(cur_line), barrier_end);
    }
...
static void branch_predict_reorg(rtx f)
{
    rtx insn;
    rtx last_insn;
    rtx barrier;
    ...
    /* Loop moving code we don't expect to execute to the end of the function.
       Each loop after the first selects the code that was moved for further
       branch selections until there were no more changes.  */
    do
    {
        changed = 0;
        for (insn = f; insn && insn != last_insn; insn = NEXT_INSN(insn))
        {
            ...
            /* Found a branch that is expected to succeed?  See if we can move
               the code around.  */
            if (branch_predict_p > 0)
            {
                rtx old_end = barrier_end;
                barrier_end = branch_predict_move(insn, cur_line, barrier, barrier_end);
