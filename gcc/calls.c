...
static void emit_call_1(rtx, tree, tree, int32_t, int32_t, rtx, rtx, int, rtx, int);
static void special_function_p(char *, tree, int *, int *, int *, int *);
static void precompute_register_parameters(int, struct arg_data *, int *);
static void store_one_arg(struct arg_data *, rtx, int, int, int);
...
static int calls_function_1(tree exp, int which)
{
    ...
    if (type != 'e' && type != '<' && type != '1' && type != '2' && type != 'r' && type != 'b')
        return 0;
    ...
    case CALL_EXPR:
            ...
            && (TREE_CODE(TREE_OPERAND(TREE_OPERAND(exp, 0), 0)) == FUNCTION_DECL))
        {
            ...
            if ((DECL_BUILT_IN(fndecl) && DECL_FUNCTION_CODE(fndecl) == BUILT_IN_ALLOCA)
                || (DECL_SAVED_INSNS(fndecl)
                       && (FUNCTION_FLAGS(DECL_SAVED_INSNS(fndecl)) & FUNCTION_FLAGS_CALLS_ALLOCA)))
                return 1;
        }
    ...
    case SAVE_EXPR:
        ...
        if (value_member(exp, calls_function_save_exprs))
            return 0;
        calls_function_save_exprs = tree_cons(NULL_TREE, exp, calls_function_save_exprs);
        return (TREE_OPERAND(exp, 0) != 0 && calls_function_1(TREE_OPERAND(exp, 0), which));
    ...
    for (i = 0; i < length; i++)
        if (TREE_OPERAND(exp, i) != 0 && calls_function_1(TREE_OPERAND(exp, i), which))
            return 1;
...
rtx prepare_call_address(rtx funexp, tree fndecl, rtx *call_fusage, int reg_parm_seen)
{
...
static void emit_call_1(rtx funexp, tree fndecl ATTRIBUTE_UNUSED, tree funtype ATTRIBUTE_UNUSED,
    int32_t stack_size, int32_t struct_value_size, rtx next_arg_reg, rtx valreg,
    int old_inhibit_defer_pop, rtx call_fusage, int is_const)
{
    ...
    if (HAVE_call_pop && HAVE_call_value_pop
        && (RETURN_POPS_ARGS(fndecl, funtype, stack_size) > 0 || stack_size == 0))
    {
       ...
       if possible, for the sake of frame pointer elimination.  */
        ...
        if (valreg)
            pat = gen_call_value_pop(
                valreg, gen_rtx_MEM(FUNCTION_MODE, funexp), stack_size_rtx, next_arg_reg, n_pop);
        else
            pat = gen_call_pop(
                gen_rtx_MEM(FUNCTION_MODE, funexp), stack_size_rtx, next_arg_reg, n_pop);
...
static void special_function_p(char *name, tree fndecl, int *returns_twice, int *is_longjmp,
    int *is_malloc, int *may_be_alloca)
{
    *returns_twice = 0;
    *is_longjmp = 0;
    *is_malloc = 0;
    *may_be_alloca = 0;
        ...
        && DECL_CONTEXT(fndecl) == NULL_TREE && TREE_PUBLIC(fndecl))
    {
        ...
        *may_be_alloca = ((
            (IDENTIFIER_LENGTH(DECL_NAME(fndecl)) == 6 && name[0] == 'a' && !strcmp(name, "alloca"))
            || (IDENTIFIER_LENGTH(DECL_NAME(fndecl)) == 16 && name[0] == '_'
        ...
        if (tname[0] == 's')
        {
            *returns_twice = ((tname[1] == 'e'
                                  && (!strcmp(tname, "setjmp") || !strcmp(tname, "setjmp_syscall")))
                || (tname[1] == 'i' && !strcmp(tname, "sigsetjmp"))
                || (tname[1] == 'a' && !strcmp(tname, "savectx")));
            if (tname[1] == 'i' && !strcmp(tname, "siglongjmp"))
                *is_longjmp = 1;
        }
        else if ((tname[0] == 'q' && tname[1] == 's' && !strcmp(tname, "qsetjmp"))
            || (tname[0] == 'v' && tname[1] == 'f' && !strcmp(tname, "vfork")))
            *returns_twice = 1;

        else if (tname[0] == 'l' && tname[1] == 'o' && !strcmp(tname, "longjmp"))
            *is_longjmp = 1;
        /* XXX should have "malloc" attribute on functions instead
       of recognizing them by name.  */
        else if (!strcmp(tname, "malloc") || !strcmp(tname, "calloc")
           ...
           are only reserved when preceded with __.  */
            ...
            || !strcmp(name, "__builtin_new") || !strcmp(name, "__builtin_vec_new"))
            *is_malloc = 1;
    }
}
...
static void precompute_register_parameters(
    int num_actuals, struct arg_data *args, int *reg_parm_seen)
{
    int i;
        ...
        if (args[i].reg != 0 && !args[i].pass_on_stack)
        {
            *reg_parm_seen = 1;
            ...
            if (args[i].mode != TYPE_MODE(TREE_TYPE(args[i].tree_value)))
                args[i].value = convert_modes(args[i].mode,
                    TYPE_MODE(TREE_TYPE(args[i].tree_value)), args[i].value, args[i].unsignedp);
            ...
            if ((!(GET_CODE(args[i].value) == REG
                ...
                && args[i].mode != BLKmode && rtx_cost(args[i].value, SET) > 2
                && ((SMALL_REGISTER_CLASSES && *reg_parm_seen) || preserve_subexpressions_p()))
...
static rtx save_fixed_argument_area(
    int reg_parm_stack_space, rtx argblock, int *low_to_save, int *high_to_save)
{
    int i;
    ...
    if (*low_to_save >= 0)
    {
        ...
        enum machine_mode save_mode = mode_for_size(num_to_save * BITS_PER_UNIT, MODE_INT, 1);
        rtx stack_area;

        /* If we don't have the required alignment, must do this in BLKmode.  */
        if ((*low_to_save
                & (MIN(GET_MODE_SIZE(save_mode), BIGGEST_ALIGNMENT / UNITS_PER_WORD) - 1)))
            save_mode = BLKmode;

#ifdef ARGS_GROW_DOWNWARD
        stack_area = gen_rtx_MEM(
            save_mode, memory_address(save_mode, plus_constant(argblock, -*high_to_save)));
#else
        stack_area = gen_rtx_MEM(
            save_mode, memory_address(save_mode, plus_constant(argblock, *low_to_save)));
#endif
        if (save_mode == BLKmode)
        {
            save_area = assign_stack_temp(BLKmode, num_to_save, 0);
            emit_block_move(validize_mem(save_area), stack_area, GEN_INT(num_to_save),
...
static void restore_fixed_argument_area(
    rtx save_area, rtx argblock, int high_to_save, int low_to_save)
{
    enum machine_mode save_mode = GET_MODE(save_area);
#ifdef ARGS_GROW_DOWNWARD
    rtx stack_area
        = gen_rtx_MEM(save_mode, memory_address(save_mode, plus_constant(argblock, -high_to_save)));
#else
    rtx stack_area
        = gen_rtx_MEM(save_mode, memory_address(save_mode, plus_constant(argblock, low_to_save)));
#endif
    ...
    if (save_mode != BLKmode)
        ...
        emit_block_move(stack_area, validize_mem(save_area),
            GEN_INT(high_to_save - low_to_save + 1), PARM_BOUNDARY / BITS_PER_UNIT);
}
#endif
...
static void store_unaligned_arguments_into_pseudos(struct arg_data *args, int num_actuals)
{
    ...
    for (i = 0; i < num_actuals; i++)
        if (args[i].reg != 0 && !args[i].pass_on_stack && args[i].mode == BLKmode
                   ...
                   < (unsigned int)MIN(BIGGEST_ALIGNMENT, BITS_PER_WORD)))
        {
            ...
            args[i].n_aligned_regs = args[i].partial
                ? args[i].partial
                : (bytes + (UNITS_PER_WORD - 1)) / UNITS_PER_WORD;

            args[i].aligned_regs = (rtx *)xmalloc(sizeof(rtx) * args[i].n_aligned_regs);
            ...
            for (j = 0; j < args[i].n_aligned_regs; j++)
            {
                ...
                store_bit_field(reg, bitsize, big_endian_correction, word_mode,
                    extract_bit_field(word, bitsize, 0, 1, NULL_RTX, word_mode, word_mode,
...
rtx expand_call(tree exp, rtx target, int ignore)
{
    ...
    if (TREE_CODE(p) == ADDR_EXPR)
    {
        ...
        {
            if (!flag_no_inline && fndecl != current_function_decl && DECL_INLINE(fndecl)
                && DECL_SAVED_INSNS(fndecl) && RTX_INTEGRATED_P(DECL_SAVED_INSNS(fndecl)))
                is_integrable = 1;
            else if (!TREE_ADDRESSABLE(fndecl))
            {
               ...
               directly.  */
                if (DECL_INLINE(fndecl) && warn_inline && !flag_no_inline && optimize > 0)
                {
    ...
    if (is_integrable)
    {
        ...
        temp = expand_inline_function(
            fndecl, actparms, target, ignore, TREE_TYPE(exp), structure_value_addr);

        /* If inlining succeeded, return.  */
        if (temp != (rtx)(int32_t)-1)
        {
#ifdef ACCUMULATE_OUTGOING_ARGS
            ...
            {
                rtx first_insn = before_call ? NEXT_INSN(before_call) : get_insns();
                rtx insn = 0, seq = 0;
               ...
               to scan the insns.  */
                ...
                if (insn)
                {
                    ...
                    int adjust
                        = (OUTGOING_ARGS_SIZE(DECL_SAVED_INSNS(fndecl)) + reg_parm_stack_space);

                    start_sequence();
                    emit_stack_save(SAVE_BLOCK, &old_stack_level, NULL_RTX);
                    allocate_dynamic_stack_space(GEN_INT(adjust), NULL_RTX, BITS_PER_UNIT);
                    seq = get_insns();
                    end_sequence();
    ...
    special_function_p(name, fndecl, &returns_twice, &is_longjmp, &is_malloc, &may_be_alloca);
    ...
    if (pending_stack_adjust >= 32 || (pending_stack_adjust > 0 && may_be_alloca))
    ...
    if (structure_value_addr && struct_value_rtx == 0)
    {
       ...
       register in some cases.  */
        ...
        actparms = tree_cons(
            error_mark_node, make_tree(build_pointer_type(TREE_TYPE(funtype)), temp), actparms);
        structure_value_addr_parm = 1;
    }

    /* Count the arguments and set NUM_ACTUALS.  */
    for (p = actparms, i = 0; p; p = TREE_CHAIN(p))
        i++;
    num_actuals = i;
    ...
    for (p = actparms, argpos = 0; p; p = TREE_CHAIN(p), i += inc, argpos++)
    {
       ...
       with those made by function.c.  */
        ...
        if ((TREE_CODE(TYPE_SIZE(type)) != INTEGER_CST && contains_placeholder_p(TYPE_SIZE(type)))
            || TREE_ADDRESSABLE(type)
#ifdef FUNCTION_ARG_PASS_BY_REFERENCE
            || FUNCTION_ARG_PASS_BY_REFERENCE(
                   args_so_far, TYPE_MODE(type), type, argpos < n_named_args)
#endif
        )
        {
            ...
            )
            {
                ...
                if (TREE_CODE(args[i].tree_value) == TARGET_EXPR
                    && (TREE_CODE_CLASS(TREE_CODE(TREE_OPERAND(args[i].tree_value, 1))) == 'd')
                    && !REG_P(DECL_RTL(TREE_OPERAND(args[i].tree_value, 1))))
                    args[i].tree_value = TREE_OPERAND(args[i].tree_value, 1);

                args[i].tree_value
                    = build1(ADDR_EXPR, build_pointer_type(type), args[i].tree_value);
               ...
               function being called.  */
                                         ...
                                         > STACK_CHECK_MAX_VAR_SIZE * BITS_PER_UNIT))))
                {
                    ...
                    copy = gen_rtx_MEM(BLKmode,
                        allocate_dynamic_stack_space(size_rtx, NULL_RTX, TYPE_ALIGN(type)));
                }
                else
                {
                ...
                args[i].tree_value
                    = build1(ADDR_EXPR, build_pointer_type(type), make_tree(type, copy));
                type = build_pointer_type(type);
            }
        }
       ...
       we cannot consider this function call constant.  */
        ...
        if (args[i].reg == 0 || args[i].partial != 0 || reg_parm_stack_space > 0
            ...
            locate_and_pad_parm(mode, type,
                ...
                args[i].reg != 0,
#endif
                fndecl, &args_size, &args[i].offset, &args[i].size);
       ...
       have been used, etc.  */
        ...
        FUNCTION_ARG_ADVANCE(args_so_far, TYPE_MODE(type), type, argpos < n_named_args);
    }

#ifdef FINAL_REG_PARM_STACK_SPACE
    reg_parm_stack_space = FINAL_REG_PARM_STACK_SPACE(args_size.constant, args_size.var);
#endif
    ...
    if (args_size.var)
    {
       ...
       in the cse'able block and which don't.  */
        ...
        {
            args_size.var = size_binop(MAX_EXPR, args_size.var, size_int(reg_parm_stack_space));

#ifndef OUTGOING_REG_PARM_STACK_SPACE
            /* The area corresponding to register parameters is not to count in
               the size of the block we need.  So make the adjustment.  */
            args_size.var = size_binop(MINUS_EXPR, args_size.var, size_int(reg_parm_stack_space));
#endif
        }
    }
    else
...
#ifdef PREFERRED_STACK_BOUNDARY
        args_size.constant
            = (((args_size.constant + (STACK_BYTES - 1)) / STACK_BYTES) * STACK_BYTES);
#endif

        args_size.constant = MAX(args_size.constant, reg_parm_stack_space);
    ...
    if (!must_preallocate)
    {
        ...
        if (copy_to_evaluate_size * 2 >= args_size.constant && args_size.constant > 0)
            must_preallocate = 1;
    }
    ...
    for (i = 0; i < num_actuals; i++)
                   ...
                   && calls_function(args[i].tree_value, 0)))
        {
            ...
            args[i].initial_value = args[i].value = protect_from_queue(args[i].initial_value, 0);
       ...
       checking).  */
        ...
        if (must_preallocate)
        {
...
#ifdef ARGS_GROW_DOWNWARD
            highest_outgoing_arg_in_use = MAX(initial_highest_arg_in_use, needed + 1);
#else
            ...
            if (initial_highest_arg_in_use)
                copy_memory(initial_stack_usage_map, stack_usage_map, initial_highest_arg_in_use);
    ...
    if (argblock)
    {
        ...
        for (i = 0; i < num_actuals; i++)
        {
            ...
            MEM_SET_IN_STRUCT_P(args[i].stack, AGGREGATE_TYPE_P(TREE_TYPE(args[i].tree_value)));
    ...
    /* Generate an rtx (probably a pseudo-register) for the address.  */
    {
        ...
        if (current_function_check_memory_usage)
            emit_library_call(chkr_check_exec_libfunc, 1, VOIDmode, 1, funexp, ptr_mode);
        emit_queue();
    }

    /* Figure out the register where the value, if any, will come back.  */
    valreg = 0;
    if (TYPE_MODE(TREE_TYPE(exp)) != VOIDmode && !structure_value_addr)
    {
        if (pcc_struct_value)
            valreg = hard_function_value(build_pointer_type(TREE_TYPE(exp)), fndecl);
        else
    ...
    save_area
        = save_fixed_argument_area(reg_parm_stack_space, argblock, &low_to_save, &high_to_save);
#endif
    ...
    for (i = 0; i < num_actuals; i++)
        if (args[i].reg == 0 || args[i].pass_on_stack)
            store_one_arg(
                &args[i], argblock, may_be_alloca, args_size.var != 0, reg_parm_stack_space);
    ...
    {
        emit_move_insn(
            struct_value_rtx, force_reg(Pmode, force_operand(structure_value_addr, NULL_RTX)));

        /* Mark the memory for the aggregate as write-only.  */
        if (current_function_check_memory_usage)
            emit_library_call(chkr_set_right_libfunc, 1, VOIDmode, 3, structure_value_addr,
                ptr_mode, GEN_INT(struct_value_size), TYPE_MODE(sizetype), GEN_INT(MEMORY_USE_WO),
        ...
        if (reg)
        {
            ...
            if (GET_CODE(reg) == PARALLEL)
            {
                emit_group_load(reg, args[i].value,
                    int_size_in_bytes(TREE_TYPE(args[i].tree_value)),
                    (TYPE_ALIGN(TREE_TYPE(args[i].tree_value)) / BITS_PER_UNIT));
            }
            ...
            /* If we have pre-computed the values to put in the registers in
               the case of non-aligned structures, copy them in now.  */
                ...
                for (j = 0; j < args[i].n_aligned_regs; j++)
                    emit_move_insn(gen_rtx_REG(word_mode, REGNO(reg) + j), args[i].aligned_regs[j]);

            else if (partial == 0 || args[i].pass_on_stack)
                move_block_to_reg(REGNO(reg), validize_mem(args[i].value), nregs, args[i].mode);
    ...
    emit_call_1(funexp, fndecl, funtype, args_size.constant, struct_value_size,
        FUNCTION_ARG(args_so_far, VOIDmode, void_type_node, 1), valreg, old_inhibit_defer_pop,
    ...
    else if (is_malloc)
    {
       ...
       anything else.  */
        last = get_last_insn();
        REG_NOTES(last) = gen_rtx_EXPR_LIST(REG_NOALIAS, temp, REG_NOTES(last));
    ...
    if (any_pending_cleanups(1) && target && REG_P(target) && REGNO(target) < FIRST_PSEUDO_REGISTER)
        target = 0;

    if (TYPE_MODE(TREE_TYPE(exp)) == VOIDmode || ignore)
    {
        target = const0_rtx;
    }
    else if (structure_value_addr)
    {
        if (target == 0 || GET_CODE(target) != MEM)
        {
            target = gen_rtx_MEM(TYPE_MODE(TREE_TYPE(exp)),
                memory_address(TYPE_MODE(TREE_TYPE(exp)), structure_value_addr));
    ...
    else if (pcc_struct_value)
    {
       ...
       never use this value more than once in one expression.  */
        target = gen_rtx_MEM(TYPE_MODE(TREE_TYPE(exp)), copy_to_reg(valreg));
        MEM_SET_IN_STRUCT_P(target, AGGREGATE_TYPE_P(TREE_TYPE(exp)));
    }
    ...
    else if (GET_CODE(valreg) == PARALLEL)
    {
        ...
        emit_group_store(target, valreg, bytes, TYPE_ALIGN(TREE_TYPE(exp)) / BITS_PER_UNIT);
    }
    ...
    if (GET_CODE(target) == REG && TYPE_MODE(TREE_TYPE(exp)) != BLKmode
        && GET_MODE(target) != TYPE_MODE(TREE_TYPE(exp)))
    {
        ...
        if (GET_MODE(target) != promote_mode(type, TYPE_MODE(type), &unsignedp, 1))
            abort();
    ...
    else
    {
        ...
        for (i = 0; i < num_actuals; i++)
            if (args[i].save_area)
            {
                ...
                rtx stack_area = gen_rtx_MEM(
                    save_mode, memory_address(save_mode, XEXP(args[i].stack_slot, 0)));

                if (save_mode != BLKmode)
                    emit_move_insn(stack_area, args[i].save_area);
                else
                    emit_block_move(stack_area, validize_mem(args[i].save_area),
                        GEN_INT(args[i].size.constant), PARM_BOUNDARY / BITS_PER_UNIT);
            }
...
void emit_library_call(rtx orgfun, int no_queue, enum machine_mode outmode, int nargs, ...)
{
    va_list p;
    /* Total size in bytes of all the stack-parms scanned so far.  */
    struct args_size args_size;
    ...
    for (count = 0; count < nargs; count++)
    {
       ...
       must do it earlier where we know the signedness of the arg.  */
        if (mode == BLKmode || (GET_MODE(val) != mode && GET_MODE(val) != VOIDmode))
            abort();
       ...
       either emit_move_insn or emit_push_insn will do that.  */
        ...
        if (argvec[count].reg && GET_CODE(argvec[count].reg) == PARALLEL)
            abort();
#ifdef FUNCTION_ARG_PARTIAL_NREGS
        argvec[count].partial = FUNCTION_ARG_PARTIAL_NREGS(args_so_far, mode, NULL_TREE, 1);
#else
        argvec[count].partial = 0;
#endif

        locate_and_pad_parm(mode, NULL_TREE, argvec[count].reg && argvec[count].partial == 0,
            NULL_TREE, &args_size, &argvec[count].offset, &argvec[count].size);
    ...
    original_args_size = args_size;
#ifdef PREFERRED_STACK_BOUNDARY
    args_size.constant = (((args_size.constant + (STACK_BYTES - 1)) / STACK_BYTES) * STACK_BYTES);
#endif
    ...
    if (low_to_save >= 0)
    {
        ...
        /* If we don't have the required alignment, must do this in BLKmode.  */
        if ((low_to_save & (MIN(GET_MODE_SIZE(save_mode), BIGGEST_ALIGNMENT / UNITS_PER_WORD) - 1)))
            save_mode = BLKmode;
        ...
        if (!(reg != 0 && partial == 0))
        {
            ...
            if (i != upper_bound)
            {
                ...
                enum machine_mode save_mode
                    = mode_for_size(argvec[argnum].size.constant * BITS_PER_UNIT, MODE_INT, 1);
                rtx stack_area = gen_rtx_MEM(save_mode,
                    memory_address(
                        save_mode, plus_constant(argblock, argvec[argnum].offset.constant)));
                argvec[argnum].save_area = gen_reg_rtx(save_mode);
                emit_move_insn(argvec[argnum].save_area, stack_area);
            }
#endif
            emit_push_insn(val, mode, NULL_TREE, NULL_RTX, 0, partial, reg, 0, argblock,
                GEN_INT(argvec[argnum].offset.constant), reg_parm_stack_space);
    ...
    emit_call_1(fun, get_identifier(XSTR(orgfun, 0)),
        build_function_type(
            outmode == VOIDmode ? void_type_node : type_for_mode(outmode, 0), NULL_TREE),
        args_size.constant, 0, FUNCTION_ARG(args_so_far, VOIDmode, void_type_node, 1),
        outmode != VOIDmode ? hard_libcall_value(outmode) : NULL_RTX, old_inhibit_defer_pop + 1,
        call_fusage, no_queue);
    ...
    for (count = 0; count < nargs; count++)
        if (argvec[count].save_area)
        {
            ...
            rtx stack_area = gen_rtx_MEM(save_mode,
                memory_address(save_mode, plus_constant(argblock, argvec[count].offset.constant)));
...
rtx emit_library_call_value(
    rtx orgfun, rtx value, int no_queue, enum machine_mode outmode, int nargs, ...)
{
    va_list p;
    /* Total size in bytes of all the stack-parms scanned so far.  */
    struct args_size args_size;
    ...
    if (mem_value && struct_value_rtx == 0 && !pcc_struct_value)
    {
        ...
        locate_and_pad_parm(Pmode, NULL_TREE, argvec[count].reg && argvec[count].partial == 0,
            NULL_TREE, &args_size, &argvec[count].offset, &argvec[count].size);
  ...
  if (! no_queue)
    ...
    {
        emit_move_insn(
            struct_value_rtx, force_reg(Pmode, force_operand(XEXP(mem_value, 0), NULL_RTX)));
        if (GET_CODE(struct_value_rtx) == REG)
            use_reg(&call_fusage, struct_value_rtx);
    }
    ...
    emit_call_1(fun, get_identifier(XSTR(orgfun, 0)),
        build_function_type(type_for_mode(outmode, 0), NULL_TREE), args_size.constant,
        struct_value_size, FUNCTION_ARG(args_so_far, VOIDmode, void_type_node, 1),
        mem_value == 0 ? hard_libcall_value(outmode) : NULL_RTX, old_inhibit_defer_pop + 1,
        call_fusage, is_const);
...
static rtx 
target_for_arg (tree type, rtx size, rtx args_addr, struct args_size offset)
{
  rtx target;
    ...
    int reg_parm_stack_space)
{
    ...
    if (argblock && !variable_size && arg->stack)
    {
        ...
        if (i != upper_bound)
        {
            ...
            rtx stack_area
                = gen_rtx_MEM(save_mode, memory_address(save_mode, XEXP(arg->stack_slot, 0)));

            if (save_mode == BLKmode)
            {
                arg->save_area = assign_stack_temp(BLKmode, arg->size.constant, 0);
                MEM_SET_IN_STRUCT_P(arg->save_area, AGGREGATE_TYPE_P(TREE_TYPE(arg->tree_value)));
                preserve_temp_slots(arg->save_area);
                emit_block_move(validize_mem(arg->save_area), stack_area,
                    GEN_INT(arg->size.constant), PARM_BOUNDARY / BITS_PER_UNIT);
            }
            else
            {
    ...
    if (arg->value == 0)
    {
       ...
       doesn't agree, convert the mode.  */
        ...
        if (arg->mode != TYPE_MODE(TREE_TYPE(pval)))
            arg->value
                = convert_modes(arg->mode, TYPE_MODE(TREE_TYPE(pval)), arg->value, arg->unsignedp);
       ...
       data.  */
        if (current_function_check_memory_usage && GET_CODE(arg->stack) == MEM)
        {
            emit_library_call(chkr_set_right_libfunc, 1, VOIDmode, 3, XEXP(arg->stack, 0), ptr_mode,
                ARGS_SIZE_RTX(arg->size), TYPE_MODE(sizetype), GEN_INT(MEMORY_USE_RW),
                TYPE_MODE(integer_type_node));
        }
    }
    else if (arg->mode != BLKmode)
    {
       ...
       round up to a multiple of the alignment for arguments.  */
        if (none != FUNCTION_ARG_PADDING(arg->mode, TREE_TYPE(pval)))
            used = (((size + PARM_BOUNDARY / BITS_PER_UNIT - 1) / (PARM_BOUNDARY / BITS_PER_UNIT))
                * (PARM_BOUNDARY / BITS_PER_UNIT));

        /* This isn't already where we want it on the stack, so put it there.
       This can either be done with push or copy insns.  */
        emit_push_insn(arg->value, arg->mode, TREE_TYPE(pval), NULL_RTX, 0, partial, reg,
            used - size, argblock, ARGS_SIZE_RTX(arg->offset), reg_parm_stack_space);
    }
    else
    {
       ...
       of the allocation unit for arguments.  */
        ...
        emit_push_insn(arg->value, arg->mode, TREE_TYPE(pval), size_rtx,
            TYPE_ALIGN(TREE_TYPE(pval)) / BITS_PER_UNIT, partial, reg, excess, argblock,
