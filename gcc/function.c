...
static rtx assign_outer_stack_local(enum machine_mode, int32_t, int, struct function *);
static struct temp_slot *find_temp_slot_from_address(rtx);
static void put_reg_into_stack(struct function *, rtx, tree, enum machine_mode, enum machine_mode, int, int, int);
static void fixup_var_refs(rtx, enum machine_mode, int);
static struct fixup_replacement *find_fixup_replacement(struct fixup_replacement **, rtx);
static void fixup_var_refs_insns(rtx, enum machine_mode, int, rtx, int);
static void fixup_var_refs_1(rtx, enum machine_mode, rtx *, rtx, struct fixup_replacement **);
...
struct function *find_function_data(tree decl)
{
    struct function *p;
...
void pop_function_context_from(tree context)
{
    ...
    current_function_contains_functions
        = p->contains_functions || p->inline_obstacks || context == current_function_decl;
...
rtx assign_stack_local(enum machine_mode mode, int32_t size, int align)
{
    rtx x, addr;
    ...
    if (virtuals_instantiated)
        addr = plus_constant(
            frame_pointer_rtx, (frame_offset + bigend_correction + STARTING_FRAME_OFFSET));
    else
        addr = plus_constant(virtual_stack_vars_rtx, frame_offset + bigend_correction);
...
static rtx assign_outer_stack_local(
    enum machine_mode mode, int32_t size, int align, struct function *function)
{
    rtx x, addr;
    int bigend_correction = 0;
    int alignment;
    ...
    /* Allocate in the memory associated with the function in whose frame
       we are assigning.  */
    push_obstacks(function->function_obstack, function->function_maybepermanent_obstack);
...
#endif
    addr = plus_constant(virtual_stack_vars_rtx, function->frame_offset + bigend_correction);
#ifndef FRAME_GROWS_DOWNWARD
    function->frame_offset += size;
#endif
    ...
    function->stack_slot_list = gen_rtx_EXPR_LIST(VOIDmode, x, function->stack_slot_list);
...
rtx assign_stack_temp(enum machine_mode mode, int32_t size, int keep)
{
    ...
    if (best_p)
    {
       ...
       for BLKmode slots, so that we can be sure of the alignment.  */
        if (GET_MODE(best_p->slot) == BLKmode)
        {
            ...
            int32_t rounded_size = CEIL_ROUND(size, alignment);
            ...
            if (best_p->size - rounded_size >= alignment)
            {
                ...
                p->slot = gen_rtx_MEM(BLKmode, plus_constant(XEXP(best_p->slot, 0), rounded_size));
                p->address = 0;
                p->rtl_expr = 0;
                p->next = temp_slots;
                temp_slots = p;

                stack_slot_list = gen_rtx_EXPR_LIST(VOIDmode, p->slot, stack_slot_list);
    ...
    if (p == 0)
    {
        int32_t frame_offset_old = frame_offset;
...
rtx assign_temp(tree type, int keep, int memory_required, int dont_promote)
{
    ...
    {
        int32_t size = int_size_in_bytes(type);
        rtx tmp;
       ...
       instead.  This is the case for Chill variable-sized strings.  */
        if (size == -1 && TREE_CODE(type) == ARRAY_TYPE && TYPE_ARRAY_MAX_SIZE(type) != NULL_TREE
...
static struct temp_slot *find_temp_slot_from_address(rtx x)
{
    ...
    for (p = temp_slots; p; p = p->next)
    {
        ...
        else if (XEXP(p->slot, 0) == x || p->address == x
            || (GET_CODE(x) == PLUS && XEXP(x, 0) == virtual_stack_vars_rtx
                   && GET_CODE(XEXP(x, 1)) == CONST_INT && INTVAL(XEXP(x, 1)) >= p->base_offset
...
void update_temp_slot_address(rtx old_addr, rtx new_addr)
{
    struct temp_slot *p = find_temp_slot_from_address(old_addr);
    ...
    else
    {
        ...
        p->address = gen_rtx_EXPR_LIST(VOIDmode, new_addr, p->address);
    }
}
...
void free_temp_slots(void)
{
    ...
    for (p = temp_slots; p; p = p->next)
        if (p->in_use && p->level == temp_slot_level && !p->keep && p->rtl_expr == 0)
            p->in_use = 0;
...
void put_var_into_stack(tree decl)
{
    ...
    decl_mode = (TREE_CODE(decl) == SAVE_EXPR ? TYPE_MODE(TREE_TYPE(decl)) : DECL_MODE(decl));
    ...
    if (DECL_NONLOCAL(decl) && GET_CODE(reg) == MEM && GET_CODE(XEXP(reg, 0)) == REG
    ...
    /* If we can't use ADDRESSOF, make sure we see through one we already
       generated.  */
    if (!can_use_addressof && GET_CODE(reg) == MEM && GET_CODE(XEXP(reg, 0)) == ADDRESSOF)
    ...
    if (GET_CODE(reg) == REG)
    {
       ...
       in a register until we know we actually need the address.  */
        if (can_use_addressof)
            ...
            put_reg_into_stack(function, reg, TREE_TYPE(decl), promoted_mode, decl_mode,
                TREE_SIDE_EFFECTS(decl), 0, TREE_USED(decl) || DECL_INITIAL(decl) != 0);
    }
    else if (GET_CODE(reg) == CONCAT)
    {
       ...
       We do it so they end up consecutive.  */
        ...
        put_reg_into_stack(function, XEXP(reg, 1), part_type, part_mode, part_mode,
            TREE_SIDE_EFFECTS(decl), 0, TREE_USED(decl) || DECL_INITIAL(decl) != 0);
        put_reg_into_stack(function, XEXP(reg, 0), part_type, part_mode, part_mode,
            TREE_SIDE_EFFECTS(decl), 0, TREE_USED(decl) || DECL_INITIAL(decl) != 0);
#else
        put_reg_into_stack(function, XEXP(reg, 0), part_type, part_mode, part_mode,
            TREE_SIDE_EFFECTS(decl), 0, TREE_USED(decl) || DECL_INITIAL(decl) != 0);
...
static void put_reg_into_stack(struct function *function, rtx reg, tree type,
    enum machine_mode promoted_mode, enum machine_mode decl_mode, int volatile_p,
    int original_regno, int used_p)
{
    rtx new_reg = 0;
    ...
    if (function)
    {
        if (regno < function->max_parm_reg)
            new_reg = function->parm_reg_stack_loc[regno];
        if (new_reg == 0)
            new_reg = assign_outer_stack_local(decl_mode, GET_MODE_SIZE(decl_mode), 0, function);
    }
    else
    {
        if (regno < max_parm_reg)
            new_reg = parm_reg_stack_loc[regno];
        if (new_reg == 0)
            new_reg = assign_stack_local(decl_mode, GET_MODE_SIZE(decl_mode), 0);
    }

    PUT_MODE(reg, decl_mode);
    XEXP(reg, 0) = XEXP(new_reg, 0);
    /* `volatile' bit means one thing for MEMs, another entirely for REGs.  */
    ...
    /* If this is a memory ref that contains aggregate components,
       ...
       __builtin_va_alist.  */
    MEM_SET_IN_STRUCT_P(reg, AGGREGATE_TYPE_P(type) || MEM_IN_STRUCT_P(new_reg));
    ...
    if (used_p && function != 0)
    {
       ...
       on saveable obstack.  */
        temp = (struct var_refs_queue *)savealloc(sizeof(struct var_refs_queue));
        temp->modified = reg;
...
static void fixup_var_refs(rtx var, enum machine_mode promoted_mode, int unsignedp)
{
    tree pending;
    ...
    for (; stack; stack = stack->next)
    {
        ...
        fixup_var_refs_insns(var, promoted_mode, unsignedp, stack->first, stack->next != 0);
...
static struct fixup_replacement *find_fixup_replacement(
    struct fixup_replacement **replacements, rtx x)
{
    struct fixup_replacement *p;

    /* See if we have already replaced this.  */
    for (p = *replacements; p && p->old_rtx != x; p = p->next)
        ;
...
static void fixup_var_refs_insns(
    rtx var, enum machine_mode promoted_mode, int unsignedp, rtx insn, int toplevel)
{
    rtx call_dest = 0;
        ...
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i')
        {
                                     ...
                                     || XEXP(XEXP(PATTERN(insn), 0), 1) == var))))
            {
                if ((note = find_reg_note(insn, REG_LIBCALL, NULL_RTX)) != 0)
                    ...
                    remove_note(XEXP(note, 0), find_reg_note(XEXP(note, 0), REG_RETVAL, NULL_RTX));
            ...
            else if (toplevel && (set = single_set(insn)) != 0
                && SET_DEST(set) == var
                /* If this represents the result of an insn group,
                   don't delete the insn.  */
                && find_reg_note(insn, REG_RETVAL, NULL_RTX) == 0
                && (rtx_equal_p(SET_SRC(set), var)
                       || (GET_CODE(SET_SRC(set)) == REG && (prev = prev_nonnote_insn(insn)) != 0
            ...
            else
            {
                ...
                if (SMALL_REGISTER_CLASSES)
                {
                        ...
                        && reg_mentioned_p(call_dest, PATTERN(insn)))
                    {
                        ...
                        PATTERN(insn) = replace_rtx(PATTERN(insn), call_dest, temp);
                    }

                    if (GET_CODE(insn) == CALL_INSN && GET_CODE(PATTERN(insn)) == SET)
                        call_dest = SET_DEST(PATTERN(insn));
                    else if (GET_CODE(insn) == CALL_INSN && GET_CODE(PATTERN(insn)) == PARALLEL
               ...
               it here.  */
                ...
                fixup_var_refs_1(var, promoted_mode, &PATTERN(insn), insn, &replacements);
               ...
               the last such instruction emitted.  */
                ...
                while (replacements)
                {
                    if (GET_CODE(replacements->new_rtx) == REG)
                    {
                        rtx insert_before;
                        rtx seq;

                        /* OLD might be a (subreg (mem)).  */
                        if (GET_CODE(replacements->old_rtx) == SUBREG)
                            replacements->old_rtx = fixup_memory_subreg(replacements->old_rtx, insn, 0);
                        else
                            replacements->old_rtx = fixup_stack_1(replacements->old_rtx, insn);
                       ...
                       eliminate much of the waste.  */
                        ...
                        if (GET_MODE(replacements->new_rtx) != GET_MODE(replacements->old_rtx))
                        {
                            start_sequence();
                            convert_move(replacements->new_rtx, replacements->old_rtx, unsignedp);
                            seq = gen_sequence();
                            end_sequence();
                        }
                        else
                            seq = gen_move_insn(replacements->new_rtx, replacements->old_rtx);
            ...
            for (note = REG_NOTES(insn); note; note = XEXP(note, 1))
                if (GET_CODE(note) != INSN_LIST)
                    XEXP(note, 0) = walk_fixup_memory_subreg(XEXP(note, 0), insn, 1);
        }
        insn = next;
    }
}
...
static void fixup_var_refs_1(rtx var, enum machine_mode promoted_mode, rtx *loc,
    rtx insn, struct fixup_replacement **replacements)
{
    int i;
    rtx x = *loc;
    ...
    case MEM:
        if (var == x)
        {
            ...
            if (replacement->new_rtx)
            {
                *loc = replacement->new_rtx;
                return;
            }

            *loc = replacement->new_rtx = x = fixup_stack_1(x, insn);

            /* Unless we are forcing memory to register or we changed the mode,
               we can leave things the way they are if the insn is valid.  */

            INSN_CODE(insn) = -1;
            if (!flag_force_mem && GET_MODE(x) == promoted_mode && recog_memoized(insn) >= 0)
                return;

            *loc = replacement->new_rtx = gen_reg_rtx(promoted_mode);
            return;
        }
       ...
       MATCH_DUPs.  */
        ...
        if (reg_mentioned_p(var, x))
        {
            ...
            if (replacement->new_rtx == 0)
                replacement->new_rtx = copy_most_rtx(x, var);

            *loc = x = replacement->new_rtx;
        }
        break;
    ...
    case ZERO_EXTRACT:
       ...
       by optimize_bit_field, and do not survive to get here.  */
        if (XEXP(x, 0) == var || (GET_CODE(XEXP(x, 0)) == SUBREG && SUBREG_REG(XEXP(x, 0)) == var))
        {
            ...
            {
                if (GET_MODE_BITSIZE(GET_MODE(tem)) > GET_MODE_BITSIZE(GET_MODE(var)))
                {
                    replacement = find_fixup_replacement(replacements, var);
                    if (replacement->new_rtx == 0)
                        replacement->new_rtx = gen_reg_rtx(GET_MODE(var));
                    SUBREG_REG(tem) = replacement->new_rtx;
                }
                else
            ...
            if (!flag_force_mem && GET_CODE(XEXP(x, 1)) == CONST_INT
                && GET_CODE(XEXP(x, 2)) == CONST_INT && !mode_dependent_address_p(XEXP(tem, 0))
                && !MEM_VOLATILE_P(tem))
            {
                enum machine_mode wanted_mode = VOIDmode;
                enum machine_mode is_mode = GET_MODE(tem);
                int32_t pos = INTVAL(XEXP(x, 2));
                ...
                if (wanted_mode != VOIDmode && GET_MODE_SIZE(wanted_mode) < GET_MODE_SIZE(is_mode))
                {
                    int32_t offset = pos / BITS_PER_UNIT;
                    rtx old_pos = XEXP(x, 2);
                    rtx newmem;

                    pos %= GET_MODE_BITSIZE(wanted_mode);

                    newmem = gen_rtx_MEM(wanted_mode, plus_constant(XEXP(tem, 0), offset));
    ...
    case SUBREG:
        if (SUBREG_REG(x) == var)
        {
            ...
            if (GET_MODE_SIZE(GET_MODE(x)) > GET_MODE_SIZE(GET_MODE(var)))
            {
                ...
                if (replacement->new_rtx == 0)
                    replacement->new_rtx = gen_reg_rtx(GET_MODE(var));
                SUBREG_REG(x) = replacement->new_rtx;
                return;
            }
            ...
            replacement->new_rtx = *loc = fixup_memory_subreg(x, insn, 0);

            INSN_CODE(insn) = -1;
            if (!flag_force_mem && recog_memoized(insn) >= 0)
                return;

            *loc = replacement->new_rtx = gen_reg_rtx(GET_MODE(x));
            return;
        }
        break;
    ...
    case SET:
        ...
        if (GET_CODE(SET_DEST(x)) == SIGN_EXTRACT || GET_CODE(SET_DEST(x)) == ZERO_EXTRACT)
            optimize_bit_field(x, insn, 0);
        if (GET_CODE(SET_SRC(x)) == SIGN_EXTRACT || GET_CODE(SET_SRC(x)) == ZERO_EXTRACT)
       ...
       into a register and then store it back out.  */
        if (GET_CODE(SET_DEST(x)) == ZERO_EXTRACT && GET_CODE(XEXP(SET_DEST(x), 0)) == SUBREG
            && SUBREG_REG(XEXP(SET_DEST(x), 0)) == var
            && (GET_MODE_SIZE(GET_MODE(XEXP(SET_DEST(x), 0))) > GET_MODE_SIZE(GET_MODE(var))))
        {
            replacement = find_fixup_replacement(replacements, var);
            if (replacement->new_rtx == 0)
                replacement->new_rtx = gen_reg_rtx(GET_MODE(var));

            SUBREG_REG(XEXP(SET_DEST(x), 0)) = replacement->new_rtx;
            emit_insn_after(gen_move_insn(var, replacement->new_rtx), insn);
        }
       ...
       insn into a pseudo and store the low part of the pseudo into VAR.  */
        if (GET_CODE(SET_DEST(x)) == SUBREG && SUBREG_REG(SET_DEST(x)) == var
            && (GET_MODE_SIZE(GET_MODE(SET_DEST(x))) > GET_MODE_SIZE(GET_MODE(var))))
        {
            SET_DEST(x) = tem = gen_reg_rtx(GET_MODE(SET_DEST(x)));
            emit_insn_after(gen_move_insn(var, gen_lowpart(GET_MODE(var), tem)), insn);
            break;
        }

        {
            ...
            while (GET_CODE(dest) == SUBREG || GET_CODE(dest) == STRICT_LOW_PART
                || GET_CODE(dest) == SIGN_EXTRACT || GET_CODE(dest) == ZERO_EXTRACT)
                dest = XEXP(dest, 0);
            ...
            if ((SET_SRC(x) == var
                    || (GET_CODE(SET_SRC(x)) == SUBREG && SUBREG_REG(SET_SRC(x)) == var))
                && (GET_CODE(SET_DEST(x)) == REG
                       || (GET_CODE(SET_DEST(x)) == SUBREG
                              && GET_CODE(SUBREG_REG(SET_DEST(x))) == REG))
                && GET_MODE(var) == promoted_mode && x == single_set(insn))
            {
                rtx pat;

                replacement = find_fixup_replacement(replacements, SET_SRC(x));
                if (replacement->new_rtx)
                    SET_SRC(x) = replacement->new_rtx;
                else if (GET_CODE(SET_SRC(x)) == SUBREG)
                    SET_SRC(x) = replacement->new_rtx = fixup_memory_subreg(SET_SRC(x), insn, 0);
                else
                    SET_SRC(x) = replacement->new_rtx = fixup_stack_1(SET_SRC(x), insn);
            ...
            if ((SET_DEST(x) == var
                    || (GET_CODE(SET_DEST(x)) == SUBREG && SUBREG_REG(SET_DEST(x)) == var))
            ...
            if (dest == var)
            {
                ...
                emit_insn_after(
                    gen_move_insn(fixeddest, gen_lowpart(GET_MODE(fixeddest), temp)), insn);
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        ...
        if (fmt[i] == 'E')
        {
            ...
            for (j = 0; j < XVECLEN(x, i); j++)
                fixup_var_refs_1(var, promoted_mode, &XVECEXP(x, i, j), insn, replacements);
        }
    }
}
...
static rtx fixup_memory_subreg(rtx x, rtx insn, int uncritical)
{
    ...
    if (GET_MODE_SIZE(mode) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(x))) && !uncritical)
        abort();
...
static rtx walk_fixup_memory_subreg(rtx x, rtx insn, int uncritical)
{
    enum rtx_code code;
    const char *fmt;
    int i;
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        ...
        if (fmt[i] == 'E')
        {
            int j;
            for (j = 0; j < XVECLEN(x, i); j++)
                XVECEXP(x, i, j) = walk_fixup_memory_subreg(XVECEXP(x, i, j), insn, uncritical);
        }
    }
    return x;
}
...
static rtx fixup_stack_1(rtx x, rtx insn)
{
    ...
    if (code == MEM)
    {
       ...
       (displacement is too large), compute the sum in a register.  */
        if (GET_CODE(ad) == PLUS && GET_CODE(XEXP(ad, 0)) == REG
...
static void optimize_bit_field(rtx body, rtx insn, rtx *equiv_mem)
{
    ...
    if (GET_CODE(SET_DEST(body)) == SIGN_EXTRACT || GET_CODE(SET_DEST(body)) == ZERO_EXTRACT)
    ...
    if (GET_CODE(XEXP(bitfield, 1)) == CONST_INT && GET_CODE(XEXP(bitfield, 2)) == CONST_INT
        && ((mode = mode_for_size(INTVAL(XEXP(bitfield, 1)), MODE_INT, 1)) != BLKmode)
        && INTVAL(XEXP(bitfield, 2)) % INTVAL(XEXP(bitfield, 1)) == 0)
    {
       ...
       and that it is safe to change the machine mode.  */
        ...
        else if (GET_CODE(XEXP(bitfield, 0)) == REG && equiv_mem != 0)
        ...
        else if (GET_CODE(XEXP(bitfield, 0)) == SUBREG && equiv_mem != 0
            && GET_CODE(SUBREG_REG(XEXP(bitfield, 0))) == REG)
            memref = equiv_mem[REGNO(SUBREG_REG(XEXP(bitfield, 0)))];

        if (memref && !mode_dependent_address_p(XEXP(memref, 0)) && !MEM_VOLATILE_P(memref))
        {
            ...
            int32_t offset = INTVAL(XEXP(bitfield, 2));
            rtx insns;
            ...
            start_sequence();
            memref = change_address(memref, mode, plus_constant(XEXP(memref, 0), offset));
            insns = get_insns();
            end_sequence();
            ...
            if (destflag)
            {
                ...
                if (!CONSTANT_ADDRESS_P(SET_SRC(body)))
                {
                    ...
                    while (GET_CODE(src) == SUBREG && SUBREG_WORD(src) == 0)
                        src = SUBREG_REG(src);
            ...
            else
            {
                ...
                while (GET_CODE(dest) == SUBREG && SUBREG_WORD(dest) == 0
                ...
                else
                {
                    ...
                    start_sequence();
                    convert_move(newreg, memref, GET_CODE(SET_SRC(body)) == ZERO_EXTRACT);
                    seq = get_insns();
                    end_sequence();
...
#define STACK_DYNAMIC_OFFSET(FNDECL)                                                               \
    (current_function_outgoing_args_size + REG_PARM_STACK_SPACE(FNDECL) + (STACK_POINTER_OFFSET))

#else
#define STACK_DYNAMIC_OFFSET(FNDECL) (current_function_outgoing_args_size + (STACK_POINTER_OFFSET))
#endif
...
rtx gen_mem_addressof(rtx reg, tree decl)
{
...
static void put_addressof_into_stack(rtx r)
{
    ...
    put_reg_into_stack(0, reg, TREE_TYPE(decl), GET_MODE(reg), DECL_MODE(decl),
        TREE_SIDE_EFFECTS(decl), ADDRESSOF_REGNO(r), TREE_USED(decl) || DECL_INITIAL(decl) != 0);
}
...
static void purge_addressof_1(rtx *loc, rtx insn, int force, int store)
{
    rtx x;
    RTX_CODE code;
    int i, j;
...
restart:
    ...
    if (code == ADDRESSOF && GET_CODE(XEXP(x, 0)) == MEM)
    {
       ...
       overwriting a REG rtx which is always shared.  */
        ...
        start_sequence();
        if (!validate_change(insn, loc, force_operand(sub, NULL_RTX), 0))
            abort();
    ...
    else if (code == MEM && GET_CODE(XEXP(x, 0)) == ADDRESSOF && !force)
    {
        ...
        if (GET_CODE(sub) == REG && (MEM_VOLATILE_P(x) || GET_MODE(x) == BLKmode))
        {
        ...
        else if (GET_CODE(sub) == REG && GET_MODE(x) != GET_MODE(sub))
        {
            ...
            if (!insn)
            {
               ...
               was replaced by.  */
                ...
                {
                    rtx y = XEXP(tem, 0);
                    if (GET_CODE(y) == MEM && rtx_equal_p(XEXP(x, 0), XEXP(y, 0)))
                    {
                       ...
                       This is especially true of REG_RETVAL.  */
                        ...
                        if (GET_MODE(x) != GET_MODE(y))
                        {
                            ...
                            if (GET_MODE_SIZE(GET_MODE(x)) > UNITS_PER_WORD
                                && (GET_MODE_SIZE(GET_MODE(x)) > GET_MODE_SIZE(GET_MODE(z))))
                            {
            ...
            if (size_x <= size_sub && int_mode_for_mode(GET_MODE(sub)) != BLKmode)
            {
               ...
               in memory.  */
                ...
                if (store)
                {
                    ...
                    start_sequence();
                    store_bit_field(sub, size_x, 0, GET_MODE(x), val, GET_MODE_SIZE(GET_MODE(sub)),
                ...
                {
                    start_sequence();
                    val = extract_bit_field(sub, size_x, 0, 1, NULL_RTX, GET_MODE(x), GET_MODE(x),
               ...
               on the REG_NOTES.  */
                purge_addressof_replacements = gen_rtx_EXPR_LIST(
                    VOIDmode, x, gen_rtx_EXPR_LIST(VOIDmode, val, purge_addressof_replacements));
        ...
        else if (validate_change(insn, loc, sub, 0))
        {
            /* Remember the replacement so that the same one can be done
               on the REG_NOTES.  */
            purge_addressof_replacements = gen_rtx_EXPR_LIST(
                VOIDmode, x, gen_rtx_EXPR_LIST(VOIDmode, sub, purge_addressof_replacements));
            goto restart;
        }
    give_up:;
...
void purge_addressof(rtx insns)
{
    ...
    for (insn = insns; insn; insn = NEXT_INSN(insn))
        ...
        {
            purge_addressof_1(&PATTERN(insn), insn, asm_noperands(PATTERN(insn)) > 0, 0);
...
static void instantiate_decls(tree fndecl, int valid_only)
{
    ...
    {
        int32_t size = int_size_in_bytes(TREE_TYPE(decl));
...
static void instantiate_decls_1(tree let, int valid_only)
{
    ...
    for (t = BLOCK_VARS(let); t; t = TREE_CHAIN(t))
        instantiate_decl(DECL_RTL(t), int_size_in_bytes(TREE_TYPE(t)), valid_only);
...
static void instantiate_decl(rtx x, int size, int valid_only)
{
    enum machine_mode mode;
    rtx addr;
    ...
    addr = XEXP(x, 0);
    if (CONSTANT_P(addr) || (GET_CODE(addr) == ADDRESSOF && GET_CODE(XEXP(addr, 0)) == REG)
        || (GET_CODE(addr) == REG
               && (REGNO(addr) < FIRST_VIRTUAL_REGISTER || REGNO(addr) > LAST_VIRTUAL_REGISTER)))
        return;
...
restart:
    ...
    case SET:
       ...
       of non-local gotos.  */
        if (SET_DEST(x) == virtual_incoming_args_rtx)
            new_reg = arg_pointer_rtx, offset = -in_arg_offset;
        else if (SET_DEST(x) == virtual_stack_vars_rtx)
            new_reg = frame_pointer_rtx, offset = -var_offset;
        else if (SET_DEST(x) == virtual_stack_dynamic_rtx)
            new_reg = stack_pointer_rtx, offset = -dynamic_offset;
        else if (SET_DEST(x) == virtual_outgoing_args_rtx)
            new_reg = stack_pointer_rtx, offset = -out_arg_offset;
        else if (SET_DEST(x) == virtual_cfa_rtx)
            new_reg = arg_pointer_rtx, offset = -cfa_offset;

        if (new_reg)
        {
            /* The only valid sources here are PLUS or REG.  Just do
               the simplest possible thing to handle them.  */
            if (GET_CODE(SET_SRC(x)) != REG && GET_CODE(SET_SRC(x)) != PLUS)
                abort();
            ...
            if (!validate_change(object, &SET_SRC(x), temp, 0) || !extra_insns)
                abort();
    ...
    case PLUS:
        ...
        if (CONSTANT_P(XEXP(x, 1)))
        {
            ...
            if (GET_CODE(XEXP(x, 0)) == PLUS)
            {
                ...
                instantiate_virtual_regs_1(&XEXP(XEXP(x, 0), 1), object, extra_insns);
                new_reg = gen_rtx_PLUS(Pmode, new_reg, XEXP(XEXP(x, 0), 1));
            }
            ...
            /* If the new constant is zero, try to replace the sum with just
               the register.  */
            if (new_offset == const0_rtx && validate_change(object, loc, new_reg, 0))
                return 1;
            ...
            old = XEXP(x, 0);
            if (offset == 0
                    ? !validate_change(object, &XEXP(x, 0), new_reg, 0)
                    : (XEXP(x, 0) = new_reg, !validate_change(object, &XEXP(x, 1), new_offset, 0)))
            {
                if (!extra_insns)
                {
                    XEXP(x, 0) = old;
               ...
               constant with that register.  */
                ...
                else
                {
                    ...
                    XEXP(x, 0) = old;
                    new_reg = gen_rtx_PLUS(Pmode, new_reg, new_offset);

                    start_sequence();
                    temp = force_operand(new_reg, NULL_RTX);
                    seq = get_insns();
                    end_sequence();
    ...
    case MEM:
       ...
       of a shared rtx.  */
        ...
        temp = XEXP(x, 0);
        if (CONSTANT_ADDRESS_P(temp) || temp == arg_pointer_rtx || temp == frame_pointer_rtx)
            return 1;

        if (GET_CODE(temp) == PLUS && CONSTANT_ADDRESS_P(XEXP(temp, 1))
            && (XEXP(temp, 0) == frame_pointer_rtx || XEXP(temp, 0) == arg_pointer_rtx))
            return 1;

        if (temp == virtual_stack_vars_rtx || temp == virtual_incoming_args_rtx
            || (GET_CODE(temp) == PLUS && CONSTANT_ADDRESS_P(XEXP(temp, 1))
                          ...
                          || XEXP(temp, 0) == virtual_incoming_args_rtx)))
        {
            ...
            if (instantiate_virtual_regs_1(&XEXP(x, 0), object ? object : x, 0))
                return 1;
    ...
    case CLOBBER:
       ...
       just make the recursive call, since there's no chance of a problem. */
        ...
        if ((GET_CODE(XEXP(x, 0)) == MEM
                && instantiate_virtual_regs_1(&XEXP(XEXP(x, 0), 0), XEXP(x, 0), 0))
            || (GET_CODE(XEXP(x, 0)) == REG && instantiate_virtual_regs_1(&XEXP(x, 0), object, 0)))
            return 1;
    ...
    case REG:
       ...
       in front of this insn and substitute the temporary.  */
        ...
        if (new_reg)
        {
            temp = plus_constant(new_reg, offset);
            if (!validate_change(object, loc, temp, 0))
            {
                if (!extra_insns)
                    return 0;
    ...
    for (i = 0; i < GET_RTX_LENGTH(code); i++, fmt++)
        if (*fmt == 'e')
        {
            ...
            for (j = 0; j < XVECLEN(x, i); j++)
                if (!instantiate_virtual_regs_1(&XVECEXP(x, i, j), object, extra_insns))
                    return 0;
       ...
       if nothing local refers to them.  */
        if (GET_CODE(insn) == CODE_LABEL)
        {
            ...
            /* Remove it from the nonlocal_label list, to avoid confusing
               flow.  */
            for (t = nonlocal_labels, last_t = 0; t; last_t = t, t = TREE_CHAIN(t))
        ...
        if (GET_CODE(insn) == INSN)
        {
            ...
            if (can_delete
                || (nonlocal_goto_stack_level != 0
                       && reg_mentioned_p(nonlocal_goto_stack_level, PATTERN(insn))))
                delete_insn(insn);
        }
    }
}
...
rtx nonlocal_label_rtx_list(void)
{
    tree t;
    rtx x = 0;
...
void use_variable(rtx rtl)
{
    ...
    else if (GET_CODE(rtl) == MEM && GET_CODE(XEXP(rtl, 0)) == REG
...
void use_variable_after(rtx rtl, rtx insn)
{
    if (GET_CODE(rtl) == REG)
        /* This is a register variable.  */
...
void assign_parms(tree fndecl, int second_time)
{
    tree parm;
    rtx entry_parm = NULL;
    rtx stack_parm = NULL;
    ...
    int hide_last_arg = (current_function_varargs && fnargs && (parm = tree_last(fnargs)) != 0
        && DECL_NAME(parm) && (!strcmp(IDENTIFIER_POINTER(DECL_NAME(parm)), "__builtin_va_alist")));
    ...
    if (aggregate_value_p(DECL_RESULT(fndecl)) && !current_function_returns_pcc_struct
    ...
    for (parm = fnargs; parm; parm = TREE_CHAIN(parm))
    {
       ...
       anonymous args.  */
        int last_named = ((TREE_CHAIN(parm) == 0 || DECL_NAME(TREE_CHAIN(parm)) == 0)
       ...
       the last named arg as if it were anonymous too.  */
        ...
        if (TREE_TYPE(parm) == error_mark_node
            ...
            || TREE_CODE(parm) != PARM_DECL || passed_type == NULL)
        {
            DECL_INCOMING_RTL(parm) = DECL_RTL(parm) = gen_rtx_MEM(BLKmode, const0_rtx);
            TREE_USED(parm) = 1;
            continue;
        }
       ...
       verified that the modes are the same.  */
        if (DECL_TRANSPARENT_UNION(parm) || TYPE_TRANSPARENT_UNION(passed_type))
       ...
       0 means it arrives on the stack.  */
        ...
        entry_parm = FUNCTION_INCOMING_ARG(args_so_far, promoted_mode, passed_type, named_arg);
#else
       ...
       Also, indicate when RTL generation is to be suppressed.  */
        if (last_named && !varargs_setup)
        {
            SETUP_INCOMING_VARARGS(args_so_far, promoted_mode, passed_type,
                current_function_pretend_args_size, second_time);
            varargs_setup = 1;
        }
#endif
       ...
       0 as it was the previous time.  */
        ...
        locate_and_pad_parm(promoted_mode, passed_type,
            ...
            FUNCTION_INCOMING_ARG(
                args_so_far, promoted_mode, passed_type, (named_arg || varargs_setup))
                != 0,
#else
            FUNCTION_ARG(args_so_far, promoted_mode, passed_type, named_arg || varargs_setup) != 0,
#endif
#endif
        ...
        if (!second_time)
        {
            ...
            if (offset_rtx == const0_rtx)
                ...
                stack_parm = gen_rtx_MEM(
                    promoted_mode, gen_rtx_PLUS(Pmode, internal_arg_pointer, offset_rtx));
       ...
       but for now that's not worth bothering with.  */
        ...
        {
            int nregs
                = FUNCTION_ARG_PARTIAL_NREGS(args_so_far, promoted_mode, passed_type, named_arg);
                ...
                if (!second_time)
                {
                    ...
                    if (GET_CODE(entry_parm) == PARALLEL)
                        emit_group_store(validize_mem(stack_parm), entry_parm,
                            ...
                            (TYPE_ALIGN(TREE_TYPE(parm)) / BITS_PER_UNIT));
                    else
                        move_block_from_reg(REGNO(entry_parm), validize_mem(stack_parm), nregs,
                            int_size_in_bytes(TREE_TYPE(parm)));
                }
       ...
       We'll make another stack slot, if we need one.  */
        {
            int thisparm_boundary = FUNCTION_ARG_BOUNDARY(promoted_mode, passed_type);
       ...
       don't store it back in that same slot.  */
        if (entry_parm != 0 && nominal_mode != BLKmode && nominal_mode != passed_mode)
            stack_parm = 0;
       ...
       Set DECL_RTL to that place.  */
        ...
        if (nominal_mode == BLKmode || GET_CODE(entry_parm) == PARALLEL)
        {
            /* If a BLKmode arrives in registers, copy it to a stack slot.
               Handle calls that pass values in multiple non-contiguous
               locations.  The Irix 6 ABI has examples of this.  */
            if (GET_CODE(entry_parm) == REG || GET_CODE(entry_parm) == PARALLEL)
            {
                int size_stored = CEIL_ROUND(int_size_in_bytes(TREE_TYPE(parm)), UNITS_PER_WORD);
               ...
               if it becomes a problem.  */
                ...
                if (stack_parm == 0)
                {
                    stack_parm = assign_stack_local(GET_MODE(entry_parm), size_stored, 0);
        ...
        else if (!((obey_regdecls && !DECL_REGISTER(parm) && !DECL_INLINE(fndecl))
                     /* layout_decl may set this.  */
                     || TREE_ADDRESSABLE(parm)
                     || TREE_SIDE_EFFECTS(parm)
                     /* If -ffloat-store specified, don't put explicit
                        float variables into registers.  */
                     || (flag_float_store && TREE_CODE(TREE_TYPE(parm)) == REAL_TYPE))
            ...
            || passed_pointer || parm == function_result_decl)
        {
            ...
            promoted_nominal_mode = promote_mode(TREE_TYPE(parm), nominal_mode, &unsignedp, 0);
            ...
            if (passed_pointer)
            {
                DECL_RTL(parm) = gen_rtx_MEM(TYPE_MODE(TREE_TYPE(passed_type)), parmreg);
            ...
            if (nominal_mode != passed_mode || promoted_nominal_mode != promoted_mode)
            {
                int save_tree_used;
               ...
               parameters have been moved.  */
                ...
                expand_assignment(parm, make_tree(nominal_type, tempreg), 0, 0);
            ...
            /* CYGNUS LOCAL -- FUNCTION_ARG_KEEP_AS_REFERENCE/meissner */
                ...
                && !FUNCTION_ARG_KEEP_AS_REFERENCE(
                       args_so_far, passed_mode, passed_type, !last_named)
#endif
                /* END CYGNUS LOCAL */
                && !((obey_regdecls && !DECL_REGISTER(parm) && !DECL_INLINE(fndecl))
                       /* layout_decl may set this.  */
                       || TREE_ADDRESSABLE(parm)
                       || TREE_SIDE_EFFECTS(parm)
                       /* If -ffloat-store specified, don't put explicit
            ...
            /* ??? Later add code to handle the case that if the argument isn't
               modified, don't do the copy.  */
                ...
                && FUNCTION_ARG_CALLEE_COPIES(
                       args_so_far, TYPE_MODE(DECL_ARG_TYPE(parm)), DECL_ARG_TYPE(parm), named_arg)
               ...
               registers that haven't been copied to pseudos yet.  */
                ...
                if (TYPE_SIZE(type) == 0 || TREE_CODE(TYPE_SIZE(type)) != INTEGER_CST)
                    ...
                    copy = gen_rtx_MEM(BLKmode,
                        allocate_dynamic_stack_space(expr_size(parm), NULL_RTX, TYPE_ALIGN(type)));
                else
                    copy = assign_stack_temp(TYPE_MODE(type), int_size_in_bytes(type), 1);
                ...
                if (current_function_check_memory_usage)
                    emit_library_call(chkr_set_right_libfunc, 1, VOIDmode, 3, XEXP(copy, 0),
                        ptr_mode, GEN_INT(int_size_in_bytes(type)), TYPE_MODE(sizetype),
                        GEN_INT(MEMORY_USE_RW), TYPE_MODE(integer_type_node));
                conversion_insns = get_insns();
                did_conversion = 1;
                end_sequence();
            }
            ...
            if (regno >= max_parm_reg)
            {
               ...
               precisely correct.  */
                ...
                new_regs = (rtx *)savealloc(max_parm_reg * sizeof(rtx));
                copy_memory(
                    (char *)parm_reg_stack_loc, (char *)new_regs, old_max_parm_reg * sizeof(rtx));
                zero_memory((char *)(new_regs + old_max_parm_reg),
                    (max_parm_reg - old_max_parm_reg) * sizeof(rtx));
                parm_reg_stack_loc = new_regs;
            }
            ...
            if (GET_CODE(parmreg) == CONCAT)
            {
                ...
                if (stack_parm != 0)
                {
                    parm_reg_stack_loc[regnor] = gen_realpart(submode, stack_parm);
                    parm_reg_stack_loc[regnoi] = gen_imagpart(submode, stack_parm);
                }
                else
                {
            ...
            if (nominal_mode == passed_mode && !did_conversion && stack_parm != 0
                && GET_CODE(stack_parm) == MEM && stack_offset.var == 0
                && reg_mentioned_p(virtual_incoming_args_rtx, XEXP(stack_parm, 0)))
            {
                ...
                if (GET_CODE(parmreg) == CONCAT)
                    ...
                    for (sinsn = linsn; sinsn != 0; sinsn = prev_nonnote_insn(sinsn))
                    {
                        ...
                        if (set != 0 && SET_DEST(set) == regno_reg_rtx[regnoi])
                            REG_NOTES(sinsn) = gen_rtx_EXPR_LIST(
                                REG_EQUIV, parm_reg_stack_loc[regnoi], REG_NOTES(sinsn));
                        else if (set != 0 && SET_DEST(set) == regno_reg_rtx[regnor])
                            REG_NOTES(sinsn) = gen_rtx_EXPR_LIST(
                                REG_EQUIV, parm_reg_stack_loc[regnor], REG_NOTES(sinsn));
                    }
                else if ((set = single_set(linsn)) != 0 && SET_DEST(set) == parmreg)
                    REG_NOTES(linsn) = gen_rtx_EXPR_LIST(REG_EQUIV, stack_parm, REG_NOTES(linsn));
            }

            /* For pointer data type, suggest pointer register.  */
            if (POINTER_TYPE_P(TREE_TYPE(parm)))
                mark_reg_pointer(parmreg, (TYPE_ALIGN(TREE_TYPE(TREE_TYPE(parm))) / BITS_PER_UNIT));
        }
        else
        {
            ...
            if (promoted_mode != nominal_mode)
            {
                ...
                push_to_sequence(conversion_insns);
                entry_parm = convert_to_mode(nominal_mode, tempreg, TREE_UNSIGNED(TREE_TYPE(parm)));
                if (stack_parm)
                {
                    /* ??? This may need a big-endian conversion on sparc64.  */
                    stack_parm = change_address(stack_parm, nominal_mode, NULL_RTX);
                }
                conversion_insns = get_insns();
                did_conversion = 1;
                end_sequence();
            ...
            if (entry_parm != stack_parm)
            {
                if (stack_parm == 0)
                {
                    stack_parm = assign_stack_local(
                        GET_MODE(entry_parm), GET_MODE_SIZE(GET_MODE(entry_parm)), 0);
                    /* If this is a memory ref that contains aggregate components,
                ...
                {
                    push_to_sequence(conversion_insns);
                    emit_move_insn(validize_mem(stack_parm), validize_mem(entry_parm));
                    conversion_insns = get_insns();
                    end_sequence();
                }
                else
            ...
            {
                push_to_sequence(conversion_insns);
                emit_library_call(chkr_set_right_libfunc, 1, VOIDmode, 3, XEXP(stack_parm, 0),
                    ptr_mode, GEN_INT(GET_MODE_SIZE(GET_MODE(entry_parm))), TYPE_MODE(sizetype),
                    GEN_INT(MEMORY_USE_RW), TYPE_MODE(integer_type_node));
       ...
       function's incoming structure pointer, set up the result.  */
        if (parm == function_result_decl)
        {
            ...
            DECL_RTL(result) = gen_rtx_MEM(DECL_MODE(result), DECL_RTL(parm));

            MEM_SET_IN_STRUCT_P(DECL_RTL(result), AGGREGATE_TYPE_P(restype));
        }
    ...
    current_function_args_size = MAX(current_function_args_size, REG_PARM_STACK_SPACE(fndecl));
#endif
#endif
...
#define STACK_BYTES (PREFERRED_STACK_BOUNDARY / BITS_PER_UNIT)
    ...
    current_function_args_size
        = ((current_function_args_size + STACK_BYTES - 1) / STACK_BYTES) * STACK_BYTES;
#endif

#ifdef ARGS_GROW_DOWNWARD
    current_function_arg_offset_rtx = (stack_args_size.var == 0
            ? GEN_INT(-stack_args_size.constant)
            : expand_expr(
                  size_binop(MINUS_EXPR, stack_args_size.var, size_int(-stack_args_size.constant)),
    ...
    current_function_pops_args
        = RETURN_POPS_ARGS(fndecl, TREE_TYPE(fndecl), current_function_args_size);
...
rtx promoted_input_arg(int regno, enum machine_mode *pmode, int *punsignedp)
{
    tree arg;

    for (arg = DECL_ARGUMENTS(current_function_decl); arg; arg = TREE_CHAIN(arg))
        if (GET_CODE(DECL_INCOMING_RTL(arg)) == REG && REGNO(DECL_INCOMING_RTL(arg)) == regno
            && TYPE_MODE(DECL_ARG_TYPE(arg)) == TYPE_MODE(TREE_TYPE(arg)))
        {
            ...
            if (mode == GET_MODE(DECL_INCOMING_RTL(arg)) && mode != DECL_MODE(arg))
            {
                *pmode = DECL_MODE(arg);
...
void locate_and_pad_parm(enum machine_mode passed_mode, tree type, int in_regs, tree fndecl,
    struct args_size *initial_offset_ptr, struct args_size *offset_ptr,
    struct args_size *arg_size_ptr)
{
    tree sizetree = type ? size_in_bytes(type) : size_int(GET_MODE_SIZE(passed_mode));
    ...
    if (!in_regs)
    {
        ...
        if (reg_parm_stack_space > 0)
        {
            ...
            {
                initial_offset_ptr->var = size_binop(
                    MAX_EXPR, ARGS_SIZE_TREE(*initial_offset_ptr), size_int(reg_parm_stack_space));
    ...
    if (initial_offset_ptr->var)
    {
        ...
        offset_ptr->var = size_binop(MINUS_EXPR, integer_zero_node, initial_offset_ptr->var);
    }
    else
    {
    ...
    if (initial_offset_ptr->var)
    {
        arg_size_ptr->var = size_binop(MINUS_EXPR,
            size_binop(MINUS_EXPR, integer_zero_node, initial_offset_ptr->var),
            offset_ptr->var);
    }
    else
    {
        arg_size_ptr->constant = (-initial_offset_ptr->constant - offset_ptr->constant);
    }
...
static void pad_to_arg_alignment(struct args_size *offset_ptr, int boundary)
{
    ...
    if (boundary > BITS_PER_UNIT)
    {
        if (offset_ptr->var)
        {
            offset_ptr->var =
                ...
                round_up
#endif
                (ARGS_SIZE_TREE(*offset_ptr), boundary / BITS_PER_UNIT);
            offset_ptr->constant = 0; /*?*/
        }
        else
            offset_ptr->constant =
...
static void pad_below(struct args_size *offset_ptr, enum machine_mode passed_mode, tree sizetree)
{
    if (passed_mode != BLKmode)
    {
        if (GET_MODE_BITSIZE(passed_mode) % PARM_BOUNDARY)
            offset_ptr->constant += (((GET_MODE_BITSIZE(passed_mode) + PARM_BOUNDARY - 1)
...
#ifdef ARGS_GROW_DOWNWARD
static tree round_down(tree value, int divisor)
{
    return size_binop(
        MULT_EXPR, size_binop(FLOOR_DIV_EXPR, value, size_int(divisor)), size_int(divisor));
}
#endif
...
void uninitialized_vars_warning(tree block)
{
    ...
    for (decl = BLOCK_VARS(block); decl; decl = TREE_CHAIN(decl))
    {
        if (TREE_CODE(decl) == VAR_DECL
            ...
            && !AGGREGATE_TYPE_P(TREE_TYPE(decl)) && DECL_RTL(decl) != 0
            ...
            && DECL_INITIAL(decl) == NULL_TREE && regno_uninitialized(REGNO(DECL_RTL(decl))))
            warning_with_decl(decl, "`%s' might be used uninitialized in this function");
        if (TREE_CODE(decl) == VAR_DECL && DECL_RTL(decl) != 0 && GET_CODE(DECL_RTL(decl)) == REG
            && regno_clobbered_at_setjmp(REGNO(DECL_RTL(decl))))
            warning_with_decl(decl, "variable `%s' might be clobbered by `longjmp' or `vfork'");
    }
...
{
    tree decl;
    for (decl = DECL_ARGUMENTS(current_function_decl); decl; decl = TREE_CHAIN(decl))
        if (DECL_RTL(decl) != 0 && GET_CODE(DECL_RTL(decl)) == REG
            && regno_clobbered_at_setjmp(REGNO(DECL_RTL(decl))))
...
void setjmp_protect(tree block)
{
    tree decl, sub;
    for (decl = BLOCK_VARS(block); decl; decl = TREE_CHAIN(decl))
        if ((TREE_CODE(decl) == VAR_DECL || TREE_CODE(decl) == PARM_DECL) && DECL_RTL(decl) != 0
            && (GET_CODE(DECL_RTL(decl)) == REG
...
rtx lookup_static_chain(tree decl)
{
    ...
    if (context == 0 || (TREE_CODE(decl) == FUNCTION_DECL && DECL_NO_STATIC_CHAIN(decl)))
        return 0;
...
rtx trampoline_address(tree function)
{
    ...
    for (link = trampoline_list; link; link = TREE_CHAIN(link))
        if (TREE_PURPOSE(link) == function)
            return round_trampoline_addr(XEXP(RTL_EXPR_RTL(TREE_VALUE(link)), 0));

    for (fp = outer_function_chain; fp; fp = fp->next)
        for (link = fp->trampoline_list; link; link = TREE_CHAIN(link))
            if (TREE_PURPOSE(link) == function)
            {
                tramp = fix_lexical_addr(XEXP(RTL_EXPR_RTL(TREE_VALUE(link)), 0), function);
    ...
    if (fn_context != current_function_decl && fn_context != inline_function_decl)
        for (fp = outer_function_chain; fp; fp = fp->next)
...
#define TRAMPOLINE_REAL_SIZE (TRAMPOLINE_SIZE)
    ...
    if (fp != 0)
    {
        push_obstacks(fp->function_maybepermanent_obstack, fp->function_maybepermanent_obstack);
...
static rtx round_trampoline_addr(rtx tramp)
{
    ...
    temp = expand_binop(Pmode, add_optab, tramp, GEN_INT(TRAMPOLINE_ALIGNMENT / BITS_PER_UNIT - 1),
        temp, 0, OPTAB_LIB_WIDEN);
    tramp = expand_binop(Pmode, and_optab, temp, GEN_INT(-TRAMPOLINE_ALIGNMENT / BITS_PER_UNIT),
        temp, 0, OPTAB_LIB_WIDEN);
#endif
    return tramp;
}
...
tree *identify_blocks(tree block, rtx insns)
{
    int n_blocks;
    tree *block_vector;
    int *block_stack;
...
tree reorder_blocks(tree *block_vector, tree block, rtx insns)
{
    ...
    for (insn = insns; insn; insn = NEXT_INSN(insn))
    {
        ...
        if (GET_CODE(insn) == NOTE)
            ...
            case NOTE_INSN_BLOCK_END:
                BLOCK_SUBBLOCKS(current_block) = blocks_nreverse(BLOCK_SUBBLOCKS(current_block));
...
static int all_blocks(tree block, tree *vector)
{
    ...
    while (block)
    {
        ...
        n_blocks += all_blocks(BLOCK_SUBBLOCKS(block), vector ? vector + n_blocks : 0);
...
void init_function_start(tree subr, const char *filename, int line)
{
    ...
    current_function_needs_context = (decl_function_context(current_function_decl) != 0
    ...
    if (warn_aggregate_return && AGGREGATE_TYPE_P(TREE_TYPE(DECL_RESULT(subr))))
        warning("function returns an aggregate");

    current_function_returns_pointer = POINTER_TYPE_P(TREE_TYPE(DECL_RESULT(subr)));
...
void expand_main_function(void)
{
    ...
    emit_library_call(gen_rtx_SYMBOL_REF(Pmode, NAME__MAIN), 0, VOIDmode, 0);
...
void expand_function_start(tree subr, int parms_have_cleanups)
{
    ...
    current_function_check_memory_usage
        = (flag_check_memory_usage && !DECL_NO_CHECK_MEMORY_USAGE(current_function_decl));

    current_function_instrument_entry_exit
        = (flag_instrument_function_entry_exit && !DECL_NO_INSTRUMENT_FUNCTION_ENTRY_EXIT(subr));
    ...
    if (current_function_needs_context)
    {
       ...
       conflicts with regs used for parameters.  */
        if (!SMALL_REGISTER_CLASSES || GET_CODE(static_chain_incoming_rtx) == REG)
    ...
    if (aggregate_value_p(DECL_RESULT(subr)))
    {
        ...
        if (value_address)
        {
            DECL_RTL(DECL_RESULT(subr)) = gen_rtx_MEM(DECL_MODE(DECL_RESULT(subr)), value_address);
            MEM_SET_IN_STRUCT_P(
                DECL_RTL(DECL_RESULT(subr)), AGGREGATE_TYPE_P(TREE_TYPE(DECL_RESULT(subr))));
        }
    }
    ...
    /* Scalar, returned in a register.  */
    {
        ...
        DECL_RTL(DECL_RESULT(subr)) = FUNCTION_OUTGOING_VALUE(TREE_TYPE(DECL_RESULT(subr)), subr);
#else
        DECL_RTL(DECL_RESULT(subr)) = FUNCTION_VALUE(TREE_TYPE(DECL_RESULT(subr)), subr);
#endif
...
#endif
            last_ptr = copy_to_reg(gen_rtx_MEM(Pmode, memory_address(Pmode, last_ptr)));

            /* If we are not optimizing, ensure that we know that this
               piece of context is live over the entire function.  */
            if (!optimize)
                save_expr_regs = gen_rtx_EXPR_LIST(VOIDmode, last_ptr, save_expr_regs);
        }
    }
    ...
    if (current_function_instrument_entry_exit)
    {
        ...
        if (GET_CODE(fun) == MEM)
            ...
            abort();
        emit_library_call(profile_function_entry_libfunc, 0, VOIDmode, 2, fun, Pmode,
            expand_builtin_return_addr(BUILT_IN_RETURN_ADDRESS, 0, frame_pointer_rtx), Pmode);
    }
...
static void diddle_return_value(void (*doit)(rtx, void *), void *arg)
{
    ...
    pcc = (current_function_returns_struct || current_function_returns_pcc_struct);

    if ((GET_CODE(outgoing) == REG && REGNO(outgoing) >= FIRST_PSEUDO_REGISTER) || pcc)
    {
...
void expand_function_end(const char *filename, int line, int end_bindings)
{
    int i;
    tree link;
    ...
    if (arg_pointer_save_area)
    {
       ...
       have to check it and fix it if necessary.  */
        rtx seq;
        start_sequence();
        emit_move_insn(validize_mem(arg_pointer_save_area), virtual_incoming_args_rtx);
        seq = gen_sequence();
        end_sequence();
    ...
    for (link = trampoline_list; link; link = TREE_CHAIN(link))
    {
       ...
       initializing trampolines.  */
        if (initial_trampoline == 0)
        {
            ...
            initial_trampoline = gen_rtx_MEM(BLKmode, assemble_trampoline_template());
            resume_temporary_allocation();
        }
#endif
        ...
        emit_block_move(blktramp, initial_trampoline, GEN_INT(TRAMPOLINE_SIZE),
    ...
    if (flag_stack_check && !STACK_CHECK_BUILTIN)
    {
        ...
        for (insn = get_insns(); insn; insn = NEXT_INSN(insn))
            ...
            {
                start_sequence();
                probe_stack_range(STACK_CHECK_PROTECT, GEN_INT(STACK_CHECK_MAX_FRAME_SIZE));
                seq = get_insns();
                end_sequence();
                emit_insns_before(seq, tail_recursion_reentry);
                break;
    ...
    if (warn_unused && extra_warnings)
    {
        ...
        for (decl = DECL_ARGUMENTS(current_function_decl); decl; decl = TREE_CHAIN(decl))
            if (!TREE_USED(decl) && TREE_CODE(decl) == PARM_DECL && DECL_NAME(decl)
                && !DECL_ARTIFICIAL(decl))
                warning_with_decl(decl, "unused parameter `%s'");
    }

    /* Delete handlers for nonlocal gotos if nothing uses them.  */
    if (nonlocal_goto_handler_slots != 0 && !current_function_has_nonlocal_label)
        delete_handlers();
    ...
    if (current_function_instrument_entry_exit)
    {
        ...
        if (GET_CODE(fun) == MEM)
            ...
            abort();
        emit_library_call(profile_function_exit_libfunc, 0, VOIDmode, 2, fun, Pmode,
            expand_builtin_return_addr(BUILT_IN_RETURN_ADDRESS, 0, frame_pointer_rtx), Pmode);
    }
    ...
    if (DECL_RTL(DECL_RESULT(current_function_decl)) != 0
        ...
        && (REGNO(DECL_RTL(DECL_RESULT(current_function_decl))) >= FIRST_PSEUDO_REGISTER))
    {
        rtx real_decl_result;

#ifdef FUNCTION_OUTGOING_VALUE
        real_decl_result = FUNCTION_OUTGOING_VALUE(
            TREE_TYPE(DECL_RESULT(current_function_decl)), current_function_decl);
#else
        real_decl_result
       ...
       the mode computed in expand_return.  */
        if (GET_MODE(real_decl_result) == BLKmode)
            PUT_MODE(real_decl_result, GET_MODE(DECL_RTL(DECL_RESULT(current_function_decl))));
        emit_move_insn(real_decl_result, DECL_RTL(DECL_RESULT(current_function_decl)));
    ...
    if (current_function_returns_struct || current_function_returns_pcc_struct)
    {
        ...
        tree type = TREE_TYPE(DECL_RESULT(current_function_decl));
#ifdef FUNCTION_OUTGOING_VALUE
        rtx outgoing = FUNCTION_OUTGOING_VALUE(build_pointer_type(type), current_function_decl);
#else
        rtx outgoing = FUNCTION_VALUE(build_pointer_type(type), current_function_decl);
#endif
...
#if defined(HAVE_prologue) || defined(HAVE_epilogue)
static int *record_insns(rtx insns)
{
    int *vec;
...
void thread_prologue_and_epilogue_insns(rtx f ATTRIBUTE_UNUSED)
{
#ifdef HAVE_prologue
    ...
    if (HAVE_prologue)
    {
       ...
       if they form a basic block unto themselves.  */
        if (x_basic_block_head && n_basic_blocks && GET_CODE(BLOCK_HEAD(0)) != CODE_LABEL)
    ...
    if (HAVE_epilogue)
    {
        ...
        if (!(prev && GET_CODE(prev) == BARRIER))
        {
            ...
            while (prev && GET_CODE(prev) == INSN && GET_CODE(PATTERN(prev)) == USE)
            {
                tem = prev;
