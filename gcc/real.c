...
static void eadd1(uint16_t *, uint16_t *, uint16_t *);
static void ediv(uint16_t *, uint16_t *, uint16_t *);
static void emul(uint16_t *, uint16_t *, uint16_t *);
...
static void ltoe(int32_t *, uint16_t *);
static void ultoe(uint32_t *, uint16_t *);
static void eifrac(uint16_t *, int32_t *, uint16_t *);
static void euifrac(uint16_t *, uint32_t *, uint16_t *);
...
/* Round real toward zero to int32_t;
...
/* Round real toward zero to uint32_t
...
void ereal_from_int(double *d, int32_t i, int32_t j, enum machine_mode mode)
{
    ...
    eldexp(eone, 32, df);
    ultoe((uint32_t *)&high, dg);
    emul(dg, df, dg);
    ultoe((uint32_t *)&low, df);
    eadd(df, dg, dg);
    if (sign)
        eneg(dg);
...
void ereal_to_int(int32_t *low, int32_t *high, double rr)
{
    ...
    euifrac(dg, (uint32_t *)high, dh);
    emul(df, dh, dg); /* fractional part is the low word */
    euifrac(dg, (uint32_t *)low, dh);
    if (s)
    {
        /* complement and add 1 */
        *high = ~(*high);
...
/*
  ...
  ei[NI-1]	low guard word	(0x8000 bit is rounding place)
    ...
    eifrac (e, &l, frac)    e to int32_t and e type fraction
    euifrac (e, &l, frac)   e to uint32_t and e type fraction
    ...
    ltoe (&l, e)		int32_t to e type
    ultoe (&l, e)		uint32_t to e type
...
/* Find signed int32_t integer I and floating point fractional
...
/* Find uint32_t integer I and floating point fractional part
...
/* Convert a DFmode target `double' value to a double.
   ...
   The DFmode is stored as an array of int32_t in the target's
