...
static void find_reload_regs(struct insn_chain *chain, FILE *);
static void find_tworeg_group(struct insn_chain *, int, FILE *);
static void find_group(struct insn_chain *, int, FILE *);
static int possible_group_p(struct insn_chain *, int);
static void count_possible_groups(struct insn_chain *, int);
static int modes_equiv_for_class_p(enum machine_mode, enum machine_mode, enum reg_class);
static void delete_caller_save_insns(void);

static void spill_failure(rtx);
static void new_spill_reg(struct insn_chain *, int, int, int, FILE *);
...
static void mark_reload_reg_in_use(int, int, enum reload_type, enum machine_mode);
static void clear_reload_reg_in_use(int, int, enum reload_type, enum machine_mode);
...
static int allocate_reload_reg(struct insn_chain *, int, int, int);
...
void init_reload(void)
{
    ...
    {
        tem = gen_rtx_PLUS(Pmode, gen_rtx_REG(Pmode, FRAME_POINTER_REGNUM), gen_rtx_REG(Pmode, i));
...
{
    int regno;
    EXECUTE_IF_SET_IN_REG_SET(from, FIRST_PSEUDO_REGISTER, regno, {
...
int reload(rtx first, int global, FILE *dumpfile)
{
    ...
    pseudo_forbidden_regs = (HARD_REG_SET *)xmalloc(max_regno * sizeof(HARD_REG_SET));
    pseudo_previous_regs = (HARD_REG_SET *)xmalloc(max_regno * sizeof(HARD_REG_SET));
    ...
    for (insn = first; insn; insn = NEXT_INSN(insn))
    {
                ...
                if (i > LAST_VIRTUAL_REGISTER)
                {
                    ...
                    else if (function_invariant_p(x))
                    {
                        ...
                        else if (x == frame_pointer_rtx || x == arg_pointer_rtx)
                        {
                            ...
                            reg_equiv_memory_loc[i] = force_const_mem(GET_MODE(SET_DEST(set)), x);
                    }
                    else
                        continue;
                    ...
                    if (GET_CODE(x) != MEM || rtx_equal_p(SET_SRC(set), x))
                        reg_equiv_init[i] = gen_rtx_INSN_LIST(VOIDmode, insn, reg_equiv_init[i]);
                }
            }
        }
        ...
        else if (set && GET_CODE(SET_DEST(set)) == MEM && GET_CODE(SET_SRC(set)) == REG
            && reg_equiv_memory_loc[REGNO(SET_SRC(set))]
            && rtx_equal_p(SET_DEST(set), reg_equiv_memory_loc[REGNO(SET_SRC(set))]))
            reg_equiv_init[REGNO(SET_SRC(set))]
                = gen_rtx_INSN_LIST(VOIDmode, insn, reg_equiv_init[REGNO(SET_SRC(set))]);
    ...
    offsets_at = (int(*)[NUM_ELIMINABLE_REGS])(real_at_ptr - get_first_label_num());
    ...
    for (;;)
    {
        ...
        for (i = FIRST_PSEUDO_REGISTER; i < max_regno; i++)
            if (reg_renumber[i] < 0 && reg_equiv_memory_loc[i])
            {
                ...
                if (strict_memory_address_p(GET_MODE(regno_reg_rtx[i]), XEXP(x, 0)))
                    ...
                    || (GET_CODE(XEXP(x, 0)) == PLUS && GET_CODE(XEXP(XEXP(x, 0), 0)) == REG
                           && (REGNO(XEXP(XEXP(x, 0), 0)) < FIRST_PSEUDO_REGISTER)
        ...
        for (chain = insns_need_reload; chain != 0; chain = chain->next_need_reload)
    ...
    if (insns_need_reload != 0 || something_needs_elimination || something_needs_operands_changed)
    {
    ...
    if (!frame_pointer_needed)
        for (i = 0; i < n_basic_blocks; i++)
            CLEAR_REGNO_REG_SET(basic_block_live_at_start[i], FRAME_POINTER_REGNUM);
...
failed:
    ...
    for (insn = first; insn; insn = NEXT_INSN(insn))
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i')
        {
            ...
            if ((GET_CODE(PATTERN(insn)) == USE && find_reg_note(insn, REG_EQUAL, NULL_RTX))
            ...
            while (*pnote != 0)
            {
                if (REG_NOTE_KIND(*pnote) == REG_DEAD || REG_NOTE_KIND(*pnote) == REG_UNUSED
                    || REG_NOTE_KIND(*pnote) == REG_RETVAL || REG_NOTE_KIND(*pnote) == REG_LIBCALL)
                    *pnote = XEXP(*pnote, 1);
                else
                    pnote = &XEXP(*pnote, 1);
            }
    ...
    {
        int32_t size = get_frame_size() + STACK_CHECK_FIXED_FRAME_SIZE;
...
static void calculate_needs_all_insns(int global)
{
    ...
    for (pchain = &reload_insn_chain; *pchain != 0; pchain = &(*pchain)->next)
    {
        ...
        if (GET_CODE(insn) == CODE_LABEL || GET_CODE(insn) == JUMP_INSN
            || (GET_RTX_CLASS(GET_CODE(insn)) == 'i' && REG_NOTES(insn) != 0))
        ...
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i')
        {
            ...
            if (set && GET_CODE(SET_DEST(set)) == REG && reg_renumber[REGNO(SET_DEST(set))] < 0
            ...
            {
                rtx set = single_set(insn);
                if (set && SET_SRC(set) == SET_DEST(set) && GET_CODE(SET_SRC(set)) == REG
...
static void calculate_needs(struct insn_chain *chain)
{
    int i;
    ...
    for (i = 0; i < n_reloads; i++)
    {
        ...
        if (reload_reg_rtx[i] != 0 || reload_optional[i] != 0
            || (reload_out[i] == 0 && reload_in[i] == 0 && !reload_secondary_p[i]))
            continue;
    ...
    for (i = 0; i < N_REG_CLASSES; i++)
    {
        ...
        for (j = 0; j <= 1; j++)
        {
            ...
            {
                in_max = MAX(in_max,
                    (insn_needs.in_addr[k].regs[j][i] + insn_needs.in_addr_addr[k].regs[j][i]));
                out_max = MAX(out_max, insn_needs.out_addr[k].regs[j][i]);
                out_max = MAX(out_max, insn_needs.out_addr_addr[k].regs[j][i]);
            }
            ...
            in_max = MAX(
                MAX(insn_needs.op_addr.regs[j][i], insn_needs.op_addr_reload.regs[j][i]), in_max);

            out_max = MAX(out_max, insn_needs.insn.regs[j][i]);

            insn_needs.input.regs[j][i] = MAX(insn_needs.input.regs[j][i]
                    + insn_needs.op_addr.regs[j][i] + insn_needs.insn.regs[j][i],
                in_max + insn_needs.input.regs[j][i]);

            insn_needs.output.regs[j][i] += out_max;
            insn_needs.other.regs[j][i]
                += MAX(MAX(insn_needs.input.regs[j][i], insn_needs.output.regs[j][i]),
        ...
        in_max
            = MAX(MAX(insn_needs.op_addr.groups[i], insn_needs.op_addr_reload.groups[i]), in_max);
        out_max = MAX(out_max, insn_needs.insn.groups[i]);

        insn_needs.input.groups[i] = MAX(
            insn_needs.input.groups[i] + insn_needs.op_addr.groups[i] + insn_needs.insn.groups[i],
            in_max + insn_needs.input.groups[i]);

        insn_needs.output.groups[i] += out_max;
        insn_needs.other.groups[i]
            += MAX(MAX(insn_needs.input.groups[i], insn_needs.output.groups[i]),
...
/* If pseudo REG conflicts with one of our reload registers, mark it as
   spilled.  */
static void maybe_mark_pseudo_spilled(int reg)
{
    int i;
...
static void find_reload_regs(struct insn_chain *chain, FILE *dumpfile)
{
    ...
    EXECUTE_IF_SET_IN_REG_SET(
        chain->live_before, FIRST_PSEUDO_REGISTER, i, { maybe_mark_pseudo_spilled(i); });
    EXECUTE_IF_SET_IN_REG_SET(
        chain->live_after, FIRST_PSEUDO_REGISTER, i, { maybe_mark_pseudo_spilled(i); });
...
static int possible_group_p(struct insn_chain *chain, int regno)
{
    ...
    for (i = 0; i < FIRST_PSEUDO_REGISTER - 1; i++)
    {
        ...
        if (spill_reg_order[i] < 0 && spill_reg_order[i + 1] < 0
            && !TEST_HARD_REG_BIT(bad_spill_regs, i) && !TEST_HARD_REG_BIT(bad_spill_regs, i + 1))
            return 1;

        /* A pair of one already spilled and one we can spill does it
           provided the one already spilled is not otherwise reserved.  */
        if (spill_reg_order[i] < 0 && !TEST_HARD_REG_BIT(bad_spill_regs, i)
            && spill_reg_order[i + 1] >= 0 && !TEST_HARD_REG_BIT(chain->counted_for_groups, i + 1)
            && !TEST_HARD_REG_BIT(chain->counted_for_nongroups, i + 1))
            return 1;
        if (spill_reg_order[i + 1] < 0 && !TEST_HARD_REG_BIT(bad_spill_regs, i + 1)
            && spill_reg_order[i] >= 0 && !TEST_HARD_REG_BIT(chain->counted_for_groups, i)
...
static void alter_reg(int i, int from_reg)
{
    ...
    REGNO(regno_reg_rtx[i]) = reg_renumber[i] >= 0 ? reg_renumber[i] : i;
        ...
        && reg_equiv_memory_loc[i] == 0)
    {
        ...
        if (from_reg == -1)
        {
            ...
            x = assign_stack_local(
                GET_MODE(regno_reg_rtx[i]), total_size, inherent_size == total_size ? 0 : -1);

            RTX_UNCHANGING_P(x) = RTX_UNCHANGING_P(regno_reg_rtx[i]);
        }
        /* Reuse a stack slot if possible.  */
        else if (spill_stack_slot[from_reg] != 0 && spill_stack_slot_width[from_reg] >= total_size
        ...
        else
        {
            ...
            {
                if (GET_MODE_SIZE(GET_MODE(spill_stack_slot[from_reg])) > inherent_size)
        ...
        {
            x = gen_rtx_MEM(GET_MODE(regno_reg_rtx[i]), plus_constant(XEXP(x, 0), adjust));
...
static void set_label_offsets(rtx x, rtx insn, int initial_p)
{
    ...
    case CODE_LABEL:
        ...
        if (!offsets_known_at[CODE_LABEL_NUMBER(x)])
        {
            for (i = 0; i < NUM_ELIMINABLE_REGS; i++)
                offsets_at[CODE_LABEL_NUMBER(x)][i]
                    = (initial_p ? reg_eliminate[i].initial_offset : reg_eliminate[i].offset);
        ...
        else if (x == insn && (tem = prev_nonnote_insn(insn)) != 0 && GET_CODE(tem) == BARRIER)
    ...
    case ADDR_DIFF_VEC:
        ...
        for (i = 0; i < (unsigned)XVECLEN(x, code == ADDR_DIFF_VEC); i++)
            set_label_offsets(XVECEXP(x, code == ADDR_DIFF_VEC, i), insn, initial_p);
        return;
...
rtx eliminate_regs(rtx x, enum machine_mode mem_mode, rtx insn)
{
            ...
            for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS]; ep++)
                if (ep->from_rtx == x && ep->can_eliminate)
                {
                    if (!mem_mode
                        ...
                        && !(insn != 0
                               && (GET_CODE(insn) == EXPR_LIST || GET_CODE(insn) == INSN_LIST)))
                        ep->ref_outside_mem = 1;
                    return plus_constant(ep->to_rtx, ep->previous_offset);
                }
        }
        else if (reg_renumber[regno] < 0 && reg_equiv_constant && reg_equiv_constant[regno]
            && !CONSTANT_P(reg_equiv_constant[regno]))
            return eliminate_regs(copy_rtx(reg_equiv_constant[regno]), mem_mode, insn);
        return x;

    case PLUS:
        /* If this is the sum of an eliminable register and a constant, rework
           the sum.   */
        if (GET_CODE(XEXP(x, 0)) == REG && REGNO(XEXP(x, 0)) < FIRST_PSEUDO_REGISTER
            && CONSTANT_P(XEXP(x, 1)))
        {
            for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS]; ep++)
                if (ep->from_rtx == XEXP(x, 0) && ep->can_eliminate)
                {
                    ...
                    if (mem_mode != 0 && GET_CODE(XEXP(x, 1)) == CONST_INT
                        ...
                        return gen_rtx_PLUS(
                            Pmode, ep->to_rtx, plus_constant(XEXP(x, 1), ep->previous_offset));
                }
...

        {
            ...
            if (new0 != XEXP(x, 0) || new1 != XEXP(x, 1))
            {
                ...
                if (GET_CODE(new0) == PLUS && GET_CODE(new1) == REG
                    && REGNO(new1) >= FIRST_PSEUDO_REGISTER && reg_renumber[REGNO(new1)] < 0
                    && reg_equiv_constant != 0 && reg_equiv_constant[REGNO(new1)] != 0)
                    new1 = reg_equiv_constant[REGNO(new1)];
                else if (GET_CODE(new1) == PLUS && GET_CODE(new0) == REG
                    && REGNO(new0) >= FIRST_PSEUDO_REGISTER && reg_renumber[REGNO(new0)] < 0
        ...
        if (GET_CODE(XEXP(x, 0)) == REG && REGNO(XEXP(x, 0)) < FIRST_PSEUDO_REGISTER
            ...
            for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS]; ep++)
                if (ep->from_rtx == XEXP(x, 0) && ep->can_eliminate)
                {
                    ...
                    return plus_constant(gen_rtx_MULT(Pmode, ep->to_rtx, XEXP(x, 1)),
    ...
    {
        rtx new0 = eliminate_regs(XEXP(x, 0), mem_mode, insn);
        rtx new1 = XEXP(x, 1) ? eliminate_regs(XEXP(x, 1), mem_mode, insn) : 0;
    ...
    case ASM_OPERANDS:
    {
        ...
        if (ASM_OPERANDS_INPUT_VEC(x) != old_asm_operands_vec)
        {
            ...
            for (i = 0; i < ASM_OPERANDS_INPUT_LENGTH(x); i++)
                temp_vec[i] = eliminate_regs(ASM_OPERANDS_INPUT(x, i), mem_mode, insn);
            ...
            if (i == ASM_OPERANDS_INPUT_LENGTH(x))
                ...
                new_asm_operands_vec = gen_rtvec_v(ASM_OPERANDS_INPUT_LENGTH(x), temp_vec);
        }
    ...
    case SET:
        ...
        if (GET_CODE(SET_DEST(x)) == REG)
        {
            ...
            for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS]; ep++)
                if (ep->to_rtx == SET_DEST(x) && SET_DEST(x) != frame_pointer_rtx)
                {
                    /* If it is being incremented, adjust the offset.  Otherwise,
                       this elimination can't be done.  */
                    rtx src = SET_SRC(x);

                    if (GET_CODE(src) == PLUS && XEXP(src, 0) == SET_DEST(x)
        ...
        /* Now avoid the loop below in this common case.  */
        {
            ...
            if (GET_CODE(SET_DEST(x)) == REG && GET_CODE(new0) == MEM && insn != 0
                && GET_CODE(insn) != EXPR_LIST && GET_CODE(insn) != INSN_LIST)
...
static int eliminate_regs_in_insn(rtx insn, int replace)
{
        ...
        && REGNO(SET_DEST(old_set)) < FIRST_PSEUDO_REGISTER)
    {
        ...
        if (GET_CODE(SET_SRC(old_set)) == PLUS && GET_CODE(XEXP(SET_SRC(old_set), 0)) == REG
            && GET_CODE(XEXP(SET_SRC(old_set), 1)) == CONST_INT)
            for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS]; ep++)
                if (ep->from_rtx == XEXP(SET_SRC(old_set), 0) && ep->can_eliminate)
                {
    ...
    if (new_body != old_body)
    {
        ...
        if (old_set != 0
            && ((GET_CODE(SET_SRC(old_set)) == REG
                    && (GET_CODE(new_body) != SET || GET_CODE(SET_SRC(new_body)) != REG))
...
static void mark_not_eliminable(rtx dest, rtx x)
{
    ...
    for (i = 0; i < NUM_ELIMINABLE_REGS; i++)
        if (reg_eliminate[i].can_eliminate && dest == reg_eliminate[i].to_rtx
            && (GET_CODE(x) != SET || GET_CODE(SET_SRC(x)) != PLUS || XEXP(SET_SRC(x), 0) != dest
                   || GET_CODE(XEXP(SET_SRC(x), 1)) != CONST_INT))
        {
            reg_eliminate[i].can_eliminate_previous = reg_eliminate[i].can_eliminate = 0;
            num_eliminable--;
        }
}
...
static void update_eliminables(HARD_REG_SET *pset)
{
    ...
    for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS]; ep++)
    {
        ...
        if (!ep->can_eliminate && ep->can_eliminate_previous)
        {
            /* Find the current elimination for ep->from, if there is a
               new one.  */
            for (op = reg_eliminate; op < &reg_eliminate[NUM_ELIMINABLE_REGS]; op++)
    ...
    for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS]; ep++)
    {
        if (ep->can_eliminate && ep->from == FRAME_POINTER_REGNUM && ep->to != FRAME_POINTER_REGNUM)
...
static void init_elim_table(void)
{
    ...
    {
        reg_eliminate
            = (struct elim_table *)xmalloc(sizeof(struct elim_table) * NUM_ELIMINABLE_REGS);
        zero_memory(reg_eliminate, sizeof(struct elim_table) * NUM_ELIMINABLE_REGS);
    }
    ...
    for (ep = reg_eliminate, ep1 = reg_eliminate_1; ep < &reg_eliminate[NUM_ELIMINABLE_REGS];
         ep++, ep1++)
    {
        ep->from = ep1->from;
        ep->to = ep1->to;
        ep->can_eliminate = ep->can_eliminate_previous = (CAN_ELIMINATE(ep->from, ep->to)
...
static void spill_hard_reg(int regno, FILE *dumpfile, int cant_eliminate)
{
    ...
    for (i = FIRST_PSEUDO_REGISTER; i < max_regno; i++)
        if (reg_renumber[i] >= 0 && reg_renumber[i] <= regno
            && (reg_renumber[i] + HARD_REGNO_NREGS(reg_renumber[i], PSEUDO_REGNO_MODE(i)) > regno))
...
static int finish_spills(int global, FILE *dumpfile)
{
    struct insn_chain *chain;
    ...
    if (global)
    {
        ...
        {
            EXECUTE_IF_SET_IN_REG_SET(chain->live_before, FIRST_PSEUDO_REGISTER, i,
                { ior_hard_reg_set(pseudo_forbidden_regs + i, &chain->used_spill_regs); });
            EXECUTE_IF_SET_IN_REG_SET(chain->live_after, FIRST_PSEUDO_REGISTER, i,
                { ior_hard_reg_set(pseudo_forbidden_regs + i, &chain->used_spill_regs); });
        }
...
static void scan_paradoxical_subregs(rtx x)
{
    ...
    case SUBREG:
        if (GET_CODE(SUBREG_REG(x)) == REG
            && GET_MODE_SIZE(GET_MODE(x)) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(x))))
            reg_max_ref_width[REGNO(SUBREG_REG(x))] = GET_MODE_SIZE(GET_MODE(x));
        return;
...
static void count_pseudo(struct hard_reg_n_uses *n_uses, int reg)
{
    int r = reg_renumber[reg];
    int nregs;
...
static void order_regs_for_reload(struct insn_chain *chain)
{
    ...
    for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
    {
        ...
        if (fixed_regs[i] || REGNO_REG_SET_P(chain->live_before, i)
        ...
        EXECUTE_IF_SET_IN_REG_SET(
            chain->live_before, FIRST_PSEUDO_REGISTER, j, { count_pseudo(hard_reg_n_uses, j); });
        EXECUTE_IF_SET_IN_REG_SET(
            chain->live_after, FIRST_PSEUDO_REGISTER, j, { count_pseudo(hard_reg_n_uses, j); });
    }
    ...
    qsort(hard_reg_n_uses, FIRST_PSEUDO_REGISTER, sizeof hard_reg_n_uses[0], hard_reg_use_compare);
    ...
    for (chain = reload_insn_chain; chain; chain = chain->next)
    {
        ...
        else if (GET_RTX_CLASS(GET_CODE(insn)) == 'i')
        {
            ...
            if ((GET_CODE(PATTERN(insn)) == USE || GET_CODE(PATTERN(insn)) == CLOBBER)
                ...
                XEXP(XEXP(PATTERN(insn), 0), 0) = eliminate_regs(
                    XEXP(XEXP(PATTERN(insn), 0), 0), GET_MODE(XEXP(PATTERN(insn), 0)), NULL_RTX);
            ...
            if (!chain->need_elim && !chain->need_reload && !chain->need_operand_change)
                n_reloads = 0;
            ...
            else
            {
                ...
                find_reloads(insn, 1, spill_indirect_levels, live_known, spill_reg_order);
            }
                ...
                if (asm_noperands(PATTERN(insn)) >= 0)
                    for (p = NEXT_INSN(prev); p != next; p = NEXT_INSN(p))
                        if (p != insn && GET_RTX_CLASS(GET_CODE(p)) == 'i'
                            && (recog_memoized(p) < 0 || (extract_insn(p), !constrain_operands(1))))
                        {
                            error_for_asm(insn, "`asm' operand requires impossible reload");
                            PUT_CODE(p, NOTE);
            ...
            for (i = n_reloads - 1; i >= 0; i--)
            {
                ...
                if (in_reg)
                {
                    ...
                    if ((code == POST_INC || code == POST_DEC)
                        && TEST_HARD_REG_BIT(reg_reloaded_valid, REGNO(reload_reg_rtx[i]))
...

                    {
                        ...
                        for (p = PREV_INSN(old_next); p != prev; p = PREV_INSN(p))
                        {
                            ...
                            if (n == 1)
                            {
                                n = validate_replace_rtx(
                                    reload_reg, gen_rtx(code, mode, reload_reg), p);
                                ...
                                if (!n)
                                {
                                    validate_replace_rtx(
                                        gen_rtx(code, mode, reload_reg), reload_reg, p);
                                    break;
                                }
                            }
                            break;
                        ...
                        if (n == 1)
                            REG_NOTES(p) = gen_rtx_EXPR_LIST(REG_INC, reload_reg, REG_NOTES(p));
                        else
...
static void forget_old_reloads_1(rtx x ATTRIBUTE_UNUSED, rtx ignored)
{
    ...
    else
    {
        ...
        for (i = 0; i < nr; i++)
            ...
            if (n_reloads == 0 || !TEST_HARD_REG_BIT(reg_is_output_reload, regno + i))
...
static void mark_reload_reg_in_use(
    int regno, int opnum, enum reload_type type, enum machine_mode mode)
{
...
static void clear_reload_reg_in_use(
    int regno, int opnum, enum reload_type type, enum machine_mode mode)
{
    int nregs = HARD_REGNO_NREGS(regno, mode);
    ...
    if (check_opnum || check_any)
    {
        for (i = n_reloads - 1; i >= 0; i--)
        {
            if (reload_when_needed[i] == type && (check_any || reload_opnum[i] == opnum)
                && reload_reg_rtx[i])
            {
                int conflict_start = true_regnum(reload_reg_rtx[i]);
                int conflict_end
                    = (conflict_start + HARD_REGNO_NREGS(conflict_start, reload_mode[i]));
...
static int reload_reg_free_p(int regno, int opnum, enum reload_type type)
{
    int i;
...
static int reload_reg_reaches_end_p(int regno, int opnum, enum reload_type type)
{
    int i;
...
int reloads_conflict(int r1, int r2)
{
    ...
    {
    case RELOAD_FOR_INPUT:
        return (r2_type == RELOAD_FOR_INSN || r2_type == RELOAD_FOR_OPERAND_ADDRESS
            || r2_type == RELOAD_FOR_OPADDR_ADDR || r2_type == RELOAD_FOR_INPUT
            || ((r2_type == RELOAD_FOR_INPUT_ADDRESS || r2_type == RELOAD_FOR_INPADDR_ADDRESS)
                   && r2_opnum > r1_opnum));
    ...
    case RELOAD_FOR_OUTPUT:
        return (r2_type == RELOAD_FOR_INSN || r2_type == RELOAD_FOR_OUTPUT
            || ((r2_type == RELOAD_FOR_OUTPUT_ADDRESS || r2_type == RELOAD_FOR_OUTADDR_ADDRESS)
                   && r2_opnum >= r1_opnum));

    case RELOAD_FOR_INSN:
        return (r2_type == RELOAD_FOR_INPUT || r2_type == RELOAD_FOR_OUTPUT
            || r2_type == RELOAD_FOR_INSN || r2_type == RELOAD_FOR_OPERAND_ADDRESS);
...
/* Return 1 if the value in reload reg REGNO, as used by a reload
   ...
   register.  */
static int reload_reg_free_for_value_p(int regno, int opnum, enum reload_type type, rtx value,
    rtx out, int reloadnum, int ignore_address_reloads)
{
    int time1;
    int i;
    int copy = 0;
    ...
    for (i = 0; i < n_reloads; i++)
    {
        ...
        if (reg && GET_CODE(reg) == REG
            ...
            && i != reloadnum)
        {
            if (!reload_in[i] || !rtx_equal_p(reload_in[i], value) || reload_out[i] || out)
            {
                int time2;
                switch (reload_when_needed[i])
                {
                ...
                case RELOAD_FOR_INPUT_ADDRESS:
                    if (type == RELOAD_FOR_INPUT && opnum == reload_opnum[i]
                        && ignore_address_reloads && !reload_out[reloadnum])
                        continue;
                ...
                if ((time1 >= time2
                        && (!reload_in[i] || reload_out[i] || !rtx_equal_p(reload_in[i], value)))
                    || (out && reload_out_reg[reloadnum] && time2 >= MAX_RECOG_OPERANDS * 4 + 3))
                    return 0;
            }
        }
    }
...
static int allocate_reload_reg(struct insn_chain *chain, int r, int last_reload, int noerror)
{
    rtx insn = chain->insn;
    ...
    for (pass = 0; pass < 2; pass++)
    {
        ...
        for (count = 0; count < n_spills; count++)
        {
            ...
            if ((reload_reg_free_p(regnum, reload_opnum[r], reload_when_needed[r])
                    || (reload_in[r]
    ...
    if (HARD_REGNO_MODE_OK(regno, reload_mode[r]))
    {
        ...
        if (!(reload_in[r] != 0 && test_mode != VOIDmode && !HARD_REGNO_MODE_OK(regno, test_mode)))
            if (!(reload_out[r] != 0 && !HARD_REGNO_MODE_OK(regno, GET_MODE(reload_out[r]))))
            {
                /* The reg is OK.  */
                last_spill_reg = i;
...
failure:
    ...
    error_for_asm(insn, "`asm' operand constraint incompatible with operand size");
    reload_in[r] = 0;
    reload_out[r] = 0;
    reload_reg_rtx[r] = 0;
    reload_optional[r] = 1;
...
static void choose_reload_regs(struct insn_chain *chain)
{
    ...
    for (j = 0; j < n_reloads; j++)
    {
        ...
        if (reload_reg_rtx[j])
            mark_reload_reg_in_use(
                REGNO(reload_reg_rtx[j]), reload_opnum[j], reload_when_needed[j], reload_mode[j]);
    }

    if (n_reloads > 1)
        qsort(reload_order, n_reloads, sizeof(short), reload_reg_class_lower);

    copy_memory((char *)reload_reg_rtx, (char *)save_reload_reg_rtx, sizeof reload_reg_rtx);
    ...
    copy_memory(
        (char *)reload_override_in, (char *)save_reload_override_in, sizeof reload_override_in);
    copy_memory(
        (char *)reload_spill_index, (char *)save_reload_spill_index, sizeof reload_spill_index);
    ...
    COPY_HARD_REG_SET(save_reload_reg_used_in_op_addr, reload_reg_used_in_op_addr);

    COPY_HARD_REG_SET(save_reload_reg_used_in_op_addr_reload, reload_reg_used_in_op_addr_reload);

    COPY_HARD_REG_SET(save_reload_reg_used_in_insn, reload_reg_used_in_insn);
    COPY_HARD_REG_SET(save_reload_reg_used_in_other_addr, reload_reg_used_in_other_addr);

    for (i = 0; i < reload_n_operands; i++)
    {
        COPY_HARD_REG_SET(save_reload_reg_used_in_output[i], reload_reg_used_in_output[i]);
        COPY_HARD_REG_SET(save_reload_reg_used_in_input[i], reload_reg_used_in_input[i]);
        COPY_HARD_REG_SET(save_reload_reg_used_in_input_addr[i], reload_reg_used_in_input_addr[i]);
        COPY_HARD_REG_SET(
            save_reload_reg_used_in_inpaddr_addr[i], reload_reg_used_in_inpaddr_addr[i]);
        COPY_HARD_REG_SET(
            save_reload_reg_used_in_output_addr[i], reload_reg_used_in_output_addr[i]);
        COPY_HARD_REG_SET(
            save_reload_reg_used_in_outaddr_addr[i], reload_reg_used_in_outaddr_addr[i]);
    }
    ...
    for (inheritance = optimize > 0; inheritance >= 0; inheritance--)
    {
        /* Process the reloads in order of preference just found.
           ...
           home in a register needed by an earlier reload with a tighter
           constraint.
        ...
        for (j = 0; j < n_reloads; j++)
        {
                ...
                if (regno >= 0 && reg_last_reload_reg[regno] != 0)
                {
                    ...
                    if ((GET_MODE_SIZE(GET_MODE(last_reg))
                        ...
                        && (reload_nregs[r] == max_group_size
                               || !TEST_HARD_REG_BIT(reg_class_contents[(int)group_class], i))
                        && reload_reg_free_for_value_p(i, reload_opnum[r], reload_when_needed[r],
                               reload_in[r], const0_rtx, r, 1))
                    {
                        /* If a group is needed, verify that all the subsequent
                           registers still have their values intact.  */
                        int nr = HARD_REGNO_NREGS(i, reload_mode[r]);
                        int k;
                        ...
                        if (k == nr)
                        {
                            ...
                            last_reg
                                = (GET_MODE(last_reg) == mode ? last_reg : gen_rtx_REG(mode, i));
                            ...
                            for (i1 = 0; i1 < n_earlyclobbers; i1++)
                                if (reg_overlap_mentioned_for_reload_p(
                                        reg_last_reload_reg[regno], reload_earlyclobbers[i1]))
                                    break;
                            ...
                            if (i1 != n_earlyclobbers
                                ...
                                || (TEST_HARD_REG_BIT(reg_used_in_insn, i) && reload_out[r]
                                       && !TEST_HARD_REG_BIT(reg_reloaded_dead, i))
                                /* Don't clobber the frame pointer.  */
                                || (i == FRAME_POINTER_REGNUM && reload_out[r])
                                /* Don't really use the inherited spill reg
                                   if we need it wider than we've got it.  */
                                || (GET_MODE_SIZE(reload_mode[r]) > GET_MODE_SIZE(mode))
                                || !TEST_HARD_REG_BIT(
                                       reg_class_contents[(int)reload_reg_class[r]], i)
                            ...
                            else
                            {
                                ...
                                /* Mark the register as in use for this part of
                                   the insn.  */
                                mark_reload_reg_in_use(
                                    i, reload_opnum[r], reload_when_needed[r], reload_mode[r]);
                                reload_reg_rtx[r] = last_reg;
                                reload_inherited[r] = 1;
                                reload_inheritance_insn[r] = reg_reloaded_insn[i];
                                reload_spill_index[r] = i;
                                for (k = 0; k < nr; k++)
                                    SET_HARD_REG_BIT(reload_reg_used_for_inherit, i + k);
                            }
                        }
                    }
                }
            ...
            if (inheritance && reload_in[r] != 0 && !reload_inherited[r] && reload_out[r] == 0
                && (CONSTANT_P(reload_in[r]) || GET_CODE(reload_in[r]) == PLUS
                       || GET_CODE(reload_in[r]) == REG || GET_CODE(reload_in[r]) == MEM)
                && (reload_nregs[r] == max_group_size
                       || !reg_classes_intersect_p(reload_reg_class[r], group_class)))
            {
                ...
                if (equiv != 0
                    && ((TEST_HARD_REG_BIT(reload_reg_used_at_all, regno)
                            && !reload_reg_free_for_value_p(regno, reload_opnum[r],
                                   reload_when_needed[r], reload_in[r], reload_out[r], r, 1))
                           || !TEST_HARD_REG_BIT(
                                  reg_class_contents[(int)reload_reg_class[r]], regno)))
                    equiv = 0;
                ...
                if (equiv != 0)
                    for (i = 0; i < n_earlyclobbers; i++)
                        if (reg_overlap_mentioned_for_reload_p(equiv, reload_earlyclobbers[i]))
                        {
                ...
                if (equiv != 0 && regno != FRAME_POINTER_REGNUM)
                {
                    ...
                    for (k = 0; k < nr; k++)
                    {
                        ...
                        {
                            mark_reload_reg_in_use(
                                regno, reload_opnum[r], reload_when_needed[r], reload_mode[r]);
        ...
        copy_memory((char *)save_reload_reg_rtx, (char *)reload_reg_rtx, sizeof reload_reg_rtx);
        copy_memory(
            (char *)save_reload_inherited, (char *)reload_inherited, sizeof reload_inherited);
        copy_memory((char *)save_reload_inheritance_insn,
            (char *)reload_inheritance_insn,
            sizeof reload_inheritance_insn);
        copy_memory(
            (char *)save_reload_override_in, (char *)reload_override_in, sizeof reload_override_in);
        copy_memory(
            (char *)save_reload_spill_index, (char *)reload_spill_index, sizeof reload_spill_index);
        ...
        COPY_HARD_REG_SET(reload_reg_used_in_op_addr, save_reload_reg_used_in_op_addr);
        COPY_HARD_REG_SET(
            reload_reg_used_in_op_addr_reload, save_reload_reg_used_in_op_addr_reload);
        COPY_HARD_REG_SET(reload_reg_used_in_insn, save_reload_reg_used_in_insn);
        COPY_HARD_REG_SET(reload_reg_used_in_other_addr, save_reload_reg_used_in_other_addr);

        for (i = 0; i < reload_n_operands; i++)
        {
            COPY_HARD_REG_SET(reload_reg_used_in_input[i], save_reload_reg_used_in_input[i]);
            COPY_HARD_REG_SET(reload_reg_used_in_output[i], save_reload_reg_used_in_output[i]);
            COPY_HARD_REG_SET(
                reload_reg_used_in_input_addr[i], save_reload_reg_used_in_input_addr[i]);
            COPY_HARD_REG_SET(
                reload_reg_used_in_inpaddr_addr[i], save_reload_reg_used_in_inpaddr_addr[i]);
            COPY_HARD_REG_SET(
                reload_reg_used_in_output_addr[i], save_reload_reg_used_in_output_addr[i]);
            COPY_HARD_REG_SET(
                reload_reg_used_in_outaddr_addr[i], save_reload_reg_used_in_outaddr_addr[i]);
        }
    }
    ...
    for (pass = flag_expensive_optimizations; pass >= 0; pass--)
    {
        for (j = 0; j < n_reloads; j++)
        {
            ...
            for (i = check_regnum + nr - 1; i >= check_regnum; i--)
                if (!reload_reg_free_for_value_p(i, reload_opnum[r], reload_when_needed[r],
                        reload_in[r], (reload_inherited[r] ? reload_out[r] : const0_rtx), r, 1))
                {
                    cant_inherit = 1;
                    break;
                }
                   ...
                   || (reload_in[j] == 0 && reload_out[j] == 0 && !reload_secondary_p[j])))
        {
            ...
            if (spill_reg_order[regno] >= 0)
                clear_reload_reg_in_use(
                    regno, reload_opnum[j], reload_when_needed[j], reload_mode[j]);
            reload_reg_rtx[j] = 0;
        }
    ...
    for (j = 0; j < n_reloads; j++)
    {
        ...
        if (reload_out_reg[r] != 0 && GET_CODE(reload_out_reg[r]) == REG && reload_reg_rtx[r] != 0)
        {
            ...
            if (reload_when_needed[r] != RELOAD_OTHER && reload_when_needed[r] != RELOAD_FOR_OUTPUT
...
void deallocate_reload_reg(int r)
{
    ...
    reload_reg_rtx[r] = 0;
    if (spill_reg_order[regno] >= 0)
        clear_reload_reg_in_use(regno, reload_opnum[r], reload_when_needed[r], reload_mode[r]);
...
static void merge_assigned_reloads(rtx insn)
{
    ...
    for (i = 0; i < n_reloads; i++)
    {
        ...
        if (reload_in[i] == 0 || reload_when_needed[i] == RELOAD_OTHER || reload_out[i] != 0
            || reload_reg_rtx[i] == 0 || reg_set_p(reload_reg_rtx[i], insn))
            continue;
        ...
        for (j = 0; j < n_reloads; j++)
        {
            if (i == j || reload_reg_rtx[j] == 0
                || !reg_overlap_mentioned_p(reload_reg_rtx[j], reload_reg_rtx[i]))
                continue;
            ...
            if (!rtx_equal_p(reload_reg_rtx[i], reload_reg_rtx[j]) || reload_out[j] != 0
                || reload_in[j] == 0 || !rtx_equal_p(reload_in[i], reload_in[j]))
            {
        ...
        if (j == n_reloads && max_input_address_opnum <= min_conflicting_input_opnum)
        {
            for (j = 0; j < n_reloads; j++)
                if (i != j && reload_reg_rtx[j] != 0
                    && rtx_equal_p(reload_reg_rtx[i], reload_reg_rtx[j])
                    && (!conflicting_input || reload_when_needed[j] == RELOAD_FOR_INPUT_ADDRESS
            ...
            if (reload_when_needed[i] == RELOAD_OTHER)
                for (j = 0; j < n_reloads; j++)
                    if (reload_in[j] != 0 && reload_when_needed[i] != RELOAD_OTHER
                        && reg_overlap_mentioned_for_reload_p(reload_in[j], reload_in[i]))
...
static void emit_reload_insns(struct insn_chain *chain)
{
    ...
    for (j = 0; j < reload_n_operands; j++)
        input_reload_insns[j] = input_address_reload_insns[j] = inpaddr_address_reload_insns[j]
            = output_reload_insns[j] = output_address_reload_insns[j]
            = outaddr_address_reload_insns[j] = other_output_reload_insns[j] = 0;
    ...
    for (j = 0; j < n_reloads; j++)
    {
        ...
        if (reload_reg_rtx[j] && REGNO(reload_reg_rtx[j]) < FIRST_PSEUDO_REGISTER)
            new_spill_reg_store[REGNO(reload_reg_rtx[j])] = 0;

        old = (reload_in[j] && GET_CODE(reload_in[j]) == MEM ? reload_in_reg[j] : reload_in[j]);
            ...
            && !rtx_equal_p(reload_reg_rtx[j], old) && reload_reg_rtx[j] != 0)
        {
            ...
            if (reload_secondary_in_reload[j] >= 0
                && reload_secondary_in_icode[j] == CODE_FOR_nothing && optimize)
                oldequiv = find_equiv_reg(
                    old, insn, reload_reg_class[reload_secondary_in_reload[j]], -1, NULL, 0, mode);
#endif
            ...
            if (oldequiv == 0 && optimize
                && (GET_CODE(old) == MEM
                       || (GET_CODE(old) == REG && REGNO(old) >= FIRST_PSEUDO_REGISTER
            ...
            if (oldequiv)
            {
                ...
                if (!reload_reg_free_for_value_p(regno, reload_opnum[j], reload_when_needed[j],
                ...
                if (oldequiv != 0
                    && ((REGNO_REG_CLASS(regno) != reload_reg_class[j]
                            && (REGISTER_MOVE_COST(REGNO_REG_CLASS(regno), reload_reg_class[j])
                                   >= MEMORY_MOVE_COST(mode, reload_reg_class[j], 1)))
#ifdef SECONDARY_INPUT_RELOAD_CLASS
                           || (SECONDARY_INPUT_RELOAD_CLASS(reload_reg_class[j], mode, oldequiv)
                                  != NO_REGS)
#endif
                               ))
                    oldequiv = 0;
            ...
            if (oldequiv == 0 && reload_override_in[j] && GET_CODE(reload_in_reg[j]) == REG)
            {
                oldequiv = old;
                old = reload_in_reg[j];
            }
            ...
            if (optimize && GET_CODE(oldequiv) == REG && REGNO(oldequiv) < FIRST_PSEUDO_REGISTER
                && spill_reg_store[REGNO(oldequiv)] && GET_CODE(old) == REG
                && (dead_or_set_p(insn, spill_reg_stored_to[REGNO(oldequiv)])
                       || rtx_equal_p(spill_reg_stored_to[REGNO(oldequiv)], reload_out_reg[j])))
            ...
            if (GET_MODE(oldequiv) != VOIDmode && mode != GET_MODE(oldequiv))
            ...
            if (reload_out[j] && !reload_out_reg[j])
            {
                ...
                if (optimize && GET_CODE(oldequiv) == REG && REGNO(oldequiv) < FIRST_PSEUDO_REGISTER
                    && spill_reg_store[REGNO(oldequiv)] && GET_CODE(old) == REG
                    && (dead_or_set_p(insn, spill_reg_stored_to[REGNO(oldequiv)])
                           || rtx_equal_p(spill_reg_stored_to[REGNO(oldequiv)], old)))
                    delete_output_reload(insn, j, REGNO(oldequiv));
                ...
                new_spill_reg_store[REGNO(reloadreg)]
                    = inc_for_reload(reloadreg, oldequiv, reload_out[j], reload_inc[j]);
            }
            ...
            else if (optimize && GET_CODE(old) == REG && REGNO(old) >= FIRST_PSEUDO_REGISTER
                && dead_or_set_p(insn, old)
                /* This is unsafe if some other reload
                   uses the same reg first.  */
                && reload_reg_free_for_value_p(REGNO(reloadreg), reload_opnum[j],
                       reload_when_needed[j], old, reload_out[j], j, 0))
            {
                rtx temp = PREV_INSN(insn);
                while (temp && GET_CODE(temp) == NOTE)
                    temp = PREV_INSN(temp);
                if (temp && GET_CODE(temp) == INSN && GET_CODE(PATTERN(temp)) == SET
                    && SET_DEST(PATTERN(temp)) == old
                    /* Make sure we can access insn_operand_constraint.  */
                    && asm_noperands(PATTERN(temp)) < 0
                    /* This is unsafe if prev insn rejects our reload reg.  */
                    && constraint_accepts_reg_p(
                           insn_operand_constraint[recog_memoized(temp)][0], reloadreg)
            ...
            if (!special)
            {
                ...
                if (reload_secondary_in_reload[j] >= 0)
                {
                    ...
                    if (GET_CODE(oldequiv) == REG && REGNO(oldequiv) >= FIRST_PSEUDO_REGISTER
                        && (reg_equiv_memory_loc[REGNO(oldequiv)] != 0
                               || reg_equiv_constant[REGNO(oldequiv)] != 0))
                    {
                        if (!reg_equiv_mem[REGNO(oldequiv)] || num_not_at_initial_offset)
                    ...
                    {
                        if (!reg_equiv_mem[REGNO(old)] || num_not_at_initial_offset)
                    ...
                    {
                        enum reg_class new_class = SECONDARY_INPUT_RELOAD_CLASS(
                            reload_reg_class[j], mode, real_oldequiv);
                        ...
                        else
                        {
                            ...
                            if (!TEST_HARD_REG_BIT(
                                    reg_class_contents[(int)new_class], REGNO(second_reload_reg)))
                            ...
                            else
                            {
                                ...
                                {
                                    if (!HARD_REGNO_MODE_OK(REGNO(second_reload_reg), new_mode))
                                        oldequiv = old, real_oldequiv = real_old;
                                    else
                                        second_reload_reg
                                            = gen_rtx_REG(new_mode, REGNO(second_reload_reg));
                                }
                            }
                        }
                    }
                    ...
                    if (second_reload_reg)
                    {
                        ...
                        {
                            emit_insn(GEN_FCN(icode)(reloadreg, real_oldequiv, second_reload_reg));
                            special = 1;
                        }
                        else
                        {
                            ...
                            if (tertiary_icode != CODE_FOR_nothing)
                            {
                                ...
                                emit_insn((GEN_FCN(tertiary_icode)(
                                    second_reload_reg, real_oldequiv, third_reload_reg)));
                            }
                            else
                                gen_reload(second_reload_reg, real_oldequiv, reload_opnum[j],
                                    reload_when_needed[j]);
                ...
                if (!special && !rtx_equal_p(reloadreg, oldequiv))
                {
                    ...
                    if ((GET_CODE(oldequiv) == REG && REGNO(oldequiv) >= FIRST_PSEUDO_REGISTER
                            && (reg_equiv_memory_loc[REGNO(oldequiv)] != 0
                                   || reg_equiv_constant[REGNO(oldequiv)] != 0))
                        || (GET_CODE(oldequiv) == SUBREG && GET_CODE(SUBREG_REG(oldequiv)) == REG
                               && (REGNO(SUBREG_REG(oldequiv)) >= FIRST_PSEUDO_REGISTER)
                               && ((reg_equiv_memory_loc[REGNO(SUBREG_REG(oldequiv))] != 0)
                                      || (reg_equiv_constant[REGNO(SUBREG_REG(oldequiv))] != 0))))
                        real_oldequiv = reload_in[j];
                    gen_reload(reloadreg, real_oldequiv, reload_opnum[j], reload_when_needed[j]);
                }
            }
        ...
        if (optimize && reload_inherited[j] && reload_in[j] && GET_CODE(reload_in[j]) == MEM
            && GET_CODE(reload_in_reg[j]) == MEM && reload_spill_index[j] >= 0
            && TEST_HARD_REG_BIT(reg_reloaded_valid, reload_spill_index[j]))
        {
            expect_occurrences = count_occurrences(PATTERN(insn), reload_in[j]) == 1 ? 0 : -1;
            reload_in[j] = regno_reg_rtx[reg_reloaded_contents[reload_spill_index[j]]];
        }
        ...
        if (optimize && (reload_inherited[j] || reload_override_in[j]) && reload_reg_rtx[j]
	  ...
	  && REGNO (spill_reg_stored_to[REGNO (reload_reg_rtx[j])])
	     >= FIRST_PSEUDO_REGISTER
            ...
            /* The insn might have already some references to stackslots
               ...
               original pseudo.  */
            && (dead_or_set_p(insn, spill_reg_stored_to[REGNO(reload_reg_rtx[j])])
                   || rtx_equal_p(
                          spill_reg_stored_to[REGNO(reload_reg_rtx[j])], reload_out_reg[j])))
           ...
           store.  */
        {
            ...
            if (pseudo && GET_CODE(pseudo) == REG && !rtx_equal_p(reload_in_reg[j], pseudo)
                && REGNO(pseudo) >= FIRST_PSEUDO_REGISTER && reg_last_reload_reg[REGNO(pseudo)])
            {
                ...
                /* We don't need to test full validity of last_regno for
                   ...
                   matches the pseudo.  */
                if (reg_reloaded_contents[last_regno] == pseudo_no && spill_reg_store[last_regno]
        ...
        if (old != 0 && reload_reg_rtx[j] != old && reload_reg_rtx[j] != 0)
        {
            ...
            else if (GET_CODE(old) == SUBREG && GET_CODE(SUBREG_REG(old)) == REG
                && 0 != (note = find_reg_note(insn, REG_UNUSED, SUBREG_REG(old))))
            {
                XEXP(note, 0) = gen_lowpart_common(GET_MODE(old), reload_reg_rtx[j]);
                continue;
            }
            ...
            if (reload_secondary_out_reload[j] >= 0)
            {
                ...
                if ((SECONDARY_OUTPUT_RELOAD_CLASS(reload_reg_class[j], mode, real_old) != NO_REGS))
                {
                            ...
                            if (GET_CODE(real_old) == SUBREG
                                ...
                                && 0
                                    != (tem = gen_lowpart_common(
                                            GET_MODE(SUBREG_REG(real_old)), reloadreg)))
                                real_old = SUBREG_REG(real_old), reloadreg = tem;

                            gen_reload(reloadreg, second_reloadreg, reload_opnum[j],
                                reload_when_needed[j]);
                            emit_insn(
                                (GEN_FCN(tertiary_icode)(real_old, reloadreg, third_reloadreg)));
                            special = 1;
                        }
            ...
            if (!special)
            {
                ...
                if (!flag_expensive_optimizations || GET_CODE(old) != REG
                    || !(set = single_set(insn)) || rtx_equal_p(old, SET_DEST(set))
                    || !reg_mentioned_p(old, SET_SRC(set)) || !regno_clobbered_p(REGNO(old), insn))
                    gen_reload(old, reloadreg, reload_opnum[j], reload_when_needed[j]);
            }
            ...
            for (p = get_insns(); p; p = NEXT_INSN(p))
                if (GET_RTX_CLASS(GET_CODE(p)) == 'i')
                {
                    ...
                    {
                        rtx set = single_set(insn);
                        if (reload_spill_index[j] < 0 && set && SET_SRC(set) == reload_reg_rtx[j])
                        {
                        ...
                        if (REGNO(reload_reg_rtx[j]) < FIRST_PSEUDO_REGISTER)
                        {
                            ...
                            else if (s >= 0 && SET_SRC(set) == reload_reg_rtx[j]
                                && SET_DEST(set) == reload_reg_rtx[s])
                            {
                                ...
                                {
                                    SET_HARD_REG_BIT(reg_is_output_reload, REGNO(s_reg));
    ...
    for (j = 0; j < n_reloads; j++)
    {
        ...
        /* If this is a non-inherited input reload from a pseudo, we must
           ...
           being reloaded.  */
        if (reload_in_reg[r] != 0 && !(reload_inherited[r] || reload_override_in[r]))
        {
        ...
        {
            int nr = HARD_REGNO_NREGS(i, GET_MODE(reload_reg_rtx[r]));
            int k;
            ...
            for (k = 0; k < nr; k++)
            {
                if (reload_reg_reaches_end_p(i + k, reload_opnum[r], reload_when_needed[r]))
                    part_reaches_end = 1;
                else
                    all_reaches_end = 0;
            }
            ...
            if (all_reaches_end)
            {
                           ...
                           || GET_CODE(reload_out_reg[r]) == REG))
                {
                    ...
                    if (nregno < FIRST_PSEUDO_REGISTER)
                        for (k = 1; k < nnr; k++)
                            reg_last_reload_reg[nregno + k] = (nr == nnr
                    ...
                    for (k = 0; k < nr; k++)
                    {
                        ...
                        reg_reloaded_contents[i + k]
                            = (nregno >= FIRST_PSEUDO_REGISTER || nr != nnr ? nregno : nregno + k);
                ...
                else if (reload_out_reg[r] == 0 && reload_in[r] != 0
                    ...
                    && !reg_set_p(reload_reg_rtx[r], PATTERN(insn)))
                {
                    ...
                    nnr = (nregno >= FIRST_PSEUDO_REGISTER
                            ? 1
                            : HARD_REGNO_NREGS(nregno, GET_MODE(reload_reg_rtx[r])));
                    ...
                    if (!reload_inherited[r] || (reload_out[r] && !reload_out_reg[r]))
                        spill_reg_store[i] = 0;
                   ...
                   || (GET_CODE(reload_out[r]) == MEM && GET_CODE(reload_out_reg[r]) == REG)))
        {
            ...
            if (nregno >= FIRST_PSEUDO_REGISTER)
            {
                ...
                if (src_reg && GET_CODE(src_reg) == REG && REGNO(src_reg) < FIRST_PSEUDO_REGISTER)
                {
...
rtx gen_reload(rtx out, rtx in, int opnum, enum reload_type type)
{
    ...
    if (GET_CODE(in) == SUBREG
        && (GET_MODE_SIZE(GET_MODE(in)) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(in))))
        && (tem = gen_lowpart_common(GET_MODE(SUBREG_REG(in)), out)) != 0)
        in = SUBREG_REG(in), out = tem;
    else if (GET_CODE(out) == SUBREG
        && (GET_MODE_SIZE(GET_MODE(out)) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(out))))
    ...
    if (GET_CODE(in) == PLUS
        && (GET_CODE(XEXP(in, 0)) == REG || GET_CODE(XEXP(in, 0)) == SUBREG
               || GET_CODE(XEXP(in, 0)) == MEM)
        && (GET_CODE(XEXP(in, 1)) == REG || GET_CODE(XEXP(in, 1)) == SUBREG
               || CONSTANT_P(XEXP(in, 1)) || GET_CODE(XEXP(in, 1)) == MEM))
    {
        ...
        if (GET_CODE(XEXP(in, 1)) == REG && REGNO(out) == REGNO(XEXP(in, 1)))
...
static void delete_output_reload(rtx insn, int j, int last_reload_reg)
{
    ...
    for (i1 = NEXT_INSN(output_reload_insn); i1 != insn; i1 = NEXT_INSN(i1))
    {
    ...
    if (reload_out[j] != reload_in[j] && REG_N_DEATHS(REGNO(reg)) == 1
        && REG_N_SETS(REGNO(reg)) == 1 && REG_BASIC_BLOCK(REGNO(reg)) >= 0
        && find_regno_note(insn, REG_DEAD, REGNO(reg)))
    {
        ...
        for (i2 = PREV_INSN(insn); i2; i2 = PREV_INSN(i2))
        {
            ...
            if (set != 0 && SET_DEST(set) == reg)
                continue;
            if (GET_CODE(i2) == CODE_LABEL || GET_CODE(i2) == JUMP_INSN)
                break;
...
static void delete_address_reloads(rtx dead_insn, rtx current_insn)
{
    ...
    if (!set || !set2 || GET_CODE(SET_SRC(set)) != PLUS || GET_CODE(SET_SRC(set2)) != PLUS
        || GET_CODE(XEXP(SET_SRC(set), 1)) != CONST_INT
        || GET_CODE(XEXP(SET_SRC(set2), 1)) != CONST_INT)
        return;
    dst = SET_DEST(set);
    if (!rtx_equal_p(dst, SET_DEST(set2)) || !rtx_equal_p(dst, XEXP(SET_SRC(set), 0))
        || !rtx_equal_p(dst, XEXP(SET_SRC(set2), 0))
        || (INTVAL(XEXP(SET_SRC(set), 1)) != -INTVAL(XEXP(SET_SRC(set2), 1))))
        return;
    delete_insn(prev);
    delete_insn(next);
}
...
static void delete_address_reloads_1(rtx dead_insn, rtx x, rtx current_insn)
{
    ...
    if (code != REG)
    {
        ...
        for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
        {
            ...
            {
                for (j = XVECLEN(x, i) - 1; j >= 0; j--)
                    delete_address_reloads_1(dead_insn, XVECEXP(x, i, j), current_insn);
            }
        }
        return;
    }
    ...
    if (!set)
        return;
    dst = SET_DEST(set);
    if (GET_CODE(dst) != REG || !rtx_equal_p(dst, x))
        return;
...
static rtx inc_for_reload(rtx reloadreg, rtx in, rtx value, int inc_amount)
{
    ...
    if (in == value)
    {
        ...
        last = get_last_insn();
        add_insn
            = emit_insn(gen_rtx_SET(VOIDmode, incloc, gen_rtx_PLUS(GET_MODE(incloc), incloc, inc)));
...
static void reload_cse_invalidate_regno(int regno, enum machine_mode mode, int clobber)
{
    ...
    for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
    {
        ...
        {
            if (XEXP(x, 0) != 0 && refers_to_regno_p(regno, endregno, XEXP(x, 0), NULL))
            {
    ...
    for (i = 0; i < regno; i++)
    {
        ...
        for (x = reg_values[i]; x; x = XEXP(x, 1))
        {
            if (XEXP(x, 0) != 0)
            {
                ...
                if (refers_to_regno_p(regno, endregno, invalidate_regno_rtx, NULL))
                {
...
static int reload_cse_mem_conflict_p(rtx mem_base, rtx val)
{
    ...
    case MEM:
        if (GET_MODE(mem_base) == BLKmode || GET_MODE(val) == BLKmode)
            return 1;
...
static void reload_cse_invalidate_mem(rtx mem_rtx)
{
    ...
    for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
    {
        ...
        for (x = reg_values[i]; x; x = XEXP(x, 1))
        {
            if (XEXP(x, 0) != 0 && reload_cse_mem_conflict_p(mem_rtx, XEXP(x, 0)))
            {
                /* If this is the only entry on the list, clear
                   reg_values[i].  Otherwise, just clear this entry on
                   the list.  */
...
static void reload_cse_invalidate_rtx(rtx dest ATTRIBUTE_UNUSED, rtx ignore)
{
...
static int reload_cse_regno_equal_p(int regno, rtx val, enum machine_mode mode)
{
    rtx x;
    ...
    for (x = reg_values[regno]; x; x = XEXP(x, 1))
        if (XEXP(x, 0) != 0 && rtx_equal_p(XEXP(x, 0), val)
            && (!flag_float_store || GET_CODE(XEXP(x, 0)) != MEM
                   || GET_MODE_CLASS(GET_MODE(x)) != MODE_FLOAT)
            && (GET_CODE(val) != CONST_INT || mode == GET_MODE(x)
...
static int reload_cse_noop_set_p(rtx set, rtx insn)
{
    rtx src, dest;
        ...
        else if (sreg >= 0)
        {
            ...
            for (x = reg_values[sreg]; x; x = XEXP(x, 1))
            {
                ...
                if (dest_mode == GET_MODE(x))
                    tmp = XEXP(x, 0);
                else if (GET_MODE_BITSIZE(dest_mode) < GET_MODE_BITSIZE(GET_MODE(x)))
                    tmp = gen_lowpart_common(dest_mode, XEXP(x, 0));
                else
                    continue;

                if (tmp && reload_cse_regno_equal_p(dreg, tmp, dest_mode))
                {
                    ret = 1;
                    break;
                }
    ...
    else if (GET_CODE(dest) == MEM)
    {
        ...
        if (sreg >= 0 && reload_cse_regno_equal_p(sreg, dest, dest_mode) && !side_effects_p(dest))
            ret = 1;
    }
...
static int reload_cse_simplify_set(rtx set, rtx insn)
{
    ...
    if (GET_CODE(src) == MEM && MEMORY_MOVE_COST(GET_MODE(src), dclass, 1) < 2)
        return 0;

    /* If the constant is cheaper than a register, don't change it.  */
    if (CONSTANT_P(src) && rtx_cost(src, SET) < 2)
        return 0;

    dest_mode = GET_MODE(SET_DEST(set));
    for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
    {
        if (i != dreg && REGISTER_MOVE_COST(REGNO_REG_CLASS(i), dclass) == 2
            && reload_cse_regno_equal_p(i, src, dest_mode))
        {
            ...
            validated = validate_change(insn, &SET_SRC(set), gen_rtx_REG(dest_mode, i), 1);
...
static int reload_cse_simplify_operands(rtx insn)
{
    ...
    for (i = 0; i < recog_n_operands; i++)
    {
        ...
        validate_change(insn, recog_operand_loc[i], gen_rtx_REG(mode, op_alt_regno[i][j]), 1);
    }
    ...
    for (i = recog_n_dups - 1; i >= 0; i--)
    {
        ...
        validate_change(insn, recog_dup_loc[i], gen_rtx_REG(mode, op_alt_regno[op][j]), 1);
    }
...
static void reload_cse_check_clobber(rtx dest ATTRIBUTE_UNUSED, rtx ignore)
{
...
static void reload_cse_record_set(rtx set, rtx body)
{
    rtx dest, src, x;
    int dreg, sreg;
    enum machine_mode dest_mode;
    ...
    x = dest;
    while (GET_CODE(x) == SUBREG || GET_CODE(x) == ZERO_EXTRACT || GET_CODE(x) == SIGN_EXTRACT
    ...
    if ((dreg < 0 && GET_CODE(dest) != MEM) || side_effects_p(src) || side_effects_p(dest))
    {
        reload_cse_invalidate_rtx(dest, NULL_RTX);
        return;
    }
    ...
    if (reg_mentioned_p(cc0_rtx, src) || reg_mentioned_p(cc0_rtx, dest))
    {
        reload_cse_invalidate_rtx(dest, NULL_RTX);
        return;
    }
        ...
        if (sreg >= 0)
        {
            ...
            for (x = reg_values[sreg]; x; x = XEXP(x, 1))
            {
                ...
                if (dest_mode == GET_MODE(XEXP(x, 0)))
                    tmp = XEXP(x, 0);
                else if (GET_MODE_BITSIZE(dest_mode) > GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))))
                    continue;
                else
                    tmp = gen_lowpart_common(dest_mode, XEXP(x, 0));
                if (tmp)
                    reg_values[dreg] = gen_rtx_EXPR_LIST(dest_mode, tmp, reg_values[dreg]);
            }
        }
        else
    ...
    else if (GET_CODE(dest) == MEM)
    {
        ...
        if (sreg >= 0 && !side_effects_p(dest))
            reg_values[sreg] = gen_rtx_EXPR_LIST(dest_mode, dest, reg_values[sreg]);
    }
    else
    {
...
static void reload_combine(void)
{
    ...
    for (insn = get_last_insn(); insn; insn = PREV_INSN(insn))
    {
        ...
        set = single_set(insn);
        if (set != NULL_RTX && GET_CODE(SET_DEST(set)) == REG
            && (HARD_REGNO_NREGS(REGNO(SET_DEST(set)), GET_MODE(SET_DEST(set))) == 1)
            && GET_CODE(SET_SRC(set)) == PLUS && GET_CODE(XEXP(SET_SRC(set), 1)) == REG
            ...
            && last_label_ruid < reg_state[REGNO(SET_DEST(set))].use_ruid)
        {
            ...
            if (TEST_HARD_REG_BIT(reg_class_contents[INDEX_REG_CLASS], regno)
                || TEST_HARD_REG_BIT(reg_class_contents[INDEX_REG_CLASS], REGNO(base)))
            {
                const_reg = reg;
                reg_sum = plus;
            }
            ...
            /* Check that PREV_SET is indeed (set (REGX) (CONST_INT)) and that
               ...
               create.  */
            if (prev_set && GET_CODE(SET_SRC(prev_set)) == CONST_INT
                && rtx_equal_p(SET_DEST(prev_set), reg) && reg_state[regno].use_index >= 0
                && reg_state[REGNO(base)].store_ruid <= reg_state[regno].use_ruid && reg_sum)
            {
                int i;
                ...
                if (reg_state[regno].offset != const0_rtx)
                    validate_change(prev, &SET_SRC(prev_set),
                        GEN_INT(INTVAL(SET_SRC(prev_set)) + INTVAL(reg_state[regno].offset)), 1);
                /* Now for every use of REG that we have recorded, replace REG
                   with REG_SUM.  */
                for (i = reg_state[regno].use_index; i < RELOAD_COMBINE_MAX_USES; i++)
                    validate_change(reg_state[regno].reg_use[i].insn,
                        reg_state[regno].reg_use[i].usep, reg_sum, 1);
                ...
                if (apply_change_group())
                {
                    ...
                    if (reg_state[regno].offset != const0_rtx)
                    {
                        ...
                        {
                            if (REG_NOTE_KIND(*np) == REG_EQUAL || REG_NOTE_KIND(*np) == REG_EQUIV)
                                *np = XEXP(*np, 1);
                            else
                                np = &XEXP(*np, 1);
                        }
        ...
        {
            if (REG_NOTE_KIND(note) == REG_INC && GET_CODE(XEXP(note, 0)) == REG)
            {
...
static void reload_combine_note_store(rtx dst, rtx set)
{
    int regno = 0;
    int i;
    ...
    if (GET_CODE(set) != SET || GET_CODE(SET_DEST(set)) == ZERO_EXTRACT
        || GET_CODE(SET_DEST(set)) == SIGN_EXTRACT || GET_CODE(SET_DEST(set)) == STRICT_LOW_PART)
    {
    ...
    for (insn = first; insn; insn = NEXT_INSN(insn), move2add_luid++)
    {
        ...
        if (GET_CODE(pat) == SET && GET_CODE(SET_DEST(pat)) == REG)
        {
            ...
            if (reg_set_luid[regno] > last_label_luid
                && (GET_MODE_SIZE(GET_MODE(reg)) <= GET_MODE_SIZE(reg_mode[regno]))
                && GET_CODE(reg_offset[regno]) == CONST_INT)
            {
                ...
                {
                    int success ATTRIBUTE_UNUSED = 0;
                    rtx new_src = GEN_INT(INTVAL(src) - INTVAL(reg_offset[regno]));
                    ...
                    if (new_src == const0_rtx)
                        ...
                        success
                            = validate_change(insn, &PATTERN(insn), gen_add2_insn(reg, new_src), 0);
                ...
                /* Try to transform (set (REGX) (REGY))
                   ...
                   to
                                    ...
                                    ...
                                    (set (REGX) (plus (REGX) (CONST_INT B-A))) */
                else if (GET_CODE(src) == REG && reg_base_reg[regno] == REGNO(src)
                    && reg_set_luid[regno] > reg_set_luid[REGNO(src)])
                {
                    ...
                    if (next)
                        set = single_set(next);
                    if (next && set && SET_DEST(set) == reg && GET_CODE(SET_SRC(set)) == PLUS
                        && XEXP(SET_SRC(set), 0) == reg
                        && GET_CODE(XEXP(SET_SRC(set), 1)) == CONST_INT)
                    {
                        rtx src3 = XEXP(SET_SRC(set), 1);
                        rtx new_src = GEN_INT(INTVAL(src3) - INTVAL(reg_offset[regno]));
                        int success = 0;

                        if (new_src == const0_rtx)
                            /* See above why we create (set (reg) (reg)) here.  */
                            success = validate_change(next, &SET_SRC(set), reg, 0);
                        else if ((rtx_cost(new_src, PLUS) < 2 + rtx_cost(src3, SET))
                            && have_add2_insn(GET_MODE(reg)))
...
static void move2add_note_store(rtx dst, rtx set)
{
    int regno = 0;
    int i;
    ...
    if (HARD_REGNO_NREGS(regno, mode) == 1 && GET_CODE(set) == SET
        && GET_CODE(SET_DEST(set)) != ZERO_EXTRACT && GET_CODE(SET_DEST(set)) != SIGN_EXTRACT
        && GET_CODE(SET_DEST(set)) != STRICT_LOW_PART)
    {
        ...
        case PLUS:
        {
            ...
            {
                if (REGNO(src0) != regno || reg_offset[regno] != const0_rtx)
                {
