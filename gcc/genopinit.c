...
const char *optabs[] = { "extendtab[(int) %B][(int) %A][0] = CODE_FOR_%(extend%a\%b2%)",
    ...
    "bcc_gen_fctn[(int) %C] = gen_%(b%c%)", "setcc_gen_code[(int) %C] = CODE_FOR_%(s%c%)",
...
static void gen_insn(rtx insn)
{
    ...
    for (pindex = 0; pindex < sizeof optabs / sizeof optabs[0]; pindex++)
    {
        ...
        for (pp += 2, np = name; matches && !(pp[0] == '%' && pp[1] == ')'); pp++)
        {
            if (*pp != '%')
            {
                if (*pp != *np++)
                ...
                case 'b':
                    ...
                    for (i = ((int)MAX_MACHINE_MODE) - 1; i >= 0; i--)
                    {
                        ...
                        if (*p == 0 && (!force_int || mode_class[i] == MODE_INT)
