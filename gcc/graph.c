...
static int node_data(FILE *fp, rtx tmp_rtx)
{
    int result = 0;
...
edge: { sourcename: \"%s.0\" targetname: \"%s.%d\" }\n",
                current_function_name, current_function_name, XINT(tmp_rtx, 0));
            break;
        case no_graph:
            break;
        }
...
label: \"%s %d\n",
            ...
            GET_CODE(tmp_rtx) == NOTE ? "lightgrey"
                                      : GET_CODE(tmp_rtx) == INSN
                    ? "green"
                    : GET_CODE(tmp_rtx) == JUMP_INSN ? "darkgreen"
                                                     : GET_CODE(tmp_rtx) == CALL_INSN
                            ? "darkgreen"
...
darkgrey\n  shape: ellipse"
    ...
    {
        static const char *note_names[] = { NULL, "deleted", "block_beg", "block_end", "loop_beg",
            "loop_end", "function_end", "setjmp", "loop_cont", "loop_vtop", "prologue_end",
            "epilogue_beg", "deleted_label", "function_beg", "eh_region_beg", "eh_region_end",
            "repeated_line_number", "range_start", "range_end", "live" };

        fprintf(fp, " %s", XINT(tmp_rtx, 4) < 0 ? note_names[-XINT(tmp_rtx, 4)] : "");
    }
...
static void end_bb(FILE *fp, int bb ATTRIBUTE_UNUSED)
{
    switch (graph_dump_format)
    {
    case vcg:
...
void print_rtl_graph_with_bb(const char *base, const char *suffix, rtx rtx_first)
{
    ...
    else
    {
       ...
       successors.  */
        ...
        for (tmp_rtx = NEXT_INSN(rtx_first); NULL != tmp_rtx; tmp_rtx = NEXT_INSN(tmp_rtx))
        {
            int did_output;
            int edge_printed = 0;
            rtx next_insn;
            ...
            if (start[INSN_UID(tmp_rtx)] < 0 && end[INSN_UID(tmp_rtx)] < 0)
            {
                if (GET_CODE(tmp_rtx) == BARRIER)
                    continue;
                if (GET_CODE(tmp_rtx) == NOTE && (1 || in_bb_p[INSN_UID(tmp_rtx)] == NOT_IN_BB))
                    continue;
            }
               ...
               basic block.  */
                for (p = s_succs[bb]; p != NULL; p = p->next)
                {
                    ...
                    if (bb_succ >= 0)
                    {
                        ...
                        draw_edge(fp, INSN_UID(tmp_rtx), INSN_UID(block_head),
                    ...
                    {
                        draw_edge(fp, INSN_UID(tmp_rtx), 999999, next_insn != 0, 0);
            ...
            if (!edge_printed)
            {
                ...
                if (next_insn == 0 || GET_CODE(next_insn) != BARRIER)
                    draw_edge(fp, XINT(tmp_rtx, 0), next_insn ? INSN_UID(next_insn) : 999999, 0, 0);
                else
                {
                    ...
                    while (next_insn
                        && (GET_CODE(next_insn) == NOTE || GET_CODE(next_insn) == BARRIER));

                    draw_edge(fp, XINT(tmp_rtx, 0), next_insn ? INSN_UID(next_insn) : 999999, 0, 2);
                }
            }
        }
...
void clean_graph_dump_file(const char *base, const char *suffix)
{
    size_t namelen = strlen(base);
    size_t suffixlen = strlen(suffix);
    size_t extlen = strlen(graph_ext[graph_dump_format]) + 1;
...
void finish_graph_dump_file(const char *base, const char *suffix)
{
    size_t namelen = strlen(base);
    size_t suffixlen = strlen(suffix);
    size_t extlen = strlen(graph_ext[graph_dump_format]) + 1;
