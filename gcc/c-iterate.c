...
static void isn_append(struct iter_stack_node *, struct iter_stack_node *);
...
void iterator_for_loop_start(tree idecl)
{
...
void iterator_for_loop_end(tree idecl)
{
...
void iterator_expand(tree stmt)
{
    tree iter_list;
...
static void expand_stmt_with_iterators_1(tree stmt, tree iter_list)
{
    if (iter_list == 0)
...
static tree collect_iterators(tree exp, tree list)
{
    if (exp == 0)
        return list;
    ...
    default:
        ...
        case '2':
        case '<':
            return collect_iterators(
                TREE_OPERAND(exp, 0), collect_iterators(TREE_OPERAND(exp, 1), list));
...
static void iterator_loop_prologue(tree idecl, rtx *start_note, rtx *end_note)
{
    tree expr;

    /* Force the save_expr in DECL_INITIAL to be calculated
       if it hasn't been calculated yet.  */
    expand_expr(DECL_INITIAL(idecl), const0_rtx, VOIDmode, EXPAND_NORMAL);
...
static void iterator_loop_epilogue(tree idecl, rtx *start_note, rtx *end_note)
{
    tree test, incr;
...
static void isn_append(struct iter_stack_node *x, struct iter_stack_node *y)
{
    if (x->first == 0)
        return;
...
void push_iterator_stack(void)
{
    struct iter_stack_node *new_top
        = (struct iter_stack_node *)obstack_alloc(&ixp_obstack, sizeof(struct iter_stack_node));
...
static void add_ixpansion(tree idecl, rtx pro_start, rtx pro_end, rtx epi_start, rtx epi_end)
{
    ...
    newix = (struct ixpansion *)obstack_alloc(&ixp_obstack, sizeof(struct ixpansion));
    newix->ixdecl = idecl;
...
static void delete_ixpansion(tree idecl)
{
    ...
    for (ix = sublevel_ixpansions.first; ix; ix = ix->next)
        if (ix->ixdecl == idecl)
        {
            ...
            if (ix->ixprologue_start == 0)
                error_with_decl(idecl, "`for (%s)' appears within implicit iteration");
            else
            {
                rtx insn;
                ...
                for (insn = NEXT_INSN(ix->ixprologue_start); insn != ix->ixprologue_end;
                     insn = NEXT_INSN(insn))
                    delete_insn(insn);
                for (insn = NEXT_INSN(ix->ixepilogue_start); insn != ix->ixepilogue_end;
                     insn = NEXT_INSN(insn))
                    delete_insn(insn);
            }
...
struct ixpansion *pixl(struct ixpansion *head)
{
...
void pis(void)
{
    ...
    for (stack_node = iter_stack; stack_node; stack_node = stack_node->next)
