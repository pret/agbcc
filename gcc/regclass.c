...
#define N_REG_INTS ((FIRST_PSEUDO_REGISTER + (HOST_BITS_PER_INT - 1)) / HOST_BITS_PER_INT)

static unsigned int_reg_class_contents[N_REG_CLASSES][N_REG_INTS] = REG_CLASS_CONTENTS;
...
static void init_reg_sets_1(void)
{
    ...
    for (i = 0; i < N_REG_CLASSES; i++)
    {
        for (j = 0; j < N_REG_CLASSES; j++)
        {
            ...
            {
                GO_IF_HARD_REG_SUBSET(reg_class_contents[k], c, subclass1);
                continue;
    ...
    for (i = 0; i < N_REG_CLASSES; i++)
    {
        ...
        for (j = i + 1; j < N_REG_CLASSES; j++)
        {
            ...
            GO_IF_HARD_REG_SUBSET(reg_class_contents[i], reg_class_contents[j], subclass);
            continue;
        subclass:
...
enum machine_mode choose_hard_reg_mode(int regno, int nregs)
{
    ...
    for (mode = GET_CLASS_NARROWEST_MODE(MODE_INT); mode != VOIDmode;
        ...
        if (HARD_REGNO_NREGS(regno, mode) == nregs && HARD_REGNO_MODE_OK(regno, mode))
            found_mode = mode;
    ...
    if (HARD_REGNO_NREGS(regno, CCmode) == nregs && HARD_REGNO_MODE_OK(regno, CCmode))
        return CCmode;
...
void fix_register(char *name, int fixed, int call_used)
{
    ...
    if ((i = decode_reg_name(name)) >= 0)
    {
        if ((i == STACK_POINTER_REGNUM || i == FRAME_POINTER_REGNUM)
            && (fixed == 0 || call_used == 0))
        {
            static const char *what_option[2][2]
                = { { "call-saved", "call-used" }, { "no-such-option", "fixed" } };

            error("can't use '%s' as a %s register", name, what_option[fixed][call_used]);
        }
        else
        {
            fixed_regs[i] = fixed;
...
static void record_reg_classes(int, int, rtx *, enum machine_mode *, const char **, rtx);
...
static rtx scan_one_insn(rtx insn, int pass)
{
    ...
    if (pat_code == USE || pat_code == CLOBBER || pat_code == ASM_INPUT || pat_code == ADDR_VEC
    ...
    if (set != 0 && GET_CODE(SET_DEST(set)) == REG && GET_CODE(SET_SRC(set)) == MEM
        && (note = find_reg_note(insn, REG_EQUIV, NULL_RTX)) != 0 && GET_CODE(XEXP(note, 0)) == MEM)
    {
        costs[REGNO(SET_DEST(set))].mem_cost
            -= (MEMORY_MOVE_COST(GET_MODE(SET_DEST(set)), GENERAL_REGS, 1) * loop_cost);
        record_address_regs(XEXP(SET_SRC(set), 0), BASE_REG_CLASS, loop_cost * 2);
        return insn;
    }
    ...
    if (pass == 0 && optimize && recog_n_operands >= 3 && recog_constraints[1][0] == '0'
        && recog_constraints[1][1] == 0 && CONSTANT_P(recog_operand[1])
        && !rtx_equal_p(recog_operand[0], recog_operand[1])
        && !rtx_equal_p(recog_operand[0], recog_operand[2]) && GET_CODE(recog_operand[0]) == REG
        && MODES_TIEABLE_P(GET_MODE(recog_operand[0]), recog_operand_mode[1]))
    {
        rtx previnsn = prev_real_insn(insn);
        rtx dest = gen_lowpart(recog_operand_mode[1], recog_operand[0]);
        rtx newinsn = emit_insn_before(gen_move_insn(dest, recog_operand[1]), insn);
    ...
    for (i = 0; i < recog_n_operands; i++)
    {
        ...
        if (GET_CODE(recog_operand[i]) == MEM)
            record_address_regs(XEXP(recog_operand[i], 0), BASE_REG_CLASS, loop_cost * 2);
        else if (constraints[i][0] == 'p')
            record_address_regs(recog_operand[i], BASE_REG_CLASS, loop_cost * 2);
    }
    ...
    for (i = 0; i < recog_n_operands - 1; i++)
        if (constraints[i][0] == '%')
        {
            ...
            record_reg_classes(
                recog_n_alternatives, recog_n_operands, recog_operand, modes, xconstraints, insn);
        }

    record_reg_classes(
        recog_n_alternatives, recog_n_operands, recog_operand, modes, constraints, insn);

    /* Now add the cost for each operand to the total costs for
       its register.  */

    for (i = 0; i < recog_n_operands; i++)
        if (GET_CODE(recog_operand[i]) == REG && REGNO(recog_operand[i]) >= FIRST_PSEUDO_REGISTER)
        {
                ...
                for (m = VOIDmode; (int)m < (int)MAX_MACHINE_MODE;
                    ...
                    if (HARD_REGNO_MODE_OK(j, m))
                    {
                        ...
                        if ((0
                                ...
                                || (SECONDARY_RELOAD_CLASS(BASE_REG_CLASS, m, r) != NO_REGS)
#else
#ifdef SECONDARY_INPUT_RELOAD_CLASS
                                || (SECONDARY_INPUT_RELOAD_CLASS(BASE_REG_CLASS, m, r) != NO_REGS)
#endif
#ifdef SECONDARY_OUTPUT_RELOAD_CLASS
                                || (SECONDARY_OUTPUT_RELOAD_CLASS(BASE_REG_CLASS, m, r) != NO_REGS)
#endif
#endif
                                    )
        ...
        for (i = FIRST_PSEUDO_REGISTER; i < nregs; i++)
        {
            ...
            for (rclass = (int)ALL_REGS - 1; rclass > 0; rclass --)
            {
               ...
               invalid for a operand that was auto-incremented.  */
                if (CLASS_MAX_NREGS(rclass, PSEUDO_REGNO_MODE(i)) > reg_class_size[rclass]
#ifdef FORBIDDEN_INC_DEC_CLASSES
...
static void record_reg_classes(
    int n_alts, int n_ops, rtx *ops, enum machine_mode *modes, const char **constraints, rtx insn)
{
    int alt;
    int i, j;
    rtx set;
    ...
    for (alt = 0; alt < n_alts; alt++)
    {
        ...
        for (i = 0; i < n_ops; i++)
        {
            ...
            if (p[0] >= '0' && p[0] <= '0' + i && (p[1] == ',' || p[1] == 0))
            {
                ...
                else if (GET_CODE(ops[j]) != REG || REGNO(ops[j]) < FIRST_PSEUDO_REGISTER)
                {
            ...
            while (*p && (c = *p++) != ',')
                ...
                case '>':
                    if (GET_CODE(op) == MEM
                        && (GET_CODE(XEXP(op, 0)) == PRE_INC || GET_CODE(XEXP(op, 0)) == POST_INC))
                        win = 1;
                    break;
                ...
                case 'G':
                case 'H':
                    if (GET_CODE(op) == CONST_DOUBLE && CONST_DOUBLE_OK_FOR_LETTER_P(op, c))
                        win = 1;
                    break;
                ...
                case 'O':
                case 'P':
                    if (GET_CODE(op) == CONST_INT && CONST_OK_FOR_LETTER_P(INTVAL(op), c))
                        win = 1;
                    break;
                ...
                case 'g':
                    if (GET_CODE(op) == MEM || (CONSTANT_P(op)))
                        win = 1;
                    allows_mem = 1;
                case 'r':
                    classes[i] = reg_class_subunion[(int)classes[i]][(int)GENERAL_REGS];
                    break;

                default:
                    classes[i] = reg_class_subunion[(int)classes[i]][(int)REG_CLASS_FROM_LETTER(c)];
                }
            ...
            if (GET_CODE(op) == REG && REGNO(op) >= FIRST_PSEUDO_REGISTER)
            {
                ...
                else
                {
                    ...
                    for (rclass = 0; rclass < N_REG_CLASSES; rclass ++)
                        pp->cost[rclass] = may_move_cost[rclass][(int)classes[i]];

                    /* If the alternative actually allows memory, make things
                       a bit cheaper since we won't need an extra insn to
                       load it.  */

                    pp->mem_cost = (MEMORY_MOVE_COST(mode, classes[i], 1) - allows_mem);
            ...
            /* Otherwise, if this alternative wins, either because we
               ...
               the proper class, there is no cost for this alternative.  */
                ...
                || (GET_CODE(op) == REG && reg_fits_class_p(op, classes[i], 0, GET_MODE(op))))
                ;
       ...
       about this alternative.  */
        ...
        for (i = 0; i < n_ops; i++)
            if (GET_CODE(ops[i]) == REG && REGNO(ops[i]) >= FIRST_PSEUDO_REGISTER)
            {
                struct costs *pp = &op_costs[i], *qq = &this_op_costs[i];
                int scale = 1 + (recog_op_type[i] == OP_INOUT);

                pp->mem_cost = MIN(pp->mem_cost, (qq->mem_cost + alt_cost) * scale);

                for (rclass = 0; rclass < N_REG_CLASSES; rclass ++)
                    pp->cost[rclass] = MIN(pp->cost[rclass], (qq->cost[rclass] + alt_cost) * scale);
            }
    }
    ...
    if ((set = single_set(insn)) != 0 && ops[0] == SET_DEST(set) && ops[1] == SET_SRC(set)
                ...
                if (regno >= FIRST_PSEUDO_REGISTER && prefclass != 0
                    ...
                    for (rclass = 0; rclass < N_REG_CLASSES; rclass++)
                        if (TEST_HARD_REG_BIT(reg_class_contents[rclass], regno)
                            && reg_class_size[rclass] == CLASS_MAX_NREGS(rclass, mode))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    if (!TEST_HARD_REG_BIT(reg_class_contents[rclass], regno + nr))
                                        break;
                                }
...
static int copy_cost(rtx x, enum machine_mode mode, enum reg_class rclass, int to_p)
{
#ifdef HAVE_SECONDARY_RELOADS
...
#ifdef SECONDARY_INPUT_RELOAD_CLASS
    if (to_p == 1)
        secondary_class = SECONDARY_INPUT_RELOAD_CLASS(rclass, mode, x);
#endif

#ifdef SECONDARY_OUTPUT_RELOAD_CLASS
    if (!to_p)
        secondary_class = SECONDARY_OUTPUT_RELOAD_CLASS(rclass, mode, x);
#endif

    if (secondary_class != NO_REGS)
        return (
            move_cost[(int)secondary_class][(int)rclass] + copy_cost(x, mode, secondary_class, 2));
#endif /* HAVE_SECONDARY_RELOADS */
    ...
    if (GET_CODE(x) == MEM || rclass == NO_REGS)
        return MEMORY_MOVE_COST(mode, rclass, to_p);

    else if (GET_CODE(x) == REG)
        return move_cost[(int)REGNO_REG_CLASS(REGNO(x))][(int)rclass];
...
static void record_address_regs(rtx x, enum reg_class rclass, int scale)
{
       ...
       the register is being used as a pointer.  */

        {
            ...
            if (MAX_REGS_PER_ADDRESS == 1)
                record_address_regs(arg0, rclass, scale);
            ...
            else if (INDEX_REG_CLASS == BASE_REG_CLASS)
            {
                record_address_regs(arg0, rclass, scale);
                if (!CONSTANT_P(arg1))
                    record_address_regs(arg1, rclass, scale);
            }
            ...
            else if (code0 == REG && code1 == REG && REGNO(arg0) < FIRST_PSEUDO_REGISTER
                && (REG_OK_FOR_BASE_P(arg0) || REG_OK_FOR_INDEX_P(arg0)))
                record_address_regs(
                    arg1, REG_OK_FOR_BASE_P(arg0) ? INDEX_REG_CLASS : BASE_REG_CLASS, scale);
            else if (code0 == REG && code1 == REG && REGNO(arg1) < FIRST_PSEUDO_REGISTER
                && (REG_OK_FOR_BASE_P(arg1) || REG_OK_FOR_INDEX_P(arg1)))
                record_address_regs(
                    arg0, REG_OK_FOR_BASE_P(arg1) ? INDEX_REG_CLASS : BASE_REG_CLASS, scale);
#endif
            ...
            else if ((code0 == REG && REGNO_POINTER_FLAG(REGNO(arg0))) || code1 == MULT)
            {
                record_address_regs(arg0, BASE_REG_CLASS, scale);
                record_address_regs(arg1, INDEX_REG_CLASS, scale);
            }
            else if ((code1 == REG && REGNO_POINTER_FLAG(REGNO(arg1))) || code0 == MULT)
            {
                record_address_regs(arg0, INDEX_REG_CLASS, scale);
    ...
    case PRE_DEC:
       ...
       show it is being used in an INC_DEC context.  */
        ...
        record_address_regs(XEXP(x, 0), rclass, 2 * scale);
        break;
    ...
    case REG:
    {
        ...
        for (i = 0; i < N_REG_CLASSES; i++)
            pp->cost[i] += (may_move_cost[i][(int)rclass] * scale) / 2;
    }
    break;
    ...
    default:
    {
        ...
        for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
            if (fmt[i] == 'e')
                record_address_regs(XEXP(x, i), rclass, scale);
    }
    }
}
...
static int auto_inc_dec_reg_p(rtx reg, enum machine_mode mode)
{
    if (HAVE_POST_INCREMENT && memory_address_p(mode, gen_rtx_POST_INC(Pmode, reg)))
        return 1;

    if (HAVE_POST_DECREMENT && memory_address_p(mode, gen_rtx_POST_DEC(Pmode, reg)))
        return 1;

    if (HAVE_PRE_INCREMENT && memory_address_p(mode, gen_rtx_PRE_INC(Pmode, reg)))
        return 1;

    if (HAVE_PRE_DECREMENT && memory_address_p(mode, gen_rtx_PRE_DEC(Pmode, reg)))
        return 1;
    ...
    if (num_regs > regno_allocated)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                prefclass_buffer = (char *)xrealloc((char *)prefclass_buffer, regno_allocated);

                altclass_buffer = (char *)xrealloc((char *)altclass_buffer, regno_allocated);
            }
        }
...
void reg_scan(rtx f, int nregs, int repeat)
{
    ...
    for (insn = f; insn; insn = NEXT_INSN(insn))
        if (GET_CODE(insn) == INSN || GET_CODE(insn) == CALL_INSN || GET_CODE(insn) == JUMP_INSN)
        {
            if (GET_CODE(PATTERN(insn)) == PARALLEL && XVECLEN(PATTERN(insn), 0) > max_parallel)
...
void reg_scan_update(rtx first, rtx last, int old_max_regno)
{
    rtx insn;
...
static void reg_scan_mark_refs(rtx x, rtx insn, int note_flag, int min_regno)
{
    enum rtx_code code;
    rtx dest;
    rtx note;
    ...
    case SET:
        ...
        for (dest = SET_DEST(x); GET_CODE(dest) == SUBREG || GET_CODE(dest) == STRICT_LOW_PART
             || GET_CODE(dest) == ZERO_EXTEND;
             dest = XEXP(dest, 0))
            ;

        if (GET_CODE(dest) == REG && REGNO(dest) >= min_regno)
       ...
       on the type.  */
        ...
        if (GET_CODE(SET_DEST(x)) == REG && REGNO(SET_DEST(x)) >= FIRST_PSEUDO_REGISTER
            ...
            && REG_N_SETS(REGNO(SET_DEST(x))) == 1 && !REG_USERVAR_P(SET_DEST(x))
            && !REGNO_POINTER_FLAG(REGNO(SET_DEST(x)))
            && ((GET_CODE(SET_SRC(x)) == REG && REGNO_POINTER_FLAG(REGNO(SET_SRC(x))))
                   || ((GET_CODE(SET_SRC(x)) == PLUS || GET_CODE(SET_SRC(x)) == LO_SUM)
                          && GET_CODE(XEXP(SET_SRC(x), 1)) == CONST_INT
                          && GET_CODE(XEXP(SET_SRC(x), 0)) == REG
                          && REGNO_POINTER_FLAG(REGNO(XEXP(SET_SRC(x), 0))))
                   || GET_CODE(SET_SRC(x)) == CONST || GET_CODE(SET_SRC(x)) == SYMBOL_REF
...
int reg_class_subset_p(enum reg_class c1, enum reg_class c2)
{
    ...
    win:
        return 1;
    GO_IF_HARD_REG_SUBSET(reg_class_contents[(int)c1], reg_class_contents[(int)c2], win);
    return 0;
}
...
void regset_release_memory(void)
{
