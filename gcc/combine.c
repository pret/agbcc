...
static uint32_t *reg_last_set_nonzero_bits;
...
/* Similar to SUBST, but NEWVAL is an int expression.  Note that substitution
   for the value of a int32_t value (including CONST_INT) is
   not safe.  */
...
static rtx make_extraction(enum machine_mode, rtx, int, rtx, int, int, int, int);
static rtx extract_left_shift(rtx, int);
static rtx make_compound_operation(rtx, enum rtx_code);
static int get_pos_from_mask(uint32_t, int *);
static rtx force_to_mode(rtx, enum machine_mode, uint32_t, rtx, int);
static rtx if_then_else_cond(rtx, rtx *, rtx *);
static rtx known_cond(rtx, enum rtx_code, rtx, rtx);
static int rtx_equal_for_field_assignment_p(rtx, rtx);
static rtx make_field_assignment(rtx);
static rtx apply_distributive_law(rtx);
static rtx simplify_and_const_int(rtx, enum machine_mode, rtx, uint32_t);
static uint32_t nonzero_bits(rtx, enum machine_mode);
static int num_sign_bit_copies(rtx, enum machine_mode);
static int merge_outer_ops(
    enum rtx_code *, int32_t *, enum rtx_code, int32_t, enum machine_mode, int *);
...
static rtx gen_unary(enum rtx_code, enum machine_mode, enum machine_mode, rtx);
...
void combine_instructions(rtx f, int nregs)
{
    ...
    reg_nonzero_bits = (uint32_t *)alloca(nregs * sizeof(int32_t));
    reg_sign_bit_copies = (char *)alloca(nregs * sizeof(char));

    zero_memory((char *)reg_nonzero_bits, nregs * sizeof(int32_t));
    ...
    reg_last_set_mode = (enum machine_mode *)alloca(nregs * sizeof(enum machine_mode));
    reg_last_set_nonzero_bits = (uint32_t *)alloca(nregs * sizeof(int32_t));
    reg_last_set_sign_bit_copies = (char *)alloca(nregs * sizeof(char));
    ...
    nonzero_bits_mode = mode_for_size(32, MODE_INT, 0);
    ...
    for (insn = f; insn; insn = next ? next : NEXT_INSN(insn))
    {
        ...
        if (this_basic_block + 1 < n_basic_blocks && BLOCK_HEAD(this_basic_block + 1) == insn)
            this_basic_block++;
        ...
        else if (GET_RTX_CLASS(GET_CODE(insn)) == 'i')
        {
            ...
            for (links = LOG_LINKS(insn); links; links = XEXP(links, 1))
                for (nextlinks = LOG_LINKS(XEXP(links, 0)); nextlinks;
                    ...
                    if ((next = try_combine(insn, XEXP(links, 0), XEXP(nextlinks, 0))) != 0)
                        goto retry;
            ...
            if (GET_CODE(insn) == JUMP_INSN && (prev = prev_nonnote_insn(insn)) != 0
                && GET_CODE(prev) == INSN && sets_cc0_p(PATTERN(prev)))
            {
                if ((next = try_combine(insn, prev, NULL_RTX)) != 0)
                    goto retry;

                for (nextlinks = LOG_LINKS(prev); nextlinks; nextlinks = XEXP(nextlinks, 1))
                    if ((next = try_combine(insn, prev, XEXP(nextlinks, 0))) != 0)
                        goto retry;
            }

            /* Do the same for an insn that explicitly references CC0.  */
            if (GET_CODE(insn) == INSN && (prev = prev_nonnote_insn(insn)) != 0
                && GET_CODE(prev) == INSN && sets_cc0_p(PATTERN(prev))
            ...
            for (links = LOG_LINKS(insn); links; links = XEXP(links, 1))
                if (GET_CODE(XEXP(links, 0)) == INSN && GET_CODE(PATTERN(XEXP(links, 0))) == SET
                    && reg_mentioned_p(cc0_rtx, SET_SRC(PATTERN(XEXP(links, 0))))
                    && (prev = prev_nonnote_insn(XEXP(links, 0))) != 0 && GET_CODE(prev) == INSN
...
static void init_reg_last_arrays(void)
{
    ...
    zero_memory((char *)reg_last_set_nonzero_bits, nregs * sizeof(int32_t));
...
/* Called via note_stores.  If X is a pseudo that is narrower than
   32 and is being set, record what bits are known zero.
...
static void set_nonzero_bits_and_sign_copies(rtx x, rtx set)
{
    int num;
    ...
    if (GET_CODE(x) == REG
        ...
        && GET_MODE_BITSIZE(GET_MODE(x)) <= 32)
    {
                   ...
                   && SUBREG_REG(SET_DEST(set)) == x))
        {
            ...
            if (GET_MODE_BITSIZE(GET_MODE(x)) < BITS_PER_WORD && GET_CODE(src) == CONST_INT
                && INTVAL(src) > 0
                && 0 != (INTVAL(src) & ((int32_t)1 << (GET_MODE_BITSIZE(GET_MODE(x)) - 1))))
                src = GEN_INT(INTVAL(src) | ((int32_t)(-1) << GET_MODE_BITSIZE(GET_MODE(x))));
#endif

            reg_nonzero_bits[REGNO(x)] |= nonzero_bits(src, nonzero_bits_mode);
            num = num_sign_bit_copies(SET_SRC(set), GET_MODE(x));
            if (reg_sign_bit_copies[REGNO(x)] == 0 || reg_sign_bit_copies[REGNO(x)] > num)
...
static int can_combine_p(rtx insn ATTRIBUTE_UNUSED, rtx i3, rtx pred, rtx succ, rtx *pdest, rtx *psrc)
{
    int i;
    rtx set = 0, src, dest;
    rtx p;
    ...
    rtx link;
#endif
    int all_adjacent = (succ ? (next_active_insn(insn) == succ && next_active_insn(succ) == i3)
    ...
    if (GET_CODE(PATTERN(insn)) == SET)
        set = PATTERN(insn);
    else if (GET_CODE(PATTERN(insn)) == PARALLEL && GET_CODE(XVECEXP(PATTERN(insn), 0, 0)) == SET)
    {
        for (i = 0; i < XVECLEN(PATTERN(insn), 0); i++)
        {
            ...
            case USE:
                ...
                if (GET_CODE(XEXP(elt, 0)) == REG && GET_CODE(PATTERN(i3)) == PARALLEL)
                {
                    ...
                    do
                    {
                        ...
                        if (GET_CODE(i3elt) == USE && GET_CODE(XEXP(i3elt, 0)) == REG
                            && (REGNO(XEXP(i3elt, 0)) == regno
                                       ? reg_set_between_p(XEXP(elt, 0), PREV_INSN(insn), i3)
            ...
            case SET:
                /* Ignore SETs whose result isn't used but not those that
                   have side-effects.  */
                if (find_reg_note(insn, REG_UNUSED, SET_DEST(elt)) && !side_effects_p(elt))
                    break;
    ...
    if (GET_CODE(PATTERN(i3)) == PARALLEL)
        for (i = XVECLEN(PATTERN(i3), 0) - 1; i >= 0; i--)
            if (GET_CODE(XVECEXP(PATTERN(i3), 0, i)) == CLOBBER
                && (reg_overlap_mentioned_p(XEXP(XVECEXP(PATTERN(i3), 0, i), 0), src)
    ...
    if (GET_CODE(src) == ASM_OPERANDS || volatile_refs_p(src))
    {
        ...
        for (p = NEXT_INSN(insn); p != i3; p = NEXT_INSN(p))
            if (GET_RTX_CLASS(GET_CODE(p)) == 'i' && p != succ && volatile_refs_p(PATTERN(p)))
                return 0;
    }
    ...
    if (GET_CODE(src) == ASM_OPERANDS && GET_CODE(dest) == REG
        && REGNO(dest) < FIRST_PSEUDO_REGISTER)
        return 0;
    ...
    for (p = NEXT_INSN(insn); p != i3; p = NEXT_INSN(p))
        if (GET_RTX_CLASS(GET_CODE(p)) == 'i' && p != succ && volatile_insn_p(PATTERN(p)))
            return 0;
    ...
    for (link = REG_NOTES(insn); link; link = XEXP(link, 1))
        if (REG_NOTE_KIND(link) == REG_INC
            && (GET_CODE(i3) == JUMP_INSN || reg_used_between_p(XEXP(link, 0), insn, i3)
    ...
    if (p && p != pred && GET_CODE(p) == INSN && sets_cc0_p(PATTERN(p)) && !all_adjacent)
        return 0;
#endif
...
static int sets_function_arg_p(rtx pat)
{
    ...
    case SET:
        ...
        return (GET_CODE(inner_dest) == REG && REGNO(inner_dest) < FIRST_PSEUDO_REGISTER
...
static int combinable_i3pat(
    rtx i3, rtx *loc, rtx i2dest, rtx i1dest, int i1_not_in_src, rtx *pi3dest_killed)
{
    rtx x = *loc;
    ...
    if (GET_CODE(x) == SET)
    {
        ...
        if ((inner_dest != dest
            ...
            || (GET_CODE(inner_dest) == REG && REGNO(inner_dest) < FIRST_PSEUDO_REGISTER
                   && (!HARD_REGNO_MODE_OK(REGNO(inner_dest), GET_MODE(inner_dest))
                          || (SMALL_REGISTER_CLASSES && GET_CODE(src) != CALL
                                 && !REG_USERVAR_P(inner_dest)
                                 && (FUNCTION_VALUE_REGNO_P(REGNO(inner_dest))
                                        || (FUNCTION_ARG_REGNO_P(REGNO(inner_dest)) && i3 != 0
        ...
        if (pi3dest_killed && GET_CODE(dest) == REG && reg_referenced_p(dest, PATTERN(i3))
            && REGNO(dest) != FRAME_POINTER_REGNUM
            && (REGNO(dest) != ARG_POINTER_REGNUM || !fixed_regs[REGNO(dest)])
    ...
    else if (GET_CODE(x) == PARALLEL)
    {
        ...
        for (i = 0; i < XVECLEN(x, 0); i++)
            if (!combinable_i3pat(
                    i3, &XVECEXP(x, 0, i), i2dest, i1dest, i1_not_in_src, pi3dest_killed))
                return 0;
    }
...
static rtx try_combine(rtx i3, rtx i2, rtx i1)
{
    ...
    if (i1 == 0 && GET_CODE(i3) == INSN && GET_CODE(PATTERN(i3)) == SET
        ...
        && find_reg_note(i3, REG_DEAD, SET_SRC(PATTERN(i3))) && GET_CODE(PATTERN(i2)) == PARALLEL
        ...
        && !reg_overlap_mentioned_p(SET_SRC(PATTERN(i3)), SET_DEST(PATTERN(i3)))
        && next_real_insn(i2) == i3)
    {
        ...
        for (i = 0; i < XVECLEN(p2, 0); i++)
            if ((GET_CODE(XVECEXP(p2, 0, i)) == SET || GET_CODE(XVECEXP(p2, 0, i)) == CLOBBER)
                && reg_overlap_mentioned_p(SET_DEST(PATTERN(i3)), SET_DEST(XVECEXP(p2, 0, i))))
                break;
    ...
    if (i1 == 0 && GET_CODE(PATTERN(i2)) == PARALLEL && XVECLEN(PATTERN(i2), 0) >= 2
        && GET_CODE(XVECEXP(PATTERN(i2), 0, 0)) == SET
        && (GET_MODE_CLASS(GET_MODE(SET_DEST(XVECEXP(PATTERN(i2), 0, 0)))) == MODE_CC)
        && GET_CODE(SET_SRC(XVECEXP(PATTERN(i2), 0, 0))) == COMPARE
        && XEXP(SET_SRC(XVECEXP(PATTERN(i2), 0, 0)), 1) == const0_rtx
        && GET_CODE(XVECEXP(PATTERN(i2), 0, 1)) == SET
        && GET_CODE(SET_DEST(XVECEXP(PATTERN(i2), 0, 1))) == REG
        && rtx_equal_p(
               XEXP(SET_SRC(XVECEXP(PATTERN(i2), 0, 0)), 0), SET_SRC(XVECEXP(PATTERN(i2), 0, 1))))
    {
        ...
        if (i == 1)
        {
            ...
            subst_prev_insn = i1 = gen_rtx_INSN(VOIDmode, INSN_UID(i2), NULL_RTX, i2,
                XVECEXP(PATTERN(i2), 0, 1), -1, NULL_RTX, NULL_RTX);

            SUBST(PATTERN(i2), XVECEXP(PATTERN(i2), 0, 0));
            SUBST(XEXP(SET_SRC(PATTERN(i2)), 0), SET_DEST(PATTERN(i1)));
        }
    }
#endif
    ...
    if (!combinable_i3pat(
            i3, &PATTERN(i3), i2dest, i1dest, i1 && i2dest_in_i1src && i1_feeds_i3, &i3dest_killed))
    {
        undo_all();
        return 0;
    }
    ...
    /* See if any of the insns is a MULT operation.  Unless one is, we will
       ...
       here.  */
    if (GET_CODE(i2src) == MULT || (i1 != 0 && GET_CODE(i1src) == MULT)
        || (GET_CODE(PATTERN(i3)) == SET && GET_CODE(SET_SRC(PATTERN(i3))) == MULT))
        have_mult = 1;
    ...
    for (link = REG_NOTES(i3); link; link = XEXP(link, 1))
        if (REG_NOTE_KIND(link) == REG_INC
            && (reg_overlap_mentioned_p(XEXP(link, 0), PATTERN(i2))
                   || (i1 != 0 && reg_overlap_mentioned_p(XEXP(link, 0), PATTERN(i1)))))
        {
            undo_all();
            return 0;
        }
    ...
    i2pat
        = (GET_CODE(PATTERN(i2)) == PARALLEL ? gen_rtx_SET(VOIDmode, i2dest, i2src) : PATTERN(i2));
    ...
    if (current_function_uses_expect)
    {
        ...
        /* Convert:
           ...
           into:
                  ...
                  (set (pc)
                       (if_then_else (<comparison> (expect (register)
           (const_int)) (const_int 0)) (const_int)) (label_ref ...) (pc))) */
        if (i2 && i3 && (set2 = single_set(i2)) != NULL_RTX && GET_CODE(SET_DEST(set2)) == REG
            && GET_CODE((compare = SET_SRC(set2))) == COMPARE
            && GET_CODE((expect = XEXP(compare, 0))) == EXPECT
            && GET_CODE((expint = XEXP(compare, 1))) == CONST_INT && GET_CODE(i3) == JUMP_INSN
            && (set3 = single_set(i3)) != NULL_RTX && SET_DEST(set3) == pc_rtx
            && GET_CODE(SET_SRC(set3)) == IF_THEN_ELSE
            && GET_RTX_CLASS(GET_CODE((test = XEXP(SET_SRC(set3), 0)))) == '<'
            && rtx_equal_p(SET_DEST(set2), (expreg = XEXP(XEXP(SET_SRC(set3), 0), 0))))
        {
            int32_t cmp_value = INTVAL(XEXP(expect, 1));
            int32_t exp_value = INTVAL(expint);
            uint32_t ucmp_value = cmp_value;
            uint32_t uexp_value = exp_value;
            int32_t value;
            ...
            newi2pat = gen_rtx(SET, VOIDmode, SET_DEST(set2),
                gen_rtx(COMPARE, GET_MODE(SET_SRC(set2)), XEXP(expect, 0), expint));

            newpat = gen_rtx(SET, VOIDmode, pc_rtx,
                gen_rtx(IF_THEN_ELSE, VOIDmode,
                    gen_rtx(GET_CODE(test), GET_MODE(test),
                        gen_rtx(EXPECT, GET_MODE(SET_DEST(set2)), SET_DEST(set2), GEN_INT(value)),
                        const0_rtx),
                    XEXP(SET_SRC(set3), 1), XEXP(SET_SRC(set3), 2)));
    ...
    if (i1 == 0 && added_sets_2 && GET_CODE(PATTERN(i3)) == SET
        && GET_CODE(SET_SRC(PATTERN(i3))) == COMPARE && XEXP(SET_SRC(PATTERN(i3)), 1) == const0_rtx
        && rtx_equal_p(XEXP(SET_SRC(PATTERN(i3)), 0), i2dest))
    {
        ...
        if (undobuf.other_insn == 0
            && (cc_use = find_single_use(SET_DEST(newpat), i3, &undobuf.other_insn))
            && ((compare_mode = SELECT_CC_MODE(GET_CODE(*cc_use), i2src, const0_rtx))
                   != GET_MODE(SET_DEST(newpat))))
        {
            ...
            if (regno < FIRST_PSEUDO_REGISTER
                || (REG_N_SETS(regno) == 1 && !added_sets_2 && !REG_USERVAR_P(SET_DEST(newpat))))
            {
                if (regno >= FIRST_PSEUDO_REGISTER)
                ...
                SUBST(SET_SRC(newpat), gen_rtx_combine(COMPARE, compare_mode, i2src, const0_rtx));
            }
            else
                undobuf.other_insn = 0;
        }
...
#endif
    {
        ...
        subst_low_cuid = INSN_CUID(i2);
        newpat = subst(PATTERN(i3), i2dest, i2src, 0, !i1_feeds_i3 && i1dest_in_i1src);
        undobuf.previous_undos = undobuf.undos;
    ...
    if (i1 && GET_CODE(newpat) != CLOBBER)
    {
        ...
        if (!combinable_i3pat(NULL_RTX, &newpat, i1dest, NULL_RTX, 0, NULL))
        {
            undo_all();
            return 0;
        }
    ...
    if ((FIND_REG_INC_NOTE(i2, NULL_RTX) != 0 && i2_is_used + added_sets_2 > 1)
        || (i1 != 0 && FIND_REG_INC_NOTE(i1, NULL_RTX) != 0
               && (n_occurrences + added_sets_1 + (added_sets_2 && !i1_feeds_i3) > 1))
        ...
        /* Fail if this new pattern is a MULT and we didn't have one before
           at the outer level.  */
        || (GET_CODE(newpat) == SET && GET_CODE(SET_SRC(newpat)) == MULT && !have_mult))
    {
        undo_all();
        return 0;
    }
    ...
    if (added_sets_1 || added_sets_2)
    {
        ...
        if (added_sets_1)
            XVECEXP(newpat, 0, --total_sets)
                = (GET_CODE(PATTERN(i1)) == PARALLEL ? gen_rtx_SET(VOIDmode, i1dest, i1src)
                                                     : PATTERN(i1));
        ...
        if (added_sets_2)
        {
            ...
            if (i1 == 0)
                ...
                XVECEXP(newpat, 0, --total_sets) = subst(i2pat, i1dest, i1src, 0, 0);
        }
    }
...
validate_replacement:
    ...
    if (insn_code_number < 0 && GET_CODE(newpat) == PARALLEL && XVECLEN(newpat, 0) == 2
        && GET_CODE(XVECEXP(newpat, 0, 0)) == SET && GET_CODE(XVECEXP(newpat, 0, 1)) == SET
        && GET_CODE(SET_DEST(XVECEXP(newpat, 0, 1))) == REG
        && find_reg_note(i3, REG_UNUSED, SET_DEST(XVECEXP(newpat, 0, 1)))
        && !side_effects_p(SET_SRC(XVECEXP(newpat, 0, 1))) && asm_noperands(newpat) < 0)
    {
        newpat = XVECEXP(newpat, 0, 0);
        insn_code_number = recog_for_combine(&newpat, i3, &new_i3_notes);
    }

    else if (insn_code_number < 0 && GET_CODE(newpat) == PARALLEL && XVECLEN(newpat, 0) == 2
        && GET_CODE(XVECEXP(newpat, 0, 0)) == SET && GET_CODE(XVECEXP(newpat, 0, 1)) == SET
        && GET_CODE(SET_DEST(XVECEXP(newpat, 0, 0))) == REG
        && find_reg_note(i3, REG_UNUSED, SET_DEST(XVECEXP(newpat, 0, 0)))
        && !side_effects_p(SET_SRC(XVECEXP(newpat, 0, 0))) && asm_noperands(newpat) < 0)
    {
    ...
    if (i1 && insn_code_number < 0 && GET_CODE(newpat) == SET && asm_noperands(newpat) < 0)
    {
        rtx m_split, *split;
        rtx ni2dest = i2dest;
        ...
        if (m_split == 0 && !reg_overlap_mentioned_p(ni2dest, newpat))
        {
            ...
            if (GET_MODE(SET_DEST(newpat)) != GET_MODE(i2dest)
                && GET_MODE(SET_DEST(newpat)) != VOIDmode && GET_CODE(i2dest) == REG
                && (REGNO(i2dest) < FIRST_PSEUDO_REGISTER
                       || (REG_N_SETS(REGNO(i2dest)) == 1 && !added_sets_2
                              && !REG_USERVAR_P(i2dest))))
                ni2dest = gen_rtx_REG(GET_MODE(SET_DEST(newpat)), REGNO(i2dest));

            m_split = split_insns(gen_rtx_PARALLEL(VOIDmode,
                                      gen_rtvec(2, newpat, gen_rtx_CLOBBER(VOIDmode, ni2dest))),
                i3);
        }

        if (m_split && GET_CODE(m_split) == SEQUENCE && XVECLEN(m_split, 0) == 2
            && (next_real_insn(i2) == i3
                   || !use_crosses_set_p(PATTERN(XVECEXP(m_split, 0, 0)), INSN_CUID(i2))))
        {
            rtx i2set, i3set;
            ...
            if (i2_code_number >= 0 && i2set && i3set
                && (next_real_insn(i2) == i3 || !reg_used_between_p(SET_DEST(i2set), i2, i3)))
            ...
            if (insn_code_number >= 0)
            {
                ...
                while (GET_CODE(new_i3_dest) == ZERO_EXTRACT
                    || GET_CODE(new_i3_dest) == STRICT_LOW_PART || GET_CODE(new_i3_dest) == SUBREG)
                    new_i3_dest = XEXP(new_i3_dest, 0);

                while (GET_CODE(new_i2_dest) == ZERO_EXTRACT
                    || GET_CODE(new_i2_dest) == STRICT_LOW_PART || GET_CODE(new_i2_dest) == SUBREG)
                    new_i2_dest = XEXP(new_i2_dest, 0);

                if (GET_CODE(new_i3_dest) == REG && GET_CODE(new_i2_dest) == REG
        ...
        if (insn_code_number < 0 && (split = find_split_point(&newpat, i3)) != 0
            ...
            && (GET_MODE(*split) == GET_MODE(i2dest) || GET_MODE(*split) == VOIDmode
                   || REGNO(i2dest) < FIRST_PSEUDO_REGISTER
                   || (REG_N_SETS(REGNO(i2dest)) == 1 && !added_sets_2 && !REG_USERVAR_P(i2dest)))
            && (next_real_insn(i2) == i3 || !use_crosses_set_p(*split, INSN_CUID(i2)))
            ...
            && !reg_referenced_p(i2dest, newpat))
        {
            ...
            if (split_code == MULT && GET_CODE(XEXP(*split, 1)) == CONST_INT
                && (i = exact_log2(INTVAL(XEXP(*split, 1)))) >= 0)
            {
                SUBST(*split, gen_rtx_combine(ASHIFT, split_mode, XEXP(*split, 0), GEN_INT(i)));
    ...
    else if (i1 && insn_code_number < 0 && asm_noperands(newpat) < 0 && GET_CODE(newpat) == PARALLEL
        && XVECLEN(newpat, 0) == 2 && GET_CODE(XVECEXP(newpat, 0, 0)) == SET
        && GET_CODE(SET_SRC(XVECEXP(newpat, 0, 0))) == SIGN_EXTEND
        && GET_CODE(XVECEXP(newpat, 0, 1)) == SET
        && rtx_equal_p(SET_SRC(XVECEXP(newpat, 0, 1)), XEXP(SET_SRC(XVECEXP(newpat, 0, 0)), 0))
        && !use_crosses_set_p(SET_SRC(XVECEXP(newpat, 0, 1)), INSN_CUID(i2))
        && GET_CODE(SET_DEST(XVECEXP(newpat, 0, 1))) != ZERO_EXTRACT
        && GET_CODE(SET_DEST(XVECEXP(newpat, 0, 1))) != STRICT_LOW_PART
        && !(temp = SET_DEST(XVECEXP(newpat, 0, 1)),
               (GET_CODE(temp) == REG && reg_nonzero_bits[REGNO(temp)] != 0
                   && GET_MODE_BITSIZE(GET_MODE(temp)) < BITS_PER_WORD
                   && GET_MODE_BITSIZE(GET_MODE(temp)) < HOST_BITS_PER_INT
                   && (reg_nonzero_bits[REGNO(temp)] != GET_MODE_MASK(word_mode))))
        && !(GET_CODE(SET_DEST(XVECEXP(newpat, 0, 1))) == SUBREG
               && (temp = SUBREG_REG(SET_DEST(XVECEXP(newpat, 0, 1))),
                      (GET_CODE(temp) == REG && reg_nonzero_bits[REGNO(temp)] != 0
                          ...
                          && GET_MODE_BITSIZE(GET_MODE(temp)) < HOST_BITS_PER_INT
                          && (reg_nonzero_bits[REGNO(temp)] != GET_MODE_MASK(word_mode)))))
        && !reg_overlap_mentioned_p(SET_DEST(XVECEXP(newpat, 0, 1)), SET_SRC(XVECEXP(newpat, 0, 1)))
        && !find_reg_note(i3, REG_UNUSED, SET_DEST(XVECEXP(newpat, 0, 0))))
    {
        rtx ni2dest;
        ...
        newpat = XVECEXP(newpat, 0, 1);
        SUBST(SET_SRC(newpat), gen_lowpart_for_combine(GET_MODE(SET_SRC(newpat)), ni2dest));
        i2_code_number = recog_for_combine(&newi2pat, i2, &new_i2_notes);
        ...
        if (insn_code_number >= 0)
        {
            ...
            for (insn = NEXT_INSN(i3); insn
                 && (this_basic_block == n_basic_blocks - 1
    ...
    /* Similarly, check for a case where we have a PARALLEL of two independent
       ...
       other insns to combine, but the destination of that SET is still live.  */
        ...
        && !reg_referenced_p(SET_DEST(XVECEXP(newpat, 0, 1)), XVECEXP(newpat, 0, 0))
        && !reg_referenced_p(SET_DEST(XVECEXP(newpat, 0, 0)), XVECEXP(newpat, 0, 1)))
    {
    ...
    if (undobuf.other_insn)
    {
        ...
        other_code_number = recog_for_combine(&other_pat, undobuf.other_insn, &new_other_notes);
        ...
        distribute_notes(
            new_other_notes, undobuf.other_insn, undobuf.other_insn, NULL_RTX, NULL_RTX, NULL_RTX);
    }
...

    {
        ...
        rtx elim_i2
            = ((newi2pat && reg_set_p(i2dest, newi2pat)) || i2dest_in_i2src || i2dest_in_i1src
                    ? 0
                    : i2dest);
        rtx elim_i1
            = (i1 == 0 || i1dest_in_i1src || (newi2pat && reg_set_p(i1dest, newi2pat)) ? 0
                                                                                       : i1dest);
        ...
        if (i3_subst_into_i2)
        {
            for (i = 0; i < XVECLEN(PATTERN(i2), 0); i++)
                if (GET_CODE(SET_DEST(XVECEXP(PATTERN(i2), 0, i))) == REG
                    ...
                    && !find_reg_note(i2, REG_UNUSED, SET_DEST(XVECEXP(PATTERN(i2), 0, i))))
                    for (temp = NEXT_INSN(i2); temp
                         && (this_basic_block == n_basic_blocks - 1
        ...
        if (newi2pat)
        {
            ...
            move_deaths(newpat, NULL_RTX, i1 ? INSN_CUID(i1) : INSN_CUID(i2), i3, &midnotes);

        /* Distribute all the LOG_LINKS and REG_NOTES from I1, I2, and I3.  */
        if (i3notes)
            distribute_notes(i3notes, i3, i3, newi2pat ? i2 : NULL_RTX, elim_i2, elim_i1);
        if (i2notes)
            distribute_notes(i2notes, i2, i3, newi2pat ? i2 : NULL_RTX, elim_i2, elim_i1);
        if (i1notes)
            distribute_notes(i1notes, i1, i3, newi2pat ? i2 : NULL_RTX, elim_i2, elim_i1);
        if (midnotes)
            distribute_notes(midnotes, NULL_RTX, i3, newi2pat ? i2 : NULL_RTX, elim_i2, elim_i1);
        ...
        if (i3dest_killed)
        {
            ...
            if (newi2pat && reg_set_p(i3dest_killed, newi2pat))
                distribute_notes(gen_rtx_EXPR_LIST(REG_DEAD, i3dest_killed, NULL_RTX), NULL_RTX, i2,
                    NULL_RTX, elim_i2, elim_i1);
            else
                distribute_notes(gen_rtx_EXPR_LIST(REG_DEAD, i3dest_killed, NULL_RTX), NULL_RTX, i3,
                    newi2pat ? i2 : NULL_RTX, elim_i2, elim_i1);
        }
        ...
        if (i2dest_in_i2src)
        {
            ...
            if (newi2pat && reg_set_p(i2dest, newi2pat))
                distribute_notes(gen_rtx_EXPR_LIST(REG_DEAD, i2dest, NULL_RTX), NULL_RTX, i2,
                    NULL_RTX, NULL_RTX, NULL_RTX);
            else
                distribute_notes(gen_rtx_EXPR_LIST(REG_DEAD, i2dest, NULL_RTX), NULL_RTX, i3,
                    newi2pat ? i2 : NULL_RTX, NULL_RTX, NULL_RTX);
        }
        ...
        if (i1dest_in_i1src)
        {
            ...
            if (newi2pat && reg_set_p(i1dest, newi2pat))
                distribute_notes(gen_rtx_EXPR_LIST(REG_DEAD, i1dest, NULL_RTX), NULL_RTX, i2,
                    NULL_RTX, NULL_RTX, NULL_RTX);
            else
                distribute_notes(gen_rtx_EXPR_LIST(REG_DEAD, i1dest, NULL_RTX), NULL_RTX, i3,
                    newi2pat ? i2 : NULL_RTX, NULL_RTX, NULL_RTX);
        }
        ...
        if (GET_CODE(i2dest) == REG)
        {
            ...
            for (link = LOG_LINKS(i3); link; link = XEXP(link, 1))
                if ((set = single_set(XEXP(link, 0))) != 0 && rtx_equal_p(i2dest, SET_DEST(set)))
            ...
            if (!added_sets_2 && (newi2pat == 0 || !reg_mentioned_p(i2dest, newi2pat))
                && !i2dest_in_i2src)
            {
                regno = REGNO(i2dest);
                REG_N_SETS(regno)--;
                if (REG_N_SETS(regno) == 0 && !REGNO_REG_SET_P(basic_block_live_at_start[0], regno))
                    REG_N_REFS(regno) = 0;
            }
        }
        ...
        if (i1 && GET_CODE(i1dest) == REG)
        {
            ...
            for (link = LOG_LINKS(i3); link; link = XEXP(link, 1))
                if ((set = single_set(XEXP(link, 0))) != 0 && rtx_equal_p(i1dest, SET_DEST(set)))
        ...
        if ((GET_CODE(newpat) == RETURN || simplejump_p(i3))
            && ((temp = next_nonnote_insn(i3)) == NULL_RTX || GET_CODE(temp) != BARRIER))
    ...
    if (added_links_insn && (newi2pat == 0 || INSN_CUID(added_links_insn) < INSN_CUID(i2))
...
static rtx *find_split_point(rtx *loc, rtx insn)
{
    ...
    case MEM:
        ...
        if (GET_CODE(XEXP(x, 0)) == CONST || GET_CODE(XEXP(x, 0)) == SYMBOL_REF)
        {
            SUBST(XEXP(x, 0),
                gen_rtx_combine(
                    LO_SUM, Pmode, gen_rtx_combine(HIGH, Pmode, XEXP(x, 0)), XEXP(x, 0)));
            ...
            && !memory_address_p(GET_MODE(x), XEXP(x, 0)))
        {
            ...
            rtx seq = split_insns(gen_rtx_SET(VOIDmode, reg, XEXP(x, 0)), subst_insn);
            ...
            /* If that didn't work, perhaps the first operand is complex and
               ...
               and have a constant moved through some previous computation.  */
                ...
                && !(GET_CODE(XEXP(XEXP(x, 0), 0)) == SUBREG
                       && (GET_RTX_CLASS(GET_CODE(SUBREG_REG(XEXP(XEXP(x, 0), 0)))) == 'o')))
                return &XEXP(XEXP(x, 0), 0);
        }
        break;
    ...
    case SET:
        ...
        if (SET_DEST(x) == cc0_rtx && GET_CODE(SET_SRC(x)) != COMPARE
            && GET_CODE(SET_SRC(x)) != ZERO_EXTRACT && GET_RTX_CLASS(GET_CODE(SET_SRC(x))) != 'o'
        ...
        if (GET_CODE(SET_DEST(x)) == ZERO_EXTRACT
            && (GET_MODE_BITSIZE(GET_MODE(XEXP(SET_DEST(x), 0))) <= 32)
            && GET_CODE(XEXP(SET_DEST(x), 1)) == CONST_INT
            && GET_CODE(XEXP(SET_DEST(x), 2)) == CONST_INT && GET_CODE(SET_SRC(x)) == CONST_INT
            && ((INTVAL(XEXP(SET_DEST(x), 1)) + INTVAL(XEXP(SET_DEST(x), 2)))
            ...
            && !side_effects_p(XEXP(SET_DEST(x), 0)))
        {
            ...
            uint32_t mask = ((int32_t)1 << len) - 1;

            if ((uint32_t)src == mask)
                SUBST(SET_SRC(x), gen_binary(IOR, mode, dest, GEN_INT(src << pos)));
            else
                SUBST(SET_SRC(x),
                    gen_binary(IOR, mode,
                        gen_binary(AND, mode, dest, GEN_INT(~(mask << pos) & GET_MODE_MASK(mode))),
                        GEN_INT(src << pos)));
        ...
        case AND:
            ...
            if (GET_CODE(XEXP(SET_SRC(x), 1)) == CONST_INT && GET_CODE(XEXP(SET_SRC(x), 0)) == REG
                && (pos = exact_log2(INTVAL(XEXP(SET_SRC(x), 1)))) >= 7
                && GET_CODE(SET_DEST(x)) == REG
                && (split = find_single_use(SET_DEST(x), insn, NULL)) != 0
                && (GET_CODE(*split) == EQ || GET_CODE(*split) == NE)
                && XEXP(*split, 0) == SET_DEST(x) && XEXP(*split, 1) == const0_rtx)
            {
                rtx extraction = make_extraction(
                    GET_MODE(SET_DEST(x)), XEXP(SET_SRC(x), 0), pos, NULL_RTX, 1, 1, 0, 0);
                if (extraction != 0)
                {
        ...
        case NE:
            ...
            if (STORE_FLAG_VALUE == -1 && XEXP(SET_SRC(x), 1) == const0_rtx
                ...
                && 1 <= (pos = exact_log2(
                             nonzero_bits(XEXP(SET_SRC(x), 0), GET_MODE(XEXP(SET_SRC(x), 0))))))
            {
                enum machine_mode mode = GET_MODE(XEXP(SET_SRC(x), 0));

                SUBST(SET_SRC(x),
                    gen_rtx_combine(NEG, mode,
                        gen_rtx_combine(LSHIFTRT, mode, XEXP(SET_SRC(x), 0), GEN_INT(pos))));
        ...
        if (len && pos >= 0 && pos + len <= GET_MODE_BITSIZE(GET_MODE(inner)))
        {
            ...
            {
                SUBST(SET_SRC(x),
                    gen_rtx_combine(AND, mode,
                        gen_rtx_combine(
                            LSHIFTRT, mode, gen_lowpart_for_combine(mode, inner), GEN_INT(pos)),
                        GEN_INT(((int32_t)1 << len) - 1)));
            ...
            {
                SUBST(SET_SRC(x),
                    gen_rtx_combine(unsignedp ? LSHIFTRT : ASHIFTRT, mode,
                        gen_rtx_combine(ASHIFT, mode, gen_lowpart_for_combine(mode, inner),
                            GEN_INT(GET_MODE_BITSIZE(mode) - len - pos)),
        ...
        if ((GET_RTX_CLASS(GET_CODE(SET_SRC(x))) == '2'
            ...
            && (GET_RTX_CLASS(GET_CODE(XEXP(SET_SRC(x), 0))) == 'o'
                   || (GET_CODE(XEXP(SET_SRC(x), 0)) == SUBREG
                          && (GET_RTX_CLASS(GET_CODE(SUBREG_REG(XEXP(SET_SRC(x), 0)))) == 'o'))))
        ...
        if (GET_CODE(XEXP(x, 0)) == NOT && GET_CODE(XEXP(x, 1)) == NOT)
        {
            SUBST(*loc,
                gen_rtx_combine(NOT, GET_MODE(x),
                    gen_rtx_combine(code == IOR ? AND : IOR, GET_MODE(x), XEXP(XEXP(x, 0), 0),
                        XEXP(XEXP(x, 1), 0))));
            return find_split_point(loc, insn);
        }
...
#define COMBINE_RTX_EQUAL_P(X, Y)                                                                  \
    ((X) == (Y)                                                                                    \
        || (GET_CODE(X) == REG && GET_CODE(Y) == REG && REGNO(X) == REGNO(Y)                       \
    ...
    if (!in_dest && code == REG && GET_CODE(from) == REG && REGNO(x) == REGNO(from))
    ...
    if (code == PARALLEL && GET_CODE(XVECEXP(x, 0, 0)) == SET
        && GET_CODE(SET_SRC(XVECEXP(x, 0, 0))) == ASM_OPERANDS)
    {
        replacement = subst(XVECEXP(x, 0, 0), from, to, 0, unique_copy);

        /* If this substitution failed, this whole thing fails.  */
        if (GET_CODE(replacement) == CLOBBER && XEXP(replacement, 0) == const0_rtx)
            return replacement;

        SUBST(XVECEXP(x, 0, 0), replacement);

        for (i = XVECLEN(x, 0) - 1; i >= 1; i--)
        {
            rtx dest = SET_DEST(XVECEXP(x, 0, i));

            if (GET_CODE(dest) != REG && GET_CODE(dest) != CC0 && GET_CODE(dest) != PC)
            {
                replacement = subst(dest, from, to, 0, unique_copy);

                /* If this substitution failed, this whole thing fails.  */
                if (GET_CODE(replacement) == CLOBBER && XEXP(replacement, 0) == const0_rtx)
                    return replacement;

                SUBST(SET_DEST(XVECEXP(x, 0, i)), replacement);
            }
        }
    }
    else
    {
        ...
        if (code == SET
            && (GET_CODE(SET_DEST(x)) == REG || GET_CODE(SET_DEST(x)) == CC0
        ...
        for (i = 0; i < len; i++)
        {
            if (fmt[i] == 'E')
            {
                ...
                for (j = XVECLEN(x, i) - 1; j >= 0; j--)
                {
                    ...
                    {
                        replacement = (unique_copy && n_occurrences ? copy_rtx(to) : to);
                        n_occurrences++;
                    }
                    else
                    {
                        replacement = subst(XVECEXP(x, i, j), from, to, 0, unique_copy);
                    ...
                    SUBST(XVECEXP(x, i, j), replacement);
                }
            }
            else if (fmt[i] == 'e')
            {
                if (COMBINE_RTX_EQUAL_P(XEXP(x, i), from))
                {
                    ...
                    if (GET_CODE(to) == SUBREG
                        && !MODES_TIEABLE_P(GET_MODE(to), GET_MODE(SUBREG_REG(to)))
                        && !(code == SUBREG
                               && MODES_TIEABLE_P(GET_MODE(x), GET_MODE(SUBREG_REG(to))))
#ifdef HAVE_cc0
                    ...
                    /* If we are in a SET_DEST, suppress most cases unless we
                       ...
                       SET_DEST.  */
                    replacement = subst(XEXP(x, i), from, to,
                        (((in_dest
...
static rtx simplify_rtx(rtx x, enum machine_mode op0_mode, int last, int in_dest)
{
    enum rtx_code code = GET_CODE(x);
    ...
    if ((code == PLUS || code == MINUS || code == MULT) && GET_CODE(XEXP(x, 0)) == ASHIFTRT
        && GET_CODE(XEXP(XEXP(x, 0), 0)) == PLUS && GET_CODE(XEXP(XEXP(XEXP(x, 0), 0), 0)) == ASHIFT
        ...
        && (temp = simplify_binary_operation(
                ASHIFTRT, mode, XEXP(XEXP(XEXP(x, 0), 0), 1), XEXP(XEXP(x, 0), 1)))
            != 0)
    {
        rtx tmp2 = simplify_shift_const(NULL_RTX, ASHIFT, mode,
            XEXP(XEXP(XEXP(XEXP(x, 0), 0), 0), 0), INTVAL(XEXP(XEXP(x, 0), 1)));

        tmp2 = simplify_shift_const(NULL_RTX, ASHIFTRT, mode, tmp2, INTVAL(XEXP(XEXP(x, 0), 1)));

        SUBST(XEXP(x, 0), gen_binary(PLUS, mode, tmp2, temp));
    }
    ...
    if (((GET_RTX_CLASS(code) == '2' || GET_RTX_CLASS(code) == 'c' || GET_RTX_CLASS(code) == '<')
            && ((GET_RTX_CLASS(GET_CODE(XEXP(x, 0))) != 'o'
                    && !(GET_CODE(XEXP(x, 0)) == SUBREG
                           && (GET_RTX_CLASS(GET_CODE(SUBREG_REG(XEXP(x, 0)))) == 'o')))
                   || (GET_RTX_CLASS(GET_CODE(XEXP(x, 1))) != 'o'
        ...
        || (GET_RTX_CLASS(code) == '1'
               && ((GET_RTX_CLASS(GET_CODE(XEXP(x, 0))) != 'o'
                      && !(GET_CODE(XEXP(x, 0)) == SUBREG
                             && (GET_RTX_CLASS(GET_CODE(SUBREG_REG(XEXP(x, 0)))) == 'o'))))))
    {
        rtx cond, true_cond, false_cond;

        cond = if_then_else_cond(x, &true_cond, &false_cond);
        if (cond != 0
            /* If everything is a comparison, what we have is highly unlikely
               to be simpler, so don't use it.  */
            && !(GET_RTX_CLASS(code) == '<'
                   && (GET_RTX_CLASS(GET_CODE(true_cond)) == '<'
                          || GET_RTX_CLASS(GET_CODE(false_cond)) == '<')))
        {
            rtx cop1 = const0_rtx;
            ...
            true_cond = subst(true_cond, pc_rtx, pc_rtx, 0, 0);
            false_cond = subst(false_cond, pc_rtx, pc_rtx, 0, 0);
            ...
            if (true_cond == const_true_rtx && false_cond == const0_rtx)
                x = gen_binary(cond_code, mode, cond, cop1);
            else if (true_cond == const0_rtx && false_cond == const_true_rtx)
                x = gen_binary(reverse_condition(cond_code), mode, cond, cop1);

            /* Likewise, we can make the negate of a comparison operation
               if the result values are - STORE_FLAG_VALUE and zero.  */
            else if (GET_CODE(true_cond) == CONST_INT && INTVAL(true_cond) == -STORE_FLAG_VALUE
                && false_cond == const0_rtx)
                x = gen_unary(NEG, mode, mode, gen_binary(cond_code, mode, cond, cop1));
            else if (GET_CODE(false_cond) == CONST_INT && INTVAL(false_cond) == -STORE_FLAG_VALUE
                && true_cond == const0_rtx)
                x = gen_unary(
                    NEG, mode, mode, gen_binary(reverse_condition(cond_code), mode, cond, cop1));
            else
                return gen_rtx_IF_THEN_ELSE(
                    mode, gen_binary(cond_code, VOIDmode, cond, cop1), true_cond, false_cond);
    ...
    case '1':
        ...
        break;
    case '<':
        temp = simplify_relational_operation(code, op0_mode, XEXP(x, 0), XEXP(x, 1));
    ...
    case 'b':
    case '3':
        temp = simplify_ternary_operation(code, mode, op0_mode, XEXP(x, 0), XEXP(x, 1), XEXP(x, 2));
        break;
    }
    ...
    if ((code == PLUS || code == MINUS || code == MULT || code == AND || code == IOR || code == XOR
            || code == DIV || code == UDIV || code == SMAX || code == SMIN || code == UMAX
            || code == UMIN)
        && INTEGRAL_MODE_P(mode))
    {
        if (GET_CODE(XEXP(x, 0)) == code)
        {
            ...
            inner = simplify_binary_operation(
                code == MINUS ? PLUS : code == DIV ? MULT : code == UDIV ? MULT : code, mode,
                inner_op0, inner_op1);
            ...
            if (inner == 0 && GET_RTX_CLASS(code) == 'c')
            {
                ...
                inner = simplify_binary_operation(code, mode, XEXP(XEXP(x, 0), 0), XEXP(x, 1));
            }
    ...
    case SUBREG:
            ...
            && (GET_MODE_SIZE(mode) <= GET_MODE_SIZE(GET_MODE(SUBREG_REG(x)))))
        {
            ...
            if (MEM_VOLATILE_P(SUBREG_REG(x)) || mode_dependent_address_p(XEXP(inner, 0)))
                return gen_rtx_CLOBBER(mode, const0_rtx);

            /* Note if the plus_constant doesn't make a valid address
               then this combination won't be accepted.  */
            x = gen_rtx_MEM(mode,
                plus_constant(XEXP(inner, 0), (SUBREG_WORD(x) * UNITS_PER_WORD + endian_offset)));
        ...
        {
            if (mode == GET_MODE(SUBREG_REG(SUBREG_REG(x))) && SUBREG_WORD(x) == 0
                && SUBREG_WORD(SUBREG_REG(x)) == 0)
                return SUBREG_REG(SUBREG_REG(x));

            SUBST_INT(SUBREG_WORD(x), SUBREG_WORD(x) + SUBREG_WORD(SUBREG_REG(x)));
        ...
        if (GET_CODE(SUBREG_REG(x)) == REG && REGNO(SUBREG_REG(x)) < FIRST_PSEUDO_REGISTER
            && REGNO(SUBREG_REG(x)) != FRAME_POINTER_REGNUM
            && REGNO(SUBREG_REG(x)) != ARG_POINTER_REGNUM
            && REGNO(SUBREG_REG(x)) != STACK_POINTER_REGNUM)
        {
            if (HARD_REGNO_MODE_OK(REGNO(SUBREG_REG(x)) + SUBREG_WORD(x), mode))
                return gen_rtx_REG(mode, REGNO(SUBREG_REG(x)) + SUBREG_WORD(x));
            else
                return gen_rtx_CLOBBER(mode, const0_rtx);
        }
        ...
        if (CONSTANT_P(SUBREG_REG(x)) && op0_mode != VOIDmode
            && GET_MODE_SIZE(mode) == UNITS_PER_WORD && GET_MODE_SIZE(op0_mode) > UNITS_PER_WORD
            && GET_MODE_CLASS(mode) == MODE_INT)
        {
            temp = operand_subword(SUBREG_REG(x), SUBREG_WORD(x), 0, op0_mode);
            if (temp)
                return temp;
        }
        ...
        if (CONSTANT_P(SUBREG_REG(x)) && (SUBREG_WORD(x) == 0)
    ...
    case NOT:
        ...
        if (GET_CODE(XEXP(x, 0)) == PLUS && XEXP(XEXP(x, 0), 1) == constm1_rtx)
            return gen_rtx_combine(NEG, mode, XEXP(XEXP(x, 0), 0));

        /* Similarly, (not (neg X)) is (plus X -1).  */
        if (GET_CODE(XEXP(x, 0)) == NEG)
            return gen_rtx_combine(PLUS, mode, XEXP(XEXP(x, 0), 0), constm1_rtx);

        /* (not (xor X C)) for C constant is (xor X D) with D = ~ C.  */
        if (GET_CODE(XEXP(x, 0)) == XOR && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT
            && (temp = simplify_unary_operation(NOT, mode, XEXP(XEXP(x, 0), 1), mode)) != 0)
        ...
        if (GET_CODE(XEXP(x, 0)) == ASHIFT && XEXP(XEXP(x, 0), 0) == const1_rtx)
            return gen_rtx_ROTATE(
                mode, gen_unary(NOT, mode, mode, const1_rtx), XEXP(XEXP(x, 0), 1));

        if (GET_CODE(XEXP(x, 0)) == SUBREG && subreg_lowpart_p(XEXP(x, 0))
            ...
            && XEXP(SUBREG_REG(XEXP(x, 0)), 0) == const1_rtx)
        {
            ...
            x = gen_rtx_ROTATE(inner_mode, gen_unary(NOT, inner_mode, inner_mode, const1_rtx),
        ...
        if (STORE_FLAG_VALUE == -1 && GET_RTX_CLASS(GET_CODE(XEXP(x, 0))) == '<'
            && reversible_comparison_p(XEXP(x, 0)))
            return gen_rtx_combine(reverse_condition(GET_CODE(XEXP(x, 0))), mode,
        ...
        if (STORE_FLAG_VALUE == -1 && XEXP(x, 1) == const1_rtx && GET_CODE(XEXP(x, 0)) == ASHIFTRT
            && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT
        ...
        if (GET_CODE(XEXP(x, 0)) == IOR || GET_CODE(XEXP(x, 0)) == AND)
        {
            ...
            if (GET_CODE(in2) == NOT)
                in2 = XEXP(in2, 0);
            else if (GET_CODE(in2) == CONST_INT && GET_MODE_BITSIZE(mode) <= 32)
            ...
            return gen_rtx_combine(GET_CODE(XEXP(x, 0)) == IOR ? AND : IOR, mode, in1, in2);
        }
        break;

    case NEG:
        /* (neg (plus X 1)) can become (not X).  */
        if (GET_CODE(XEXP(x, 0)) == PLUS && XEXP(XEXP(x, 0), 1) == const1_rtx)
        ...
        if (GET_CODE(XEXP(x, 0)) == MINUS
            ...
            return gen_binary(MINUS, mode, XEXP(XEXP(x, 0), 1), XEXP(XEXP(x, 0), 0));
        ...
        {
            temp = simplify_unary_operation(NEG, mode, XEXP(XEXP(x, 0), 0), mode);
            if (temp)
            {
        ...
        if (GET_CODE(temp) == ASHIFTRT && GET_CODE(XEXP(temp, 1)) == CONST_INT
            && INTVAL(XEXP(temp, 1)) == GET_MODE_BITSIZE(mode) - 1)
            return simplify_shift_const(temp, LSHIFTRT, mode, XEXP(temp, 0), INTVAL(XEXP(temp, 1)));
        ...
        if (GET_CODE(temp) != REG
            && !(GET_CODE(temp) == SUBREG && GET_CODE(SUBREG_REG(temp)) == REG)
            && (i = exact_log2(nonzero_bits(temp, mode))) >= 0)
        {
            rtx temp1 = simplify_shift_const(NULL_RTX, ASHIFTRT, mode,
                simplify_shift_const(NULL_RTX, ASHIFT, mode, temp, GET_MODE_BITSIZE(mode) - 1 - i),
                GET_MODE_BITSIZE(mode) - 1 - i);

            /* If all we did was surround TEMP with the two shifts, we
               haven't improved anything, so don't use it.  Otherwise,
               we are better off with TEMP1.  */
            if (GET_CODE(temp1) != ASHIFTRT || GET_CODE(XEXP(temp1, 0)) != ASHIFT
    ...
    case TRUNCATE:
        if (GET_MODE_BITSIZE(mode) <= 32
            && TRULY_NOOP_TRUNCATION(
                   GET_MODE_BITSIZE(mode), GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0)))))
            SUBST(XEXP(x, 0),
                force_to_mode(XEXP(x, 0), GET_MODE(XEXP(x, 0)), GET_MODE_MASK(mode), NULL_RTX, 0));

        /* (truncate:SI ({sign,zero}_extend:DI foo:SI)) == foo:SI.  */
        if ((GET_CODE(XEXP(x, 0)) == SIGN_EXTEND || GET_CODE(XEXP(x, 0)) == ZERO_EXTEND)
            && GET_MODE(XEXP(XEXP(x, 0), 0)) == mode)
            return XEXP(XEXP(x, 0), 0);

        /* (truncate:SI (OP:DI ({sign,zero}_extend:DI foo:SI))) is
           (OP:SI foo:SI) if OP is NEG or ABS.  */
        if ((GET_CODE(XEXP(x, 0)) == ABS || GET_CODE(XEXP(x, 0)) == NEG)
            && (GET_CODE(XEXP(XEXP(x, 0), 0)) == SIGN_EXTEND
                   || GET_CODE(XEXP(XEXP(x, 0), 0)) == ZERO_EXTEND)
            && GET_MODE(XEXP(XEXP(XEXP(x, 0), 0), 0)) == mode)
            return gen_unary(GET_CODE(XEXP(x, 0)), mode, mode, XEXP(XEXP(XEXP(x, 0), 0), 0));

        /* (truncate:SI (subreg:DI (truncate:SI X) 0)) is
           (truncate:SI x).  */
        if (GET_CODE(XEXP(x, 0)) == SUBREG && GET_CODE(SUBREG_REG(XEXP(x, 0))) == TRUNCATE
        ...
        if (TRULY_NOOP_TRUNCATION(GET_MODE_BITSIZE(mode), GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))))
            && num_sign_bit_copies(XEXP(x, 0), GET_MODE(XEXP(x, 0))) >= GET_MODE_BITSIZE(mode) + 1)
        ...
        if (GET_MODE_BITSIZE(mode) <= 32
            ...
            && ((int32_t)STORE_FLAG_VALUE & ~GET_MODE_MASK(mode)) == 0)
            return gen_lowpart_for_combine(mode, XEXP(x, 0));
        ...
        if (GET_MODE_BITSIZE(mode) <= 32
            && ((int32_t)STORE_FLAG_VALUE & ~GET_MODE_MASK(mode)) == 0
            && (temp = get_last_value(XEXP(x, 0))) && GET_RTX_CLASS(GET_CODE(temp)) == '<')
            return gen_lowpart_for_combine(mode, XEXP(x, 0));
    ...
    case FLOAT_TRUNCATE:
        ...
        if (GET_CODE(XEXP(x, 0)) == FLOAT_EXTEND && GET_MODE(XEXP(XEXP(x, 0), 0)) == mode)
            return XEXP(XEXP(x, 0), 0);
    ...
    case COMPARE:
        ...
        if ((TARGET_FLOAT_FORMAT != IEEE_FLOAT_FORMAT || !FLOAT_MODE_P(GET_MODE(XEXP(x, 0)))
                || flag_fast_math)
    ...
    case PLUS:
        ...
        if (GET_CODE(XEXP(x, 0)) == PLUS && CONSTANT_ADDRESS_P(XEXP(XEXP(x, 0), 1)))
            return gen_binary(PLUS, mode, gen_binary(PLUS, mode, XEXP(XEXP(x, 0), 0), XEXP(x, 1)),
                XEXP(XEXP(x, 0), 1));
        ...
        if (GET_CODE(XEXP(x, 0)) == XOR && GET_CODE(XEXP(x, 1)) == CONST_INT
            && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT
            ...
            && ((GET_CODE(XEXP(XEXP(x, 0), 0)) == AND
                    && GET_CODE(XEXP(XEXP(XEXP(x, 0), 0), 1)) == CONST_INT
                    && (INTVAL(XEXP(XEXP(XEXP(x, 0), 0), 1)) == ((int32_t)1 << (i + 1)) - 1))
                   || (GET_CODE(XEXP(XEXP(x, 0), 0)) == ZERO_EXTEND
                          && (GET_MODE_BITSIZE(GET_MODE(XEXP(XEXP(XEXP(x, 0), 0), 0))) == i + 1))))
        ...
        /* (plus (comparison A B) C) can become (neg (rev-comp A B)) if
           ...
           below.  */
        if (GET_RTX_CLASS(GET_CODE(XEXP(x, 0))) == '<' && reversible_comparison_p(XEXP(x, 0))
            && ((STORE_FLAG_VALUE == -1 && XEXP(x, 1) == const1_rtx)
                   || (STORE_FLAG_VALUE == 1 && XEXP(x, 1) == constm1_rtx)))
            return gen_unary(NEG, mode, mode,
                gen_binary(reverse_condition(GET_CODE(XEXP(x, 0))), mode, XEXP(XEXP(x, 0), 0),
                    XEXP(XEXP(x, 0), 1)));
        ...
        /* If only the low-order bit of X is possibly nonzero, (plus x -1)
           ...
           "a = (b & 8) == 0;"  */
        if (XEXP(x, 1) == constm1_rtx && GET_CODE(XEXP(x, 0)) != REG
            && !(GET_CODE(XEXP(x, 0)) == SUBREG && GET_CODE(SUBREG_REG(XEXP(x, 0))) == REG)
            && nonzero_bits(XEXP(x, 0), mode) == 1)
            return simplify_shift_const(NULL_RTX, ASHIFTRT, mode,
                simplify_shift_const(NULL_RTX, ASHIFT, mode,
                    gen_rtx_combine(XOR, mode, XEXP(x, 0), const1_rtx), GET_MODE_BITSIZE(mode) - 1),
        ...
        if (GET_MODE_BITSIZE(mode) <= 32
            && (nonzero_bits(XEXP(x, 0), mode) & nonzero_bits(XEXP(x, 1), mode)) == 0)
    ...
    case MINUS:
        ...
        if (STORE_FLAG_VALUE == 1 && XEXP(x, 0) == const1_rtx
            && GET_RTX_CLASS(GET_CODE(XEXP(x, 1))) == '<' && reversible_comparison_p(XEXP(x, 1)))
            return gen_binary(reverse_condition(GET_CODE(XEXP(x, 1))), mode, XEXP(XEXP(x, 1), 0),
                XEXP(XEXP(x, 1), 1));

        /* (minus <foo> (and <foo> (const_int -pow2))) becomes
           (and <foo> (const_int pow2-1))  */
        if (GET_CODE(XEXP(x, 1)) == AND && GET_CODE(XEXP(XEXP(x, 1), 1)) == CONST_INT
            && exact_log2(-INTVAL(XEXP(XEXP(x, 1), 1))) >= 0
            && rtx_equal_p(XEXP(XEXP(x, 1), 0), XEXP(x, 0)))
            return simplify_and_const_int(
                NULL_RTX, mode, XEXP(x, 0), -INTVAL(XEXP(XEXP(x, 1), 1)) - 1);
    ...
    case MULT:
        ...
        if (GET_CODE(XEXP(x, 0)) == PLUS)
        {
            x = apply_distributive_law(
                gen_binary(PLUS, mode, gen_binary(MULT, mode, XEXP(XEXP(x, 0), 0), XEXP(x, 1)),
                    gen_binary(MULT, mode, XEXP(XEXP(x, 0), 1), XEXP(x, 1))));
    ...
    case UDIV:
        ...
        if (GET_CODE(XEXP(x, 1)) == CONST_INT && (i = exact_log2(INTVAL(XEXP(x, 1)))) >= 0
            && (GET_CODE(XEXP(x, 0)) == ASHIFT || GET_CODE(XEXP(x, 0)) == LSHIFTRT
                   || GET_CODE(XEXP(x, 0)) == ASHIFTRT || GET_CODE(XEXP(x, 0)) == ROTATE
    ...
    case LEU:
                   ...
                   ))
        {
            ...
            if (STORE_FLAG_VALUE == 1 && new_code == NE && GET_MODE_CLASS(mode) == MODE_INT
                && op1 == const0_rtx && nonzero_bits(op0, mode) == 1)
                return gen_lowpart_for_combine(mode, expand_compound_operation(op0));

            else if (STORE_FLAG_VALUE == 1 && new_code == NE && GET_MODE_CLASS(mode) == MODE_INT
                && op1 == const0_rtx && (num_sign_bit_copies(op0, mode) == GET_MODE_BITSIZE(mode)))
            {
                op0 = expand_compound_operation(op0);
                return gen_unary(NEG, mode, mode, gen_lowpart_for_combine(mode, op0));
            }

            else if (STORE_FLAG_VALUE == 1 && new_code == EQ && GET_MODE_CLASS(mode) == MODE_INT
                && op1 == const0_rtx && nonzero_bits(op0, mode) == 1)
            {
                op0 = expand_compound_operation(op0);
                return gen_binary(XOR, mode, gen_lowpart_for_combine(mode, op0), const1_rtx);
            }
            ...
            {
                op0 = expand_compound_operation(op0);
                return gen_unary(NOT, mode, mode, gen_lowpart_for_combine(mode, op0));
            }
            ...
            if (new_code == NE && GET_MODE_CLASS(mode) == MODE_INT
                ...
                && ((STORE_FLAG_VALUE & GET_MODE_MASK(mode))
                       == (uint32_t)1 << (GET_MODE_BITSIZE(mode) - 1))
                && op1 == const0_rtx && mode == GET_MODE(op0)
                && (i = exact_log2(nonzero_bits(op0, mode))) >= 0)
            {
                x = simplify_shift_const(NULL_RTX, ASHIFT, mode, expand_compound_operation(op0),
                    GET_MODE_BITSIZE(mode) - 1 - i);
    ...
    case ABS:
        ...
        if (GET_CODE(XEXP(x, 0)) == FFS || GET_CODE(XEXP(x, 0)) == ABS
            || ((GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) <= 32)
                   && ((nonzero_bits(XEXP(x, 0), GET_MODE(XEXP(x, 0)))
                           & ((int32_t)1 << (GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) - 1)))
                          == 0)))
            return XEXP(x, 0);
    ...
    case ROTATERT:
        ...
        if (GET_CODE(XEXP(x, 1)) == CONST_INT)
            return simplify_shift_const(x, code, mode, XEXP(x, 0), INTVAL(XEXP(x, 1)));

#ifdef SHIFT_COUNT_TRUNCATED
        else if (SHIFT_COUNT_TRUNCATED && GET_CODE(XEXP(x, 1)) != REG)
            SUBST(XEXP(x, 1),
                force_to_mode(XEXP(x, 1), GET_MODE(x),
                    ((int32_t)1 << exact_log2(GET_MODE_BITSIZE(GET_MODE(x)))) - 1, NULL_RTX,
                    0));
#endif
...
static rtx simplify_if_then_else(rtx x)
{
    enum machine_mode mode = GET_MODE(x);
    rtx cond = XEXP(x, 0);
    rtx true_cond = XEXP(x, 1);
    rtx false_cond = XEXP(x, 2);
    ...
    if (comparison_p && true_cond == const_true_rtx && false_cond == const0_rtx)
        return gen_binary(true_code, mode, XEXP(cond, 0), XEXP(cond, 1));

    /* Also when the truth value has to be reversed.  */
    if (comparison_p && reversible_comparison_p(cond) && true_cond == const0_rtx
        && false_cond == const_true_rtx)
        return gen_binary(reverse_condition(true_code), mode, XEXP(cond, 0), XEXP(cond, 1));

    /* Sometimes we can simplify the arm of an IF_THEN_ELSE if a register used
       in it is being compared against certain values.  Get the true_cond and false
       comparisons and see if that says anything about the value of each arm.  */

    if (comparison_p && reversible_comparison_p(cond) && GET_CODE(XEXP(cond, 0)) == REG)
    {
        int32_t nzb;
        ...
        if (false_code == EQ)
        {
            ...
            temp = true_cond, true_cond = false_cond, false_cond = temp;
        }
        ...
        if (true_code == EQ && true_val == const0_rtx
            ...
            && (num_sign_bit_copies(from, GET_MODE(from)) == GET_MODE_BITSIZE(GET_MODE(from))))
        ...
        if (reg_mentioned_p(from, true_cond))
            true_cond = subst(
                known_cond(copy_rtx(true_cond), true_code, from, true_val), pc_rtx, pc_rtx, 0, 0);
        if (reg_mentioned_p(from, false_cond))
            false_cond = subst(
                known_cond(copy_rtx(false_cond), false_code, from, false_val), pc_rtx, pc_rtx, 0, 0);

        SUBST(XEXP(x, 1), swapped ? false_cond : true_cond);
        SUBST(XEXP(x, 2), swapped ? true_cond : false_cond);

        true_cond = XEXP(x, 1), false_cond = XEXP(x, 2), true_code = GET_CODE(cond);
    }
    ...
    if (comparison_p && reversible_comparison_p(cond)
        && (true_cond == pc_rtx || (CONSTANT_P(true_cond) && GET_CODE(false_cond) != CONST_INT && false_cond != pc_rtx)
               || true_cond == const0_rtx
               || (GET_RTX_CLASS(GET_CODE(true_cond)) == 'o' && GET_RTX_CLASS(GET_CODE(false_cond)) != 'o')
               || (GET_CODE(true_cond) == SUBREG && GET_RTX_CLASS(GET_CODE(SUBREG_REG(true_cond))) == 'o'
                      && GET_RTX_CLASS(GET_CODE(false_cond)) != 'o')
               || reg_mentioned_p(true_cond, false_cond) || rtx_equal_p(false_cond, XEXP(cond, 0))))
    {
        true_code = reverse_condition(true_code);
        SUBST(XEXP(x, 0), gen_binary(true_code, GET_MODE(cond), XEXP(cond, 0), XEXP(cond, 1)));

        SUBST(XEXP(x, 1), false_cond);
        SUBST(XEXP(x, 2), true_cond);

        temp = true_cond, true_cond = false_cond, false_cond = temp, cond = XEXP(x, 0);
    ...
    if (rtx_equal_p(true_cond, false_cond) && !side_effects_p(cond))
        return true_cond;

    /* Convert a == b ? b : a to "a".  */
    if (true_code == EQ && !side_effects_p(cond) && rtx_equal_p(XEXP(cond, 0), false_cond)
        && rtx_equal_p(XEXP(cond, 1), true_cond))
        return false_cond;
    else if (true_code == NE && !side_effects_p(cond) && rtx_equal_p(XEXP(cond, 0), true_cond)
        && rtx_equal_p(XEXP(cond, 1), false_cond))
        return true_cond;

    /* Look for cases where we have (abs x) or (neg (abs X)).  */

    if (GET_MODE_CLASS(mode) == MODE_INT && GET_CODE(false_cond) == NEG
        && rtx_equal_p(true_cond, XEXP(false_cond, 0)) && comparison_p && rtx_equal_p(true_cond, XEXP(cond, 0))
        && !side_effects_p(true_cond))
        switch (true_code)
        {
        case GT:
        case GE:
            return gen_unary(ABS, mode, mode, true_cond);
        case LT:
        case LE:
            return gen_unary(NEG, mode, mode, gen_unary(ABS, mode, mode, true_cond));
        default:
            break;
        }
    ...
    if ((!FLOAT_MODE_P(mode) || flag_fast_math) && comparison_p && rtx_equal_p(XEXP(cond, 0), true_cond)
        && rtx_equal_p(XEXP(cond, 1), false_cond) && !side_effects_p(cond))
        switch (true_code)
        {
        case GE:
        case GT:
            return gen_binary(SMAX, mode, true_cond, false_cond);
        case LE:
        case LT:
            return gen_binary(SMIN, mode, true_cond, false_cond);
        case GEU:
        case GTU:
            return gen_binary(UMAX, mode, true_cond, false_cond);
        case LEU:
        case LTU:
            return gen_binary(UMIN, mode, true_cond, false_cond);
        default:
            break;
        }
    ...
    if ((STORE_FLAG_VALUE == 1 || STORE_FLAG_VALUE == -1) && comparison_p && mode != VOIDmode
        && !side_effects_p(x))
    {
        rtx t = make_compound_operation(true_cond, SET);
        rtx f = make_compound_operation(false_cond, SET);
        ...
        if ((GET_CODE(t) == PLUS || GET_CODE(t) == MINUS || GET_CODE(t) == IOR || GET_CODE(t) == XOR
                || GET_CODE(t) == ASHIFT || GET_CODE(t) == LSHIFTRT || GET_CODE(t) == ASHIFTRT)
            && rtx_equal_p(XEXP(t, 0), f))
            c1 = XEXP(t, 1), op = GET_CODE(t), z = f;

        /* If an identity-zero op is commutative, check whether there
           would be a match if we swapped the operands.  */
        else if ((GET_CODE(t) == PLUS || GET_CODE(t) == IOR || GET_CODE(t) == XOR)
            && rtx_equal_p(XEXP(t, 1), f))
            c1 = XEXP(t, 0), op = GET_CODE(t), z = f;
        else if (GET_CODE(t) == SIGN_EXTEND
            && (GET_CODE(XEXP(t, 0)) == PLUS || GET_CODE(XEXP(t, 0)) == MINUS
                   || GET_CODE(XEXP(t, 0)) == IOR || GET_CODE(XEXP(t, 0)) == XOR
                   || GET_CODE(XEXP(t, 0)) == ASHIFT || GET_CODE(XEXP(t, 0)) == LSHIFTRT
                   || GET_CODE(XEXP(t, 0)) == ASHIFTRT)
            && GET_CODE(XEXP(XEXP(t, 0), 0)) == SUBREG && subreg_lowpart_p(XEXP(XEXP(t, 0), 0))
            && rtx_equal_p(SUBREG_REG(XEXP(XEXP(t, 0), 0)), f)
            && (num_sign_bit_copies(f, GET_MODE(f))
                   > (GET_MODE_BITSIZE(mode) - GET_MODE_BITSIZE(GET_MODE(XEXP(XEXP(t, 0), 0))))))
        {
            c1 = XEXP(XEXP(t, 0), 1);
            z = f;
            op = GET_CODE(XEXP(t, 0));
            extend_op = SIGN_EXTEND;
            m = GET_MODE(XEXP(t, 0));
        }
        else if (GET_CODE(t) == SIGN_EXTEND
            && (GET_CODE(XEXP(t, 0)) == PLUS || GET_CODE(XEXP(t, 0)) == IOR
                   || GET_CODE(XEXP(t, 0)) == XOR)
            && GET_CODE(XEXP(XEXP(t, 0), 1)) == SUBREG && subreg_lowpart_p(XEXP(XEXP(t, 0), 1))
            && rtx_equal_p(SUBREG_REG(XEXP(XEXP(t, 0), 1)), f)
            && (num_sign_bit_copies(f, GET_MODE(f))
                   > (GET_MODE_BITSIZE(mode) - GET_MODE_BITSIZE(GET_MODE(XEXP(XEXP(t, 0), 1))))))
        {
            c1 = XEXP(XEXP(t, 0), 0);
            z = f;
            op = GET_CODE(XEXP(t, 0));
            ...
            && rtx_equal_p(SUBREG_REG(XEXP(XEXP(t, 0), 0)), f)
            && ((nonzero_bits(f, GET_MODE(f)) & ~GET_MODE_MASK(GET_MODE(XEXP(XEXP(t, 0), 0))))
                   == 0))
        {
            c1 = XEXP(XEXP(t, 0), 1);
            z = f;
            ...
            && rtx_equal_p(SUBREG_REG(XEXP(XEXP(t, 0), 1)), f)
            && ((nonzero_bits(f, GET_MODE(f)) & ~GET_MODE_MASK(GET_MODE(XEXP(XEXP(t, 0), 1))))
                   == 0))
        {
            c1 = XEXP(XEXP(t, 0), 0);
            z = f;
        ...
        if (z)
        {
            temp = subst(gen_binary(true_code, m, cond_op0, cond_op1), pc_rtx, pc_rtx, 0, 0);
            temp = gen_binary(MULT, m, temp, gen_binary(MULT, m, c1, const_true_rtx));
    ...
    if (true_code == NE && XEXP(cond, 1) == const0_rtx && false_cond == const0_rtx
        && GET_CODE(true_cond) == CONST_INT
        && ((1 == nonzero_bits(XEXP(cond, 0), mode) && (i = exact_log2(INTVAL(true_cond))) >= 0)
               || ((num_sign_bit_copies(XEXP(cond, 0), mode) == GET_MODE_BITSIZE(mode))
                      && (i = exact_log2(-INTVAL(true_cond))) >= 0)))
        return simplify_shift_const(
            NULL_RTX, ASHIFT, mode, gen_lowpart_for_combine(mode, XEXP(cond, 0)), i);
...
static rtx simplify_set(rtx x)
{
    rtx src = SET_SRC(x);
    rtx dest = SET_DEST(x);
    enum machine_mode mode = GET_MODE(src) != VOIDmode ? GET_MODE(src) : GET_MODE(dest);
    rtx other_insn;
    rtx *cc_use;
    ...
    if ((GET_CODE(src) == COMPARE
        ...
        && GET_RTX_CLASS(GET_CODE(*cc_use)) == '<' && rtx_equal_p(XEXP(*cc_use, 0), dest))
    {
        ...
        if (compare_mode != GET_MODE(dest))
        {
            ...
            if (regno < FIRST_PSEUDO_REGISTER || (REG_N_SETS(regno) == 1 && !REG_USERVAR_P(dest)))
            {
                if (regno >= FIRST_PSEUDO_REGISTER)
                    SUBST(regno_reg_rtx[regno], new_dest);
        ...
        if (new_code != old_code)
        {
            ...
            SUBST(*cc_use, gen_rtx_combine(new_code, GET_MODE(*cc_use), dest, const0_rtx));
            ...
            if (((old_code == NE && new_code == EQ) || (old_code == EQ && new_code == NE))
                && !other_changed && op1 == const0_rtx
                && GET_MODE_BITSIZE(GET_MODE(op0)) <= 32
                && exact_log2(mask = nonzero_bits(op0, GET_MODE(op0))) >= 0)
            {
                rtx pat = PATTERN(other_insn), note = 0;

                if ((recog_for_combine(&pat, other_insn, &note) < 0 && !check_asm_operands(pat)))
                {
        ...
        {
            SUBST(SET_SRC(x), gen_rtx_combine(COMPARE, compare_mode, op0, op1));
            src = SET_SRC(x);
        }
        else
        {
    ...
    if (GET_CODE(src) == SUBREG && subreg_lowpart_p(src)
        ...
        && (((GET_MODE_SIZE(GET_MODE(src)) + (UNITS_PER_WORD - 1)) / UNITS_PER_WORD)
               == ((GET_MODE_SIZE(GET_MODE(SUBREG_REG(src))) + (UNITS_PER_WORD - 1))
                      / UNITS_PER_WORD))
#ifdef CLASS_CANNOT_CHANGE_SIZE
        && !(GET_CODE(dest) == REG && REGNO(dest) < FIRST_PSEUDO_REGISTER
               && (TEST_HARD_REG_BIT(
                      reg_class_contents[(int)CLASS_CANNOT_CHANGE_SIZE], REGNO(dest)))
               && (GET_MODE_SIZE(GET_MODE(src)) != GET_MODE_SIZE(GET_MODE(SUBREG_REG(src)))))
#endif
        && (GET_CODE(dest) == REG
               || (GET_CODE(dest) == SUBREG && GET_CODE(SUBREG_REG(dest)) == REG)))
    {
        SUBST(SET_DEST(x), gen_lowpart_for_combine(GET_MODE(SUBREG_REG(src)), dest));
    ...
    if (GET_CODE(src) == SUBREG && subreg_lowpart_p(src)
        && LOAD_EXTEND_OP(GET_MODE(SUBREG_REG(src))) != NIL && SUBREG_WORD(src) == 0
        && (GET_MODE_SIZE(GET_MODE(src)) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(src))))
        && GET_CODE(SUBREG_REG(src)) == MEM)
    {
        SUBST(SET_SRC(x),
            gen_rtx_combine(
                LOAD_EXTEND_OP(GET_MODE(SUBREG_REG(src))), GET_MODE(src), XEXP(src, 0)));
    ...
    if (GET_CODE(dest) != PC && GET_CODE(src) == IF_THEN_ELSE
        && GET_MODE_CLASS(GET_MODE(src)) == MODE_INT
        && (GET_CODE(XEXP(src, 0)) == EQ || GET_CODE(XEXP(src, 0)) == NE)
        && XEXP(XEXP(src, 0), 1) == const0_rtx && GET_MODE(src) == GET_MODE(XEXP(XEXP(src, 0), 0))
#ifdef HAVE_conditional_move
        && !can_conditionally_move_p(GET_MODE(src))
#endif
        && (num_sign_bit_copies(XEXP(XEXP(src, 0), 0), GET_MODE(XEXP(XEXP(src, 0), 0)))
               == GET_MODE_BITSIZE(GET_MODE(XEXP(XEXP(src, 0), 0))))
        && !side_effects_p(src))
    {
        rtx true_cond = (GET_CODE(XEXP(src, 0)) == NE ? XEXP(src, 1) : XEXP(src, 2));
        rtx false_cond = (GET_CODE(XEXP(src, 0)) == NE ? XEXP(src, 2) : XEXP(src, 1));
        rtx term1 = const0_rtx, term2, term3;

        if (GET_CODE(true_cond) == IOR && rtx_equal_p(XEXP(true_cond, 0), false_cond))
            term1 = false_cond, true_cond = XEXP(true_cond, 1), false_cond = const0_rtx;
        else if (GET_CODE(true_cond) == IOR && rtx_equal_p(XEXP(true_cond, 1), false_cond))
            term1 = false_cond, true_cond = XEXP(true_cond, 0), false_cond = const0_rtx;
        else if (GET_CODE(false_cond) == IOR && rtx_equal_p(XEXP(false_cond, 0), true_cond))
            term1 = true_cond, false_cond = XEXP(false_cond, 1), true_cond = const0_rtx;
        else if (GET_CODE(false_cond) == IOR && rtx_equal_p(XEXP(false_cond, 1), true_cond))
            term1 = true_cond, false_cond = XEXP(false_cond, 0), true_cond = const0_rtx;

        term2 = gen_binary(AND, GET_MODE(src), XEXP(XEXP(src, 0), 0), true_cond);
        term3 = gen_binary(AND, GET_MODE(src),
            gen_unary(NOT, GET_MODE(src), GET_MODE(src), XEXP(XEXP(src, 0), 0)), false_cond);

        SUBST(SET_SRC(x),
            gen_binary(IOR, GET_MODE(src), gen_binary(IOR, GET_MODE(src), term1, term2), term3));
...
static rtx simplify_logical(rtx x, int last)
{
    ...
    case AND:
        ...
        if (GET_CODE(op0) == XOR && rtx_equal_p(XEXP(op0, 0), op1) && !side_effects_p(op1))
            x = gen_binary(AND, mode, gen_unary(NOT, mode, mode, XEXP(op0, 1)), op1);

        if (GET_CODE(op0) == XOR && rtx_equal_p(XEXP(op0, 1), op1) && !side_effects_p(op1))
            x = gen_binary(AND, mode, gen_unary(NOT, mode, mode, XEXP(op0, 0)), op1);
        ...
        if (GET_CODE(op1) == CONST_INT)
        {
            ...
            /* If we have (ior (and (X C1) C2)) and the next restart would be
               ...
               and then exit.  */
            if (last && GET_CODE(x) == IOR && GET_CODE(op0) == AND
                && GET_CODE(XEXP(op0, 1)) == CONST_INT && GET_CODE(op1) == CONST_INT)
                return gen_binary(IOR, mode,
                    gen_binary(
                        AND, mode, XEXP(op0, 0), GEN_INT(INTVAL(XEXP(op0, 1)) & ~INTVAL(op1))),
                    op1);

            if (GET_CODE(x) != AND)
                return x;

            if (GET_RTX_CLASS(GET_CODE(x)) == 'c' || GET_RTX_CLASS(GET_CODE(x)) == '2')
                op0 = XEXP(x, 0), op1 = XEXP(x, 1);
        }

        /* Convert (A | B) & A to A.  */
        if (GET_CODE(op0) == IOR
            && (rtx_equal_p(XEXP(op0, 0), op1) || rtx_equal_p(XEXP(op0, 1), op1))
        ...
        {
            x = apply_distributive_law(
                gen_binary(GET_CODE(op0), mode, gen_binary(AND, mode, XEXP(op0, 0), op1),
        ...
        if (GET_CODE(op1) == IOR || GET_CODE(op1) == XOR)
            return apply_distributive_law(
                gen_binary(GET_CODE(op1), mode, gen_binary(AND, mode, XEXP(op1, 0), op0),
    ...
    case IOR:
        ...
        if (GET_CODE(op1) == CONST_INT && GET_MODE_BITSIZE(mode) <= 32
        ...
        {
            x = apply_distributive_law(
                gen_binary(AND, mode, gen_binary(IOR, mode, XEXP(op0, 0), op1),
        ...
        {
            x = apply_distributive_law(
                gen_binary(AND, mode, gen_binary(IOR, mode, XEXP(op1, 0), op0),
        ...
        if (((GET_CODE(op0) == ASHIFT && GET_CODE(op1) == LSHIFTRT)
            ...
            && (INTVAL(XEXP(op0, 1)) + INTVAL(XEXP(op1, 1)) == GET_MODE_BITSIZE(mode)))
            return gen_rtx_ROTATE(
                mode, XEXP(op0, 0), (GET_CODE(op0) == ASHIFT ? XEXP(op0, 1) : XEXP(op1, 1)));
        ...
        if (GET_CODE(op1) == CONST_INT && GET_CODE(op0) == ASHIFTRT
            ...
            && GET_CODE(XEXP(op0, 1)) == CONST_INT && INTVAL(XEXP(op0, 1)) < 32)
        {
            int count = INTVAL(XEXP(op0, 1));
            int32_t mask = INTVAL(op1) << count;

            if (mask >> count == INTVAL(op1) && (mask & nonzero_bits(XEXP(op0, 0), mode)) == 0)
            {
                SUBST(XEXP(XEXP(op0, 0), 1), GEN_INT(INTVAL(XEXP(XEXP(op0, 0), 1)) | mask));
                return op0;
            }
        }
        break;
    ...
    case XOR:
        ...
        if (GET_CODE(op0) == AND && rtx_equal_p(XEXP(op0, 1), op1) && !side_effects_p(op1))
            return gen_binary(AND, mode, gen_unary(NOT, mode, mode, XEXP(op0, 0)), op1);

        else if (GET_CODE(op0) == AND && rtx_equal_p(XEXP(op0, 0), op1) && !side_effects_p(op1))
            return gen_binary(AND, mode, gen_unary(NOT, mode, mode, XEXP(op0, 1)), op1);

        /* (xor (comparison foo bar) (const_int 1)) can become the reversed
           comparison if STORE_FLAG_VALUE is 1.  */
        if (STORE_FLAG_VALUE == 1 && op1 == const1_rtx && GET_RTX_CLASS(GET_CODE(op0)) == '<'
            && reversible_comparison_p(op0))
            return gen_rtx_combine(
                reverse_condition(GET_CODE(op0)), mode, XEXP(op0, 0), XEXP(op0, 1));

        /* (lshiftrt foo C) where C is the number of bits in FOO minus 1
           is (lt foo (const_int 0)), so we can perform the above
           simplification if STORE_FLAG_VALUE is 1.  */

        if (STORE_FLAG_VALUE == 1 && op1 == const1_rtx && GET_CODE(op0) == LSHIFTRT
        ...
        if (GET_MODE_BITSIZE(mode) <= 32
            && ((STORE_FLAG_VALUE & GET_MODE_MASK(mode))
                   == (uint32_t)1 << (GET_MODE_BITSIZE(mode) - 1))
            && op1 == const_true_rtx && GET_RTX_CLASS(GET_CODE(op0)) == '<'
            && reversible_comparison_p(op0))
            return gen_rtx_combine(
                reverse_condition(GET_CODE(op0)), mode, XEXP(op0, 0), XEXP(op0, 1));
        break;
...
static rtx expand_compound_operation(rtx x)
{
    ...
    case SIGN_EXTRACT:
        ...
        if (GET_CODE(XEXP(x, 1)) != CONST_INT || GET_CODE(XEXP(x, 2)) != CONST_INT
    ...
    if (GET_CODE(x) == ZERO_EXTEND)
    {
        /* (zero_extend:DI (truncate:SI foo:DI)) is just foo:DI if we
           ...
           set.  */
        if (GET_CODE(XEXP(x, 0)) == TRUNCATE && GET_MODE(XEXP(XEXP(x, 0), 0)) == GET_MODE(x)
            && GET_MODE_BITSIZE(GET_MODE(x)) <= 32
        ...
        if (GET_CODE(XEXP(x, 0)) == SUBREG && GET_MODE(SUBREG_REG(XEXP(x, 0))) == GET_MODE(x)
            && subreg_lowpart_p(XEXP(x, 0))
            && GET_MODE_BITSIZE(GET_MODE(x)) <= 32
        ...
        if (GET_CODE(XEXP(x, 0)) == TRUNCATE && GET_MODE(XEXP(XEXP(x, 0), 0)) == GET_MODE(x)
            ...
            && (GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) <= 32)
            && ((int32_t)STORE_FLAG_VALUE & ~GET_MODE_MASK(GET_MODE(XEXP(x, 0)))) == 0)
            return XEXP(XEXP(x, 0), 0);
        ...
        if (flag_expensive_optimizations
            && ((GET_MODE_BITSIZE(GET_MODE(x)) <= 32
                    && ((nonzero_bits(XEXP(x, 0), GET_MODE(x))
                            & ~(((uint32_t)GET_MODE_MASK(GET_MODE(XEXP(x, 0)))) >> 1))
                           == 0))
                   || (GET_RTX_CLASS(GET_CODE(XEXP(x, 0))) == '<'
                          && (GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) <= 32)
                          && (((int32_t)STORE_FLAG_VALUE
                                  & ~(((uint32_t)GET_MODE_MASK(GET_MODE(XEXP(x, 0)))) >> 1))
                                 == 0))))
        {
    ...
    if (modewidth >= pos - len)
        tem = simplify_shift_const(NULL_RTX, unsignedp ? LSHIFTRT : ASHIFTRT, GET_MODE(x),
            simplify_shift_const(NULL_RTX, ASHIFT, GET_MODE(x), XEXP(x, 0), modewidth - pos - len),
            modewidth - len);

    else if (unsignedp && len < 32)
        tem = simplify_and_const_int(NULL_RTX, GET_MODE(x),
            simplify_shift_const(NULL_RTX, LSHIFTRT, GET_MODE(x), XEXP(x, 0), pos),
            ((int32_t)1 << len) - 1);
    else
...
static rtx expand_field_assignment(rtx x)
{
    ...
    while (1)
    {
        if (GET_CODE(SET_DEST(x)) == STRICT_LOW_PART && GET_CODE(XEXP(SET_DEST(x), 0)) == SUBREG)
        {
            ...
            && GET_CODE(XEXP(SET_DEST(x), 1)) == CONST_INT)
        {
            ...
            if (GET_CODE(pos) == CONST_INT && INTVAL(pos) + len > GET_MODE_BITSIZE(GET_MODE(inner)))
        ...
        /* A SUBREG between two modes that occupy the same numbers of words
           can be done by moving the SUBREG to the source.  */
            ...
            && (((GET_MODE_SIZE(GET_MODE(SET_DEST(x))) + (UNITS_PER_WORD - 1)) / UNITS_PER_WORD)
                   == ((GET_MODE_SIZE(GET_MODE(SUBREG_REG(SET_DEST(x)))) + (UNITS_PER_WORD - 1))
                          / UNITS_PER_WORD)))
        {
            x = gen_rtx_SET(VOIDmode, SUBREG_REG(SET_DEST(x)),
                gen_lowpart_for_combine(GET_MODE(SUBREG_REG(SET_DEST(x))), SET_SRC(x)));
            continue;
        }
        else
            break;
        ...
        x = gen_rtx_SET(VOIDmode, copy_rtx(inner),
            gen_binary(IOR, compute_mode,
                gen_binary(AND, compute_mode,
                    gen_unary(NOT, compute_mode, compute_mode,
                        gen_binary(ASHIFT, compute_mode, mask, pos)),
                    inner),
                gen_binary(ASHIFT, compute_mode,
                    gen_binary(
                        AND, compute_mode, gen_lowpart_for_combine(compute_mode, SET_SRC(x)), mask),
                    pos)));
    }
...
static rtx make_extraction(enum machine_mode mode, rtx inner, int pos, rtx pos_rtx, int len,
    int unsignedp, int in_dest, int in_compare)
{
    ...
    if (tmode != BLKmode && !(spans_byte && inner_mode != tmode)
        && ((pos_rtx == 0 && (pos % BITS_PER_WORD) == 0 && GET_CODE(inner) != MEM
                && (!in_dest
                       || (GET_CODE(inner) == REG
                              && (movstrict_optab->handlers[(int)tmode].insn_code
                                     != CODE_FOR_nothing))))
               || (GET_CODE(inner) == MEM && pos_rtx == 0
                      && (pos % (STRICT_ALIGNMENT ? GET_MODE_ALIGNMENT(tmode) : BITS_PER_UNIT)) == 0
                                    ...
                                    && !MEM_VOLATILE_P(inner))))))
    {
        ...
        if (GET_CODE(inner) == MEM)
        {
            ...
            ret = gen_rtx_MEM(tmode, plus_constant(XEXP(inner, 0), offset));
            RTX_UNCHANGING_P(ret) = RTX_UNCHANGING_P(inner);
            MEM_COPY_ATTRIBUTES(ret, inner);
        }
        else if (GET_CODE(inner) == REG)
        {
            ...
            if (tmode != inner_mode)
                ret = gen_rtx_SUBREG(tmode, inner, pos / BITS_PER_WORD);
            else
                ret = inner;
        }
        else
            ret = force_to_mode(inner, tmode,
                len >= 32 ? GET_MODE_MASK(tmode)
                                              : ((int32_t)1 << len) - 1,
                NULL_RTX, 0);
        ...
        if (in_dest)
            return (GET_CODE(ret) == MEM
                    ? ret
                    : (GET_CODE(ret) != SUBREG ? gen_rtx_CLOBBER(tmode, const0_rtx)
                                               : gen_rtx_combine(STRICT_LOW_PART, VOIDmode, ret)));

        /* Otherwise, sign- or zero-extend unless we already are in the
           proper mode.  */

        return (mode == tmode ? ret
                              : gen_rtx_combine(unsignedp ? ZERO_EXTEND : SIGN_EXTEND, mode, ret));
    }

    /* Unless this is a COMPARE or we have a funny memory reference,
       don't do anything with zero-extending field extracts starting at
       the low-order bit since they are simple AND operations.  */
    if (pos_rtx == 0 && pos == 0 && !in_dest && !in_compare && !spans_byte && unsignedp)
        return 0;
    ...
    /* Unless we are allowed to span bytes, reject this if we would be
       ...
       undefined.  */
    if (!spans_byte
        && ((pos_rtx == 0 && pos + len > GET_MODE_BITSIZE(is_mode)) || (pos_rtx != 0 && len != 1)))
        return 0;
    ...
    if (mode != VOIDmode && GET_MODE_SIZE(extraction_mode) < GET_MODE_SIZE(mode))
    ...
    if (GET_CODE(inner) != MEM)
        ...
        && (mode_dependent_address_p(XEXP(inner, 0)) || MEM_VOLATILE_P(inner)))
    ...
    /* If INNER has a wider mode, make it smaller.  If this is a constant
       ...
       the value.  */
    if (wanted_inner_mode != VOIDmode && GET_MODE_SIZE(wanted_inner_mode) < GET_MODE_SIZE(is_mode)
        && ((GET_CODE(inner) == MEM
                      ...
                      || (!mode_dependent_address_p(XEXP(inner, 0)) && !MEM_VOLATILE_P(inner))))))
    {
        ...
        {
            rtx newmem = gen_rtx_MEM(wanted_inner_mode, plus_constant(XEXP(inner, 0), offset));
    ...
    else if (GET_CODE(inner) != MEM)
    {
        if (GET_MODE(inner) != wanted_inner_mode
            && (pos_rtx != 0 || orig_pos + len > GET_MODE_BITSIZE(wanted_inner_mode)))
            return 0;

        inner = force_to_mode(inner, wanted_inner_mode,
            pos_rtx || len + orig_pos >= 32
                ? GET_MODE_MASK(wanted_inner_mode)
                : (((int32_t)1 << len) - 1) << orig_pos,
            NULL_RTX, 0);
    }

    /* Adjust mode of POS_RTX, if needed.  If we want a wider mode, we
       have to zero extend.  Otherwise, we can just use a SUBREG.  */
    if (pos_rtx != 0 && GET_MODE_SIZE(pos_mode) > GET_MODE_SIZE(GET_MODE(pos_rtx)))
        pos_rtx = gen_rtx_combine(ZERO_EXTEND, pos_mode, pos_rtx);
    else if (pos_rtx != 0 && GET_MODE_SIZE(pos_mode) < GET_MODE_SIZE(GET_MODE(pos_rtx)))
    ...
    ret = gen_rtx_combine(
        unsignedp ? ZERO_EXTRACT : SIGN_EXTRACT, extraction_mode, inner, GEN_INT(len), pos_rtx);
    if (!in_dest)
        ret = gen_lowpart_for_combine(mode, ret);
...
static rtx extract_left_shift(rtx x, int count)
{
    ...
    case ASHIFT:
        ...
        if (GET_CODE(XEXP(x, 1)) == CONST_INT && INTVAL(XEXP(x, 1)) >= count)
            return simplify_shift_const(
                NULL_RTX, ASHIFT, mode, XEXP(x, 0), INTVAL(XEXP(x, 1)) - count);
        break;
    ...
    case AND:
        ...
        if (GET_CODE(XEXP(x, 1)) == CONST_INT
            && (INTVAL(XEXP(x, 1)) & ((((int32_t)1 << count)) - 1)) == 0
            && (tem = extract_left_shift(XEXP(x, 0), count)) != 0)
            return gen_binary(code, mode, tem, GEN_INT(INTVAL(XEXP(x, 1)) >> count));
...
static rtx make_compound_operation(rtx x, enum rtx_code in_code)
{
    ...
    next_code = (code == MEM || code == PLUS || code == MINUS
            ? MEM
            : ((code == COMPARE || GET_RTX_CLASS(code) == '<') && XEXP(x, 1) == const0_rtx)
                ? COMPARE
    ...
    case ASHIFT:
        ...
        if (in_code == MEM && GET_CODE(XEXP(x, 1)) == CONST_INT
            && INTVAL(XEXP(x, 1)) < 32 && INTVAL(XEXP(x, 1)) >= 0)
        {
            ret = make_compound_operation(XEXP(x, 0), next_code);
            ret = gen_rtx_combine(MULT, mode, ret, GEN_INT((int32_t)1 << INTVAL(XEXP(x, 1))));
        }
        break;
    ...
    case AND:
        ...
        if (GET_CODE(XEXP(x, 0)) == LSHIFTRT && (i = exact_log2(INTVAL(XEXP(x, 1)) + 1)) >= 0)
        {
            ret = make_compound_operation(XEXP(XEXP(x, 0), 0), next_code);
            ret = make_extraction(mode, ret, 0, XEXP(XEXP(x, 0), 1), i, 1, 0, in_code == COMPARE);
        }

        /* Same as previous, but for (subreg (lshiftrt ...)) in first op.  */
        else if (GET_CODE(XEXP(x, 0)) == SUBREG && subreg_lowpart_p(XEXP(x, 0))
            && GET_CODE(SUBREG_REG(XEXP(x, 0))) == LSHIFTRT
            && (i = exact_log2(INTVAL(XEXP(x, 1)) + 1)) >= 0)
        {
            ret = make_compound_operation(XEXP(SUBREG_REG(XEXP(x, 0)), 0), next_code);
            ret = make_extraction(GET_MODE(SUBREG_REG(XEXP(x, 0))), ret, 0,
                XEXP(SUBREG_REG(XEXP(x, 0)), 1), i, 1, 0, in_code == COMPARE);
        }
        /* Same as previous, but for (xor/ior (lshiftrt...) (lshiftrt...)).  */
        else if ((GET_CODE(XEXP(x, 0)) == XOR || GET_CODE(XEXP(x, 0)) == IOR)
            && GET_CODE(XEXP(XEXP(x, 0), 0)) == LSHIFTRT
            && GET_CODE(XEXP(XEXP(x, 0), 1)) == LSHIFTRT
            && (i = exact_log2(INTVAL(XEXP(x, 1)) + 1)) >= 0)
        {
            /* Apply the distributive law, and then try to make extractions.  */
            ret = gen_rtx_combine(GET_CODE(XEXP(x, 0)), mode,
                gen_rtx_AND(mode, XEXP(XEXP(x, 0), 0), XEXP(x, 1)),
                gen_rtx_AND(mode, XEXP(XEXP(x, 0), 1), XEXP(x, 1)));
            ret = make_compound_operation(ret, in_code);
        }

        /* If we are have (and (rotate X C) M) and C is larger than the number
           of bits in M, this is an extraction.  */

        else if (GET_CODE(XEXP(x, 0)) == ROTATE && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT
            && (i = exact_log2(INTVAL(XEXP(x, 1)) + 1)) >= 0 && i <= INTVAL(XEXP(XEXP(x, 0), 1)))
        {
            ret = make_compound_operation(XEXP(XEXP(x, 0), 0), next_code);
            ret = make_extraction(mode, ret, (GET_MODE_BITSIZE(mode) - INTVAL(XEXP(XEXP(x, 0), 1))),
        ...
        /* On machines without logical shifts, if the operand of the AND is
           ...
           bits, we can replace the logical shift with an arithmetic shift.  */
            ...
            && GET_CODE(XEXP(x, 0)) == LSHIFTRT && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT
            && INTVAL(XEXP(XEXP(x, 0), 1)) >= 0
            && INTVAL(XEXP(XEXP(x, 0), 1)) < 32
            && mode_width <= 32)
        {
            uint32_t mask = GET_MODE_MASK(mode);

            mask >>= INTVAL(XEXP(XEXP(x, 0), 1));
            if ((INTVAL(XEXP(x, 1)) & ~mask) == 0)
                SUBST(XEXP(x, 0),
                    gen_rtx_combine(ASHIFTRT, mode,
                        make_compound_operation(XEXP(XEXP(x, 0), 0), next_code),
                        XEXP(XEXP(x, 0), 1)));
        }
        ...
        /* If the constant is one less than a power of two, this might be
           ...
           we are in a COMPARE.  */
            ...
            ret = make_extraction(mode, make_compound_operation(XEXP(x, 0), next_code), 0, NULL_RTX,
                i, 1, 0, in_code == COMPARE);

        /* If we are in a comparison and this is an AND with a power of two,
           convert this into the appropriate bit extract.  */
        else if (in_code == COMPARE && (i = exact_log2(INTVAL(XEXP(x, 1)))) >= 0)
            ret = make_extraction(
                mode, make_compound_operation(XEXP(x, 0), next_code), i, NULL_RTX, 1, 1, 0, 1);
    ...
    case LSHIFTRT:
        ...
        {
            ret = gen_rtx_combine(
                ASHIFTRT, mode, make_compound_operation(XEXP(x, 0), next_code), XEXP(x, 1));
            break;
        }
    ...
    case ASHIFTRT:
        ...
        if (GET_CODE(rhs) == CONST_INT && GET_CODE(lhs) == ASHIFT
            && GET_CODE(XEXP(lhs, 1)) == CONST_INT && INTVAL(rhs) >= INTVAL(XEXP(lhs, 1)))
        {
            ret = make_compound_operation(XEXP(lhs, 0), next_code);
            ret = make_extraction(mode, ret, INTVAL(rhs) - INTVAL(XEXP(lhs, 1)), NULL_RTX,
                mode_width - INTVAL(rhs), code == LSHIFTRT, 0, in_code == COMPARE);
        }
        ...
        if (GET_RTX_CLASS(GET_CODE(lhs)) != 'o'
            && !(GET_CODE(lhs) == SUBREG && (GET_RTX_CLASS(GET_CODE(SUBREG_REG(lhs))) == 'o'))
            && GET_CODE(rhs) == CONST_INT && INTVAL(rhs) < 32
            && (ret = extract_left_shift(lhs, INTVAL(rhs))) != 0)
            ret = make_extraction(mode, make_compound_operation(ret, next_code), 0, NULL_RTX,
                mode_width - INTVAL(rhs), code == LSHIFTRT, 0, in_code == COMPARE);
    ...
    case SUBREG:
        ...
        if (GET_CODE(tem) != GET_CODE(SUBREG_REG(x))
            && GET_MODE_SIZE(mode) < GET_MODE_SIZE(GET_MODE(tem)) && subreg_lowpart_p(x))
        {
            rtx newer = force_to_mode(tem, mode, GET_MODE_MASK(mode), NULL_RTX, 0);
        ...
        if ((GET_CODE(tem) == SIGN_EXTEND || GET_CODE(tem) == ZERO_EXTEND) && subreg_lowpart_p(x))
        {
            if (GET_MODE_SIZE(mode) > GET_MODE_SIZE(GET_MODE(tem))
                || (GET_MODE_SIZE(mode) > GET_MODE_SIZE(GET_MODE(XEXP(tem, 0)))))
    ...
    if (ret)
    {
        x = gen_lowpart_for_combine(mode, ret);
        code = GET_CODE(x);
    }
    ...
    for (i = 0; i < GET_RTX_LENGTH(code); i++)
        if (fmt[i] == 'e')
        {
            ret = make_compound_operation(XEXP(x, i), next_code);
            SUBST(XEXP(x, i), ret);
        }
...
static int get_pos_from_mask(uint32_t m, int *plen)
{
...
static rtx force_to_mode(
    rtx x, enum machine_mode mode, uint32_t mask, rtx reg, int just_select)
{
    enum rtx_code code = GET_CODE(x);
    int next_select = just_select || code == XOR || code == NOT || code == NEG;
    enum machine_mode op_mode;
    uint32_t fuller_mask, nonzero;
    rtx op0, op1, temp;
    ...
    /* We want to perform the operation is its present mode unless we know
       ...
       in MODE.  */
    op_mode = ((GET_MODE_CLASS(mode) == GET_MODE_CLASS(GET_MODE(x)) && code_to_optab[(int)code] != 0
    ...
    if (op_mode)
        fuller_mask = (GET_MODE_BITSIZE(op_mode) >= 32
                ? GET_MODE_MASK(op_mode)
                : ((int32_t)1 << (floor_log2(mask) + 1)) - 1);
    else
        fuller_mask = ~(int32_t)0;
    ...
    {
        int32_t cval = INTVAL(x) & mask;
        int width = GET_MODE_BITSIZE(mode);

        /* If MODE is narrower that int32_t and CVAL is a negative
           number, sign extend it.  */
        if (width > 0 && width < 32
            && (cval & ((int32_t)1 << (width - 1))) != 0)
            cval |= (int32_t)-1 << width;
    ...
    case REG:
        if (reg != 0
            && (rtx_equal_p(get_last_value(reg), x) || rtx_equal_p(reg, get_last_value(x))))
            x = reg;
        break;
    ...
    case AND:
        ...
        if (GET_CODE(XEXP(x, 1)) == CONST_INT)
        {
            x = simplify_and_const_int(x, op_mode, XEXP(x, 0), mask & INTVAL(XEXP(x, 1)));
            ...
            if (GET_CODE(x) == AND && GET_CODE(XEXP(x, 1)) == CONST_INT
                ...
                && GET_MODE_BITSIZE(GET_MODE(x)) <= 32)
            {
                int32_t cval = (INTVAL(XEXP(x, 1)) | (GET_MODE_MASK(GET_MODE(x)) & ~mask));
    ...
    case PLUS:
...

        {
            ...
            /* If MODE is narrower than int32_t and mask is a negative
               number, sign extend it.  */

            if (width < 32 && (smask & ((int32_t)1 << (width - 1))) != 0)
                smask |= (int32_t)-1 << width;

            if (GET_CODE(XEXP(x, 1)) == CONST_INT && exact_log2(-smask) >= 0)
            {
#ifdef STACK_BIAS
                if (STACK_BIAS
                    && (XEXP(x, 0) == stack_pointer_rtx || XEXP(x, 0) == frame_pointer_rtx))
                {
                    int sp_alignment = STACK_BOUNDARY / BITS_PER_UNIT;
                    uint32_t sp_mask = GET_MODE_MASK(mode);

                    sp_mask &= ~(sp_alignment - 1);
                    if ((sp_mask & ~mask) == 0 && ((INTVAL(XEXP(x, 1)) - STACK_BIAS) & ~mask) != 0)
                        return force_to_mode(
                ...
                if ((nonzero_bits(XEXP(x, 0), mode) & ~mask) == 0
                    ...
                    return force_to_mode(plus_constant(XEXP(x, 0), INTVAL(XEXP(x, 1)) & mask), mode,
    ...
    case XOR:
        ...
        if (GET_CODE(XEXP(x, 0)) == LSHIFTRT && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT
            && INTVAL(XEXP(XEXP(x, 0), 1)) >= 0
            && INTVAL(XEXP(XEXP(x, 0), 1)) < 32
            && GET_CODE(XEXP(x, 1)) == CONST_INT
            && ((INTVAL(XEXP(XEXP(x, 0), 1)) + floor_log2(INTVAL(XEXP(x, 1))))
                   < GET_MODE_BITSIZE(GET_MODE(x)))
            && (INTVAL(XEXP(x, 1)) & ~nonzero_bits(XEXP(x, 0), GET_MODE(x))) == 0)
        {
            temp = GEN_INT((INTVAL(XEXP(x, 1)) & mask) << INTVAL(XEXP(XEXP(x, 0), 1)));
            temp = gen_binary(GET_CODE(x), GET_MODE(x), XEXP(XEXP(x, 0), 0), temp);
    ...
    binop:
        ...
        op0 = gen_lowpart_for_combine(
            op_mode, force_to_mode(XEXP(x, 0), mode, mask, reg, next_select));
        op1 = gen_lowpart_for_combine(
            op_mode, force_to_mode(XEXP(x, 1), mode, mask, reg, next_select));
        ...
        /* If OP1 is a CONST_INT and X is an IOR or XOR, clear bits outside
           ...
           on them being off.  */
        if (GET_CODE(op1) == CONST_INT && (code == IOR || code == XOR) && (INTVAL(op1) & mask) != 0)
    ...
    case ASHIFT:
        ...
        if (!(GET_CODE(XEXP(x, 1)) == CONST_INT && INTVAL(XEXP(x, 1)) >= 0
            ...
            && !(GET_MODE(XEXP(x, 1)) != VOIDmode
                   && (nonzero_bits(XEXP(x, 1), GET_MODE(XEXP(x, 1)))
                          < (uint32_t)GET_MODE_BITSIZE(mode))))
            break;
        ...
        if (GET_CODE(XEXP(x, 1)) == CONST_INT && INTVAL(XEXP(x, 1)) >= 0
            ...
            && GET_MODE_BITSIZE(op_mode) <= 32)
            mask >>= INTVAL(XEXP(x, 1));
        else
            mask = fuller_mask;

        op0 = gen_lowpart_for_combine(
            op_mode, force_to_mode(XEXP(x, 0), op_mode, mask, reg, next_select));
    ...
    case LSHIFTRT:
        ...
        if (GET_CODE(XEXP(x, 1)) == CONST_INT && INTVAL(XEXP(x, 1)) < 32
            && GET_MODE_BITSIZE(op_mode) <= 32)
        {
            rtx inner = XEXP(x, 0);
            ...
            /* We can only change the mode of the shift if we can do arithmetic
               ...
               OP_MODE.  */
            if (GET_MODE_BITSIZE(op_mode) > 32
        ...
        if (GET_CODE(x) == LSHIFTRT && GET_CODE(XEXP(x, 1)) == CONST_INT
            && ((INTVAL(XEXP(x, 1)) + num_sign_bit_copies(XEXP(x, 0), GET_MODE(XEXP(x, 0))))
                   >= GET_MODE_BITSIZE(GET_MODE(x)))
            && exact_log2(mask + 1) >= 0
            && (num_sign_bit_copies(XEXP(x, 0), GET_MODE(XEXP(x, 0))) >= exact_log2(mask + 1)))
            x = gen_binary(LSHIFTRT, GET_MODE(x), XEXP(x, 0),
                GEN_INT(GET_MODE_BITSIZE(GET_MODE(x)) - exact_log2(mask + 1)));
        break;
    ...
    case ASHIFTRT:
        ...
        if (GET_MODE_BITSIZE(GET_MODE(x)) <= 32
            && (mask == ((uint32_t)1 << (GET_MODE_BITSIZE(GET_MODE(x)) - 1))))
            return force_to_mode(XEXP(x, 0), mode, mask, reg, next_select);
            ...
            && INTVAL(XEXP(x, 1)) < 32)
        {
            ...
            /* If the considered data is wider then int32_t, we can't
            ...
            if (GET_MODE_BITSIZE(GET_MODE(x)) > 32)
            {
                ...
                if (GET_MODE_BITSIZE(GET_MODE(x)) - INTVAL(XEXP(x, 1)) < 32)
                    nonzero >>= INTVAL(XEXP(x, 1)) + 32
                        - GET_MODE_BITSIZE(GET_MODE(x));
            }
            else
            {
            ...
            if ((mask & ~nonzero) == 0 || (i = exact_log2(mask)) >= 0)
            {
                x = simplify_shift_const(x, LSHIFTRT, GET_MODE(x), XEXP(x, 0),
                    i < 0 ? INTVAL(XEXP(x, 1)) : GET_MODE_BITSIZE(GET_MODE(x)) - 1 - i);
        ...
        if ((GET_CODE(x) == LSHIFTRT || GET_CODE(x) == ASHIFTRT)
            ...
            && (INTVAL(XEXP(x, 1)) <= GET_MODE_BITSIZE(GET_MODE(x)) - (floor_log2(mask) + 1))
            && GET_CODE(XEXP(x, 0)) == ASHIFT && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT
            && INTVAL(XEXP(XEXP(x, 0), 1)) == INTVAL(XEXP(x, 1)))
            return force_to_mode(XEXP(XEXP(x, 0), 0), mode, mask, reg, next_select);
    ...
    case ROTATERT:
        ...
        {
            temp = simplify_binary_operation(
                code == ROTATE ? ROTATERT : ROTATE, GET_MODE(x), GEN_INT(mask), XEXP(x, 1));
            if (temp && GET_CODE(temp) == CONST_INT)
                SUBST(XEXP(x, 0),
                    force_to_mode(XEXP(x, 0), GET_MODE(x), INTVAL(temp), reg, next_select));
        }
        break;
    ...
    case NOT:
        ...
        if (GET_CODE(XEXP(x, 0)) == LSHIFTRT && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT
            && INTVAL(XEXP(XEXP(x, 0), 1)) >= 0
            && (INTVAL(XEXP(XEXP(x, 0), 1)) + floor_log2(mask) < GET_MODE_BITSIZE(GET_MODE(x)))
            && INTVAL(XEXP(XEXP(x, 0), 1)) < 32)
        {
    ...
    case IF_THEN_ELSE:
        ...
        SUBST(XEXP(x, 1),
            gen_lowpart_for_combine(
                GET_MODE(x), force_to_mode(XEXP(x, 1), mode, mask, reg, next_select)));
        SUBST(XEXP(x, 2),
            gen_lowpart_for_combine(
                GET_MODE(x), force_to_mode(XEXP(x, 2), mode, mask, reg, next_select)));
        break;
...
static rtx if_then_else_cond(rtx x, rtx *ptrue, rtx *pfalse)
{
    ...
    if (GET_RTX_CLASS(code) == '1' && (cond0 = if_then_else_cond(XEXP(x, 0), &true0, &false0)) != 0)
    {
    ...
    else if (GET_RTX_CLASS(code) == 'c' || GET_RTX_CLASS(code) == '2' || GET_RTX_CLASS(code) == '<')
    {
        ...
        if ((cond0 != 0 || cond1 != 0) && !(cond0 != 0 && cond1 != 0 && !rtx_equal_p(cond0, cond1)))
        {
        ...
        if ((STORE_FLAG_VALUE == 1 || STORE_FLAG_VALUE == -1)
            && (code == PLUS || code == IOR || code == XOR || code == MINUS || code == UMAX)
            && GET_CODE(XEXP(x, 0)) == MULT && GET_CODE(XEXP(x, 1)) == MULT)
        {
            ...
            if (GET_RTX_CLASS(GET_CODE(cond0)) == '<' && GET_RTX_CLASS(GET_CODE(cond1)) == '<'
                && reversible_comparison_p(cond1)
                && ((GET_CODE(cond0) == reverse_condition(GET_CODE(cond1))
                        && rtx_equal_p(XEXP(cond0, 0), XEXP(cond1, 0))
                        && rtx_equal_p(XEXP(cond0, 1), XEXP(cond1, 1)))
                       || ((swap_condition(GET_CODE(cond0)) == reverse_condition(GET_CODE(cond1)))
                ...
                && !side_effects_p(x))
            {
                ...
                *pfalse = gen_binary(MULT, mode,
                    (code == MINUS ? gen_unary(NEG, mode, mode, op1) : op1), const_true_rtx);
                return cond0;
            }
        }
        ...
        /* Similarly for MULT, AND and UMIN, execpt that for these the result
           is always zero.  */
        if ((STORE_FLAG_VALUE == 1 || STORE_FLAG_VALUE == -1)
            && (code == MULT || code == AND || code == UMIN) && GET_CODE(XEXP(x, 0)) == MULT
    ...
    /* If X is a normal SUBREG with both inner and outer modes integral,
       ...
       if there is a condition.  */
        ...
        && GET_MODE_SIZE(mode) <= GET_MODE_SIZE(GET_MODE(SUBREG_REG(x)))
        && 0 != (cond0 = if_then_else_cond(SUBREG_REG(x), &true0, &false0)))
    {
        *ptrue = force_to_mode(true0, mode, GET_MODE_MASK(mode), NULL_RTX, 0);
        *pfalse = force_to_mode(false0, mode, GET_MODE_MASK(mode), NULL_RTX, 0);
    ...
    else if (CONSTANT_P(x) || ((cond0 = get_last_value(x)) != 0 && CONSTANT_P(cond0)))
        ;
...
static rtx known_cond(rtx x, enum rtx_code cond, rtx reg, rtx val)
{
    ...
    if (code == ABS && rtx_equal_p(XEXP(x, 0), reg) && val == const0_rtx)
        ...
        case LT:
        case LE:
            return gen_unary(NEG, GET_MODE(XEXP(x, 0)), GET_MODE(XEXP(x, 0)), XEXP(x, 0));
        default:
            break;
        }
    ...
    else if (GET_RTX_CLASS(code) == '<' || GET_RTX_CLASS(code) == 'c')
    {
        ...
        if (rtx_equal_p(XEXP(x, 0), reg) && rtx_equal_p(XEXP(x, 1), val))
        {
            if (GET_RTX_CLASS(code) == '<')
                return (comparison_dominates_p(cond, code)
                        ? const_true_rtx
                        : (comparison_dominates_p(cond, reverse_condition(code)) ? const0_rtx : x));

            else if (code == SMAX || code == SMIN || code == UMIN || code == UMAX)
            {
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        if (fmt[i] == 'e')
            ...
            for (j = XVECLEN(x, i) - 1; j >= 0; j--)
                SUBST(XVECEXP(x, i, j), known_cond(XVECEXP(x, i, j), cond, reg, val));
    }
...
static int rtx_equal_for_field_assignment_p(rtx x, rtx y)
{
    ...
    if (GET_CODE(x) == MEM && GET_CODE(y) == SUBREG && GET_CODE(SUBREG_REG(y)) == MEM
        && rtx_equal_p(SUBREG_REG(y), gen_lowpart_for_combine(GET_MODE(SUBREG_REG(y)), x)))
        return 1;

    if (GET_CODE(y) == MEM && GET_CODE(x) == SUBREG && GET_CODE(SUBREG_REG(x)) == MEM
        && rtx_equal_p(SUBREG_REG(x), gen_lowpart_for_combine(GET_MODE(SUBREG_REG(x)), y)))
        return 1;
...
static rtx make_field_assignment(rtx x)
{
    ...
    if (GET_CODE(src) == AND && GET_CODE(XEXP(src, 0)) == ROTATE
        && GET_CODE(XEXP(XEXP(src, 0), 0)) == CONST_INT && INTVAL(XEXP(XEXP(src, 0), 0)) == -2
        && rtx_equal_for_field_assignment_p(dest, XEXP(src, 1)))
    {
        assign = make_extraction(VOIDmode, dest, 0, XEXP(XEXP(src, 0), 1), 1, 1, 1, 0);
        if (assign != 0)
        ...
        && rtx_equal_for_field_assignment_p(dest, XEXP(src, 1)))
    {
        assign = make_extraction(VOIDmode, dest, 0, XEXP(SUBREG_REG(XEXP(src, 0)), 1), 1, 1, 1, 0);
        if (assign != 0)
            return gen_rtx_SET(VOIDmode, assign, const0_rtx);
        return x;
    }
    ...
    if (GET_CODE(rhs) == AND && GET_CODE(XEXP(rhs, 1)) == CONST_INT
        && rtx_equal_for_field_assignment_p(XEXP(rhs, 0), dest))
        c1 = INTVAL(XEXP(rhs, 1)), other = lhs;
    else if (GET_CODE(lhs) == AND && GET_CODE(XEXP(lhs, 1)) == CONST_INT
    ...
    if (pos < 0 || pos + len > GET_MODE_BITSIZE(GET_MODE(dest))
        || GET_MODE_BITSIZE(GET_MODE(dest)) > 32
    ...
    mode = (GET_CODE(assign) == STRICT_LOW_PART ? GET_MODE(XEXP(assign, 0)) : GET_MODE(assign));

    /* Shift OTHER right POS places and make it the source, restricting it
       to the proper length and mode.  */

    src = force_to_mode(simplify_shift_const(NULL_RTX, LSHIFTRT, GET_MODE(src), other, pos), mode,
        GET_MODE_BITSIZE(mode) >= 32 ? GET_MODE_MASK(mode)
                                                         : ((int32_t)1 << len) - 1,
        dest, 0);
...
static rtx apply_distributive_law(rtx x)
{
    ...
    if (code != IOR && code != AND && code != XOR && code != PLUS && code != MINUS)
        return x;
    ...
    /* If either operand is a primitive we can't do anything, so get out
       fast.  */
    if (GET_RTX_CLASS(GET_CODE(lhs)) == 'o' || GET_RTX_CLASS(GET_CODE(rhs)) == 'o')
        return x;
    ...
    case SUBREG:
        ...
        if (GET_MODE(SUBREG_REG(lhs)) != GET_MODE(SUBREG_REG(rhs))
            || SUBREG_WORD(lhs) != SUBREG_WORD(rhs) || !subreg_lowpart_p(lhs)
            || (GET_MODE_CLASS(GET_MODE(lhs)) != GET_MODE_CLASS(GET_MODE(SUBREG_REG(lhs))))
            || (GET_MODE_SIZE(GET_MODE(lhs)) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(lhs))))
            || GET_MODE_SIZE(GET_MODE(SUBREG_REG(lhs))) > UNITS_PER_WORD)
            return x;

        tem = gen_binary(code, GET_MODE(SUBREG_REG(lhs)), SUBREG_REG(lhs), SUBREG_REG(rhs));
    ...
    /* Set LHS and RHS to the inner operands (A and B in the example
       ...
       commutative.  */
    if (GET_RTX_CLASS(inner_code) == 'c' && rtx_equal_p(XEXP(lhs, 0), XEXP(rhs, 0)))
        other = XEXP(lhs, 0), lhs = XEXP(lhs, 1), rhs = XEXP(rhs, 1);
    else if (GET_RTX_CLASS(inner_code) == 'c' && rtx_equal_p(XEXP(lhs, 0), XEXP(rhs, 1)))
        other = XEXP(lhs, 0), lhs = XEXP(lhs, 1), rhs = XEXP(rhs, 0);
    else if (GET_RTX_CLASS(inner_code) == 'c' && rtx_equal_p(XEXP(lhs, 1), XEXP(rhs, 0)))
    ...
    return gen_binary(inner_code, GET_MODE(x), apply_distributive_law(tem), other);
}
...
static rtx simplify_and_const_int(rtx x, enum machine_mode mode, rtx varop, uint32_t constop)
{
    uint32_t nonzero;
    int width = GET_MODE_BITSIZE(mode);
    int i;
    ...
    if (BITS_PER_WORD < 32 && BITS_PER_WORD == width
        && (nonzero & ((int32_t)1 << (width - 1))))
        nonzero |= ((int32_t)(-1) << width);
    ...
    if (GET_CODE(varop) == IOR || GET_CODE(varop) == XOR)
        return gen_lowpart_for_combine(mode,
            apply_distributive_law(gen_binary(GET_CODE(varop), GET_MODE(varop),
                simplify_and_const_int(NULL_RTX, GET_MODE(varop), XEXP(varop, 0), constop),
                simplify_and_const_int(NULL_RTX, GET_MODE(varop), XEXP(varop, 1), constop))));
    ...
    if (x && GET_CODE(XEXP(x, 0)) == SUBREG && GET_MODE(XEXP(x, 0)) == mode
    ...
    {
        if (GET_CODE(XEXP(x, 1)) != CONST_INT || (uint32_t)INTVAL(XEXP(x, 1)) != constop)
...
static uint32_t nonzero_bits(rtx x, enum machine_mode mode)
{
    uint32_t nonzero = GET_MODE_MASK(mode);
    uint32_t inner_nz;
    enum rtx_code code;
    int mode_width = GET_MODE_BITSIZE(mode);
    rtx tem;
    ...
    case REG:
        ...
        if ((x == frame_pointer_rtx || x == stack_pointer_rtx
                || (REGNO(x) >= FIRST_VIRTUAL_REGISTER && REGNO(x) <= LAST_VIRTUAL_REGISTER))
#ifdef STACK_BIAS
            && !STACK_BIAS
#endif
        )
        ...
        if (reg_last_set_value[REGNO(x)] != 0 && reg_last_set_mode[REGNO(x)] == mode
            && (REG_N_SETS(REGNO(x)) == 1 || reg_last_set_label[REGNO(x)] == label_tick)
        ...
        if (tem)
        {
            ...
            if (GET_MODE_BITSIZE(GET_MODE(x)) < mode_width && GET_CODE(tem) == CONST_INT
                && INTVAL(tem) > 0
                && 0 != (INTVAL(tem) & ((int32_t)1 << (GET_MODE_BITSIZE(GET_MODE(x)) - 1))))
                tem = GEN_INT(INTVAL(tem) | ((int32_t)(-1) << GET_MODE_BITSIZE(GET_MODE(x))));
#endif
    ...
    case CONST_INT:
        ...
        if (INTVAL(x) > 0 && mode_width < BITS_PER_WORD
            && 0 != (INTVAL(x) & ((int32_t)1 << (mode_width - 1))))
            return (INTVAL(x) | ((int32_t)(-1) << mode_width));
#endif
    ...
    case LEU:
        ...
        if (GET_MODE_CLASS(mode) == MODE_INT && mode_width <= 32)
    ...
    case SIGN_EXTEND:
        ...
        if (GET_MODE(XEXP(x, 0)) != VOIDmode)
        {
            ...
            if (inner_nz & (((int32_t)1 << (GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) - 1))))
                inner_nz |= (GET_MODE_MASK(mode) & ~GET_MODE_MASK(GET_MODE(XEXP(x, 0))));
        }
    ...
    case AND:
        nonzero &= (nonzero_bits(XEXP(x, 0), mode) & nonzero_bits(XEXP(x, 1), mode));
        break;
    ...
    case UMOD:
        ...
        {
            uint32_t nz0 = nonzero_bits(XEXP(x, 0), mode);
            uint32_t nz1 = nonzero_bits(XEXP(x, 1), mode);
            int width0 = floor_log2(nz0) + 1;
            int width1 = floor_log2(nz1) + 1;
            int low0 = floor_log2(nz0 & -nz0);
            int low1 = floor_log2(nz1 & -nz1);
            int32_t op0_maybe_minusp = (nz0 & ((int32_t)1 << (mode_width - 1)));
            int32_t op1_maybe_minusp = (nz1 & ((int32_t)1 << (mode_width - 1)));
            ...
            if (result_width < mode_width)
                nonzero &= ((int32_t)1 << result_width) - 1;

            if (result_low > 0)
                nonzero &= ~(((int32_t)1 << result_low) - 1);
        }
        break;

    case ZERO_EXTRACT:
        if (GET_CODE(XEXP(x, 1)) == CONST_INT && INTVAL(XEXP(x, 1)) < 32)
            nonzero &= ((int32_t)1 << INTVAL(XEXP(x, 1))) - 1;
        break;
    ...
    case SUBREG:
        ...
        if (SUBREG_PROMOTED_VAR_P(x) && SUBREG_PROMOTED_UNSIGNED_P(x))
            nonzero = (GET_MODE_MASK(GET_MODE(x)) & nonzero_bits(SUBREG_REG(x), GET_MODE(x)));

        /* If the inner mode is a single word for both the host and target
           machines, we can compute this from which bits of the inner
           object might be nonzero.  */
        if (GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(x))) <= BITS_PER_WORD
            && (GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(x))) <= 32))
        {
            ...
            if (LOAD_EXTEND_OP(GET_MODE(SUBREG_REG(x))) == SIGN_EXTEND
                    ? (nonzero & (1L << (GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(x))) - 1)))
...
#endif
            {
                ...
                if (GET_MODE_SIZE(GET_MODE(x)) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(x))))
                    nonzero
                        |= (GET_MODE_MASK(GET_MODE(x)) & ~GET_MODE_MASK(GET_MODE(SUBREG_REG(x))));
            }
        }
        break;
    ...
    case ROTATE:
            ...
            && INTVAL(XEXP(x, 1)) < 32)
        {
            ...
            uint32_t mode_mask = GET_MODE_MASK(inner_mode);
            uint32_t op_nonzero = nonzero_bits(XEXP(x, 0), mode);
            uint32_t inner = op_nonzero & mode_mask;
            uint32_t outer = 0;
            ...
            else if (code == ASHIFTRT)
            {
                ...
                if (inner & ((int32_t)1 << (width - 1 - count)))
                    inner |= (((int32_t)1 << count) - 1) << (width - count);
            }
            else if (code == ASHIFT)
                inner <<= count;
            else
                inner = ((inner << (count % width) | (inner >> (width - (count % width))))
                    & mode_mask);
    ...
    case FFS:
        ...
        nonzero = ((int32_t)1 << (floor_log2(mode_width) + 1)) - 1;
        break;

    case IF_THEN_ELSE:
        nonzero &= (nonzero_bits(XEXP(x, 1), mode) | nonzero_bits(XEXP(x, 2), mode));
        break;
...
static int num_sign_bit_copies(rtx x, enum machine_mode mode)
{
    enum rtx_code code = GET_CODE(x);
    int bitwidth;
    ...
    if (bitwidth < GET_MODE_BITSIZE(GET_MODE(x)))
        return MAX(
            1, (num_sign_bit_copies(x, GET_MODE(x)) - (GET_MODE_BITSIZE(GET_MODE(x)) - bitwidth)));
    ...
    case CONST_INT:
        ...
        if (bitwidth <= 32
            && (nonzero & ((int32_t)1 << (bitwidth - 1))) != 0)
    ...
    case SUBREG:
        ...
        if (bitwidth <= GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(x))))
        {
            ...
            return MAX(1, (num0 - (GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(x))) - bitwidth)));
        }
    ...
    case TRUNCATE:
        ...
        return MAX(1, (num0 - (GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) - bitwidth)));
    ...
    case ROTATERT:
            ...
            && INTVAL(XEXP(x, 1)) < bitwidth)
        {
            ...
            return MAX(
                1, num0 - (code == ROTATE ? INTVAL(XEXP(x, 1)) : bitwidth - INTVAL(XEXP(x, 1))));
        }
        break;
    ...
    case NEG:
        ...
        if (num0 > 1 && (((int32_t)1 << (bitwidth - 1)) & nonzero))
            num0--;
    ...
    case MINUS:
        ...
        if (code == PLUS && XEXP(x, 1) == constm1_rtx && bitwidth <= 32)
        {
            nonzero = nonzero_bits(XEXP(x, 0), mode);
            if ((((int32_t)1 << (bitwidth - 1)) & nonzero) == 0)
                return (
                    nonzero == 1 || nonzero == 0 ? bitwidth : bitwidth - floor_log2(nonzero) - 1);
        }
    ...
    case MULT:
        ...
        if (result > 0
            && (bitwidth > 32
                   || (((nonzero_bits(XEXP(x, 0), mode) & ((int32_t)1 << (bitwidth - 1)))
                           != 0)
                          && ((nonzero_bits(XEXP(x, 1), mode)
                                  & ((int32_t)1 << (bitwidth - 1)))
                                 != 0))))
            result--;
    ...
    case UDIV:
        ...
        if (bitwidth > 32)
            return 1;
        else if ((nonzero_bits(XEXP(x, 0), mode) & ((int32_t)1 << (bitwidth - 1))) != 0)
            return 1;
        else
            return num_sign_bit_copies(XEXP(x, 0), mode);
    ...
    case DIV:
        ...
        if (result > 1
            && (bitwidth > 32
                   || (nonzero_bits(XEXP(x, 1), mode) & ((int32_t)1 << (bitwidth - 1))) != 0))
            result--;
...
/* This function is called from `simplify_shift_const' to merge two
   ...
   is smaller than or equal to 32.
...
static int merge_outer_ops(enum rtx_code *pop0, int32_t *pconst0, enum rtx_code op1,
    int32_t const1, enum machine_mode mode, int *pcomp_p)
{
    enum rtx_code op0 = *pop0;
    int32_t const0 = *pconst0;
    int width = GET_MODE_BITSIZE(mode);
    ...
    const0 &= GET_MODE_MASK(mode);
    if (const0 == 0 && (op0 == IOR || op0 == XOR || op0 == PLUS))
        op0 = NIL;
    else if (const0 == 0 && op0 == AND)
        op0 = SET;
    else if ((uint32_t)const0 == GET_MODE_MASK(mode) && op0 == AND)
        op0 = NIL;
    ...
    if (BITS_PER_WORD < 32 && BITS_PER_WORD == width
        && (const0 & ((int32_t)1 << (width - 1))))
        const0 |= ((int32_t)(-1) << width);
...
static rtx simplify_shift_const(
    rtx x, enum rtx_code code, enum machine_mode result_mode, rtx varop, int count)
{
    enum rtx_code orig_code = code;
    int orig_count = count;
    enum machine_mode mode = result_mode;
    enum machine_mode shift_mode, tmode;
    int mode_words = (GET_MODE_SIZE(mode) + (UNITS_PER_WORD - 1)) / UNITS_PER_WORD;
    /* We form (outer_op (code varop count) (outer_const)).  */
    enum rtx_code outer_op = NIL;
    int32_t outer_const = 0;
    rtx const_rtx;
    int complement_p = 0;
    rtx ret;
    ...
    while (count != 0)
    {
        ...
        /* An arithmetic right shift of a quantity known to be -1 or 0
           is a no-op.  */
        if (code == ASHIFTRT
            && (num_sign_bit_copies(varop, shift_mode) == GET_MODE_BITSIZE(shift_mode)))
        {
            count = 0;
            break;
        }
        ...
        if (code == ASHIFTRT
            && (count + num_sign_bit_copies(varop, shift_mode) >= GET_MODE_BITSIZE(shift_mode)))
            count = GET_MODE_BITSIZE(shift_mode) - 1;
        ...
        if (GET_MODE_BITSIZE(shift_mode) <= 32 && code == ASHIFTRT
            && ((nonzero_bits(varop, shift_mode)
                    & ((int32_t)1 << (GET_MODE_BITSIZE(shift_mode) - 1)))
                   == 0))
            code = LSHIFTRT;
        ...
        case SIGN_EXTRACT:
        case ZERO_EXTRACT:
            ret = expand_compound_operation(varop);
            if (ret != varop)
            {
                varop = ret;
                continue;
        ...
        case MEM:
            ...
            if ((code == ASHIFTRT || code == LSHIFTRT) && !mode_dependent_address_p(XEXP(varop, 0))
                && !MEM_VOLATILE_P(varop)
                && (tmode = mode_for_size(GET_MODE_BITSIZE(mode) - count, MODE_INT, 1)) != BLKmode)
            {
                ret = gen_rtx_MEM(tmode, plus_constant(XEXP(varop, 0), count / BITS_PER_UNIT));
                RTX_UNCHANGING_P(ret) = RTX_UNCHANGING_P(varop);
                MEM_COPY_ATTRIBUTES(ret, varop);
                varop = gen_rtx_combine(code == ASHIFTRT ? SIGN_EXTEND : ZERO_EXTEND, mode, ret);
                count = 0;
                continue;
            }
            break;
        ...
        case USE:
            ...
            {
                ret = copy_rtx(XEXP(varop, 0));
                SUBST(XEXP(ret, 0), plus_constant(XEXP(ret, 0), count / BITS_PER_UNIT));
        ...
        case SUBREG:
            ...
            if (subreg_lowpart_p(varop)
                && (GET_MODE_SIZE(GET_MODE(SUBREG_REG(varop))) > GET_MODE_SIZE(GET_MODE(varop)))
                && (((GET_MODE_SIZE(GET_MODE(SUBREG_REG(varop))) + (UNITS_PER_WORD - 1))
                        / UNITS_PER_WORD)
                       == mode_words))
            {
        ...
        case MULT:
            ...
            if (GET_CODE(XEXP(varop, 1)) == CONST_INT && exact_log2(INTVAL(XEXP(varop, 1))) >= 0)
            {
        ...
        case ROTATE:
            /* Here we have two nested shifts.  The result is usually the
               AND of a ret shift with a mask.  We compute the result below.  */
            if (GET_CODE(XEXP(varop, 1)) == CONST_INT && INTVAL(XEXP(varop, 1)) >= 0
                && INTVAL(XEXP(varop, 1)) < GET_MODE_BITSIZE(GET_MODE(varop))
                && GET_MODE_BITSIZE(result_mode) <= 32
                && GET_MODE_BITSIZE(mode) <= 32)
            {
                ...
                if (code == ASHIFT && first_code == ASHIFTRT
                    && (GET_MODE_BITSIZE(result_mode) - GET_MODE_BITSIZE(GET_MODE(varop))) == count)
                {
                    /* C3 has the low-order C1 bits zero.  */

                    mask = (GET_MODE_MASK(mode) & ~(((int32_t)1 << first_count) - 1));

                    varop = simplify_and_const_int(NULL_RTX, result_mode, XEXP(varop, 0), mask);
                    varop = simplify_shift_const(NULL_RTX, ASHIFT, result_mode, varop, count);
                    count = first_count;
                    code = ASHIFTRT;
                    continue;
                }
                ...
                if (code == ASHIFTRT && first_code == ASHIFT && GET_MODE(varop) == shift_mode
                    && (num_sign_bit_copies(XEXP(varop, 0), shift_mode) > first_count))
                {
                    count -= first_count;
                    if (count < 0)
                ...
                if (code == ASHIFTRT || (code == ROTATE && first_code == ASHIFTRT)
                    || GET_MODE_BITSIZE(mode) > 32
                    || (GET_MODE(varop) != result_mode
                           && (first_code == ASHIFTRT || first_code == LSHIFTRT
                                  || first_code == ROTATE || code == ROTATE)))
                    break;
                ...
                mask_rtx = simplify_binary_operation(code, result_mode, mask_rtx, GEN_INT(count));

                /* Give up if we can't compute an outer operation to use.  */
                if (mask_rtx == 0 || GET_CODE(mask_rtx) != CONST_INT
                    || !merge_outer_ops(&outer_op, &outer_const, AND, INTVAL(mask_rtx), result_mode,
                           &complement_p))
                    break;
            ...
            else if (GET_CODE(varop) == code && GET_CODE(XEXP(varop, 1)) != CONST_INT
                && 0
                    != (ret
                           = simplify_binary_operation(code, mode, XEXP(varop, 0), GEN_INT(count))))
            {
                varop = gen_rtx_combine(code, mode, ret, XEXP(varop, 1));
                count = 0;
                continue;
            }
            break;
        ...
        case NOT:
            ...
            varop = gen_rtx_combine(XOR, mode, XEXP(varop, 0), GEN_INT(GET_MODE_MASK(mode)));
            continue;
        ...
        case XOR:
            ...
            {
                count = 0;
                varop = gen_rtx_combine(LE, GET_MODE(varop), XEXP(varop, 1), const0_rtx);
            ...
            if (GET_CODE(XEXP(varop, 1)) == CONST_INT
                && (ret = simplify_binary_operation(
                        code, result_mode, XEXP(varop, 1), GEN_INT(count)))
                    != 0
                && GET_CODE(ret) == CONST_INT
                && merge_outer_ops(&outer_op, &outer_const, GET_CODE(varop), INTVAL(ret),
            ...
            {
                rtx lhs = simplify_shift_const(NULL_RTX, code, shift_mode, XEXP(varop, 0), count);
                rtx rhs = simplify_shift_const(NULL_RTX, code, shift_mode, XEXP(varop, 1), count);
        ...
        case EQ:
            /* convert (lshiftrt (eq FOO 0) C) to (xor FOO 1) if STORE_FLAG_VALUE
               ...
               that may be nonzero.  */
            if (code == LSHIFTRT && XEXP(varop, 1) == const0_rtx
                && GET_MODE(XEXP(varop, 0)) == result_mode
                && count == GET_MODE_BITSIZE(result_mode) - 1
                && GET_MODE_BITSIZE(result_mode) <= 32
                && ((STORE_FLAG_VALUE & ((int32_t)1 << (GET_MODE_BITSIZE(result_mode) - 1))))
                && nonzero_bits(XEXP(varop, 0), result_mode) == 1
                && merge_outer_ops(
                       &outer_op, &outer_const, XOR, (int32_t)1, result_mode, &complement_p))
            {
                varop = XEXP(varop, 0);
                count = 0;
                continue;
        ...
        case NEG:
            ...
            if (code == ASHIFT
                && merge_outer_ops(
                       &outer_op, &outer_const, NEG, (int32_t)0, result_mode, &complement_p))
            {
                varop = XEXP(varop, 0);
                continue;
            }
        ...
        case PLUS:
            ...
            if (code == LSHIFTRT && count == GET_MODE_BITSIZE(result_mode) - 1
                && XEXP(varop, 1) == constm1_rtx && nonzero_bits(XEXP(varop, 0), result_mode) == 1
                && merge_outer_ops(
                       &outer_op, &outer_const, XOR, (int32_t)1, result_mode, &complement_p))
            {
                count = 0;
            ...
            if ((code == ASHIFTRT || code == LSHIFTRT) && count < 32
                && nonzero_bits(XEXP(varop, 1), result_mode) >> count == 0
                && (nonzero_bits(XEXP(varop, 1), result_mode)
                       & nonzero_bits(XEXP(varop, 0), result_mode))
                    == 0)
            {
                varop = XEXP(varop, 0);
                continue;
            }
            else if ((code == ASHIFTRT || code == LSHIFTRT) && count < 32
                && GET_MODE_BITSIZE(result_mode) <= 32
                && 0 == (nonzero_bits(XEXP(varop, 0), result_mode) >> count)
                && 0
                    == (nonzero_bits(XEXP(varop, 0), result_mode)
                           & nonzero_bits(XEXP(varop, 1), result_mode)))
            {
            ...
            if (code == ASHIFT && GET_CODE(XEXP(varop, 1)) == CONST_INT
                && (ret = simplify_binary_operation(
                        ASHIFT, result_mode, XEXP(varop, 1), GEN_INT(count)))
                    != 0
                && GET_CODE(ret) == CONST_INT
                && merge_outer_ops(
                       &outer_op, &outer_const, PLUS, INTVAL(ret), result_mode, &complement_p))
            {
                varop = XEXP(varop, 0);
                continue;
            }
        ...
        case MINUS:
            ...
            {
                count = 0;
                varop = gen_rtx_combine(GT, GET_MODE(varop), XEXP(varop, 1), const0_rtx);
        ...
        case TRUNCATE:
            ...
            if (code == LSHIFTRT && GET_CODE(XEXP(varop, 0)) == LSHIFTRT
                && GET_CODE(XEXP(XEXP(varop, 0), 1)) == CONST_INT
                && (INTVAL(XEXP(XEXP(varop, 0), 1)) >= (GET_MODE_BITSIZE(GET_MODE(XEXP(varop, 0)))
                                                           - GET_MODE_BITSIZE(GET_MODE(varop)))))
            {
                rtx varop_inner = XEXP(varop, 0);

                varop_inner = gen_rtx_combine(LSHIFTRT, GET_MODE(varop_inner), XEXP(varop_inner, 0),
                    GEN_INT(count + INTVAL(XEXP(varop_inner, 1))));
                varop = gen_rtx_combine(TRUNCATE, GET_MODE(varop), varop_inner);
                count = 0;
                continue;
            }
            break;
    ...
    if (x && GET_RTX_CLASS(GET_CODE(x)) == '2' && GET_CODE(XEXP(x, 1)) == CONST_INT
        && INTVAL(XEXP(x, 1)) == count)
        const_rtx = XEXP(x, 1);
    else
        const_rtx = GEN_INT(count);

    if (x && GET_CODE(XEXP(x, 0)) == SUBREG && GET_MODE(XEXP(x, 0)) == shift_mode
        && SUBREG_REG(XEXP(x, 0)) == varop)
        varop = XEXP(x, 0);
    ...
    ret = simplify_binary_operation(code, shift_mode, varop, const_rtx);
    if (ret != 0)
        x = ret;
    else
    {
    ...
    if (outer_op != NIL && GET_CODE(x) == code && GET_CODE(XEXP(x, 1)) == CONST_INT)
        x = simplify_shift_const(x, code, shift_mode, XEXP(x, 0), INTVAL(XEXP(x, 1)));

    /* If we were doing a LSHIFTRT in a wider mode than it was originally,
       turn off all the bits that the shift would have turned off.  */
    if (orig_code == LSHIFTRT && result_mode != shift_mode)
        x = simplify_and_const_int(
            NULL_RTX, shift_mode, x, GET_MODE_MASK(result_mode) >> orig_count);
    ...
    if (outer_op != NIL)
    {
        if (GET_MODE_BITSIZE(result_mode) < 32)
        {
            ...
            if (BITS_PER_WORD < 32 && BITS_PER_WORD == width
                && (outer_const & ((int32_t)1 << (width - 1))))
                outer_const |= ((int32_t)(-1) << width);
        }
...
static int recog_for_combine(rtx *pnewpat, rtx insn, rtx *pnotes)
{
    ...
    if (insn_code_number < 0 && !check_asm_operands(pat) && GET_CODE(pat) == PARALLEL)
    {
        int pos;
    ...
    if (num_clobbers_to_add)
    {
        rtx newpat = gen_rtx_PARALLEL(VOIDmode,
            gen_rtvec(GET_CODE(pat) == PARALLEL ? XVECLEN(pat, 0) + num_clobbers_to_add
        ...
        for (i = XVECLEN(newpat, 0) - num_clobbers_to_add; i < XVECLEN(newpat, 0); i++)
        {
            if (GET_CODE(XEXP(XVECEXP(newpat, 0, i), 0)) == REG
                && !reg_dead_at_p(XEXP(XVECEXP(newpat, 0, i), 0), insn))
                return -1;
            notes = gen_rtx_EXPR_LIST(REG_UNUSED, XEXP(XVECEXP(newpat, 0, i), 0), notes);
        }
        pat = newpat;
    }
...
static rtx gen_lowpart_for_combine(enum machine_mode mode, rtx x)
{
    ...
    if (GET_MODE_SIZE(mode) > UNITS_PER_WORD
        && !((GET_MODE(x) == VOIDmode && (GET_CODE(x) == CONST_INT || GET_CODE(x) == CONST_DOUBLE))
    ...
    if (result != 0 && GET_CODE(result) == SUBREG && GET_CODE(SUBREG_REG(result)) == REG
        && REGNO(SUBREG_REG(result)) >= FIRST_PSEUDO_REGISTER
        && (GET_MODE_SIZE(GET_MODE(result)) != GET_MODE_SIZE(GET_MODE(SUBREG_REG(result)))))
    ...
    if (GET_CODE(x) == MEM)
    {
        ...
        ret = gen_rtx_MEM(mode, plus_constant(XEXP(x, 0), offset));
        RTX_UNCHANGING_P(ret) = RTX_UNCHANGING_P(x);
...
static rtx gen_rtx_combine(enum rtx_code code, enum machine_mode mode, ...)
{
    ...
    for (undo = undobuf.undos; undo != undobuf.previous_undos; undo = undo->next)
        if (!undo->is_int && GET_CODE(undo->old_contents.r) == code
...
static rtx gen_binary(enum rtx_code code, enum machine_mode mode, rtx op0, rtx op1)
{
    ...
    if (GET_RTX_CLASS(code) == 'c'
        && (GET_CODE(op0) == CONST_INT || (CONSTANT_P(op0) && GET_CODE(op1) != CONST_INT)))
    ...
    if (GET_RTX_CLASS(code) == 'c'
        && ((CONSTANT_P(op0) && GET_CODE(op1) != CONST_INT)
               || (GET_RTX_CLASS(GET_CODE(op0)) == 'o' && GET_RTX_CLASS(GET_CODE(op1)) != 'o')
               || (GET_CODE(op0) == SUBREG && GET_RTX_CLASS(GET_CODE(SUBREG_REG(op0))) == 'o'
...
static rtx gen_unary(
    enum rtx_code code, enum machine_mode mode, enum machine_mode op0_mode, rtx op0)
{
...
static enum rtx_code simplify_comparison(enum rtx_code code, rtx *pop0, rtx *pop1)
{
    ...
    while (1)
    {
        ...
        if (GET_CODE(op0) == GET_CODE(op1)
            ...
            && INTVAL(XEXP(op0, 1)) < 32 && XEXP(op0, 1) == XEXP(op1, 1))
        {
        ...
        else if (GET_CODE(op0) == AND && GET_CODE(op1) == AND && GET_CODE(XEXP(op0, 1)) == CONST_INT
            && GET_CODE(XEXP(op1, 1)) == CONST_INT)
        {
            rtx inner_op0 = XEXP(op0, 0);
            rtx inner_op1 = XEXP(op1, 0);
            int32_t c0 = INTVAL(XEXP(op0, 1));
            int32_t c1 = INTVAL(XEXP(op1, 1));
            int changed = 0;

            if (GET_CODE(inner_op0) == SUBREG && GET_CODE(inner_op1) == SUBREG
                && (GET_MODE_SIZE(GET_MODE(inner_op0))
                       > GET_MODE_SIZE(GET_MODE(SUBREG_REG(inner_op0))))
                && (GET_MODE(SUBREG_REG(inner_op0)) == GET_MODE(SUBREG_REG(inner_op1)))
                && (GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(inner_op0))) <= 32)
                && (0
                       == ((~c0)
                              & nonzero_bits(
                                    SUBREG_REG(inner_op0), GET_MODE(SUBREG_REG(inner_op0)))))
                && (0
                       == ((~c1)
                              & nonzero_bits(
                                    SUBREG_REG(inner_op1), GET_MODE(SUBREG_REG(inner_op1))))))
            {
                ...
                for (tmode = GET_CLASS_NARROWEST_MODE(GET_MODE_CLASS(GET_MODE(op0)));
                     tmode != GET_MODE(op0);
                     tmode = GET_MODE_WIDER_MODE(tmode))
                    if ((uint32_t)c0 == GET_MODE_MASK(tmode))
                    {
        ...
        /* If both operands are NOT, we can strip off the outer operation
           ...
           NEG, except that this must be an equality comparison.  */
            ...
            || (GET_CODE(op0) == NEG && GET_CODE(op1) == NEG && (code == EQ || code == NE)))
    ...
    while (GET_CODE(op1) == CONST_INT)
    {
        ...
        if (GET_MODE_CLASS(mode) != MODE_INT
            && !(mode == VOIDmode
                   && (GET_CODE(op0) == COMPARE || GET_RTX_CLASS(GET_CODE(op0)) == '<')))
            break;
        ...
        /* If we are comparing against a constant power of two and the value
           ...
           with zero.  */
        if (const_op
            && (code == EQ || code == NE || code == GE || code == GEU || code == LT || code == LTU)
            && mode_width <= 32 && exact_log2(const_op) >= 0
            && nonzero_bits(op0, mode) == (uint32_t)const_op)
        {
        ...
        if (const_op == -1
            && (code == EQ || code == NE || code == GT || code == LE || code == GEU || code == LTU)
        ...
        case LE:
            ...
            else if (const_op == 0 && mode_width <= 32
                && (nonzero_bits(op0, mode) & ((int32_t)1 << (mode_width - 1))) == 0)
                code = EQ;
            break;
        ...
        case LTU:
            ...
            else if ((mode_width <= 32)
                && (const_op == (int32_t)1 << (mode_width - 1)))
            {
        ...
        case LEU:
            ...
            /* (unsigned) <= 0x7fffffff is equivalent to >= 0.  */
                ...
                && (const_op == ((int32_t)1 << (mode_width - 1)) - 1))
            {
                const_op = 0, op1 = const0_rtx;
                code = GE;
            }
        ...
        unsigned_comparison_p = (code == LTU || code == LEU || code == GTU || code == LEU);

        /* If this is a sign bit comparison and we can do arithmetic in
           MODE, say that we will only be needing the sign bit of OP0.  */
        if (sign_bit_comparison_p && GET_MODE_BITSIZE(mode) <= 32)
            op0 = force_to_mode(
                op0, mode, ((int32_t)1 << (GET_MODE_BITSIZE(mode) - 1)), NULL_RTX, 0);
        ...
        case ZERO_EXTRACT:
            ...
            if (GET_CODE(XEXP(op0, 0)) == CONST_INT && XEXP(op0, 1) == const1_rtx
        ...
        case ROTATE:
            ...
            if (equality_comparison_p && GET_CODE(XEXP(op0, 1)) == CONST_INT
                && (tem = simplify_binary_operation(ROTATERT, mode, op1, XEXP(op0, 1))) != 0)
            {
                op0 = XEXP(op0, 0);
                op1 = tem;
                continue;
            ...
            if (const_op == 0 && sign_bit_comparison_p && GET_CODE(XEXP(op0, 1)) == CONST_INT
                && mode_width <= 32)
            {
                op0 = simplify_and_const_int(NULL_RTX, mode, XEXP(op0, 0),
                    ((int32_t)1 << (mode_width - 1 - INTVAL(XEXP(op0, 1)))));
                code = (code == LT ? NE : EQ);
                continue;
            }
        ...
        case SIGN_EXTEND:
            ...
            if (!unsigned_comparison_p
                && (GET_MODE_BITSIZE(GET_MODE(XEXP(op0, 0))) <= 32)
                && ((uint32_t)const_op
                       < (((uint32_t)1 << (GET_MODE_BITSIZE(GET_MODE(XEXP(op0, 0))) - 1)))))
            {
                op0 = XEXP(op0, 0);
                continue;
            }
        ...
        case SUBREG:
            ...
            if (mode_width <= 32 && subreg_lowpart_p(op0)
                && GET_CODE(SUBREG_REG(op0)) == PLUS
                && GET_CODE(XEXP(SUBREG_REG(op0), 1)) == CONST_INT
                && INTVAL(XEXP(SUBREG_REG(op0), 1)) < 0
                && (-INTVAL(XEXP(SUBREG_REG(op0), 1)) < (int32_t)(GET_MODE_MASK(mode) / 2))
                && (uint32_t)const_op < GET_MODE_MASK(mode) / 2
                && (0
                           == (nonzero_bits(XEXP(SUBREG_REG(op0), 0), GET_MODE(SUBREG_REG(op0)))
                                  & ~GET_MODE_MASK(mode))
                       || (num_sign_bit_copies(XEXP(SUBREG_REG(op0), 0), GET_MODE(SUBREG_REG(op0)))
            ...
            if (subreg_lowpart_p(op0) && GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(op0))) < mode_width)
                /* Fall through */;
            else
                break;
        ...
        case ZERO_EXTEND:
            if ((unsigned_comparison_p || equality_comparison_p)
                && (GET_MODE_BITSIZE(GET_MODE(XEXP(op0, 0))) <= 32)
                && ((uint32_t)const_op < GET_MODE_MASK(GET_MODE(XEXP(op0, 0)))))
            {
                op0 = XEXP(op0, 0);
                continue;
            }
        ...
        case PLUS:
            /* (eq (plus X A) B) -> (eq X (minus B A)).  We can only do
               ...
               overflows.  */
            if (equality_comparison_p
                && 0 != (tem = simplify_binary_operation(MINUS, mode, op1, XEXP(op0, 1))))
            {
                op0 = XEXP(op0, 0);
                op1 = tem;
                continue;
            ...
            if (const_op == 0 && XEXP(op0, 1) == constm1_rtx && GET_CODE(XEXP(op0, 0)) == ABS
        ...
        case MINUS:
            /* (eq (minus A B) C) -> (eq A (plus B C)) or
               ...
               overflows.  */
            if (equality_comparison_p
                && 0 != (tem = simplify_binary_operation(PLUS, mode, XEXP(op0, 1), op1)))
            {
                op0 = XEXP(op0, 0);
                op1 = tem;
                continue;
            ...
            if (equality_comparison_p
                && 0 != (tem = simplify_binary_operation(MINUS, mode, XEXP(op0, 0), op1)))
            {
                op0 = XEXP(op0, 1);
                op1 = tem;
                continue;
        ...
        case XOR:
            ...
            if (equality_comparison_p
                && 0 != (tem = simplify_binary_operation(XOR, mode, XEXP(op0, 1), op1)))
            {
                op0 = XEXP(op0, 0);
                op1 = tem;
                continue;
        ...
        case GEU:
            ...
            if (code == NE || (code == EQ && reversible_comparison_p(op0))
                || (GET_MODE_BITSIZE(GET_MODE(op0)) <= 32
                       && GET_MODE_CLASS(GET_MODE(op0)) == MODE_INT
                       && (STORE_FLAG_VALUE
                              & (((int32_t)1 << (GET_MODE_BITSIZE(GET_MODE(op0)) - 1))))
                       && (code == LT || (code == GE && reversible_comparison_p(op0)))))
            {
                code
                    = (code == LT || code == NE ? GET_CODE(op0) : reverse_condition(GET_CODE(op0)));
                op0 = tem, op1 = tem1;
                continue;
            }
            break;
        ...
        case AND:
            ...
            if (const_op == 0 && equality_comparison_p && GET_CODE(XEXP(op0, 0)) == ASHIFT
                && XEXP(XEXP(op0, 0), 0) == const1_rtx)
            {
                op0 = simplify_and_const_int(op0, mode,
                    gen_rtx_combine(LSHIFTRT, mode, XEXP(op0, 1), XEXP(XEXP(op0, 0), 1)),
                    (int32_t)1);
                continue;
            }
            ...
            if (const_op == 0 && equality_comparison_p && mode_width <= 32
                && GET_CODE(XEXP(op0, 1)) == CONST_INT && GET_CODE(XEXP(op0, 0)) == LSHIFTRT
                && GET_CODE(XEXP(XEXP(op0, 0), 1)) == CONST_INT
                && INTVAL(XEXP(XEXP(op0, 0), 1)) >= 0
                && INTVAL(XEXP(XEXP(op0, 0), 1)) < 32)
            {
            ...
            if (equality_comparison_p && const_op == 0 && GET_CODE(XEXP(op0, 1)) == CONST_INT
                && mode_width <= 32
                && ((INTVAL(XEXP(op0, 1)) & GET_MODE_MASK(mode))
                       == (uint32_t)1 << (mode_width - 1)))
            {
                op0 = XEXP(op0, 0);
                code = (code == EQ ? GE : LT);
                continue;
            ...
            if ((equality_comparison_p || unsigned_comparison_p)
                && GET_CODE(XEXP(op0, 1)) == CONST_INT
                && (i = exact_log2((INTVAL(XEXP(op0, 1)) & GET_MODE_MASK(mode)) + 1)) >= 0
                && const_op >> i == 0 && (tmode = mode_for_size(i, MODE_INT, 1)) != BLKmode)
            {
            ...
            if (GET_CODE(XEXP(op0, 0)) == SUBREG
                && ((mode_width >= GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(XEXP(op0, 0)))))
                       || subreg_lowpart_p(XEXP(op0, 0)))
                && GET_CODE(XEXP(op0, 1)) == CONST_INT && mode_width <= 32
                && (GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(XEXP(op0, 0)))) <= 32)
                && (INTVAL(XEXP(op0, 1)) & ~mask) == 0
                && 0 == (~GET_MODE_MASK(GET_MODE(SUBREG_REG(XEXP(op0, 0)))) & INTVAL(XEXP(op0, 1)))
                && (uint32_t)INTVAL(XEXP(op0, 1)) != mask
                && ((uint32_t)INTVAL(XEXP(op0, 1))
                       != GET_MODE_MASK(GET_MODE(SUBREG_REG(XEXP(op0, 0))))))

            {
                op0 = gen_lowpart_for_combine(mode,
                    gen_binary(AND, GET_MODE(SUBREG_REG(XEXP(op0, 0))), SUBREG_REG(XEXP(op0, 0)),
                        XEXP(op0, 1)));
                continue;
            }
        ...
        case ASHIFT:
            /* If we have (compare (ashift FOO N) (const_int C)) and
               ...
               zero.  */
            if (GET_CODE(XEXP(op0, 1)) == CONST_INT && INTVAL(XEXP(op0, 1)) >= 0
                && ((INTVAL(XEXP(op0, 1)) + !equality_comparison_p) < 32)
                && ((const_op & (((int32_t)1 << INTVAL(XEXP(op0, 1))) - 1)) == 0)
                && mode_width <= 32
                && (nonzero_bits(XEXP(op0, 0), mode)
                       & ~(mask >> (INTVAL(XEXP(op0, 1)) + !equality_comparison_p)))
                    == 0)
            {
            ...
            /* If this an equality comparison with zero and we are shifting
               ...
               low-order bit.  */
            if (const_op == 0 && equality_comparison_p && GET_CODE(XEXP(op0, 1)) == CONST_INT
                && INTVAL(XEXP(op0, 1)) == mode_width - 1)
            {
                op0 = simplify_and_const_int(NULL_RTX, mode, XEXP(op0, 0), (int32_t)1);
                continue;
            }
            break;
        ...
        case ASHIFTRT:
            ...
            if (equality_comparison_p && const_op == 0 && GET_CODE(XEXP(op0, 1)) == CONST_INT)
            {
                op0 = simplify_shift_const(
                    NULL_RTX, LSHIFTRT, mode, XEXP(op0, 0), INTVAL(XEXP(op0, 1)));
                continue;
            }

            /* If OP0 is a sign extension and CODE is not an unsigned comparison,
               do the comparison in a narrower mode.  */
            if (!unsigned_comparison_p && GET_CODE(XEXP(op0, 1)) == CONST_INT
                && GET_CODE(XEXP(op0, 0)) == ASHIFT && XEXP(op0, 1) == XEXP(XEXP(op0, 0), 1)
                && (tmode = mode_for_size(mode_width - INTVAL(XEXP(op0, 1)), MODE_INT, 1))
                    != BLKmode
                && ((uint32_t)const_op <= GET_MODE_MASK(tmode)
                       || ((uint32_t)-const_op <= GET_MODE_MASK(tmode))))
            {
        ...
        case LSHIFTRT:
            /* If we have (compare (xshiftrt FOO N) (const_int C)) and
               ...
               overflow occurs.  */
            if (GET_CODE(XEXP(op0, 1)) == CONST_INT && INTVAL(XEXP(op0, 1)) >= 0
                && INTVAL(XEXP(op0, 1)) < 32
                && mode_width <= 32
                && (nonzero_bits(XEXP(op0, 0), mode)
                       & (((int32_t)1 << INTVAL(XEXP(op0, 1))) - 1))
                    == 0
                && (const_op == 0 || (floor_log2(const_op) + INTVAL(XEXP(op0, 1)) < mode_width)))
            {
                const_op <<= INTVAL(XEXP(op0, 1));
                op1 = GEN_INT(const_op);
                op0 = XEXP(op0, 0);
            ...
            if (const_op == 0 && (equality_comparison_p || sign_bit_comparison_p)
                && GET_CODE(XEXP(op0, 1)) == CONST_INT && INTVAL(XEXP(op0, 1)) == mode_width - 1)
            {
                op0 = XEXP(op0, 0);
    ...
    if (GET_CODE(op0) == SUBREG && subreg_lowpart_p(op0)
        && GET_MODE_CLASS(GET_MODE(op0)) == MODE_INT && (code == NE || code == EQ)
        && ((GET_MODE_SIZE(GET_MODE(op0)) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(op0))))))
    {
        ...
        && GET_MODE_CLASS(GET_MODE(op0)) == MODE_INT && (code == NE || code == EQ)
        && (GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(op0))) <= 32)
        && (nonzero_bits(SUBREG_REG(op0), GET_MODE(SUBREG_REG(op0)))
               & ~GET_MODE_MASK(GET_MODE(op0)))
            == 0
        && (tem = gen_lowpart_for_combine(GET_MODE(SUBREG_REG(op0)), op1),
               (nonzero_bits(tem, GET_MODE(SUBREG_REG(op0))) & ~GET_MODE_MASK(GET_MODE(op0))) == 0))
    ...
    mode = GET_MODE(op0);
    if (mode != VOIDmode && GET_MODE_CLASS(mode) == MODE_INT && GET_MODE_SIZE(mode) < UNITS_PER_WORD
        && cmp_optab->handlers[(int)mode].insn_code == CODE_FOR_nothing)
        for (tmode = GET_MODE_WIDER_MODE(mode);
             (tmode != VOIDmode && GET_MODE_BITSIZE(tmode) <= 32);
             tmode = GET_MODE_WIDER_MODE(tmode))
            if (cmp_optab->handlers[(int)tmode].insn_code != CODE_FOR_nothing)
            {
                ...
                if (((code == EQ || code == NE || code == GEU || code == GTU || code == LEU
                         || code == LTU)
                ...
                if (op1 == const0_rtx && (code == LT || code == GE)
                    && GET_MODE_BITSIZE(mode) <= 32)
                {
                    op0 = gen_binary(AND, tmode, gen_lowpart_for_combine(tmode, op0),
                        GEN_INT((int32_t)1 << (GET_MODE_BITSIZE(mode) - 1)));
                    code = (code == LT) ? NE : EQ;
                    break;
                }
            }
...
static int reversible_comparison_p(rtx x)
{
    if (TARGET_FLOAT_FORMAT != IEEE_FLOAT_FORMAT || flag_fast_math || GET_CODE(x) == NE
        || GET_CODE(x) == EQ)
        return 1;
    ...
    case MODE_CC:
        ...
        return (x && GET_CODE(x) == COMPARE && !FLOAT_MODE_P(GET_MODE(XEXP(x, 0))));
...
static void update_table_tick(rtx x)
{
    ...
    if (code == REG)
    {
        int regno = REGNO(x);
        int endregno
            = regno + (regno < FIRST_PSEUDO_REGISTER ? HARD_REGNO_NREGS(regno, GET_MODE(x)) : 1);
...
static void record_value_for_reg(rtx reg, rtx insn, rtx value)
{
    int regno = REGNO(reg);
    int endregno
        = regno + (regno < FIRST_PSEUDO_REGISTER ? HARD_REGNO_NREGS(regno, GET_MODE(reg)) : 1);
    int i;
    ...
    for (i = regno; i < endregno; i++)
    {
        ...
        reg_last_set_mode[i] = (enum machine_mode)0;
    ...
    /* The value being assigned might refer to X (like in "x++;").  In that
       ...
       infinite loops.  */
    if (value && !get_last_value_validate(&value, insn, reg_last_set_label[regno], 0))
    {
        value = copy_rtx(value);
        if (!get_last_value_validate(&value, insn, reg_last_set_label[regno], 1))
            value = 0;
    }
    ...
    if (value)
    {
        ...
        reg_last_set_sign_bit_copies[regno] = num_sign_bit_copies(value, GET_MODE(reg));
    }
}
...
static void record_dead_and_set_regs_1(rtx dest, rtx setter)
{
    ...
    if (GET_CODE(dest) == REG)
    {
        ...
        else if (GET_CODE(setter) == SET && GET_CODE(SET_DEST(setter)) == SUBREG
            && SUBREG_REG(SET_DEST(setter)) == dest
            && GET_MODE_BITSIZE(GET_MODE(dest)) <= BITS_PER_WORD
            && subreg_lowpart_p(SET_DEST(setter)))
            record_value_for_reg(
                dest, record_dead_insn, gen_lowpart_for_combine(GET_MODE(dest), SET_SRC(setter)));
        else
...
static void record_dead_and_set_regs(rtx insn)
{
    ...
    for (link = REG_NOTES(insn); link; link = XEXP(link, 1))
    {
        if (REG_NOTE_KIND(link) == REG_DEAD && GET_CODE(XEXP(link, 0)) == REG)
        {
            ...
            int endregno = regno
                + (regno < FIRST_PSEUDO_REGISTER ? HARD_REGNO_NREGS(regno, GET_MODE(XEXP(link, 0)))
                                                 : 1);
...
static int get_last_value_validate(rtx *loc, rtx insn, int tick, int replace)
{
    ...
    else if (GET_CODE(x) == MEM && !RTX_UNCHANGING_P(x) && INSN_CUID(insn) <= mem_last_set)
    {
        if (replace)
            *loc = gen_rtx_CLOBBER(GET_MODE(x), const0_rtx);
        return replace;
    ...
    for (i = 0; i < len; i++)
        if ((fmt[i] == 'e' && get_last_value_validate(&XEXP(x, i), insn, tick, replace) == 0)
...
static rtx get_last_value(rtx x)
{
    ...
    if (value == 0 || (REG_N_SETS(regno) != 1 && reg_last_set_label[regno] != label_tick))
        return 0;
    ...
    if (INSN_CUID(reg_last_set[regno]) >= subst_low_cuid)
    {
        ...
        for (insn = prev_nonnote_insn(subst_insn); insn
             && ((GET_CODE(insn) == INSN && GET_CODE(PATTERN(insn)) == USE)
                    || GET_CODE(insn) == BARRIER || INSN_CUID(insn) >= subst_low_cuid);
             insn = prev_nonnote_insn(insn))
            ;

        if (insn && (set = single_set(insn)) != 0 && rtx_equal_p(SET_DEST(set), x))
        {
            value = SET_SRC(set);
            ...
            if (reg_mentioned_p(x, value))
                value = replace_rtx(copy_rtx(value), x, gen_rtx_CLOBBER(GET_MODE(x), const0_rtx));
    ...
    if (get_last_value_validate(&value, reg_last_set[regno], reg_last_set_label[regno], 0))
        return value;
    ...
    value = copy_rtx(value);
    if (get_last_value_validate(&value, reg_last_set[regno], reg_last_set_label[regno], 1))
        return value;
...
static int use_crosses_set_p(rtx x, int from_cuid)
{
    ...
    if (code == REG)
    {
        ...
        for (; regno < endreg; regno++)
            if (reg_last_set[regno] && INSN_CUID(reg_last_set[regno]) > from_cuid)
                return 1;
        return 0;
    }
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        ...
        else if (fmt[i] == 'e' && use_crosses_set_p(XEXP(x, i), from_cuid))
            return 1;
    }
    return 0;
}
...
static void reg_dead_at_p_1(rtx dest, rtx x)
{
    int regno, endregno;
    ...
    regno = REGNO(dest);
    endregno
        = regno + (regno < FIRST_PSEUDO_REGISTER ? HARD_REGNO_NREGS(regno, GET_MODE(dest)) : 1);
...
static int reg_dead_at_p(rtx reg, rtx insn)
{
    ...
    reg_dead_endregno = reg_dead_regno
        + (reg_dead_regno < FIRST_PSEUDO_REGISTER ? HARD_REGNO_NREGS(reg_dead_regno, GET_MODE(reg))
                                                  : 1);
...
static void mark_used_regs_combine(rtx x)
{
    ...
    case REG:
        ...
        if (regno < FIRST_PSEUDO_REGISTER)
        {
            ...
            if (regno == STACK_POINTER_REGNUM || (regno == ARG_POINTER_REGNUM && fixed_regs[regno])
    ...
    case SET:
    {
        ...
        while (GET_CODE(testreg) == SUBREG || GET_CODE(testreg) == ZERO_EXTRACT
            || GET_CODE(testreg) == SIGN_EXTRACT || GET_CODE(testreg) == STRICT_LOW_PART)
...
rtx remove_death(int regno, rtx insn)
{
...
static void move_deaths(rtx x, rtx maybe_kill_insn, int from_cuid, rtx to_insn, rtx *pnotes)
{
    ...
    if (code == REG)
    {
        ...
        if (before_dead && after_dead && INSN_CUID(before_dead) >= from_cuid
            && (INSN_CUID(after_dead) < INSN_CUID(to_insn)
                   || (where_dead != after_dead && INSN_CUID(after_dead) == INSN_CUID(to_insn))))
        {
            ...
            if (note != 0 && regno < FIRST_PSEUDO_REGISTER
                && (GET_MODE_SIZE(GET_MODE(XEXP(note, 0))) > GET_MODE_SIZE(GET_MODE(x))))
            {
                int deadregno = REGNO(XEXP(note, 0));
                int deadend = (deadregno + HARD_REGNO_NREGS(deadregno, GET_MODE(XEXP(note, 0))));
                int ourend = regno + HARD_REGNO_NREGS(regno, GET_MODE(x));
                int i;

                for (i = deadregno; i < deadend; i++)
                    if (i < regno || i >= ourend)
                        REG_NOTES(where_dead) = gen_rtx_EXPR_LIST(
                            REG_DEAD, gen_rtx_REG(reg_raw_mode[i], i), REG_NOTES(where_dead));
            }
            /* If we didn't find any note, or if we found a REG_DEAD note that
               ...
               their own REG_DEAD notes lying around.  */
                ...
                && regno < FIRST_PSEUDO_REGISTER && HARD_REGNO_NREGS(regno, GET_MODE(x)) > 1)
            {
                int ourend = regno + HARD_REGNO_NREGS(regno, GET_MODE(x));
                int i, offset;
                rtx oldnotes = 0;
                ...
                for (i = regno + offset; i < ourend; i++)
                    move_deaths(gen_rtx_REG(reg_raw_mode[i], i), maybe_kill_insn, from_cuid,
                        to_insn, &oldnotes);
            }
    ...
    else if (GET_CODE(x) == SET)
    {
        ...
        if (GET_CODE(dest) == ZERO_EXTRACT || GET_CODE(dest) == STRICT_LOW_PART
            || (GET_CODE(dest) == SUBREG
                   && (((GET_MODE_SIZE(GET_MODE(dest)) + UNITS_PER_WORD - 1) / UNITS_PER_WORD)
                          == ((GET_MODE_SIZE(GET_MODE(SUBREG_REG(dest))) + UNITS_PER_WORD - 1)
                                 / UNITS_PER_WORD))))
        {
        ...
        if (GET_CODE(dest) == MEM)
            move_deaths(XEXP(dest, 0), maybe_kill_insn, from_cuid, to_insn, pnotes);
        return;
    }
    ...
    for (i = 0; i < len; i++)
    {
        if (fmt[i] == 'E')
        {
            int j;
            for (j = XVECLEN(x, i) - 1; j >= 0; j--)
                move_deaths(XVECEXP(x, i, j), maybe_kill_insn, from_cuid, to_insn, pnotes);
        }
        else if (fmt[i] == 'e')
...
static void distribute_notes(rtx notes, rtx from_insn, rtx i3, rtx i2, rtx elim_i2, rtx elim_i1)
{
    ...
    for (note = notes; note; note = next_note)
    {
        ...
        case REG_NOALIAS:
            ...
            if (from_insn == i3 && (XEXP(note, 0) == 0 || CONSTANT_P(XEXP(note, 0))))
                place = i3;
            break;
        ...
        case REG_DEAD:
            ...
            if (from_insn && GET_CODE(from_insn) == CALL_INSN
            ...
            if (place == 0)
            {
                for (tem = prev_nonnote_insn(i3);
                     place == 0 && tem && (GET_CODE(tem) == INSN || GET_CODE(tem) == CALL_INSN);
                     tem = prev_nonnote_insn(tem))
                {
                    ...
                    if (reg_set_p(XEXP(note, 0), PATTERN(tem)))
                    {
                        ...
                        if (set != 0)
                            for (inner_dest = SET_DEST(set); GET_CODE(inner_dest) == STRICT_LOW_PART
                        ...
                        )
                        {
                            ...
                            distribute_notes(
                                REG_NOTES(tem), tem, tem, NULL_RTX, NULL_RTX, NULL_RTX);
                            ...
                            if (cc0_setter)
                            {
                                ...
                                distribute_notes(REG_NOTES(cc0_setter), cc0_setter, cc0_setter,
                                    NULL_RTX, NULL_RTX, NULL_RTX);
                        ...
                        else if (reg_referenced_p(XEXP(note, 0), PATTERN(tem)))
                        {
                            ...
                            if (!find_regno_note(tem, REG_UNUSED, REGNO(XEXP(note, 0))))
                                REG_NOTES(tem)
                                    = gen_rtx_EXPR_LIST(REG_UNUSED, XEXP(note, 0), REG_NOTES(tem));
                        }
                        else
                        {
                        ...
                        || (GET_CODE(tem) == CALL_INSN && find_reg_fusage(tem, USE, XEXP(note, 0))))
                    {
                        place = tem;
                        ...
                        if (i2 && INSN_UID(place) <= max_uid_cuid
                            && INSN_CUID(place) > INSN_CUID(i2) && from_insn
                ...
                {
                    place = emit_insn_after(gen_rtx_USE(VOIDmode, XEXP(note, 0)), tem);
            ...
            if (place && REG_NOTE_KIND(note) == REG_DEAD)
            {
                ...
                {
                    int endregno = regno + HARD_REGNO_NREGS(regno, GET_MODE(XEXP(note, 0)));
                    int all_used = 1;
                    int i;
                    ...
                    for (i = regno; i < endregno; i++)
                            ...
                            && !find_regno_fusage(place, USE, i))
                        {
                            ...
                            for (p = place; GET_CODE(PREV_INSN(p)) == INSN
                                 && GET_CODE(PATTERN(PREV_INSN(p))) == USE;
                                 p = PREV_INSN(p))
                                if (rtx_equal_p(piece, XEXP(PATTERN(PREV_INSN(p)), 0)))
                                {
                                    p = 0;
                                    break;
                                }
                            ...
                            if (p)
                            {
                                rtx use_insn = emit_insn_before(gen_rtx_USE(VOIDmode, piece), p);
                                REG_NOTES(use_insn)
                                    = gen_rtx_EXPR_LIST(REG_DEAD, piece, REG_NOTES(use_insn));
                            }
                    ...
                    if (!all_used)
                    {
                        ...
                        for (i = regno; i < endregno; i++)
                        {
                            ...
                            if ((reg_referenced_p(piece, PATTERN(place))
                                ...
                                && !reg_bitfield_target_p(piece, PATTERN(place)))
                                REG_NOTES(place)
                                    = gen_rtx_EXPR_LIST(REG_DEAD, piece, REG_NOTES(place));
                        }
        ...
        else if ((REG_NOTE_KIND(note) == REG_DEAD || REG_NOTE_KIND(note) == REG_UNUSED)
            && GET_CODE(XEXP(note, 0)) == REG)
            REG_N_DEATHS(REGNO(XEXP(note, 0)))--;
        ...
        if (place2)
        {
            ...
            REG_NOTES(place2) = gen_rtx_fmt_ee(
                GET_CODE(note), (enum machine_mode)REG_NOTE_KIND(note), XEXP(note, 0), REG_NOTES(place2));
        }
    }
}
...
static void distribute_links(rtx links)
{
    ...
    for (link = links; link; link = next_link)
    {
        ...
        if (GET_CODE(XEXP(link, 0)) == NOTE || (set = single_set(XEXP(link, 0))) == 0)
            continue;

        reg = SET_DEST(set);
        while (GET_CODE(reg) == SUBREG || GET_CODE(reg) == ZERO_EXTRACT
            || GET_CODE(reg) == SIGN_EXTRACT || GET_CODE(reg) == STRICT_LOW_PART)
            reg = XEXP(reg, 0);
        ...
        for (insn = NEXT_INSN(XEXP(link, 0)); (insn
                 && (this_basic_block == n_basic_blocks - 1
                        || BLOCK_HEAD(this_basic_block + 1) != insn));
             insn = NEXT_INSN(insn))
            if (GET_RTX_CLASS(GET_CODE(insn)) == 'i' && reg_overlap_mentioned_p(reg, PATTERN(insn)))
            {
                if (reg_referenced_p(reg, PATTERN(insn)))
                    place = insn;
                break;
            }
            else if (GET_CODE(insn) == CALL_INSN && find_reg_fusage(insn, USE, reg))
            {
                place = insn;
                break;
            }
        ...
        if (place)
        {
            ...
            if (link2 == 0)
            {
                ...
                /* Set added_links_insn to the earliest insn we added a
                   link to.  */
                if (added_links_insn == 0 || INSN_CUID(added_links_insn) > INSN_CUID(place))
...
{
    while (insn != 0 && INSN_UID(insn) > max_uid_cuid && GET_CODE(insn) == INSN
...
{
    fprintf(file,
        "\n;; Combiner totals: %d attempts, %d substitutions (%d requiring "
        "new space),\n;; %d successes.\n",
