...
tree convert_to_pointer(tree type, tree expr)
{
    ...
    case CHAR_TYPE:
        ...
        return convert_to_pointer(type, convert(type_for_size(POINTER_SIZE, 0), expr));
...
tree convert_to_real(tree type, tree expr)
{
    ...
    {
    case REAL_TYPE:
        return build1(flag_float_store ? CONVERT_EXPR : NOP_EXPR, type, expr);
...
tree convert_to_integer(tree type, tree expr)
{
    ...
    case REFERENCE_TYPE:
        if (integer_zerop(expr))
            ...
            expr = fold(build1(CONVERT_EXPR, type_for_size(POINTER_SIZE, 0), expr));
    ...
    case CHAR_TYPE:
       ...
       correctness.  */
        ...
        if (TREE_CODE_CLASS(ex_form) == '<')
        {
            ...
            || ex_form == TRUTH_OR_EXPR || ex_form == TRUTH_ORIF_EXPR || ex_form == TRUTH_XOR_EXPR)
        {
       ...
       to TYPE.  */
        else if (TREE_CODE(type) == ENUMERAL_TYPE || outprec != GET_MODE_BITSIZE(TYPE_MODE(type)))
            return build1(
                NOP_EXPR, type, convert(type_for_mode(TYPE_MODE(type), TREE_UNSIGNED(type)), expr));
       ...
       make a wider result--are handled by "shorten" in build_binary_op.  */
        ...
        case RSHIFT_EXPR:
            ...
            if (TREE_CODE(TREE_OPERAND(expr, 1)) == INTEGER_CST
                && tree_int_cst_lt(TREE_OPERAND(expr, 1),
                       convert(TREE_TYPE(TREE_OPERAND(expr, 1)), integer_one_node)))
                goto trunc1;
            break;
        ...
        trunc1:
        {
            ...
            if (outprec >= BITS_PER_WORD || TRULY_NOOP_TRUNCATION(outprec, inprec)
                ...
                || inprec > TYPE_PRECISION(TREE_TYPE(arg1)))
            {
                ...
                if (TREE_CODE(typex) == ENUMERAL_TYPE)
                    typex = type_for_size(TYPE_PRECISION(typex), TREE_UNSIGNED(typex));
                ...
                if (TYPE_PRECISION(typex) != inprec)
                {
                    ...
                    typex = ((TREE_UNSIGNED(TREE_TYPE(expr)) || TREE_UNSIGNED(TREE_TYPE(arg0))
                                 || TREE_UNSIGNED(TREE_TYPE(arg1)))
                            ? unsigned_type(typex)
                            : signed_type(typex));
                    return convert(type,
                        fold(build(ex_form, typex, convert(typex, arg0), convert(typex, arg1), 0)));
                }
            }
        }
        break;
        ...
        case BIT_NOT_EXPR:
               ...
               since we must test the sign before truncation.  */
            {
                ...
                if (TYPE_PRECISION(typex) != inprec)
                {
                    /* Don't do unsigned arithmetic where signed was wanted,
                       or vice versa.  */
                    typex = (TREE_UNSIGNED(TREE_TYPE(expr)) ? unsigned_type(typex)
                                                            : signed_type(typex));
                    return convert(
                        type, fold(build1(ex_form, typex, convert(typex, TREE_OPERAND(expr, 0)))));
                }
            }
        ...
        case COND_EXPR:
            ...
            return fold(build(COND_EXPR, type, TREE_OPERAND(expr, 0),
                convert(type, TREE_OPERAND(expr, 1)), convert(type, TREE_OPERAND(expr, 2))));
...
tree convert_to_complex(tree type, tree expr)
{
    ...
    case CHAR_TYPE:
        return build(
            COMPLEX_EXPR, type, convert(subtype, expr), convert(subtype, integer_zero_node));
    ...
    case COMPLEX_TYPE:
    {
        ...
        else
        {
            ...
            return fold(build(COMPLEX_EXPR, type,
                convert(subtype, fold(build1(REALPART_EXPR, TREE_TYPE(TREE_TYPE(expr)), expr))),
                convert(subtype, fold(build1(IMAGPART_EXPR, TREE_TYPE(TREE_TYPE(expr)), expr)))));
        }
    }
