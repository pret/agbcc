...
static rtx extract_fixed_bit_field(enum machine_mode, rtx, int, int, int, rtx, int, int);
...
void init_expmed(void)
{
    ...
    shiftadd_insn = emit_insn(gen_rtx_SET(
        VOIDmode, reg, gen_rtx_PLUS(word_mode, gen_rtx_MULT(word_mode, reg, const0_rtx), reg)));

    shiftsub_insn = emit_insn(gen_rtx_SET(
        VOIDmode, reg, gen_rtx_MINUS(word_mode, gen_rtx_MULT(word_mode, reg, const0_rtx), reg)));
    ...
    for (m = 1; m < MAX_BITS_PER_WORD; m++)
    {
        ...
        XEXP(XEXP(SET_SRC(PATTERN(shiftadd_insn)), 0), 1) = GEN_INT((int32_t)1 << m);
        if (recog(PATTERN(shiftadd_insn), shiftadd_insn, &dummy) >= 0)
            shiftadd_cost[m] = rtx_cost(SET_SRC(PATTERN(shiftadd_insn)), SET);

        XEXP(XEXP(SET_SRC(PATTERN(shiftsub_insn)), 0), 1) = GEN_INT((int32_t)1 << m);
    ...
    sdiv_pow2_cheap = (rtx_cost(gen_rtx_DIV(word_mode, reg, GEN_INT(32)), SET) <= 2 * add_cost);
    smod_pow2_cheap = (rtx_cost(gen_rtx_MOD(word_mode, reg, GEN_INT(32)), SET) <= 2 * add_cost);
         ...
         mode = GET_MODE_WIDER_MODE(mode))
    {
        ...
        {
            mul_widen_cost[(int)wider_mode] = rtx_cost(gen_rtx_MULT(wider_mode,
                                                           gen_rtx_ZERO_EXTEND(wider_mode, reg),
                                                           gen_rtx_ZERO_EXTEND(wider_mode, reg)),
                SET);
            mul_highpart_cost[(int)mode]
                = rtx_cost(gen_rtx_TRUNCATE(mode,
    ...
    rtx value, int align, int total_size)
{
    ...
    if (total_size >= 0)
        total_size -= (bitpos / BIGGEST_ALIGNMENT * (BIGGEST_ALIGNMENT / BITS_PER_UNIT));
    ...
    if (GET_MODE_SIZE(fieldmode) >= UNITS_PER_WORD
        && (GET_CODE(op0) != MEM || !SLOW_UNALIGNED_ACCESS
               || (offset * BITS_PER_UNIT % bitsize == 0 && align % GET_MODE_SIZE(fieldmode) == 0))
        && bitpos == 0 && bitsize == GET_MODE_BITSIZE(fieldmode))
    {
       ...
       can be done with just SUBREG.  */
        if (GET_MODE(op0) != fieldmode)
        {
            ...
            {
                if (GET_MODE(SUBREG_REG(op0)) == fieldmode || GET_MODE_CLASS(fieldmode) == MODE_INT)
                    op0 = SUBREG_REG(op0);
                else
            ...
            if (GET_CODE(op0) == REG)
                ...
                op0 = change_address(op0, fieldmode, plus_constant(XEXP(op0, 0), offset));
        }
    ...
    if (GET_CODE(op0) != MEM && (bitpos == 0) && bitsize == GET_MODE_BITSIZE(fieldmode)
        && (GET_MODE(op0) == fieldmode
               || (movstrict_optab->handlers[(int)fieldmode].insn_code != CODE_FOR_nothing)))
    {
        /* Get appropriate low part of the value being stored.  */
        if (GET_CODE(value) == CONST_INT || GET_CODE(value) == REG)
            value = gen_lowpart(fieldmode, value);
        else if (!(GET_CODE(value) == SYMBOL_REF || GET_CODE(value) == LABEL_REF
    ...
    if (bitsize > BITS_PER_WORD)
    {
       ...
       result in an abort.  */
        ...
        for (i = 0; i < nwords; i++)
        {
            ...
            store_bit_field(op0, MIN(BITS_PER_WORD, bitsize - i * BITS_PER_WORD),
                bitnum + bit_offset, word_mode,
                operand_subword_force(
                    value, wordnum, (GET_MODE(value) == VOIDmode ? fieldmode : GET_MODE(value))),
                align, total_size);
        }
        return value;
    }
    ...
    if (GET_CODE(op0) != MEM)
    {
        if (offset != 0 || GET_MODE_SIZE(GET_MODE(op0)) > UNITS_PER_WORD)
        {
            if (GET_CODE(op0) != REG)
            {
            ...
            op0 = gen_rtx_SUBREG(mode_for_size(BITS_PER_WORD, MODE_INT, 0), op0, offset);
        }
        offset = 0;
    }
    else
    ...
    int bitpos, rtx value, int struct_align)
{
    ...
    else
    {
       ...
       a word, we won't be doing the extraction the normal way.  */
        ...
        mode = get_best_mode(bitsize, bitpos + offset * BITS_PER_UNIT, struct_align * BITS_PER_UNIT,
            word_mode, GET_CODE(op0) == MEM && MEM_VOLATILE_P(op0));
        ...
        if (mode == VOIDmode)
        {
            ...
            store_split_bit_field(
                op0, bitsize, bitpos + offset * BITS_PER_UNIT, value, struct_align);
            return;
        }
       ...
       OFFSET.  */
        if (bitpos >= total_bits)
        {
            ...
            bitpos -= ((bitpos / total_bits) * (total_bits / BITS_PER_UNIT) * BITS_PER_UNIT);
        }
       ...
       Then alter OP0 to refer to that word.  */
        ...
        op0 = change_address(op0, mode, plus_constant(XEXP(op0, 0), offset));
    }
    ...
    if (GET_CODE(value) == CONST_INT)
    {
        ...
        if (v == 0)
            all_zero = 1;
        else if ((bitsize < 32 && v == ((int32_t)1 << bitsize) - 1)
            || (bitsize == 32 && v == -1))
            all_one = 1;
    ...
    else
    {
        ...
        if (must_and)
            value = expand_binop(mode, and_optab, value, mask_rtx(mode, 0, bitsize, 0), NULL_RTX, 1,
                OPTAB_LIB_WIDEN);
        if (bitpos > 0)
            value = expand_shift(LSHIFT_EXPR, mode, value, build_int_2(bitpos, 0), NULL_RTX, 1);
    }
    ...
    if (!all_one)
    {
        temp = expand_binop(mode, and_optab, op0, mask_rtx(mode, bitpos, bitsize, 1), subtarget, 1,
            OPTAB_LIB_WIDEN);
        subtarget = temp;
    }
    else
    ...
    if (!all_zero)
        temp = expand_binop(mode, ior_optab, temp, value, subtarget, 1, OPTAB_LIB_WIDEN);
    if (op0 != temp)
...
static void store_split_bit_field(rtx op0, int bitsize, int bitpos, rtx value, int align)
{
    int unit;
    int bitsdone = 0;
    ...
    if ((CONSTANT_P(value) && GET_CODE(value) != CONST_INT) || GET_CODE(value) == LO_SUM)
    {
        ...
        if (word && (value != word))
            ...
            value = gen_lowpart_common(word_mode,
                force_reg(GET_MODE(value) != VOIDmode ? GET_MODE(value) : word_mode, value));
    }
    ...
    while (bitsdone < bitsize)
    {
       ...
       recurse forever.  */
        ...
        if (GET_CODE(value) == CONST_INT)
            part = GEN_INT(((uint32_t)(INTVAL(value)) >> bitsdone)
                & (((int32_t)1 << thissize) - 1));
        else
            part = extract_fixed_bit_field(word_mode, value, 0, thissize, bitsdone, NULL_RTX, 1,
       ...
       the current word starting from the base register.  */
        if (GET_CODE(op0) == SUBREG)
        {
            word = operand_subword_force(
                SUBREG_REG(op0), SUBREG_WORD(op0) + offset, GET_MODE(SUBREG_REG(op0)));
            offset = 0;
        }
        ...
        store_fixed_bit_field(word, offset * unit / BITS_PER_UNIT, thissize, thispos, part, align);
        bitsdone += thissize;
    }
}
    ...
    enum machine_mode mode, enum machine_mode tmode, int align, int total_size)
{
    ...
    if (((GET_CODE(op0) != MEM
             && TRULY_NOOP_TRUNCATION(GET_MODE_BITSIZE(mode), GET_MODE_BITSIZE(GET_MODE(op0))))
            || (GET_CODE(op0) == MEM
        ...
        && ((bitsize >= BITS_PER_WORD && bitsize == GET_MODE_BITSIZE(mode)
               ...
               || (mode_for_size(bitsize, GET_MODE_CLASS(tmode), 0) != BLKmode
                     ...
                     inefficient code from the Thumb port when -mbig-endian.  */
                      && (bitpos == 0))))
    {
        enum machine_mode mode1 = mode_for_size(bitsize, GET_MODE_CLASS(tmode), 0);

        if (mode1 != GET_MODE(op0))
        {
            if (GET_CODE(op0) == SUBREG)
            {
                if (GET_MODE(SUBREG_REG(op0)) == mode1 || GET_MODE_CLASS(mode1) == MODE_INT)
                    op0 = SUBREG_REG(op0);
                else
                    /* Else we've got some float mode source being extracted into
                       a different float mode destination -- this combination of
            ...
            if (GET_CODE(op0) == REG)
                ...
                op0 = change_address(op0, mode1, plus_constant(XEXP(op0, 0), offset));
        }
        if (mode1 != mode)
    ...
    if (bitsize > BITS_PER_WORD)
    {
       ...
       be less than full.  */
        ...
        for (i = 0; i < nwords; i++)
        {
            ...
            rtx result_part
                = extract_bit_field(op0, MIN(BITS_PER_WORD, bitsize - i * BITS_PER_WORD),
                    bitnum + bit_offset, 1, target_part, mode, word_mode, align, total_size);
        ...
        target = expand_shift(LSHIFT_EXPR, mode, target,
            build_int_2(GET_MODE_BITSIZE(mode) - bitsize, 0), NULL_RTX, 0);
    ...
    if (unsignedp)
    {
                   ...
                   && (bitsize + bitpos > extzv_bitsize)))
        {
            ...
            if (GET_CODE(xop0) == MEM)
            {
                ...
                if (!((*insn_operand_predicate[(int)CODE_FOR_extzv][1])(xop0, GET_MODE(xop0))))
                {
                    ...
                    if (GET_MODE(xop0) == BLKmode
                        ...
                        bestmode = get_best_mode(
                            bitsize, bitnum, align * BITS_PER_UNIT, maxmode, MEM_VOLATILE_P(xop0));
                    else
                    ...
                    xbitpos = bitnum % unit;
                    xop0 = change_address(xop0, bestmode, plus_constant(XEXP(xop0, 0), xoffset));
                    ...
                    xop0 = change_address(xop0, byte_mode, plus_constant(XEXP(xop0, 0), xoffset));
            ...
            if (xtarget == 0 || (flag_force_mem && GET_CODE(xtarget) == MEM))
            ...
            if (GET_MODE(xtarget) != maxmode)
            {
                ...
                {
                    int wider = (GET_MODE_SIZE(maxmode) > GET_MODE_SIZE(GET_MODE(xtarget)));
            ...
            if (!((*insn_operand_predicate[(int)CODE_FOR_extzv][0])(xtarget, maxmode)))
                xtarget = gen_reg_rtx(maxmode);

            bitsize_rtx = GEN_INT(bitsize);
            bitpos_rtx = GEN_INT(xbitpos);

            pat = gen_extzv(protect_from_queue(xtarget, 1), xop0, bitsize_rtx, bitpos_rtx);
            if (pat)
            {
                emit_insn(pat);
                target = xtarget;
            ...
            else
            {
                ...
                target = extract_fixed_bit_field(
                    itmode, op0, offset, bitsize, bitpos, target, 1, align);
            }
        }
        else
        extzv_loses:
            target
                = extract_fixed_bit_field(itmode, op0, offset, bitsize, bitpos, target, 1, align);
    }
    else
    {
        /* Here we can assume that the desired field is and integer. */
        target = extract_fixed_bit_field(tmode, op0, offset, bitsize, bitpos, target, 0, align);
    }
    ...
    if (GET_MODE(target) != tmode && GET_MODE(target) != mode)
    {
       ...
       value via a SUBREG.  */
        ...
        {
            target = convert_to_mode(
                mode_for_size(GET_MODE_BITSIZE(tmode), MODE_INT, 0), target, unsignedp);
    ...
    int bitsize, int bitpos, rtx target, int unsignedp, int align)
{
    ...
    if (GET_CODE(op0) == SUBREG || GET_CODE(op0) == REG)
    {
        /* Special treatment for a bit field split across two registers.  */
        if (bitsize + bitpos > BITS_PER_WORD)
            return extract_split_bit_field(op0, bitsize, bitpos, unsignedp, align);
    }
    else
    {
        /* Get the proper mode to use for this field.  We want a mode that
       ...
       a word, we won't be doing the extraction the normal way.  */
        ...
        mode = get_best_mode(bitsize, bitpos + offset * BITS_PER_UNIT, align * BITS_PER_UNIT,
            word_mode, GET_CODE(op0) == MEM && MEM_VOLATILE_P(op0));

        if (mode == VOIDmode)
            /* The only way this should occur is if the field spans word
               boundaries.  */
            return extract_split_bit_field(
                op0, bitsize, bitpos + offset * BITS_PER_UNIT, unsignedp, align);
    ...
    if (unsignedp)
    {
        if (bitpos)
        {
            ...
            rtx subtarget
                = (target != 0 && GET_CODE(target) == REG && !REG_FUNCTION_VALUE_P(target) ? target
                                                                                           : 0);
            if (tmode != mode)
                subtarget = 0;
	  ...
	  || tmode != mode
            ...
            return expand_binop(GET_MODE(op0), and_optab, op0,
                mask_rtx(GET_MODE(op0), 0, bitsize, 0), target, 1, OPTAB_LIB_WIDEN);
        return op0;
    }
    ...
    return expand_shift(
        RSHIFT_EXPR, mode, op0, build_int_2(GET_MODE_BITSIZE(mode) - bitsize, 0), target, 0);
}
...
{
    int32_t masklow, maskhigh;

    if (bitpos < 32)
        masklow = (int32_t)-1 << bitpos;
    else
        masklow = 0;

    if (bitpos + bitsize < 32)
        masklow &= ((uint32_t)-1 >> (32 - bitpos - bitsize));

    if (bitpos <= 32)
        maskhigh = -1;
    else
        maskhigh = (int32_t)-1 << (bitpos - 32);

    if (bitpos + bitsize > 32)
        maskhigh &= ((uint32_t)-1 >> (2 * 32 - bitpos - bitsize));
    else
        maskhigh = 0;
...
static rtx extract_split_bit_field(rtx op0, int bitsize, int bitpos, int unsignedp, int align)
{
    int unit;
    int bitsdone = 0;
    rtx result = NULL_RTX;
    ...
    while (bitsdone < bitsize)
    {
       ...
       extract_fixed_bit_field wants offset in bytes.  */
        part = extract_fixed_bit_field(
            word_mode, word, offset * unit / BITS_PER_UNIT, thissize, thispos, 0, 1, align);
        bitsdone += thissize;

        /* Shift this part into place for the result.  */
        if (bitsdone != thissize)
            part = expand_shift(
                LSHIFT_EXPR, word_mode, part, build_int_2(bitsdone - thissize, 0), 0, 1);
        ...
        if (first)
            ...
            result = expand_binop(word_mode, ior_optab, part, result, NULL_RTX, 1, OPTAB_LIB_WIDEN);
    ...
    /* Signed bit field: sign-extend with two arithmetic shifts.  */
    result = expand_shift(
        LSHIFT_EXPR, word_mode, result, build_int_2(BITS_PER_WORD - bitsize, 0), NULL_RTX, 0);
    return expand_shift(
        RSHIFT_EXPR, word_mode, result, build_int_2(BITS_PER_WORD - bitsize, 0), NULL_RTX, 0);
}
...
{
    rtx value = expand_binop(GET_MODE(target), add_optab, target, inc, target, 0, OPTAB_LIB_WIDEN);
    if (value != target)
...
{
    rtx value = expand_binop(GET_MODE(target), sub_optab, target, dec, target, 0, OPTAB_LIB_WIDEN);
    if (value != target)
        emit_move_insn(target, value);
}
    ...
    rtx target, int unsignedp)
{
    ...
    if (SHIFT_COUNT_TRUNCATED)
    {
        if (GET_CODE(op1) == CONST_INT
            && ((uint32_t)INTVAL(op1) >= (uint32_t)GET_MODE_BITSIZE(mode)))
            op1 = GEN_INT((uint32_t)INTVAL(op1) % GET_MODE_BITSIZE(mode));
        else if (GET_CODE(op1) == SUBREG && SUBREG_WORD(op1) == 0)
            op1 = SUBREG_REG(op1);
    }
#endif
    ...
    for (tries = 0; temp == 0 && tries < 3; tries ++)
    {
        ...
        if (rotate)
        {
            ...
            else if (methods == OPTAB_LIB_WIDEN)
            {
               ...
               code below.  */
                ...
                tree other_amount = fold(build(MINUS_EXPR, type,
                    convert(type, build_int_2(GET_MODE_BITSIZE(mode), 0)), amount));

                shifted = force_reg(mode, shifted);

                temp = expand_shift(
                    left ? LSHIFT_EXPR : RSHIFT_EXPR, mode, shifted, new_amount, subtarget, 1);
                temp1 = expand_shift(
                    left ? RSHIFT_EXPR : LSHIFT_EXPR, mode, shifted, other_amount, 0, 1);
                return expand_binop(mode, ior_optab, temp, temp1, target, unsignedp, methods);
            }

            temp = expand_binop(
                mode, left ? rotl_optab : rotr_optab, shifted, op1, target, unsignedp, methods);

            /* If we don't have the rotate, but we are rotating by a constant
               that is in range, try a rotate in the opposite direction.  */

            if (temp == 0 && GET_CODE(op1) == CONST_INT && INTVAL(op1) > 0
                && INTVAL(op1) < GET_MODE_BITSIZE(mode))
                temp = expand_binop(mode, left ? rotr_optab : rotl_optab, shifted,
                    GEN_INT(GET_MODE_BITSIZE(mode) - INTVAL(op1)), target, unsignedp, methods);
        }
        else if (unsignedp)
            temp = expand_binop(
                mode, left ? ashl_optab : lshr_optab, shifted, op1, target, unsignedp, methods);
       ...
       use an arithmetic right-shift instead of a logical one.  */
        if (temp == 0 && !rotate && (!unsignedp || (!left && methods == OPTAB_WIDEN)))
        {
            ...
            temp = expand_binop(
                mode, left ? ashl_optab : ashr_optab, shifted, op1, target, unsignedp, methods1);
        }
...
static void synth_mult(struct algorithm *, uint32_t, int);
static uint32_t choose_multiplier(uint32_t, int, int, uint32_t *, int *, int *);
static uint32_t invert_mod2n(uint32_t, int);
/* Compute and return the best algorithm for multiplying by T.
   The algorithm must cost less than cost_limit
   If retval.cost >= COST_LIMIT, no algorithm was found and all
   other field of the returned struct are undefined.  */

static void synth_mult(struct algorithm *alg_out, uint32_t t, int cost_limit)
{
    int m;
    ...
    alg_out->cost = cost_limit;
    copy_memory((char *)best_alg->op, (char *)alg_out->op, alg_out->ops * sizeof *alg_out->op);
    copy_memory((char *)best_alg->log, (char *)alg_out->log, alg_out->ops * sizeof *alg_out->log);
}
    ...
    enum machine_mode mode, rtx op0, rtx op1, rtx target, int unsignedp)
{
    ...
    if (GET_CODE(op1) == CONST_DOUBLE && GET_MODE_CLASS(GET_MODE(op1)) == MODE_INT
        && HOST_BITS_PER_INT >= BITS_PER_WORD && CONST_DOUBLE_HIGH(op1) == 0)
        const_op1 = GEN_INT(CONST_DOUBLE_LOW(op1));
    else if (HOST_BITS_PER_INT < GET_MODE_BITSIZE(mode) && GET_CODE(op1) == CONST_INT
        && INTVAL(op1) < 0)
        const_op1 = 0;
       ...
       `unsigned int' */
        ...
        if (alg.cost < mult_cost)
        {
            ...
            for (opno = 1; opno < alg.ops; opno++)
            {
                ...
                rtx add_target
                    = (opno == alg.ops - 1 && target != 0 && variant != add_variant && !preserve)
                    ? target
                    : 0;
                ...
                case alg_shift:
                    accum
                        = expand_shift(LSHIFT_EXPR, mode, accum, build_int_2(log, 0), NULL_RTX, 0);
                    val_so_far <<= log;
                    break;

                case alg_add_t_m2:
                    tem = expand_shift(LSHIFT_EXPR, mode, op0, build_int_2(log, 0), NULL_RTX, 0);
                    accum = force_operand(
                        gen_rtx_PLUS(mode, accum, tem), add_target ? add_target : accum_target);
                    val_so_far += (int32_t)1 << log;
                    break;

                case alg_sub_t_m2:
                    tem = expand_shift(LSHIFT_EXPR, mode, op0, build_int_2(log, 0), NULL_RTX, 0);
                    accum = force_operand(
                        gen_rtx_MINUS(mode, accum, tem), add_target ? add_target : accum_target);
                    val_so_far -= (int32_t)1 << log;
                    break;

                case alg_add_t2_m:
                    accum = expand_shift(
                        LSHIFT_EXPR, mode, accum, build_int_2(log, 0), shift_subtarget, 0);
                    accum = force_operand(
                        gen_rtx_PLUS(mode, accum, op0), add_target ? add_target : accum_target);
                ...
                case alg_sub_t2_m:
                    accum = expand_shift(
                        LSHIFT_EXPR, mode, accum, build_int_2(log, 0), shift_subtarget, 0);
                    accum = force_operand(
                        gen_rtx_MINUS(mode, accum, op0), add_target ? add_target : accum_target);
                    val_so_far = (val_so_far << log) - 1;
                    break;

                case alg_add_factor:
                    tem = expand_shift(LSHIFT_EXPR, mode, accum, build_int_2(log, 0), NULL_RTX, 0);
                    accum = force_operand(
                        gen_rtx_PLUS(mode, accum, tem), add_target ? add_target : accum_target);
                ...
                case alg_sub_factor:
                    ...
                    accum = force_operand(gen_rtx_MINUS(mode, tem, accum),
                        (add_target ? add_target : preserve ? 0 : tem));
               ...
               multiplication sequences.  */
                ...
                insn = get_last_insn();
                REG_NOTES(insn) = gen_rtx_EXPR_LIST(
                    REG_EQUAL, gen_rtx_MULT(mode, op0, GEN_INT(val_so_far)), REG_NOTES(insn));
            }
    ...
    op0 = expand_binop(mode, smul_optab, op0, op1, target, unsignedp, OPTAB_LIB_WIDEN);
    if (op0 == 0)
        abort();
    return op0;
}
...
/* Choose a minimal N + 1 bit approximation to 1/D that can be used to
   ...
   The width of operations is N (should be <= 32), the
   ...
   where m is the full 32 + 1 bit multiplier.  */

static uint32_t choose_multiplier(uint32_t d, int n, int precision,
    uint32_t *multiplier_ptr, int *post_shift_ptr, int *lgup_ptr)
{
    uint32_t mhigh_hi, mhigh_lo;
    uint32_t mlow_hi, mlow_lo;
    int lgup, post_shift;
    int pow, pow2;
    uint32_t nh, nl, dummy1, dummy2;
    ...
    div_and_round_double(TRUNC_DIV_EXPR, 1, nl, nh, d, (int32_t)0, (int32_t *)&mlow_lo,
        (int32_t *)&mlow_hi, (int32_t *)&dummy1, (int32_t *)&dummy2);

    /* mhigh = (2^(N + lgup) + 2^N + lgup - precision)/d */
    if (pow2 >= 32)
        nh |= (uint32_t)1 << (pow2 - 32);
    else
        nl |= (uint32_t)1 << pow2;
    div_and_round_double(TRUNC_DIV_EXPR, 1, nl, nh, d, (int32_t)0, (int32_t *)&mhigh_lo,
        (int32_t *)&mhigh_hi, (int32_t *)&dummy1, (int32_t *)&dummy2);
    ...
    {
        uint32_t ml_lo = (mlow_hi << (32 - 1)) | (mlow_lo >> 1);
        uint32_t mh_lo = (mhigh_hi << (32 - 1)) | (mhigh_lo >> 1);
        if (ml_lo >= mh_lo)
            break;
...
static uint32_t invert_mod2n(uint32_t x, int n)
{
    ...
    mask = (n == 32 ? ~(uint32_t)0 : ((uint32_t)1 << n) - 1);
    ...
    rtx op1, rtx target, int unsignedp)
{
    ...
    tem = expand_shift(
        RSHIFT_EXPR, mode, op0, build_int_2(GET_MODE_BITSIZE(mode) - 1, 0), NULL_RTX, 0);
    tem = expand_and(tem, op1, NULL_RTX);
    adj_operand = force_operand(gen_rtx_fmt_ee(adj_code, mode, adj_operand, tem), adj_operand);

    tem = expand_shift(
        RSHIFT_EXPR, mode, op1, build_int_2(GET_MODE_BITSIZE(mode) - 1, 0), NULL_RTX, 0);
    tem = expand_and(tem, op0, NULL_RTX);
    target = force_operand(gen_rtx_fmt_ee(adj_code, mode, adj_operand, tem), target);
    ...
    rtx target, int unsignedp, int max_cost)
{
    ...
    if (GET_MODE_BITSIZE(wider_mode) <= HOST_BITS_PER_INT)
        ...
        wide_op1 = immed_double_const(cnst1,
            (unsignedp ? (int32_t)0 : -(cnst1 >> (32 - 1))),
            wider_mode);

    /* expand_mult handles constant multiplication of word_mode
       or narrower.  It does a poor job for large modes.  */
    if (size < BITS_PER_WORD && mul_cost[(int)wider_mode] + shift_cost[size - 1] < max_cost)
    {
       ...
       multiply.  Maybe change expand_binop to handle widening multiply?  */
        ...
        tem = expand_shift(RSHIFT_EXPR, wider_mode, tem, build_int_2(size, 0), NULL_RTX, 1);
    ...
    if (mul_highpart_cost[(int)mode] < max_cost)
    {
        ...
        target = expand_binop(
            mode, mul_highpart_optab, op0, wide_op1, target, unsignedp, OPTAB_DIRECT);
        if (target)
            return target;
    }
    ...
    if (mul_highpart_cost[(int)mode] + 2 * shift_cost[size - 1] + 4 * add_cost < max_cost)
    {
        ...
        if (target)
            ...
            return expand_mult_highpart_adjust(mode, target, op0, op1, target, unsignedp);
    }
    ...
    if (moptab->handlers[(int)wider_mode].insn_code != CODE_FOR_nothing
        && (mul_widen_cost[(int)wider_mode] + 2 * shift_cost[size - 1] + 4 * add_cost < max_cost))
    {
        rtx regop1 = force_reg(mode, op1);
        tem = expand_binop(wider_mode, moptab, op0, regop1, NULL_RTX, !unsignedp, OPTAB_WIDEN);
        if (tem != 0)
        {
            ...
            /* We used the wrong signedness.  Adjust the result.  */
            return expand_mult_highpart_adjust(mode, tem, op0, op1, target, unsignedp);
        }
    }
    ...
    rtx op1, rtx target, int unsignedp)
{
    ...
    static int32_t last_div_const = 0;
  ...
  size = GET_MODE_BITSIZE (mode);
    ...
    max_cost = div_cost[(int)compute_mode]
        - (rem_flag && !(last_div_const != 0 && op1_is_constant && INTVAL(op1) == last_div_const)
                  ? mul_cost[(int)compute_mode] + add_cost
                  : 0);
    ...
    if (op1 != const0_rtx)
        ...
        case TRUNC_DIV_EXPR:
            if (op1_is_constant)
            {
                if (unsignedp)
                {
                    ...
                    if (EXACT_POWER_OF_2_OR_ZERO_P(d))
                    {
                        ...
                        if (rem_flag)
                        {
                            remainder = expand_binop(compute_mode, and_optab, op0,
                                GEN_INT(((int32_t)1 << pre_shift) - 1), remainder, 1,
                                OPTAB_LIB_WIDEN);
                            if (remainder)
                                return gen_lowpart(mode, remainder);
                        }
                        quotient = expand_shift(RSHIFT_EXPR, compute_mode, op0,
                            build_int_2(pre_shift, 0), tquotient, 1);
                    }
                    else if (size <= 32)
                    {
                        if (d >= ((uint32_t)1 << (size - 1)))
                        {
                            /* Most significant bit of divisor is set; emit an scc
                               insn.  */
                            quotient
                                = emit_store_flag(tquotient, GEU, op0, op1, compute_mode, 1, 1);
                            if (quotient == 0)
                                goto fail1;
                        }
                        else
                        {
                            ...
                            mh = choose_multiplier(d, size, size, &ml, &post_shift, &dummy);
                            ...
                            if (mh != 0)
                            {
                                ...
                                extra_cost
                                    = (shift_cost[post_shift - 1] + shift_cost[1] + 2 * add_cost);
                                t1 = expand_mult_highpart(
                                    compute_mode, op0, ml, NULL_RTX, 1, max_cost - extra_cost);
                                if (t1 == 0)
                                    goto fail1;
                                t2 = force_operand(gen_rtx_MINUS(compute_mode, op0, t1), NULL_RTX);
                                t3 = expand_shift(
                                    RSHIFT_EXPR, compute_mode, t2, build_int_2(1, 0), NULL_RTX, 1);
                                t4 = force_operand(gen_rtx_PLUS(compute_mode, t1, t3), NULL_RTX);
                                quotient = expand_shift(RSHIFT_EXPR, compute_mode, t4,
                                    build_int_2(post_shift - 1, 0), tquotient, 1);
                            }
                            else
                            {
                                rtx t1, t2;
                                ...
                                t1 = expand_shift(RSHIFT_EXPR, compute_mode, op0,
                                    build_int_2(pre_shift, 0), NULL_RTX, 1);
                                extra_cost = (shift_cost[pre_shift] + shift_cost[post_shift]);
                                t2 = expand_mult_highpart(
                                    compute_mode, t1, ml, NULL_RTX, 1, max_cost - extra_cost);
                                if (t2 == 0)
                                    goto fail1;
                                quotient = expand_shift(RSHIFT_EXPR, compute_mode, t2,
                                    build_int_2(post_shift, 0), tquotient, 1);
                            }
                        }
                    }
                    ...
                    insn = get_last_insn();
                    if (insn != last && (set = single_set(insn)) != 0 && SET_DEST(set) == quotient)
                        REG_NOTES(insn) = gen_rtx_EXPR_LIST(
                            REG_EQUAL, gen_rtx_UDIV(compute_mode, op0, op1), REG_NOTES(insn));
                }
                else /* TRUNC_DIV, signed */
                {
                    uint32_t ml;
                    int lgup, post_shift;
                    int32_t d = INTVAL(op1);
                    ...
                    if (d == 1)
                        ...
                        quotient = expand_unop(compute_mode, neg_optab, op0, tquotient, 0);
                    else if (abs_d == (uint32_t)1 << (size - 1))
                    {
                        /* This case is not handled correctly below.  */
                        quotient = emit_store_flag(tquotient, EQ, op0, op1, compute_mode, 1, 1);
                        if (quotient == 0)
                            goto fail1;
                    }
                    ...
                    else if (EXACT_POWER_OF_2_OR_ZERO_P(abs_d))
                    {
                        ...
                        if (abs_d != 2 && BRANCH_COST < 3)
                        {
                            ...
                            do_cmp_and_jump(t1, const0_rtx, GE, compute_mode, label);
                            expand_inc(t1, GEN_INT(abs_d - 1));
                            emit_label(label);
                            quotient = expand_shift(
                                RSHIFT_EXPR, compute_mode, t1, build_int_2(lgup, 0), tquotient, 0);
                        }
                        else
                        {
                            rtx t1, t2, t3;
                            ...
                            t2 = expand_shift(RSHIFT_EXPR, compute_mode, t1,
                                build_int_2(size - lgup, 0), NULL_RTX, 1);
                            t3 = force_operand(gen_rtx_PLUS(compute_mode, op0, t2), NULL_RTX);
                            quotient = expand_shift(
                                RSHIFT_EXPR, compute_mode, t3, build_int_2(lgup, 0), tquotient, 0);
                        }
                        ...
                        {
                            insn = get_last_insn();
                            if (insn != last && (set = single_set(insn)) != 0
                                && SET_DEST(set) == quotient)
                                REG_NOTES(insn) = gen_rtx_EXPR_LIST(REG_EQUAL,
                                    gen_rtx_DIV(compute_mode, op0, GEN_INT(abs_d)),
                                    REG_NOTES(insn));

                            quotient = expand_unop(compute_mode, neg_optab, quotient, quotient, 0);
                        }
                    }
                    else if (size <= 32)
                    {
                        choose_multiplier(abs_d, size, size - 1, &ml, &post_shift, &lgup);
                        if (ml < (uint32_t)1 << (size - 1))
                        {
                            rtx t1, t2, t3;

                            extra_cost = (shift_cost[post_shift] + shift_cost[size - 1] + add_cost);
                            t1 = expand_mult_highpart(
                                compute_mode, op0, ml, NULL_RTX, 0, max_cost - extra_cost);
                            if (t1 == 0)
                                goto fail1;
                            t2 = expand_shift(RSHIFT_EXPR, compute_mode, t1,
                            ...
                            if (d < 0)
                                quotient
                                    = force_operand(gen_rtx_MINUS(compute_mode, t3, t2), tquotient);
                            else
                                quotient
                                    = force_operand(gen_rtx_MINUS(compute_mode, t2, t3), tquotient);
                        }
                        else
                        {
                            rtx t1, t2, t3, t4;

                            ml |= (~(uint32_t)0) << (size - 1);
                            extra_cost
                                = (shift_cost[post_shift] + shift_cost[size - 1] + 2 * add_cost);
                            t1 = expand_mult_highpart(
                                compute_mode, op0, ml, NULL_RTX, 0, max_cost - extra_cost);
                            if (t1 == 0)
                                goto fail1;
                            t2 = force_operand(gen_rtx_PLUS(compute_mode, t1, op0), NULL_RTX);
                            ...
                            if (d < 0)
                                quotient
                                    = force_operand(gen_rtx_MINUS(compute_mode, t4, t3), tquotient);
                            else
                                quotient
                                    = force_operand(gen_rtx_MINUS(compute_mode, t3, t4), tquotient);
                        }
                    }
                    else /* Too wide mode to use tricky code */
                        break;
                    ...
                    if (insn != last && (set = single_set(insn)) != 0 && SET_DEST(set) == quotient)
                        REG_NOTES(insn) = gen_rtx_EXPR_LIST(
                            REG_EQUAL, gen_rtx_DIV(compute_mode, op0, op1), REG_NOTES(insn));
                }
                break;
            }
        fail1:
        ...
        case FLOOR_MOD_EXPR:
            ...
            if (op1_is_constant && 32 >= size)
            {
                ...
                if (d > 0)
                {
                    ...
                    if (EXACT_POWER_OF_2_OR_ZERO_P(d))
                    {
                        ...
                        {
                            remainder = expand_binop(compute_mode, and_optab, op0,
                                GEN_INT(((int32_t)1 << pre_shift) - 1), remainder, 0,
                                OPTAB_LIB_WIDEN);
                            if (remainder)
                                return gen_lowpart(mode, remainder);
                        }
                        quotient = expand_shift(RSHIFT_EXPR, compute_mode, op0,
                            build_int_2(pre_shift, 0), tquotient, 0);
                    }
                    else
                    {
                        rtx t1, t2, t3, t4;

                        mh = choose_multiplier(d, size, size - 1, &ml, &post_shift, &lgup);
                        if (mh)
                            abort();

                        t1 = expand_shift(
                            RSHIFT_EXPR, compute_mode, op0, build_int_2(size - 1, 0), NULL_RTX, 0);
                        t2 = expand_binop(
                            compute_mode, xor_optab, op0, t1, NULL_RTX, 0, OPTAB_WIDEN);
                        extra_cost = (shift_cost[post_shift] + shift_cost[size - 1] + 2 * add_cost);
                        t3 = expand_mult_highpart(
                            compute_mode, t2, ml, NULL_RTX, 1, max_cost - extra_cost);
                        if (t3 != 0)
                        {
                            t4 = expand_shift(RSHIFT_EXPR, compute_mode, t3,
                                build_int_2(post_shift, 0), NULL_RTX, 1);
                            quotient = expand_binop(
                                compute_mode, xor_optab, t4, t1, tquotient, 0, OPTAB_WIDEN);
                        }
                    }
                }
                else
                {
                    ...
                    t1 = force_operand(gen_rtx_PLUS(compute_mode, op0, constm1_rtx), NULL_RTX);
                    t2 = expand_binop(compute_mode, ior_optab, op0, t1, NULL_RTX, 0, OPTAB_WIDEN);
                    nsign = expand_shift(
                        RSHIFT_EXPR, compute_mode, t2, build_int_2(size - 1, 0), NULL_RTX, 0);
                    t3 = force_operand(gen_rtx_MINUS(compute_mode, t1, nsign), NULL_RTX);
                    t4 = expand_divmod(0, TRUNC_DIV_EXPR, compute_mode, t3, op1, NULL_RTX, 0);
                    if (t4)
                    {
                        rtx t5;
                        t5 = expand_unop(compute_mode, one_cmpl_optab, nsign, NULL_RTX, 0);
                        quotient = force_operand(gen_rtx_PLUS(compute_mode, t4, t5), tquotient);
                    }
                }
            }
            ...
            if (rem_flag)
            {
                remainder = GET_CODE(target) == REG ? target : gen_reg_rtx(compute_mode);
                quotient = gen_reg_rtx(compute_mode);
            }
            else
            {
                quotient = GET_CODE(target) == REG ? target : gen_reg_rtx(compute_mode);
                remainder = gen_reg_rtx(compute_mode);
            }

            if (expand_twoval_binop(sdivmod_optab, op0, op1, quotient, remainder, 0))
            {
                ...
                tem = expand_binop(compute_mode, xor_optab, op0, op1, NULL_RTX, 0, OPTAB_WIDEN);
               ...
               by conditionally adjusting op0 *and* the result.  */
            {
                ...
                tem = expand_binop(
                    compute_mode, sdiv_optab, adjusted_op0, op1, quotient, 0, OPTAB_LIB_WIDEN);
                if (tem != quotient)
        ...
        case CEIL_MOD_EXPR:
            if (unsignedp)
            {
                if (op1_is_constant && EXACT_POWER_OF_2_OR_ZERO_P(INTVAL(op1)))
                {
                    rtx t1, t2, t3;
                    uint32_t d = INTVAL(op1);
                    t1 = expand_shift(RSHIFT_EXPR, compute_mode, op0, build_int_2(floor_log2(d), 0),
                        tquotient, 1);
                    t2 = expand_binop(
                        compute_mode, and_optab, op0, GEN_INT(d - 1), NULL_RTX, 1, OPTAB_LIB_WIDEN);
                    t3 = gen_reg_rtx(compute_mode);
                    t3 = emit_store_flag(t3, NE, t2, const0_rtx, compute_mode, 1, 1);
                    if (t3 == 0)
                    {
                        rtx lab;
                        lab = gen_label_rtx();
                        ...
                        quotient = force_operand(gen_rtx_PLUS(compute_mode, t1, t3), tquotient);
                    break;
                }
                ...
                if (expand_twoval_binop(udivmod_optab, op0, op1, quotient, remainder, 1))
                {
                    /* This could be computed with a branch-less sequence.
                       Save that for later.  */
                    rtx label = gen_label_rtx();
                   ...
                   by conditionally adjusting op0 *and* the result.  */
                {
                    ...
                    label1 = gen_label_rtx();
                    label2 = gen_label_rtx();
                    do_cmp_and_jump(adjusted_op0, const0_rtx, NE, compute_mode, label1);
                    ...
                    expand_dec(adjusted_op0, const1_rtx);
                    tem = expand_binop(
                        compute_mode, udiv_optab, adjusted_op0, op1, quotient, 1, OPTAB_LIB_WIDEN);
                    if (tem != quotient)
                        emit_move_insn(quotient, tem);
                    expand_inc(quotient, const1_rtx);
                    emit_label(label2);
            ...
            else /* signed */
            {
                   ...
                   by conditionally adjusting op0 *and* the result.  */
                {
                    ...
                    label5 = gen_label_rtx();
                    do_cmp_and_jump(op1, const0_rtx, LT, compute_mode, label2);
                    do_cmp_and_jump(adjusted_op0, const0_rtx, GT, compute_mode, label1);
                    tem = expand_binop(
                        compute_mode, sdiv_optab, adjusted_op0, op1, quotient, 0, OPTAB_LIB_WIDEN);
                    if (tem != quotient)
                        emit_move_insn(quotient, tem);
                    ...
                    emit_barrier();
                    emit_label(label2);
                    do_cmp_and_jump(adjusted_op0, const0_rtx, LT, compute_mode, label3);
                    tem = expand_binop(
                        compute_mode, sdiv_optab, adjusted_op0, op1, quotient, 0, OPTAB_LIB_WIDEN);
                    if (tem != quotient)
                        emit_move_insn(quotient, tem);
        ...
        case EXACT_DIV_EXPR:
            if (op1_is_constant && 32 >= size)
            {
                ...
                t1 = expand_mult(compute_mode, op0, GEN_INT(ml), NULL_RTX, unsignedp);
                quotient = expand_shift(
                    RSHIFT_EXPR, compute_mode, t1, build_int_2(post_shift, 0), NULL_RTX, unsignedp);

                insn = get_last_insn();
                REG_NOTES(insn) = gen_rtx_EXPR_LIST(REG_EQUAL,
                    gen_rtx_fmt_ee(unsignedp ? UDIV : DIV, compute_mode, op0, op1),
                    REG_NOTES(insn));
            }
            break;
        ...
        case ROUND_MOD_EXPR:
            if (unsignedp)
            {
                ...
                if (expand_twoval_binop(udivmod_optab, op0, op1, quotient, remainder, 1) == 0)
                {
                    rtx tem;
                    quotient = expand_binop(
                        compute_mode, udiv_optab, op0, op1, quotient, 1, OPTAB_LIB_WIDEN);
                    tem = expand_mult(compute_mode, quotient, op1, NULL_RTX, 1);
                    remainder = expand_binop(
                        compute_mode, sub_optab, op0, tem, remainder, 1, OPTAB_LIB_WIDEN);
                }
                tem = plus_constant(op1, -1);
                tem = expand_shift(RSHIFT_EXPR, compute_mode, tem, build_int_2(1, 0), NULL_RTX, 1);
            ...
            else
            {
                ...
                if (expand_twoval_binop(sdivmod_optab, op0, op1, quotient, remainder, 0) == 0)
                {
                    rtx tem;
                    quotient = expand_binop(
                        compute_mode, sdiv_optab, op0, op1, quotient, 0, OPTAB_LIB_WIDEN);
                    tem = expand_mult(compute_mode, quotient, op1, NULL_RTX, 0);
                    remainder = expand_binop(
                        compute_mode, sub_optab, op0, tem, remainder, 0, OPTAB_LIB_WIDEN);
                }
                abs_rem = expand_abs(compute_mode, remainder, NULL_RTX, 0, 0);
                abs_op1 = expand_abs(compute_mode, op1, NULL_RTX, 0, 0);
                tem = expand_shift(
                    LSHIFT_EXPR, compute_mode, abs_rem, build_int_2(1, 0), NULL_RTX, 1);
                do_cmp_and_jump(tem, abs_op1, LTU, compute_mode, label);
                tem = expand_binop(compute_mode, xor_optab, op0, op1, NULL_RTX, 0, OPTAB_WIDEN);
                mask = expand_shift(
                    RSHIFT_EXPR, compute_mode, tem, build_int_2(size - 1, 0), NULL_RTX, 0);
                tem = expand_binop(
                    compute_mode, xor_optab, mask, const1_rtx, NULL_RTX, 0, OPTAB_WIDEN);
                tem = expand_binop(compute_mode, sub_optab, tem, mask, NULL_RTX, 0, OPTAB_WIDEN);
                expand_inc(quotient, tem);
                tem = expand_binop(compute_mode, xor_optab, mask, op1, NULL_RTX, 0, OPTAB_WIDEN);
                tem = expand_binop(compute_mode, sub_optab, tem, mask, NULL_RTX, 0, OPTAB_WIDEN);
    ...
    if (quotient == 0)
    {
        ...
        if (rem_flag)
        {
            ...
            remainder = sign_expand_binop(compute_mode, umod_optab, smod_optab, op0, op1, target,
                unsignedp,
                ((optab2->handlers[(int)compute_mode].insn_code != CODE_FOR_nothing)
                        ? OPTAB_DIRECT
                        : OPTAB_WIDEN));
            if (remainder == 0)
            {
               ...
               without a library call?  */
                remainder = gen_reg_rtx(compute_mode);
                if (!expand_twoval_binop((unsignedp ? udivmod_optab : sdivmod_optab), op0, op1,
       ...
       is set to the one of the two optabs that the call below will use.  */
        quotient = sign_expand_binop(compute_mode, udiv_optab, sdiv_optab, op0, op1,
            rem_flag ? NULL_RTX : target, unsignedp,
            ((optab2->handlers[(int)compute_mode].insn_code != CODE_FOR_nothing) ? OPTAB_DIRECT
                                                                                 : OPTAB_WIDEN));
        ...
        if (quotient == 0)
        {
            ...
            quotient = gen_reg_rtx(compute_mode);
            if (!expand_twoval_binop(unsignedp ? udivmod_optab : sdivmod_optab, op0, op1, quotient,
                    NULL_RTX, unsignedp))
            {
                quotient = 0;
                if (!rem_flag)
    ...
    if (rem_flag)
    {
        ...
        if (quotient == 0)
            ...
            remainder = sign_expand_binop(
                compute_mode, umod_optab, smod_optab, op0, op1, target, unsignedp, OPTAB_LIB_WIDEN);
        else
        {
            /* We divided.  Now finish doing X - Y * (X / Y).  */
            remainder = expand_mult(compute_mode, quotient, op1, NULL_RTX, unsignedp);
            remainder = expand_binop(
                compute_mode, sub_optab, op0, remainder, target, unsignedp, OPTAB_LIB_WIDEN);
        }
    }
...
tree make_tree(tree type, rtx x)
{
    ...
    case CONST_INT:
        t = build_int_2(INTVAL(x),
            (TREE_UNSIGNED(type) && (GET_MODE_BITSIZE(TYPE_MODE(type)) < 32))
                    || INTVAL(x) >= 0
                ? 0
                : -1);
        TREE_TYPE(t) = type;
    ...
    case PLUS:
        return fold(
            build(PLUS_EXPR, type, make_tree(type, XEXP(x, 0)), make_tree(type, XEXP(x, 1))));

    case MINUS:
        return fold(
            build(MINUS_EXPR, type, make_tree(type, XEXP(x, 0)), make_tree(type, XEXP(x, 1))));
    ...
    case MULT:
        return fold(
            build(MULT_EXPR, type, make_tree(type, XEXP(x, 0)), make_tree(type, XEXP(x, 1))));

    case ASHIFT:
        return fold(
            build(LSHIFT_EXPR, type, make_tree(type, XEXP(x, 0)), make_tree(type, XEXP(x, 1))));

    case LSHIFTRT:
        return fold(convert(type,
            build(RSHIFT_EXPR, unsigned_type(type), make_tree(unsigned_type(type), XEXP(x, 0)),
                make_tree(type, XEXP(x, 1)))));

    case ASHIFTRT:
        return fold(convert(type,
            build(RSHIFT_EXPR, signed_type(type), make_tree(signed_type(type), XEXP(x, 0)),
                make_tree(type, XEXP(x, 1)))));
    ...
    case DIV:
        ...
        return fold(convert(
            type, build(TRUNC_DIV_EXPR, t, make_tree(t, XEXP(x, 0)), make_tree(t, XEXP(x, 1)))));
    case UDIV:
...
rtx expand_mult_add(rtx x, rtx target, rtx mult, rtx add, enum machine_mode mode, int unsignedp)
{
    ...
    tree add_type = (GET_MODE(add) == VOIDmode ? type : type_for_mode(GET_MODE(add), unsignedp));
...
rtx expand_and(rtx op0, rtx op1, rtx target)
{
...
rtx emit_store_flag(rtx target, enum rtx_code code, rtx op0, rtx op1, enum machine_mode mode,
    int unsignedp, int normalizep)
{
    rtx subtarget;
    ...
    if (op1 == const0_rtx && (code == LT || code == GE) && GET_MODE_CLASS(mode) == MODE_INT
        && (normalizep || STORE_FLAG_VALUE == 1
               || (GET_MODE_BITSIZE(mode) <= 32
                      && ((STORE_FLAG_VALUE & GET_MODE_MASK(mode))
                             == (int32_t)1 << (GET_MODE_BITSIZE(mode) - 1)))))
    {
        subtarget = target;
       ...
       first.  */
        ...
        if (STORE_FLAG_VALUE == 1 || normalizep)
            ...
            op0 = expand_shift(RSHIFT_EXPR, mode, op0, size_int(GET_MODE_BITSIZE(mode) - 1),
    ...
    if (icode != CODE_FOR_nothing)
    {
       ...
       if the scc insn fails.  So call it ourselves before setting LAST.  */
        ...
        comparison = compare_from_rtx(op0, op1, code, unsignedp, mode, NULL_RTX, 0);
        if (GET_CODE(comparison) == CONST_INT)
            return (comparison == const0_rtx
                    ? const0_rtx
                    : normalizep == 1 ? const1_rtx
                                      : normalizep == -1 ? constm1_rtx : const_true_rtx);
        ...
        if (pattern)
        {
            ...
            if (GET_MODE_SIZE(target_mode) > GET_MODE_SIZE(compare_mode))
            {
                convert_move(target, subtarget,
                    (GET_MODE_BITSIZE(compare_mode) <= 32)
                        && 0
                            == (STORE_FLAG_VALUE
                                   & ((int32_t)1 << (GET_MODE_BITSIZE(compare_mode) - 1))));
                op0 = target;
            ...
            else if (GET_MODE_BITSIZE(compare_mode) <= 32
                && (STORE_FLAG_VALUE & ((int32_t)1 << (GET_MODE_BITSIZE(compare_mode) - 1))))
                op0 = expand_shift(RSHIFT_EXPR, compare_mode, op0,
                    size_int(GET_MODE_BITSIZE(compare_mode) - 1), subtarget, normalizep == 1);
    ...
    subtarget = (!flag_expensive_optimizations && (target_mode == mode)) ? target : NULL_RTX;
    ...
    if (BRANCH_COST > 0 && GET_MODE_CLASS(mode) == MODE_INT && (code == EQ || code == NE)
        && op1 != const0_rtx)
    {
        tem = expand_binop(mode, xor_optab, op0, op1, subtarget, 1, OPTAB_WIDEN);

        if (tem == 0)
            tem = expand_binop(mode, sub_optab, op0, op1, subtarget, 1, OPTAB_WIDEN);
        if (tem != 0)
            tem = emit_store_flag(target, code, tem, const0_rtx, mode, unsignedp, normalizep);
        if (tem == 0)
            delete_insns_since(last);
        return tem;
    }
    ...
    if (BRANCH_COST == 0 || GET_MODE_CLASS(mode) != MODE_INT || op1 != const0_rtx
        || (code != EQ && code != NE && (BRANCH_COST <= 1 || (code != LE && code != GT))))
        return 0;
    ...
    if (normalizep == 0)
    {
        ...
        else if (GET_MODE_BITSIZE(mode) <= 32
            && ((STORE_FLAG_VALUE & GET_MODE_MASK(mode))
    ...
    if (code == LE)
    {
        ...
        tem = expand_binop(mode, sub_optab, op0, const1_rtx, subtarget, 0, OPTAB_WIDEN);
        if (tem)
            tem = expand_binop(mode, ior_optab, op0, tem, subtarget, 0, OPTAB_WIDEN);
    }
    ...
    if (code == GT)
    {
        ...
        tem = expand_shift(
            RSHIFT_EXPR, mode, op0, size_int(GET_MODE_BITSIZE(mode) - 1), subtarget, 0);
        tem = expand_binop(mode, sub_optab, tem, op0, subtarget, 0, OPTAB_WIDEN);
    }
    ...
    if (code == EQ || code == NE)
    {
       ...
       as the operation.  */
        ...
        {
            if (code == EQ)
                tem = expand_binop(mode, sub_optab, tem, const1_rtx, subtarget, 0, OPTAB_WIDEN);
            else
       ...
       are expensive.  */
        ...
        if (tem == 0 && (code == NE || BRANCH_COST > 1))
        {
            ...
            tem = expand_binop(mode, ior_optab, tem, op0, subtarget, 0, OPTAB_WIDEN);
    ...
    if (tem && normalizep)
        tem = expand_shift(RSHIFT_EXPR, mode, tem, size_int(GET_MODE_BITSIZE(mode) - 1), subtarget,
            normalizep == 1);
...
rtx emit_store_flag_force(rtx target, enum rtx_code code, rtx op0, rtx op1, enum machine_mode mode,
    int unsignedp, int normalizep)
{
    rtx tem, label;
