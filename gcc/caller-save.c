...
static enum insn_code reg_save_code[FIRST_PSEUDO_REGISTER][MAX_MOVE_MAX / MIN_UNITS_PER_WORD + 1];
...
static void insert_one_insn(struct insn_chain *, int, enum insn_code, rtx);
...
void setup_save_areas(void)
{
    ...
    for (i = FIRST_PSEUDO_REGISTER; i < max_regno; i++)
        if (reg_renumber[i] >= 0 && REG_N_CALLS_CROSSED(i) > 0)
        {
            ...
            int endregno = regno + HARD_REGNO_NREGS(regno, GET_MODE(regno_reg_rtx[i]));
    ...
    for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
        for (j = MOVE_MAX_WORDS; j > 0; j--)
        {
            ...
            regno_save_mem[i][j] = assign_stack_local(
                regno_save_mode[i][j], GET_MODE_SIZE(regno_save_mode[i][j]), 0);
            ...
            for (k = 0; k < j; k++)
            {
                ...
                rtx temp = gen_rtx_MEM(regno_save_mode[i + k][1], XEXP(regno_save_mem[i][j], 0));

                regno_save_mem[i + k][1] = adj_offsettable_operand(temp, k * UNITS_PER_WORD);
            }
        }
}
    ...
    for (chain = reload_insn_chain; chain != 0; chain = next)
    {
        ...
        if (chain->next == 0 || chain->next->block > chain->block)
        {
            ...
            if (n_regs_saved)
                for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
                    if (TEST_HARD_REG_BIT(hard_regs_saved, regno))
                        regno += insert_restore(
                            chain, GET_CODE(insn) == JUMP_INSN, regno, MOVE_MAX_WORDS);
        }
    }
}
...
static void mark_referenced_regs(rtx x)
{
    ...
    {
        int regno = REGNO(x);
        int hardregno = (regno < FIRST_PSEUDO_REGISTER ? regno : reg_renumber[regno]);
...
static int insert_restore(struct insn_chain *chain, int before_p, int regno, int maxrestore)
{
    int i;
    rtx pat = NULL_RTX;
    ...
    for (i = maxrestore; i > 0; i--)
    {
        ...
        pat = gen_rtx_SET(VOIDmode,
            gen_rtx_REG(GET_MODE(regno_save_mem[regno][i]), regno),
...
static int insert_save(struct insn_chain *chain, int before_p, int regno, HARD_REG_SET *to_save)
{
    int i;
    rtx pat = NULL_RTX;
    enum insn_code code = CODE_FOR_nothing;
...
static void insert_one_insn(struct insn_chain *chain, int before_p, enum insn_code code, rtx pat)
{
    ...
    if ((GET_CODE(insn) == INSN || GET_CODE(insn) == JUMP_INSN) && before_p
