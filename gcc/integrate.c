...
#define INTEGRATE_THRESHOLD(DECL)                                                                  \
    (optimize_size ? (1 + (3 * list_length(DECL_ARGUMENTS(DECL)) / 2))                             \
...
static void integrate_decl_tree(tree, int, struct inline_remap *);
static void save_constants_in_decl_trees(tree);
static void subst_constants(rtx *, rtx, struct inline_remap *);
...
rtx get_label_from_map(struct inline_remap *map, int i)
{
...
const char *function_cannot_inline_p(tree fndecl)
{
    ...
    if ((last && TREE_VALUE(last) != void_type_node) || current_function_varargs)
    ...
    for (insn = get_insns();
         insn && !(GET_CODE(insn) == NOTE && NOTE_LINE_NUMBER(insn) == NOTE_INSN_FUNCTION_BEG);
         insn = NEXT_INSN(insn))
    {
        if (insn && GET_CODE(insn) == NOTE && NOTE_LINE_NUMBER(insn) == NOTE_INSN_EH_REGION_BEG)
...
static rtx save_for_inline_eh_labelmap(rtx label)
{
...
static rtx initialize_for_inline(
    tree fndecl, int min_labelno, int max_labelno, int max_reg, int copy)
{
    ...
    function_flags = (current_function_calls_alloca * FUNCTION_FLAGS_CALLS_ALLOCA
    ...
    for (parms = DECL_ARGUMENTS(fndecl), i = 0; parms; parms = TREE_CHAIN(parms), i++)
    {
    ...
    return gen_inline_header_rtx(NULL_RTX, NULL_RTX, min_labelno, max_labelno, max_parm_reg,
        max_reg, current_function_args_size, current_function_pops_args, stack_slot_list,
        forced_labels, function_flags, current_function_outgoing_args_size, arg_vector,
        (rtx)DECL_INITIAL(fndecl), (rtvec)regno_reg_rtx, regno_pointer_flag, regno_pointer_align,
...
static void finish_inline(tree fndecl, rtx head)
{
...
static void adjust_copied_decl_tree(tree block)
{
    ...
    for (subblock = BLOCK_SUBBLOCKS(block); subblock; subblock = TREE_CHAIN(subblock))
...
void save_for_inline_copying(tree fndecl)
{
    ...
    for (i = max_reg - 1; i > LAST_VIRTUAL_REGISTER; i--)
        reg_map[i] = (rtx)obstack_copy(function_maybepermanent_obstack, regno_reg_rtx[i], len);
    ...
    for (i = LAST_VIRTUAL_REGISTER + 1; i < max_reg; i++)
        if (GET_CODE(regno_reg_rtx[i]) == MEM)
            XEXP(regno_reg_rtx[i], 0) = copy_for_inline(XEXP(regno_reg_rtx[i], 0));

    /* Copy the parm_reg_stack_loc array, and substitute for all of the rtx
       contained in it.  */
    new2 = (rtx *)savealloc(max_parm_reg * sizeof(rtx));
    copy_memory((char *)parm_reg_stack_loc, (char *)new2, max_parm_reg * sizeof(rtx));
    ...
    for (insn = NEXT_INSN(insn); insn; insn = NEXT_INSN(insn))
    {
        ...
        case NOTE:
            ...
            {
                int new_region = CODE_LABEL_NUMBER(label_map[NOTE_BLOCK_NUMBER(copy)]);

                /* we have to duplicate the handlers for the original */
                if (NOTE_LINE_NUMBER(copy) == NOTE_INSN_EH_REGION_BEG)
                    duplicate_eh_handlers(
                        NOTE_BLOCK_NUMBER(copy), new_region, save_for_inline_eh_labelmap);
        ...
        case CALL_INSN:
            ...
            if (GET_CODE(insn) == CALL_INSN)
                CALL_INSN_FUNCTION_USAGE(copy) = copy_for_inline(CALL_INSN_FUNCTION_USAGE(insn));
    ...
    for (insn = NEXT_INSN(get_insns()); insn; insn = NEXT_INSN(insn))
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i' && insn_map[INSN_UID(insn)])
            REG_NOTES(insn_map[INSN_UID(insn)]) = copy_for_inline(REG_NOTES(insn));
...
static tree copy_and_set_decl_abstract_origin(tree node)
{
...
static tree copy_decl_tree(tree block)
{
...
static void copy_decl_rtls(tree block)
{
    tree t;
...
void save_for_inline_nocopy(tree fndecl)
{
    rtx insn;
    rtx head;
    rtx first_nonparm_insn;
    ...
    head = initialize_for_inline(fndecl, get_first_label_num(), max_label_num(), max_reg_num(), 0);
...
static void save_constants(rtx *px)
{
    rtx x;
    int i, j;
...
again:
    ...
    {
        *px = gen_rtx_ADDRESS(GET_MODE(x), gen_rtx_CONST(get_pool_mode(x), get_pool_constant(x)));
        save_constants(&XEXP(*px, 0));
...
static void note_modified_parmregs(rtx reg, rtx x ATTRIBUTE_UNUSED)
{
    if (GET_CODE(reg) == REG && in_nonparm_insns && REGNO(reg) < max_parm_reg
        && REGNO(reg) >= FIRST_PSEUDO_REGISTER && parmdecl_map[REGNO(reg)] != 0)
...
static rtx copy_for_inline(rtx orig)
{
    ...
    case CONST_DOUBLE:
       ...
       it correctly.  Using the old CONST_DOUBLE_MEM data is wrong.  */
        if (GET_MODE_CLASS(GET_MODE(x)) == MODE_FLOAT)
        {
            ...
            return immed_double_const(CONST_DOUBLE_LOW(x), CONST_DOUBLE_HIGH(x), VOIDmode);

    case CONST:
        /* Get constant pool entry for constant in the pool.  */
        if (RTX_INTEGRATED_P(x))
            return validize_mem(force_const_mem(GET_MODE(x), copy_for_inline(XEXP(x, 0))));
        break;
    ...
    case MEM:
       ...
       entry.  */
        ...
        if (CONSTANT_ADDRESS_P(XEXP(x, 0)) && GET_CODE(XEXP(x, 0)) != LABEL_REF
            && !(GET_CODE(XEXP(x, 0)) == CONST
                   && (GET_CODE(XEXP(XEXP(x, 0), 0)) == PLUS
                          && ((GET_CODE(XEXP(XEXP(XEXP(x, 0), 0), 0)) == LABEL_REF)
                                 || (GET_CODE(XEXP(XEXP(XEXP(x, 0), 0), 0)) == ADDRESS)))))
            return x;
        break;
       ...
       clear its TREE_READONLY to prevent certain optimizations.  */
        {
            ...
            while (GET_CODE(dest) == STRICT_LOW_PART || GET_CODE(dest) == ZERO_EXTRACT
                || GET_CODE(dest) == SUBREG)
                dest = XEXP(dest, 0);

            if (GET_CODE(dest) == REG && REGNO(dest) < max_parm_reg
    ...
    copy_memory((char *)orig, (char *)x,
        (sizeof(*x) - sizeof(x->fld) + sizeof(x->fld[0]) * GET_RTX_LENGTH(code)));
    ...
    for (i = 0; i < GET_RTX_LENGTH(code); i++)
    {
        ...
        case 'E':
            if (XVEC(x, i) != NULL && XVECLEN(x, i) != 0)
            {
                ...
                for (j = 0; j < XVECLEN(x, i); j++)
                    XVECEXP(x, i, j) = copy_for_inline(XVECEXP(x, i, j));
            }
            break;
        }
    }
...
#define FIXED_BASE_PLUS_P(X)                                                                       \
    (GET_CODE(X) == PLUS && GET_CODE(XEXP(X, 1)) == CONST_INT && GET_CODE(XEXP(X, 0)) == REG       \
...
static void process_reg_param(struct inline_remap *map, rtx loc, rtx copy)
{
    if ((GET_CODE(copy) != REG && GET_CODE(copy) != SUBREG)
        || (GET_CODE(copy) == REG && REG_USERVAR_P(loc) && !REG_USERVAR_P(copy))
        || (GET_CODE(copy) == REG && REGNO(copy) < FIRST_PSEUDO_REGISTER))
    {
...
static rtx expand_inline_function_eh_labelmap(rtx label)
{
    int index = CODE_LABEL_NUMBER(label);
...
rtx expand_inline_function(
    tree fndecl, tree parms, rtx target, int ignore, tree type, rtx structure_value_addr)
{
    ...
    for (formal = DECL_ARGUMENTS(fndecl), actual = parms; formal;
         formal = TREE_CHAIN(formal), actual = TREE_CHAIN(actual))
    {
        ...
        if (mode != TYPE_MODE(TREE_TYPE(arg))
            ...
            || (mode == BLKmode
                   && (TYPE_MAIN_VARIANT(TREE_TYPE(arg)) != TYPE_MAIN_VARIANT(TREE_TYPE(formal)))))
            return (rtx)(int32_t)-1;
    }
    ...
    for (formal = DECL_ARGUMENTS(fndecl), actual = parms, i = 0; formal;
       ...
       argument.  */
            ...
            && REGNO(XEXP(loc, 0)) > LAST_VIRTUAL_REGISTER)
        {
            rtx stack_slot = assign_stack_temp(
                TYPE_MODE(TREE_TYPE(arg)), int_size_in_bytes(TREE_TYPE(arg)), 1);
            MEM_SET_IN_STRUCT_P(stack_slot, AGGREGATE_TYPE_P(TREE_TYPE(arg)));
        ...
        else if (GET_CODE(loc) != MEM)
        {
            if (GET_MODE(loc) != TYPE_MODE(TREE_TYPE(arg)))
                ...
                arg_vals[i] = convert_modes(GET_MODE(loc),
                    ...
                    expand_expr(arg, NULL_RTX, mode, EXPAND_SUM),
        ...
        if (arg_vals[i] != 0
            && (!TREE_READONLY(formal)
                  ...
                  temps will never overlap the target.  */
                   || (target != 0 && !invisiref
                          && (GET_CODE(arg_vals[i]) == REG || GET_CODE(arg_vals[i]) == SUBREG
        ...
        if (arg_vals[i] != 0 && GET_CODE(arg_vals[i]) == REG && POINTER_TYPE_P(TREE_TYPE(formal)))
            mark_reg_pointer(
                arg_vals[i], (TYPE_ALIGN(TREE_TYPE(TREE_TYPE(formal))) / BITS_PER_UNIT));
    }
    ...
    real_label_map = (rtx *)xmalloc((max_labelno) * sizeof(rtx));
    ...
    map->const_equiv_map = (rtx *)alloca(map->const_equiv_map_size * sizeof(rtx));
    zero_memory((char *)map->const_equiv_map, map->const_equiv_map_size * sizeof(rtx));

    map->const_age_map = (unsigned *)alloca(map->const_equiv_map_size * sizeof(unsigned));
    zero_memory((char *)map->const_age_map, map->const_equiv_map_size * sizeof(unsigned));
    map->const_age = 0;
    ...
    if (GET_CODE(parm_insns) == NOTE && NOTE_LINE_NUMBER(parm_insns) > 0)
    {
        rtx note = emit_note(NOTE_SOURCE_FILE(parm_insns), NOTE_LINE_NUMBER(parm_insns));
        if (note)
    ...
    for (formal = DECL_ARGUMENTS(fndecl), i = 0; formal; formal = TREE_CHAIN(formal), i++)
    {
        loc = RTVEC_ELT(arg_vector, i);

        if (GET_CODE(loc) == MEM
            /* Exclude case handled above.  */
            && !(GET_CODE(XEXP(loc, 0)) == REG && REGNO(XEXP(loc, 0)) > LAST_VIRTUAL_REGISTER))
        {
            rtx note = emit_note(DECL_SOURCE_FILE(formal), DECL_SOURCE_LINE(formal));
            if (note)
                RTX_INTEGRATED_P(note) = 1;
    ...
    else if (GET_CODE(loc) == MEM)
    {
       ...
       the place that it will be referenced from.  */
        ...
        {
            temp = force_reg(Pmode, force_operand(structure_value_addr, NULL_RTX));
            map->reg_map[REGNO(XEXP(loc, 0))] = temp;
            if ((CONSTANT_P(structure_value_addr) || GET_CODE(structure_value_addr) == ADDRESSOF
    ...
    else if (GET_CODE(loc) == REG)
    {
       ...
       its type, since expand_function_start may have promoted it.  */
        enum machine_mode arriving_mode = GET_MODE(DECL_RTL(DECL_RESULT(fndecl)));
        rtx reg_to_map;

        /* Don't use MEMs as direct targets because on some machines
       substituting a MEM for a REG makes invalid insns.
       Let the combiner substitute the MEM if that is valid.  */
        if (target == 0 || GET_CODE(target) != REG || GET_MODE(target) != departing_mode)
        {
        ...
        if (arriving_mode != departing_mode)
        {
            ...
            if (GET_MODE_BITSIZE(arriving_mode) > BITS_PER_WORD)
            {
                if (!TRULY_NOOP_TRUNCATION(
                        GET_MODE_BITSIZE(departing_mode), GET_MODE_BITSIZE(arriving_mode)))
    ...
    /* Initialize label_map.  get_label_from_map will actually make
       the labels.  */
    zero_memory((char *)&map->label_map[min_labelno], (max_labelno - min_labelno) * sizeof(rtx));
    ...
    for (insn = insns; insn; insn = NEXT_INSN(insn))
    {
        ...
        case INSN:
            ...
            set = single_set(insn);
            copy = 0;
            if (GET_CODE(pattern) == USE && GET_CODE(XEXP(pattern, 0)) == REG
            ...
            if (GET_CODE(pattern) == USE && find_reg_note(insn, REG_EH_CONTEXT, 0) != 0)
                get_eh_context();

            /* Ignore setting a function value that we don't want to use.  */
            if (map->inline_target == 0 && set != 0 && GET_CODE(SET_DEST(set)) == REG
                && REG_FUNCTION_VALUE_P(SET_DEST(set)))
            {
                if (volatile_refs_p(SET_SRC(set)))
                {
                    ...
                    SET_DEST(new_set) = gen_reg_rtx(GET_MODE(SET_DEST(new_set)));
                }
                /* If the source and destination are the same and it
               has a note on it, keep the insn.  */
                else if (rtx_equal_p(SET_DEST(set), SET_SRC(set)) && REG_NOTES(insn) != 0)
                    copy = emit_insn(copy_rtx_and_substitute(pattern, map));
                else
                    break;
            }
            ...
            else if (static_chain_value != 0 && set != 0 && GET_CODE(SET_DEST(set)) == REG
                && rtx_equal_p(SET_DEST(set), static_chain_incoming_rtx))
                break;

            /* If this is setting the static chain pseudo, set it from
               the value we want to give it instead.  */
            else if (static_chain_value != 0 && set != 0
                && rtx_equal_p(SET_SRC(set), static_chain_incoming_rtx))
            {
                ...
                && NOTE_LINE_NUMBER(insn) != NOTE_INSN_DELETED)
            {
                ...
                if (copy
                    && (NOTE_LINE_NUMBER(copy) == NOTE_INSN_EH_REGION_BEG
                           || NOTE_LINE_NUMBER(copy) == NOTE_INSN_EH_REGION_END))
                {
                    rtx label = get_label_from_map(map, NOTE_BLOCK_NUMBER(copy));
    ...
    map->const_age++;
    for (insn = insns; insn; insn = NEXT_INSN(insn))
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i' && map->insn_map[INSN_UID(insn)]
            && REG_NOTES(insn))
        {
    ...
    BLOCK_ABSTRACT_ORIGIN(block)
        = (DECL_ABSTRACT_ORIGIN(fndecl) == NULL ? fndecl : DECL_ABSTRACT_ORIGIN(fndecl));
    poplevel(0, 0, 0);
    ...
    {
        target
            = gen_rtx_MEM(TYPE_MODE(type), memory_address(TYPE_MODE(type), structure_value_addr));
...
static void integrate_decl_tree(tree let, int level, struct inline_remap *map)
{
    tree t, node;
...
static void save_constants_in_decl_trees(tree let)
{
    tree t;
...
rtx copy_rtx_and_substitute(rtx orig, struct inline_remap *map)
{
       ...
       go through their `reg_map'.  */
...
#endif
                map->reg_map[regno] = temp = force_reg(Pmode, force_operand(loc, NULL_RTX));

#ifdef STACK_BOUNDARY
                mark_reg_pointer(map->reg_map[regno], STACK_BOUNDARY / BITS_PER_UNIT);
#endif
        ...
        if (map->reg_map[regno] == NULL)
        {
            ...
            if (map->regno_pointer_flag[regno])
                mark_reg_pointer(map->reg_map[regno], map->regno_pointer_align[regno]);
        }
        return map->reg_map[regno];
    ...
    case SUBREG:
        ...
        if (GET_CODE(copy) == SUBREG)
            return gen_rtx_SUBREG(
                GET_MODE(orig), SUBREG_REG(copy), SUBREG_WORD(orig) + SUBREG_WORD(copy));
        else if (GET_CODE(copy) == CONCAT)
        {
            ...
            return gen_rtx_SUBREG(GET_MODE(orig), copy, SUBREG_WORD(orig));

    case ADDRESSOF:
        copy = gen_rtx_ADDRESSOF(mode, copy_rtx_and_substitute(XEXP(orig, 0), map), 0);
    ...
    case CODE_LABEL:
        LABEL_PRESERVE_P(get_label_from_map(map, CODE_LABEL_NUMBER(orig))) = LABEL_PRESERVE_P(orig);
    ...
    case LABEL_REF:
       ...
       this function's labels.  */
        LABEL_REF_NONLOCAL_P(copy) = (LABEL_REF_NONLOCAL_P(orig)
       ...
       remapped label.  Otherwise, symbols are returned unchanged.  */
        if (CONSTANT_POOL_ADDRESS_P(orig))
        {
            ...
            if (GET_CODE(constant) == LABEL_REF)
                return XEXP(
                    force_const_mem(GET_MODE(orig), copy_rtx_and_substitute(constant, map)), 0);
        }
        else if (SYMBOL_REF_NEED_ADJUST(orig))
        {
            eif_eh_map = map;
            return rethrow_symbol_map(orig, expand_inline_function_eh_labelmap);
        }
    ...
    case CONST_DOUBLE:
       ...
       duplicate of a CONST_DOUBLE we have already seen.  */
        if (GET_MODE_CLASS(GET_MODE(orig)) == MODE_FLOAT)
        {
            ...
            return immed_double_const(CONST_DOUBLE_LOW(orig), CONST_DOUBLE_HIGH(orig), VOIDmode);
    ...
    case ADDRESS:
       ...
       return its address.  */
        ...
        temp = force_const_mem(
            GET_MODE(XEXP(orig, 0)), copy_rtx_and_substitute(XEXP(XEXP(orig, 0), 0), map));
    ...
    case SET:
       ...
       this will result in unnecessarily bad code, but should work.  */
        if (SET_DEST(orig) == virtual_stack_vars_rtx || SET_DEST(orig) == virtual_incoming_args_rtx)
        {
            ...
            loc_offset = GET_CODE(equiv_loc) == REG ? 0 : INTVAL(XEXP(equiv_loc, 1));
            return gen_rtx_SET(VOIDmode, SET_DEST(orig),
                force_operand(
                    plus_constant(copy_rtx_and_substitute(SET_SRC(orig), map), -loc_offset),
                    NULL_RTX));
        }
        break;
        ...
        case 'E':
            ...
            if (XVEC(orig, i) != NULL && XVECLEN(orig, i) != 0)
            {
                ...
                for (j = 0; j < XVECLEN(copy, i); j++)
                    XVECEXP(copy, i, j) = copy_rtx_and_substitute(XVECEXP(orig, i, j), map);
            }
            break;
...
void try_constants(rtx insn, struct inline_remap *map)
{
    int i;
        ...
        if (GET_CODE(map->equiv_sets[i].dest) == REG)
        {
            ...
            if (regno < map->const_equiv_map_size
                && (map->const_equiv_map[regno] == 0
                       ...
                       || !rtx_equal_p(map->const_equiv_map[regno], map->equiv_sets[i].equiv)))
            {
...
static void subst_constants(rtx *loc, rtx insn, struct inline_remap *map)
{
       ...
       hard regs used as user variables with constants.  */
        {
            ...
            if (!(regno < FIRST_PSEUDO_REGISTER && REG_USERVAR_P(x))
                && regno < map->const_equiv_map_size && map->const_equiv_map[regno] != 0
                && map->const_age_map[regno] >= map->const_age)
    ...
    case SET:
    {
        ...
        while (GET_CODE(*dest_loc) == ZERO_EXTRACT || GET_CODE(*dest_loc) == SUBREG
        ...
        if (GET_CODE(dest) == SUBREG && GET_MODE_SIZE(GET_MODE(dest)) <= UNITS_PER_WORD
            && GET_MODE_SIZE(GET_MODE(SUBREG_REG(dest))) <= UNITS_PER_WORD
            && (GET_MODE_SIZE(GET_MODE(SUBREG_REG(dest))) <= GET_MODE_SIZE(GET_MODE(dest)))
            && (tem = gen_lowpart_if_possible(GET_MODE(SUBREG_REG(dest)), src)))
    ...
    if ((GET_RTX_CLASS(code) == 'c' || code == NE || code == EQ) && CONSTANT_P(XEXP(x, 0))
...
void mark_stores(rtx dest, rtx x ATTRIBUTE_UNUSED)
{
    int regno = -1;
    enum machine_mode mode;
    ...
    {
        int last_reg
            = (regno >= FIRST_PSEUDO_REGISTER ? regno : regno + HARD_REGNO_NREGS(regno, mode) - 1);
        int i;

        /* Ignore virtual stack var or virtual arg register since those
       are handled separately.  */
        if (regno != VIRTUAL_INCOMING_ARGS_REGNUM && regno != VIRTUAL_STACK_VARS_REGNUM)
...
static void restore_constants(rtx *px)
{
    rtx x = *px;
    int i, j;
    const char *fmt;
    ...
    if (GET_CODE(x) == CONST_DOUBLE)
    {
       ...
       it correctly.  Using the old CONST_DOUBLE_MEM data is wrong.  */
        if (GET_MODE_CLASS(GET_MODE(x)) == MODE_FLOAT)
        {
            ...
            *px = immed_double_const(CONST_DOUBLE_LOW(x), CONST_DOUBLE_HIGH(x), VOIDmode);
    }
...
static void set_decl_origin_self(tree decl)
{
    if (DECL_ABSTRACT_ORIGIN(decl) == NULL_TREE)
    {
        ...
        if (TREE_CODE(decl) == FUNCTION_DECL)
        {
            ...
            if (DECL_INITIAL(decl) != NULL_TREE && DECL_INITIAL(decl) != error_mark_node)
...
static void set_block_abstract_flags(tree stmt, int setting)
{
    ...
    for (subblock = BLOCK_SUBBLOCKS(stmt); subblock != NULL_TREE; subblock = BLOCK_CHAIN(subblock))
...
void output_inline_function(tree fndecl)
{
    ...
    for (last = FIRST_PARM_INSN(head); NEXT_INSN(last); last = NEXT_INSN(last))
    {
