...
void push_label_entry(struct label_node **stack, rtx rlabel, tree tlabel)
{
    struct label_node *newnode = (struct label_node *)xmalloc(sizeof(struct label_node));
...
rtx pop_label_entry(struct label_node **stack)
{
    rtx label;
...
tree top_label_entry(struct label_node **stack)
{
    if (!*stack)
        return NULL_TREE;
...
int find_all_handler_type_matches(void ***array)
{
    struct handler_info *handler, *last ATTRIBUTE_UNUSED;
    int x, y;
    void *val;
    void **ptr;
    int max_ptr;
...
void add_partial_entry(tree handler)
{
...
int expand_eh_region_start_tree(tree decl, tree cleanup)
{
    ...
    if (exceptions_via_longjmp && protect_cleanup_actions_with_terminate)
    {
       ...
       are 2 arguments, and if the second argument is 2.  */
        ...
        if (TREE_CODE(cleanup) == CALL_EXPR && (args = TREE_OPERAND(cleanup, 1))
            && (func = TREE_OPERAND(cleanup, 0)) && (arg = TREE_VALUE(args))
            && (args = TREE_CHAIN(args))

            /* is the second argument 2?  */
            && TREE_CODE(TREE_VALUE(args)) == INTEGER_CST && TREE_INT_CST_LOW(TREE_VALUE(args)) == 2
...
void expand_start_try_stmts(void)
{
    if (!doing_eh(1))
        return;
...
void start_catch_handler(tree rtime)
{
    rtx handler_label;
    int insn_region_num;
    int eh_region_entry;
    ...
    if (rtime != NULL_TREE && rtime != CATCH_ALL_TYPE)
    {
        ...
        call_rtx = emit_library_call_value(
            eh_rtime_match_libfunc, NULL_RTX, 0, SImode, 1, rtime_address, Pmode);

        /* Did the function return true? */
        emit_cmp_insn(call_rtx, const0_rtx, EQ, NULL_RTX, GET_MODE(call_rtx), 0, 0);
...
void expand_start_all_catch(void)
{
    struct eh_entry *entry;
    tree label;
    rtx outer_context;
    ...
    for (; entry->finalization != integer_zero_node; entry = dequeue_eh_entry(&ehqueue))
    {
        rtx prev;
...
void expand_end_all_catch(void)
{
...
void end_protect_partials(void)
{
    while (protect_list)
    {
...
void set_exception_lang_code(int code)
{
    language_code = code;
}

/* This routine will set the language version code for exceptions. */
void set_exception_version_code(int code)
{
    version_code = code;
}
...
int is_exception_handler_label(int lab)
{
    rtx x;
    for (x = exception_handler_labels; x; x = XEXP(x, 1))
...
rtx get_reg_for_handler(void)
{
    rtx reg1;
    reg1 = FUNCTION_VALUE(build_pointer_type(void_type_node), current_function_decl);
    return reg1;
}
