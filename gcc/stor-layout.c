...
static enum machine_mode smallest_mode_for_size(unsigned int, enum mode_class);
...
tree variable_size(tree size)
{
    ...
    if (TREE_CONSTANT(size) || global_bindings_p() < 0 || contains_placeholder_p(size))
        return size;
...
{
    return size_binop(
        MULT_EXPR, size_binop(CEIL_DIV_EXPR, value, size_int(divisor)), size_int(divisor));
}
...
void layout_decl(tree decl, unsigned known_align)
{
    ...
    if (code != VAR_DECL && code != PARM_DECL && code != RESULT_DECL && code != FIELD_DECL
        && code != TYPE_DECL)
        abort();
    ...
    else if (DECL_ALIGN(decl) == 0 || (!DECL_PACKED(decl) && TYPE_ALIGN(type) > DECL_ALIGN(decl)))
    ...
    if (code == FIELD_DECL)
    {
        ...
        if (maximum_field_alignment != 0)
            DECL_ALIGN(decl) = MIN(DECL_ALIGN(decl), (unsigned)maximum_field_alignment);
        ...
        && GET_MODE_CLASS(TYPE_MODE(type)) == MODE_INT)
    {
        ...
        {
            DECL_ALIGN(decl) = MAX((unsigned)GET_MODE_ALIGNMENT(xmode), DECL_ALIGN(decl));
    ...
    if (DECL_BIT_FIELD(decl) && TYPE_MODE(type) == BLKmode && known_align % TYPE_ALIGN(type) == 0
        && DECL_SIZE(decl) != 0
    ...
    for (field = TYPE_FIELDS(rec); field; field = TREE_CHAIN(field))
    {
       ...
       is meaningless.  */
        ...
        if (PCC_BITFIELD_TYPE_MATTERS && TREE_TYPE(field) != error_mark_node
            && DECL_BIT_FIELD_TYPE(field) && !integer_zerop(TYPE_SIZE(TREE_TYPE(field))))
        {
       ...
       own alignment?  */
        ...
        if (const_size % desired_align != 0 || (var_align % desired_align != 0 && var_size != 0))
        {
            /* No, we need to skip space before this field.
               Bump the cumulative size to multiple of field alignment.  */

            if (var_size == 0 || var_align % desired_align == 0)
                const_size = CEIL(const_size, desired_align) * desired_align;
            else
            {
                if (const_size > 0)
                    var_size = size_binop(PLUS_EXPR, var_size, bitsize_int(const_size, 0L));
                const_size = 0;
        ...
        if (PCC_BITFIELD_TYPE_MATTERS && TREE_CODE(field) == FIELD_DECL
            && TREE_TYPE(field) != error_mark_node && DECL_BIT_FIELD_TYPE(field)
            && !DECL_PACKED(field) && maximum_field_alignment == 0
            && !integer_zerop(DECL_SIZE(field)))
        {
            ...
            if (((const_size + field_size + type_align - 1) / type_align - const_size / type_align)
...
/* No existing machine description uses this parameter.
   So I have made it in this aspect identical to PCC_BITFIELD_TYPE_MATTERS.  */
            ...
            && !DECL_PACKED(field) && !integer_zerop(DECL_SIZE(field)))
        {
            ...
            if (const_size / type_align != (const_size + field_size - 1) / type_align)
                const_size = CEIL(const_size, type_align) * type_align;
        }
#endif
        ...
        if (var_size && const_size)
            DECL_FIELD_BITPOS(field) = size_binop(PLUS_EXPR, var_size, bitsize_int(const_size, 0L));
        else if (var_size)
...

        {
            ...
            if (dsize == 0)
                /* Do nothing.  */;
            else if (TREE_CODE(dsize) == INTEGER_CST && !TREE_CONSTANT_OVERFLOW(dsize)
        ...
        /* CYGNUS LOCAL v850/law */
        {
            ...
            if (offset_info_file && !DECL_BIT_FIELD_TYPE(field))
            {
                ...
                if (offset && TREE_CODE(offset) == INTEGER_CST)
                {
                    ...
                    if (msw == 0)
                        fprintf(offset_info_file, "\t.equ %s_%s,%d\n",
                            ...
                            IDENTIFIER_POINTER(DECL_NAME(field)), lsw / BITS_PER_UNIT);
                }
            }
        }
        /* END CYGNUS LOCAL */
...
static void layout_union(tree rec)
{
    ...
    for (field = TYPE_FIELDS(rec); field; field = TREE_CHAIN(field))
    {
       ...
       forces the entire union to have `int' alignment.  */
        ...
        if (TREE_CODE(rec) == UNION_TYPE)
        {
            ...
            var_size = fold(build(COND_EXPR, sizetype, DECL_QUALIFIER(field), DECL_SIZE(field),
    ...
    if (NULL == var_size)
        TYPE_SIZE(rec) = bitsize_int(CEIL(const_size, BITS_PER_UNIT) * BITS_PER_UNIT, 0L);
    else if (const_size == 0)
        TYPE_SIZE(rec) = var_size;
    else
        TYPE_SIZE(rec)
            = size_binop(MAX_EXPR, var_size, round_up(bitsize_int(const_size, 0L), BITS_PER_UNIT));
...
void layout_type(tree type)
{
    ...
    case CHAR_TYPE:
        ...
        TYPE_MODE(type) = smallest_mode_for_size(TYPE_PRECISION(type), MODE_INT);
    ...
    case COMPLEX_TYPE:
        ...
        TYPE_MODE(type) = mode_for_size(2 * TYPE_PRECISION(TREE_TYPE(type)),
            (TREE_CODE(TREE_TYPE(type)) == INTEGER_TYPE ? MODE_COMPLEX_INT : MODE_COMPLEX_FLOAT),
            0);
        TYPE_SIZE(type) = bitsize_int(GET_MODE_BITSIZE(TYPE_MODE(type)), 0L);
        TYPE_SIZE_UNIT(type) = size_int(GET_MODE_SIZE(TYPE_MODE(type)));
        break;
    ...
    case ARRAY_TYPE:
    {
        ...
        if (index && TYPE_MAX_VALUE(index) && TYPE_MIN_VALUE(index) && TYPE_SIZE(element))
        {
            ...
            if (TREE_CODE(ub) == MAX_EXPR && TREE_CODE(TREE_OPERAND(ub, 0)) == MINUS_EXPR
                && integer_onep(TREE_OPERAND(TREE_OPERAND(ub, 0), 1))
                && operand_equal_p(TREE_OPERAND(TREE_OPERAND(ub, 0), 0), lb, 0))
                ub = TREE_OPERAND(ub, 1);
            else if (TREE_CODE(ub) == MAX_EXPR && TREE_CODE(TREE_OPERAND(ub, 1)) == MINUS_EXPR
                && integer_onep(TREE_OPERAND(TREE_OPERAND(ub, 1), 1))
                && operand_equal_p(TREE_OPERAND(TREE_OPERAND(ub, 1), 0), lb, 0))
                ub = TREE_OPERAND(ub, 0);

            /* The initial subtraction should happen in the original type so
               that (possible) negative values are handled appropriately.  */
            length = size_binop(
                PLUS_EXPR, size_one_node, fold(build(MINUS_EXPR, TREE_TYPE(lb), ub, lb)));

            /* If neither bound is a constant and sizetype is signed, make
               sure the size is never negative.  We should really do this
               if *either* bound is non-constant, but this is the best
               compromise between C and Ada.  */
            if (!TREE_UNSIGNED(sizetype) && TREE_CODE(TYPE_MIN_VALUE(index)) != INTEGER_CST
            ...
            if (TYPE_PACKED(type) && INTEGRAL_TYPE_P(element))
            {
                ...
                if (maxvalue - minvalue == 1 && (maxvalue == 1 || maxvalue == 0))
            ...
            if (TYPE_SIZE_UNIT(element) != 0 && element_size != integer_one_node)
            {
                TYPE_SIZE_UNIT(type) = size_binop(MULT_EXPR, TYPE_SIZE_UNIT(element), length);
            }
        }
...
#ifdef ROUND_TYPE_ALIGN
        TYPE_ALIGN(type) = ROUND_TYPE_ALIGN(type, TYPE_ALIGN(element), BITS_PER_UNIT);
#else
        ...
        if (TYPE_SIZE(type) != 0
            ...
            && (TYPE_MODE(TREE_TYPE(type)) != BLKmode || TYPE_NO_FORCE_BLK(TREE_TYPE(type))))
        {
            TYPE_MODE(type) = mode_for_size(TREE_INT_CST_LOW(TYPE_SIZE(type)), MODE_INT, 1);
    ...
    case RECORD_TYPE:
        ...
        if (TREE_CODE(TYPE_SIZE(type)) == INTEGER_CST)
        {
            ...
            for (field = TYPE_FIELDS(type); field; field = TREE_CHAIN(field))
            {
                ...
                if (TYPE_MODE(TREE_TYPE(field)) == BLKmode && !TYPE_NO_FORCE_BLK(TREE_TYPE(field)))
                    goto record_lose;
               ...
               since extract_bit_field can't handle that in registers.  */
                if (bitpos / BITS_PER_WORD
                        != ((TREE_INT_CST_LOW(DECL_SIZE(field)) + bitpos - 1) / BITS_PER_WORD)
...
#define SET_WORD_SIZE BITS_PER_WORD
            ...
            int size_in_bits = (TREE_INT_CST_LOW(TYPE_MAX_VALUE(TYPE_DOMAIN(type)))
                - TREE_INT_CST_LOW(TYPE_MIN_VALUE(TYPE_DOMAIN(type))) + 1);
            int rounded_size = ((size_in_bits + alignment - 1) / alignment) * alignment;
    ...
    if (TYPE_MODE(type) != BLKmode && TYPE_MODE(type) != VOIDmode
        && (STRICT_ALIGNMENT
               || (TREE_CODE(type) != RECORD_TYPE && TREE_CODE(type) != UNION_TYPE
                      && TREE_CODE(type) != QUAL_UNION_TYPE && TREE_CODE(type) != ARRAY_TYPE)))
        TYPE_ALIGN(type) = GET_MODE_ALIGNMENT(TYPE_MODE(type));

        /* Do machine-dependent extra alignment.  */
#ifdef ROUND_TYPE_ALIGN
    TYPE_ALIGN(type) = ROUND_TYPE_ALIGN(type, TYPE_ALIGN(type), BITS_PER_UNIT);
#endif

#ifdef ROUND_TYPE_SIZE
    if (TYPE_SIZE(type) != 0)
        TYPE_SIZE(type) = ROUND_TYPE_SIZE(type, TYPE_SIZE(type), TYPE_ALIGN(type));
#endif
    ...
    {
        TYPE_SIZE_UNIT(type) = size_binop(FLOOR_DIV_EXPR, TYPE_SIZE(type), size_int(BITS_PER_UNIT));
    }
...
tree make_signed_type(int precision)
{
    ...
    TYPE_MIN_VALUE(type) = build_int_2(
        ...
        (((int32_t)(-1) << (precision - 32 - 1 > 0
                  ? precision - 32 - 1
                  : 0))));
    TYPE_MAX_VALUE(type) = build_int_2(
        (precision - 32 > 0 ? -1 : ((int32_t)1 << (precision - 1)) - 1),
        (precision - 32 - 1 > 0
                ? (((int32_t)1 << (precision - 32 - 1))) - 1
                : 0));
...
void fixup_unsigned_type(tree type)
{
    ...
    TYPE_MAX_VALUE(type) = build_int_2(
        precision - 32 >= 0 ? -1 : ((int32_t)1 << precision) - 1,
        precision - 32 > 0
            ? ((uint32_t)~0
                  >> (32 - (precision - 32)))
            : 0);
...
enum machine_mode get_best_mode(
    int bitsize, int bitpos, int align, enum machine_mode largest_mode, int volatilep)
{
    enum machine_mode mode;
    int unit = 0;
        ...
        && !volatilep)
    {
             ...
             tmode = GET_MODE_WIDER_MODE(tmode))
        {
            ...
            if (bitpos / unit == (bitpos + bitsize - 1) / unit && unit <= BITS_PER_WORD
...
void save_storage_status(struct function *p ATTRIBUTE_UNUSED)
{
...
void restore_storage_status(struct function *p ATTRIBUTE_UNUSED)
{
#if 0
