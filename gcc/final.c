...
void init_final(const char *filename)
{
    ...
    max_block_depth = 20;
    /* CYGNUS LOCAL LRS */
    pending_blocks = (struct block_seq *)xmalloc(20 * sizeof(struct block_seq));
    /* END CYGNUS LOCAL */
    final_sequence = 0;
}
...
static void print_wint(FILE *file, int32_t value)
{
    const char *fmt = int32_t_PRINT_DEC;
    if (flag_hex_asm)
    {
        fmt = int32_t_PRINT_HEX;
        if (value < 0)
        {
            fputc('-', file);
            value = -value;
...
int final_addr_vec_align(rtx addr_vec)
{
...
int align_fuzz(rtx start, rtx end, int known_align_log, unsigned growth)
{
...
int insn_current_reference_address(rtx branch)
{
    ...
    if (INSN_SHUID(branch) < INSN_SHUID(dest))
    {
        /* Forward branch. */
        return (
            insn_last_address + insn_lengths[seq_uid] - align_fuzz(seq, dest, length_unit_log, ~0));
    }
    else
    {
        /* Backward branch. */
        return (insn_current_address + align_fuzz(dest, seq, length_unit_log, ~0));
    }
}
...
void shorten_branches(rtx first)
{
    rtx insn;
    int max_uid;
    int i;
...
#define MAX_CODE_ALIGN 16
    ...
    zero_memory(
        (char *)label_align, (max_labelno - min_labelno + 1) * sizeof(struct label_alignment));
    ...
    for (insn = get_insns(), i = 1; insn; insn = NEXT_INSN(insn))
    {
        ...
        else if (GET_CODE(insn) == CODE_LABEL)
        {
            ...
            if (JUMP_TABLES_IN_TEXT_SECTION
                ...
                if (next && GET_CODE(next) == JUMP_INSN)
                {
                    ...
                    if (GET_CODE(nextbody) == ADDR_VEC || GET_CODE(nextbody) == ADDR_DIFF_VEC)
                    {
       ...
       sequences in order to handle reorg output efficiently.  */
        else if (GET_CODE(insn) == NOTE && NOTE_LINE_NUMBER(insn) == NOTE_INSN_LOOP_BEG)
        {
            rtx label;
            int nest = 0;
            ...
            {
                if (GET_CODE(label) == NOTE && NOTE_LINE_NUMBER(label) == NOTE_INSN_LOOP_BEG)
                    nest++;
                else if (GET_CODE(label) == NOTE && NOTE_LINE_NUMBER(label) == NOTE_INSN_LOOP_END
                    && --nest == 0)
                    break;
    ...
    for (insn_current_address = FIRST_INSN_ADDRESS, insn = first; insn != 0;
         insn_current_address += insn_lengths[uid], insn = NEXT_INSN(insn))
    {
        ...
        if (GET_CODE(insn) == NOTE || GET_CODE(insn) == BARRIER || GET_CODE(insn) == CODE_LABEL)
            continue;
        ...
        else if (GET_CODE(body) == SEQUENCE)
        {
            ...
            for (i = 0; i < XVECLEN(body, 0); i++)
            {
                ...
                if (asm_noperands(PATTERN(XVECEXP(body, 0, i))) >= 0)
                    inner_length
                        = (asm_insn_count(PATTERN(inner_insn)) * insn_default_length(inner_insn));
                else
...
void final_start_function(rtx first, FILE *file, int optimize)
{
    block_depth = 0;
...
void final_end_function(rtx first, FILE *file, int optimize)
{
    if (app_on)
    {
...
rtx final_scan_insn(rtx insn, FILE *file, int optimize, int prescan, int nopeepholes)
{
#ifdef HAVE_cc0
    rtx set;
#endif
    ...
    case NOTE:
       ...
       on certain machines.  */
        ...
        if (NOTE_LINE_NUMBER(insn) == NOTE_INSN_EH_REGION_BEG && !exceptions_via_longjmp)
        {
        ...
        if (NOTE_LINE_NUMBER(insn) == NOTE_INSN_EH_REGION_END && !exceptions_via_longjmp)
        {
                   ...
                   || write_symbols == DWARF_DEBUG || write_symbols == DWARF2_DEBUG))
        {
            ...
            if (block_depth == max_block_depth)
            {
                ...
                max_block_depth *= 2;
                pending_blocks = (struct block_seq *)xrealloc(
                    pending_blocks, (max_block_depth * sizeof(struct block_seq)));
            }
            ...
            for (note = NEXT_INSN(insn); note; note = NEXT_INSN(note))
            {
               ...
               so make sure the preceding line number stays.  */
                ...
                else if (GET_CODE(note) == NOTE && NOTE_LINE_NUMBER(note) > 0)
                {
                    ...
                    int num;
                    for (num = NOTE_LINE_NUMBER(insn) + 1; num < NOTE_LINE_NUMBER(note); num++)
       ...
       and NOTICE_UPDATE_CC alone can set incorrect status.  */
        if (0 /* optimize && LABEL_NUSES (insn) == 1*/)
        {
            ...
            /* If the LABEL_REFS field of this label has been set to point
               ...
               predecessor.  */
            if (barrier && GET_CODE(barrier) == BARRIER && jump && GET_CODE(jump) == JUMP_INSN
                && (prev = prev_nonnote_insn(jump)) && GET_CODE(prev) == INSN)
            {
        ...
        if (NEXT_INSN(insn) != 0 && GET_CODE(NEXT_INSN(insn)) == JUMP_INSN)
        {
            ...
            if (GET_CODE(nextbody) == ADDR_VEC || GET_CODE(nextbody) == ADDR_DIFF_VEC)
            {
               ...
               to the back end macros.  */
                ...
                ASM_OUTPUT_CASE_LABEL(file, "L", CODE_LABEL_NUMBER(insn), NEXT_INSN(insn));
#else
    ...
    default:
    {
        ...
        if (GET_CODE(body) == ADDR_VEC || GET_CODE(body) == ADDR_DIFF_VEC)
        {
            ...
            ASM_OUTPUT_CASE_END(file, CODE_LABEL_NUMBER(PREV_INSN(insn)), insn);
#endif
#endif
	    ...
	    rtx set = single_set(insn);
            ...
            if (set && GET_CODE(SET_DEST(set)) == CC0 && insn != last_ignored_compare)
            {
                ...
                else if (GET_CODE(SET_SRC(set)) == COMPARE)
                {
                    if (GET_CODE(XEXP(SET_SRC(set), 0)) == SUBREG)
                        XEXP(SET_SRC(set), 0) = alter_subreg(XEXP(SET_SRC(set), 0));
                    if (GET_CODE(XEXP(SET_SRC(set), 1)) == SUBREG)
                        XEXP(SET_SRC(set), 1) = alter_subreg(XEXP(SET_SRC(set), 1));
                }
                if ((cc_status.value1 != 0 && rtx_equal_p(SET_SRC(set), cc_status.value1))
                    || (cc_status.value2 != 0 && rtx_equal_p(SET_SRC(set), cc_status.value2)))
                {
        ...
        if (final_sequence == 0 && prescan >= 0 && GET_CODE(insn) == INSN && GET_CODE(body) == SET
            && GET_CODE(SET_SRC(body)) == REG && GET_CODE(SET_DEST(body)) == REG
        ...
        if (cc_status.flags != 0 && GET_CODE(insn) == JUMP_INSN && GET_CODE(body) == SET
            && SET_DEST(body) == pc_rtx && GET_CODE(SET_SRC(body)) == IF_THEN_ELSE
        ...
        if (cc_status.flags != 0 && set != 0)
        {
            rtx cond_rtx, then_rtx, else_rtx;

            if (GET_CODE(insn) != JUMP_INSN && GET_CODE(SET_SRC(set)) == IF_THEN_ELSE)
            {
...
void cleanup_subreg_operands(rtx insn)
{
    ...
    for (i = 0; i < recog_n_operands; i++)
    {
        ...
        else if (GET_CODE(recog_operand[i]) == PLUS || GET_CODE(recog_operand[i]) == MULT)
    ...
    for (i = 0; i < recog_n_dups; i++)
    {
        ...
        else if (GET_CODE(*recog_dup_loc[i]) == PLUS || GET_CODE(*recog_dup_loc[i]) == MULT)
...
rtx alter_subreg(rtx x)
{
    ...
    if (GET_CODE(y) == REG)
    {
       ...
       gen_lowpart and friends.  */
        ...
        REGNO(x) = ALTER_HARD_SUBREG(GET_MODE(x), SUBREG_WORD(x), GET_MODE(y), REGNO(y));
#else
...
static rtx walk_alter_subreg(rtx x)
{
    switch (GET_CODE(x))
    {
    case PLUS:
...
static void output_operand(rtx x, int code)
{
...
void output_address(rtx x)
{
    ...
    else if (GET_CODE(x) == PLUS)
    {
        ...
        {
            fprintf(asm_out_file, "[%s, %s]", reg_names[REGNO(XEXP(x, 0))],
    ...
    case CONST_DOUBLE:
        if (GET_MODE(x) == VOIDmode)
        {
            ...
            if (CONST_DOUBLE_HIGH(x))
                fprintf(file, int32_t_PRINT_DOUBLE_HEX, CONST_DOUBLE_HIGH(x),
                    CONST_DOUBLE_LOW(x));
            else if (CONST_DOUBLE_LOW(x) < 0)
                fprintf(file, int32_t_PRINT_HEX, CONST_DOUBLE_LOW(x));
            else
