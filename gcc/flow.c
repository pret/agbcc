...
static void propagate_block(regset, rtx, rtx, int, regset, int);
...
static void mark_set_regs(regset, regset, rtx, rtx, regset);
static void mark_set_1(regset, regset, rtx, rtx, regset);
#ifdef AUTO_INC_DEC
static void find_auto_inc(regset, rtx, rtx);
static int try_pre_increment_1(rtx);
static int try_pre_increment(rtx, rtx, int32_t);
#endif
static void mark_used_regs(regset, regset, rtx, int, rtx);
void dump_flow_info(FILE *);
static void add_pred_succ(int, int, int_list_ptr *, int_list_ptr *, int *, int *);
static int_list_ptr alloc_int_list_node(int_list_block **);
static int_list_ptr add_int_list_node(int_list_block **, int_list **, int);
/* CYGNUS LOCAL LRS */
void init_regset_vector(regset *, int, struct obstack *);
...
static void find_basic_blocks_1(rtx f, rtx nonlocal_labels)
{
    ...
    for (eh_note = NULL_RTX, insn = f, i = -1, prev_code = JUMP_INSN, depth = 1; insn;
         insn = NEXT_INSN(insn))
    {
        code = GET_CODE(insn);
        if (code == NOTE)
        ...
        /* A basic block starts at label, or after something that can jump.  */
            ...
            || (GET_RTX_CLASS(code) == 'i'
                   && (prev_code == JUMP_INSN || (prev_code == CALL_INSN && call_had_abnormal_edge)
                          || prev_code == BARRIER)))
        {
            BLOCK_HEAD(++i) = insn;
            BLOCK_END(i) = insn;
        ...
        if (GET_RTX_CLASS(code) == 'i')
        {
            ...
            for (note = REG_NOTES(insn); note; note = XEXP(note, 1))
                if (REG_NOTE_KIND(note) == REG_LABEL && XEXP(note, 0) != eh_return_stub_label)
                    label_value_list = gen_rtx_EXPR_LIST(VOIDmode, XEXP(note, 0), label_value_list);
        }
        ...
        if (GET_CODE(insn) == NOTE)
        {
            ...
            {
                if (eh_note)
                    nested_eh_region[NOTE_BLOCK_NUMBER(insn)] = NOTE_BLOCK_NUMBER(XEXP(eh_note, 0));
                else
       ...
       for every insn, since most insns can throw.  */
        else if (eh_note && (asynchronous_exceptions || (GET_CODE(insn) == CALL_INSN)))
            active_eh_region[INSN_UID(insn)] = NOTE_BLOCK_NUMBER(XEXP(eh_note, 0));
        BLOCK_NUM(insn) = i;
...
void set_block_num(rtx insn, int bb)
{
    if (INSN_UID(insn) >= max_uid_for_flow)
    {
        /* Add one-eighth the size so we don't keep calling xrealloc.  */
        max_uid_for_flow = INSN_UID(insn) + (INSN_UID(insn) + 7) / 8;
        uid_block_number = (int *)xrealloc(uid_block_number, (max_uid_for_flow + 1) * sizeof(int));
    }
    BLOCK_NUM(insn) = bb;
}
...
/* Make an edge in the cfg from block PRED to the block starting with
   label LABEL.  */
static void add_edge_to_label(int pred, rtx label)
{
...
static void make_edges(int i)
{
    ...
    if (i + 1 < n_basic_blocks)
    {
        for (insn = PREV_INSN(BLOCK_HEAD(i + 1)); insn && GET_CODE(insn) == NOTE;
             insn = PREV_INSN(insn))
            ;
    ...
    /* Now scan the insns for this block, we may need to make edges for some of
       ...
       labels, etc).  */
    for (insn = BLOCK_HEAD(i); insn != NEXT_INSN(BLOCK_END(i)); insn = NEXT_INSN(insn))
    {
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i')
        {
                ...
                && (note = find_reg_note(insn, REG_EH_RETHROW, NULL_RTX)))
            {
                ...
                else
                {
                    ...
                    pending_eh_region = gen_rtx_EXPR_LIST(VOIDmode, insn, pending_eh_region);
                }
            }
            ...
            /* If this is a CALL_INSN, then mark it as reaching the active EH
               ...
               Also mark the CALL_INSN as reaching any nonlocal goto sites.  */
                ...
                || (GET_CODE(insn) == CALL_INSN && !find_reg_note(insn, REG_RETVAL, NULL_RTX)))
            {
    ...
    while (pending_eh_region != NULL_RTX)
    {
        ...
        for (region = nested_eh_region[region]; region; region = nested_eh_region[region])
        {
            handler_info *ptr = get_first_handler(region);
            for (; ptr; ptr = ptr->next)
...
static void delete_unreachable_blocks(void)
{
    ...
    for (i = j = 0; i < n_basic_blocks; i++)
    {
        ...
        if (i != j)
        {
            ...
            basic_block_computed_jump_target[j] = basic_block_computed_jump_target[i];
        }
        j++;
    }
        ...
        for (j = i + 1; j < n_basic_blocks; j++)
            if (block_live_static[j])
            {
                ...
                if (GET_CODE(insn) == JUMP_INSN
                    ...
                    && simplejump_p(insn) && (label = XEXP(SET_SRC(PATTERN(insn)), 0), 1)
                    && INSN_UID(label) != 0 && BLOCK_NUM(label) == j)
                {
                    PUT_CODE(insn, NOTE);
                    NOTE_LINE_NUMBER(insn) = NOTE_INSN_DELETED;
                    NOTE_SOURCE_FILE(insn) = 0;
...
static int set_noop_p(rtx set)
{
    rtx src = SET_SRC(set);
    rtx dst = SET_DEST(set);
    if (GET_CODE(src) == REG && GET_CODE(dst) == REG && REGNO(src) == REGNO(dst))
        return 1;
    if (GET_CODE(src) != SUBREG || GET_CODE(dst) != SUBREG || SUBREG_WORD(src) != SUBREG_WORD(dst))
        return 0;
    src = SUBREG_REG(src);
    dst = SUBREG_REG(dst);
    if (GET_CODE(src) == REG && GET_CODE(dst) == REG && REGNO(src) == REGNO(dst))
...
/* Return nonzero if an insn consists only of SETs, each of which only sets a
   value to itself.  */
static int noop_move_p(rtx insn)
{
...
static void notice_stack_pointer_modification(rtx x, rtx pat ATTRIBUTE_UNUSED)
{
    if (x == stack_pointer_rtx
       ...
       regarding Embedded Side-Effects on Addresses.  */
        || (GET_CODE(x) == MEM
               && (GET_CODE(XEXP(x, 0)) == PRE_DEC || GET_CODE(XEXP(x, 0)) == PRE_INC
                      || GET_CODE(XEXP(x, 0)) == POST_DEC || GET_CODE(XEXP(x, 0)) == POST_INC)
...
static void record_volatile_insns(rtx f)
{
    rtx insn;
    for (insn = f; insn; insn = NEXT_INSN(insn))
    {
        ...
        {
            if (GET_CODE(PATTERN(insn)) != USE && volatile_refs_p(PATTERN(insn)))
                INSN_VOLATILE(insn) = 1;
        ...
        if (current_function_sp_is_unchanging && GET_RTX_CLASS(GET_CODE(insn)) == 'i')
...
static void life_analysis_1(rtx f, int nregs)
{
    int first_pass;
    int changed;
    ...
    init_regset_vector(basic_block_new_live_at_end, n_basic_blocks, &flow_obstack);
        ...
        for (i = n_basic_blocks - 1; i >= 0; i--)
        {
            ...
            if (!first_pass)
            {
               ...
               is one of the significant regs of this basic block).  */
                ...
                EXECUTE_IF_AND_COMPL_IN_REG_SET(
                    basic_block_new_live_at_end[i], basic_block_live_at_end[i], 0, j, {
                        consider = 1;
            ...
            else
            {
               ...
               by propagation backwards through the block.  */
                COPY_REG_SET(basic_block_live_at_end[i], basic_block_new_live_at_end[i]);
                COPY_REG_SET(basic_block_live_at_start[i], basic_block_live_at_end[i]);
                propagate_block(basic_block_live_at_start[i], BLOCK_HEAD(i), BLOCK_END(i), 0,
                    first_pass ? basic_block_significant[i] : (regset)0, i);
            }
    ...
    if (n_basic_blocks > 0)
        EXECUTE_IF_SET_IN_REG_SET(basic_block_live_at_start[0], FIRST_PSEUDO_REGISTER, i,
    ...
    for (i = 0; i < n_basic_blocks; i++)
    {
        propagate_block(basic_block_live_at_end[i], BLOCK_HEAD(i), BLOCK_END(i), 1, (regset)0, i);
#ifdef USE_C_ALLOCA
        alloca(0);
#endif
    }
  ...
  EXECUTE_IF_SET_IN_REG_SET (regs_live_at_setjmp,
    ...
    EXECUTE_IF_SET_IN_REG_SET(regs_live_at_setjmp, FIRST_PSEUDO_REGISTER, i, {
        if (regno_reg_rtx[i] != 0)
        {
            REG_LIVE_LENGTH(i) = -1;
            REG_BASIC_BLOCK(i) = -1;
...
void allocate_for_life_analysis(void)
{
    ...
    init_regset_vector(basic_block_live_at_start, n_basic_blocks, function_obstack);
    ...
    regset old, rtx first, rtx last, int final, regset significant, int bnum)
{
    ...
    if (final)
    {
       ...
       Mark them as not local to any one basic block. */
        EXECUTE_IF_SET_IN_REG_SET(old, 0, i, { REG_BASIC_BLOCK(i) = REG_BLOCK_GLOBAL; });
    }
       ...
       and then LIVE regs turned on.  */
        ...
        else if (GET_RTX_CLASS(GET_CODE(insn)) == 'i')
        {
            ...
            int insn_is_dead = (insn_dead_p(PATTERN(insn), old, 0, REG_NOTES(insn))
                ...
                && !INSN_VOLATILE(insn));
            int libcall_is_dead
                = (insn_is_dead && note != 0 && libcall_dead_p(PATTERN(insn), old, note, insn));
...
#ifdef AUTO_INC_DEC
            {
                ...
                if (!reload_completed && final && x != 0 && GET_CODE(SET_DEST(x)) == REG
                    && (GET_CODE(SET_SRC(x)) == PLUS || GET_CODE(SET_SRC(x)) == MINUS)
            ...
            else if (GET_CODE(PATTERN(insn)) == SET && SET_DEST(PATTERN(insn)) == stack_pointer_rtx
                && GET_CODE(SET_SRC(PATTERN(insn))) == PLUS
                && XEXP(SET_SRC(PATTERN(insn)), 0) == stack_pointer_rtx
            ...
            else
            {
               ...
               Find all regs now live and record this for them.  */
                ...
                if (GET_CODE(insn) == CALL_INSN && final)
                    EXECUTE_IF_SET_IN_REG_SET(old, 0, i, { REG_N_CALLS_CROSSED(i)++; });

                /* LIVE gets the regs used in INSN;
               DEAD gets those set by it.  Dead insns don't make anything
               live.  */

                mark_set_regs(old, dead, PATTERN(insn), final ? insn : NULL_RTX, significant);
               ...
               mark_used_regs will set it live if it is referenced.  */
                ...
                if (!insn_is_dead && GET_CODE(insn) == CALL_INSN)
                {
                    ...
                    for (note = CALL_INSN_FUNCTION_USAGE(insn); note; note = XEXP(note, 1))
                        if (GET_CODE(XEXP(note, 0)) == USE)
                            mark_used_regs(old, live, SET_DEST(XEXP(note, 0)), final, insn);
                    ...
                    for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
                        if (global_regs[i])
                            mark_used_regs(old, live, gen_rtx_REG(reg_raw_mode[i], i), final, insn);
...
static int insn_dead_p(rtx x, regset needed, int call_ok, rtx notes ATTRIBUTE_UNUSED)
{
    ...
    if (code == SET)
    {
        ...
        while (
            GET_CODE(r) == SUBREG || GET_CODE(r) == STRICT_LOW_PART || GET_CODE(r) == ZERO_EXTRACT)
            r = SUBREG_REG(r);
    ...
    else if (code == PARALLEL)
    {
        ...
        for (i--; i >= 0; i--)
            if (GET_CODE(XVECEXP(x, 0, i)) != CLOBBER && GET_CODE(XVECEXP(x, 0, i)) != USE
...
int regno_uninitialized(int regno)
{
    if (n_basic_blocks == 0
        || (regno < FIRST_PSEUDO_REGISTER
               && (global_regs[regno] || fixed_regs[regno] || FUNCTION_ARG_REGNO_P(regno))))
        return 0;
...
int regno_clobbered_at_setjmp(int regno)
{
    ...
    return ((REG_N_SETS(regno) > 1 || REGNO_REG_SET_P(basic_block_live_at_start[0], regno))
...
static void invalidate_mems_from_autoinc(rtx insn)
{
...
static void mark_set_1(regset needed, regset dead, rtx x, rtx insn, regset significant)
{
    ...
    if (GET_CODE(reg) == MEM || GET_CODE(reg) == REG)
    {
        ...
        while (temp)
        {
            if ((GET_CODE(reg) == MEM && output_dependence(XEXP(temp, 0), reg))
                || (GET_CODE(reg) == REG && reg_overlap_mentioned_p(reg, XEXP(temp, 0))))
            {
    ...
    if (GET_CODE(reg) == REG && (regno = REGNO(reg), regno != FRAME_POINTER_REGNUM)
        && !(regno == ARG_POINTER_REGNUM && fixed_regs[regno])
        ...
        if (insn)
        {
            ...
            if (!some_not_needed)
            {
               ...
               later.  */
                if (y && (BLOCK_NUM(y) == blocknum)
                    && (regno >= FIRST_PSEUDO_REGISTER || asm_noperands(PATTERN(y)) < 0))
                    LOG_LINKS(y) = gen_rtx_INSN_LIST(VOIDmode, insn, LOG_LINKS(y));
            }
            else if (!some_needed)
            {
               ...
               Indicate this by marking the reg being set as dying here.  */
                REG_NOTES(insn) = gen_rtx_EXPR_LIST(REG_UNUSED, reg, REG_NOTES(insn));
            ...
            else
            {
               ...
               be rare.  */
                ...
                for (i = HARD_REGNO_NREGS(regno, GET_MODE(reg)) - 1; i >= 0; i--)
...
static void find_auto_inc(regset needed, rtx x, rtx insn)
{
    rtx addr = XEXP(x, 0);
    int32_t offset = 0;
    rtx set;
    ...
    if (GET_CODE(addr) == REG)
    {
        ...
        if ((incr = reg_next_use[regno]) != 0 && (set = single_set(incr)) != 0
            && GET_CODE(set) == SET
            ...
            && GET_CODE(insn) != JUMP_INSN && (y = SET_SRC(set), GET_CODE(y) == PLUS)
            && XEXP(y, 0) == addr && GET_CODE(XEXP(y, 1)) == CONST_INT
            && ((HAVE_POST_INCREMENT && (INTVAL(XEXP(y, 1)) == size && offset == 0))
                   || (HAVE_POST_DECREMENT && (INTVAL(XEXP(y, 1)) == -size && offset == 0))
                   || (HAVE_PRE_INCREMENT && (INTVAL(XEXP(y, 1)) == size && offset == size))
                   || (HAVE_PRE_DECREMENT && (INTVAL(XEXP(y, 1)) == -size && offset == -size)))
            /* Make sure this reg appears only once in this insn.  */
            && (use = find_use_as_address(PATTERN(insn), addr, offset), use != 0 && use != (rtx)1))
        {
            rtx q = SET_DEST(set);
            enum rtx_code inc_code = (INTVAL(XEXP(y, 1)) == size ? (offset ? PRE_INC : POST_INC)
            ...
            if (dead_or_set_p(incr, addr))
            {
               ...
               needed updates below.  */
                if (!validate_change(insn, &XEXP(x, 0), gen_rtx_fmt_e(inc_code, Pmode, addr), 0))
                    return;
            }
                ...
                && !reg_set_between_p(q, PREV_INSN(insn), incr))
            {
               ...
               so is not correct in the pre-inc case.  */
                ...
                validate_change(insn, &XEXP(x, 0), gen_rtx_fmt_e(inc_code, Pmode, q), 1);
               ...
               Otherwise, invalidate it.  */
                if (GET_CODE(PREV_INSN(insn)) == INSN && GET_CODE(PATTERN(PREV_INSN(insn))) == SET
            ...
            REG_NOTES(insn) = gen_rtx_EXPR_LIST(REG_INC, addr, REG_NOTES(insn));
...
retry:
       ...
       => mark it as needed.  */
        ...
        regno = REGNO(x);
        {
            ...
            if (final)
            {
                ...
                if (some_not_needed && !dead_or_set_p(insn, x)
#if 0
		&& (regno >= FIRST_PSEUDO_REGISTER || ! fixed_regs[regno])
                ...
                )
                {
                    ...
                    if (regno < FIRST_PSEUDO_REGISTER && HARD_REGNO_NREGS(regno, GET_MODE(x)) > 1)
                    {
                    ...
                    if (!some_needed)
                    {
                        REG_NOTES(insn) = gen_rtx_EXPR_LIST(REG_DEAD, x, REG_NOTES(insn));
                        REG_N_DEATHS(regno)++;
                    }
                    else
                    {
    ...
    case SET:
    {
        ...
        while (GET_CODE(testreg) == STRICT_LOW_PART || GET_CODE(testreg) == ZERO_EXTRACT
            || GET_CODE(testreg) == SIGN_EXTRACT || GET_CODE(testreg) == SUBREG)
        {
            if (GET_CODE(testreg) == SUBREG && GET_CODE(SUBREG_REG(testreg)) == REG
            ...
            if (GET_CODE(testreg) == SUBREG && !(REG_SIZE(SUBREG_REG(testreg)) > REG_SIZE(testreg)))
                ;
            else
                mark_dest = 1;
        ...
        if ((GET_CODE(testreg) == PARALLEL && GET_MODE(testreg) == BLKmode)
            || (GET_CODE(testreg) == REG && (regno = REGNO(testreg), regno != FRAME_POINTER_REGNUM)
                   && !(regno == ARG_POINTER_REGNUM && fixed_regs[regno])))
       ...
       traditional asms unlike their normal usage.  */
        if (code == ASM_OPERANDS)
        {
            ...
            for (j = 0; j < ASM_OPERANDS_INPUT_LENGTH(x); j++)
                mark_used_regs(needed, live, ASM_OPERANDS_INPUT(x, j), final, insn);
        }
        break;
    }
...
static int try_pre_increment_1(rtx insn)
{
    /* Find the next use of this reg.  If in same basic block,
       make it do pre-increment or pre-decrement if appropriate.  */
    rtx x = single_set(insn);
    int32_t amount = ((GET_CODE(SET_SRC(x)) == PLUS ? 1 : -1) * INTVAL(XEXP(SET_SRC(x), 1)));
    ...
    if (y != 0
       ...
       mode would be better.  */
        && !dead_or_set_p(y, SET_DEST(x)) && try_pre_increment(y, SET_DEST(x), amount))
    {
...
static int try_pre_increment(rtx insn, rtx reg, int32_t amount)
{
    ...
    if (!validate_change(insn, &XEXP(use, 0),
            gen_rtx_fmt_e(
                amount > 0 ? (do_post ? POST_INC : PRE_INC) : (do_post ? POST_DEC : PRE_DEC), Pmode,
                reg),
            0))
        return 0;
...
rtx find_use_as_address(rtx x, rtx reg, int32_t plusconst)
{
    ...
    if (code == MEM && GET_CODE(XEXP(x, 0)) == PLUS && XEXP(XEXP(x, 0), 0) == reg
        && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT && INTVAL(XEXP(XEXP(x, 0), 1)) == plusconst)
        return x;
...
void dump_flow_info(FILE *file)
{
    ...
    for (i = FIRST_PSEUDO_REGISTER; i < max_regno; i++)
        if (REG_N_REFS(i))
        {
            ...
            if (REG_N_SETS(i))
                fprintf(file, "; set %d time%s", REG_N_SETS(i), (REG_N_SETS(i) == 1) ? "" : "s");
...
void print_rtl_with_bb(FILE *outf, rtx rtx_first)
{
    ...
    else
    {
        ...
        enum bb_state *in_bb_p = (enum bb_state *)alloca(max_uid * sizeof(enum bb_state));
        ...
        for (i = n_basic_blocks - 1; i >= 0; i--)
        {
            ...
            {
                in_bb_p[INSN_UID(x)]
                    = (in_bb_p[INSN_UID(x)] == NOT_IN_BB) ? IN_ONE_BB : IN_MULTIPLE_BB;
                if (x == BLOCK_END(i))
                    break;
            }
        }
        ...
        for (tmp_rtx = rtx_first; NULL != tmp_rtx; tmp_rtx = NEXT_INSN(tmp_rtx))
        {
            ...
            {
                fprintf(outf, ";; Start of basic block %d, registers live:", bb);

                EXECUTE_IF_SET_IN_REG_SET(basic_block_live_at_start[bb], 0, i, {
                    fprintf(outf, " %d", i);
                    if (i < FIRST_PSEUDO_REGISTER)
                        fprintf(outf, " [%s]", reg_names[i]);
                });
                putc('\n', outf);
            }

            if (in_bb_p[INSN_UID(tmp_rtx)] == NOT_IN_BB && GET_CODE(tmp_rtx) != NOTE
...
static int_list_ptr alloc_int_list_node(int_list_block **head_ptr)
{
...
static int_list_ptr add_int_list_node(int_list_block **blk_list, int_list **list, int val)
{
...
void free_int_list(int_list_block **blk_list)
{
...
static void add_pred_succ(int pred_bb, int succ_bb, int_list_ptr *s_preds, int_list_ptr *s_succs,
    int *num_preds, int *num_succs)
{
...
int compute_preds_succs(
    int_list_ptr *s_preds, int_list_ptr *s_succs, int *num_preds, int *num_succs, int split_edges)
{
    int bb;
    int changed = 0;
    ...
    for (bb = 0; bb < n_basic_blocks; bb++)
    {
        ...
        for (p = basic_block_succ[bb]; p; p = p->next)
            add_pred_succ(bb, INT_LIST_VAL(p), s_preds, s_succs, num_preds, num_succs);
       ...
       this block reaches the exit block.  */
        if ((code == JUMP_INSN && GET_CODE(PATTERN(jump)) == RETURN)
            || (((code == JUMP_INSN && condjump_p(jump) && !simplejump_p(jump))
                    || code != JUMP_INSN)
                   && bb == n_basic_blocks - 1))
...
void dump_bb_data(FILE *file, int_list_ptr *preds, int_list_ptr *succs, int live_info)
{
    int bb;
    int_list_ptr p;
...
void compute_dominators(
    sbitmap *dominators, sbitmap *post_dominators, int_list_ptr *s_preds, int_list_ptr *s_succs)
{
    ...
    while (changed)
    {
        ...
        {
            sbitmap_intersect_of_predecessors(temp_bitmap[bb], dominators, bb, s_preds);
            SET_BIT(temp_bitmap[bb], bb);
            changed |= sbitmap_a_and_b(dominators[bb], dominators[bb], temp_bitmap[bb]);
            sbitmap_intersect_of_successors(temp_bitmap[bb], post_dominators, bb, s_succs);
            SET_BIT(temp_bitmap[bb], bb);
            changed |= sbitmap_a_and_b(post_dominators[bb], post_dominators[bb], temp_bitmap[bb]);
        }
        passes++;
    }
...
void merge_blocks(rtx f)
{
    ...
    for (bb = 0; bb < n_basic_blocks; bb++)
    {
       ...
       property makes it relatively easy to detect this case.  */
        ...
        /* A tablejump will "jump" to the next instruction, which is the jump
           table itself.  */
        if (temp && GET_CODE(temp) == JUMP_INSN && JUMP_LABEL(temp)
    ...
    if (n_blocks_merged != 0)
    {
        for (i = 0; i < n_basic_blocks; i++)
        {
            ...
            {
                rtx start, end, next, insertpoint;
               ...
               TRAILER_BLOCK + 1 to preserve the meaning of the code.  */
                if (GET_CODE(end) != BARRIER)
                {
                    ...
                    if (GET_CODE(start) != CODE_LABEL)
                    {
                        label = gen_label_rtx();
                        LABEL_NUSES(label) = 1;
                        BLOCK_HEAD(trailer_block + 1) = emit_label_after(label, PREV_INSN(start));
                    }
                    else
                    {
                        label = start;
                    ...
                    jump = emit_jump_insn_after(gen_jump(label), BLOCK_END(trailer_block));
...
void recompute_reg_usage(rtx f, int loop_step)
{
    rtx insn;
    int i, max_reg;
