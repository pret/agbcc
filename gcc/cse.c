...
#define FIXED_REGNO_P(N) ((N) == FRAME_POINTER_REGNUM || fixed_regs[N] || global_regs[N])
...
#define CHEAP_REGNO(N)                                                                             \
    ((N) == FRAME_POINTER_REGNUM || (N) == STACK_POINTER_REGNUM || (N) == ARG_POINTER_REGNUM       \
        || ((N) >= FIRST_VIRTUAL_REGISTER && (N) <= LAST_VIRTUAL_REGISTER)                         \
        || ((N) < FIRST_PSEUDO_REGISTER && FIXED_REGNO_P(N) && REGNO_REG_CLASS(N) != NO_REGS))
...
#define CHEAP_REG(N)                                                                               \
    ((REG_USERVAR_P(N) && REGNO(N) < FIRST_PSEUDO_REGISTER) || CHEAP_REGNO(REGNO(N)))

#define COST(X)                                                                                    \
    (GET_CODE(X) == REG ? (CHEAP_REG(X) ? 0 : REGNO(X) >= FIRST_PSEUDO_REGISTER ? 1 : 2)           \
                        : notreg_cost(X))
...
#define CSE_ADDRESS_COST(RTX)                                                                      \
    ((GET_CODE(RTX) == ADDRESSOF && REG_P(XEXP((RTX), 0))) ? -1 : ADDRESS_COST(RTX))
#endif
...
struct cse_basic_block_data
{
    ...
    struct branch_path path[PATHLENGTH];
};
...
#define FIXED_BASE_PLUS_P(X)                                                                       \
    ((X) == frame_pointer_rtx || (X) == arg_pointer_rtx || (X) == virtual_stack_vars_rtx           \
...
#define NONZERO_BASE_PLUS_P(X)                                                                     \
    ((X) == frame_pointer_rtx || (X) == virtual_stack_vars_rtx || (X) == virtual_incoming_args_rtx \
        || (GET_CODE(X) == PLUS && GET_CODE(XEXP(X, 1)) == CONST_INT                               \
               && (XEXP(X, 0) == frame_pointer_rtx || XEXP(X, 0) == arg_pointer_rtx                \
                      || XEXP(X, 0) == virtual_stack_vars_rtx                                      \
                      || XEXP(X, 0) == virtual_incoming_args_rtx))                                 \
        || (X) == stack_pointer_rtx || (X) == virtual_stack_dynamic_rtx                            \
        || (X) == virtual_outgoing_args_rtx                                                        \
        || (GET_CODE(X) == PLUS && GET_CODE(XEXP(X, 1)) == CONST_INT                               \
               && (XEXP(X, 0) == stack_pointer_rtx || XEXP(X, 0) == virtual_stack_dynamic_rtx      \
...
static enum rtx_code find_comparison_args(
    enum rtx_code, rtx *, rtx *, enum machine_mode *, enum machine_mode *);
static rtx cse_gen_binary(enum rtx_code, enum machine_mode, rtx, rtx);
...
{
    return ((GET_CODE(x) == SUBREG && GET_CODE(SUBREG_REG(x)) == REG
                && GET_MODE_CLASS(GET_MODE(x)) == MODE_INT
                && GET_MODE_CLASS(GET_MODE(SUBREG_REG(x))) == MODE_INT
                && (GET_MODE_SIZE(GET_MODE(x)) < GET_MODE_SIZE(GET_MODE(SUBREG_REG(x))))
                && subreg_lowpart_p(x)
                && TRULY_NOOP_TRUNCATION(
                       GET_MODE_BITSIZE(GET_MODE(x)), GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(x)))))
            ? (CHEAP_REG(SUBREG_REG(x)) ? 0
                                        : (REGNO(SUBREG_REG(x)) >= FIRST_PSEUDO_REGISTER ? 1 : 2))
            : rtx_cost(x, SET) * 2);
}
...
int rtx_cost(rtx x ATTRIBUTE_UNUSED, enum rtx_code outer_code)
{
    int i, j;
    enum rtx_code code;
    const char *fmt;
    ...
    case MULT:
       ...
       because if we are considering it, we would output it as a shift.  */
        if (GET_CODE(XEXP(x, 1)) == CONST_INT && exact_log2(INTVAL(XEXP(x, 1))) >= 0)
            total = 2;
        else
    ...
    case SUBREG:
       ...
       the mode, the more expensive it is.  */
        if (!MODES_TIEABLE_P(GET_MODE(x), GET_MODE(SUBREG_REG(x))))
            return COSTS_N_INSNS(2 + GET_MODE_SIZE(GET_MODE(x)) / UNITS_PER_WORD);
        return 2;
#ifdef RTX_COSTS
...
static void new_basic_block(void)
{
    ...
    copy_memory((char *)all_minus_one, (char *)reg_in_table, max_reg * sizeof(int));
    ...
    {
        struct table_elt *current, *next;
        for (current = table[i]; current; current = next)
        {
            next = current->next_same_hash;
...
static void make_new_qty(int reg)
{
    int q;
...
static void make_regs_eqv(int newreg, int oldreg)
{
    int lastr, firstr;
    int q = reg_qty[oldreg];

    /* Nothing should become eqv until it has a "non-invalid" qty number.  */
    if (!REGNO_QTY_VALID_P(oldreg))
        abort();
    ...
    if (!(firstr < FIRST_PSEUDO_REGISTER && FIXED_REGNO_P(firstr))
       ...
       either.  */
        && (newreg >= FIRST_PSEUDO_REGISTER || REGNO_REG_CLASS(newreg) != NO_REGS)
        && ((newreg < FIRST_PSEUDO_REGISTER &&FIXED_REGNO_P(newreg))
               || (newreg >= FIRST_PSEUDO_REGISTER
                      && (firstr < FIRST_PSEUDO_REGISTER
                             || ((uid_cuid[REGNO_LAST_UID(newreg)] > cse_basic_block_end
                                     || (uid_cuid[REGNO_FIRST_UID(newreg)] < cse_basic_block_start))
                                    && (uid_cuid[REGNO_LAST_UID(newreg)]
                                           > uid_cuid[REGNO_LAST_UID(firstr)]))))))
    {
        reg_prev_eqv[firstr] = newreg;
        reg_next_eqv[newreg] = firstr;
        reg_prev_eqv[newreg] = -1;
        qty_first_reg[q] = newreg;
    }
    else
    {
       ...
       equivalent for anything.  */
        ...
        reg_next_eqv[newreg] = reg_next_eqv[lastr];
        if (reg_next_eqv[lastr] >= 0)
            reg_prev_eqv[reg_next_eqv[lastr]] = newreg;
        else
            qty_last_reg[q] = newreg;
        reg_next_eqv[lastr] = newreg;
        reg_prev_eqv[newreg] = lastr;
    }
}

/* Remove REG from its equivalence class.  */

static void delete_reg_equiv(int reg)
{
    int q = reg_qty[reg];
    int p, n;
...
static int mention_regs(rtx x)
{
    ...
    if (code == REG)
    {
        int regno = REGNO(x);
        int endregno
            = regno + (regno >= FIRST_PSEUDO_REGISTER ? 1 : HARD_REGNO_NREGS(regno, GET_MODE(x)));
        int i;
    ...
    {
        if (GET_CODE(XEXP(x, 0)) == REG && !REGNO_QTY_VALID_P(REGNO(XEXP(x, 0))))
        ...
        if (GET_CODE(XEXP(x, 1)) == REG && !REGNO_QTY_VALID_P(REGNO(XEXP(x, 1))))
...
static int insert_regs(rtx x, struct table_elt *classp, int modified)
{
    if (GET_CODE(x) == REG)
    {
       ...
       wrong mode for that equivalence, don't do anything here.  */
        ...
        if (REGNO_QTY_VALID_P(regno) && qty_mode[reg_qty[regno]] != GET_MODE(x))
            return 0;

        if (modified || !REGNO_QTY_VALID_P(regno))
        {
            if (classp)
                for (classp = classp->first_same_value; classp != 0;
        ...
        && !REGNO_QTY_VALID_P(REGNO(SUBREG_REG(x))))
    {
       ...
       will do the right thing.  */
        if (reg_in_table[regno] >= 0 && reg_tick[regno] == reg_in_table[regno] + 1)
            reg_tick[regno]++;
        mention_regs(x);
        return 1;
    }
...
static void free_element(struct table_elt *elt)
{
...
static struct table_elt *lookup_for_remove(rtx x, unsigned hash, enum machine_mode mode)
{
    ...
    if (GET_CODE(x) == REG)
    {
       ...
       invalidating (REG:SI 0) also invalidates (REG:DF 0).  */
        for (p = table[hash]; p; p = p->next_same_hash)
            if (GET_CODE(p->exp) == REG && REGNO(p->exp) == regno)
                return p;
    }
    else
    {
...
{
    struct table_elt *p = lookup(x, safe_hash(x, VOIDmode) % NBUCKETS, GET_MODE(x));
    /* If we are looking for a CONST_INT, the mode doesn't really matter, as
       long as we are narrowing.  So if we looked in vain for a mode narrower
       than word_mode before, look for word_mode now.  */
    if (p == 0 && code == CONST_INT && GET_MODE_SIZE(GET_MODE(x)) < GET_MODE_SIZE(word_mode))
    {
        x = copy_rtx(x);
...
static struct table_elt *insert(
    rtx x, struct table_elt *classp, unsigned hash, enum machine_mode mode)
{
    ...
    elt->is_const = (CONSTANT_P(x)
        ...
        || (RTX_UNCHANGING_P(x) && GET_CODE(x) == REG && REGNO(x) >= FIRST_PSEUDO_REGISTER)
        || FIXED_BASE_PLUS_P(x));
    ...
    if (classp)
    {
        ...
        else
        {
            ...
            for (p = classp; (next = p->next_same_value) && CHEAPER(next, elt); p = next)
                ;
    ...
    if (elt->is_const && classp && GET_CODE(classp->exp) == REG && GET_CODE(x) != REG)
    {
    ...
    else if (GET_CODE(x) == REG && classp && !qty_const[reg_qty[REGNO(x)]] && !elt->is_const)
    {
        struct table_elt *p;
        ...
        for (p = classp; p != 0; p = p->next_same_value)
        {
            ...
            {
                qty_const[reg_qty[REGNO(x)]] = gen_lowpart_if_possible(GET_MODE(x), p->exp);
...
{
    struct table_elt *elt, *next, *newtable;
    ...
    for (elt = class2; elt; elt = next)
    {
       ...
       hash code (it also isn't necessary).  */
        if (GET_CODE(exp) == REG || exp_equiv_p(exp, exp, 1, 0))
        {
            ...
            newtable = insert(exp, class1, hash, mode);
            newtable->in_memory = hash_arg_in_memory;
            newtable->in_struct = hash_arg_in_struct;
        }
    }
}
...
static void invalidate(rtx x, enum machine_mode full_mode)
{
    ...
    {
        int regno = REGNO(x);
        unsigned hash = HASH(x, GET_MODE(x));
       ...
       overlap these registers.  */
        ...
        {
            int32_t in_table = TEST_HARD_REG_BIT(hard_regs_in_table, regno);
            int endregno = regno + HARD_REGNO_NREGS(regno, GET_MODE(x));
            ...
            if (in_table)
                for (hash = 0; hash < NBUCKETS; hash++)
                    for (p = table[hash]; p; p = next)
                    {
                        ...
                        if (GET_CODE(p->exp) != REG || REGNO(p->exp) >= FIRST_PSEUDO_REGISTER)
                            continue;

                        tregno = REGNO(p->exp);
                        tendregno = tregno + HARD_REGNO_NREGS(tregno, GET_MODE(p->exp));
...
static void remove_invalid_refs(int regno)
{
    int i;
    struct table_elt *p, *next;
    ...
    for (i = 0; i < NBUCKETS; i++)
        ...
        {
            next = p->next_same_hash;
            if (GET_CODE(p->exp) != REG && refers_to_regno_p(regno, regno + 1, p->exp, NULL))
                remove_from_table(p, i);
        }
}

/* Likewise for a subreg with subreg_reg WORD and mode MODE.  */
static void remove_invalid_subreg_refs(int regno, int word, enum machine_mode mode)
{
    int i;
    struct table_elt *p, *next;
    ...
    for (i = 0; i < NBUCKETS; i++)
        for (p = table[i]; p; p = next)
        {
            ...
            if (GET_CODE(p->exp) != REG
                && (GET_CODE(exp) != SUBREG || GET_CODE(SUBREG_REG(exp)) != REG
                       || REGNO(SUBREG_REG(exp)) != regno
                       || (((SUBREG_WORD(exp) + (GET_MODE_SIZE(GET_MODE(exp)) - 1) / UNITS_PER_WORD)
                               >= word)
                              && SUBREG_WORD(exp) <= end))
...
static void rehash_using_reg(rtx x)
{
    ...
    if (GET_CODE(x) != REG || reg_in_table[REGNO(x)] < 0
...
static rtx use_related_value(rtx x, struct table_elt *elt)
{
    ...
    else if (elt == 0 && GET_CODE(x) == CONST)
    {
        rtx subexp = get_related_value(x);
        if (subexp != 0)
            relt = lookup(subexp, safe_hash(subexp, GET_MODE(subexp)) % NBUCKETS, GET_MODE(subexp));
    }
...
repeat:
    ...
    case REG:
    {
        ...
        if (regno < FIRST_PSEUDO_REGISTER
            && (global_regs[regno]
                   || (SMALL_REGISTER_CLASSES && !fixed_regs[regno] && regno != FRAME_POINTER_REGNUM
    ...
    case SUBREG:
    {
        if (GET_CODE(SUBREG_REG(x)) == REG)
        {
            hash += (((unsigned)SUBREG << 7) + REGNO(SUBREG_REG(x)) + SUBREG_WORD(x));
            return hash;
        }
        break;
    }
    ...
    for (; i >= 0; i--)
    {
        if (fmt[i] == 'e')
        {
            ...
            hash += canon_hash(tem, (enum machine_mode)0);
        }
        else if (fmt[i] == 'E')
            for (j = 0; j < XVECLEN(x, i); j++)
                hash += canon_hash(XVECEXP(x, i, j), (enum machine_mode)0);
        else if (fmt[i] == 's')
        {
...
static int exp_equiv_p(rtx x, rtx y, int validate, int equal_values)
{
    ...
    if (code != GET_CODE(y))
    {
       ...
       equivalent.  We only have to validate if Y is a register.  */
        if (CONSTANT_P(x) && GET_CODE(y) == REG && REGNO_QTY_VALID_P(REGNO(y))
            && GET_MODE(y) == qty_mode[reg_qty[REGNO(y)]]
            && rtx_equal_p(x, qty_const[reg_qty[REGNO(y)]])
            && (!validate || reg_in_table[REGNO(y)] == reg_tick[REGNO(y)]))
            return 1;

        if (CONSTANT_P(y) && code == REG && REGNO_QTY_VALID_P(REGNO(x))
            && GET_MODE(x) == qty_mode[reg_qty[REGNO(x)]]
    ...
    case REG:
    {
        int regno = REGNO(y);
        int endregno
            = regno + (regno >= FIRST_PSEUDO_REGISTER ? 1 : HARD_REGNO_NREGS(regno, GET_MODE(y)));
        int i;
    ...
    case EQ:
        return ((exp_equiv_p(XEXP(x, 0), XEXP(y, 0), validate, equal_values)
                    && exp_equiv_p(XEXP(x, 1), XEXP(y, 1), validate, equal_values))
            || (exp_equiv_p(XEXP(x, 0), XEXP(y, 1), validate, equal_values)
                   && exp_equiv_p(XEXP(x, 1), XEXP(y, 0), validate, equal_values)));
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        ...
        case 'E':
            if (XVECLEN(x, i) != XVECLEN(y, i))
                return 0;
            for (j = 0; j < XVECLEN(x, i); j++)
                if (!exp_equiv_p(XVECEXP(x, i, j), XVECEXP(y, i, j), validate, equal_values))
                    return 0;
            break;
...
static int cse_rtx_varies_p(rtx x)
{
    ...
    if (GET_CODE(x) == REG && REGNO_QTY_VALID_P(REGNO(x))
        && GET_MODE(x) == qty_mode[reg_qty[REGNO(x)]] && qty_const[reg_qty[REGNO(x)]] != 0)
        return 0;

    if (GET_CODE(x) == PLUS && GET_CODE(XEXP(x, 1)) == CONST_INT && GET_CODE(XEXP(x, 0)) == REG
    ...
    if (GET_CODE(x) == PLUS && GET_CODE(XEXP(x, 0)) == REG && GET_CODE(XEXP(x, 1)) == REG
        && REGNO_QTY_VALID_P(REGNO(XEXP(x, 0)))
        && (GET_MODE(XEXP(x, 0)) == qty_mode[reg_qty[REGNO(XEXP(x, 0))]])
        && qty_const[reg_qty[REGNO(XEXP(x, 0))]] && REGNO_QTY_VALID_P(REGNO(XEXP(x, 1)))
        && (GET_MODE(XEXP(x, 1)) == qty_mode[reg_qty[REGNO(XEXP(x, 1))]])
...
static rtx canon_reg(rtx x, rtx insn)
{
    ...
    case REG:
    {
        ...
        if (REGNO(x) < FIRST_PSEUDO_REGISTER || !REGNO_QTY_VALID_P(REGNO(x)))
            return x;
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        ...
        if (fmt[i] == 'e')
        {
            rtx tmp = canon_reg(XEXP(x, i), insn);
            int insn_code;

            /* If replacing pseudo with hard reg or vice versa, ensure the
               insn remains valid.  Likewise if the insn has MATCH_DUPs.  */
            if (insn != 0 && tmp != 0 && GET_CODE(tmp) == REG && GET_CODE(XEXP(x, i)) == REG
                && (((REGNO(tmp) < FIRST_PSEUDO_REGISTER)
                        != (REGNO(XEXP(x, i)) < FIRST_PSEUDO_REGISTER))
                       || (insn_code = recog_memoized(insn)) < 0 || insn_n_dups[insn_code] > 0))
                validate_change(insn, &XEXP(x, i), tmp, 1);
            else
                XEXP(x, i) = tmp;
        }
        else if (fmt[i] == 'E')
...
static void find_best_addr(rtx insn, rtx *loc)
{
    struct table_elt *elt;
    rtx addr = *loc;
#ifdef ADDRESS_COST
    ...
    /* Do not try to replace constant addresses or addresses of local and
       ...
       addresses is costly.  */
    if ((GET_CODE(addr) == PLUS && GET_CODE(XEXP(addr, 0)) == REG
        ...
        || (GET_CODE(addr) == REG
               && (regno = REGNO(addr),
                      regno == FRAME_POINTER_REGNUM || regno == ARG_POINTER_REGNUM))
        || GET_CODE(addr) == ADDRESSOF || CONSTANT_ADDRESS_P(addr))
        return;
    ...
    if (elt)
    {
        ...
        for (elt = elt->first_same_value; elt; elt = elt->next_same_value)
            if (elt->cost < our_cost
                ...
                && validate_change(insn, loc, canon_reg(copy_rtx(elt->exp), NULL_RTX), 0))
                return;
    }
#else
       ...
       one that hasn't already been tried.  */
        ...
        while (found_better)
        {
            ...
            for (p = elt->first_same_value; p; p = p->next_same_value)
                if (!p->flag)
                {
                    if ((GET_CODE(p->exp) == REG || exp_equiv_p(p->exp, p->exp, 1, 0))
            ...
            if (found_better)
            {
                if (validate_change(insn, loc, canon_reg(copy_rtx(best_elt->exp), NULL_RTX), 0))
                    return;
                else
                    best_elt->flag = 1;
            }
    ...
    if (flag_expensive_optimizations
        && (GET_RTX_CLASS(GET_CODE(*loc)) == '2' || GET_RTX_CLASS(GET_CODE(*loc)) == 'c')
        && GET_CODE(XEXP(*loc, 0)) == REG && GET_CODE(XEXP(*loc, 1)) == CONST_INT)
    {
        rtx c = XEXP(*loc, 1);
       ...
       one that hasn't already been tried.  */
        ...
        while (found_better)
        {
            ...
            found_better = 0;
            for (p = elt->first_same_value, count = 0; p && count < 32;
                 p = p->next_same_value, count++)
                if (!p->flag && (GET_CODE(p->exp) == REG || exp_equiv_p(p->exp, p->exp, 1, 0)))
                {
                    rtx newgen = cse_gen_binary(GET_CODE(*loc), Pmode, p->exp, c);

                    if ((CSE_ADDRESS_COST(newgen) < best_addr_cost
                            || (CSE_ADDRESS_COST(newgen) == best_addr_cost
                                   && (COST(newgen) + 1) >> 1 > best_rtx_cost)))
                    {
                        found_better = 1;
                        best_addr_cost = CSE_ADDRESS_COST(newgen);
                        best_rtx_cost = (COST(newgen) + 1) >> 1;
                        best_elt = p;
                        best_rtx = newgen;
                    }
                }
            ...
            if (found_better)
            {
                if (validate_change(insn, loc, canon_reg(copy_rtx(best_rtx), NULL_RTX), 0))
                    return;
                else
                    best_elt->flag = 1;
            }
...
static enum rtx_code find_comparison_args(enum rtx_code code, rtx *parg1, rtx *parg2,
    enum machine_mode *pmode1, enum machine_mode *pmode2)
{
    ...
    while (arg2 == CONST0_RTX(GET_MODE(arg1)))
    {
       ...
       STORE_FLAG_VALUE, get the inner arguments.  */
        ...
        {
            if (code == NE
                || (GET_MODE_CLASS(GET_MODE(arg1)) == MODE_INT && code == LT
                       && STORE_FLAG_VALUE == -1)
#ifdef FLOAT_STORE_FLAG_VALUE
                || (GET_MODE_CLASS(GET_MODE(arg1)) == MODE_FLOAT && FLOAT_STORE_FLAG_VALUE < 0)
#endif
            )
                x = arg1;
            else if (code == EQ
                || (GET_MODE_CLASS(GET_MODE(arg1)) == MODE_INT && code == GE
                       && STORE_FLAG_VALUE == -1)
#ifdef FLOAT_STORE_FLAG_VALUE
                || (GET_MODE_CLASS(GET_MODE(arg1)) == MODE_FLOAT && FLOAT_STORE_FLAG_VALUE < 0)
#endif
       ...
       and related forms, but let's wait until we see them occurring.  */
        ...
        if (x == 0)
            ...
            p = lookup(arg1, safe_hash(arg1, GET_MODE(arg1)) % NBUCKETS, GET_MODE(arg1));
        if (p)
        ...
        for (; p; p = p->next_same_value)
        {
                       ...
                       && GET_RTX_CLASS(GET_CODE(p->exp)) == '<'))
            {
                ...
                break;
                         ...
                         || (code == GE && GET_MODE_CLASS(inner_mode) == MODE_INT
                                && (GET_MODE_BITSIZE(inner_mode) <= 32)
                                && (STORE_FLAG_VALUE
                                       & ((int32_t)1 << (GET_MODE_BITSIZE(inner_mode) - 1))))
#ifdef FLOAT_STORE_FLAG_VALUE
...
rtx simplify_unary_operation(
    enum rtx_code code, enum machine_mode mode, rtx op, enum machine_mode op_mode)
{
        ...
        && (GET_CODE(op) == CONST_DOUBLE || GET_CODE(op) == CONST_INT))
    {
        ...
        {
            d = (double)(~hv);
            d *= ((double)((int32_t)1 << (32 / 2))
                * (double)((int32_t)1 << (32 / 2)));
            d += (double)(uint32_t)(~lv);
            d = (-d - 1.0);
        }
        else
        {
        ...
        && (GET_CODE(op) == CONST_DOUBLE || GET_CODE(op) == CONST_INT))
    {
        ...
        else if (GET_MODE_BITSIZE(op_mode) >= 32 * 2)
            ;
        else
    ...
    if (GET_CODE(op) == CONST_INT && width <= 32 && width > 0)
    {
        int32_t arg0 = INTVAL(op);
        int32_t val;
        ...
        case ZERO_EXTEND:
            ...
            else if (GET_MODE_BITSIZE(op_mode) < 32)
                val = arg0 & ~((int32_t)(-1) << GET_MODE_BITSIZE(op_mode));
            else
                return 0;
            break;
        ...
        case SIGN_EXTEND:
            ...
            {
                val = arg0 & ~((int32_t)(-1) << GET_MODE_BITSIZE(op_mode));
                if (val & ((int32_t)1 << (GET_MODE_BITSIZE(op_mode) - 1)))
                    val -= (int32_t)1 << GET_MODE_BITSIZE(op_mode);
            }
            else
                return 0;
            break;
        ...
        && (GET_CODE(op) == CONST_DOUBLE || GET_CODE(op) == CONST_INT))
    {
        ...
        case ZERO_EXTEND:
            if (op_mode == VOIDmode || GET_MODE_BITSIZE(op_mode) > 32)
                return 0;
        ...
        case SIGN_EXTEND:
            ...
            else
            {
                lv = l1 & GET_MODE_MASK(op_mode);
                if (GET_MODE_BITSIZE(op_mode) < 32
                    && (lv & ((int32_t)1 << (GET_MODE_BITSIZE(op_mode) - 1))) != 0)
                    lv -= (int32_t)1 << GET_MODE_BITSIZE(op_mode);
...
#if !defined(REAL_IS_NOT_DOUBLE) || defined(REAL_ARITHMETIC)
    else if (GET_CODE(op) == CONST_DOUBLE && GET_MODE_CLASS(mode) == MODE_FLOAT)
    {
        double d;
        jmp_buf handler;
        rtx x;
    ...
    else if (GET_CODE(op) == CONST_DOUBLE && GET_MODE_CLASS(GET_MODE(op)) == MODE_FLOAT
        && GET_MODE_CLASS(mode) == MODE_INT && width <= 32 && width > 0)
    {
        double d;
        jmp_buf handler;
        int32_t val;
...
rtx simplify_binary_operation(enum rtx_code code, enum machine_mode mode, rtx op0, rtx op1)
{
...
#if !defined(REAL_IS_NOT_DOUBLE) || defined(REAL_ARITHMETIC)
    if (GET_MODE_CLASS(mode) == MODE_FLOAT && GET_CODE(op0) == CONST_DOUBLE
        && GET_CODE(op1) == CONST_DOUBLE && mode == GET_MODE(op0) && mode == GET_MODE(op1))
    {
        double f0, f1, value;
        jmp_buf handler;
    ...
    if (GET_MODE_CLASS(mode) == MODE_INT && width == 32 * 2
        && (GET_CODE(op0) == CONST_DOUBLE || GET_CODE(op0) == CONST_INT)
        && (GET_CODE(op1) == CONST_DOUBLE || GET_CODE(op1) == CONST_INT))
    {
        int32_t l1, l2, h1, h2, lv, hv;
        ...
        case SMIN:
            if (h1 < h2 || (h1 == h2 && ((uint32_t)l1 < (uint32_t)l2)))
                lv = l1, hv = h1;
            else
                lv = l2, hv = h2;
            break;
        ...
        case UMIN:
            if ((uint32_t)h1 < (uint32_t)h2
                || (h1 == h2 && ((uint32_t)l1 < (uint32_t)l2)))
                lv = l1, hv = h1;
            else
                lv = l2, hv = h2;
            break;
        ...
        case ROTATERT:
            ...
            if (code == LSHIFTRT || code == ASHIFTRT)
                rshift_double(l1, h1, l2, GET_MODE_BITSIZE(mode), &lv, &hv, code == ASHIFTRT);
            else if (code == ASHIFT)
    ...
    if (GET_CODE(op0) != CONST_INT || GET_CODE(op1) != CONST_INT || width > 32
        || width == 0)
    {
       ...
       there are some cases worth simplifying.  */
        ...
        case PLUS:
            ...
            if (TARGET_FLOAT_FORMAT == IEEE_FLOAT_FORMAT && FLOAT_MODE_P(mode) && !flag_fast_math)
                break;
            ...
            if (CONSTANT_P(op0) && GET_MODE(op0) != VOIDmode && GET_CODE(op1) == CONST_INT)
                return plus_constant(op0, INTVAL(op1));
            else if (CONSTANT_P(op1) && GET_MODE(op1) != VOIDmode && GET_CODE(op0) == CONST_INT)
            ...
            {
                int32_t coeff0 = 1, coeff1 = 1;
                rtx lhs = op0, rhs = op1;
                int had_mult = 0;

                if (GET_CODE(lhs) == NEG)
                    coeff0 = -1, lhs = XEXP(lhs, 0);
                else if (GET_CODE(lhs) == MULT && GET_CODE(XEXP(lhs, 1)) == CONST_INT)
                {
                    coeff0 = INTVAL(XEXP(lhs, 1)), lhs = XEXP(lhs, 0);
                    had_mult = 1;
                }
                else if (GET_CODE(lhs) == ASHIFT && GET_CODE(XEXP(lhs, 1)) == CONST_INT
                    && INTVAL(XEXP(lhs, 1)) >= 0 && INTVAL(XEXP(lhs, 1)) < 32)
                {
                    coeff0 = ((int32_t)1) << INTVAL(XEXP(lhs, 1));
                    lhs = XEXP(lhs, 0);
                }

                if (GET_CODE(rhs) == NEG)
                    coeff1 = -1, rhs = XEXP(rhs, 0);
                else if (GET_CODE(rhs) == MULT && GET_CODE(XEXP(rhs, 1)) == CONST_INT)
                {
                    coeff1 = INTVAL(XEXP(rhs, 1)), rhs = XEXP(rhs, 0);
                    had_mult = 1;
                }
                else if (GET_CODE(rhs) == ASHIFT && GET_CODE(XEXP(rhs, 1)) == CONST_INT
                    && INTVAL(XEXP(rhs, 1)) >= 0 && INTVAL(XEXP(rhs, 1)) < 32)
                {
                    coeff1 = ((int32_t)1) << INTVAL(XEXP(rhs, 1));
                    rhs = XEXP(rhs, 0);
                }

                if (rtx_equal_p(lhs, rhs))
                {
                    tem = cse_gen_binary(MULT, mode, lhs, GEN_INT(coeff0 + coeff1));
            ...
            if (INTEGRAL_MODE_P(mode)
                && (GET_CODE(op0) == PLUS || GET_CODE(op0) == MINUS || GET_CODE(op1) == PLUS
                       || GET_CODE(op1) == MINUS)
        ...
        case MULT:
            ...
            if ((TARGET_FLOAT_FORMAT != IEEE_FLOAT_FORMAT || !FLOAT_MODE_P(mode) || flag_fast_math)
                && op1 == CONST0_RTX(mode) && !side_effects_p(op0))
                return op1;
            ...
            if (GET_CODE(op1) == CONST_DOUBLE && GET_MODE_CLASS(GET_MODE(op1)) == MODE_FLOAT)
            {
                double d;
                jmp_buf handler;
                int op1is2, op1ism1;
        ...
        case IOR:
            ...
            if (((GET_CODE(op0) == NOT && rtx_equal_p(XEXP(op0, 0), op1))
                ...
                && !side_effects_p(op0) && GET_MODE_CLASS(mode) != MODE_CC)
                return constm1_rtx;
            break;
        ...
        case XOR:
            ...
            if (op0 == op1 && !side_effects_p(op0) && GET_MODE_CLASS(mode) != MODE_CC)
                return const0_rtx;
            break;
        ...
        case UDIV:
            /* Convert divide by power of two into shift (divide by 1 handled
               below).  */
            if (GET_CODE(op1) == CONST_INT && (arg1 = exact_log2(INTVAL(op1))) > 0)
        ...
        case DIV:
            ...
            if ((TARGET_FLOAT_FORMAT != IEEE_FLOAT_FORMAT || !FLOAT_MODE_P(mode) || flag_fast_math)
                && op0 == CONST0_RTX(mode) && !side_effects_p(op1))
                return op0;
            ...
            /* Change division by a constant into multiplication.  Only do
               ...
               general.  */
            else if (GET_CODE(op1) == CONST_DOUBLE && GET_MODE_CLASS(GET_MODE(op1)) == MODE_FLOAT
                && op1 != CONST0_RTX(mode) && flag_fast_math)
            {
                double d;
                REAL_VALUE_FROM_CONST_DOUBLE(d, op1);
                ...
                if (!REAL_VALUES_EQUAL(d, dconst0))
                {
                    ...
                    return gen_rtx_MULT(mode, op0, CONST_DOUBLE_FROM_REAL_VALUE(d, mode));
#else
                    return gen_rtx_MULT(mode, op0, CONST_DOUBLE_FROM_REAL_VALUE(1. / d, mode));
#endif
                }
            }
#endif
        ...
        case UMOD:
            ...
            if (GET_CODE(op1) == CONST_INT && exact_log2(INTVAL(op1)) > 0)
                return gen_rtx_AND(mode, op0, GEN_INT(INTVAL(op1) - 1));

            /* ... fall through ...  */

        case MOD:
            if ((op0 == const0_rtx || op1 == const1_rtx) && !side_effects_p(op0)
                && !side_effects_p(op1))
                return const0_rtx;
            break;
        ...
        case ROTATE:
            ...
            if (GET_CODE(op0) == CONST_INT && width <= 32
                && INTVAL(op0) == GET_MODE_MASK(mode) && !side_effects_p(op1))
                return op0;
        ...
        case SMIN:
            if (width <= 32 && GET_CODE(op1) == CONST_INT
                && INTVAL(op1) == (int32_t)1 << (width - 1) && !side_effects_p(op0))
                return op1;
        ...
        case SMAX:
            if (width <= 32 && GET_CODE(op1) == CONST_INT
                && (INTVAL(op1) == (uint32_t)GET_MODE_MASK(mode) >> 1)
                && !side_effects_p(op0))
                return op1;
            else if (rtx_equal_p(op0, op1) && !side_effects_p(op0))
                return op0;
    ...
    if (width < 32)
    {
        arg0 &= ((int32_t)1 << width) - 1;
        arg1 &= ((int32_t)1 << width) - 1;

        arg0s = arg0;
        if (arg0s & ((int32_t)1 << (width - 1)))
            arg0s |= ((int32_t)(-1) << width);

        arg1s = arg1;
        if (arg1s & ((int32_t)1 << (width - 1)))
            arg1s |= ((int32_t)(-1) << width);
    }
    else
    {
        arg0s = arg0;
    ...
    case ROTATERT:
        ...
        arg1 %= width;
        val = ((((uint32_t)arg0) << (width - arg1)) | (((uint32_t)arg0) >> arg1));
        break;
    ...
    case ROTATE:
        ...
        arg1 %= width;
        val = ((((uint32_t)arg0) << arg1) | (((uint32_t)arg0) >> (width - arg1)));
        break;
    ...
    case UMIN:
        val = ((uint32_t)arg0 <= (uint32_t)arg1 ? arg0 : arg1);
        break;
    ...
    if (width < 32
        && ((val & ((int32_t)(-1) << (width - 1))) != ((int32_t)(-1) << (width - 1))))
        val &= ((int32_t)1 << width) - 1;
...
static rtx simplify_plus_minus(enum rtx_code code, enum machine_mode mode, rtx op0, rtx op1)
{
    rtx ops[8];
    int negs[8];
    rtx result, tem;
    ...
    if (i + n_consts > input_ops || (i + n_consts == input_ops && n_consts <= input_consts))
        return 0;
...
static rtx cse_gen_binary(enum rtx_code code, enum machine_mode mode, rtx op0, rtx op1)
{
    rtx tem;
    ...
    if (code == PLUS && GET_CODE(op1) == CONST_INT && GET_MODE(op0) != VOIDmode)
        return plus_constant(op0, INTVAL(op1));
    else if (code == MINUS && GET_CODE(op1) == CONST_INT && GET_MODE(op0) != VOIDmode)
        return plus_constant(op0, -INTVAL(op1));
    else
...
rtx simplify_relational_operation(enum rtx_code code, enum machine_mode mode, rtx op0, rtx op1)
{
    ...
    if (INTEGRAL_MODE_P(mode) && op1 != const0_rtx
        ...
        && 0 != (tem = simplify_binary_operation(MINUS, mode, op0, op1)) && code != GTU
        && code != GEU && code != LTU && code != LEU)
        return simplify_relational_operation(signed_condition(code), mode, tem, const0_rtx);

    /* For non-IEEE floating-point, if the two operands are equal, we know the
       result.  */
    if (rtx_equal_p(op0, op1)
        && (TARGET_FLOAT_FORMAT != IEEE_FLOAT_FORMAT || !FLOAT_MODE_P(GET_MODE(op0))
               || flag_fast_math))
    ...
    {
        int width = GET_MODE_BITSIZE(mode);
        int32_t l0s, h0s, l1s, h1s;
        uint32_t l0u, h0u, l1u, h1u;
        ...
        /* If WIDTH is nonzero and smaller than 32,
       we have to sign or zero-extend the values.  */
        ...
        if (width != 0 && width < 32)
        {
            l0u &= ((int32_t)1 << width) - 1;
            l1u &= ((int32_t)1 << width) - 1;

            if (l0s & ((int32_t)1 << (width - 1)))
                l0s |= ((int32_t)(-1) << width);

            if (l1s & ((int32_t)1 << (width - 1)))
                l1s |= ((int32_t)(-1) << width);
        }
    ...
    else
    {
        ...
        case EQ:
            ...
            if (((NONZERO_BASE_PLUS_P(op0) && op1 == const0_rtx) || GET_CODE(op0) == LABEL_REF)
        ...
        case LEU:
            /* Unsigned values are never greater than the largest
               unsigned value.  */
            if (GET_CODE(op1) == CONST_INT && INTVAL(op1) == GET_MODE_MASK(mode)
...
rtx simplify_ternary_operation(enum rtx_code code, enum machine_mode mode,
    enum machine_mode op0_mode, rtx op0, rtx op1, rtx op2)
{
    ...
    case SIGN_EXTRACT:
    case ZERO_EXTRACT:
        if (GET_CODE(op0) == CONST_INT && GET_CODE(op1) == CONST_INT && GET_CODE(op2) == CONST_INT
            && INTVAL(op1) + INTVAL(op2) <= GET_MODE_BITSIZE(op0_mode)
            && width <= 32)
        {
            /* Extracting a bit-field from a constant */
            int32_t val = INTVAL(op0);
            ...
            {
                /* First zero-extend.  */
                val &= ((int32_t)1 << INTVAL(op1)) - 1;
                /* If desired, propagate sign bit.  */
                if (code == SIGN_EXTRACT && (val & ((int32_t)1 << (INTVAL(op1) - 1))))
                    val |= ~(((int32_t)1 << INTVAL(op1)) - 1);
            }
    ...
    case IF_THEN_ELSE:
        ...
        if (GET_CODE(op0) == NE && !side_effects_p(op0) && rtx_equal_p(XEXP(op0, 0), op1)
            && rtx_equal_p(XEXP(op0, 1), op2))
            return op1;
        else if (GET_CODE(op0) == EQ && !side_effects_p(op0) && rtx_equal_p(XEXP(op0, 1), op1)
            && rtx_equal_p(XEXP(op0, 0), op2))
            return op2;
        else if (GET_RTX_CLASS(GET_CODE(op0)) == '<' && !side_effects_p(op0))
        {
            rtx temp;
            temp = simplify_relational_operation(
                GET_CODE(op0), op0_mode, XEXP(op0, 0), XEXP(op0, 1));
...
static rtx fold_rtx(rtx x, rtx insn)
{
    ...
    case CONST:
       ...
       CONST_INT, but that seems to be a safe assumption.  */
        if (GET_CODE(XEXP(x, 0)) == CONSTANT_P_RTX)
        {
            ...
            return (x != 0 && (GET_CODE(x) == CONST_INT || GET_CODE(x) == CONST_DOUBLE)
                    ? const1_rtx
                    : const0_rtx);
        }
    ...
    case PC:
       ...
       lets us fold switch statements on the Vax.  */
        if (insn && GET_CODE(insn) == JUMP_INSN)
        {
            ...
            if (next && GET_CODE(next) == CODE_LABEL && NEXT_INSN(next) != 0
    ...
    case SUBREG:
        ...
        if ((ret = lookup_as_function(x, CONST_INT)) != 0
            || (ret = lookup_as_function(x, CONST_DOUBLE)) != 0)
            return ret;
       ...
       Similarly if we find an integer constant.  */
        ...
        if (GET_MODE_SIZE(mode) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(x))))
        {
            ...
            if (GET_MODE_SIZE(mode) <= UNITS_PER_WORD && GET_MODE_SIZE(imode) <= UNITS_PER_WORD
                && (elt = lookup(SUBREG_REG(x), HASH(SUBREG_REG(x), imode), imode)) != 0)
                for (elt = elt->first_same_value; elt; elt = elt->next_same_value)
                {
                    if (CONSTANT_P(elt->exp) && GET_MODE(elt->exp) == VOIDmode)
                        return elt->exp;

                    if (GET_CODE(elt->exp) == SUBREG && GET_MODE(SUBREG_REG(elt->exp)) == mode
       ...
       integral mode or extracting the low part.  */
        ...
        if (folded_arg0 != SUBREG_REG(x))
        {
            ...
            if (GET_MODE_CLASS(mode) == MODE_INT && GET_MODE_SIZE(mode) == UNITS_PER_WORD
                ...
                ret = operand_subword(folded_arg0, SUBREG_WORD(x), 0, GET_MODE(SUBREG_REG(x)));
            if (ret == 0 && subreg_lowpart_p(x))
                ret = gen_lowpart_if_possible(mode, folded_arg0);
            if (ret)
                return ret;
        }
       ...
       can use the inner value.  */
        ...
        if (GET_CODE(folded_arg0) == REG
            && GET_MODE_SIZE(mode) < GET_MODE_SIZE(GET_MODE(folded_arg0)) && subreg_lowpart_p(x))
        {
            struct table_elt *elt;
            ...
            /* We can use HASH here since we know that canon_hash won't be
               called.  */
            elt = lookup(
                folded_arg0, HASH(folded_arg0, GET_MODE(folded_arg0)), GET_MODE(folded_arg0));
            ...
            for (; elt; elt = elt->next_same_value)
            {
                ...
                if (GET_RTX_CLASS(GET_CODE(elt->exp)) == '1' && GET_CODE(elt->exp) != SIGN_EXTEND
                    && GET_CODE(elt->exp) != ZERO_EXTEND && GET_CODE(XEXP(elt->exp, 0)) == SUBREG
                    && GET_MODE(SUBREG_REG(XEXP(elt->exp, 0))) == mode)
                {
                    ...
                    if (op0)
                        ret = simplify_unary_operation(GET_CODE(elt->exp), mode, op0, mode);
                }
                else if ((GET_RTX_CLASS(GET_CODE(elt->exp)) == '2'
                             || GET_RTX_CLASS(GET_CODE(elt->exp)) == 'c')
                    && eltcode != DIV && eltcode != MOD && eltcode != UDIV && eltcode != UMOD
                    && eltcode != ASHIFTRT && eltcode != LSHIFTRT && eltcode != ROTATE
                    && eltcode != ROTATERT
                    && ((GET_CODE(XEXP(elt->exp, 0)) == SUBREG
                            && (GET_MODE(SUBREG_REG(XEXP(elt->exp, 0))) == mode))
                           || CONSTANT_P(XEXP(elt->exp, 0)))
                    && ((GET_CODE(XEXP(elt->exp, 1)) == SUBREG
                            && (GET_MODE(SUBREG_REG(XEXP(elt->exp, 1))) == mode))
                           || CONSTANT_P(XEXP(elt->exp, 1))))
                {
                    ...
                    if (op0 && op1 && GET_CODE(elt->exp) == ASHIFT && GET_CODE(op1) == CONST_INT
                        && INTVAL(op1) >= GET_MODE_BITSIZE(mode))
                    {
                        ...
                        ret = simplify_binary_operation(GET_CODE(elt->exp), mode, op0, op1);
                }

                else if (GET_CODE(elt->exp) == SUBREG && GET_MODE(SUBREG_REG(elt->exp)) == mode
                    && (GET_MODE_SIZE(GET_MODE(folded_arg0)) <= UNITS_PER_WORD)
                    && exp_equiv_p(elt->exp, elt->exp, 1, 0))
                    ret = copy_rtx(SUBREG_REG(elt->exp));
    ...
    case NEG:
       ...
       If so, (NOT Y) simplifies to Z.  Similarly for NEG.  */
        ret = lookup_as_function(XEXP(x, 0), code);
        if (ret)
            return fold_rtx(copy_rtx(XEXP(ret, 0)), insn);
        break;
    ...
    case MEM:
       ...
       MEM in an invalid way since we have no insn to validate against.  */
...

        {
            ...
            if (GET_CODE(addr) == REG && REGNO_QTY_VALID_P(REGNO(addr))
            ...
            else if (GET_CODE(addr) == LO_SUM && GET_CODE(XEXP(addr, 1)) == SYMBOL_REF)
                base = XEXP(addr, 1);
            ...
            if (base && GET_CODE(base) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P(base))
            {
                ...
                if (GET_MODE_CLASS(mode) == MODE_INT && GET_MODE_SIZE(mode) == UNITS_PER_WORD
                    ...
                    && (new_rtx = operand_subword(constant, offset / UNITS_PER_WORD, 0, const_mode))
                        != 0)
                    return new_rtx;

                if (offset == 0 && (new_rtx = gen_lowpart_if_possible(mode, constant)) != 0)
                    return new_rtx;
            }
            ...
            if (base && GET_CODE(base) == LABEL_REF)
            {
                    ...
                    && GET_CODE(PATTERN(table_insn)) == ADDR_VEC)
                {
                    ...
                    if (offset >= 0
                        && (offset / GET_MODE_SIZE(GET_MODE(table)) < XVECLEN(table, 0)))
                        return XVECEXP(table, 0, offset / GET_MODE_SIZE(GET_MODE(table)));
                }
                if (table_insn && GET_CODE(table_insn) == JUMP_INSN
                    && GET_CODE(PATTERN(table_insn)) == ADDR_DIFF_VEC)
                {
                    ...
                    if (offset >= 0
                        && (offset / GET_MODE_SIZE(GET_MODE(table)) < XVECLEN(table, 1)))
                    {
                        offset /= GET_MODE_SIZE(GET_MODE(table));
                        ret = gen_rtx_MINUS(Pmode, XVECEXP(table, 1, offset), XEXP(table, 0));

                        if (GET_MODE(table) != Pmode)
                            ret = gen_rtx_TRUNCATE(GET_MODE(table), ret);

                        /* Indicate this is a constant.  This isn't a
                           valid form of CONST, but it will only be used
                           to fold the next insns and then discarded, so
                           it should be safe.  */
                        return gen_rtx_CONST(GET_MODE(ret), ret);
                    }
                }
            }
    ...
    case ASM_OPERANDS:
        for (i = XVECLEN(x, 3) - 1; i >= 0; i--)
            validate_change(insn, &XVECEXP(x, 3, i), fold_rtx(XVECEXP(x, 3, i), insn), 0);
        break;
        ...
        if (fmt[i] == 'e')
        {
            ...
            case REG:
                /* This is the same as calling equiv_constant; it is duplicated
                   here for speed.  */
                if (REGNO_QTY_VALID_P(REGNO(arg)) && qty_const[reg_qty[REGNO(arg)]] != 0
                    && GET_CODE(qty_const[reg_qty[REGNO(arg)]]) != REG
                    && GET_CODE(qty_const[reg_qty[REGNO(arg)]]) != PLUS)
                    const_arg
                        = gen_lowpart_if_possible(GET_MODE(arg), qty_const[reg_qty[REGNO(arg)]]);
                break;
            ...
            if (const_arg == 0 || const_arg == folded_arg || COST(const_arg) > COST(folded_arg))
            ...
            for (j = 0; j < 2 && replacements[j] && COST(replacements[j]) < COST(XEXP(x, i)); j++)
            {
    ...
    case '1':
    {
        ...
        /* We can't simplify extension ops unless we know the
           original mode.  */
        if ((code == ZERO_EXTEND || code == SIGN_EXTEND) && mode_arg0 == VOIDmode)
            break;
        ...
        ret = simplify_unary_operation(
            code, mode, const_arg0 ? const_arg0 : folded_arg0, mode_arg0);
        if (ret != 0 && is_const)
            ret = gen_rtx_CONST(mode, ret);
    }
    break;
    ...
    case '<':
       ...
       do anything if both operands are already known to be constant.  */
        ...
        if (const_arg0 == 0 || const_arg1 == 0)
        {
            ...
            rtx true_cond = const_true_rtx, false_cond = const0_rtx;
            enum machine_mode mode_arg1;

#ifdef FLOAT_STORE_FLAG_VALUE
            if (GET_MODE_CLASS(mode) == MODE_FLOAT)
            {
                true_cond = CONST_DOUBLE_FROM_REAL_VALUE(FLOAT_STORE_FLAG_VALUE, mode);
                false_cond = CONST0_RTX(mode);
            }
#endif

            code = find_comparison_args(code, &folded_arg0, &folded_arg1, &mode_arg0, &mode_arg1);
            const_arg0 = equiv_constant(folded_arg0);
     ...
     come out as 0.  */
		      || GET_CODE (folded_arg0) == SYMBOL_REF
#endif
                           || GET_CODE(folded_arg0) == LABEL_REF || GET_CODE(folded_arg0) == CONST))
                {
                    if (code == EQ)
                        return false_cond;
                    else if (code == NE)
               ...
               since x might be a NaN.  */
                ...
                if ((TARGET_FLOAT_FORMAT != IEEE_FLOAT_FORMAT || !FLOAT_MODE_P(mode_arg0)
                        || flag_fast_math)
                    && (folded_arg0 == folded_arg1
                           || (GET_CODE(folded_arg0) == REG && GET_CODE(folded_arg1) == REG
                                  && (reg_qty[REGNO(folded_arg0)] == reg_qty[REGNO(folded_arg1)]))
                           || ((p0 = lookup(folded_arg0,
                                    (safe_hash(folded_arg0, mode_arg0) % NBUCKETS), mode_arg0))
                                  && (p1 = lookup(folded_arg1,
                                          (safe_hash(folded_arg1, mode_arg0) % NBUCKETS),
                                          mode_arg0))
                                  && p0->first_same_value == p1->first_same_value)))
                    return ((code == EQ || code == LE || code == GE || code == LEU || code == GEU)
                            ? true_cond
                            : false_cond);
                    ...
                    if (REGNO_QTY_VALID_P(REGNO(folded_arg0))
                        && (comparison_dominates_p(qty_comparison_code[qty], code)
                               || (comparison_dominates_p(
                                       qty_comparison_code[qty], reverse_condition(code))
                                      && !FLOAT_MODE_P(mode_arg0)))
                        && (rtx_equal_p(qty_comparison_const[qty], folded_arg1)
                               || (const_arg1 && rtx_equal_p(qty_comparison_const[qty], const_arg1))
                               || (GET_CODE(folded_arg1) == REG
                                      && (reg_qty[REGNO(folded_arg1)] == qty_comparison_qty[qty]))))
                        return (
                            comparison_dominates_p(qty_comparison_code[qty], code) ? true_cond : false_cond);
                }
            }
        }
       ...
       determine the result of this comparison.  */
        ...
        if (const_arg1 == const0_rtx)
        {
            ...
            if (y != 0 && (inner_const = equiv_constant(XEXP(y, 1))) != 0
                && GET_CODE(inner_const) == CONST_INT && INTVAL(inner_const) != 0)
            {
                int sign_bitnum = GET_MODE_BITSIZE(mode_arg0) - 1;
                int has_sign = (32 >= sign_bitnum
                    && (INTVAL(inner_const) & ((int32_t)1 << sign_bitnum)));
                rtx true_cond = const_true_rtx, false_cond = const0_rtx;
        ...
        ret = simplify_relational_operation(code, mode_arg0, const_arg0 ? const_arg0 : folded_arg0,
            const_arg1 ? const_arg1 : folded_arg1);
#ifdef FLOAT_STORE_FLAG_VALUE
        if (ret != 0 && GET_MODE_CLASS(mode) == MODE_FLOAT)
            ret = ((ret == const0_rtx)
                    ? CONST0_RTX(mode)
    ...
    case 'c':
        ...
        case PLUS:
            ...
            {
                rtx y = GET_CODE(folded_arg0) == MINUS ? folded_arg0
                ...
                if ((y = (GET_CODE(folded_arg0) == CONST ? folded_arg0
                        ...
                        != 0
                    && GET_CODE(XEXP(y, 0)) == MINUS && GET_CODE(XEXP(XEXP(y, 0), 1)) == LABEL_REF
            ...
            {
                rtx y = GET_CODE(folded_arg1) == MINUS ? folded_arg1
            ...
            if (const_arg1 != 0 && GET_CODE(const_arg1) == CONST_INT && INTVAL(const_arg1) < 0
                && -INTVAL(const_arg1) >= 0 && GET_CODE(folded_arg1) == REG)
            {
                rtx new_const = GEN_INT(-INTVAL(const_arg1));
                struct table_elt *p
                    = lookup(new_const, safe_hash(new_const, mode) % NBUCKETS, mode);

                if (p)
                    for (p = p->first_same_value; p; p = p->next_same_value)
                        if (GET_CODE(p->exp) == REG)
                            return cse_gen_binary(
                                MINUS, mode, folded_arg0, canon_reg(p->exp, NULL_RTX));
            }
            goto from_plus;
        ...
        case MINUS:
            ...
            if (const_arg1 != 0 && GET_CODE(const_arg1) == CONST_INT)
            {
                ...
                if (y && GET_CODE(XEXP(y, 1)) == CONST_INT)
                    return fold_rtx(plus_constant(copy_rtx(y), -INTVAL(const_arg1)), NULL_RTX);
            }
        ...
        case ASHIFTRT:
            ...
            if (GET_CODE(folded_arg0) == REG && const_arg1 && GET_CODE(const_arg1) == CONST_INT)
            {
                int is_shift = (code == ASHIFT || code == ASHIFTRT || code == LSHIFTRT);
                rtx y = lookup_as_function(folded_arg0, code);
                rtx inner_const;
                enum rtx_code associate_code;
                rtx new_const;

                if (y == 0 || 0 == (inner_const = equiv_constant(fold_rtx(XEXP(y, 1), 0)))
               ...
               identical powers of two with post decrement.  */
                ...
                if (code == PLUS && INTVAL(const_arg1) == INTVAL(inner_const)
                    && ((HAVE_PRE_INCREMENT && exact_log2(INTVAL(const_arg1)) >= 0)
                           || (HAVE_POST_INCREMENT && exact_log2(INTVAL(const_arg1)) >= 0)
                           || (HAVE_PRE_DECREMENT && exact_log2(-INTVAL(const_arg1)) >= 0)
                           || (HAVE_POST_DECREMENT && exact_log2(-INTVAL(const_arg1)) >= 0)))
                    break;

                /* Compute the code used to compose the constants.  For example,
               A/C1/C2 is A/(C1 * C2), so if CODE == DIV, we want MULT.  */

                associate_code = (code == MULT || code == DIV || code == UDIV
                        ? MULT
                        : is_shift || code == PLUS || code == MINUS ? PLUS : code);

                new_const
                    = simplify_binary_operation(associate_code, mode, const_arg1, inner_const);
        ...
        ret = simplify_binary_operation(code, mode, const_arg0 ? const_arg0 : folded_arg0,
            const_arg1 ? const_arg1 : folded_arg1);
        break;

    case 'o':
        /* (lo_sum (high X) X) is simply X.  */
        if (code == LO_SUM && const_arg0 != 0 && GET_CODE(const_arg0) == HIGH
    ...
    case '3':
    case 'b':
        ret = simplify_ternary_operation(code, mode, mode_arg0,
            const_arg0 ? const_arg0 : folded_arg0, const_arg1 ? const_arg1 : folded_arg1,
...
{
    if (GET_CODE(x) == REG && REGNO_QTY_VALID_P(REGNO(x)) && qty_const[reg_qty[REGNO(x)]])
...
rtx gen_lowpart_if_possible(enum machine_mode mode, rtx x)
{
    ...
    else if (GET_CODE(x) == MEM)
    {
        ...
        if (!memory_address_p(mode, XEXP(ret, 0)))
            return 0;
...
static void record_jump_cond(
    enum rtx_code code, enum machine_mode mode, rtx op0, rtx op1, int reversed_nonequality)
{
        ...
        && (GET_MODE_SIZE(GET_MODE(op0)) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(op0)))))
    {
        ...
        record_jump_cond(code, mode, SUBREG_REG(op0),
            tem ? tem : gen_rtx_SUBREG(inner_mode, op1, 0), reversed_nonequality);
    }

    if (code == EQ && GET_CODE(op1) == SUBREG
        && (GET_MODE_SIZE(GET_MODE(op1)) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(op1)))))
    {
        enum machine_mode inner_mode = GET_MODE(SUBREG_REG(op1));
        rtx tem = gen_lowpart_if_possible(inner_mode, op0);

        record_jump_cond(code, mode, SUBREG_REG(op1),
            tem ? tem : gen_rtx_SUBREG(inner_mode, op0, 0), reversed_nonequality);
    }
    ...
    if (code == NE && GET_CODE(op0) == SUBREG && subreg_lowpart_p(op0)
        && (GET_MODE_SIZE(GET_MODE(op0)) < GET_MODE_SIZE(GET_MODE(SUBREG_REG(op0)))))
    {
        enum machine_mode inner_mode = GET_MODE(SUBREG_REG(op0));
        rtx tem = gen_lowpart_if_possible(inner_mode, op1);
    ...
    if (code == NE && GET_CODE(op1) == SUBREG && subreg_lowpart_p(op1)
        && (GET_MODE_SIZE(GET_MODE(op1)) < GET_MODE_SIZE(GET_MODE(SUBREG_REG(op1)))))
    {
        enum machine_mode inner_mode = GET_MODE(SUBREG_REG(op1));
        rtx tem = gen_lowpart_if_possible(inner_mode, op0);
    ...
    if ((op0_elt != 0 && op1_elt != 0 && op0_elt->first_same_value == op1_elt->first_same_value)
    ...
    if (code != EQ || FLOAT_MODE_P(GET_MODE(op0)))
    {
       ...
       do anything.  */
        ...
        if ((reversed_nonequality && FLOAT_MODE_P(mode)) || GET_CODE(op0) != REG || op1 == 0)
            return;
...
static void cse_insn(rtx insn, rtx libcall_insn)
{
    ...
    if (GET_CODE(x) == SET)
    {
       ...
       that were unconditional jumps to begin with.  */
        if (SET_DEST(x) == pc_rtx && GET_CODE(SET_SRC(x)) == LABEL_REF)
            ;
       ...
       anything in that case.  */
        for (i = 0; i < lim; i++)
        {
            ...
            if (GET_CODE(y) == CLOBBER)
            {
                ...
                if (GET_CODE(clobbered) == REG || GET_CODE(clobbered) == SUBREG)
        ...
        for (i = 0; i < lim; i++)
        {
            ...
            if (GET_CODE(y) == SET)
            {
               ...
               ignore the result of apply_change_group.  */
                ...
                else if (SET_DEST(y) == pc_rtx && GET_CODE(SET_SRC(y)) == LABEL_REF)
                    ;
                else
                    sets[n_sets++].rtl = y;
            }
    ...
    if (n_sets == 1 && REG_NOTES(insn) != 0 && (tem = find_reg_note(insn, REG_EQUAL, NULL_RTX)) != 0
    ...
    for (i = 0; i < n_sets; i++)
    {
        ...
        if ((GET_CODE(tmp) == REG && GET_CODE(src) == REG
                && ((REGNO(tmp) < FIRST_PSEUDO_REGISTER) != (REGNO(src) < FIRST_PSEUDO_REGISTER)))
            || (insn_code = recog_memoized(insn)) < 0 || insn_n_dups[insn_code] > 0)
            validate_change(insn, &SET_SRC(sets[i].rtl), tmp, 1);
        else
            SET_SRC(sets[i].rtl) = tmp;

        if (GET_CODE(dest) == ZERO_EXTRACT || GET_CODE(dest) == SIGN_EXTRACT)
        {
            validate_change(insn, &XEXP(dest, 1), canon_reg(XEXP(dest, 1), insn), 1);
            validate_change(insn, &XEXP(dest, 2), canon_reg(XEXP(dest, 2), insn), 1);
        }
	  ...
	  || GET_CODE (SET_DEST (sets[i].rtl)) == SIGN_EXTRACT)
	{
	  ...
	  if (GET_CODE (src) == CONST_INT
	      ...
	      && INTVAL (width) < 32
	      && (INTVAL (src) & ((int32_t) (-1) << INTVAL (width))))
	    src_folded
	      = GEN_INT (INTVAL (src) & (((int32_t) 1
					  << INTVAL (width)) - 1));
	}
#endif
       ...
       hash table.  */
        ...
        if (GET_CODE(src) == MEM && find_reg_note(insn, REG_EQUIV, src) != 0
	...
	sets[i].src_volatile = 1;
        ...
        if (src_const == 0
            && (CONSTANT_P(src_folded)
                  ...
                  to fold switch statements when an ADDR_DIFF_VEC is used.  */
                   || (GET_CODE(src_folded) == MINUS && GET_CODE(XEXP(src_folded, 0)) == LABEL_REF
       ...
       isn't, set ELT to it.  */
        if (src_const_elt && elt && src_const_elt->first_same_value != elt->first_same_value)
                   ...
                   || (src_const_elt && src_const_elt->related_value != 0)))
        {
            ...
            if (src_related)
            {
                ...
                {
                    if (elt->first_same_value != src_related_elt->first_same_value)
       ...
       wider mode.  */
        ...
        if (src_const && src_related == 0 && GET_CODE(src_const) == CONST_INT
            && GET_MODE_CLASS(mode) == MODE_INT && GET_MODE_BITSIZE(mode) < BITS_PER_WORD)
        {
            enum machine_mode wider_mode;

            for (wider_mode = GET_MODE_WIDER_MODE(mode);
                 GET_MODE_BITSIZE(wider_mode) <= BITS_PER_WORD && src_related == 0;
                 wider_mode = GET_MODE_WIDER_MODE(wider_mode))
            {
                ...
                for (const_elt = const_elt->first_same_value; const_elt;
                     const_elt = const_elt->next_same_value)
                    if (GET_CODE(const_elt->exp) == REG)
                    {
                        src_related = gen_lowpart_if_possible(mode, const_elt->exp);
                        break;
                    }
            }
        }
       ...
       value.  */
        ...
        if (flag_expensive_optimizations && !src_related && GET_CODE(src) == AND
            && GET_CODE(XEXP(src, 1)) == CONST_INT && GET_MODE_SIZE(mode) < UNITS_PER_WORD)
        {
            enum machine_mode tmode;
            rtx new_and = gen_rtx_AND(VOIDmode, NULL_RTX, XEXP(src, 1));

            for (tmode = GET_MODE_WIDER_MODE(mode); GET_MODE_SIZE(tmode) <= UNITS_PER_WORD;
                 tmode = GET_MODE_WIDER_MODE(tmode))
            {
                ...
                if (inner)
                {
                    ...
                    for (larger_elt = larger_elt->first_same_value; larger_elt;
                         larger_elt = larger_elt->next_same_value)
                        if (GET_CODE(larger_elt->exp) == REG)
                        {
                            src_related = gen_lowpart_if_possible(mode, larger_elt->exp);
                            break;
                        }
       ...
       beneficial these machines.  */
        ...
        if (flag_expensive_optimizations && src_related == 0
            && (GET_MODE_SIZE(mode) < UNITS_PER_WORD) && GET_MODE_CLASS(mode) == MODE_INT
            && GET_CODE(src) == MEM && !do_not_record && LOAD_EXTEND_OP(mode) != NIL)
        {
            enum machine_mode tmode;
                 ...
                 tmode = GET_MODE_WIDER_MODE(tmode))
            {
                ...
                larger_elt = lookup(memory_extend_rtx, HASH(memory_extend_rtx, tmode), tmode);
                if (larger_elt == 0)
                    continue;
       ...
       `src_related'.  */
        ...
        for (p = elt; p; p = p->next_same_value)
        {
            ...
            /* Also skip paradoxical subregs, unless that's what we're
               looking for.  */
            if (code == SUBREG
                && (GET_MODE_SIZE(GET_MODE(p->exp)) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(p->exp))))
                && !(src != 0 && GET_CODE(src) == SUBREG && GET_MODE(src) == GET_MODE(p->exp)
                       && (GET_MODE_SIZE(GET_MODE(SUBREG_REG(src)))
            ...
            if (src && GET_CODE(src) == code && rtx_equal_p(src, p->exp))
                src = 0;
            else if (src_folded && GET_CODE(src_folded) == code && rtx_equal_p(src_folded, p->exp))
                src_folded = 0;
       ...
       cheaper even though it looks more expensive.  */
        ...
        while (1)
        {
            ...
            while (elt && GET_CODE(elt->exp) != REG && !exp_equiv_p(elt->exp, elt->exp, 1, 0))
                elt = elt->next_same_value;

            /* A paradoxical subreg would be bad here: it'll be the right
               size, but later may be adjusted so that the upper bits aren't
               what we want.  So reject it.  */
            if (elt != 0 && GET_CODE(elt->exp) == SUBREG
                && (GET_MODE_SIZE(GET_MODE(elt->exp))
                       > GET_MODE_SIZE(GET_MODE(SUBREG_REG(elt->exp))))
                /* It is okay, though, if the rtx we're trying to match
               will ignore any of the bits we can't predict.  */
                && !(src != 0 && GET_CODE(src) == SUBREG && GET_MODE(src) == GET_MODE(elt->exp)
                       && (GET_MODE_SIZE(GET_MODE(SUBREG_REG(src)))
                              < GET_MODE_SIZE(GET_MODE(SUBREG_REG(elt->exp))))))
            {
                elt = elt->next_same_value;
           ...
           src_folded, src, src_eqv, src_related and hash table entry.  */
            if (src_folded_cost <= src_cost && src_folded_cost <= src_eqv_cost
                && src_folded_cost <= src_related_cost && src_folded_cost <= src_elt_cost)
            {
                trial = src_folded, src_folded_cost = 10000;
                if (src_folded_force_flag)
                    trial = force_const_mem(mode, trial);
            }
            else if (src_cost <= src_eqv_cost && src_cost <= src_related_cost
                && src_cost <= src_elt_cost)
                trial = src, src_cost = 10000;
            else if (src_eqv_cost <= src_related_cost && src_eqv_cost <= src_elt_cost)
            ...
            if (n_sets == 1 && dest == pc_rtx
                && (trial == pc_rtx || (GET_CODE(trial) == LABEL_REF && !condjump_p(insn))))
            {
               ...
               insns work), so we must branch around the table.  */
                if (GET_CODE(trial) == CODE_LABEL && NEXT_INSN(trial) != 0
            ...
            /* A libcall return register would also be bad news if we're
               ...
               not making the substitution at this time.  */
                ...
                && !(GET_CODE(old_src) == REG && !reg_mentioned_p(old_src, REG_NOTES(libcall_insn)))
            ...
            else if (validate_change(insn, &SET_SRC(sets[i].rtl), trial, 0))
            {
               ...
               to the RETVAL insn.  */
                if (libcall_insn
                    && (GET_CODE(old_src) == REG || GET_CODE(old_src) == SUBREG
                           || GET_CODE(old_src) == MEM))
                    replace_rtx(
                        REG_NOTES(libcall_insn), old_src, canon_reg(SET_SRC(sets[i].rtl), insn));

                /* The result of apply_change_group can be ignored; see
               canon_reg.  */

                validate_change(
                    insn, &SET_SRC(sets[i].rtl), canon_reg(SET_SRC(sets[i].rtl), insn), 1);
                apply_change_group();
                break;
            }
            ...
            else if (constant_pool_entries_cost && CONSTANT_P(trial)
                && !(GET_CODE(trial) == CONST && GET_CODE(XEXP(trial, 0)) == TRUNCATE)
                && (src_folded == 0 || (GET_CODE(src_folded) != MEM && !src_folded_force_flag))
                && GET_MODE_CLASS(mode) != MODE_CC && mode != VOIDmode)
            {
       ...
       their lifetimes will likely abut instead of overlapping.  */
        if (GET_CODE(dest) == REG && REGNO_QTY_VALID_P(REGNO(dest))
            && qty_mode[reg_qty[REGNO(dest)]] == GET_MODE(dest)
            && qty_first_reg[reg_qty[REGNO(dest)]] != REGNO(dest) && GET_CODE(src) == REG
            && REGNO(src) == REGNO(dest)
            /* Don't do this if the original insn had a hard reg as
               SET_SRC.  */
            && (GET_CODE(sets[i].src) != REG || REGNO(sets[i].src) >= FIRST_PSEUDO_REGISTER))
        /* We can't call canon_reg here because it won't do anything if
           SRC is a hard register.  */
        {
            int first = qty_first_reg[reg_qty[REGNO(src)]];
            rtx new_src = (first >= FIRST_PSEUDO_REGISTER ? regno_reg_rtx[first]
       ...
       won't help anything.  */
        if (n_sets == 1 && src_const && GET_CODE(dest) == REG && GET_CODE(src_const) != REG)
        {
            ...
            if (tem)
                ...
                REG_NOTES(insn) = gen_rtx_EXPR_LIST(REG_EQUAL, src_const, REG_NOTES(insn));
           ...
           the last set for this quantity was for this register.  */
            ...
            if (REGNO_QTY_VALID_P(REGNO(dest)) && qty_const[reg_qty[REGNO(dest)]] == const0_rtx)
            {
                ...
                if ((tem = single_set(const_insn)) != 0 && rtx_equal_p(SET_DEST(tem), dest))
                {
                    if (note)
                        XEXP(note, 0) = const_insn;
                    else
                        REG_NOTES(insn) = gen_rtx_INSN_LIST(REG_WAS_0, const_insn, REG_NOTES(insn));
                }
            }
        }
       ...
       to the MEM or REG within it.  */
        while (GET_CODE(dest) == SIGN_EXTRACT || GET_CODE(dest) == ZERO_EXTRACT
       ...
       may not equal what was stored, due to truncation.  */
            ...
            || GET_CODE(SET_DEST(sets[i].rtl)) == SIGN_EXTRACT)
        {
            ...
            if (src_const != 0 && GET_CODE(src_const) == CONST_INT && GET_CODE(width) == CONST_INT
                && INTVAL(width) < 32
                && !(INTVAL(src_const) & ((int32_t)(-1) << INTVAL(width))))
       ...
       it.  If it was a computed branch, delete it and re-emit.  */
        else if (dest == pc_rtx && GET_CODE(src) == LABEL_REF)
        {
            ...
            {
                rtx tmp = emit_jump_insn_before(gen_jump(XEXP(src, 0)), insn);
                JUMP_LABEL(tmp) = XEXP(src, 0);
            ...
            while (NEXT_INSN(p) != 0 && GET_CODE(NEXT_INSN(p)) != BARRIER
            ...
            if (NEXT_INSN(insn) == 0 || GET_CODE(NEXT_INSN(insn)) != BARRIER)
                emit_barrier_before(NEXT_INSN(insn));

            /* We might have two BARRIERs separated by notes.  Delete the second
               one if so.  */

            if (p != insn && NEXT_INSN(p) != 0 && GET_CODE(NEXT_INSN(p)) == BARRIER)
                delete_insn(NEXT_INSN(p));
       ...
       processing for this assignment.  */
        ...
        {
            if (GET_CODE(dest) == REG || GET_CODE(dest) == SUBREG || GET_CODE(dest) == MEM)
                invalidate(dest, VOIDmode);
            else if (GET_CODE(dest) == STRICT_LOW_PART || GET_CODE(dest) == ZERO_EXTRACT)
       ...
       insn as a floating-point operand.  */
        if (dest == cc0_rtx)
        {
            ...
            if (FLOAT_MODE_P(mode))
                this_insn_cc0 = gen_rtx_COMPARE(VOIDmode, this_insn_cc0, CONST0_RTX(mode));
        }
#endif
    }
        ...
        && !rtx_equal_p(src_eqv, SET_DEST(sets[0].rtl)))
    {
       ...
       to src_eqv_elt.  */
        for (i = 0; i < n_sets; i++)
            if (sets[i].rtl && sets[i].src_elt == 0 && rtx_equal_p(SET_SRC(sets[i].rtl), src_eqv))
    ...
    for (i = 0; i < n_sets; i++)
            ...
            && !rtx_equal_p(SET_SRC(sets[i].rtl), SET_DEST(sets[i].rtl)))
        {
            ...
            else
            {
                ...
                rtx src = sets[i].src;
                rtx dest = SET_DEST(sets[i].rtl);
                enum machine_mode mode = GET_MODE(src) == VOIDmode ? GET_MODE(dest) : GET_MODE(src);
                ...
                if (sets[i].src_const && sets[i].src_const_elt == 0 && src != sets[i].src_const
                    && !rtx_equal_p(sets[i].src_const, src))
                    sets[i].src_elt
                        = insert(sets[i].src_const, classp, sets[i].src_const_hash, mode);
            }
        }
        ...
        if (sets[i].rtl)
        {
            ...
            else
            {
               ...
               it leaves reg_in_table as -1 .  */
                int regno = REGNO(x);
                int endregno = regno
                    + (regno >= FIRST_PSEUDO_REGISTER ? 1 : HARD_REGNO_NREGS(regno, GET_MODE(x)));
                int i;
    ...
    for (i = 0; i < n_sets; i++)
        if (sets[i].rtl)
        {
            ...
            /* Don't record value if we are not supposed to risk allocating
               ...
               memory.  */
            if ((flag_float_store && GET_CODE(dest) == MEM && FLOAT_MODE_P(GET_MODE(dest)))
            ...
            if (GET_CODE(inner_dest) == MEM && GET_CODE(XEXP(inner_dest, 0)) == ADDRESSOF)
                ...
                elt = insert(dest, sets[i].src_elt, sets[i].dest_hash, GET_MODE(dest));

            elt->in_memory = (GET_CODE(sets[i].inner_dest) == MEM
                && (!RTX_UNCHANGING_P(sets[i].inner_dest)
                       || FIXED_BASE_PLUS_P(XEXP(sets[i].inner_dest, 0))));
            ...
            if (GET_CODE(dest) == SUBREG
                && (((GET_MODE_SIZE(GET_MODE(SUBREG_REG(dest))) - 1) / UNITS_PER_WORD)
                       == (GET_MODE_SIZE(GET_MODE(dest)) - 1) / UNITS_PER_WORD)
                && (GET_MODE_SIZE(GET_MODE(dest)) >= GET_MODE_SIZE(GET_MODE(SUBREG_REG(dest))))
                && sets[i].src_elt != 0)
            {
                enum machine_mode new_mode = GET_MODE(SUBREG_REG(dest));
                struct table_elt *elt, *classp = 0;

                for (elt = sets[i].src_elt->first_same_value; elt; elt = elt->next_same_value)
                {
                    rtx new_src = 0;
                    unsigned src_hash;
                    struct table_elt *src_elt;
                    ...
                    if (GET_CODE(elt->exp) != REG && !exp_equiv_p(elt->exp, elt->exp, 1, 0))
                        continue;
                    ...
                    /* Ignore invalid entries.  */
                    while (classp && GET_CODE(classp->exp) != REG
    ...
    if (n_sets == 1 && sets[0].rtl && GET_CODE(SET_DEST(sets[0].rtl)) == REG
        && NEXT_INSN(PREV_INSN(insn)) == insn && GET_CODE(SET_SRC(sets[0].rtl)) == REG
        && REGNO(SET_SRC(sets[0].rtl)) >= FIRST_PSEUDO_REGISTER
        && REGNO_QTY_VALID_P(REGNO(SET_SRC(sets[0].rtl)))
        && (qty_first_reg[reg_qty[REGNO(SET_SRC(sets[0].rtl))]] == REGNO(SET_DEST(sets[0].rtl))))
    {
            ...
            && SET_DEST(PATTERN(prev)) == SET_SRC(sets[0].rtl))
        {
            ...
            validate_change(insn, &SET_DEST(sets[0].rtl), SET_SRC(sets[0].rtl), 1);
    ...
    if (GET_CODE(insn) == JUMP_INSN && n_sets == 1 && GET_CODE(x) == SET
    ...
    if (prev_insn && GET_CODE(prev_insn) == INSN && (tem = single_set(prev_insn)) != 0
        && SET_DEST(tem) == cc0_rtx && !reg_mentioned_p(cc0_rtx, x))
    {
...
static int note_mem_written(rtx addr)
{
    /* Pushing or popping the stack invalidates just the stack pointer.  */
    if ((GET_CODE(addr) == PRE_DEC || GET_CODE(addr) == PRE_INC || GET_CODE(addr) == POST_DEC
            || GET_CODE(addr) == POST_INC)
        && GET_CODE(XEXP(addr, 0)) == REG && REGNO(XEXP(addr, 0)) == STACK_POINTER_REGNUM)
    {
...
static void invalidate_from_clobbers(rtx x)
{
    if (GET_CODE(x) == CLOBBER)
    {
        ...
        if (ref)
        {
            if (GET_CODE(ref) == REG || GET_CODE(ref) == SUBREG || GET_CODE(ref) == MEM)
                invalidate(ref, VOIDmode);
            else if (GET_CODE(ref) == STRICT_LOW_PART || GET_CODE(ref) == ZERO_EXTRACT)
...
static rtx cse_process_notes(rtx x, rtx object)
{
    ...
    case SUBREG:
    {
        rtx tmp = cse_process_notes(XEXP(x, 0), object);
        /* We don't substitute VOIDmode constants into these rtx,
           since they would impede folding.  */
        if (GET_MODE(tmp) != VOIDmode)
            validate_change(object, &XEXP(x, 0), tmp, 0);
        return x;
    }
    ...
    case REG:
        ...
        if (REGNO_QTY_VALID_P(REGNO(x)) && qty_const[i] != 0
            && (CONSTANT_P(qty_const[i]) || GET_CODE(qty_const[i]) == REG))
        {
            rtx low = gen_lowpart_if_possible(GET_MODE(x), qty_const[i]);
            if (low)
                return low;
        }
    ...
    for (i = 0; i < GET_RTX_LENGTH(code); i++)
        if (fmt[i] == 'e')
            validate_change(object, &XEXP(x, i), cse_process_notes(XEXP(x, i), object), 0);
...
static void cse_around_loop(rtx loop_start)
{
    ...
    if (insn == 0 || GET_CODE(insn) != NOTE || NOTE_LINE_NUMBER(insn) != NOTE_INSN_LOOP_BEG)
        return;
    ...
    if (last_jump_equiv_class)
        for (p = last_jump_equiv_class->first_same_value; p; p = p->next_same_value)
        {
            if (GET_CODE(p->exp) == MEM || GET_CODE(p->exp) == REG
                || (GET_CODE(p->exp) == SUBREG && GET_CODE(SUBREG_REG(p->exp)) == REG))
                invalidate(p->exp, VOIDmode);
            else if (GET_CODE(p->exp) == STRICT_LOW_PART || GET_CODE(p->exp) == ZERO_EXTRACT)
    ...
    for (insn = NEXT_INSN(loop_start); GET_CODE(insn) != CALL_INSN && GET_CODE(insn) != CODE_LABEL
         && INSN_UID(insn) < max_insn_uid
         && !(GET_CODE(insn) == NOTE && NOTE_LINE_NUMBER(insn) == NOTE_INSN_LOOP_END);
         insn = NEXT_INSN(insn))
    {
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i'
            && (GET_CODE(PATTERN(insn)) == SET || GET_CODE(PATTERN(insn)) == CLOBBER))
            cse_set_around_loop(PATTERN(insn), insn, loop_start);
        else if (GET_RTX_CLASS(GET_CODE(insn)) == 'i' && GET_CODE(PATTERN(insn)) == PARALLEL)
            for (i = XVECLEN(PATTERN(insn), 0) - 1; i >= 0; i--)
                if (GET_CODE(XVECEXP(PATTERN(insn), 0, i)) == SET
                    || GET_CODE(XVECEXP(PATTERN(insn), 0, i)) == CLOBBER)
                    cse_set_around_loop(XVECEXP(PATTERN(insn), 0, i), insn, loop_start);
    }
}
...
static void invalidate_skipped_set(rtx dest, rtx set)
{
    ...
    if (code == MEM
       ...
       variable address.  */
        && (MEM_IN_STRUCT_P(dest) || GET_MODE(dest) == BLKmode || cse_rtx_varies_p(XEXP(dest, 0))))
    {
        invalidate_memory();
        return;
    }
...
static void invalidate_skipped_block(rtx start)
{
    ...
    for (insn = start; insn && GET_CODE(insn) != CODE_LABEL; insn = NEXT_INSN(insn))
    {
        if (GET_RTX_CLASS(GET_CODE(insn)) != 'i')
            continue;
...
static void cse_check_loop_start(rtx x, rtx set ATTRIBUTE_UNUSED)
{
    if (cse_check_loop_start_value == 0 || GET_CODE(x) == CC0 || GET_CODE(x) == PC)
        return;
...
static void cse_set_around_loop(rtx x, rtx insn, rtx loop_start)
{
    ...
    if (GET_CODE(x) == SET && GET_CODE(SET_DEST(x)) != PC && GET_CODE(SET_DEST(x)) != CC0
        && GET_CODE(SET_SRC(x)) != REG)
    {
        src_elt
            = lookup(SET_SRC(x), HASH(SET_SRC(x), GET_MODE(SET_DEST(x))), GET_MODE(SET_DEST(x)));

        if (src_elt)
            for (src_elt = src_elt->first_same_value; src_elt; src_elt = src_elt->next_same_value)
                    ...
                    && COST(src_elt->exp) < COST(SET_SRC(x)))
                {
                   ...
                   a label or CALL_INSN.  */
                    ...
                    for (p = prev_nonnote_insn(loop_start);
                         p && GET_CODE(p) != CALL_INSN && GET_CODE(p) != CODE_LABEL;
                         p = prev_nonnote_insn(p))
                        if ((set = single_set(p)) != 0 && GET_CODE(SET_DEST(set)) == REG
                            ...
                            && rtx_equal_p(SET_SRC(set), SET_SRC(x)))
                        {
                            ...
                            if (cse_check_loop_start_value
                                && validate_change(insn, &SET_SRC(x), src_elt->exp, 0))
                            {
                                ...
                                rtx move = gen_move_insn(src_elt->exp, SET_DEST(set));
                                if (nregs != max_reg_num())
                                {
                                    if (!validate_change(insn, &SET_SRC(x), SET_SRC(set), 0))
                                        abort();
                                }
                                else
    ...
    if (GET_CODE(SET_DEST(x)) == REG || GET_CODE(SET_DEST(x)) == SUBREG
        ...
        invalidate(SET_DEST(x), VOIDmode);
    else if (GET_CODE(SET_DEST(x)) == STRICT_LOW_PART || GET_CODE(SET_DEST(x)) == ZERO_EXTRACT)
...
void cse_end_of_basic_block(
    rtx insn, struct cse_basic_block_data *data, int follow_jumps, int after_loop, int skip_blocks)
{
    rtx p = insn, q;
    int nsets = 0;
    ...
    while (p && GET_CODE(p) != CODE_LABEL)
    {
       ...
       the NOTE_INSN_LOOP_END.  */
        ...
        if (!after_loop && GET_CODE(p) == NOTE && NOTE_LINE_NUMBER(p) == NOTE_INSN_LOOP_END)
            break;

        /* Don't cse over a call to setjmp; on some machines (eg vax)
       the regs restored by the longjmp come from
       a later time than the setjmp.  */
        if (GET_CODE(p) == NOTE && NOTE_LINE_NUMBER(p) == NOTE_INSN_SETJMP)
            break;

        /* A PARALLEL can have lots of SETs in it,
       especially if it is really an ASM_OPERANDS.  */
        if (GET_RTX_CLASS(GET_CODE(p)) == 'i' && GET_CODE(PATTERN(p)) == PARALLEL)
       ...
       registers set in the block when following the jump.  */
            ...
            && GET_CODE(p) == JUMP_INSN && GET_CODE(PATTERN(p)) == SET
            && GET_CODE(SET_SRC(PATTERN(p))) == IF_THEN_ELSE && JUMP_LABEL(p) != 0
            && LABEL_NUSES(JUMP_LABEL(p)) == 1 && NEXT_INSN(JUMP_LABEL(p)) != 0)
        {
            for (q = PREV_INSN(JUMP_LABEL(p)); q; q = PREV_INSN(q))
                if ((GET_CODE(q) != NOTE || NOTE_LINE_NUMBER(q) == NOTE_INSN_LOOP_END
    ...
    while (insn)
    {
       ...
       past the end of this basic block, if that should prove useful.  */
        ...
        else
        {
            ...
            if (cse_jumps_altered == 0 || (flag_cse_follow_jumps == 0 && flag_cse_skip_blocks == 0))
                insn = temp;
...
static rtx cse_basic_block(
    rtx from, rtx to, struct branch_path *next_branch, int around_loop)
{
    rtx insn;
    int to_usage = 0;
    ...
    qty_comparison_code = (enum rtx_code *)alloca((max_qty - max_reg) * sizeof(enum rtx_code));
    ...
    for (insn = from; insn != to; insn = NEXT_INSN(insn))
    {
       ...
       want to count the use in that jump.  */
        ...
        if (to != 0 && NEXT_INSN(insn) == to && GET_CODE(to) == CODE_LABEL
            && --LABEL_NUSES(to) == to_usage)
        {
            struct cse_basic_block_data val;
            rtx prev;
    ...
    if ((cse_jumps_altered == 0 || (flag_cse_follow_jumps == 0 && flag_cse_skip_blocks == 0))
        && around_loop && to != 0 && GET_CODE(to) == NOTE
        && NOTE_LINE_NUMBER(to) == NOTE_INSN_LOOP_END && GET_CODE(PREV_INSN(to)) == JUMP_INSN
        && JUMP_LABEL(PREV_INSN(to)) != 0 && LABEL_NUSES(JUMP_LABEL(PREV_INSN(to))) == 1)
...
static void count_reg_usage(rtx x, int *counts, rtx dest, int incr)
{
    ...
    case SET:
       ...
       to do and can't happen now.  */
        count_reg_usage(
            SET_SRC(x), counts, side_effects_p(SET_SRC(x)) ? NULL_RTX : SET_DEST(x), incr);
        return;
...
void delete_trivially_dead_insns(rtx insns, int nreg)
{
       ...
       that we are scanning backwards.  */
        ...
        else if (GET_CODE(PATTERN(insn)) == SET)
        {
            if (GET_CODE(SET_DEST(PATTERN(insn))) == REG
                ...
                && ((tem = next_nonnote_insn(insn)) == 0 || GET_RTX_CLASS(GET_CODE(tem)) != 'i'
            ...
            for (i = XVECLEN(PATTERN(insn), 0) - 1; i >= 0; i--)
            {
                ...
                if (GET_CODE(elt) == SET)
                {
                    if (GET_CODE(SET_DEST(elt)) == REG && SET_DEST(elt) == SET_SRC(elt))
                        ;

#ifdef HAVE_cc0
                    else if (GET_CODE(SET_DEST(elt)) == CC0 && !side_effects_p(SET_SRC(elt))
                        ...
                        || counts[REGNO(SET_DEST(elt))] != 0 || side_effects_p(SET_SRC(elt)))
                        live_insn = 1;
                }
