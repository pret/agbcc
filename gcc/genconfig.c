...
static void walk_insn_part(rtx part, int recog_p, int non_pc_set_src)
{
    ...
    case SET:
        ...
        walk_insn_part(SET_SRC(part), recog_p, GET_CODE(SET_DEST(part)) != PC);
        return;
    ...
    case IF_THEN_ELSE:
       ...
       context we want it for.  */
        ...
        if (recog_p && non_pc_set_src && GET_CODE(XEXP(part, 1)) == MATCH_OPERAND
        ...
        if (recog_p)
        {
            if ((GET_CODE(XEXP(part, 1)) == SET || GET_CODE(XEXP(part, 1)) == MATCH_OPERATOR
                    || GET_CODE(XEXP(part, 1)) == PARALLEL)
                && GET_CODE(XEXP(part, 2)) == CONST_INT && INTVAL(XEXP(part, 2)) == 0)
                have_cexecute_true_flag = 1;

            if ((GET_CODE(XEXP(part, 2)) == SET || GET_CODE(XEXP(part, 2)) == MATCH_OPERATOR
                    || GET_CODE(XEXP(part, 2)) == PARALLEL)
                && GET_CODE(XEXP(part, 1)) == CONST_INT && INTVAL(XEXP(part, 1)) == 0)
