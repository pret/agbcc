...
static void copy_loop_body(
    rtx, rtx, struct inline_remap *, rtx, int, enum unroll_types, rtx, rtx, rtx, rtx);
static void iteration_info(rtx, rtx *, rtx *, rtx, rtx);
static int find_splittable_regs(enum unroll_types, rtx, rtx, rtx, int, uint32_t);
static int find_splittable_givs(struct iv_class *, enum unroll_types, rtx, rtx, rtx, int);
...
void unroll_loop(rtx loop_end, int insn_count, rtx loop_start, rtx end_insert_before,
    struct loop_info *loop_info, int strength_reduce_p)
{
    int i, j, temp;
    int unroll_number = 1;
    ...
    if (1 /* write_symbols != NO_DEBUG */)
    {
        ...
        {
            if (loop_dump_stream)
                fprintf(loop_dump_stream, "Unrolling failure: Unbalanced block notes.\n");
            return;
        }
    }
    ...
    if (loop_dump_stream && loop_info->n_iterations > 0)
    {
        ...
        fprintf(loop_dump_stream, int32_t_PRINT_DEC, loop_info->n_iterations);
        fputs(" iterations.\n", loop_dump_stream);
    }
    ...
    if (unroll_type == UNROLL_COMPLETELY)
    {
       ...
       the loop will be correctly handled by copy_loop_body.  */
        ...
        else
        {
            ...
            if (loop_dump_stream)
                fprintf(
                    loop_dump_stream, "Unrolling failure: loop does not end with a JUMP_INSN.\n");
            return;
        }
    }
    ...
    else
    {
       ...
       the real exit label.  */
        ...
        if (GET_CODE(insn) == JUMP_INSN)
        {
            ...
            if (loop_dump_stream)
                fprintf(loop_dump_stream, "Splitting not safe, because loop not entered at top.\n");
        }
        else
            copy_start = loop_start;
    }
    ...
    if (LABEL_NAME(start_label))
    {
       ...
       in this case.  */
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Unrolling failure: loop start label is gone\n");
        return;
    }

    if (unroll_type == UNROLL_NAIVE && GET_CODE(last_loop_insn) == BARRIER
    ...
    if (unroll_type == UNROLL_NAIVE && GET_CODE(last_loop_insn) == JUMP_INSN
    ...
    for (insn = copy_start; insn != loop_end; insn = NEXT_INSN(insn))
    {
        ...
        {
            if (JUMP_LABEL(insn))
                set_label_in_map(map, CODE_LABEL_NUMBER(JUMP_LABEL(insn)), JUMP_LABEL(insn));
            ...
            set_label_in_map(map, CODE_LABEL_NUMBER(XEXP(note, 0)), XEXP(note, 0));
    }
    ...
    addr_combined_regs = (struct induction **)alloca(maxregnum * sizeof(struct induction *));
    ...
    if (INSN_UID(copy_end) < max_uid_for_loop)
    {
        ...
        for (j = FIRST_PSEUDO_REGISTER; j < max_reg_before_loop; ++j)
            if (REGNO_FIRST_UID(j) > 0 && REGNO_FIRST_UID(j) <= max_uid_for_loop
                && uid_luid[REGNO_FIRST_UID(j)] >= copy_start_luid && REGNO_LAST_UID(j) > 0
                && REGNO_LAST_UID(j) <= max_uid_for_loop
                && uid_luid[REGNO_LAST_UID(j)] <= copy_end_luid)
            {
                ...
                if (set_dominates_use(
                        j, REGNO_FIRST_UID(j), REGNO_LAST_UID(j), copy_start, copy_end))
                    local_regno[j] = 1;
    ...
    if (unroll_type == UNROLL_NAIVE && !splitting_not_safe && strength_reduce_p)
    {
                ...
                loop_start, loop_info, &initial_value, &final_value, &increment, &mode))
        {
            ...
            diff = expand_binop(mode, sub_optab, copy_rtx(final_value), copy_rtx(initial_value),
                NULL_RTX, 0, OPTAB_LIB_WIDEN);

            /* Now calculate (diff % (unroll * abs (increment))) by using an
               and instruction.  */
            diff = expand_binop(GET_MODE(diff), and_optab, diff,
                GEN_INT(unroll_number * abs_inc - 1), NULL_RTX, 0, OPTAB_LIB_WIDEN);
            ...
            if (loop_info->comparison_code != NE)
            {
                emit_cmp_and_jump_insns(
                    initial_value, final_value, neg_inc ? LE : GE, NULL_RTX, mode, 0, 0, labels[1]);
            ...
            for (i = 0; i < unroll_number - 1; i++)
            {
               ...
               against, except when comparing against zero.  */
                ...
                emit_cmp_and_jump_insns(
                    diff, GEN_INT(abs_inc * cmp_const), cmp_code, NULL_RTX, mode, 0, 0, labels[i]);
            ...
            if (abs_inc != 1)
            {
                ...
                emit_cmp_and_jump_insns(
                    diff, GEN_INT(cmp_const), cmp_code, NULL_RTX, mode, 0, 0, labels[0]);
            ...
            {
                emit_label_after(labels[unroll_number - i], PREV_INSN(loop_start));

                zero_memory((char *)map->insn_map, max_insnno * sizeof(rtx));
                zero_memory((char *)map->const_equiv_map, maxregnum * sizeof(rtx));
                zero_memory((char *)map->const_age_map, maxregnum * sizeof(unsigned));
                map->const_age = 0;
                ...
                for (j = FIRST_PSEUDO_REGISTER; j < maxregnum; j++)
                    ...
                    if (local_regno[j])
                    {
                        ...
                        record_base_value(REGNO(map->reg_map[j]), regno_reg_rtx[j], 0);
                    }
               ...
               pass zero for that parameter.  */
                copy_loop_body(copy_start, copy_end, map, exit_label, 0, unroll_type, start_label,
                    loop_end, loop_start, copy_end);
            }
    ...
    if (splitting_not_safe)
        ...
        temp = find_splittable_regs(unroll_type, loop_start, loop_end, end_insert_before,
            unroll_number, loop_info->n_iterations);
    ...
    for (i = 0; i < unroll_number; i++)
    {
       ...
       it points to the test of the first unrolled copy of the loop.  */
        if (i == 0 && loop_start != copy_start)
        {
            ...
            tem = get_label_from_map(map, CODE_LABEL_NUMBER(XEXP(SET_SRC(pattern), 0)));
        ...
        copy_loop_body(copy_start, copy_end, map, exit_label, i == unroll_number - 1, unroll_type,
            start_label, loop_end, insert_before, insert_before);
    }
...
int precondition_loop_p(rtx loop_start, struct loop_info *loop_info, rtx *initial_value,
    rtx *final_value, rtx *increment, enum machine_mode *mode)
{
    if (loop_info->n_iterations > 0)
    {
        ...
        *mode = word_mode;
        ...
        if (loop_dump_stream)
        {
            fputs("Preconditioning: Success, number of iterations known, ", loop_dump_stream);
            fprintf(loop_dump_stream, int32_t_PRINT_DEC, loop_info->n_iterations);
            fputs(".\n", loop_dump_stream);
        }
        return 1;
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Preconditioning: Could not find initial value.\n");
        return 0;
    }
    else if (loop_info->increment == 0)
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Preconditioning: Could not find increment value.\n");
        return 0;
    }
    else if (GET_CODE(loop_info->increment) != CONST_INT)
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Preconditioning: Increment not a constant.\n");
        return 0;
    }
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Preconditioning: Increment not a constant power of 2.\n");
        return 0;
    }
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Preconditioning: EQ comparison loop.\n");
        return 0;
    }
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Preconditioning: Final value not invariant.\n");
        return 0;
    }
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Preconditioning: Floating point final or initial value.\n");
        return 0;
    }
    ...
    if (uid_luid[REGNO_FIRST_UID(REGNO(loop_info->iteration_var))] > INSN_LUID(loop_start))
    {
        if (loop_dump_stream)
            fprintf(
                loop_dump_stream, "Preconditioning: Iteration var not live before loop start.\n");
        return 0;
    }
    ...
    *mode = GET_MODE(*final_value);
    if (*mode == VOIDmode)
    {
        *mode = GET_MODE(*initial_value);
        ...
        && (GET_MODE_SIZE(*mode) < GET_MODE_SIZE(GET_MODE(*initial_value))))
        *mode = GET_MODE(*initial_value);
...
static void init_reg_map(struct inline_remap *map, int maxregnum)
{
    int i;
    ...
    map->reg_map[VIRTUAL_STACK_VARS_REGNUM] = regno_reg_rtx[VIRTUAL_STACK_VARS_REGNUM];
    map->reg_map[VIRTUAL_INCOMING_ARGS_REGNUM] = regno_reg_rtx[VIRTUAL_INCOMING_ARGS_REGNUM];
}
...
static rtx calculate_giv_inc(rtx pattern, rtx src_insn, int regno)
{
    rtx increment;
...
retry:
    ...
    if (GET_CODE(increment) != CONST_INT)
    {
       ...
       note to the store insn.  */
        ...
        else if (GET_CODE(increment) == IOR || GET_CODE(increment) == ASHIFT
            || GET_CODE(increment) == PLUS)
        {
            ...
            if (GET_CODE(second_part) != CONST_INT || GET_CODE(increment) != CONST_INT)
                abort();
    ...
    if (GET_CODE(XEXP(SET_SRC(pattern), 0)) != REG || REGNO(XEXP(SET_SRC(pattern), 0)) != regno)
    {
...
static rtx initial_reg_note_copy(rtx notes, struct inline_remap *map)
{
    rtx copy;
...
static void final_reg_note_copy(rtx notes, struct inline_remap *map)
{
    rtx note;
...
static void copy_loop_body(rtx copy_start, rtx copy_end, struct inline_remap *map, rtx exit_label,
    int last_iteration, enum unroll_types unroll_type, rtx start_label, rtx loop_end,
    rtx insert_before, rtx copy_notes_from)
{
    rtx insn, pattern;
    rtx set, tem, copy = 0;
    ...
    do
    {
        ...
        case INSN:
            ...
            if ((set = single_set(insn)) && GET_CODE(SET_DEST(set)) == REG
                ...
                for (tv = bl->giv; tv; tv = tv->next_iv)
                    if (tv->giv_type == DEST_ADDR && tv->same == v)
                    {
                        ...
                        if (tv->mult_val != v->mult_val)
                            this_giv_inc
                                = (this_giv_inc / INTVAL(v->mult_val) * INTVAL(tv->mult_val));
                        ...
                        if (last_iteration && unroll_type != UNROLL_COMPLETELY)
                        {
                            ...
                            if (!tv->same_insn && !tv->shared)
                            {
                                ...
                                /* The constant could be too large for an add
                                   ...
                                   here.  */
                                emit_unrolled_add(dest_reg, XEXP(value, 0), XEXP(value, 1));
                            }
                            ...
                            /* Reset the giv to be just the register again, in case
                               ...
                               above.  */
                            tv->dest_reg = plus_constant(dest_reg, -tv->const_adjust);
                            *tv->location = tv->dest_reg;
                        }
                    }
            }
                ...
                && splittable_regs[REGNO(SET_DEST(set))])
            {
                ...
                if (unroll_type == UNROLL_COMPLETELY)
                {
                    ...
                    splittable_regs[regno]
                        = plus_constant(splittable_regs[src_regno], INTVAL(giv_inc));
                ...
                else
                {
                    ...
                    if (regno < max_reg_before_loop && REG_IV_TYPE(regno) == BASIC_INDUCT)
                    {
		  ...
		  /* If non-reduced/final-value givs were split, then
		     ...
		     find_splittable_regs.  */
                    ...
                    splittable_regs[regno]
                        = GEN_INT(INTVAL(giv_inc) + INTVAL(splittable_regs[src_regno]));
                    ...
                    if (!last_iteration || (splittable_regs_updates[regno]-- != 1))
                    {
                        ...
                        record_base_value(REGNO(tem),
                            giv_inc == const0_rtx
                                ? giv_src_reg
                                : gen_rtx_PLUS(GET_MODE(giv_src_reg), giv_src_reg, giv_inc),
                            1);
                    }
                    else
        ...
        case JUMP_INSN:
            ...
            if (JUMP_LABEL(insn) == start_label && insn == copy_end && !last_iteration)
            {
               ...
               original jump label case to fall_through.  */
                ...
                if (invert_exp(pattern, copy))
                {
                    if (!redirect_exp(&pattern,
                            get_label_from_map(map, CODE_LABEL_NUMBER(JUMP_LABEL(insn))),
                            exit_label, copy))
                        abort();
                }
                else
                {
                    ...
                    LABEL_NUSES(lab) = 0;
                    if (!redirect_exp(&pattern,
                            get_label_from_map(map, CODE_LABEL_NUMBER(JUMP_LABEL(insn))), lab,
                            copy))
                        abort();
                }
            }
            ...
            if (JUMP_LABEL(insn))
            {
               ...
               the backward branch, and hence the label was not mapped.  */
                ...
                else
                {
                    ...
                    JUMP_LABEL(copy) = get_label_from_map(map, CODE_LABEL_NUMBER(JUMP_LABEL(insn)));
                }
        ...
        case CODE_LABEL:
            ...
            if (insn != start_label)
            {
                copy = emit_label(get_label_from_map(map, CODE_LABEL_NUMBER(insn)));
                map->const_age++;
            }
            break;
    ...
    if (!last_iteration)
    {
        ...
        {
            if (GET_CODE(insn) == NOTE && NOTE_LINE_NUMBER(insn) != NOTE_INSN_DELETED)
...
void emit_unrolled_add(rtx dest_reg, rtx src_reg, rtx increment)
{
    rtx result;

    result = expand_binop(
        GET_MODE(dest_reg), add_optab, src_reg, increment, dest_reg, 0, OPTAB_LIB_WIDEN);
...
int back_branch_in_range_p(rtx insn, rtx loop_start, rtx loop_end)
{
    rtx p, q, target_insn;
...
static rtx fold_rtx_mult_add(rtx mult1, rtx mult2, rtx add1, enum machine_mode mode)
{
    rtx temp, mult_res;
    rtx result;
...
rtx biv_total_increment(struct iv_class *bl, rtx loop_start, rtx loop_end)
{
    struct induction *v;
    rtx result;
    ...
    {
        if (v->always_computable && v->mult_val == const1_rtx && !v->maybe_multiple)
...
static void iteration_info(
    rtx iteration_var, rtx *initial_value, rtx *increment, rtx loop_start, rtx loop_end)
{
    struct iv_class *bl;
#if 0
  struct induction *v;
    ...
    *increment = 0;
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Loop unrolling: No reg_iv_type entry for iteration var.\n");
        return;
    }

    /* Reject iteration variables larger than the host wide int size, since they
       could result in a number of iterations greater than the range of our
       `uint32_t' variable loop_info->n_iterations.  */
    else if ((GET_MODE_BITSIZE(GET_MODE(iteration_var)) > 32))
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream,
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Loop unrolling: Iteration var not an integer.\n");
        return;
    }
    ...
    else if (REG_IV_TYPE(REGNO(iteration_var)) == GENERAL_INDUCT)
    {
       ...
       to handle this.  */
        ...
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Loop unrolling: Giv iterators are not handled.\n");
        return;
#else
       ...
       add_val.  Only useful if it is a constant.  */
        ...
        bl = reg_biv_class[REGNO(v->src_reg)];
        *initial_value = fold_rtx_mult_add(v->mult_val, bl->initial_value, v->add_val, v->mode);

        /* Increment value is mult_val times the increment value of the biv.  */

        *increment = biv_total_increment(bl, loop_start, loop_end);
        if (*increment)
            *increment = fold_rtx_mult_add(v->mult_val, *increment, const0_rtx, v->mode);
#endif
    }
    else
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Loop unrolling: Not basic or general induction var.\n");
        return;
    }
}
...
static int find_splittable_regs(enum unroll_types unroll_type, rtx loop_start, rtx loop_end,
    rtx end_insert_before, int unroll_number, uint32_t n_iterations)
{
    struct iv_class *bl;
    struct induction *v;
    rtx increment, tem;
    ...
    for (bl = loop_iv_list; bl; bl = bl->next)
    {
       ...
       a fall through at the end.  */
        ...
        if (unroll_type != UNROLL_COMPLETELY
            && (loop_number_exit_count[uid_loop_num[INSN_UID(loop_start)]]
                   || unroll_type == UNROLL_NAIVE)
            && (uid_luid[REGNO_LAST_UID(bl->regno)] >= INSN_LUID(loop_end) || !bl->init_insn
                   || INSN_UID(bl->init_insn) >= max_uid_for_loop
                   || (uid_luid[REGNO_FIRST_UID(bl->regno)] < INSN_LUID(bl->init_insn))
                   || reg_mentioned_p(bl->biv->dest_reg, SET_SRC(bl->init_set)))
            && !(biv_final_value = final_biv_value(bl, loop_start, loop_end, n_iterations)))
            biv_splittable = 0;

        /* If any of the insns setting the BIV don't do so with a simple
       PLUS, we don't know how to split it.  */
        for (v = bl->biv; biv_splittable && v; v = v->next_iv)
            if ((tem = single_set(v->insn)) == 0 || GET_CODE(SET_DEST(tem)) != REG
                || REGNO(SET_DEST(tem)) != bl->regno || GET_CODE(SET_SRC(tem)) != PLUS)
                biv_splittable = 0;
       ...
       the biv's initial value.  Otherwise, save the constant zero.  */
        ...
        if (biv_splittable == 1)
        {
            if (unroll_type == UNROLL_COMPLETELY)
            {
               ...
               pseudo reg to hold the initial value of the biv.  */
                           ...
                           || !invariant_p(bl->initial_value)))
                {
                    ...
                    emit_insn_before(gen_move_insn(tem, bl->biv->src_reg), loop_start);
            ...
            if (loop_dump_stream)
                fprintf(loop_dump_stream, "Biv %d safe to split.\n", bl->regno);
        }
       ...
       loop, and the givs aren't.  */
        ...
        result += find_splittable_givs(
            bl, unroll_type, loop_start, loop_end, increment, unroll_number);
       ...
       biv's value in their initialization code.  */
        if (biv_final_value)
        {
            ...
            if (!loop_number_exit_count[uid_loop_num[INSN_UID(loop_start)]])
                emit_insn_before(
                    gen_move_insn(bl->biv->src_reg, biv_final_value), end_insert_before);
            else
            {
               ...
               exits.  */
                ...
                emit_insn_before(gen_move_insn(tem, bl->biv->src_reg), loop_start);
                emit_insn_before(gen_move_insn(bl->biv->src_reg, biv_final_value), loop_start);
...
static int verify_addresses(struct induction *v, rtx giv_inc, int unroll_number)
{
    int ret = 1;
    rtx orig_addr = *v->location;
    rtx last_addr = plus_constant(v->dest_reg, INTVAL(giv_inc) * (unroll_number - 1));
...
static int find_splittable_givs(struct iv_class *bl, enum unroll_types unroll_type, rtx loop_start,
    rtx loop_end, rtx increment, int unroll_number)
{
    ...
    for (v = bl->giv; v; v = v->next_iv)
        for (v2 = v->next_iv; v2; v2 = v2->next_iv)
            if (v->insn == v2->insn && rtx_equal_p(v->new_reg, v2->new_reg) && !v2->same_insn)
                v2->same_insn = v;
    ...
    for (v = bl->giv; v; v = v->next_iv)
    {
       ...
       won't reach here if they aren't.  */
        if (v->giv_type != DEST_ADDR
            && (!v->always_computable || back_branch_in_range_p(v->insn, loop_start, loop_end)))
            continue;

        /* The giv increment value must be a constant.  */
        giv_inc = fold_rtx_mult_add(v->mult_val, increment, const0_rtx, v->mode);
       ...
       DEST_ADDR giv.  */
        ...
        if (unroll_type != UNROLL_COMPLETELY
            ...
            && (REGNO(v->dest_reg) >= max_reg_before_loop
                   ...
                   || (uid_luid[REGNO_LAST_UID(REGNO(v->dest_reg))] >= INSN_LUID(loop_end)))
            /* Givs made from biv increments are missed by the above test, so
               test explicitly for them.  */
            && (REGNO(v->dest_reg) < first_increment_giv || REGNO(v->dest_reg) > last_increment_giv)
	  ...
	  v->src_reg = tem;
       ...
       giv's initial value.  Otherwise, save the constant zero for it.  */
        ...
        if (unroll_type == UNROLL_COMPLETELY)
        {
            ...
            value = fold_rtx_mult_add(v->mult_val, biv_initial_value, v->add_val, v->mode);
        }
        else
            value = const0_rtx;
        ...
        if (v->new_reg)
        {
            ...
            {
                if (loop_dump_stream)
                    fprintf(loop_dump_stream, "giv combined with unreduced giv not split.\n");
                continue;
            }
            ...
            else if (v->giv_type == DEST_REG)
            {
               ...
               is going before the loop start.  */
                if (unroll_type == UNROLL_COMPLETELY && GET_CODE(value) != CONST_INT
                    && GET_CODE(value) != REG
                    && (GET_CODE(value) != PLUS || GET_CODE(XEXP(value, 0)) != REG
                           || GET_CODE(XEXP(value, 1)) != CONST_INT))
                {
                    rtx tem = gen_reg_rtx(v->mode);
                    record_base_value(REGNO(tem), v->add_val, 0);
                    emit_iv_add_mult(bl->initial_value, v->mult_val, v->add_val, tem, loop_start);
                    value = tem;
                }
            ...
            else
            {
               ...
               each.  */
                else if (unroll_type != UNROLL_COMPLETELY && v->giv_type == DEST_ADDR && v->same
                    && v->same->giv_type == DEST_ADDR
                    && v->same->unrolled
                ...
                else if (unroll_type != UNROLL_COMPLETELY)
                {
                    ...
                    if (same && same->derived_from)
                    {
                       ...
                       about same->derived_from being in turn derived.  */
                        ...
                        new_reg = replace_rtx(new_reg, same->dest_reg, same->new_reg);
                    }
                    ...
                    {
                        v->dest_reg = plus_constant(tem, INTVAL(XEXP(new_reg, 1)));
                    ...
                    if (v->dest_reg == tem && !verify_addresses(v, giv_inc, unroll_number))
                    {
                        for (v2 = v->next_iv; v2; v2 = v2->next_iv)
                            if (v2->same_insn == v)
                                v2->same_insn = 0;
                    ...
                    emit_insn_before(gen_rtx_SET(VOIDmode, tem, copy_rtx(v->new_reg)), loop_start);
                    if (recog_memoized(PREV_INSN(loop_start)) < 0)
                    {
                       ...
                       force_operand instead of emit_iv_add_mult.  */
                        ...
                        if (loop_dump_stream)
                            fprintf(loop_dump_stream, "Invalid init insn, rewritten.\n");
                    }
                }
                else
                {
	  ...
	  fprintf (stderr, "Giv %d at insn %d not reduced\n",
       ...
       where this may not be a register.  */
        ...
        if (loop_dump_stream)
        {
            ...
            fprintf(
                loop_dump_stream, "Giv %d at insn %d safe to split.\n", regnum, INSN_UID(v->insn));
        }
    }
...
static int reg_dead_after_loop(rtx reg, rtx loop_start, rtx loop_end)
{
    rtx insn, label;
    enum rtx_code code;
    int jump_count = 0;
    ...
    for (label = loop_number_exit_labels[this_loop_num]; label; label = LABEL_NEXTREF(label))
        label_count++;
...
rtx final_biv_value(struct iv_class *bl, rtx loop_start, rtx loop_end, uint32_t n_iterations)
{
    rtx increment, tem;
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Final biv value for %d, reversed biv.\n", bl->regno);
    ...
    if (n_iterations != 0 && !loop_number_exit_count[uid_loop_num[INSN_UID(loop_start)]]
        && invariant_p(bl->initial_value))
    {
        increment = biv_total_increment(bl, loop_start, loop_end);
        ...
        if (increment && invariant_p(increment))
        {
            ...
            emit_iv_add_mult(
                increment, GEN_INT(n_iterations), bl->initial_value, tem, NEXT_INSN(loop_end));

            if (loop_dump_stream)
                fprintf(loop_dump_stream, "Final biv value for %d, calculated.\n", bl->regno);
    ...
    if (reg_dead_after_loop(bl->biv->src_reg, loop_start, loop_end))
    {
        if (loop_dump_stream)
            fprintf(
                loop_dump_stream, "Final biv value for %d, biv dead after loop exit.\n", bl->regno);
...
rtx final_giv_value(struct induction *v, rtx loop_start, rtx loop_end, uint32_t n_iterations)
{
    struct iv_class *bl;
    rtx insn;
    rtx increment, tem;
    ...
    if (n_iterations != 0 && !loop_number_exit_count[uid_loop_num[INSN_UID(loop_start)]])
    {
       ...
       be valid if it is invariant.  */
        ...
        if (increment && invariant_p(increment) && invariant_p(bl->initial_value))
        {
            ...
            record_base_value(REGNO(tem), bl->biv->add_val, 0);
            emit_iv_add_mult(
                increment, GEN_INT(n_iterations), bl->initial_value, tem, insert_before);

            /* Subtract off extra increments as we find them.  */
            for (insn = NEXT_INSN(v->insn); insn != loop_end; insn = NEXT_INSN(insn))
            {
                ...
                for (biv = bl->biv; biv; biv = biv->next_iv)
                    ...
                    {
                        start_sequence();
                        tem = expand_binop(GET_MODE(tem), sub_optab, tem, biv->add_val, NULL_RTX, 0,
                            OPTAB_LIB_WIDEN);
                        seq = gen_sequence();
                        end_sequence();
...
static rtx loop_find_equiv_value(rtx loop_start, rtx reg)
{
    rtx insn, set;
    rtx ret;
    ...
    for (insn = PREV_INSN(loop_start); insn; insn = PREV_INSN(insn))
    {
        ...
        else if (GET_RTX_CLASS(GET_CODE(insn)) == 'i' && reg_set_p(reg, insn))
        {
            /* We found the last insn before the loop that sets the register.
               If it sets the entire register, and has a REG_EQUAL note,
               then use the value of the REG_EQUAL note.  */
            if ((set = single_set(insn)) && (SET_DEST(set) == reg))
            {
               ...
               problems later if we use them.  */
                if (note && GET_CODE(XEXP(note, 0)) != EXPR_LIST && CONSTANT_P(XEXP(note, 0)))
                    ret = XEXP(note, 0);
                else
                    ret = SET_SRC(set);
            }
...
static rtx subtract_reg_term(rtx op, rtx reg)
{
    if (op == reg)
        return const0_rtx;
    if (GET_CODE(op) == PLUS)
...
static rtx find_common_reg_term(rtx op0, rtx op1)
{
...
uint32_t
loop_iterations(rtx loop_start, rtx loop_end, struct loop_info *loop_info)
{
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Loop iterations: No final conditional branch found.\n");
        return 0;
    }
    ...
    do
        vtop = PREV_INSN(vtop);
    while (GET_CODE(vtop) != NOTE || NOTE_LINE_NUMBER(vtop) > 0
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Loop iterations: Comparison not against register.\n");
        return 0;
    }
       ...
       where reg1 != reg2.  */
               ...
               the latter case for now since it is not so common.  */
                ...
                if (temp == reg1)
                    final_value = (const2 == const0_rtx)
                        ? reg1
                        : gen_rtx_PLUS(GET_MODE(reg1), reg1, const2);
            }
        }
        else if (loop_info->vtop && GET_CODE(reg2) == CONST_INT)
        {
            ...
            if (GET_CODE(temp) == MINUS && REG_P(XEXP(temp, 0)))
            {
                ...
                if (GET_CODE(temp2) == PLUS && XEXP(temp2, 0) == XEXP(temp, 1))
    ...
    if (loop_info->vtop && (reg_term = find_common_reg_term(initial_value, final_value)))
    {
    ...
    {
        if (loop_dump_stream)
            fprintf(loop_dump_stream, "Loop iterations: Increment value can't be calculated.\n");
        return 0;
    }
    ...
    if (GET_CODE(increment) != CONST_INT)
    {
        ...
        if (GET_CODE(increment) != CONST_INT)
        {
            if (loop_dump_stream)
            {
                fprintf(loop_dump_stream, "Loop iterations: Increment value not constant ");
    ...
    if (GET_CODE(initial_value) != CONST_INT)
    {
        if (loop_dump_stream)
        {
            fprintf(loop_dump_stream, "Loop iterations: Initial value not constant ");
    ...
    else if (GET_CODE(final_value) != CONST_INT)
    {
        if (loop_dump_stream)
        {
            fprintf(loop_dump_stream, "Loop iterations: Final value not constant ");
    ...
    if (unsigned_p)
        final_larger = ((uint32_t)INTVAL(final_value) > (uint32_t)INTVAL(initial_value))
            - ((uint32_t)INTVAL(final_value) < (uint32_t)INTVAL(initial_value));
    else
...
#endif
        if (REGNO(x) < max_reg_before_loop && REG_IV_TYPE(REGNO(x)) == BASIC_INDUCT)
...
int set_dominates_use(int regno, int first_uid, int last_uid, rtx copy_start, rtx copy_end)
{
    int passed_jump = 0;
    ...
    if (GET_RTX_CLASS(GET_CODE(p)) != 'i' || !dead_or_set_regno_p(p, regno))
        return 0;
