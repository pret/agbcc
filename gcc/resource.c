...
static rtx find_dead_or_set_registers(
    rtx, struct resources *, rtx *, int, struct resources, struct resources);

/* Utility function called from mark_target_live_regs via note_stores.
   It deadens any CLOBBERed registers and livens any SET registers.  */

static void update_live_status(rtx dest, rtx x)
{
    int first_regno, last_regno;
    int i;

    if (GET_CODE(dest) != REG && (GET_CODE(dest) != SUBREG || GET_CODE(SUBREG_REG(dest)) != REG))
        return;
...
static int find_basic_block(rtx insn)
{
    int i;
    ...
    for (insn = prev_nonnote_insn(insn); insn && GET_CODE(insn) != BARRIER;
    ...
    for (insn = next_nonnote_insn(insn); insn && GET_CODE(insn) == CODE_LABEL;
...
static rtx next_insn_no_annul(rtx insn)
{
    if (insn)
    {
        /* If INSN is an annulled branch, skip any insns from the target
       of the branch.  */
        if (INSN_ANNULLED_BRANCH_P(insn) && NEXT_INSN(PREV_INSN(insn)) != insn)
...
void mark_referenced_resources(
    register rtx x, register struct resources *res, register int include_delayed_effects)
{
    ...
    case CALL_INSN:
        if (include_delayed_effects)
        {
            ...
            if (next && GET_CODE(next) == NOTE && NOTE_LINE_NUMBER(next) == NOTE_INSN_SETJMP)
                    ...
                    if (GET_CODE(XEXP(link, 0)) == USE)
                    {
                        for (i = 1; i < seq_size; i++)
                        {
                            ...
                            if (GET_CODE(slot_pat) == SET
                                && rtx_equal_p(SET_DEST(slot_pat), SET_DEST(XEXP(link, 0))))
                                break;
                        }
                        if (i >= seq_size)
                            mark_referenced_resources(SET_DEST(XEXP(link, 0)), res, 0);
                    }
            }
        }
    ...
    for (i = 0; i < GET_RTX_LENGTH(code); i++)
        ...
        case 'E':
            for (j = 0; j < XVECLEN(x, i); j++)
                mark_referenced_resources(XVECEXP(x, i, j), res, include_delayed_effects);
            break;
        }
}
...
static rtx find_dead_or_set_registers(rtx target, struct resources *res, rtx *jump_target,
    int jump_count, struct resources set, struct resources needed)
{
            ...
            if (jump_count++ < 10)
            {
                ...
                else if (condjump_p(this_jump_insn) || condjump_in_parallel_p(this_jump_insn))
                {
                    ...
                    find_dead_or_set_registers(
                        JUMP_LABEL(this_jump_insn), &target_res, 0, jump_count, target_set, needed);
...
void mark_set_resources(
    register rtx x, register struct resources *res, int in_dest, int include_delayed_effects)
{
...
restart:
    ...
    case SET:
       ...
       effects of the calling routine.  */
        ...
        mark_set_resources(
            SET_DEST(x), res, (include_delayed_effects || GET_CODE(SET_SRC(x)) != CALL), 0);
    ...
    case SEQUENCE:
        for (i = 0; i < XVECLEN(x, 0); i++)
            if (!(INSN_ANNULLED_BRANCH_P(XVECEXP(x, 0, 0)) && INSN_FROM_TARGET_P(XVECEXP(x, 0, i))))
                mark_set_resources(XVECEXP(x, 0, i), res, 0, include_delayed_effects);
        return;
    ...
    case SUBREG:
        ...
        {
            if (GET_CODE(SUBREG_REG(x)) != REG)
                mark_set_resources(SUBREG_REG(x), res, in_dest, include_delayed_effects);
            else
            {
                int regno = REGNO(SUBREG_REG(x)) + SUBREG_WORD(x);
                int last_regno = regno + HARD_REGNO_NREGS(regno, GET_MODE(x));
    ...
    for (i = 0; i < GET_RTX_LENGTH(code); i++)
        ...
        case 'E':
            for (j = 0; j < XVECLEN(x, i); j++)
                mark_set_resources(XVECEXP(x, i, j), res, in_dest, include_delayed_effects);
            break;
        }
}
...
void mark_target_live_regs(rtx insns, rtx target, struct resources *res)
{
    int b = -1;
    int i;
    ...
    {
        for (tinfo = target_hash_table[INSN_UID(target) % TARGET_HASH_PRIME]; tinfo;
             tinfo = tinfo->next)
       ...
       start of the basic block has been deleted.  */
        if (tinfo && tinfo->block != -1 && !INSN_DELETED_P(BLOCK_HEAD(tinfo->block)))
            b = tinfo->block;
    }
    ...
    if (b != -1)
    {
       ...
       hard regs.  */
        ...
        EXECUTE_IF_SET_IN_REG_SET(regs_live, FIRST_PSEUDO_REGISTER, i, {
            if ((regno = reg_renumber[i]) >= 0)
                for (j = regno; j < regno + HARD_REGNO_NREGS(regno, PSEUDO_REGNO_MODE(i)); j++)
                ...
                || GET_CODE(real_insn) == JUMP_INSN || GET_CODE(real_insn) == CALL_INSN)
            {
                for (link = REG_NOTES(real_insn); link; link = XEXP(link, 1))
                        ...
                        && REGNO(XEXP(link, 0)) < FIRST_PSEUDO_REGISTER)
                    {
                        ...
                        int last_regno = (first_regno
                            + HARD_REGNO_NREGS(first_regno, GET_MODE(XEXP(link, 0))));
    ...
    if (jump_insn)
    {
        ...
        mark_target_live_regs(insns, next_active_insn(jump_target), &new_resources);
        CLEAR_RESOURCE(&set);
        CLEAR_RESOURCE(&needed);
...
void init_resource_info(rtx epilogue_insn)
{
    int i;
    ...
    target_hash_table
        = (struct target_info **)xmalloc((TARGET_HASH_PRIME * sizeof(struct target_info *)));
    zero_memory((char *)target_hash_table, TARGET_HASH_PRIME * sizeof(struct target_info *));
...
void clear_hashed_info_for_insn(rtx insn)
{
    struct target_info *tinfo;

    if (target_hash_table != NULL)
    {
        for (tinfo = target_hash_table[INSN_UID(insn) % TARGET_HASH_PRIME]; tinfo;
             tinfo = tinfo->next)
...
void incr_ticks_for_insn(rtx insn)
{
...
/* Add TRIAL to the set of resources used at the end of the current
   function. */
void mark_end_of_function_resources(rtx trial, int include_delayed_effects)
{
    mark_referenced_resources(trial, &end_of_function_needs, include_delayed_effects);
}
...
rtx find_free_register(rtx current_insn, char *class_str, int mode, HARD_REG_SET *reg_set)
{
    int i, j;
    struct resources used;
    unsigned char clet = class_str[0];
    enum reg_class class = (clet == 'r' ? GENERAL_REGS : REG_CLASS_FROM_LETTER(clet));
    ...
    for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
    {
        ...
        {
            if (TEST_HARD_REG_BIT(*reg_set, i + j) || TEST_HARD_REG_BIT(used.regs, i + j))
            {
                success = 0;
                break;
            }
