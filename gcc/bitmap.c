...
static inline bitmap_element *bitmap_element_allocate(void)
{
...
static inline void bitmap_element_link(bitmap head, bitmap_element *element)
{
    ...
    {
        for (ptr = head->current; ptr->prev != 0 && ptr->prev->indx > indx; ptr = ptr->prev)
            ;
    ...
    {
        for (ptr = head->current; ptr->next != 0 && ptr->next->indx < indx; ptr = ptr->next)
            ;
...
static inline bitmap_element *bitmap_find_bit(bitmap head, unsigned int bit)
{
    bitmap_element *element;
    uint32_t indx = bit / BITMAP_ELEMENT_ALL_BITS;

    if (head->current == 0)
        return 0;

    if (head->indx > indx)
        for (element = head->current; element->prev != 0 && element->indx > indx;
             element = element->prev)
            ;

    else
        for (element = head->current; element->next != 0 && element->indx < indx;
...
void bitmap_clear_bit(bitmap head, int bit)
{
    ...
    if (ptr != 0)
    {
        unsigned bit_num = bit % (unsigned)32;
        unsigned word_num = ((bit / (unsigned)32) % BITMAP_ELEMENT_WORDS);
        ptr->bits[word_num] &= ~(((uint32_t)1) << bit_num);
...
void bitmap_set_bit(bitmap head, int bit)
{
    ...
    unsigned word_num = ((bit / (unsigned)32) % BITMAP_ELEMENT_WORDS);
    unsigned bit_num = bit % (unsigned)32;
    uint32_t bit_val = ((uint32_t)1) << bit_num;
...
int bitmap_bit_p(bitmap head, int bit)
{
    ...
    bit_num = bit % (unsigned)32;
    word_num = ((bit / (unsigned)32) % BITMAP_ELEMENT_WORDS);

    return (ptr->bits[word_num] & (((uint32_t)1) << bit_num)) != 0;
}

/* Store in bitmap TO the result of combining bitmap FROM1 and
   FROM2 using a specific bit manipulation.  */

void bitmap_operation(bitmap to, bitmap from1, bitmap from2, enum bitmap_bits operation)
{
    bitmap_element *delete_list = 0;
    bitmap_element *from1_ptr = from1->first;
    bitmap_element *from2_ptr = from2->first;
    unsigned int indx1 = (from1_ptr) ? from1_ptr->indx : ~(uint32_t)0;
    unsigned int indx2 = (from2_ptr) ? from2_ptr->indx : ~(uint32_t)0;
    ...
    while (from1_ptr != 0 || from2_ptr != 0)
    {
       ...
       missing links.  */
        if (indx1 == indx2)
        {
            ...
            indx1 = (from1_ptr) ? from1_ptr->indx : ~(uint32_t)0;
            from2_ptr = from2_ptr->next;
            indx2 = (from2_ptr) ? from2_ptr->indx : ~(uint32_t)0;
        }
        else if (indx1 < indx2)
        {
            indx = indx1;
...
void bitmap_release_memory(void)
{
    bitmap_free = 0;
    if (bitmap_obstack_init)
    {
