...
rtx gen_rtx_CONST_INT(enum machine_mode mode, int32_t arg)
{
...
rtx gen_rtx_REG(enum machine_mode mode, int regno)
{
...
rtx gen_rtx(enum rtx_code code, enum machine_mode mode, ...)
{
    ...
    if (code == CONST_INT)
        rt_val = gen_rtx_CONST_INT(mode, va_arg(p, int32_t));
    else if (code == REG)
...
rtx gen_reg_rtx(enum machine_mode mode)
{
    ...
    if (GET_MODE_CLASS(mode) == MODE_COMPLEX_FLOAT || GET_MODE_CLASS(mode) == MODE_COMPLEX_INT)
    {
       ...
       pseudos overstrains reload on some machines like the 386.  */
        ...
        enum machine_mode partmode = mode_for_size(size * BITS_PER_UNIT,
            (GET_MODE_CLASS(mode) == MODE_COMPLEX_FLOAT ? MODE_FLOAT : MODE_INT),
            0);
    ...
    if (reg_rtx_no == regno_pointer_flag_length)
    {
        ...
        zero_memory(
            (char *)&new1[regno_pointer_flag_length], regno_pointer_flag_length * sizeof(rtx));
        regno_reg_rtx = new1;
...
rtx gen_lowpart_common(enum machine_mode mode, rtx x)
{
    ...
    else if (GET_CODE(x) == REG)
    {
       ...
       notably Sparc.  */
        if (REGNO(x) < FIRST_PSEUDO_REGISTER && !HARD_REGNO_MODE_OK(REGNO(x) + word, mode)
            && HARD_REGNO_MODE_OK(REGNO(x), GET_MODE(x)))
            return 0;
        else if (REGNO(x) < FIRST_PSEUDO_REGISTER
            /* integrate.c can't handle parts of a return value register. */
            && (!REG_FUNCTION_VALUE_P(x) || !rtx_equal_function_value_matters)
#ifdef CLASS_CANNOT_CHANGE_SIZE
            && !(GET_MODE_SIZE(mode) != GET_MODE_SIZE(GET_MODE(x))
                   && GET_MODE_CLASS(GET_MODE(x)) != MODE_COMPLEX_INT
                   && GET_MODE_CLASS(GET_MODE(x)) != MODE_COMPLEX_FLOAT
                   && (TEST_HARD_REG_BIT(
                          reg_class_contents[(int)CLASS_CANNOT_CHANGE_SIZE], REGNO(x))))
#endif
            /* We want to keep the stack, frame, and arg pointers
           special.  */
            && x != frame_pointer_rtx && x != arg_pointer_rtx && x != stack_pointer_rtx)
        ...
        && (GET_CODE(x) == CONST_INT || GET_CODE(x) == CONST_DOUBLE))
    {
       ...
       for this mode.  */
        ...
        if (GET_MODE_BITSIZE(mode) >= 2 * 32)
            return x;
        else if (GET_MODE_BITSIZE(mode) > 32)
            return 0;
        else if (GET_MODE_BITSIZE(mode) == 32)
            return (GET_CODE(x) == CONST_INT ? x : GEN_INT(CONST_DOUBLE_LOW(x)));
        else
        {
            /* MODE must be narrower than 32.  */
            int width = GET_MODE_BITSIZE(mode);
            int32_t val = (GET_CODE(x) == CONST_INT ? INTVAL(x) : CONST_DOUBLE_LOW(x));

            /* Sign extend to int32_t.  */
            val = val << (32 - width) >> (32 - width);

            return (GET_CODE(x) == CONST_INT && INTVAL(x) == val ? x : GEN_INT(val));
        }
    }
    ...
    else if (((HOST_FLOAT_FORMAT == TARGET_FLOAT_FORMAT && 32 == BITS_PER_WORD)
                 || flag_pretend_float)
        && GET_MODE_CLASS(mode) == MODE_FLOAT && GET_MODE_SIZE(mode) == UNITS_PER_WORD
...
#else
    {
        ...
        return CONST_DOUBLE_FROM_REAL_VALUE(u.d, mode);
                 ...
                 || flag_pretend_float)
        && GET_MODE_CLASS(mode) == MODE_FLOAT && GET_MODE_SIZE(mode) == 2 * UNITS_PER_WORD
    ...
    /* We need an extra case for machines where 32 is the
    ...
    /* Similarly, if this is converting a floating-point value into a
       ...
       compatible.  */
                 ...
                 || flag_pretend_float)
        && GET_MODE_CLASS(mode) == MODE_INT && GET_CODE(x) == CONST_DOUBLE
        && GET_MODE_CLASS(GET_MODE(x)) == MODE_FLOAT && GET_MODE_BITSIZE(mode) == BITS_PER_WORD)
    ...
    /* Similarly, if this is converting a floating-point value into a
       ...
       compatible.  */
                 ...
                 || flag_pretend_float)
        && GET_MODE_CLASS(mode) == MODE_INT && GET_CODE(x) == CONST_DOUBLE
        && GET_MODE_CLASS(GET_MODE(x)) == MODE_FLOAT && GET_MODE_BITSIZE(mode) == 2 * BITS_PER_WORD)
    {
        rtx lowpart = operand_subword(x, word, 0, GET_MODE(x));
        rtx highpart = operand_subword(x, word + 1, 0, GET_MODE(x));

        if (lowpart && GET_CODE(lowpart) == CONST_INT && highpart
            && GET_CODE(highpart) == CONST_INT)
...
rtx gen_highpart(enum machine_mode mode, rtx x)
{
    ...
    else if (GET_CODE(x) == CONST_INT)
    {
        if (32 <= BITS_PER_WORD)
            return const0_rtx;
        return GEN_INT(INTVAL(x) >> (32 - BITS_PER_WORD));
    }
    else if (GET_CODE(x) == MEM)
    {
        int offset = 0;
        ...
        if (GET_MODE_SIZE(mode) < UNITS_PER_WORD)
            offset -= (GET_MODE_SIZE(mode) - MIN(UNITS_PER_WORD, GET_MODE_SIZE(GET_MODE(x))));
    ...
    else if (GET_CODE(x) == REG)
    {
       ...
       to always interpret the subreg offset parameter as bytes or bits.  */
        ...
        if (REGNO(x) < FIRST_PSEUDO_REGISTER)
            word = (HARD_REGNO_NREGS(REGNO(x), GET_MODE(x)) - HARD_REGNO_NREGS(REGNO(x), mode));
        else
            word = ((GET_MODE_SIZE(GET_MODE(x)) - MAX(GET_MODE_SIZE(mode), UNITS_PER_WORD))
                / UNITS_PER_WORD);
...
rtx operand_subword(rtx op, int i, int validate_address, enum machine_mode mode)
{
    int32_t val;
    int size_ratio = 32 / BITS_PER_WORD;
    ...
    if (mode != BLKmode && (i + 1) * UNITS_PER_WORD > GET_MODE_SIZE(mode))
        return const0_rtx;
    ...
    if (GET_CODE(op) == REG)
    {
        /* If the register is not valid for MODE, return 0.  If we don't
       do this, there is no way to fix up the resulting REG later.  */
        if (REGNO(op) < FIRST_PSEUDO_REGISTER && !HARD_REGNO_MODE_OK(REGNO(op) + i, word_mode))
            return 0;
        else if (REGNO(op) >= FIRST_PSEUDO_REGISTER
            || (REG_FUNCTION_VALUE_P(op) && rtx_equal_function_value_matters)
            /* We want to keep the stack, frame, and arg pointers
           special.  */
            || op == frame_pointer_rtx || op == arg_pointer_rtx || op == stack_pointer_rtx)
    ...
    else if (GET_CODE(op) == CONCAT)
    {
        ...
        return operand_subword(XEXP(op, 1), i - partwords, validate_address, mode);
    }
    ...
    val = (i / size_ratio == 0
            ...
            : (GET_CODE(op) == CONST_INT ? (INTVAL(op) < 0 ? ~0 : 0) : CONST_DOUBLE_HIGH(op)));
    ...
    if (BITS_PER_WORD < 32
        && ((val & ((int32_t)(-1) << (bits_per_word - 1)))
               != ((int32_t)(-1) << (bits_per_word - 1))))
        val &= ((int32_t)1 << bits_per_word) - 1;
    ...
    if (BITS_PER_WORD < 32 && (val & ((int32_t)1 << (bits_per_word - 1))))
        val |= ((int32_t)(-1) << bits_per_word);
...
rtx operand_subword_force(rtx op, int i, enum machine_mode mode)
{
...
{
    rtx header
        = gen_rtx_INLINE_HEADER(VOIDmode, cur_insn_uid++, NULL_RTX, first_insn, first_parm_insn,
            first_labelno, last_labelno, max_parm_regnum, max_regnum, args_size, pops_args,
            stack_slots, forced_labels, function_flags, outgoing_args_size, original_arg_vector,
            original_decl_initial, regno_rtx, regno_flag, regno_align, parm_reg_stack_loc);
    return header;
}
...
void set_new_first_and_last_label_num(int first, int last)
{
...
void save_emit_status(struct function *p)
{
...
void restore_emit_status(struct function *p)
{
    int i;
...
rtx copy_rtx_if_shared(rtx orig)
{
    ...
    case MEM:
       ...
       or is a constant plus one of the special registers.  */
        if (CONSTANT_ADDRESS_P(XEXP(x, 0)) || XEXP(x, 0) == virtual_stack_vars_rtx
    ...
    if (x->used)
    {
        ...
        copy_memory((char *)x, (char *)copy,
            (sizeof(*copy) - sizeof(copy->fld) + sizeof(copy->fld[0]) * GET_RTX_LENGTH(code)));
        x = copy;
        copied = 1;
    }
    x->used = 1;
...
rtx next_active_insn(rtx insn)
{
    ...
    {
        insn = NEXT_INSN(insn);
        if (insn == 0 || GET_CODE(insn) == CALL_INSN || GET_CODE(insn) == JUMP_INSN
            || (GET_CODE(insn) == INSN
                   && (!reload_completed
...
rtx next_cc0_user(rtx insn)
{
    ...
    if (insn && GET_RTX_CLASS(GET_CODE(insn)) == 'i' && reg_mentioned_p(cc0_rtx, PATTERN(insn)))
        return insn;
       ...
       it, in turn, will be split (SFmode on the 29k is an example).  */
        if (GET_CODE(seq) == SEQUENCE)
        {
            ...
            for (tem = NEXT_INSN(before); tem != after; tem = NEXT_INSN(tem))
                if (!INSN_DELETED_P(tem) && GET_RTX_CLASS(GET_CODE(tem)) == 'i')
...
void emit_insn_after_with_line_notes(rtx pattern, rtx after, rtx from)
{
    ...
    if (after_line)
        emit_line_note_after(NOTE_SOURCE_FILE(after_line), NOTE_LINE_NUMBER(after_line), insn);
}
...
rtx emit_insns_before(rtx insn, rtx before)
{
    rtx last = 0;
...
rtx emit_note(const char *file, int line)
{
    ...
    if (line > 0)
    {
        if (file && last_filename && !strcmp(file, last_filename) && line == last_linenum)
            return 0;
        last_filename = file;
        last_linenum = line;
    }
    ...
    if (GET_CODE(x) == PARALLEL)
    {
        ...
        for (j = XVECLEN(x, 0) - 1; j >= 0; j--)
            if (GET_CODE(XVECEXP(x, 0, j)) == CALL)
                return CALL_INSN;
            else if (GET_CODE(XVECEXP(x, 0, j)) == SET && SET_DEST(XVECEXP(x, 0, j)) == pc_rtx)
                return JUMP_INSN;
...
void start_sequence_for_rtl_expr(tree t)
{
    start_sequence();
...
rtx gen_sequence(void)
{
    ...
    if (len == 1 && !RTX_FRAME_RELATED_P(first_insn)
...
void init_emit(void)
{
    ...
    regno_pointer_flag = (char *)savealloc(regno_pointer_flag_length);
    zero_memory(regno_pointer_flag, regno_pointer_flag_length);

    regno_pointer_align = (char *)savealloc(regno_pointer_flag_length);
    ...
    REGNO_POINTER_ALIGN(VIRTUAL_INCOMING_ARGS_REGNUM) = STACK_BOUNDARY / BITS_PER_UNIT;
    REGNO_POINTER_ALIGN(VIRTUAL_STACK_VARS_REGNUM) = STACK_BOUNDARY / BITS_PER_UNIT;
    REGNO_POINTER_ALIGN(VIRTUAL_STACK_DYNAMIC_REGNUM) = STACK_BOUNDARY / BITS_PER_UNIT;
    REGNO_POINTER_ALIGN(VIRTUAL_OUTGOING_ARGS_REGNUM) = STACK_BOUNDARY / BITS_PER_UNIT;
...
void init_emit_once(int line_numbers)
{
    ...
    {
        if (GET_MODE_BITSIZE(mode) == BITS_PER_UNIT && byte_mode == VOIDmode)
            byte_mode = mode;

        if (GET_MODE_BITSIZE(mode) == BITS_PER_WORD && word_mode == VOIDmode)
            word_mode = mode;
    }
...
#define DOUBLE_TYPE_SIZE (BITS_PER_WORD * 2)
    ...
    for (mode = GET_CLASS_NARROWEST_MODE(MODE_FLOAT); mode != VOIDmode;
         mode = GET_MODE_WIDER_MODE(mode))
    {
        if (GET_MODE_BITSIZE(mode) == DOUBLE_TYPE_SIZE && double_mode == VOIDmode)
            double_mode = mode;
    }
...
#ifdef RETURN_ADDRESS_POINTER_REGNUM
    return_address_pointer_rtx = gen_rtx_raw_REG(Pmode, RETURN_ADDRESS_POINTER_REGNUM);
#endif
    ...
    struct_value_incoming_rtx = gen_rtx_REG(Pmode, STRUCT_VALUE_INCOMING_REGNUM);
#else
