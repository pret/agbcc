...
static void output_epilogue(void)
{
...

    {
        ...
        for (d = insn_data; d; d = d->next)
        {
            ...
            for (i = 0; i < d->n_operands; i++)
                printf(
                    " %s,", ((d->predicates[i] && d->predicates[i][0]) ? d->predicates[i] : "0"));
            if (d->n_operands == 0)
                printf(" 0");
            printf(" },\n");
        }
    ...
    case MATCH_OPERAND:
        ...
        if (max_opno >= MAX_MAX_OPERANDS)
        {
            error("Too many operands (%d) in definition %s.\n", max_opno + 1,
                name_for_index(next_index_number));
            return;
        }
        if (seen[opno])
    ...
    for (start = 0; start < d->n_operands; start++)
        if (d->op_n_alternatives[start] > 0)
        {
            if (n == 0)
                ...
                error("wrong number of alternatives in operand %d of insn %s", start,
                    name_for_index(d->index_number));
        }
...
int main(int argc, char **argv)
{
    ...
    fflush(stdout);
    exit(ferror(stdout) != 0 || have_error ? EXIT_FAILURE : EXIT_SUCCESS);
