...
struct decomposition
{
    ...
    int32_t start; /* Starting offset or register number.  */
    int32_t end;   /* Ending offset or register number.  */
};
...
#define MERGABLE_RELOADS(when1, when2, op1, op2)                                                   \
    ((when1) == RELOAD_OTHER || (when2) == RELOAD_OTHER || ((when1) == (when2) && (op1) == (op2))  \
        || ((when1) == RELOAD_FOR_INPUT && (when2) == RELOAD_FOR_INPUT)                            \
        || ((when1) == RELOAD_FOR_OPERAND_ADDRESS && (when2) == RELOAD_FOR_OPERAND_ADDRESS)        \
        || ((when1) == RELOAD_FOR_OTHER_ADDRESS && (when2) == RELOAD_FOR_OTHER_ADDRESS))

/* Nonzero if these two reload purposes produce RELOAD_OTHER when merged.  */
#define MERGE_TO_OTHER(when1, when2, op1, op2)                                                     \
    ((when1) != (when2)                                                                            \
        || !((op1) == (op2) || (when1) == RELOAD_FOR_INPUT                                         \
               || (when1) == RELOAD_FOR_OPERAND_ADDRESS || (when1) == RELOAD_FOR_OTHER_ADDRESS))
...
#define ADDR_TYPE(type)                                                                            \
    ((type) == RELOAD_FOR_INPUT_ADDRESS                                                            \
            ...
            : ((type) == RELOAD_FOR_OUTPUT_ADDRESS ? RELOAD_FOR_OUTADDR_ADDRESS : (type)))

#ifdef HAVE_SECONDARY_RELOADS
#endif
static enum reg_class find_valid_class(enum machine_mode, int);
static int push_reload(rtx, rtx, rtx *, rtx *, enum reg_class, enum machine_mode, enum machine_mode, int, int, int, enum reload_type);
...
static int push_secondary_reload(int in_p, rtx x, int opnum, int optional,
    enum reg_class reload_class, enum machine_mode reload_mode, enum reload_type type,
    enum insn_code *picode)
{
    ...
    if (type == RELOAD_FOR_INPUT_ADDRESS || type == RELOAD_FOR_OUTPUT_ADDRESS
        || type == RELOAD_FOR_INPADDR_ADDRESS || type == RELOAD_FOR_OUTADDR_ADDRESS)
        secondary_type = type;
    else
    ...
    *picode = CODE_FOR_nothing;
    ...
    if (GET_CODE(x) == REG && REGNO(x) >= FIRST_PSEUDO_REGISTER && reg_equiv_mem[REGNO(x)] != 0)
    ...
    if (icode != CODE_FOR_nothing && insn_operand_predicate[(int)icode][in_p]
    ...
    if (icode != CODE_FOR_nothing)
    {
       ...
       skip.  */
        ...
        else
        {
            ...
            t_class
                = (t_letter == 'r' ? GENERAL_REGS : REG_CLASS_FROM_LETTER((unsigned char)t_letter));
            t_icode = icode;
            icode = CODE_FOR_nothing;
        }
    }
    ...
    if (in_p && rclass == reload_class && mode == reload_mode)
        abort();
    ...
    if (t_class != NO_REGS)
    {
        for (t_reload = 0; t_reload < n_reloads; t_reload++)
            if (reload_secondary_p[t_reload]
                ...
                && ((in_p && (reload_secondary_in_icode[t_reload] == CODE_FOR_nothing))
                       || (!in_p && (reload_secondary_out_icode[t_reload] == CODE_FOR_nothing)))
                && (reg_class_size[(int)t_class] == 1 || SMALL_REGISTER_CLASSES)
                && MERGABLE_RELOADS(
                       secondary_type, reload_when_needed[t_reload], opnum, reload_opnum[t_reload]))
            {
                if (in_p)
                ...
                if (MERGE_TO_OTHER(secondary_type, reload_when_needed[t_reload], opnum,
                        reload_opnum[t_reload]))
    ...
    for (s_reload = 0; s_reload < n_reloads; s_reload++)
        if (reload_secondary_p[s_reload]
            && (reg_class_subset_p(rclass, reload_reg_class[s_reload])
                   || reg_class_subset_p(reload_reg_class[s_reload], rclass))
            ...
            && (reg_class_size[(int)rclass] == 1 || SMALL_REGISTER_CLASSES)
            && MERGABLE_RELOADS(
                   secondary_type, reload_when_needed[s_reload], opnum, reload_opnum[s_reload]))
        {
            if (in_p)
            ...
            if (reg_class_subset_p(rclass, reload_reg_class[s_reload]))
                reload_reg_class[s_reload] = rclass;
    ...
    if (s_reload == n_reloads)
    {
        ...
        reload_secondary_out_icode[s_reload] = !in_p ? t_icode : CODE_FOR_nothing;
        reload_secondary_p[s_reload] = 1;
...
static enum reg_class find_valid_class(enum machine_mode m1, int n)
{
    int rclass;
    int regno;
    enum reg_class best_class = (enum reg_class)0;
    int best_size = 0;
    ...
    for (rclass = 1; rclass < N_REG_CLASSES; rclass ++)
    {
        ...
        for (regno = 0; regno < FIRST_PSEUDO_REGISTER && !bad; regno++)
            if (TEST_HARD_REG_BIT(reg_class_contents[rclass], regno)
                && TEST_HARD_REG_BIT(reg_class_contents[rclass], regno + n)
                && !HARD_REGNO_MODE_OK(regno + n, m1))
                bad = 1;

        if (!bad && reg_class_size[rclass] > best_size)
            best_class = (enum reg_class)rclass, best_size = reg_class_size[rclass];
    }
...
static int push_reload(rtx in, rtx out, rtx *inloc, rtx *outloc,
    enum reg_class rclass, enum machine_mode inmode, enum machine_mode outmode, int strict_low,
    int optional, int opnum, enum reload_type type)
{
    int i;
    int dont_share = 0;
    ...
    {
        if (GET_CODE(XEXP(in, 0)) == POST_INC || GET_CODE(XEXP(in, 0)) == POST_DEC)
            in = gen_rtx_MEM(GET_MODE(in), XEXP(XEXP(in, 0), 0));
        if (GET_CODE(XEXP(in, 0)) == PRE_INC || GET_CODE(XEXP(in, 0)) == PRE_DEC)
    ...
    if (in != 0 && GET_CODE(in) == SUBREG && SUBREG_WORD(in) == 0
        ...
        && rclass != CLASS_CANNOT_CHANGE_SIZE
#endif
        && (CONSTANT_P(SUBREG_REG(in)) || GET_CODE(SUBREG_REG(in)) == PLUS || strict_low
               || (((GET_CODE(SUBREG_REG(in)) == REG
                        && REGNO(SUBREG_REG(in)) >= FIRST_PSEUDO_REGISTER)
                       || GET_CODE(SUBREG_REG(in)) == MEM)
                      && ((GET_MODE_SIZE(inmode) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(in))))
               ...
               || (GET_CODE(SUBREG_REG(in)) == REG
                     ...
                     is handled differently in the following statement.  */
                      ...
                      && ((GET_MODE_SIZE(inmode) <= UNITS_PER_WORD
                             ...
                             || !HARD_REGNO_MODE_OK(
                                    (REGNO(SUBREG_REG(in)) + SUBREG_WORD(in)), inmode)))
#ifdef SECONDARY_INPUT_RELOAD_CLASS
               || (SECONDARY_INPUT_RELOAD_CLASS(rclass, inmode, in) != NO_REGS
                      && (SECONDARY_INPUT_RELOAD_CLASS(
                              rclass, GET_MODE(SUBREG_REG(in)), SUBREG_REG(in))
                             == NO_REGS))
#endif
#ifdef CLASS_CANNOT_CHANGE_SIZE
               || (GET_CODE(SUBREG_REG(in)) == REG && REGNO(SUBREG_REG(in)) < FIRST_PSEUDO_REGISTER
    ...
    if (in != 0 && GET_CODE(in) == SUBREG
        && (CONSTANT_P(SUBREG_REG(in))
               || (GET_CODE(SUBREG_REG(in)) == REG && REGNO(SUBREG_REG(in)) < FIRST_PSEUDO_REGISTER
                      && (!HARD_REGNO_MODE_OK(REGNO(SUBREG_REG(in)) + SUBREG_WORD(in), inmode)
                             || (GET_MODE_SIZE(inmode) <= UNITS_PER_WORD
                                    && (GET_MODE_SIZE(GET_MODE(SUBREG_REG(in))) > UNITS_PER_WORD)
                                    && ((GET_MODE_SIZE(GET_MODE(SUBREG_REG(in))) / UNITS_PER_WORD)
                                                  ...
                                                  GET_MODE(SUBREG_REG(in)))))))))
    {
       ...
       output before the outer reload.  */
        push_reload(SUBREG_REG(in), NULL_RTX, &SUBREG_REG(in), NULL,
            find_valid_class(inmode, SUBREG_WORD(in)), VOIDmode, VOIDmode, 0, 0, opnum, type);
    ...
    if (out != 0 && GET_CODE(out) == SUBREG && SUBREG_WORD(out) == 0
        ...
        && rclass != CLASS_CANNOT_CHANGE_SIZE
#endif
        && (CONSTANT_P(SUBREG_REG(out)) || strict_low
               || (((GET_CODE(SUBREG_REG(out)) == REG
                        && REGNO(SUBREG_REG(out)) >= FIRST_PSEUDO_REGISTER)
                       || GET_CODE(SUBREG_REG(out)) == MEM)
                      && ((GET_MODE_SIZE(outmode) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(out))))
                             || ((GET_MODE_SIZE(outmode) < GET_MODE_SIZE(GET_MODE(SUBREG_REG(out))))
               ...
               || (GET_CODE(SUBREG_REG(out)) == REG
                      ...
                      && ((GET_MODE_SIZE(outmode) <= UNITS_PER_WORD
                              ...
                              && ((GET_MODE_SIZE(GET_MODE(SUBREG_REG(out))) / UNITS_PER_WORD)
                                     != HARD_REGNO_NREGS(
                                            REGNO(SUBREG_REG(out)), GET_MODE(SUBREG_REG(out)))))
                             || !HARD_REGNO_MODE_OK(
                                    (REGNO(SUBREG_REG(out)) + SUBREG_WORD(out)), outmode)))
#ifdef SECONDARY_OUTPUT_RELOAD_CLASS
               || (SECONDARY_OUTPUT_RELOAD_CLASS(rclass, outmode, out) != NO_REGS
                      && (SECONDARY_OUTPUT_RELOAD_CLASS(
                              rclass, GET_MODE(SUBREG_REG(out)), SUBREG_REG(out))
                             == NO_REGS))
#endif
#ifdef CLASS_CANNOT_CHANGE_SIZE
               || (GET_CODE(SUBREG_REG(out)) == REG
    ...
    if (out != 0 && GET_CODE(out) == SUBREG && GET_CODE(SUBREG_REG(out)) == REG
        && REGNO(SUBREG_REG(out)) < FIRST_PSEUDO_REGISTER
        && (!HARD_REGNO_MODE_OK(REGNO(SUBREG_REG(out)) + SUBREG_WORD(out), outmode)
               || (GET_MODE_SIZE(outmode) <= UNITS_PER_WORD
                      && (GET_MODE_SIZE(GET_MODE(SUBREG_REG(out))) > UNITS_PER_WORD)
                      && ((GET_MODE_SIZE(GET_MODE(SUBREG_REG(out))) / UNITS_PER_WORD)
                             != HARD_REGNO_NREGS(
                                    REGNO(SUBREG_REG(out)), GET_MODE(SUBREG_REG(out)))))))
    {
       ...
       output after the outer reload.  */
        dont_remove_subreg = 1;
        push_reload(SUBREG_REG(out), SUBREG_REG(out), &SUBREG_REG(out), &SUBREG_REG(out),
            find_valid_class(outmode, SUBREG_WORD(out)), VOIDmode, VOIDmode, 0, 0, opnum,
            RELOAD_OTHER);
    }

    /* If IN appears in OUT, we can't share any input-only reload for IN.  */
    if (in != 0 && out != 0 && GET_CODE(out) == MEM && (GET_CODE(in) == REG || GET_CODE(in) == MEM)
    ...
    if (in != 0 && GET_CODE(in) == SUBREG && GET_CODE(SUBREG_REG(in)) == REG
        && REGNO(SUBREG_REG(in)) < FIRST_PSEUDO_REGISTER && !dont_remove_subreg)
        in = gen_rtx_REG(GET_MODE(in), REGNO(SUBREG_REG(in)) + SUBREG_WORD(in));

    /* Similarly for OUT.  */
    if (out != 0 && GET_CODE(out) == SUBREG && GET_CODE(SUBREG_REG(out)) == REG
        && REGNO(SUBREG_REG(out)) < FIRST_PSEUDO_REGISTER && !dont_remove_subreg)
        out = gen_rtx_REG(GET_MODE(out), REGNO(SUBREG_REG(out)) + SUBREG_WORD(out));

    /* Narrow down the class of register wanted if that is
       desirable on this machine for efficiency.  */
    if (in != 0)
        rclass = PREFERRED_RELOAD_CLASS(in, rclass);

        /* Output reloads may need analogous treatment, different in detail.  */
#ifdef PREFERRED_OUTPUT_RELOAD_CLASS
    if (out != 0)
        rclass = PREFERRED_OUTPUT_RELOAD_CLASS(out, rclass);
#endif
    ...
    if (this_insn_is_asm)
    {
        ...
        for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
            if (HARD_REGNO_MODE_OK(i, mode) && TEST_HARD_REG_BIT(reg_class_contents[(int)rclass], i))
            {
                int nregs = HARD_REGNO_NREGS(i, mode);

                int j;
                for (j = 1; j < nregs; j++)
                    if (!TEST_HARD_REG_BIT(reg_class_contents[(int)rclass], i + j))
                        break;
                if (j == nregs)
                    break;
            }
    ...
    if (rclass == NO_REGS && (optional == 0 || type != RELOAD_FOR_OUTPUT))
        abort();
    ...
    for (i = 0; i < n_reloads; i++)
        if ((reg_class_subset_p(rclass, reload_reg_class[i])
                || reg_class_subset_p(reload_reg_class[i], rclass))
            /* If the existing reload has a register, it must fit our class.  */
            && (reload_reg_rtx[i] == 0
                   || TEST_HARD_REG_BIT(
                          reg_class_contents[(int)rclass], true_regnum(reload_reg_rtx[i])))
            ...
            && (reg_class_size[(int)rclass] == 1 || SMALL_REGISTER_CLASSES)
            && MERGABLE_RELOADS(type, reload_when_needed[i], opnum, reload_opnum[i]))
            break;
    ...
    if (i == n_reloads)
        for (i = 0; i < n_reloads; i++)
            if ((reg_class_subset_p(rclass, reload_reg_class[i])
                ...
                && ((GET_CODE(in) == REG
                        && (GET_CODE(reload_in[i]) == POST_INC || GET_CODE(reload_in[i]) == POST_DEC
                               || GET_CODE(reload_in[i]) == PRE_INC
                               || GET_CODE(reload_in[i]) == PRE_DEC)
                        && MATCHES(XEXP(reload_in[i], 0), in))
                       || (GET_CODE(reload_in[i]) == REG
                              && (GET_CODE(in) == POST_INC || GET_CODE(in) == POST_DEC
                                     || GET_CODE(in) == PRE_INC || GET_CODE(in) == PRE_DEC)
...
#ifdef SECONDARY_INPUT_RELOAD_CLASS
        if (in != 0)
            secondary_in_reload = push_secondary_reload(
                1, in, opnum, optional, rclass, inmode, type, &secondary_in_icode);
#endif

#ifdef SECONDARY_OUTPUT_RELOAD_CLASS
        if (out != 0 && GET_CODE(out) != SCRATCH)
            secondary_out_reload = push_secondary_reload(
                0, out, opnum, optional, rclass, outmode, type, &secondary_out_icode);
#endif
    ...
    else
    {
       ...
       the larger mode, so that the value is valid for both modes.  */
        if (inmode != VOIDmode && GET_MODE_SIZE(inmode) > GET_MODE_SIZE(reload_inmode[i]))
            reload_inmode[i] = inmode;
        if (outmode != VOIDmode && GET_MODE_SIZE(outmode) > GET_MODE_SIZE(reload_outmode[i]))
            reload_outmode[i] = outmode;
        if (in != 0)
        {
        ...
        if (reg_class_subset_p(rclass, reload_reg_class[i]))
            reload_reg_class[i] = rclass;
        reload_optional[i] &= optional;
        if (MERGE_TO_OTHER(type, reload_when_needed[i], opnum, reload_opnum[i]))
    ...
    {
        reload_reg_rtx[i] = find_dummy_reload(in, out, inloc, outloc, inmode, outmode,
            reload_reg_class[i], i, earlyclobber_operand_p(out));

        /* If the outgoing register already contains the same value
       as the incoming one, we can dispense with loading it.
       The easiest way to tell the caller that is to give a phony
       value for the incoming operand (same as outgoing one).  */
        if (reload_reg_rtx[i] == out && (GET_CODE(in) == REG || CONSTANT_P(in))
            && 0 != find_equiv_reg(in, this_insn, (enum reg_class)0, REGNO(out), static_reload_reg_p, i, inmode))
            reload_in[i] = out;
    }
    ...
    if (reload_reg_rtx[i] == 0 && in != 0)
    {
        ...
        for (note = REG_NOTES(this_insn); note; note = XEXP(note, 1))
            if (REG_NOTE_KIND(note) == REG_DEAD && GET_CODE(XEXP(note, 0)) == REG
                && (regno = REGNO(XEXP(note, 0))) < FIRST_PSEUDO_REGISTER
                && reg_mentioned_p(XEXP(note, 0), in)
                && !refers_to_regno_for_reload_p(
                       regno, (regno + HARD_REGNO_NREGS(regno, inmode)), PATTERN(this_insn), inloc)
                /* If this is also an output reload, IN cannot be used as
                   the reload register if it is set in this insn unless IN
                   is also OUT.  */
                && (out == 0 || in == out
                       || !hard_reg_set_here_p(
                              regno, (regno + HARD_REGNO_NREGS(regno, inmode)), PATTERN(this_insn)))
                ...
                && TEST_HARD_REG_BIT(reg_class_contents[(int)rclass], regno) && !fixed_regs[regno])
            {
...
static int loc_mentioned_in_p(rtx *loc, rtx in)
{
...
static void combine_reloads(void)
{
    ...
    for (i = 0; i < n_reloads; i++)
        if (reload_in[i] && !reload_optional[i]
            && !reload_nocombine[i]
            ...
            && (CLASS_MAX_NREGS(reload_reg_class[i], reload_inmode[i])
                   == CLASS_MAX_NREGS(
                          reload_reg_class[output_reload], reload_outmode[output_reload]))
            && reload_inc[i] == 0 && reload_reg_rtx[i] == 0
            && (SMALL_REGISTER_CLASSES
                       ? (reload_reg_class[i] == reload_reg_class[output_reload])
                       : (reg_class_subset_p(reload_reg_class[i], reload_reg_class[output_reload])
                             || reg_class_subset_p(
                                    reload_reg_class[output_reload], reload_reg_class[i])))
            && (MATCHES(reload_in[i], reload_out[output_reload])
                   /* Args reversed because the first arg seems to be
                      the one that we imagine being modified
                      while the second is the one that might be affected.  */
                   || (!reg_overlap_mentioned_for_reload_p(reload_out[output_reload], reload_in[i])
                          ...
                          && !(GET_CODE(reload_in[i]) == REG
                                 && reg_overlap_mentioned_for_reload_p(
                                        reload_in[i], reload_out[output_reload]))))
            && (reg_class_size[(int)reload_reg_class[i]] || SMALL_REGISTER_CLASSES)
                   ...
                   || reload_when_needed[i] == RELOAD_FOR_OUTPUT))
        {
            ...
            {
                reload_secondary_out_reload[i] = reload_secondary_out_reload[output_reload];
                reload_secondary_out_icode[i] = reload_secondary_out_icode[output_reload];
            }

            /* If required, minimize the register class.  */
            if (reg_class_subset_p(reload_reg_class[output_reload], reload_reg_class[i]))
                reload_reg_class[i] = reload_reg_class[output_reload];
    ...
    for (note = REG_NOTES(this_insn); note; note = XEXP(note, 1))
        if (REG_NOTE_KIND(note) == REG_DEAD && GET_CODE(XEXP(note, 0)) == REG
            && !reg_overlap_mentioned_for_reload_p(XEXP(note, 0), reload_out[output_reload])
            && REGNO(XEXP(note, 0)) < FIRST_PSEUDO_REGISTER
            && HARD_REGNO_MODE_OK(REGNO(XEXP(note, 0)), reload_outmode[output_reload])
            && TEST_HARD_REG_BIT(
                   reg_class_contents[(int)reload_reg_class[output_reload]], REGNO(XEXP(note, 0)))
            ...
            && ((secondary_out = reload_secondary_out_reload[output_reload]) == -1
                   || (!(TEST_HARD_REG_BIT(reg_class_contents[(int)reload_reg_class[secondary_out]],
                           REGNO(XEXP(note, 0))))
            ...
            && !fixed_regs[REGNO(XEXP(note, 0))])
        {
            reload_reg_rtx[output_reload]
                = gen_rtx_REG(reload_outmode[output_reload], REGNO(XEXP(note, 0)));
            return;
        }
}
...
/* Try to find a reload register for an in-out reload (expressions IN and OUT).
   ...
   If so, return the rtx that proves acceptable.
...
static rtx find_dummy_reload(rtx real_in, rtx real_out, rtx *inloc, rtx *outloc,
    enum machine_mode inmode, enum machine_mode outmode, enum reg_class rclass, int for_real,
    int earlyclobber)
{
    rtx in = real_in;
    rtx out = real_out;
    ...
    if (GET_MODE_SIZE(outmode) != GET_MODE_SIZE(inmode)
        && (GET_MODE_SIZE(outmode) > UNITS_PER_WORD || GET_MODE_SIZE(inmode) > UNITS_PER_WORD))
        return 0;
    ...
    /* See if OUT will do.  */
    if (GET_CODE(out) == REG && REGNO(out) < FIRST_PSEUDO_REGISTER)
    {
        int regno = REGNO(out) + out_offset;
...
#endif
            && !refers_to_regno_for_reload_p(regno, regno + nwords, PATTERN(this_insn), outloc))
        {
            int i;
            for (i = 0; i < nwords; i++)
                if (!TEST_HARD_REG_BIT(reg_class_contents[(int)rclass], regno + i))
                    break;
    ...
    if (hard_regs_live_known && GET_CODE(in) == REG && REGNO(in) < FIRST_PSEUDO_REGISTER
        && (value == 0 || find_reg_note(this_insn, REG_UNUSED, real_out))
        && find_reg_note(this_insn, REG_DEAD, real_in) && !fixed_regs[REGNO(in)]
        && HARD_REGNO_MODE_OK(REGNO(in),
               /* The only case where out and real_out might
              have different modes is where real_out
              is a subreg, and in that case, out
              has a real mode.  */
               (GET_MODE(out) != VOIDmode ? GET_MODE(out) : outmode)))
    {
        int regno = REGNO(in) + in_offset;
        int nwords = HARD_REGNO_NREGS(regno, inmode);

        if (!refers_to_regno_for_reload_p(regno, regno + nwords, out, NULL)
            && !hard_reg_set_here_p(regno, regno + nwords, PATTERN(this_insn))
            && (!earlyclobber
                   || !refers_to_regno_for_reload_p(
                          regno, regno + nwords, PATTERN(this_insn), inloc)))
        {
            int i;
            for (i = 0; i < nwords; i++)
                if (!TEST_HARD_REG_BIT(reg_class_contents[(int)rclass], regno + i))
...
static int earlyclobber_operand_p(rtx x)
{
    int i;
...
static int hard_reg_set_here_p(int beg_regno, int end_regno, rtx x)
{
    if (GET_CODE(x) == SET || GET_CODE(x) == CLOBBER)
    {
        ...
        if (GET_CODE(op0) == REG)
        {
            ...
            if (r < end_regno && r + HARD_REGNO_NREGS(r, GET_MODE(op0)) > beg_regno)
                return 1;
        }
    }
...
int strict_memory_address_p(enum machine_mode mode, rtx addr)
{
...
int operands_match_p(rtx x, rtx y)
{
    ...
    if ((code == REG || (code == SUBREG && GET_CODE(SUBREG_REG(x)) == REG))
        && (GET_CODE(y) == REG || (GET_CODE(y) == SUBREG && GET_CODE(SUBREG_REG(y)) == REG)))
    {
        int j;
...
static struct decomposition decompose(rtx x)
{
    ...
    if (GET_CODE(x) == MEM)
    {
        ...
        if (GET_CODE(addr) == PRE_DEC || GET_CODE(addr) == PRE_INC || GET_CODE(addr) == POST_DEC
...
static int immune_p(rtx x, rtx y, struct decomposition ydata)
{
    ...
    if (!rtx_equal_p(xdata.base, ydata.base))
    {
        ...
        if (CONSTANT_P(xdata.base)
            && (ydata.base == frame_pointer_rtx || ydata.base == stack_pointer_rtx))
            return 1;
        if (CONSTANT_P(ydata.base)
            && (xdata.base == frame_pointer_rtx || xdata.base == stack_pointer_rtx))
            return 1;
...
int find_reloads(rtx insn, int replace, int ind_levels, int live_known, short *reload_reg_p)
{
    ...
    static int last_output_reload_regno ATTRIBUTE_UNUSED = -1;
    ...
    if (GET_CODE(body) == SET && GET_CODE(SET_DEST(body)) == REG
        && REGNO(SET_DEST(body)) < FIRST_PSEUDO_REGISTER && GET_CODE(SET_SRC(body)) == REG
        && REGNO(SET_SRC(body)) < FIRST_PSEUDO_REGISTER
        && REGISTER_MOVE_COST(
               REGNO_REG_CLASS(REGNO(SET_SRC(body))), REGNO_REG_CLASS(REGNO(SET_DEST(body))))
            == 2)
        return 0;
    ...
    copy_memory(
        (char *)recog_operand_mode, (char *)operand_mode, noperands * sizeof(enum machine_mode));
    copy_memory((char *)recog_constraints, (char *)constraints, noperands * sizeof(char *));
    ...
    for (i = 0; i < noperands; i++)
    {
       ...
       an in-out operand, is commutative, or should match another.  */
        ...
        while ((c = *p++))
        {
            ...
            {
                c -= '0';
                operands_match[c][i] = operands_match_p(recog_operand[c], recog_operand[i]);
    ...
    for (i = 0; i < noperands; i++)
    {
        ...
        operand_type[i] = (modified[i] == RELOAD_READ
                ? RELOAD_FOR_INPUT
                : modified[i] == RELOAD_WRITE ? RELOAD_FOR_OUTPUT : RELOAD_OTHER);
        address_type[i] = (modified[i] == RELOAD_READ
                ? RELOAD_FOR_INPUT_ADDRESS
                : modified[i] == RELOAD_WRITE ? RELOAD_FOR_OUTPUT_ADDRESS : RELOAD_OTHER);
        ...
        {
            find_reloads_address(VOIDmode, NULL, recog_operand[i], recog_operand_loc[i], i,
                operand_type[i], ind_levels, insn);

            /* If we now have a simple operand where we used to have a
               PLUS or MULT, re-recognize and try again.  */
            if ((GET_RTX_CLASS(GET_CODE(*recog_operand_loc[i])) == 'o'
                    || GET_CODE(*recog_operand_loc[i]) == SUBREG)
                && (GET_CODE(recog_operand[i]) == MULT || GET_CODE(recog_operand[i]) == PLUS))
            {
                INSN_CODE(insn) = -1;
        ...
        {
            if (find_reloads_address(GET_MODE(recog_operand[i]), recog_operand_loc[i],
                    XEXP(recog_operand[i], 0), &XEXP(recog_operand[i], 0), i, address_type[i],
                    ind_levels, insn))
        ...
        else if (code == SUBREG)
        {
            ...
            rtx op = find_reloads_toplev(recog_operand[i], i, address_type[i], ind_levels,
                set != 0 && &SET_DEST(set) == recog_operand_loc[i], insn);
            ...
            if (replace && GET_CODE(op) == MEM && GET_CODE(reg) == REG
                && (GET_MODE_SIZE(GET_MODE(reg)) >= GET_MODE_SIZE(GET_MODE(op))))
                REG_NOTES(emit_insn_before(gen_rtx_USE(VOIDmode, reg), insn))
                    = gen_rtx_EXPR_LIST(REG_EQUAL, reg_equiv_memory_loc[REGNO(reg)], NULL_RTX);
            ...
            substed_operand[i] = recog_operand[i]
                = find_reloads_toplev(recog_operand[i], i, address_type[i], ind_levels, 0, insn);
        else if (code == REG)
        {
                ...
                && (set == 0 || &SET_DEST(set) != recog_operand_loc[i]))
            {
                ...
                substed_operand[i] = recog_operand[i] = reg_equiv_constant[regno];
            }
            if (reg_equiv_memory_loc[regno] != 0
                && (reg_equiv_address[regno] != 0 || num_not_at_initial_offset))
                /* We need not give a valid is_set_dest argument since the case
                   of a constant equivalence was checked above.  */
                substed_operand[i] = recog_operand[i] = find_reloads_toplev(
                    recog_operand[i], i, address_type[i], ind_levels, 0, insn);
        }
        /* If the operand is still a register (we didn't replace it with an
       equivalent), get the preferred class to reload it into.  */
        code = GET_CODE(recog_operand[i]);
        preferred_class[i] = ((code == REG && REGNO(recog_operand[i]) >= FIRST_PSEUDO_REGISTER)
                ? reg_preferred_class(REGNO(recog_operand[i]))
                : NO_REGS);
        pref_or_nothing[i] = (code == REG && REGNO(recog_operand[i]) >= FIRST_PSEUDO_REGISTER
...
try_swapped:
         ...
         this_alternative_number++)
    {
       ...
       a bad register class to only count 1/3 as much.  */
        ...
        for (i = 0; i < noperands; i++)
        {
            ...
            while (GET_CODE(operand) == SUBREG)
            {
               ...
               be a problem accessing OPERAND in the outer mode.  */
                if (CONSTANT_P(operand)
                    ...
                    || (GET_CODE(operand) == REG && REGNO(operand) < FIRST_PSEUDO_REGISTER
                           && ((GET_MODE_SIZE(operand_mode[i]) <= UNITS_PER_WORD
                                   && (GET_MODE_SIZE(GET_MODE(operand)) > UNITS_PER_WORD)
                                   && ((GET_MODE_SIZE(GET_MODE(operand)) / UNITS_PER_WORD)
                                          != HARD_REGNO_NREGS(REGNO(operand), GET_MODE(operand))))
                                  || !HARD_REGNO_MODE_OK(
                                         REGNO(operand) + offset, operand_mode[i]))))
                    force_reload = 1;
            }
            ...
            while (*p && (c = *p++) != ',')
                ...
                case '4':
                    ...
                    if ((swapped && (c != commutative || i != commutative + 1))
                            ...
                            ? (operands_match[(c == commutative || c == commutative + 1)
                                      ? 2 * commutative + 1 - c
                                      : c][(i == commutative || i == commutative + 1)
                                      ? 2 * commutative + 1 - i
                                      : i])
                            : operands_match[c][i])
                    {
                        ...
                        if (this_alternative_offmemok[c] && GET_CODE(recog_operand[c]) == MEM
                            && this_alternative[c] == (int)NO_REGS && !this_alternative_win[c])
                            bad = 1;
                    ...
                    else
                    {
                        ...
                        value = find_dummy_reload(recog_operand[i], recog_operand[c],
                            recog_operand_loc[i], recog_operand_loc[c], operand_mode[i],
                            operand_mode[c], (enum reg_class)this_alternative[c], (enum reg_class)-1,
                ...
                case 'o':
                    ...
                    if ((GET_CODE(operand) == MEM
                        ...
                        /* Certain mem addresses will become offsettable
                           ...
                           we don't want our own handling of unoffsettables
                           to override the handling of reg_equiv_address.  */
                        || (GET_CODE(operand) == MEM && GET_CODE(XEXP(operand, 0)) == REG
                               && (ind_levels == 0
                                      || reg_equiv_address[REGNO(XEXP(operand, 0))] != 0))
                        || (GET_CODE(operand) == REG && REGNO(operand) >= FIRST_PSEUDO_REGISTER
                               && reg_renumber[REGNO(operand)] < 0
                ...
                case 's':
                    if (GET_CODE(operand) == CONST_INT
                        || (GET_CODE(operand) == CONST_DOUBLE && GET_MODE(operand) == VOIDmode))
                        break;
                case 'i':
                    if (CONSTANT_P(operand))
                        win = 1;
                ...
                case 'O':
                case 'P':
                    if (GET_CODE(operand) == CONST_INT && CONST_OK_FOR_LETTER_P(INTVAL(operand), c))
                        win = 1;
                    break;
                ...
                case 'g':
                    if (!force_reload
                        ...
                        && (GENERAL_REGS == ALL_REGS || GET_CODE(operand) != REG
                               || (REGNO(operand) >= FIRST_PSEUDO_REGISTER
                                      && reg_renumber[REGNO(operand)] < 0)))
                        win = 1;
                ...
                default:
                    this_alternative[i] = (int)
                        reg_class_subunion[this_alternative[i]][(int)REG_CLASS_FROM_LETTER(c)];
                ...
                reg:
                    ...
                    if (GET_CODE(operand) == REG
                        && reg_fits_class_p(
                               operand, (enum reg_class)this_alternative[i], offset, GET_MODE(recog_operand[i])))
                        win = 1;
                    break;
                }
            ...
            else
            {
                ...
                if (GET_CODE(operand) == REG && this_alternative[i] == (int)NO_REGS
		...
		reject--;
               ...
               force_const_mem.  */
                if (CONSTANT_P(operand)
                    /* force_const_mem does not accept HIGH.  */
                    && GET_CODE(operand) != HIGH
                    && ((PREFERRED_RELOAD_CLASS(operand, (enum reg_class)this_alternative[i])
                            == NO_REGS)
                           || no_input_reloads)
               ...
               here.  */
                ...
                if (!CONSTANT_P(operand) && (enum reg_class)this_alternative[i] != NO_REGS
                    && (PREFERRED_RELOAD_CLASS(operand, (enum reg_class)this_alternative[i])
                           == NO_REGS))
                    bad = 1;
               ...
               and objects with a REG_UNUSED note.  */
                else if (GET_CODE(operand) != SCRATCH && modified[i] != RELOAD_READ
                    && no_output_reloads && !find_reg_note(insn, REG_UNUSED, operand))
                    bad = 1;
                else if (modified[i] != RELOAD_WRITE && no_input_reloads && !const_to_mem)
                    bad = 1;
               ...
               alternative match then.  */
                if (!(GET_CODE(operand) == REG && REGNO(operand) >= FIRST_PSEUDO_REGISTER)
                    && GET_CODE(operand) != SCRATCH && !(const_to_mem && constmemok))
                    reject += 2;

                /* Input reloads can be inherited more often than output
               reloads can be removed, so penalize output reloads.  */
                if (operand_type[i] != RELOAD_FOR_INPUT && GET_CODE(operand) != SCRATCH)
                    reject++;
            }
            ...
            {
                if (!reg_class_subset_p((enum reg_class)this_alternative[i], preferred_class[i]))
                {
                    /* Since we don't have a way of forming the intersection,
                       we just do something special if the preferred class
                       is a subset of the class we have; that's the most
                       common case anyway.  */
                    if (reg_class_subset_p(preferred_class[i], (enum reg_class)this_alternative[i]))
       ...
       or any memory addresses.  */
        ...
        for (i = 0; i < noperands; i++)
            if (this_alternative_earlyclobber[i] && this_alternative_win[i])
            {
                struct decomposition early_data;
                ...
                if (this_alternative[i] == NO_REGS)
                {
                    this_alternative_earlyclobber[i] = 0;
                    if (this_insn_is_asm)
                        error_for_asm(this_insn, "`&' constraint used with no register class");
                    else
                        abort();
                }
                ...
                for (j = 0; j < noperands; j++)
                    ...
                    if ((GET_CODE(recog_operand[j]) == MEM || modified[j] != RELOAD_WRITE)
                        && j != i
                        /* Ignore things like match_operator operands.  */
                ...
                if (j != noperands)
                {
                    ...
                    this_alternative_win[i] = 0;
                    for (j = 0; j < noperands; j++)
                        if (this_alternative_matches[j] == i && this_alternative_win[j])
                        {
                            this_alternative_win[j] = 0;
                            losers++;
                        }
...
finish:
    ...
    for (i = 0; i < noperands; i++)
    {
       ...
       however they were previously categorized.  */
        ...
        if (goal_alternative_earlyclobber[i] && operand_type[i] != RELOAD_OTHER)
            operand_type[i] = (find_reg_note(insn, REG_UNUSED, recog_operand[i]) ? RELOAD_FOR_INSN
                                                                                 : RELOAD_OTHER);
    }
    ...
    for (i = 0; i < noperands; i++)
        if (!goal_alternative_win[i]
            ...
            && ((PREFERRED_RELOAD_CLASS(recog_operand[i], (enum reg_class)goal_alternative[i])
                    == NO_REGS)
                   || no_input_reloads)
            && operand_mode[i] != VOIDmode)
        {
            substed_operand[i] = recog_operand[i]
                = find_reloads_toplev(force_const_mem(operand_mode[i], recog_operand[i]), i,
                    address_type[i], ind_levels, 0, insn);
            if (alternative_allows_memconst(recog_constraints[i], goal_alternative_number))
                goal_alternative_win[i] = 1;
        }
    ...
    for (i = 0; i < noperands; i++)
        if (!goal_alternative_win[i])
        {
            ...
            else if (goal_alternative_matched[i] == -1 && goal_alternative_offmemok[i]
                && GET_CODE(recog_operand[i]) == MEM)
            {
                operand_reloadnum[i] = push_reload(XEXP(recog_operand[i], 0), NULL_RTX,
                    &XEXP(recog_operand[i], 0), NULL, MODE_BASE_REG_CLASS(VOIDmode),
                    GET_MODE(XEXP(recog_operand[i], 0)), VOIDmode, 0, 0, i, RELOAD_FOR_INPUT);
                reload_inc[operand_reloadnum[i]] = GET_MODE_SIZE(GET_MODE(recog_operand[i]));
                ...
                if (modified[i] == RELOAD_WRITE)
                {
                    for (j = 0; j < n_reloads; j++)
                    {
                        if (reload_opnum[j] == i)
                        {
                            ...
                            else if (reload_when_needed[j] == RELOAD_FOR_OUTADDR_ADDRESS)
            ...
            else if (goal_alternative_matched[i] == -1)
            {
                operand_reloadnum[i]
                    = push_reload((modified[i] != RELOAD_WRITE ? recog_operand[i] : 0),
                        ...
                        (insn_code_number < 0 ? 0 : insn_operand_strict_low[insn_code_number][i]),
                        0, i, operand_type[i]);
                if (modified[i] != RELOAD_READ && GET_CODE(recog_operand[i]) == REG)
            ...
            {
                operand_reloadnum[i]
                    = push_reload(recog_operand[i], recog_operand[goal_alternative_matched[i]],
                        recog_operand_loc[i], recog_operand_loc[goal_alternative_matched[i]],
                        (enum reg_class)goal_alternative[i], operand_mode[i],
                        operand_mode[goal_alternative_matched[i]], 0, 0, i, RELOAD_OTHER);
                operand_reloadnum[goal_alternative_matched[i]] = output_reloadnum;
                if (GET_CODE(recog_operand[goal_alternative_matched[i]]) == REG)
                    last_output_reload_regno = REGNO(recog_operand[goal_alternative_matched[i]]);
            }
            else if (modified[i] == RELOAD_WRITE
                && modified[goal_alternative_matched[i]] == RELOAD_READ)
            {
                operand_reloadnum[goal_alternative_matched[i]] = push_reload(
                    recog_operand[goal_alternative_matched[i]], recog_operand[i],
                    recog_operand_loc[goal_alternative_matched[i]], recog_operand_loc[i],
                    (enum reg_class)goal_alternative[i], operand_mode[goal_alternative_matched[i]],
                    operand_mode[i], 0, 0, i, RELOAD_OTHER);
        ...
        else if (goal_alternative_matched[i] < 0 && goal_alternative_matches[i] < 0 && optimize)
        {
            ...
            if ((GET_CODE(operand) == MEM
                ...
                && ((enum reg_class)goal_alternative[i] != NO_REGS || modified[i] == RELOAD_WRITE)
                && !no_input_reloads
                ...
                && (modified[i] == RELOAD_READ || (!no_output_reloads && !this_insn_is_asm)))
                operand_reloadnum[i]
                    = push_reload(modified[i] != RELOAD_WRITE ? recog_operand[i] : 0,
                        modified[i] != RELOAD_READ ? recog_operand[i] : 0,
                        (modified[i] != RELOAD_WRITE ? recog_operand_loc[i] : 0),
            ...
            && goal_alternative_win[goal_alternative_matches[i]] && modified[i] == RELOAD_READ
            && modified[goal_alternative_matches[i]] == RELOAD_WRITE && !no_input_reloads
            && !no_output_reloads && optimize)
        {
            ...
            if ((GET_CODE(operand) == MEM
                    || (GET_CODE(operand) == REG && REGNO(operand) >= FIRST_PSEUDO_REGISTER))
                && ((enum reg_class)goal_alternative[goal_alternative_matches[i]] != NO_REGS))
                operand_reloadnum[i] = operand_reloadnum[goal_alternative_matches[i]]
                    = push_reload(recog_operand[goal_alternative_matches[i]], recog_operand[i],
                        recog_operand_loc[goal_alternative_matches[i]], recog_operand_loc[i],
                        (enum reg_class)goal_alternative[goal_alternative_matches[i]],
                        operand_mode[goal_alternative_matches[i]], operand_mode[i], 0, 1,
                        goal_alternative_matches[i], RELOAD_OTHER);
        }
        ...
        for (j = i + 1; j < n_reloads; j++)
            if ((reload_when_needed[i] == RELOAD_FOR_INPUT_ADDRESS
                ...
                && (goal_alternative_matches[reload_opnum[i]] == reload_opnum[j]
                       || (goal_alternative_matches[reload_opnum[j]] == reload_opnum[i])))
            {
    ...
    for (i = 0; i < n_reloads; i++)
    {
        if (reload_secondary_p[i] && reload_when_needed[i] == operand_type[reload_opnum[i]])
                   ...
                   || reload_optional[operand_reloadnum[reload_opnum[i]]]))
        {
                ...
                && reload_secondary_in_reload[i] != -1)
            {
                ...
                reload_when_needed[secondary_in_reload] = RELOAD_FOR_OPADDR_ADDR;
                ...
                && reload_secondary_out_reload[i] != -1)
            {
                ...
                reload_when_needed[secondary_out_reload] = RELOAD_FOR_OPADDR_ADDR;

                /* If there's a tertiary reload we have to change it also.  */
                if (secondary_out_reload && reload_secondary_out_reload[secondary_out_reload] != -1)
        ...
        if ((reload_when_needed[i] == RELOAD_FOR_INPUT_ADDRESS
                || reload_when_needed[i] == RELOAD_FOR_INPADDR_ADDRESS)
            && operand_reloadnum[reload_opnum[i]] >= 0
            && (reload_when_needed[operand_reloadnum[reload_opnum[i]]] == RELOAD_OTHER))
        ...
        if (need_change)
        {
            for (i = 0; i < n_reloads; i++)
            {
                ...
                if (first_num < 0)
                    ...
                    reload_when_needed[i] = (enum reload_type)type;
                else
                {
        ...
        if (reload_in[i] != 0 && reload_out[i] == 0
            ...
            for (j = 0; j < n_reloads; j++)
                if (i != j && reload_in[j] != 0 && reload_out[j] == 0
                    ...
                    && reload_reg_class[i] == reload_reg_class[j] && !reload_nocombine[i]
                    && !reload_nocombine[j] && reload_reg_rtx[i] == reload_reg_rtx[j])
                {
        ...
        if (size == 1)
            for (j = 0; j < n_reloads; j++)
                if ((CLASS_MAX_NREGS(reload_reg_class[j],
                    ...
                    && reg_classes_intersect_p(rclass, reload_reg_class[j]))
                {
                    reload_nongroup[i] = 1;
                    changed = 1;
                    break;
    ...
    while (changed)
    {
        ...
        for (i = 0; i < n_reloads; i++)
        {
            ...
            if (!reload_nongroup[i] && size == 1)
                for (j = 0; j < n_reloads; j++)
                    if (reload_nongroup[j] && reloads_conflict(i, j)
                        && reg_classes_intersect_p(rclass, reload_reg_class[j]))
                    {
                        reload_nongroup[i] = 1;
                        changed = 1;
    ...
    for (i = 0; i < noperands; i++)
    {
        ...
        if (insn_code_number >= 0)
            if (insn_operand_address_p[insn_code_number][i])
                find_reloads_address(VOIDmode, NULL, recog_operand[i], recog_operand_loc[i], i,
                    RELOAD_FOR_INPUT, ind_levels, insn);
       ...
       problem.  */
        ...
        if (code == MEM)
            find_reloads_address(GET_MODE(recog_operand[i]), recog_operand_loc[i],
                XEXP(recog_operand[i], 0), &XEXP(recog_operand[i], 0), i, RELOAD_OTHER, ind_levels,
                insn);
        if (code == SUBREG)
            recog_operand[i] = *recog_operand_loc[i]
                = find_reloads_toplev(recog_operand[i], i, RELOAD_OTHER, ind_levels, is_set_dest);
        if (code == REG)
        {
            int regno = REGNO(recog_operand[i]);
            if (reg_equiv_constant[regno] != 0 && !is_set_dest)
                recog_operand[i] = *recog_operand_loc[i] = reg_equiv_constant[regno];
        }
    }
...
static rtx find_reloads_toplev(
    rtx x, int opnum, enum reload_type type, int ind_levels, int is_set_dest, rtx insn)
{
    RTX_CODE code = GET_CODE(x);
            ...
            if (reg_equiv_address[regno] || !rtx_equal_p(mem, reg_equiv_mem[regno]))
            {
               ...
               that delete_output_reload can see it.  */
                ...
                find_reloads_address(
                    GET_MODE(x), &x, XEXP(x, 0), &XEXP(x, 0), opnum, type, ind_levels, insn);
            }
        }
        return x;
    }
    if (code == MEM)
    {
        rtx tem = x;
        find_reloads_address(
            GET_MODE(x), &tem, XEXP(x, 0), &XEXP(x, 0), opnum, type, ind_levels, insn);
        return tem;
    }
    ...
    if (code == SUBREG && GET_CODE(SUBREG_REG(x)) == REG)
    {
       ...
       above should handle it).  */
        ...
        if (subreg_lowpart_p(x) && regno >= FIRST_PSEUDO_REGISTER && reg_renumber[regno] < 0
            && reg_equiv_constant[regno] != 0
            && (tem = gen_lowpart_common(GET_MODE(x), reg_equiv_constant[regno])) != 0)
            return tem;

        if (GET_MODE_BITSIZE(GET_MODE(x)) == BITS_PER_WORD && regno >= FIRST_PSEUDO_REGISTER
            && reg_renumber[regno] < 0 && reg_equiv_constant[regno] != 0
            && (tem = operand_subword(
                    reg_equiv_constant[regno], SUBREG_WORD(x), 0, GET_MODE(SUBREG_REG(x))))
                != 0)
        {
       ...
       for non-paradoxical subregs of CONST_INTs.  */
        if (regno >= FIRST_PSEUDO_REGISTER && reg_renumber[regno] < 0
            && reg_equiv_constant[regno] != 0 && GET_CODE(reg_equiv_constant[regno]) == CONST_INT
            && (GET_MODE_SIZE(GET_MODE(x)) < GET_MODE_SIZE(GET_MODE(SUBREG_REG(x)))))
        {
        ...
        if (regno >= FIRST_PSEUDO_REGISTER && reg_renumber[regno] < 0
            && reg_equiv_constant[regno] != 0 && GET_MODE(reg_equiv_constant[regno]) == VOIDmode)
            abort();
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        if (fmt[i] == 'e')
            XEXP(x, i)
                = find_reloads_toplev(XEXP(x, i), opnum, type, ind_levels, is_set_dest, insn);
    }
    return x;
}
...
static rtx make_memloc(rtx ad, int regno)
{
    ...
    rtx tem = XEXP(eliminate_regs(reg_equiv_memory_loc[regno], (enum machine_mode)0, NULL_RTX), 0);
...
static int find_reloads_address(enum machine_mode mode, rtx *memrefloc, rtx ad, rtx *loc, int opnum,
    enum reload_type type, int ind_levels, rtx insn)
{
    ...
    if (GET_CODE(ad) == REG)
    {
        ...
        if (tem != 0)
        {
            if (reg_equiv_address[regno] != 0 || num_not_at_initial_offset)
            {
                ...
                {
                    find_reloads_address(GET_MODE(tem), NULL, XEXP(tem, 0), &XEXP(tem, 0), opnum,
               ...
               address: only reg or reg+constant.  */
                                  ...
                                  && CONSTANT_P(XEXP(XEXP(tem, 0), 1)))))
                {
                    ...
                    if (replace_reloads && num_not_at_initial_offset
       ...
       subject of a CLOBBER in this insn.  */
        ...
        else if (regno < FIRST_PSEUDO_REGISTER && REGNO_MODE_OK_FOR_BASE_P(regno, mode)
            && !regno_clobbered_p(regno, this_insn))
            return 0;

        /* If we do not have one of the cases above, we must do the reload.  */
        push_reload(ad, NULL_RTX, loc, NULL, MODE_BASE_REG_CLASS(mode), GET_MODE(ad), VOIDmode, 0,
            0, opnum, type);
        return 1;
    }
    ...
    if (strict_memory_address_p(mode, ad))
    {
       ...
       So do a tree-walk to find and eliminate all such regs.  */
        ...
        if (GET_CODE(ad) == PLUS && GET_CODE(XEXP(ad, 1)) == CONST_INT
            && GET_CODE(XEXP(ad, 0)) == REG && reg_equiv_constant[REGNO(XEXP(ad, 0))] == 0)
            return 0;
    ...
    do
    {
        if (memrefloc)
        {
            LEGITIMIZE_RELOAD_ADDRESS(ad, GET_MODE(*memrefloc), opnum, type, ind_levels, win);
        }
        break;
    win:
    ...
    if (GET_CODE(ad) == MEM)
    {
       ...
       indirect addresses are valid, reload the MEM into a register.  */
        tem = ad;
        find_reloads_address(GET_MODE(ad), &tem, XEXP(ad, 0), &XEXP(ad, 0), opnum, ADDR_TYPE(type),
       ...
       taken care of above.  */
        ...
        if (ind_levels == 0 || (GET_CODE(XEXP(tem, 0)) == SYMBOL_REF && !indirect_symref_ok)
            || GET_CODE(XEXP(tem, 0)) == MEM
            || !(GET_CODE(XEXP(tem, 0)) == REG
                   || (GET_CODE(XEXP(tem, 0)) == PLUS && GET_CODE(XEXP(XEXP(tem, 0), 0)) == REG
                          && GET_CODE(XEXP(XEXP(tem, 0), 1)) == CONST_INT)))
        {
            /* Must use TEM here, not AD, since it is the one that will
               have any subexpressions reloaded, if needed.  */
            push_reload(tem, NULL_RTX, loc, NULL, MODE_BASE_REG_CLASS(mode), GET_MODE(tem),
    ...
    else if (GET_CODE(ad) == PLUS && GET_CODE(XEXP(ad, 0)) == REG
        && REGNO(XEXP(ad, 0)) < FIRST_PSEUDO_REGISTER && REG_MODE_OK_FOR_BASE_P(XEXP(ad, 0), mode)
        ...
        if (double_reg_address_ok)
        {
            ...
            *loc = ad = copy_rtx(ad);
            ...
            find_reloads_address_part(
                XEXP(ad, 1), &XEXP(ad, 1), INDEX_REG_CLASS, GET_MODE(ad), opnum, type, ind_levels);
        }
        else
        {
            ...
            find_reloads_address_part(
                ad, loc, MODE_BASE_REG_CLASS(mode), Pmode, opnum, type, ind_levels);
        }
        return 1;
    }
    ...
    /* If we have an indexed stack slot, there are three possible reasons why
       ...
       innermost PLUS.  */
        ...
        && (XEXP(XEXP(ad, 0), 0) == frame_pointer_rtx || XEXP(XEXP(ad, 0), 0) == arg_pointer_rtx
               || XEXP(XEXP(ad, 0), 0) == stack_pointer_rtx)
        && !memory_address_p(mode, ad))
    {
        *loc = ad = gen_rtx_PLUS(GET_MODE(ad),
            plus_constant(XEXP(XEXP(ad, 0), 0), INTVAL(XEXP(ad, 1))),
            XEXP(XEXP(ad, 0), 1));
        find_reloads_address_part(XEXP(ad, 0), &XEXP(ad, 0), MODE_BASE_REG_CLASS(mode),
            GET_MODE(ad), opnum, type, ind_levels);
        find_reloads_address_1(mode, XEXP(ad, 1), 1, &XEXP(ad, 1), opnum, type, 0, insn);
        ...
        && GET_CODE(XEXP(ad, 0)) == PLUS
        && (XEXP(XEXP(ad, 0), 1) == frame_pointer_rtx || XEXP(XEXP(ad, 0), 1) == arg_pointer_rtx
               || XEXP(XEXP(ad, 0), 1) == stack_pointer_rtx)
        && !memory_address_p(mode, ad))
    {
        *loc = ad = gen_rtx_PLUS(GET_MODE(ad),
            XEXP(XEXP(ad, 0), 0),
            plus_constant(XEXP(XEXP(ad, 0), 1), INTVAL(XEXP(ad, 1))));
        find_reloads_address_part(XEXP(ad, 1), &XEXP(ad, 1), MODE_BASE_REG_CLASS(mode),
            GET_MODE(ad), opnum, type, ind_levels);
        find_reloads_address_1(mode, XEXP(ad, 0), 1, &XEXP(ad, 0), opnum, type, 0, insn);
    ...
    if (CONSTANT_P(ad) && !strict_memory_address_p(mode, ad))
    {
       ...
       Unshare it so we can safely alter it.  */
        if (memrefloc && GET_CODE(ad) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P(ad))
        {
            *memrefloc = copy_rtx(*memrefloc);
            loc = &XEXP(*memrefloc, 0);
        }
...
static rtx subst_reg_equivs(rtx ad, rtx insn)
{
    ...
    case PLUS:
        ...
        if (XEXP(ad, 0) == frame_pointer_rtx && GET_CODE(XEXP(ad, 1)) == CONST_INT)
            return ad;
        break;
...
static rtx subst_indexed_address(rtx addr)
{
    ...
    if (GET_CODE(addr) == PLUS)
    {
        ...
        if (GET_CODE(op0) == REG && (regno = REGNO(op0)) >= FIRST_PSEUDO_REGISTER
            && reg_renumber[regno] < 0 && reg_equiv_constant[regno] != 0)
            op0 = reg_equiv_constant[regno];
        else if (GET_CODE(op1) == REG && (regno = REGNO(op1)) >= FIRST_PSEUDO_REGISTER
            && reg_renumber[regno] < 0 && reg_equiv_constant[regno] != 0)
            op1 = reg_equiv_constant[regno];
        else if (GET_CODE(op0) == PLUS && (tem = subst_indexed_address(op0)) != op0)
            op0 = tem;
        else if (GET_CODE(op1) == PLUS && (tem = subst_indexed_address(op1)) != op1)
            op1 = tem;
        else
            return addr;
...
static int find_reloads_address_1(enum machine_mode mode, rtx x, int context, rtx *loc, int opnum,
    enum reload_type type, int ind_levels, rtx insn)
{
    RTX_CODE code = GET_CODE(x);
    ...
    case PLUS:
    {
        ...
        if (GET_CODE(op0) == SUBREG)
        {
            ...
            if (code0 == REG && REGNO(op0) < FIRST_PSEUDO_REGISTER)
                op0 = gen_rtx_REG(word_mode, REGNO(op0) + SUBREG_WORD(orig_op0));
        }
        ...
        if (GET_CODE(op1) == SUBREG)
        {
            ...
            if (code1 == REG && REGNO(op1) < FIRST_PSEUDO_REGISTER)
                op1 = gen_rtx_REG(GET_MODE(op1), REGNO(op1) + SUBREG_WORD(orig_op1));
        }

        if (code0 == MULT || code0 == SIGN_EXTEND || code0 == TRUNCATE || code0 == ZERO_EXTEND
            || code1 == MEM)
        {
            find_reloads_address_1(mode, orig_op0, 1, &XEXP(x, 0), opnum, type, ind_levels, insn);
            find_reloads_address_1(mode, orig_op1, 0, &XEXP(x, 1), opnum, type, ind_levels, insn);
        }

        else if (code1 == MULT || code1 == SIGN_EXTEND || code1 == TRUNCATE || code1 == ZERO_EXTEND
            || code0 == MEM)
        {
            find_reloads_address_1(mode, orig_op0, 0, &XEXP(x, 0), opnum, type, ind_levels, insn);
            find_reloads_address_1(mode, orig_op1, 1, &XEXP(x, 1), opnum, type, ind_levels, insn);
        }

        else if (code0 == CONST_INT || code0 == CONST || code0 == SYMBOL_REF || code0 == LABEL_REF)
            find_reloads_address_1(mode, orig_op1, 0, &XEXP(x, 1), opnum, type, ind_levels, insn);

        else if (code1 == CONST_INT || code1 == CONST || code1 == SYMBOL_REF || code1 == LABEL_REF)
            find_reloads_address_1(mode, orig_op0, 0, &XEXP(x, 0), opnum, type, ind_levels, insn);

        else if (code0 == REG && code1 == REG)
        {
            if (REG_OK_FOR_INDEX_P(op0) && REG_MODE_OK_FOR_BASE_P(op1, mode))
                return 0;
            else if (REG_OK_FOR_INDEX_P(op1) && REG_MODE_OK_FOR_BASE_P(op0, mode))
                return 0;
            else if (REG_MODE_OK_FOR_BASE_P(op1, mode))
                find_reloads_address_1(
                    mode, orig_op0, 1, &XEXP(x, 0), opnum, type, ind_levels, insn);
            else if (REG_MODE_OK_FOR_BASE_P(op0, mode))
                find_reloads_address_1(
                    mode, orig_op1, 1, &XEXP(x, 1), opnum, type, ind_levels, insn);
            else if (REG_OK_FOR_INDEX_P(op1))
                find_reloads_address_1(
                    mode, orig_op0, 0, &XEXP(x, 0), opnum, type, ind_levels, insn);
            else if (REG_OK_FOR_INDEX_P(op0))
                find_reloads_address_1(
                    mode, orig_op1, 0, &XEXP(x, 1), opnum, type, ind_levels, insn);
            else
            {
                find_reloads_address_1(
                    mode, orig_op0, 1, &XEXP(x, 0), opnum, type, ind_levels, insn);
    ...
    case PRE_DEC:
        if (GET_CODE(XEXP(x, 0)) == REG)
        {
            ...
            if (regno >= FIRST_PSEUDO_REGISTER && reg_equiv_constant[regno] != 0)
                abort();
                ...
                && (reg_equiv_address[regno] != 0 || num_not_at_initial_offset))
            {
                ...
                if (reg_equiv_address[regno] || !rtx_equal_p(tem, reg_equiv_mem[regno]))
                {
                    /* First reload the memory location's address.
                       ...
                       need two registers.  */
                    find_reloads_address(GET_MODE(tem), &tem, XEXP(tem, 0), &XEXP(tem, 0), opnum,
                                 ...
                                 : REGNO_MODE_OK_FOR_BASE_P(regno, mode))))
            {
               ...
               Also don't do this if we can probably update x directly.  */
                rtx equiv = (GET_CODE(XEXP(x, 0)) == MEM ? XEXP(x, 0) : reg_equiv_mem[regno]);
                           ...
                           && (*insn_operand_predicate[icode][1])(equiv, Pmode)))
                {
                    loc = &XEXP(x, 0);
                    x = XEXP(x, 0);
                    reloadnum = push_reload(x, x, loc, loc,
                        (context ? INDEX_REG_CLASS : MODE_BASE_REG_CLASS(mode)), GET_MODE(x),
                        GET_MODE(x), 0, 0, opnum, RELOAD_OTHER);
                ...
                else
                {
                    reloadnum = push_reload(x, NULL_RTX, loc, NULL,
                        (context ? INDEX_REG_CLASS : MODE_BASE_REG_CLASS(mode)), GET_MODE(x),
                        GET_MODE(x), 0, 0, opnum, type);
                    reload_inc[reloadnum] = find_inc_amount(PATTERN(this_insn), XEXP(x_orig, 0));
        ...
        else if (GET_CODE(XEXP(x, 0)) == MEM)
        {
            ...
            /* We can't use ADDR_TYPE (type) here, because we need to
               write back the value after reading it, hence we actually
               need two registers.  */
            find_reloads_address(GET_MODE(x), &XEXP(x, 0), XEXP(XEXP(x, 0), 0),
                &XEXP(XEXP(x, 0), 0), opnum, type, ind_levels, insn);

            reloadnum = push_reload(x, NULL_RTX, loc, NULL,
                (context ? INDEX_REG_CLASS : MODE_BASE_REG_CLASS(mode)), GET_MODE(x), VOIDmode, 0,
                0, opnum, type);
            reload_inc[reloadnum] = find_inc_amount(PATTERN(this_insn), XEXP(x, 0));
    ...
    case MEM:
       ...
       reload1.c here.  */
        ...
        find_reloads_address(
            GET_MODE(x), loc, XEXP(x, 0), &XEXP(x, 0), opnum, ADDR_TYPE(type), ind_levels, insn);
        push_reload(*loc, NULL_RTX, loc, NULL,
            (context ? INDEX_REG_CLASS : MODE_BASE_REG_CLASS(mode)), GET_MODE(x), VOIDmode, 0, 0,
            opnum, type);
        return 1;
    ...
    case REG:
    {
        ...
        if (reg_equiv_constant[regno] != 0)
        {
            find_reloads_address_part(reg_equiv_constant[regno], loc,
                (context ? INDEX_REG_CLASS : MODE_BASE_REG_CLASS(mode)), GET_MODE(x), opnum, type,
                ind_levels);
            return 1;
        }
	...
	if (reg_equiv_mem[regno] != 0)
	  {
            ...
            && (reg_equiv_address[regno] != 0 || num_not_at_initial_offset))
        {
            ...
            {
                x = tem;
                find_reloads_address(GET_MODE(x), &x, XEXP(x, 0), &XEXP(x, 0), opnum,
                    ADDR_TYPE(type), ind_levels, insn);
            }
        }
    ...
    case SUBREG:
        if (GET_CODE(SUBREG_REG(x)) == REG)
        {
            ...
            {
                enum reg_class rclass = (context ? INDEX_REG_CLASS : MODE_BASE_REG_CLASS(mode));
                if (CLASS_MAX_NREGS(rclass, GET_MODE(SUBREG_REG(x))) > reg_class_size[rclass])
                {
                    x = find_reloads_subreg_address(x, 0, opnum, type, ind_levels, insn);
                    push_reload(
                        x, NULL_RTX, loc, NULL, rclass, GET_MODE(x), VOIDmode, 0, 0, opnum, type);
                    return 1;
                }
            }
        }
...

    {
        ...
        for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
        {
            if (fmt[i] == 'e')
                find_reloads_address_1(
                    mode, XEXP(x, i), context, &XEXP(x, i), opnum, type, ind_levels, insn);
        }
    }
...
static void find_reloads_address_part(rtx x, rtx *loc, enum reg_class rclass, enum machine_mode mode,
    int opnum, enum reload_type type, int ind_levels)
{
    if (CONSTANT_P(x) && (!LEGITIMATE_CONSTANT_P(x) || PREFERRED_RELOAD_CLASS(x, rclass) == NO_REGS))
    {
        rtx tem;
       ...
       rtl_obstack == saveable_obstack is true at this point.  */
        ...
        find_reloads_address(mode, &tem, XEXP(tem, 0), &XEXP(tem, 0), opnum, type, ind_levels, 0);
    }

    else if (GET_CODE(x) == PLUS && CONSTANT_P(XEXP(x, 1))
    ...
    push_reload(x, NULL_RTX, loc, NULL, rclass, mode, VOIDmode, 0, 0, opnum, type);
}
...
static rtx find_reloads_subreg_address(
    rtx x, int force_replace, int opnum, enum reload_type type, int ind_levels, rtx insn)
{
    int regno = REGNO(SUBREG_REG(x));
    ...
    if (reg_equiv_memory_loc[regno])
    {
       ...
       offsettable, then it must be replaced.  */
        if (!force_replace
            && (reg_equiv_address[regno] || !offsettable_memref_p(reg_equiv_mem[regno])))
            force_replace = 1;
...
void subst_reloads(void)
{
    ...
    for (i = 0; i < n_replacements; i++)
    {
        ...
        if (reloadreg)
        {
            ...
            {
                if (GET_MODE(*r->subreg_loc) == GET_MODE(SUBREG_REG(reloadreg)))
    ...
    for (r = &replacements[0]; r < &replacements[n_replacements]; r++)
    {
        ...
        else if (reloadreg && r->subreg_loc == loc)
        {
            ...
            if (GET_CODE(reloadreg) == REG)
                return gen_rtx_REG(GET_MODE(*loc), REGNO(reloadreg) + SUBREG_WORD(*loc));
...
int refers_to_regno_for_reload_p(int regno, int endregno, rtx x, rtx *loc)
{
    int i;
    RTX_CODE code;
    const char *fmt;
...
repeat:
       ...
       X must therefore either be a constant or be in memory.  */
        if (i >= FIRST_PSEUDO_REGISTER)
        {
            if (reg_equiv_memory_loc[i])
                return refers_to_regno_for_reload_p(regno, endregno, reg_equiv_memory_loc[i], NULL);
        ...
        return (endregno > i
            && regno < i + (i < FIRST_PSEUDO_REGISTER ? HARD_REGNO_NREGS(i, GET_MODE(x)) : 1));
    ...
    case SUBREG:
       ...
       registers are being modified.  Otherwise, handle normally.  */
        if (GET_CODE(SUBREG_REG(x)) == REG && REGNO(SUBREG_REG(x)) < FIRST_PSEUDO_REGISTER)
        {
            ...
            int inner_endregno = inner_regno
                + (inner_regno < FIRST_PSEUDO_REGISTER ? HARD_REGNO_NREGS(regno, GET_MODE(x)) : 1);
    ...
    case SET:
        if (&SET_DEST(x) != loc
            ...
            && ((GET_CODE(SET_DEST(x)) == SUBREG && loc != &SUBREG_REG(SET_DEST(x))
                    && GET_CODE(SUBREG_REG(SET_DEST(x))) == REG
                    && REGNO(SUBREG_REG(SET_DEST(x))) >= FIRST_PSEUDO_REGISTER
                    && refers_to_regno_for_reload_p(regno, endregno, SUBREG_REG(SET_DEST(x)), loc))
                   /* If the output is an earlyclobber operand, this is
                  a conflict.  */
                   || ((GET_CODE(SET_DEST(x)) != REG || earlyclobber_operand_p(SET_DEST(x)))
                          && refers_to_regno_for_reload_p(regno, endregno, SET_DEST(x), loc))))
            return 1;
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        if (fmt[i] == 'e' && loc != &XEXP(x, i))
        {
            ...
            else if (refers_to_regno_for_reload_p(regno, endregno, XEXP(x, i), loc))
                return 1;
        }
        else if (fmt[i] == 'E')
        {
            ...
            for (j = XVECLEN(x, i) - 1; j >= 0; j--)
                if (loc != &XVECEXP(x, i, j)
                    && refers_to_regno_for_reload_p(regno, endregno, XVECEXP(x, i, j), loc))
                    return 1;
        }
    }
    return 0;
...
int reg_overlap_mentioned_for_reload_p(rtx x, rtx in)
{
    ...
    else if (GET_CODE(x) == REG)
    {
       ...
       Therefore, it must either be in memory or be a constant.  */
        ...
        return refers_to_mem_for_reload_p(in);
    else if (GET_CODE(x) == SCRATCH || GET_CODE(x) == PC || GET_CODE(x) == CC0)
        return reg_mentioned_p(x, in);
    else
        abort();

    endregno = regno + (regno < FIRST_PSEUDO_REGISTER ? HARD_REGNO_NREGS(regno, GET_MODE(x)) : 1);
...
int refers_to_mem_for_reload_p(rtx x)
{
    ...
    if (GET_CODE(x) == REG)
        return (REGNO(x) >= FIRST_PSEUDO_REGISTER && reg_equiv_memory_loc[REGNO(x)]);

    fmt = GET_RTX_FORMAT(GET_CODE(x));
    for (i = GET_RTX_LENGTH(GET_CODE(x)) - 1; i >= 0; i--)
        if (fmt[i] == 'e'
            && (GET_CODE(XEXP(x, i)) == MEM || refers_to_mem_for_reload_p(XEXP(x, i))))
            return 1;
...
rtx find_equiv_reg(rtx goal, rtx insn, enum reg_class rclass, int other,
    short *reload_reg_p, int goalreg, enum machine_mode mode)
{
    ...
    else if (GET_CODE(goal) == MEM)
    {
        ...
        goal_const = 1;
    else if (GET_CODE(goal) == PLUS && XEXP(goal, 0) == stack_pointer_rtx
        && CONSTANT_P(XEXP(goal, 1)))
        goal_const = need_stable_sp = 1;
    else if (GET_CODE(goal) == PLUS && XEXP(goal, 0) == frame_pointer_rtx
        && CONSTANT_P(XEXP(goal, 1)))
        goal_const = 1;
    else
        return 0;
...
#ifdef OVERLAPPING_REGNO_P
    if (regno >= 0 && regno < FIRST_PSEUDO_REGISTER && OVERLAPPING_REGNO_P(regno))
        return 0;
#endif
    ...
    while (1)
    {
        ...
        if (GET_CODE(p) == INSN
            ...
            && (!(reload_reg_p != 0 && reload_reg_p != (short *)(int32_t)1)
                   ...
                   || INSN_UID(p) < reload_first_uid))
        {
            ...
            if (pat != 0
                && ((regno >= 0 && true_regnum(SET_SRC(pat)) == regno
                        && (valueno = true_regnum(valtry = SET_DEST(pat))) >= 0)
                       || (regno >= 0 && true_regnum(SET_DEST(pat)) == regno
                              && (valueno = true_regnum(valtry = SET_SRC(pat))) >= 0)
                       || (goal_const
                              && rtx_equal_p(SET_SRC(pat), goal)
                              ...
                              && (valueno = true_regnum(valtry = SET_DEST(pat))) >= 0)
                       || (goal_mem && (valueno = true_regnum(valtry = SET_DEST(pat))) >= 0
                              && rtx_renumbered_equal_p(goal, SET_SRC(pat)))
                       || (goal_mem && (valueno = true_regnum(valtry = SET_SRC(pat))) >= 0
                       ...
                       || (goal_const && (tem = find_reg_note(p, REG_EQUIV, NULL_RTX))
                       ...
                       || (goal_const && (tem = find_reg_note(p, REG_EQUIV, NULL_RTX))
                              ...
                              && 0 != (goaltry = operand_subword(XEXP(tem, 0), 0, 0, VOIDmode))
                              && rtx_equal_p(goal, goaltry)
                              && (valtry = operand_subword(SET_DEST(pat), 0, 0, VOIDmode))
                       ...
                       || (goal_const && (tem = find_reg_note(p, REG_EQUIV, NULL_RTX))
                              ...
                              && GET_MODE_CLASS(GET_MODE(XEXP(tem, 0))) == MODE_FLOAT
                              && GET_CODE(goal) == CONST_INT
                              && 0 != (goaltry = operand_subword(XEXP(tem, 0), 1, 0, VOIDmode))
                              && rtx_equal_p(goal, goaltry)
                              && (valtry = operand_subword(SET_DEST(pat), 1, 0, VOIDmode))
                              && (valueno = true_regnum(valtry)) >= 0)))
                if (other >= 0 ? valueno == other
                               : ((unsigned)valueno < FIRST_PSEUDO_REGISTER
                                     && TEST_HARD_REG_BIT(reg_class_contents[(int)rclass], valueno)))
                {
                    value = valtry;
                    where = p;
                    break;
    ...
    if (valueno == STACK_POINTER_REGNUM || regno == STACK_POINTER_REGNUM
        || (goal_mem && reg_overlap_mentioned_for_reload_p(stack_pointer_rtx, goal)))
        need_stable_sp = 1;
    ...
    if (goal_mem && value == SET_DEST(single_set(where))
        && refers_to_regno_for_reload_p(
               valueno, (valueno + HARD_REGNO_NREGS(valueno, mode)), goal, NULL))
        return 0;

    /* Reject registers that overlap GOAL.  */

    if (!goal_mem && !goal_const && regno + HARD_REGNO_NREGS(regno, mode) > valueno
    ...
    if (reload_reg_p != 0 && reload_reg_p != (short *)(int32_t)1
    ...
    if (reload_reg_p != 0)
    {
        ...
        for (i = 0; i < n_reloads; i++)
            if (reload_reg_rtx[i] != 0 && reload_in[i])
            {
                ...
                int nregs1 = HARD_REGNO_NREGS(regno1, GET_MODE(reload_reg_rtx[i]));
    ...
    while (1)
    {
       ...
       if either of the two is in a call-clobbered register, or memory.  */
        if (GET_CODE(p) == CALL_INSN
            && ((regno >= 0 && regno < FIRST_PSEUDO_REGISTER && call_used_regs[regno])
                   || (valueno >= 0 && valueno < FIRST_PSEUDO_REGISTER && call_used_regs[valueno])
                   || goal_mem || need_stable_sp))
            return 0;


#ifdef INSN_CLOBBERS_REGNO_P
        if ((valueno >= 0 && valueno < FIRST_PSEUDO_REGISTER && INSN_CLOBBERS_REGNO_P(p, valueno))
            || (regno >= 0 && regno < FIRST_PSEUDO_REGISTER && INSN_CLOBBERS_REGNO_P(p, regno)))
            return 0;
#endif
        ...
        if (GET_RTX_CLASS(GET_CODE(p)) == 'i')
        {
               ...
               and this insn P increments a register used in GOAL, return 0.  */
            {
                ...
                for (link = REG_NOTES(p); link; link = XEXP(link, 1))
                    if (REG_NOTE_KIND(link) == REG_INC && GET_CODE(XEXP(link, 0)) == REG)
                    {
...
static int find_inc_amount(rtx x, rtx inced)
{
    ...
    {
        rtx addr = XEXP(x, 0);
        if ((GET_CODE(addr) == PRE_DEC || GET_CODE(addr) == POST_DEC || GET_CODE(addr) == PRE_INC
                || GET_CODE(addr) == POST_INC)
            && XEXP(addr, 0) == inced)
...
int regno_clobbered_p(int regno, rtx insn)
{
    if (GET_CODE(PATTERN(insn)) == CLOBBER && GET_CODE(XEXP(PATTERN(insn), 0)) == REG)
