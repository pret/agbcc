...
static struct decision *add_to_sequence(rtx, struct decision_head *, const char *);
static int not_both_true(struct decision *, struct decision *, int);
static int position_merit(struct decision *, enum machine_mode, enum rtx_code);
static struct decision_head merge_trees(struct decision_head, struct decision_head);
static int break_out_subroutines(struct decision_head, enum routine_type, int);
static void write_subroutine(struct decision *, enum routine_type);
static void write_tree_1(struct decision *, const char *, struct decision *, enum routine_type);
...
static void write_tree(struct decision *, const char *, struct decision *, int, enum routine_type);
...
restart:
    ...
    case SET:
        ...
        if (GET_MODE(SET_SRC(pattern)) != VOIDmode && GET_MODE(SET_DEST(pattern)) != VOIDmode
...
static int not_both_true(struct decision *d1, struct decision *d2, int toplevel)
{
    ...
    if ((d1->enforce_mode && d2->enforce_mode && d1->mode != VOIDmode && d2->mode != VOIDmode
            && d1->mode != d2->mode)
        || (d1->code != UNKNOWN && d2->code != UNKNOWN && d1->code != d2->code)
        || (d1->test_elt_zero_int && d2->test_elt_zero_int && d1->elt_zero_int != d2->elt_zero_int)
        || (d1->test_elt_one_int && d2->test_elt_one_int && d1->elt_one_int != d2->elt_one_int)
    ...
    if ((d1->tests == 0 && d1->code == UNKNOWN) || (d2->tests == 0 && d2->code == UNKNOWN))
        return 0;
...
static int position_merit(struct decision *p, enum machine_mode mode, enum rtx_code code)
{
    ...
    if ((code != UNKNOWN
        ...
        || (code == UNKNOWN && p_mode == mode
               && (p->next == 0 || (p->next->enforce_mode ? p->next->mode : VOIDmode) != mode)))
        return 1;
    ...
    if (p_mode != mode
        && ((mode != VOIDmode && p_mode != VOIDmode) || (mode == VOIDmode && p->next == 0)))
        return 2;
...
static struct decision_head merge_trees(struct decision_head oldh, struct decision_head addh)
{
    ...
    for (add = addh.first; add; add = next)
    {
       ...
       If we have no match, place NEW after the closest match we found.  */
        ...
        for (old = oldh.last; old; old = old->prev)
        {
            ...
            if (old->tests == 0 && old->test_elt_zero_int == 0 && old->test_elt_one_int == 0
                && old->veclen == 0 && old->test_elt_zero_wide == 0 && old->dupno == -1
                && old->mode == VOIDmode && old->code == UNKNOWN
                && (old->c_test != 0 || add->c_test != 0))
                ;

            else if ((old->tests == add->tests || (old->pred >= 0 && old->pred == add->pred)
                ...
                && old->elt_zero_wide == add->elt_zero_wide && old->veclen == add->veclen
                && old->dupno == add->dupno && old->opno == add->opno && old->code == add->code
                && old->enforce_mode == add->enforce_mode && old->mode == add->mode)
            {
               ...
               with different tests on target flags.  */
                    ...
                    && !(old->c_test && add->c_test && !strcmp(old->c_test, add->c_test)))
                {
                    if (old->insn_code_number >= 0 || old->opno >= 0)
                    {
                        ...
                        mybcopy((char *)old, (char *)split, sizeof(struct decision));
                    ...
                    if (add->insn_code_number >= 0 || add->opno >= 0)
                    {
                        ...
                        mybcopy((char *)add, (char *)split, sizeof(struct decision));
                ...
                if (old->insn_code_number >= 0 && add->insn_code_number >= 0)
                {
                    ...
                    if (old->num_clobbers_to_add == 0 && add->num_clobbers_to_add > 0)
                        /* Nothing to do here.  */
                        ;
                    else if (old->num_clobbers_to_add > 0 && add->num_clobbers_to_add == 0)
                    {
                        ...
                        fatal(
                            "Two actions at one point in tree for insns \"%s\" (%d) and \"%s\" (%d)",
            ...
            if (best_merit != 0
                && ((our_merit = position_merit(old, add_mode, add->code)) < best_merit))
       ...
       the list, it might be better to insert at the top.  */
        ...
        if (old == 0 && position_merit(NULL, add_mode, add->code) < best_merit)
        {
            add->prev = 0;
            add->next = oldh.first;
            oldh.first->prev = add;
...
static int break_out_subroutines(struct decision_head head, enum routine_type type, int initial)
{
    int size = 0;
    struct decision *sub;
...
static void write_subroutine(struct decision *tree, enum routine_type type)
{
    int i;
    ...
    for (p = tree; p; p = p->next)
    {
       ...
       node needs a label and to retest either the mode or code.  */
                   ...
                   || (p->label_needed && (p->retest_mode || p->retest_code))))
        {
            ...
            if (code == UNKNOWN || codemap[(int)code] || switch_mode != mode
                || (p->label_needed && (p->retest_mode || p->retest_code)))
            {
                printf("%s}\n", indents[indent - 2]);
                switch_code = UNKNOWN;
       ...
       not testing a mode or testing a mode whose case we already saw.  */
        ...
        if (switch_mode != VOIDmode && (switch_mode != mode || (p->label_needed && p->retest_mode)))
        {
            if (mode == VOIDmode || modemap[(int)mode] || (p->label_needed && p->retest_mode))
            {
                printf("%s}\n", indents[indent - 2]);
       ...
       more than one mode, but all of the same mode).  */
        ...
        if (switch_mode == VOIDmode && mode != VOIDmode && p->next != 0 && p->next->enforce_mode
            && p->next->mode != VOIDmode)
        {
            mybzero(modemap, sizeof modemap);
            printf("%sswitch (GET_MODE (x%d))\n", indents[indent], depth);
            printf("%s{\n", indents[indent + 2]);
            indent += 4;
            printf("%sdefault:\n%sbreak;\n", indents[indent - 2], indents[indent]);
            printf("%scase %smode:\n", indents[indent - 2], GET_MODE_NAME(mode));
            modemap[(int)mode] = 1;
            switch_mode = mode;
        }
        ...
        if (switch_code == UNKNOWN && p->code != UNKNOWN && !p->ignore_code && p->next != 0
            && p->next->code != UNKNOWN)
        {
       ...
       the only way the switch and test modes can disagree.  */
        ...
        if ((mode != switch_mode && !p->ignore_mode)
            ...
            || p->test_elt_zero_int || p->test_elt_one_int || p->test_elt_zero_wide || p->veclen
            || p->dupno >= 0 || p->tests || p->num_clobbers_to_add)
        {
            printf("%sif (", indents[indent]);

            if (mode != switch_mode && !p->ignore_mode)
                printf("GET_MODE (x%d) == %smode && ", depth, GET_MODE_NAME(mode));
            ...
            if (p->test_elt_zero_wide)
            {
                /* Set offset to 1 iff the number might get propagated to
                   ...
                   for 1 << 31 .  */
                int32_t offset = p->elt_zero_wide == -2147483647 - 1;
                printf("XWINT (x%d, 0) == ", depth);
                printf(int32_t_PRINT_DEC, p->elt_zero_wide + offset);
                printf("%s && ", offset ? "-1" : "");
            }
            if (p->veclen)
        ...
        if (p->insn_code_number >= 0)
        {
            ...
            printf("%sgoto L%d;\n", indents[inner_indent], p->success.first->number);
    ...
    int initial, enum routine_type type)
{
    ...
    if (!initial && tree->subroutine_number > 0)
    {
        ...
        if (afterward)
        {
            ...
            printf("  return %s_%d (x0, insn%s);\n", name_prefix, tree->subroutine_number,
                call_suffix);
        return;
    }
    ...
    for (p = tree; p; p = p->next)
        if (p->success.first)
            write_tree(
                p->success.first, p->position, p->afterward ? p->afterward : afterward, 0, type);
}
...
static void change_state(const char *oldpos, const char *newpos, int indent)
{
...
static void mybzero(char *b, unsigned length)
{
    while (length-- > 0)
        *b++ = 0;
}

static void mybcopy(char *in, char *out, unsigned length)
{
    while (length-- > 0)
        *out++ = *in++;
}
...
from the machine description file `md'.  */\n\n");
    ...
    while (1)
    {
        ...
        if (GET_CODE(desc) == DEFINE_INSN)
            recog_tree = merge_trees(recog_tree, make_insn_sequence(desc, RECOG));
        else if (GET_CODE(desc) == DEFINE_SPLIT)
            split_tree = merge_trees(split_tree, make_insn_sequence(desc, SPLIT));
        if (GET_CODE(desc) == DEFINE_PEEPHOLE || GET_CODE(desc) == DEFINE_EXPAND)
            next_insn_code++;
        next_index++;
    }
