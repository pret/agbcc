...
static void max_operand_1(rtx x)
{
    ...
    if (code == MATCH_SCRATCH && XSTR(x, 1) != 0 && *XSTR(x, 1) != '\0')
        register_constraints = 1;
    if (code == MATCH_OPERAND || code == MATCH_OPERATOR || code == MATCH_PARALLEL)
...
static void gen_expand(rtx expand)
{
    ...
    if ((XSTR(expand, 3) == 0 || *XSTR(expand, 3) == '\0') && operands > max_dup_opno
        && XVECLEN(expand, 1) == 1)
    {
        printf("  return ");
...
static void gen_split(rtx split)
{
    ...
    if (XVEC(split, 0) == 0)
        ...
        fatal("define_split (definition %d) lacks a replacement pattern", insn_index_number);
...
static void output_add_clobbers(void)
{
    ...
    printf("\n\nvoid add_clobbers (rtx pattern, int insn_code_number)\n");
    printf("{\n");
...
static void output_init_mov_optab(void)
{
    ...
    for (i = 0; i < sizeof cc_names / sizeof cc_names[0]; i++)
    {
        ...
        for (p = cc_names[i]; *p; p++)
            printf("%c", *p >= 'A' && *p <= 'Z' ? *p - 'A' + 'a' : *p);
        printf(")\n");
        printf("    mov_optab->handlers[(int) %smode].insn_code = CODE_FOR_mov", cc_names[i]);
        for (p = cc_names[i]; *p; p++)
            printf("%c", *p >= 'A' && *p <= 'Z' ? *p - 'A' + 'a' : *p);
        printf(";\n#endif\n");
    }
