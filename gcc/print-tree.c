...
void print_node_brief(FILE *file, char *prefix, tree node, int indent)
{
    ...
    else if (class == 't')
    {
        if (TYPE_NAME(node))
        {
            if (TREE_CODE(TYPE_NAME(node)) == IDENTIFIER_NODE)
                ...
                fprintf(file, " %s", IDENTIFIER_POINTER(DECL_NAME(TYPE_NAME(node))));
        }
    }
    ...
    if (TREE_CODE(node) == INTEGER_CST)
    {
        ...
        if (TREE_INT_CST_HIGH(node) == 0)
            fprintf(file, int32_t_PRINT_UNSIGNED, TREE_INT_CST_LOW(node));
        else if (TREE_INT_CST_HIGH(node) == -1 && TREE_INT_CST_LOW(node) != 0)
        {
            ...
            fprintf(file, int32_t_PRINT_DOUBLE_HEX, TREE_INT_CST_HIGH(node),
...
void print_node(FILE *file, char *prefix, tree node, int indent)
{
    ...
    case 'd':
        ...
        fprintf(file, " file %s line %d", DECL_SOURCE_FILE(node), DECL_SOURCE_LINE(node));
        ...
        print_node_brief(file, "abstract_origin", DECL_ABSTRACT_ORIGIN(node), indent + 4);
    ...
    case 't':
        ...
        else if (TREE_CODE(node) == INTEGER_TYPE || TREE_CODE(node) == BOOLEAN_TYPE
    ...
    case 'b':
        ...
        print_node(file, "abstract_origin", BLOCK_ABSTRACT_ORIGIN(node), indent + 4);
        break;
    ...
    case 's':
       ...
       after a certain point.  Print the rtx's as rtx's.  */
        ...
        if (TREE_CODE(node) == EXPR_WITH_FILE_LOCATION)
        {
            indent_to(file, indent + 4);
            fprintf(file, "%s:%d:%d",
                (EXPR_WFL_FILENAME_NODE(node) ? EXPR_WFL_FILENAME(node) : "(no file info)"),
