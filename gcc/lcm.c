...
static void compute_antinout(int, int_list_ptr *, sbitmap *, sbitmap *, sbitmap *, sbitmap *);
static void compute_earlyinout(
    int, int, int_list_ptr *, sbitmap *, sbitmap *, sbitmap *, sbitmap *);
static void compute_delayinout(
    int, int, int_list_ptr *, sbitmap *, sbitmap *, sbitmap *, sbitmap *, sbitmap *);
static void compute_latein(int, int, int_list_ptr *, sbitmap *, sbitmap *, sbitmap *);
static void compute_isoinout(int, int_list_ptr *, sbitmap *, sbitmap *, sbitmap *, sbitmap *);
static void compute_optimal(int, sbitmap *, sbitmap *, sbitmap *);
static void compute_redundant(int, int, sbitmap *, sbitmap *, sbitmap *, sbitmap *);

/* Similarly, but for the reversed flowgraph.  */
static void compute_avinout(int, int_list_ptr *, sbitmap *, sbitmap *, sbitmap *, sbitmap *);
static void compute_fartherinout(
    int, int, int_list_ptr *, sbitmap *, sbitmap *, sbitmap *, sbitmap *);
static void compute_earlierinout(
    int, int, int_list_ptr *, sbitmap *, sbitmap *, sbitmap *, sbitmap *, sbitmap *);
static void compute_firstout(int, int, int_list_ptr *, sbitmap *, sbitmap *, sbitmap *);
static void compute_rev_isoinout(int, int_list_ptr *, sbitmap *, sbitmap *, sbitmap *, sbitmap *);
...
void pre_lcm(int n_blocks, int n_exprs, int_list_ptr *s_preds, int_list_ptr *s_succs,
    sbitmap *transp, sbitmap *antloc, sbitmap *redundant, sbitmap *optimal)
{
    ...
    compute_earlyinout(n_blocks, n_exprs, s_preds, transp, antin, earlyin, earlyout);
    free(earlyout);
    earlyout = NULL;
    ...
    compute_delayinout(n_blocks, n_exprs, s_preds, antloc, antin, earlyin, delayin, delayout);
    free(delayout);
    delayout = NULL;
    free(antin);
    antin = NULL;
...
void pre_rev_lcm(int n_blocks, int n_exprs, int_list_ptr *s_preds, int_list_ptr *s_succs,
    sbitmap *transp, sbitmap *avloc, sbitmap *redundant, sbitmap *optimal)
{
    ...
    compute_fartherinout(n_blocks, n_exprs, s_succs, transp, avout, fartherin, fartherout);
    free(fartherin);
    fartherin = NULL;
    ...
    compute_earlierinout(
        n_blocks, n_exprs, s_succs, avloc, avout, fartherout, earlierin, earlierout);
    free(earlierin);
    earlierin = NULL;
    free(avout);
    avout = NULL;
    ...
    compute_rev_isoinout(n_blocks, s_preds, avloc, firstout, rev_isoin, rev_isoout);
    free(rev_isoout);
    rev_isoout = NULL;
...
static void compute_antinout(int n_blocks, int_list_ptr *s_succs, sbitmap *antloc, sbitmap *transp,
    sbitmap *antin, sbitmap *antout)
{
    ...
    while (changed)
    {
       ...
       the convergence.  */
        for (bb = n_blocks - 1; bb >= 0; bb--)
        {
            ...
            {
                if (INT_LIST_VAL(ps) == EXIT_BLOCK || INT_LIST_VAL(ps) == ENTRY_BLOCK)
                    break;
            ...
            if (bb != n_blocks - 1)
                sbitmap_intersect_of_successors(antout[bb], antin, bb, s_succs);
...
static void compute_earlyinout(int n_blocks, int n_exprs, int_list_ptr *s_preds, sbitmap *transp,
    sbitmap *antin, sbitmap *earlyin, sbitmap *earlyout)
{
    int bb, changed, passes;
    sbitmap temp_bitmap;
    sbitmap old_changed, new_changed;
    ...
    while (changed)
    {
        ...
        for (bb = 0; bb < n_blocks; bb++)
        {
            ...
            if (bb != 0)
                sbitmap_union_of_predecessors(earlyin[bb], earlyout, bb, s_preds);
            sbitmap_not(temp_bitmap, transp[bb]);
            if (sbitmap_union_of_diff(earlyout[bb], temp_bitmap, earlyin[bb], antin[bb]))
            {
                changed = 1;
                SET_BIT(new_changed, bb);
            }
...
static void compute_delayinout(int n_blocks, int n_exprs, int_list_ptr *s_preds, sbitmap *antloc,
    sbitmap *antin, sbitmap *earlyin, sbitmap *delayin, sbitmap *delayout)
{
    int bb, changed, passes;
    ...
    while (changed)
    {
        ...
        for (bb = 0; bb < n_blocks; bb++)
        {
            if (bb != 0)
            {
                sbitmap_intersect_of_predecessors(temp_bitmap, delayout, bb, s_preds);
                changed |= sbitmap_a_or_b(delayin[bb], anti_and_early[bb], temp_bitmap);
            }
            sbitmap_not(temp_bitmap, antloc[bb]);
            changed |= sbitmap_a_and_b(delayout[bb], temp_bitmap, delayin[bb]);
        }
        passes++;
    }
...
static void compute_latein(int n_blocks, int n_exprs, int_list_ptr *s_succs, sbitmap *antloc,
    sbitmap *delayin, sbitmap *latein)
{
    int bb;
    sbitmap temp_bitmap;
    ...
    for (bb = 0; bb < n_blocks; bb++)
    {
       ...
       temp_bitmap will not be set by the following call!  */
        if (bb == n_blocks - 1)
        {
            sbitmap_intersect_of_successors(temp_bitmap, delayin, bb, s_succs);
            sbitmap_not(temp_bitmap, temp_bitmap);
        }
        else
            sbitmap_ones(temp_bitmap);
        sbitmap_a_and_b_or_c(latein[bb], delayin[bb], antloc[bb], temp_bitmap);
    }
    free(temp_bitmap);
}
...
static void compute_isoinout(int n_blocks, int_list_ptr *s_succs, sbitmap *antloc, sbitmap *latein,
    sbitmap *isoin, sbitmap *isoout)
{
    int bb, changed, passes;
    ...
    while (changed)
    {
        ...
        {
            if (bb != n_blocks - 1)
                sbitmap_intersect_of_successors(isoout[bb], isoin, bb, s_succs);
            changed |= sbitmap_union_of_diff(isoin[bb], latein[bb], isoout[bb], antloc[bb]);
        }
        passes++;
    }
}
...
static void compute_optimal(int n_blocks, sbitmap *latein, sbitmap *isoout, sbitmap *optimal)
{
    int bb;
...
static void compute_redundant(int n_blocks, int n_exprs, sbitmap *antloc, sbitmap *latein,
    sbitmap *isoout, sbitmap *redundant)
{
    int bb;
    sbitmap temp_bitmap;
...
static void compute_avinout(int n_blocks, int_list_ptr *s_preds, sbitmap *avloc, sbitmap *transp,
    sbitmap *avin, sbitmap *avout)
{
    ...
    while (changed)
    {
        ...
        {
            if (bb != 0)
                sbitmap_intersect_of_predecessors(avin[bb], avout, bb, s_preds);
            changed |= sbitmap_a_or_b_and_c(avout[bb], avloc[bb], transp[bb], avin[bb]);
        }
        passes++;
    }
}
...
static void compute_fartherinout(int n_blocks, int n_exprs, int_list_ptr *s_succs, sbitmap *transp,
    sbitmap *avout, sbitmap *fartherin, sbitmap *fartherout)
{
    int bb, changed, passes;
    sbitmap temp_bitmap;
    ...
    while (changed)
    {
        ...
        {
            if (bb != n_blocks - 1)
                sbitmap_union_of_successors(fartherout[bb], fartherin, bb, s_succs);
            sbitmap_not(temp_bitmap, transp[bb]);
            changed |= sbitmap_union_of_diff(fartherin[bb], temp_bitmap, fartherout[bb], avout[bb]);
        }
        passes++;
    }
...
static void compute_earlierinout(int n_blocks, int n_exprs, int_list_ptr *s_succs, sbitmap *avloc,
    sbitmap *avout, sbitmap *fartherout, sbitmap *earlierin, sbitmap *earlierout)
{
    int bb, changed, passes;
    ...
    while (changed)
    {
        ...
        for (bb = n_blocks - 1; bb >= 0; bb--)
        {
            if (bb != n_blocks - 1)
            {
                sbitmap_intersect_of_successors(temp_bitmap, earlierin, bb, s_succs);
                changed |= sbitmap_a_or_b(earlierout[bb], av_and_farther[bb], temp_bitmap);
            }
            sbitmap_not(temp_bitmap, avloc[bb]);
            changed |= sbitmap_a_and_b(earlierin[bb], temp_bitmap, earlierout[bb]);
        }
        passes++;
    }
...
static void compute_firstout(int n_blocks, int n_exprs, int_list_ptr *s_preds, sbitmap *avloc,
    sbitmap *earlierout, sbitmap *firstout)
{
    int bb;
    sbitmap temp_bitmap;
    ...
    for (bb = 0; bb < n_blocks; bb++)
    {
       ...
       temp_bitmap will not be set by the following call!  */
        if (bb != 0)
        {
            sbitmap_intersect_of_predecessors(temp_bitmap, earlierout, bb, s_preds);
            sbitmap_not(temp_bitmap, temp_bitmap);
        }
        else
        {
        ...
        sbitmap_a_and_b_or_c(firstout[bb], earlierout[bb], avloc[bb], temp_bitmap);
    }
    free(temp_bitmap);
}
...
static void compute_rev_isoinout(int n_blocks, int_list_ptr *s_preds, sbitmap *avloc,
    sbitmap *firstout, sbitmap *rev_isoin, sbitmap *rev_isoout)
{
    int bb, changed, passes;
    ...
    while (changed)
    {
        ...
        {
            if (bb != 0)
                sbitmap_intersect_of_predecessors(rev_isoin[bb], rev_isoout, bb, s_preds);
            changed
                |= sbitmap_union_of_diff(rev_isoout[bb], firstout[bb], rev_isoin[bb], avloc[bb]);
        }
        passes++;
    }
}
