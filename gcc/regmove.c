...
static int try_auto_increment(rtx, rtx, rtx, rtx, int32_t, int);
...
static struct related *optimize_related_values_1(struct related *, int, int, rtx, FILE *);
...
static int regclass_compatible_p(int class0, int class1)
{
    return (class0 == class1
        ...
        || (reg_class_subset_p(class1, class0) && !CLASS_LIKELY_SPILLED_P(class1)));
}

/* Generate and return an insn body to add r1 and c,
   storing the result in r0.  */
static rtx gen_add3_insn(rtx r0, rtx r1, rtx c)
{
...
static int try_auto_increment(
    rtx insn, rtx inc_insn, rtx inc_insn_set, rtx reg, int32_t increment, int pre)
{
    ...
    if (pset)
    {
       ...
       (sign_extend:SI (mem:QI ...  */
        ...
        if (use != 0 && use != (rtx)1)
        {
            ...
            if (0 || (HAVE_POST_INCREMENT && pre == 0 && (inc_code = POST_INC, increment == size))
                || (HAVE_PRE_INCREMENT && pre == 1 && (inc_code = PRE_INC, increment == size))
                || (HAVE_POST_DECREMENT && pre == 0 && (inc_code = POST_DEC, increment == -size))
                || (HAVE_PRE_DECREMENT && pre == 1 && (inc_code = PRE_DEC, increment == -size)))
            {
                if (inc_insn_set)
                    validate_change(
                        inc_insn, &SET_SRC(inc_insn_set), XEXP(SET_SRC(inc_insn_set), 0), 1);
                validate_change(insn, &XEXP(use, 0), gen_rtx_fmt_e(inc_code, Pmode, reg), 1);
...
#define REL_USE_HASH(I) ((I) % (uint32_t)REL_USE_HASH_SIZE)
...
struct rel_use_chain
{
    ...
    int32_t match_offset; /* Offset after execution of last insn. */
};
...
static int32_t add_limits[NUM_MACHINE_MODES][2];
...
static void rel_build_chain(struct rel_use *new_use, struct rel_use *match, int base)
{
    int hash;
...
/* Record the use of register ADDR in a memory reference.
   ...
   function.  */
static void rel_record_mem(
    rtx *addrp, rtx addr, int size, int pre_offs, int post_offs, rtx insn, int luid, int call_tally)
{
    static rtx auto_inc;
    rtx orig_addr = *addrp;
    int regno, base;
...
static void invalidate_related(rtx reg, int luid)
{
    int regno = REGNO(reg);
...
static void find_related(rtx *xp, rtx insn, int luid, int call_tally)
{
    rtx x = *xp;
    ...
    case SET:
    {
        ...
        do
        {
            ...
            if (GET_CODE(src_reg) != REG || GET_CODE(src_const) != CONST_INT
                || GET_CODE(dst) != REG)
                break;
            dst_regno = REGNO(dst);
            src_regno = REGNO(src_reg);
            if (src_regno < FIRST_PSEUDO_REGISTER || dst_regno < FIRST_PSEUDO_REGISTER)
                break;
            ...
            if ((regno_related[src_regno] && regno_related[src_regno]->invalidate_luid)
                || (regno_related[dst_regno] && regno_related[dst_regno]->invalidate_luid))
                break;
            ...
            if (regno_related[dst_regno] && regno_related[dst_regno]->insn)
            {
                ...
                if (!regno_related[src_regno] || !regno_related[src_regno]->insn
                    || (regno_related[dst_regno]->u.base != regno_related[src_regno]->u.base))
                    break;
            ...
            new_related->offset = regno_related[src_regno]->offset + INTVAL(XEXP(src, 1));
            new_related->invalidate_luid = 0;
            new_related->death = NULL_RTX;
        ...
        while (GET_CODE(dst) == SUBREG || GET_CODE(dst) == ZERO_EXTRACT
            || GET_CODE(dst) == SIGN_EXTRACT || GET_CODE(dst) == STRICT_LOW_PART)
            dst = XEXP(dst, 0);
        if (GET_CODE(dst) == REG)
        {
    ...
    case MEM:
    {
        ...
        {
        case REG:
            rel_record_mem(addrp, addr, size, 0, 0, insn, luid, call_tally);
            return;
        case PRE_INC:
            rel_record_mem(addrp, XEXP(addr, 0), size, size, 0, insn, luid, call_tally);
            return;
        case POST_INC:
            rel_record_mem(addrp, XEXP(addr, 0), size, 0, size, insn, luid, call_tally);
            return;
        case PRE_DEC:
            rel_record_mem(addrp, XEXP(addr, 0), size, -size, 0, insn, luid, call_tally);
            return;
    ...
    case PARALLEL:
    {
        ...
        for (i = XVECLEN(x, 0) - 1; i >= 0; i--)
        {
            ...
            if (GET_CODE(y) == SET)
            {
                ...
                dstp = &SET_DEST(y);
                while (GET_CODE(*dstp) == SUBREG || GET_CODE(*dstp) == ZERO_EXTRACT
                    || GET_CODE(*dstp) == SIGN_EXTRACT || GET_CODE(*dstp) == STRICT_LOW_PART)
                    dstp = &XEXP(*dstp, 0);
                if (GET_CODE(*dstp) == REG)
                {
                    int regno = REGNO(*dstp);
                    rtx src = SET_SRC(y);
                    if (regno_related[regno] && regno_related[regno]->insn && GET_CODE(src) == PLUS
...
static void count_sets(rtx x, rtx pat)
{
    if (GET_CODE(x) == REG)
        REG_N_SETS(REGNO(x))++;
}
...
static struct related *optimize_related_values_1(
    struct related *rel_base, int luid, int call_tally, rtx insert_after, FILE *regmove_dump_file)
{
    ...
    int mode;
    int32_t rel_base_reg_user_offset = 0;
    ...
    for (max_end_luid = 0, chain = baseinfo->chains; chain; chain = chain->prev)
    {
        ...
        if (regmove_dump_file)
            fprintf(
                regmove_dump_file, "Chain start: %d end: %d\n", chain->start_luid, chain->end_luid);
    }

    if (regmove_dump_file)
        fprintf(regmove_dump_file, "Insn %d reg %d: found %d chains.\n", INSN_UID(rel_base->insn),
            REGNO(rel_base->reg), num_chains);
    ...
    qsort(chain_starttab, num_chains, sizeof *chain_starttab, chain_starts_earlier);
    ...
    {
        if (!rel->invalidate_luid || rel->invalidate_luid > max_end_luid)
            num_av_regs++;
    }
    ...
    if (!rel_base_reg_user)
    {
        ...
        {
            struct rel_use_chain *chain = chain_starttab[i];
            if (!chain->reg && chain->start_luid && chain->chain->offset >= add_limits[mode][0]
       ...
       and refcount increments that are now obsolete.  */
        for (rel = rel_base; rel; rel = rel->prev)
        {
            ...
            for (update = rel->updates; update; update = update->prev)
            {
                ...
                if (death_insn)
                {
                    rtx death_note = find_reg_note(death_insn, REG_DEAD, rel->reg);
                    if (!death_note)
                        death_note = find_reg_note(death_insn, REG_UNUSED, rel->reg);
        ...
        for (i = num_chains - 1; i >= 0; i--)
        {
            ...
            if (!reg)
            {
                do
                    rel = rel->prev;
                while (
                    !rel->death || (rel->invalidate_luid && rel->invalidate_luid <= max_end_luid));
                reg = rel->reg;
            }
            regno = REGNO(reg);
            ...
            {
                rtx add;
                add = gen_add3_insn(reg, rel_base->reg, GEN_INT(use->offset - base_offset));
                if (!add)
                    abort();
               ...
               unless we are still using the base register.  */
                if (chain0 == last_init_chain && rel_base->death && regno != base_regno)
                {
            ...
            for (last_use = 0, chain = chain0; chain; chain = chain->linked)
            {
                ...
                if (last_use)
                {
                    rtx add = gen_add3_insn(reg, reg, GEN_INT(use->offset - last_use->offset));
                    if (!add)
                        abort();
                    if (use->insn)
                ...
                for (last_offset = use->offset; use; use = use->next_chain)
                {
                    ...
                    addr = *use->addrp;
                    if (GET_CODE(addr) != REG)
                        remove_note(use->insn, find_reg_note(use->insn, REG_INC, XEXP(addr, 0)));
            ...
            if (reg == rel->reg && rel->death
                && (rel != rel_base || last_use->luid >= last_init_chain->start_luid))
            {
                XEXP(rel->death, 0) = reg;
                PUT_MODE(
                    rel->death, (reg_set_p(reg, PATTERN(last_use->insn)) ? REG_UNUSED : REG_DEAD));
            ...
            if (regno == base_regno)
                ...
                REG_N_CALLS_CROSSED(regno) += last_use->call_tally - first_call_tally;
        }

        REG_N_CALLS_CROSSED(base_regno) += base_last_use_call_tally - rel_base->reg_set_call_tally;
    }
...
static void optimize_related_values_0(struct related *rel_base_list, int luid, int call_tally,
    rtx insert_after, FILE *regmove_dump_file)
{
    while (rel_base_list)
        rel_base_list = optimize_related_values_1(
            rel_base_list, luid, call_tally, insert_after, regmove_dump_file);
...
static void optimize_related_values(int nregs, FILE *regmove_dump_file)
{
    int b;
    rtx insn;
    int luid = 0;
         ...
         mode = GET_MODE_WIDER_MODE(mode))
    {
        ...
        if (icode == CODE_FOR_nothing || !(*insn_operand_predicate[icode][0])(reg, mode)
        ...
        set = single_set(add);
        if (!set || GET_CODE(SET_SRC(set)) != PLUS || XEXP(SET_SRC(set), 1) != const1_rtx)
            continue;
        ...
        {
            if (!validate_change(
                    add, &XEXP(SET_SRC(set), 1), GEN_INT(((int32_t)1 << p) - 1), 0))
                break;
        }
       ...
       in case the power of two is an exceptional value.  */
        ...
        {
            if (validate_change(add, &XEXP(SET_SRC(set), 1), GEN_INT(-tmp - 1), 0))
    ...
    for (insn = get_insns(); insn; insn = NEXT_INSN(insn))
    {
        ...
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i')
        {
            ...
            for (note = REG_NOTES(insn); note; note = XEXP(note, 1))
            {
                if (REG_NOTE_KIND(note) == REG_DEAD
                    || (REG_NOTE_KIND(note) == REG_UNUSED && GET_CODE(XEXP(note, 0)) == REG))
                {
                    rtx reg = XEXP(note, 0);
                    int regno = REGNO(reg);
                    if (REG_NOTE_KIND(note) == REG_DEAD && reg_set_p(reg, PATTERN(insn)))
                    {
                        remove_note(insn, note);
                        REG_N_DEATHS(regno)--;
                    }
                    else if (regno_related[regno] && !regno_related[regno]->invalidate_luid)
                    {
       ...
       justified by the little possible extra gain.  */
        if (GET_CODE(insn) == CODE_LABEL || GET_CODE(insn) == JUMP_INSN
            || (flag_exceptions && GET_CODE(insn) == CALL_INSN) || cc0_user)
        {
            optimize_related_values_0(
                rel_base_list, luid, call_tally, prev_nonnote_insn(insn), regmove_dump_file);
            rel_base_list = 0;
            if (cc0_user)
                insn = cc0_user;
        }
    }
    optimize_related_values_0(rel_base_list, luid, call_tally, get_last_insn(), regmove_dump_file);
...
int replacement_quality(rtx reg)
{
    int src_regno;
...
static int optimize_reg_copy_1(rtx insn, rtx dest, rtx src)
{
    rtx p, q;
    rtx note;
    rtx dest_death = 0;
    ...
    for (p = NEXT_INSN(insn); p; p = NEXT_INSN(p))
    {
       ...
       when we see a CALL_INSN if flag_exceptions is nonzero.  */
        ...
        if (reg_set_p(src, p)
            ...
            || (GET_CODE(PATTERN(p)) == USE && reg_overlap_mentioned_p(src, XEXP(PATTERN(p), 0))))
            break;
            ...
            for (q = next_real_insn(insn); q != next_real_insn(p); q = next_real_insn(q))
            {
                if (reg_overlap_mentioned_p(src, PATTERN(q)))
                {
                    /* If SRC is a hard register, we might miss some
                       ...
                       of cases.  */
                    if (sregno < FIRST_PSEUDO_REGISTER && reg_mentioned_p(dest, PATTERN(q)))
                        failed = 1;
               ...
               overly conservative.  */
                if (dest_death == 0 && (dest_death = find_regno_note(q, REG_DEAD, dregno)) != 0)
                {
...
static void optimize_reg_copy_2(rtx insn, rtx dest, rtx src)
{
    rtx p, q;
    rtx set;
    int sregno = REGNO(src);
    ...
    for (p = NEXT_INSN(insn); p; p = NEXT_INSN(p))
    {
       ...
       when we see a CALL_INSN if flag_exceptions is nonzero.  */
        ...
        set = single_set(p);
        if (set && SET_SRC(set) == dest && SET_DEST(set) == src && find_reg_note(p, REG_DEAD, dest))
        {
            /* We can do the optimization.  Scan forward from INSN again,
        ...
        if (reg_set_p(src, p) || find_reg_note(p, REG_DEAD, dest)
...
static void optimize_reg_copy_3(rtx insn, rtx dest, rtx src)
{
    ...
    if (src_no < FIRST_PSEUDO_REGISTER || dst_no < FIRST_PSEUDO_REGISTER
        || !find_reg_note(insn, REG_DEAD, src_reg) || REG_N_SETS(src_no) != 1)
        return;
    ...
    if (!(set = single_set(p)) || GET_CODE(SET_SRC(set)) != MEM || SET_DEST(set) != src_reg)
        return;
    ...
    if (GET_MODE_BITSIZE(GET_MODE(src_reg)) <= GET_MODE_BITSIZE(GET_MODE(src))
        && !TRULY_NOOP_TRUNCATION(
               GET_MODE_BITSIZE(GET_MODE(src)), GET_MODE_BITSIZE(GET_MODE(src_reg))))
        return;
...
static void copy_src_to_dest(rtx insn, rtx src, rtx dest, int loop_depth, int old_max_uid)
{
    rtx seq;
    rtx link;
    rtx next;
    ...
    if (GET_CODE(src) == REG && REG_LIVE_LENGTH(REGNO(src)) > 0 && GET_CODE(dest) == REG
        && REG_LIVE_LENGTH(REGNO(dest)) > 0 && (set = single_set(insn)) != NULL_RTX
        && !reg_mentioned_p(dest, SET_SRC(set)) && GET_MODE(src) == GET_MODE(dest))
    {
        ...
        if (old_num_regs != reg_rtx_no || !validate_replace_rtx(src, dest, insn))
        {
...
static int reg_is_remote_constant_p(rtx reg, rtx insn, rtx first)
{
    ...
    for (p = LOG_LINKS(insn); p; p = XEXP(p, 1))
    {
        ...
        if (s != 0 && GET_CODE(SET_DEST(s)) == REG && REGNO(SET_DEST(s)) == REGNO(reg))
        {
...
int fixup_match_2(rtx insn, rtx dst, rtx src, rtx offset, FILE *regmove_dump_file)
{
    rtx p, dst_death = 0;
    ...
    for (length = 0, p = PREV_INSN(insn); p; p = PREV_INSN(p))
    {
       ...
       when we see a CALL_INSN if flag_exceptions is nonzero.  */
        ...
        pset = single_set(p);
        if (pset && SET_DEST(pset) == dst && GET_CODE(SET_SRC(pset)) == PLUS
            && XEXP(SET_SRC(pset), 0) == src && GET_CODE(XEXP(SET_SRC(pset), 1)) == CONST_INT)
        {
            int32_t newconst = INTVAL(offset) - INTVAL(XEXP(SET_SRC(pset), 1));
            ...
            if (add && validate_change(insn, &PATTERN(insn), add, 0))
            {
                ...
                if (regmove_dump_file)
                    fprintf(regmove_dump_file, "Fixed operand of insn %d.\n", INSN_UID(insn));
       ...
       non-call insns.  */
        if (GET_CODE(p) == CALL_INSN)
        {
            ...
            if (call_used_regs[REGNO(dst)] || find_reg_fusage(p, CLOBBER, dst))
                break;
        }
...
void regmove_optimize(rtx f, int nregs, FILE *regmove_dump_file)
{
    ...
    for (pass = 0; pass <= 2; pass++)
    {
             ...
             insn = pass ? PREV_INSN(insn) : NEXT_INSN(insn))
        {
            ...
            if (flag_expensive_optimizations && !pass
                && (GET_CODE(SET_SRC(set)) == SIGN_EXTEND || GET_CODE(SET_SRC(set)) == ZERO_EXTEND)
                && GET_CODE(XEXP(SET_SRC(set), 0)) == REG && GET_CODE(SET_DEST(set)) == REG)
                optimize_reg_copy_3(insn, SET_DEST(set), SET_SRC(set));

            if (flag_expensive_optimizations && !pass && GET_CODE(SET_SRC(set)) == REG
                && GET_CODE(SET_DEST(set)) == REG)
            {
               ...
               it will become a copy where SRC is dead.  */
                    ...
                    && REGNO(SET_DEST(set)) >= FIRST_PSEUDO_REGISTER)
                {
                    ...
                    if (regno_src_regno[REGNO(SET_DEST(set))] < 0 && SET_SRC(set) != SET_DEST(set))
                    {
            ...
            for (op_no = 0; op_no < recog_n_operands; op_no++)
            {
                ...
                if (GET_CODE(dst) == SUBREG
                    && GET_MODE_SIZE(GET_MODE(dst)) >= GET_MODE_SIZE(GET_MODE(SUBREG_REG(dst))))
                {
                    src_subreg = gen_rtx_SUBREG(GET_MODE(SUBREG_REG(dst)), src, SUBREG_WORD(dst));
                    dst = SUBREG_REG(dst);
                }
               ...
               match_operand/dst must be a write-only operand.  */
                if (match.use[op_no] != READ || match.use[match_no] != WRITE)
                    continue;

                if (match.early_clobber[match_no] && count_occurrences(PATTERN(insn), src) > 1)
                    continue;
                ...
                if (operands_match_p(src, dst)
                    || (match.commutative[op_no] >= 0
                           ...
                           && (replacement_quality(recog_operand[match.commutative[op_no]])
...
/* CYGNUS LOCAL SH4-OPT */
               ...
               this is about.  */
                if (flag_expensive_optimizations && GET_MODE(dst) == Pmode
                    && GET_CODE(SET_SRC(set)) == PLUS && XEXP(SET_SRC(set), 0) == src_subreg
                    && GET_CODE(XEXP(SET_SRC(set), 1)) == CONST_INT && !related_values_optimized)
                {
                ...
                /* END CYGNUS LOCAL */
                if (fixup_match_1(
                        insn, set, src, src_subreg, dst, pass, op_no, match_no, regmove_dump_file))
                    break;
            }
        }
    }
    ...
    for (insn = get_last_insn(); insn; insn = PREV_INSN(insn))
    {
        ...
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i')
        {
            ...
            for (op_no = 0; op_no < recog_n_operands; op_no++)
            {
               ...
               operand_operand/src must be a read-only operand.  */
                ...
                if (REGNO(src) < FIRST_PSEUDO_REGISTER)
                {
                    if (GET_CODE(SET_SRC(set)) == PLUS
                        ...
                        && fixup_match_2(insn, dst, src, XEXP(SET_SRC(set), 1), regmove_dump_file))
                        break;
                    continue;
                }
                src_class = reg_preferred_class(REGNO(src));
               ...
               replace it in both instructions with match_no.  */
                ...
                for (length = 0, p = PREV_INSN(insn); p; p = PREV_INSN(p))
                {
                    ...
                    if (pset && SET_DEST(pset) == src)
                    {
                       ...
                       them have to be changed at the same time.  */
                        ...
                        {
                            if (validate_change(p, &SET_DEST(pset), dst, 0))
                                success = 1;
                            else
                            {
                               ...
                               This modifies the dst as a side-effect.  */
                                ...
                                validate_change(insn, recog_operand_loc[match_no], dst, 0);
                            }
                        }
                        break;
                    }
                ...
                if (success)
                {
                    ...
                    if (REG_N_SETS(REGNO(src)) == 0 && !regno_uninitialized(REGNO(src)))
            ...
            if (!success && copy_src != NULL_RTX)
                copy_src_to_dest(insn, copy_src, copy_dst, loop_depth, old_max_uid);
        }
    }
#endif /* REGISTER_CONSTRAINTS */
...
static int find_matches(rtx insn, struct match *matchp)
{
...
/* Try to replace output operand DST in SET, with input operand SRC.  SET is
   ...
   Return nonzero for success.  */
static int fixup_match_1(rtx insn, rtx set, rtx src, rtx src_subreg, rtx dst, int backward,
    int operand_number, int match_number, FILE *regmove_dump_file)
{
    rtx p;
    ...
    int32_t insn_const = 0, newconst = 0;
    ...
    if (!src_note)
    {
       ...
       regA is dead after or set by the second insn.  */
        ...
        if ((code == PLUS || code == LSHIFTRT || code == ASHIFT || code == ASHIFTRT)
            && XEXP(SET_SRC(set), 0) == src && GET_CODE(XEXP(SET_SRC(set), 1)) == CONST_INT)
    ...
    if (regmove_dump_file)
        fprintf(regmove_dump_file, "Could fix operand %d of insn %d matching operand %d.\n",
            ...
            if (!src_note)
            {
               ...
               is executed will be changed.  Check that this is OK.  */
                ...
                for (q = p; q; q = NEXT_INSN(q))
                {
                    ...
                    if (GET_RTX_CLASS(GET_CODE(q)) != 'i')
                        continue;
                    if (reg_overlap_mentioned_p(src, PATTERN(q)) || reg_set_p(src, q))
                        break;
                }
                if (q)
                    set2 = single_set(q);
                if (!q || !set2 || GET_CODE(SET_SRC(set2)) != code || XEXP(SET_SRC(set2), 0) != src
                    || GET_CODE(XEXP(SET_SRC(set2), 1)) != CONST_INT
                    || (SET_DEST(set2) != src && !find_reg_note(q, REG_DEAD, src)))
                {
                    ...
                    if (code == PLUS
                       ...
                       hard register.  */
                        && !(dst_note && !REG_N_CALLS_CROSSED(REGNO(dst)) && single_set(p)
                               && GET_CODE(SET_DEST(single_set(p))) == REG
                               && (REGNO(SET_DEST(single_set(p))) < FIRST_PSEUDO_REGISTER))
#ifdef HAVE_cc0
                ...
                else
                {
                    ...
                    if (code != PLUS
                        && (newconst < 0
                               || (newconst >= GET_MODE_BITSIZE(GET_MODE(SET_SRC(set2))))))
                        break;
                    if (code == PLUS)
                    {
                        post_inc = q;
               ...
               when it is called by validate_replace_rtx .  */
                validate_change(q, &XEXP(SET_SRC(set2), 1), GEN_INT(newconst), 1);
            }
    ...
    if (code == MINUS)
    {
        ...
        if ((HAVE_PRE_INCREMENT || HAVE_PRE_DECREMENT) && search_end
    ...
    if (overlap)
    {
       ...
       but we can change this by moving insn.  */
        ...
        if (code == PLUS && try_auto_increment(overlap, insn, 0, src, insn_const, 0))
            insn = overlap;
        else
#endif
        {
    ...
    else if ((HAVE_PRE_INCREMENT || HAVE_PRE_DECREMENT) && (code == PLUS || code == MINUS)
        && insn_const && try_auto_increment(p, insn, 0, src, insn_const, 1))
        insn = p;
    else if ((HAVE_POST_INCREMENT || HAVE_POST_DECREMENT) && post_inc
        ...
        && exact_log2(newconst))
    {
        ...
        for (q = post_inc; (q = NEXT_INSN(q));)
        {
            ...
            if (GET_RTX_CLASS(GET_CODE(q)) != 'i')
                continue;
            if (src != inc_dest && (reg_overlap_mentioned_p(src, PATTERN(q)) || reg_set_p(src, q)))
                break;
            if (reg_set_p(inc_dest, q))
                break;
            if (reg_overlap_mentioned_p(inc_dest, PATTERN(q)))
            {
                try_auto_increment(q, post_inc, post_inc_set, inc_dest, newconst, 1);
                break;
            }
        }
    }
    ...
    /* If that was the only time dst was set,
       ...
       won't make reload do any work.  */
    if (REG_N_SETS(REGNO(dst)) == 0 && !regno_uninitialized(REGNO(dst)))
        REG_N_REFS(REGNO(dst)) = 0;

    if (regmove_dump_file)
        fprintf(regmove_dump_file, "Fixed operand %d of insn %d matching operand %d.\n",
            operand_number, INSN_UID(insn), match_number);
    return 1;
}
...
static int stable_but_for_p(rtx x, rtx src, rtx dst)
{
...
int regmove_profitable_p(void)
{
    ...
    do /* check add_optab and ashl_optab */
             ...
             mode = GET_MODE_WIDER_MODE(mode))
        {
            ...
            if (!single_set(pat) || GET_CODE(SET_SRC(single_set(pat))) != tstoptab->code)
