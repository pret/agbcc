...
int rtx_unstable_p(rtx x)
{
    ...
    if (code == REG)
        return !(REGNO(x) == FRAME_POINTER_REGNUM || REGNO(x) == ARG_POINTER_REGNUM
            || RTX_UNCHANGING_P(x));
...
static int rtx_addr_can_trap_p(register rtx x)
{
    ...
    case REG:
        ...
        return !(x == frame_pointer_rtx || x == stack_pointer_rtx || x == arg_pointer_rtx);
    ...
    case PLUS:
       ...
       trap plus a constant integer.  */
        return (rtx_addr_can_trap_p(XEXP(x, 0)) || GET_CODE(XEXP(x, 1)) != CONST_INT);
...
get_integer_term(rtx x)
{
    ...
    if (GET_CODE(x) == MINUS && GET_CODE(XEXP(x, 1)) == CONST_INT)
...
int reg_mentioned_p(register rtx reg, register rtx in)
{
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        ...
        else if (fmt[i] == 'e' && reg_mentioned_p(reg, XEXP(in, i)))
            return 1;
    }
    return 0;
}
...
int reg_referenced_p(rtx x, rtx body)
{
    ...
    case SET:
       ...
       it is mentioned in the destination.  */
        if (GET_CODE(SET_DEST(body)) != CC0 && GET_CODE(SET_DEST(body)) != PC
            && GET_CODE(SET_DEST(body)) != REG
            && !(GET_CODE(SET_DEST(body)) == SUBREG && GET_CODE(SUBREG_REG(SET_DEST(body))) == REG
                   && (((GET_MODE_SIZE(GET_MODE(SUBREG_REG(SET_DEST(body)))) + (UNITS_PER_WORD - 1))
                           / UNITS_PER_WORD)
                          == ((GET_MODE_SIZE(GET_MODE(SET_DEST(body))) + (UNITS_PER_WORD - 1))
                                 / UNITS_PER_WORD)))
    ...
    for (insn = NEXT_INSN(from_insn); insn != to_insn; insn = NEXT_INSN(insn))
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i'
            && (reg_referenced_p(reg, PATTERN(insn))
                   || (GET_CODE(insn) == CALL_INSN && find_reg_fusage(insn, USE, reg))))
            return 1;
    return 0;
}
...
int reg_set_between_p(rtx reg, rtx from_insn, rtx to_insn)
{
    ...
    for (insn = NEXT_INSN(from_insn); insn != to_insn; insn = NEXT_INSN(insn))
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i' && reg_set_p(reg, insn))
            return 1;
    return 0;
}
...
static void reg_set_p_1(rtx x, rtx pat ATTRIBUTE_UNUSED)
{
    /* We don't want to return 1 if X is a MEM that contains a register
       within REG_SET_REG.  */

    if ((GET_CODE(x) != MEM) && reg_overlap_mentioned_p(reg_set_reg, x))
        reg_set_flag = 1;
}
        ...
        if (FIND_REG_INC_NOTE(insn, reg)
            || (GET_CODE(insn) == CALL_INSN
                  ...
                  information holds all clobbered registers.  */
                   && ((GET_CODE(reg) == REG && REGNO(reg) < FIRST_PSEUDO_REGISTER)
                          || GET_CODE(reg) == MEM || find_reg_fusage(insn, CLOBBER, reg))))
            return 1;
    ...
    else if (GET_CODE(PATTERN(insn)) == PARALLEL)
    {
        for (i = 0, set = 0; i < XVECLEN(PATTERN(insn), 0); i++)
            if (GET_CODE(XVECEXP(PATTERN(insn), 0, i)) == SET
                && (!find_reg_note(insn, REG_UNUSED, SET_DEST(XVECEXP(PATTERN(insn), 0, i)))
...
repeat:
    ...
    case REG:
       ...
       but it isn't worth it.  */
        if ((i == STACK_POINTER_REGNUM || i == ARG_POINTER_REGNUM || i == FRAME_POINTER_REGNUM)
    ...
    case SET:
        if (&SET_DEST(x) != loc
            ...
            && ((GET_CODE(SET_DEST(x)) == SUBREG && loc != &SUBREG_REG(SET_DEST(x))
                    ...
                    && refers_to_regno_p(regno, endregno, SUBREG_REG(SET_DEST(x)), loc))
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        if (fmt[i] == 'e' && loc != &XEXP(x, i))
        {
            ...
            else if (refers_to_regno_p(regno, endregno, XEXP(x, i), loc))
                return 1;
        }
        else if (fmt[i] == 'E')
        {
...
int reg_overlap_mentioned_p(rtx x, rtx in)
{
    ...
    else if (GET_CODE(x) == PARALLEL && GET_MODE(x) == BLKmode)
    {
        register int i;
...
static void reg_set_last_1(rtx x, rtx pat)
{
    int first, last;
    ...
    first = REGNO(x);
    last = first + (first < FIRST_PSEUDO_REGISTER ? HARD_REGNO_NREGS(first, GET_MODE(x)) : 1);

    if (first >= reg_set_last_last_regno || last <= reg_set_last_first_regno)
        return;
    ...
    if (GET_CODE(pat) == CLOBBER || SET_DEST(pat) != x || first != reg_set_last_first_regno
...
rtx reg_set_last(rtx x, rtx insn)
{
    ...
    reg_set_last_last_regno = reg_set_last_first_regno
        + (reg_set_last_first_regno < FIRST_PSEUDO_REGISTER
                  ? HARD_REGNO_NREGS(reg_set_last_first_regno, GET_MODE(x))
                  : 1);
    ...
    for (; insn && GET_CODE(insn) != CODE_LABEL
         && !(GET_CODE(insn) == CALL_INSN && reg_set_last_last_regno <= FIRST_PSEUDO_REGISTER);
         insn = PREV_INSN(insn))
        if (GET_RTX_CLASS(GET_CODE(insn)) == 'i')
        {
            ...
            else if (reg_set_last_value)
            {
                if (CONSTANT_P(reg_set_last_value)
                    || ((GET_CODE(reg_set_last_value) == REG
                           ...
                           && !reg_set_between_p(reg_set_last_value, insn, orig_insn)))
...
void note_stores(register rtx x, void (*fun)())
{
...
int dead_or_set_regno_p(rtx insn, int test_regno)
{
    int regno, endregno;
    rtx link;
    ...
    for (link = REG_NOTES(insn); link; link = XEXP(link, 1))
    {
        ...
        endregno = (regno >= FIRST_PSEUDO_REGISTER
                ? regno + 1
                : regno + HARD_REGNO_NREGS(regno, GET_MODE(XEXP(link, 0))));

        if (test_regno >= regno && test_regno < endregno)
            return 1;
    }

    if (GET_CODE(insn) == CALL_INSN && find_regno_fusage(insn, CLOBBER, test_regno))
        return 1;
...
rtx find_reg_note(rtx insn, enum reg_note kind, rtx datum)
{
    ...
    for (link = REG_NOTES(insn); link; link = XEXP(link, 1))
        if (REG_NOTE_KIND(link) == kind && (datum == 0 || datum == XEXP(link, 0)))
            return link;
    return 0;
}
    ...
    for (link = REG_NOTES(insn); link; link = XEXP(link, 1))
        if (REG_NOTE_KIND(link) == kind
            /* Verify that it is a register, so that scratch and MEM won't cause a
               problem here.  */
            && GET_CODE(XEXP(link, 0)) == REG && REGNO(XEXP(link, 0)) <= regno
            && ((REGNO(XEXP(link, 0))
                    + (REGNO(XEXP(link, 0)) >= FIRST_PSEUDO_REGISTER
                              ? 1
                              : HARD_REGNO_NREGS(REGNO(XEXP(link, 0)), GET_MODE(XEXP(link, 0)))))
                   > regno))
            return link;
    return 0;
}
...
int find_reg_fusage(rtx insn, enum rtx_code code, rtx datum)
{
    ...
    if (GET_CODE(datum) != REG)
    {
        ...
        for (link = CALL_INSN_FUNCTION_USAGE(insn); link; link = XEXP(link, 1))
            if (GET_CODE(XEXP(link, 0)) == code && rtx_equal_p(datum, SET_DEST(XEXP(link, 0))))
                return 1;
    }
    else
    {
...
int find_regno_fusage(rtx insn, enum rtx_code code, int regno)
{
    register rtx link;
    ...
    if (regno >= FIRST_PSEUDO_REGISTER || GET_CODE(insn) != CALL_INSN)
        return 0;
    ...
    for (link = CALL_INSN_FUNCTION_USAGE(insn); link; link = XEXP(link, 1))
    {
        ...
        if (GET_CODE(op = XEXP(link, 0)) == code && GET_CODE(SET_DEST(op)) == REG
            && (regnote = REGNO(SET_DEST(op))) <= regno
            && regnote + HARD_REGNO_NREGS(regnote, GET_MODE(SET_DEST(op))) > regno)
            return 1;
    }
...
void remove_note(register rtx insn, register rtx note)
{
    register rtx link;
...
int may_trap_p(rtx x)
{
    ...
    case UDIV:
    case UMOD:
        if (!CONSTANT_P(XEXP(x, 1)) || GET_MODE_CLASS(GET_MODE(x)) == MODE_FLOAT)
            return 1;
...
rtx replace_regs(rtx x, rtx *reg_map, int nregs, int replace_dest)
{
    register enum rtx_code code;
    register int i;
    register char *fmt;
    ...
    case SET:
        ...
        else if (GET_CODE(SET_DEST(x)) == MEM || GET_CODE(SET_DEST(x)) == STRICT_LOW_PART)
            /* Even if we are not to replace destinations, replace register if it
               is CONTAINED in destination (destination is memory or
               STRICT_LOW_PART).  */
            XEXP(SET_DEST(x), 0) = replace_regs(XEXP(SET_DEST(x), 0), reg_map, nregs, 0);
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        ...
        if (fmt[i] == 'E')
        {
            register int j;
            for (j = 0; j < XVECLEN(x, i); j++)
                XVECEXP(x, i, j) = replace_regs(XVECEXP(x, i, j), reg_map, nregs, replace_dest);
        }
    }
    return x;
}
...
static int jmp_uses_reg_or_mem(rtx x)
{
    enum rtx_code code = GET_CODE(x);
    int i, j;
    char *fmt;
    ...
    case MEM:
        return !(GET_CODE(XEXP(x, 0)) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P(XEXP(x, 0)));

    case IF_THEN_ELSE:
        return (jmp_uses_reg_or_mem(XEXP(x, 1)) || jmp_uses_reg_or_mem(XEXP(x, 2)));

    case PLUS:
    case MINUS:
    case MULT:
        return (jmp_uses_reg_or_mem(XEXP(x, 0)) || jmp_uses_reg_or_mem(XEXP(x, 1)));
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
    {
        if (fmt[i] == 'e' && jmp_uses_reg_or_mem(XEXP(x, i)))
            return 1;
...
int insn_first_p(rtx insn, rtx reference)
{
    rtx p, q;
