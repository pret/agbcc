...
boolean hash_table_init_n(struct hash_table *table,
    ...
    unsigned int size)
{
    ...
    table->table = ((struct hash_entry **)obstack_alloc(&table->memory, alloc));
    if (!table->table)
    {
        error("no memory");
        return false;
...
boolean hash_table_init(struct hash_table *table,
    struct hash_entry *(*newfunc)(struct hash_entry *, struct hash_table *, hash_table_key),
    unsigned long (*hash)(hash_table_key),
    boolean (*comp)(hash_table_key, hash_table_key))
{
    ...
    hash_table_key (*copy)(struct obstack *memory, hash_table_key key))
{
    ...
    for (hashp = table->table[index]; hashp != (struct hash_entry *)NULL; hashp = hashp->next)
    {
        if (hashp->hash == hash && (*table->comp)(hashp->key, key))
            return hashp;
    }
...
/*ARGSUSED*/
struct hash_entry *
hash_newfunc(struct hash_entry *entry, struct hash_table *table, hash_table_key p)
{
    if (entry == (struct hash_entry *)NULL)
        entry = ((struct hash_entry *)hash_allocate(table, sizeof(struct hash_entry)));
    return entry;
}

/* Allocate space in a hash table.  */

void *hash_allocate(struct hash_table *table, unsigned int size)
{
    void *ret;
...
boolean string_compare(hash_table_key k1, hash_table_key k2)
{
