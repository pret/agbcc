...
{
  bfd *abfd;
  uint32_t indx[LOCAL_SYM_CACHE_SIZE];
...
extern uint32_t bfd_elf_hash
  (const char *);
extern uint32_t bfd_elf_gnu_hash
  (const char *);
...
extern bfd_boolean _bfd_elf_set_arch_mach
  (bfd *, enum bfd_architecture, uint32_t);
...
extern asection *_bfd_elf_section_for_symbol
  (struct elf_reloc_cookie *, uint32_t, bfd_boolean);
