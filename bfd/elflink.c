	  ...
	  if ((size_t) r_symndx >= nsyms)
	    {
	      _bfd_error_handler
		/* xgettext:c-format */
		(_("%pB: bad reloc symbol index (%#" PRIx64 " >= %#x)"
		   " for offset %#" PRIx64 " in section `%pA'"),
		 abfd, (uint64_t) r_symndx, (uint32_t) nsyms,
...
error_free_dyn:
  ...
  if (info->lto_plugin_active
      ...
      for (s = abfd->sections; s != NULL; s = s->next)
	{
	  ...
	  for ( ; rel < relend; rel++)
	    {
	      uint32_t r_symndx = rel->r_info >> r_sym_shift;
	      struct elf_link_hash_entry *h;
...
elf_renumber_gnu_hash_syms (struct elf_link_hash_entry *h, void *data)
{
  ...
  val = s->hashval[h->dynindx] & ~(uint32_t) 1;
...
compute_bucket_count (struct bfd_link_info *info ATTRIBUTE_UNUSED,
		      uint32_t *hashcodes ATTRIBUTE_UNUSED,
		      uint32_t nsyms,
		      int gnu_hash)
{
  size_t best_size = 0;
  ...
  if (info->optimize)
      ...
      if (gnu_hash)
	{
	 ...
	 since the size could be large.  */
      amt = maxsize;
      amt *= sizeof (uint32_t);
      counts = (uint32_t *) bfd_malloc (amt);
      if (counts == NULL)
	return 0;
      ...
      for (i = minsize; i < maxsize; ++i)
	{
	  ...
	  memset (counts, '\0', i * sizeof (uint32_t));
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
      ...
      if (info->emit_hash)
	{
	  ...
	  /* Compute the hash values for all exported symbols.  At the same
	     ...
	     optimizations.  */
	  amt = dynsymcount * sizeof (uint32_t);
	  hashcodes = (uint32_t *) bfd_malloc (amt);
	  if (hashcodes == NULL)
	    return FALSE;
      ...
      if (info->emit_gnu_hash)
	{
	  ...
	  /* Compute the hash values for all exported symbols.  At the same
	     time store the values in an array so that we could use them for
	     optimizations.  */
	  amt = dynsymcount * 2 * sizeof (uint32_t);
	  cinfo.hashcodes = (uint32_t *) bfd_malloc (amt);
	  ...
	  if (cinfo.nsyms == 0)
	    ...
	    {
	      uint32_t maskwords, maskbitslog2, x;
	      BFD_ASSERT (cinfo.min_dynindx != -1);
		...
		cinfo.shift1 = 5;
	      ...
	      amt = bucketcount * sizeof (uint32_t) * 2;
	      ...
	      if (cinfo.bitmask == NULL)
		{
	      ...
	      cinfo.counts = (uint32_t *) (cinfo.bitmask + maskwords);
  ...
  for (i = 0; i < locsymcount; ++ i)
	...
	continue;
      ...
      printf ("Comparing string: '%s' vs. '%s' = 0x%lx\n",
	      name, candidate, (uint32_t) sym->st_value);
#endif
      ...
      || global_entry->type == bfd_link_hash_defweak)
    {
      *result = (global_entry->u.def.value
      ...
      printf ("Found GLOBAL symbol '%s' with value %8.8lx\n",
	      global_entry->root.string, (uint32_t) *result);
#endif
      return TRUE;
    }
...
static void
decode_complex_addend (uint32_t *start,   /* in bits */
		       uint32_t *oplen,   /* in bits */
		       uint32_t *len,     /* in bits */
		       uint32_t *wordsz,  /* in bytes */
		       uint32_t *chunksz, /* in bytes */
		       uint32_t *lsb0_p,
		       uint32_t *signed_p,
		       uint32_t *trunc_p,
		       uint32_t encoded)
				    ...
				    bfd_vma relocation)
{
  ...
  uint32_t start, oplen, len, wordsz, chunksz, lsb0_p, signed_p, trunc_p;
  ...
  printf ("Doing complex reloc: "
	  ...
	  oplen, (uint32_t) x, (uint32_t) mask,
	  (uint32_t) relocation);
#endif
  ...
  printf ("           relocation: %8.8lx\n"
	  ...
	  "               result: %8.8lx\n",
	  (uint32_t) relocation, (uint32_t) (mask << shift),
	  (uint32_t) ((relocation & mask) << shift), (uint32_t) x);
#endif
			   ...
			   struct elf_link_hash_entry *h)
{
  ...
  if (name == NULL
    ...
    elfsym->st_name = (uint32_t) -1;
  else
    {
      /* Call _bfd_elf_strtab_offset after _bfd_elf_strtab_finalize
	 to get the final offset for st_name.  */
      elfsym->st_name
	= (uint32_t) _bfd_elf_strtab_add (flinfo->symstrtab,
					       name, FALSE);
      if (elfsym->st_name == (uint32_t) -1)
	return 0;
    }
  ...
  for (i = 0; i < hash_table->strtabcount; i++)
    {
      ...
      if (elfsym->sym.st_name == (uint32_t) -1)
	elfsym->sym.st_name = 0;
      else
	elfsym->sym.st_name
	  = (uint32_t) _bfd_elf_strtab_offset (flinfo->symstrtab,
						    elfsym->sym.st_name);
...
elf_link_input_bfd (struct elf_final_link_info *flinfo, bfd *input_bfd)
{
  ...
  for (o = input_bfd->sections; o != NULL; o = o->next)
	  ...
	  && (o->flags & (SEC_LINKER_CREATED | SEC_GROUP)) == SEC_GROUP)
	{
	  ...
	  uint32_t symndx = sec_data->this_hdr.sh_info;
...
_bfd_elf_default_got_elt_size (bfd *abfd,
			       ...
			       uint32_t symndx ATTRIBUTE_UNUSED)
{
  const struct elf_backend_data *bed = get_elf_backend_data (abfd);
