...
{
  void *ret;
  uint32_t ul_size = (uint32_t) size;

  if (size != ul_size
      /* Note - although objalloc_alloc takes an uint32_t as its
	 ...
	 ((uint32_t) -1) bytes.  Also memory checkers will often
...
DESCRIPTION
	...
	is expected to be an uint32_t pointer.
...
{
  asection *sect;
  uint32_t *crc32 = (uint32_t *) crc32_out;
  bfd_byte *contents;
