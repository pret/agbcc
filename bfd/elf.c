		      ...
		      Elf_External_Sym_Shndx *extshndx_buf)
{
  ...
  for (esym = (const bfd_byte *) extsym_buf, isym = intsym_buf,
	...
	/* xgettext:c-format */
	_bfd_error_handler (_("%pB symbol number %u references"
			      " nonexistent SHT_SYMTAB_SHNDX section"),
			    ibfd, (uint32_t) symoffset);
	       ...
	       int relocatable_p)
{
  ...
  uint32_t outbound_syms_index;
  uint32_t outbound_shndx_index;
  int idx;
...
error_return:
  ...
  for (idx = 0; idx < symcount;)
      ...
      else
	{
	  ...
	  sym.st_name
	    = (uint32_t) _bfd_elf_strtab_add (stt, syms[idx]->name,
						   FALSE);
	  if (sym.st_name == (uint32_t) -1)
	    goto error_return;
	}
...
elfcore_grok_win32pstatus (bfd *abfd, Elf_Internal_Note *note)
{
  ...
  switch (type)
    ...
    case 3 /* NOTE_INFO_MODULE */:
      ...
      sprintf (buf, ".module/%08x", (uint32_t) base_addr);
